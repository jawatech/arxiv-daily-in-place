...
摘要：<paragraph>最近人工智能的突破性進展，將從根本上提升我們對醫療保健的研究和理解。開發一個整合的多對多架構，利用多種數據方式輸入，對多種醫療任務進行分析建模，對於統一理解現代醫學至關重要。在這項工作中，我們引入了 M3H，一個可解釋的多模態多任務機器學習醫療保健架構，它整合了來自廣泛醫療任務類別和機器學習問題類別的多種多模態輸入的學習。該架構的模組化設計確保了其可概括的數據處理、任務定義和快速模型原型製作，適用於臨床和運營醫療保健場景。我們通過驗證從四種方式（表格、時間序列、語言和視覺）訓練的模型，在四個機器學習問題類別中的 41 項醫療任務上，評估了 M3H 架構。我們的結果表明，M3H 持續產生多任務模型，其表現優於 16 個醫療部門的 37 種疾病診斷、三個醫院運營預測和一個患者表型任務中的規範單任務模型（1.1-37.2%）。跨越機器學習問題類別的監督二元分類、多類分類、迴歸和聚類。此外，該架構引入了一種新穎的注意力機制，以平衡自我利用（專注於學習源任務）和交叉探索（鼓勵從其他任務中學習）。此外，M3H 提供了可解釋性見解，說明額外任務的聯合學習如何使用提議的 TIM 分數影響源任務的學習，從而闡明任務相互依賴的動態。其適應性架構促進了定制和集成，使其成為未來人工智能驅動的醫療保健系統的強大且可擴展的候選解決方案。</paragraph>
...
摘要：本文探討了群體結構多重代理系統 (MAS) 在建立醫療必要性中的應用，此程序涉及根據臨床指南對特定患者的醫療結構化和非結構化資料進行系統性審查。我們透過將此複雜任務分解成較小、更易於管理的子任務來解決。每個子任務都由一個專業的 AI 代理處理。我們對各種提示策略對這些代理的影響進行系統性研究，並對不同的大型語言模型 (LLM) 進行基準測試，以確定它們在完成這些任務時的準確性。此外，我們探討這些代理如何提供可解釋性，從而增強系統內的信任和透明度。
...
摘要：人工智慧 (AI) 的廣泛整合在涉及 AI 啟用系統的事件中引入了責任和問責制的複雜挑戰。這些系統的互聯性、AI 誘發事件的倫理考量，加上 AI 技術的不確定性和相應法規的缺失，使得傳統的責任歸屬具有挑戰性。為此，這項工作提出了一種計算反思平衡 (CRE) 方法，以建立一個對所有利害關係人來說既連貫又符合道德規範的責任歸屬架構。計算方法提供了一種結構化的分析，克服了概念方法在處理動態和多面向情境時的局限性，展示了該架構在責任歸屬過程中可解釋性、連貫性和適應性等特性。我們探討了與平衡計算中的主張相關的初始啟動層級的關鍵作用。使用 AI 輔助醫療決策支援系統作為案例研究，我們說明了不同的初始化如何導致不同的責任分配。該架構提供了對 AI 誘發事件中問責制的寶貴見解，促進了通過持續監控、修訂和反思來開發一個可持續且有韌性的系統。
...
摘要：人工智慧支援醫療保健專業人員進行預測建模，大幅轉變臨床決策制定。本研究探討在醫療保健中對 AI 應用進行公平性和可解釋性的關鍵需求，以確保不同患者人口統計資料的公平結果。透過專注於敗血症相關死亡率的預測建模，我們提出了一種方法，該方法學習效能最佳化的預測模型，然後採用遷移學習過程來產生具有更好公平性的模型。我們的模型還引入一種基於排列的新穎特徵重要性演算法，旨在闡明每個特徵在增強預測公平性方面的貢獻。與現有的可解釋性方法專注於解釋特徵對預測效能的貢獻不同，我們提出的方法獨特地填補了理解每個特徵如何影響公平性的鴻溝。這項進展至關重要，因為敗血症的死亡率很高，且在三分之一的醫院死亡中扮演角色。我們的模型不僅有助於識別和減輕預測模型中的偏差，還透過改善模型預測的透明度和公平性來培養醫療保健利害關係者之間的信任，進而促進更公平、更值得信賴的醫療保健服務。
...
摘要：憂鬱症是現今一個重要的議題。根據世界衛生組織（WHO），2023 年有超過 2.8 億人正在與憂鬱症搏鬥。這是一個龐大的數字；若不認真看待，這些數字將會快速增加。約有 48.9 億人是社群媒體使用者。人們在 Twitter、Facebook、Reddit、Instagram 等平台上表達自己的感受和情緒。這些平台包含有價值的資訊，可用於研究目的。在各種社群媒體平台上進行了大量的研究。然而，這些努力仍存在一定的限制。特別是，先前的研究僅專注於在推文中偵測憂鬱症和憂鬱症的嚴重程度。此外，資料集標籤中存在不準確性。在這項研究工作中，使用 Twitter 資料庫中的推文，根據詞彙標籤預測了五種類型的憂鬱症（躁鬱症、重度憂鬱症、精神病性憂鬱症、非典型憂鬱症和產後憂鬱症）。可解釋的 AI 用於提供推理，重點說明代表憂鬱症類型的推文部分。使用來自 Transformer 的雙向編碼器表徵（BERT）進行特徵萃取和訓練。機器學習和深度學習方法用於訓練模型。BERT 模型呈現出最有希望的結果，達到 0.96 的整體準確度。
...
摘要：深度學習大幅轉變了醫學影像和放射線學領域，能識別醫學影像中的病理，包括電腦斷層掃描 (CT) 和 X 光掃描。然而，深度學習模型的效能，特別是在分割任務中，通常受到廣泛標註資料集需求的限制。為了應對這項挑戰，透過可解釋 AI 和反事實解釋的生成，探討了弱監督語意分割的能力。本研究的範圍是開發一種新穎的反事實內繪方法 (COIN)，透過使用生成模型將預測分類標籤從異常翻轉為正常。例如，如果分類器將輸入醫學影像 X 視為異常，表示存在病理，生成模型旨在內繪異常區域，從而逆轉分類器的原始預測標籤。此方法使我們能夠產生病理的精確分割，而無需依賴於預先存在的分割遮罩。至關重要的是，利用影像層級標籤，這比建立詳細的分割遮罩容易取得得多。該方法的有效性透過分割愛沙尼亞塔爾圖大學醫院取得的 CT 影像中的合成目標和實際腎臟腫瘤來證明。研究結果表明，COIN 遠遠優於已建立的歸因方法，例如 RISE、ScoreCAM 和 LayerCAM，以及 Singla 等人提出的替代反事實解釋方法。這些證據表明，COIN 是 CT 影像中腫瘤語意分割的一種有前途的方法，並在醫療保健中使深度學習應用更易於使用和更有效，而標註資料稀缺。
...
摘要：甲狀腺癌是一種日益嚴重的全球健康問題，需要進階的診斷方法。本篇評論探討了人工智慧與放射組學在甲狀腺癌診斷中的應用。根據 PRISMA 指南，我們檢視了多個資料庫，時間截止於 2023 年 10 月。結合多組關鍵字，我們發現了多篇與甲狀腺癌及其相關主題的英文學術出版品。在移除 109 篇重複文獻後，原始搜尋回傳了 267 篇論文。在檢視了論文摘要和標題後，我們根據預先設定的標準篩選出相關的研究，排除了 124 篇論文。在綜合分析後，我們又排除了六篇研究。在 28 篇納入的研究中，結合超音波 (US) 影像的放射組學分析證實了其在診斷甲狀腺癌方面的有效性。我們注意到各種結果，有些研究提出了優於現狀的新策略。文獻強調了人工智慧模型面臨的各種挑戰，包括可解釋性問題、資料集限制和操作員依賴性。28 篇納入研究的綜合結果提到，需要標準化工作和前瞻性多中心研究來解決這些問題。此外，我們也找出克服這些障礙的方法，例如可解釋人工智慧技術和個人化醫療技術的進步。本篇評論重點探討人工智慧和放射組學如何轉變甲狀腺癌的診斷和治療。儘管面臨挑戰，未來針對跨領域合作、臨床適用性驗證和演算法改進的研究仍有潛力改善甲狀腺癌治療的患者預後和診斷精準度。
...
摘要：近年來，乳癌盛行率急遽上升，成為全球主要的死亡原因之一。在所有癌症中，乳癌的發生率遠遠高於其他癌症。人工診斷此疾病需要大量的時間和專業知識。由於乳癌的檢測過程耗時，因此透過建立機器學習預測模型，可以協助預防乳癌的進一步擴散。機器學習和可解釋 AI 在分類中至關重要，因為它們不僅提供準確的預測，還能提供模型如何做出決策的見解，有助於理解和信賴分類結果。在本研究中，我們評估並比較了五種不同的機器學習方法在分類準確度、精確度、召回率和 F-1 分數，並使用一個主要資料集（達卡醫學院醫院的 500 名患者）。我們使用五種不同的監督式機器學習技術，包括決策樹、隨機森林、邏輯迴歸、樸素貝氏和 XGBoost，以在我們的資料集上獲得最佳結果。此外，本研究將 SHAP 分析應用於 XGBoost 模型，以解釋模型的預測並了解每個特徵對模型輸出的影響。我們比較了幾種演算法對資料進行分類的準確度，並與此領域的其他文獻進行對比。經過最終評估，本研究發現 XGBoost 達到了最佳的模型準確度，為 97%。
...
摘要：深度學習 (DL) 用於從乳房攝影圖像診斷乳癌的模型通常作為「黑盒子」運作，這使得醫療保健專業人員難以信任和理解他們的決策過程。本研究提出了一個整合架構，結合卷積神經網路 (CNN) 和可解釋人工智慧 (XAI)，以使用 CBIS-DDSM 資料集增強乳癌診斷。此方法包含一個精細的資料前處理管線和進階資料擴充技術，以對抗資料集限制，並使用預先訓練的網路（例如 VGG-16、Inception-V3 和 ResNet）進行遷移學習。我們研究的重點在於評估 XAI 在解釋模型預測中的有效性，重點利用豪斯多夫測度來評估 AI 生成的說明與專家註解之間的一致性。此方法對於 XAI 在促進 AI 輔助診斷中的可信度和道德公平性至關重要。我們研究的結果說明了 CNN 和 XAI 在推進乳癌診斷方法中的有效協作，從而促進了臨床環境中先進 AI 技術的更順暢整合。透過增強 AI 驅動決策的可解釋性，這項工作為 AI 系統和醫療從業人員之間的改善協作奠定了基礎，最終豐富了患者照護。此外，我們研究的影響遠遠超出了當前的研究方法。它鼓勵進一步研究如何結合多模式資料並改善 AI 說明，以滿足臨床實務的需求。
...
摘要：以人为本的可解释 AI (HCXAI) 提倡将社会方面纳入 AI 解释中。HCXAI 话语的核心是社会透明度 (ST) 框架，该框架旨在让 AI 系统的社会组织背景对用户可访问。在这项工作中，我们建议扩展 ST 框架以解决大型语言模型 (LLM) 中社会错误归因的风险，尤其是在心理健康等敏感领域。事实上，LLM 能够出色地模拟角色和人格，可能导致设计者的意图和用户对社会属性的认知之间出现不匹配，从而有引发情绪操控和危险行为、认知不公正和不合理信任的风险。为了解决这些问题，我们建议用第五个“W 问题”来增强 ST 框架，以澄清设计者和用户分配给 LLM 的具体社会属性。此补充旨在弥合 LLM 能力和用户认知之间的差距，促进基于 LLM 的技术的道德负责开发和使用。
...
摘要：<paragraph>背景：氣胸是一種因肺部和胸壁之間異常空氣聚集而引起的急性胸腔疾病。為了解決深度學習（DL）模型通常伴隨的不透明性，已引入可解釋人工智慧（XAI）方法，以概述與 DL 模型所做的氣胸診斷相關的區域。然而，這些解釋有時會偏離實際病灶區域，凸顯進一步改進的必要性。方法：我們提出了一種模板引導方法，將氣胸的臨床知識整合到 XAI 方法所產生的模型解釋中，從而提升這些解釋的品質。利用放射科醫師建立的一個病灶描繪，我們的做法首先產生一個模板，表示氣胸可能發生的區域。然後將這個模板疊加在模型解釋上，以濾除落在模板邊界之外的無關解釋。為了驗證其有效性，我們對三個 XAI 方法進行了比較分析，在兩個真實世界資料集中，使用和不使用我們的模板指導來解釋兩個 DL 模型。結果：在建立於三個 XAI 方法、兩個 DL 模型和兩個資料集的十二個基準情境中，所提出的方法始終改善了基準 XAI 方法。在比較模型解釋和真實病灶區域時，透過效能改善計算出的平均增量百分比，在聯集比（IoU）中為 97.8%，在 Dice 相似性係數（DSC）中為 94.1%。結論：在氣胸診斷的背景下，我們提出了一種模板引導方法，用於改善 AI 解釋。我們預期我們的模板指導將透過整合臨床領域專業知識，為闡明 AI 模型建立一種新的方法。</paragraph>
...
摘要：<paragraph>在當前的機器翻譯 (MT) 領域中，Transformer 架構脫穎而出，成為黃金標準，特別是對於高資源語言對。本研究深入探討其對低資源語言對的效用，包括英語 ↔ 愛爾蘭語和英語 ↔ 馬拉地語語言對。值得注意的是，該研究識別出最佳超參數和子詞模型類型，以顯著提升 Transformer 模型在低資源語言對中的翻譯品質。
  低資源語言的平行資料集短缺可能會阻礙 MT 的發展。為了解決這個問題，開發了 gaHealth，這是愛爾蘭語的第一個雙語健康資料語料庫。專注於健康領域，使用此領域內資料集開發的模型在 BLEU 分數方面表現出非常顯著的提升，與 LoResMT2021 共享任務中的模型相比是如此。隨後使用多維品質指標錯誤分類法進行的人工評估，展示了 Transformer 系統在降低準確性和流暢性錯誤方面的優異表現，與基於 RNN 的對應系統相比是如此。
  此外，本論文介紹了 adaptNMT 和 adaptMLLM，這兩個開源應用程式專門用於開發、微調和部署神經機器翻譯模型。這些工具大幅簡化了設定和評估流程，讓開發人員和翻譯人員都能更輕鬆地使用 MT。值得注意的是，植基於 OpenNMT 生態系統的 adaptNMT，透過強調模型開發的環境足跡，推廣了生態友善的自然語言處理研究。與 LoResMT2021 共享任務中的基準相比，adaptMLLM 微調 MLLM 證明了在兩個低資源語言對（英語 ↔ 愛爾蘭語和英語 ↔ 馬拉地語）的翻譯表現方面有進步。</paragraph>
...
視網膜影像和血管反映身體的微血管和巨血管健康狀況。它們可用於診斷糖尿病併發症，包括糖尿病視網膜病變 (DR)、神經病變、腎病變和動脈粥樣硬化性心血管疾病，以及預測心血管事件的風險。使用數位化視網膜影像進行高通量偵測糖尿病視網膜病變的人工智慧 (AI) 系統已經在臨床上採用。除了糖尿病視網膜病變篩檢之外，AI 整合也具有解決與糖尿病患者整體照護相關挑戰的巨大潛力。在這項工作中，我們旨在全面檢視有關基於視網膜影像的 AI 應用研究的文獻，這些應用與糖尿病的診斷、預後和管理相關。我們將描述整體 AI 輔助糖尿病照護的發現，包括但不限於糖尿病視網膜病變篩檢，並討論實施此類系統的障礙，包括涉及倫理、資料隱私、公平取得和可解釋性的問題。透過評估患者的健康狀態與糖尿病併發症以及未來心血管併發症的風險預後，AI 輔助視網膜影像分析有潛力成為糖尿病患者現代化個人化醫療的中心工具。
...
摘要：本研究從多個利害關係者的角度調查了不同人工智慧 (AI) 應用在教育中的可接受性，包括學生、教師和家長。承認 AI 在教育中的轉型潛力，它解決了與數據隱私、AI 代理、透明度、可解釋性和 AI 的道德部署相關的擔憂。通過小插曲方法，向參與者展示了四種場景，其中 AI 的代理、透明度、可解釋性和隱私被操縱。在每個場景之後，參與者完成了一項調查，其中記錄了他們對 AI 的全球效用、個人有用性、公正性、信心、風險以及在可用時使用每個場景的 AI 的意圖的看法。收集的數據包括 1198 名多方利益相關者參與者的最終樣本，通過合作機構和社交媒體活動進行分發，並重點關注對四個 AI 使用案例的個人回應。對數據的調解分析表明，對 AI 的接受度和信任在不同利益相關者群體中存在顯著差異。我們發現，AI 代理、透明度和可解釋性的高低水平之間的關鍵調解因素，以及使用不同教育 AI 的意圖，包括感知的全球效用、公正性和信心。該研究強調，接受 AI 在教育中的應用是一個微妙且多方面的問題，除了對不同利益相關者的看法之外，還需要仔細考慮具體的 AI 應用及其特徵。
...
摘要：<paragraph>基於可穿戴式單導程心電圖 (ECG) 裝置的遠端患者監測，在早期偵測心臟疾病方面具有顯著的潛力，特別是與用於自動化心臟疾病偵測的人工智慧 (AI) 方法結合使用時。過去已有研究應用基於深度學習的 AI 方法進行心臟疾病偵測。然而，這些模型尚未被廣泛接受為臨床診斷的可靠輔助工具，部分原因在於目前圍繞許多 AI 演算法的黑箱感知。特別是，有必要找出有助於做出準確診斷的 ECG 訊號關鍵特徵，從而增強模型的可解釋性。在本研究中，我們開發了一種視覺轉換器方法，以根據單導程 ECG 資料識別心房顫動。殘差網路 (ResNet) 方法也已開發出來，以便與視覺轉換器方法進行比較。這些模型應用於 Chapman-Shaoxing 資料集，以分類心房顫動，以及另一種常見的心律不整，竇性心動過緩和正常竇性心律的心跳。這些模型能夠識別決定最終分類的心跳關鍵區域，並強調 P 波和 T 波以及心跳持續時間和訊號振幅在區分正常竇性心律、心房顫動和竇性心動過緩方面的重要性。</paragraph>
...
摘要：本文介紹了一種使用先進大型語言模型 (LLM) 進行憂鬱症偵測和治療的新範例：生成式預訓練Transformer 4 (GPT-4)、Llama 2 聊天機器人和 Gemini。這些 LLM 經過微調，具備專業提示，可診斷、解釋和建議憂鬱症的治療介入措施。一種獨特的小樣本提示方法增強了模型根據 DSM-5 標準分析和解釋憂鬱症狀的能力。在互動階段，模型參與同理對話管理，從 PsychDB 和認知行為療法 (CBT) 指南等資源中汲取靈感，促進與經歷重度憂鬱症的個人進行支持性互動。此外，本研究引入了 Illuminate 資料庫，其中充實了各種 CBT 模組，有助於個人化治療建議。本研究使用 F1 分數、準確度、召回率、餘弦相似度和面向召回率的評估用於簡要評估 (ROUGE) 等指標評估 LLM 效能，證明其有效性。這種綜合方法將尖端的 AI 與既定的心理方法相結合，在心理保健領域提供了新的可能性，並展示了 LLM 在革新憂鬱症診斷和治療策略方面的潛力。
...
摘要：<paragraph>AI 系統的解釋很少能滿足受演算法決策 (ADM) 影響的人們的資訊需求。傳達資訊與受影響利害關係人重視的資訊之間的差距，可能會阻礙理解和遵守法規架構，例如 AI 法案。為了解決這個差距，我們提出了「XAI 新手問題庫」：受影響利害關係人資訊需求的目錄，涵蓋兩個 ADM 使用案例（就業預測和健康監測），涵蓋資料、系統背景、系統使用和系統規格等類別。在訪談研究中收集資訊需求，參與者在回答他們的詢問時收到了解釋。參與者進一步報告了他們的理解和決策信心，顯示雖然信心傾向於在收到解釋後增加，但參與者也遇到了理解上的挑戰，例如無法說明為什麼他們的理解感覺不完整。解釋進一步影響了參與者對系統風險和好處的看法，他們會根據使用案例確認或更改這些看法。當風險被認為很高時，參與者表示特別有興趣了解意圖的解釋，例如為什麼以及為了什麼目的而建立系統。透過這項工作，我們旨在透過在決定採用 ADM 系統時提供與他們相關的資訊和挑戰的概覽，來支持受影響利害關係人參與可解釋性。最後，我們透過總結我們的發現，列出六項關鍵啟示，為未來針對受影響利害關係人受眾的解釋設計提供資訊。</paragraph>
...
摘要：人工智能（AI）的快速發展，特別是在大型語言模型（LLM）和生成式 AI 領域，為各領域的應用開啟了新的途徑，但其在商學教育中的作用仍未得到充分探討。本研究首次引入了基準，用於評估七個主要 LLM 的效能，包括 OpenAI 的模型（GPT-3.5 Turbo、GPT-4 和 GPT-4 Turbo）、Google 的模型（PaLM 2、Gemini 1.0 Pro）和 Anthropic 的模型（Claude 2 和 Claude 2.1），這些模型用於 GMAT，這是研究生商學課程錄取過程中的一項重要考試。我們的分析顯示，大多數 LLM 的表現都優於人類應試者，其中 GPT-4 Turbo 不僅優於其他模型，還超過了頂尖商學院的研究生平均分數。透過案例研究，本研究探討了 GPT-4 Turbo 解釋答案、評估回應、找出錯誤、調整說明和產生替代情境的的能力。與前代相比，最新的 LLM 版本 GPT-4 Turbo、Claude 2.1 和 Gemini 1.0 Pro 在推理任務方面表現出顯著進步，突顯了它們在複雜問題解決方面的潛力。儘管 AI 在教育、評量和輔導方面的前景很明確，但仍存在挑戰。我們的研究不僅闡明了 LLM 的學術潛力，還強調了在教育中謹慎發展和應用 AI 的必要性。隨著 AI 技術的進步，建立 AI 互動的架構和協定、驗證 AI 生成的內容的準確性、確保不同學習者都能在全球各地取得內容，並創造一個 AI 支援人類專業知識的教育環境至關重要。本研究為進一步探索負責任地使用 AI 以豐富教育體驗並改善考試準備和評量方法奠定了基礎。
...
摘要：預測加護病房 (ICU) 病患的院內死亡率是最終臨床結果的關鍵。AI 已展現出優異的準確度，但缺乏可解釋性。為了解決這個問題，本文提出可解釋的多模式死亡率預測器 (X-MMP)，接近一個有效的、可解釋的 AI 解決方案，透過多模式 ICU 資料預測院內死亡率。我們在架構中採用多模式學習，能從臨床資料接收異質輸入並做出決策。此外，我們引入一種可解釋的方法，也就是分層傳播到 Transformer，作為 LRP 方法到 Transformer 的適當延伸，對多模式輸入產生解釋並揭露歸因於預測的顯著特徵。此外，每個模式對臨床結果的貢獻都可以視覺化，協助臨床醫生理解決策背後的推理。我們根據 MIMIC-III 和 MIMIC-III 波形資料庫配對子集建構一個多模式資料集。基準資料集上的全面實驗證明，我們提出的架構可以實現合理的解釋並具有競爭力的預測準確度。特別是，我們的架構可以輕鬆轉移到其他臨床任務，這有助於發現醫療保健研究中的關鍵因素。
...
摘要：<paragraph>在過去十年中，病理學中的AI方法已大幅進步。然而，由於許多挑戰，包括將研究結果轉化為臨床診斷產品的技術和法規障礙，以及缺乏標準化的介面，導致整合到常規臨床實務的進度緩慢。開放且與供應商無關的EMPAIA倡議應對了這些挑戰。在此，我們概述了EMPAIA的成就和經驗教訓。EMPAIA整合了病理學AI生態系統的各種利害關係人，即病理學家、電腦科學家和產業。在密切合作下，我們制定了技術互通性標準、AI測試和產品開發建議，以及可解釋性方法。我們實作了模組化和開源的EMPAIA平台，並成功整合了來自8個不同供應商的14個基於AI的影像分析應用程式，展示了不同的應用程式如何使用單一標準化介面。我們優先考慮需求，並評估了在歐洲和亞洲的14個不同病理實驗室中使用AI的實際臨床設置。除了技術發展外，我們還為所有利害關係人建立了一個論壇，以分享有關數位病理學和AI的資訊和經驗。商業、臨床和學術利害關係人現在可以採用EMPAIA的通用開源介面，為大規模標準化和流程簡化提供了獨特的機會。需要進一步的努力才能在常規實驗室使用中有效且廣泛地建立AI協助。為此，已成立一個永續基礎設施，即非營利組織EMPAIA International，以繼續標準化並支持廣泛實作和倡導AI輔助數位病理學的未來。</paragraph>
...
摘要：反事實解釋 (CE) 技術已引起關注，可用於向與 AI 系統互動的使用者提供見解。雖然已在醫學影像和自動駕駛等領域廣泛研究，但圖形反事實解釋 (GCE) 方法相對較少探討。GCE 會產生一個與原始圖形相似的圖形，其不同的結果基於基礎預測模型。在這些 GCE 技術中，根植於生成機制的技術儘管在其他領域（例如藝術風格和自然語言建模）中展現出令人印象深刻的成就，但受到的調查相對有限。對生成式解釋器的偏好源於它們在推理期間產生反事實實例的能力，利用輸入圖形的自主獲取擾動。基於上述依據，我們的研究引入了 RSGG-CE，一種用於反事實解釋的新型穩健隨機圖形生成器，能夠從考慮部分有序生成序列的學習潛在空間產生反事實範例。此外，我們進行量化和質化分析，以比較 RSGG-CE 與 SoA 生成式解釋器的效能，強調其產生合理的反事實候選的能力有所提升。
...
摘要：可解釋 AI 的動機之一是讓人類在使用和部署 AI 模型時做出更佳且更明智的決策。但需要仔細評估才能評估此預期是否已實現。目前的評估主要著重於解釋的演算法屬性，而涉及人類受試者的評估通常採用主觀問題來測試人類對解釋有用性的看法，而沒有依據客觀的指標和測量。在這項工作中，我們評估解釋是否能改善人類在機器學習模型開發的實際情境中進行決策。我們進行了一項混合方法使用者研究，涉及影像資料，以評估 SmoothGrad、GradCAM 和一個預言解釋在兩個任務上產生的顯著性圖：模型選擇和反事實模擬。令我們驚訝的是，我們沒有發現任何證據顯示在提供任何顯著性圖給使用者時，這些任務有顯著改善，即使是設計為易於理解且高度指示答案的合成預言解釋也是如此。儘管如此，解釋確實有助於使用者更準確地描述模型。這些發現表明對於顯著性為基礎的解釋中的有用性和誤解潛力應保持謹慎。
...
摘要：可解釋性和安全性培養信任。這些要求模型展現一致性和可靠性。要達成這些目標，必須使用和分析資料與知識以及與 AI 應用程式相關的統計和符號 AI 方法，單獨使用任何一種方法都不夠。因此，我們主張並試圖證明神經符號 AI 方法更適合用於打造值得信賴的 AI 系統。我們提出 CREST 架構，展示一致性、可靠性、使用者層級的可解釋性，以及安全性如何建構在神經符號方法上，這些方法使用資料和知識來支援關鍵應用程式的需求，例如健康和福祉。本文著重於大型語言模型 (LLM)，作為 CREST 架構內選定的 AI 系統。LLM 因其在處理廣泛自然語言處理 (NLP) 場景時的通用性而備受研究人員關注。例如，ChatGPT 和 Google 的 MedPaLM 已成為提供一般資訊和健康相關查詢的高度有前途的平台。儘管如此，這些模型仍然是黑盒子，儘管納入了人類回饋和指令引導調整。例如，ChatGPT 能產生不安全的回應，儘管已建立安全防護措施。CREST 提出了一種可行的途徑，在神經符號架構中利用程序和圖形知識，以闡明與 LLM 相關的挑戰。
...
摘要：本研究調查了已部署的人工智慧 (AI) 模型在預測 COVID-19 大流行期間及之後的死亡率方面的效能、可解釋性和穩健性。作為此類研究的第一個，我們發現貝氏神經網路 (BNN) 和智慧型訓練技術使我們的模型在資料大幅變動時仍能維持效能。我們的結果強調了開發穩健 AI 模型的重要性，即使在具有挑戰性的條件下，這些模型也能夠匹敵或超越臨床醫生預測。我們對模型可解釋性的探討揭示，隨機模型會產生更多元化和個人化的解釋，從而突顯了 AI 模型在現實世界的臨床環境中提供詳細和個別見解的必要性。此外，我們強調了量化 AI 模型中不確定性的重要性，這使臨床醫生能夠根據可靠的預測做出更明智的決策。我們的研究主張優先在醫療保健的 AI 研究中實施科學，並確保 AI 解決方案在現實世界的臨床環境中實用、有益且可持續。透過解決醫療保健環境中獨特的挑戰和複雜性，研究人員可以開發出有效改善臨床實務和患者預後的 AI 模型。
...
摘要：肺癌占英國癌症死亡人數的 21%，五年存活率在很大程度上取決於癌症被發現的期別。最近的研究表明，人工智慧方法能夠準確且早期地從例行掃描中診斷出肺癌。然而，此證據尚未轉化為臨床實務，其中一個障礙是缺乏可解釋的模型。本研究探討了將變異自動編碼器 (VAE)（一種生成式人工智慧模型）應用於肺癌病灶。提出的模型在從 LIDC-IDRI 公共資料集中的 3D 電腦斷層掃描中提取的病灶上進行訓練。透過聚類探討了 VAE 生成的 2D 切片潛在向量表示，以證明其品質，並用於肺癌診斷的 MLP 分類器模型，最佳模型達到了 AUC 0.98 和 93.1% 準確度的最新指標。群集分析顯示，VAE 潛在空間根據有意義的特徵成分（包括腫瘤大小、形狀、患者和惡性腫瘤類別）將惡性和良性病灶的資料集分開。我們還包括標準高斯 VAE (GVAE) 和較新的 Dirichlet VAE (DirVAE) 的比較分析，後者用 Dirichlet 分配取代先驗，以鼓勵具有解開特徵表示的更具可解釋性的潛在空間。最後，我們展示了與臨床有意義的特徵變化相應的潛在空間橫越的潛力。
...
摘要：現有的影像分類器輸出解釋工具可分為白盒工具（依賴模型內部）和黑盒工具（與模型無關）。由於醫療領域中 AI 的使用不斷增加，因此可解釋性工具的使用也隨之增加。現有的醫學影像解釋工作重點在於白盒工具，例如 gradcam。然而，改用黑盒工具有明顯的優點，包括可用於任何分類器以及可選擇的各種黑盒工具。在標準影像中，黑盒工具與白盒工具一樣精確。在本文中，我們比較了幾種黑盒方法在腦癌 MRI 資料集上與 gradcam 的效能。我們證明大多數黑盒工具不適合解釋醫學影像分類，並詳細分析其缺點原因。我們還展示了一個黑盒工具，基於因果可解釋性的 rex，其效能與 \gradcam 一樣好。
...
摘要：人工智慧開發社群正日益利用 Hugging Face 等託管中介，提供使用者上傳的模型和訓練資料的簡易存取。這些模型市集降低了數十萬使用者的技術部署障礙，但卻可能被用於許多潛在有害且非法的途徑。在本文中，我們說明了人工智慧系統（既可以「包含」內容，也可以是開放式工具）如何呈現迄今為止最棘手的平台治理挑戰。我們針對 Hugging Face、GitHub 和 Civitai 這三個說明性平台的數起事件提供案例研究，以探討模型市集如何審核模型。根據此分析，我們概述了產業為因應審核需求而開發的重要（但仍有限）實務：授權、存取和使用限制、自動內容審核和開放式政策開發。雖然當前的政策挑戰相當可觀，但我們最後提出了一些想法，說明平台如何能更好地動員資源，作為謹慎、公平且適度的法規存取點。
...
摘要：<paragraph>背景和目的：通过提取这些信息，基于机器或深度学习 (ML/DL) 的自动数据分析工具可以帮助临床医生和癌症研究人员从复杂的数据集中发现模式和关系。最近发表了许多基于 DL 的卵巢癌 (OC) 数据分析。这些分析在癌症的各个方面（例如，它们涉及的子域和癌症类型）和数据分析特征方面差异很大。然而，目前缺乏对这些分析在这些特征和人工智能保证 (AIA) 方面的全面理解。本系统评价旨在通过检查现有文献并明确关注关键特征和人工智能保证视角来填补这一空白。方法：使用 PRISMA 框架在三个期刊数据库中进行全面检索。分析仅包括 2015 年至 2023 年间发表在同行评审期刊上的研究。结果：在审查中，总共检查了 96 项 DL 驱动的分析。研究结果揭示了有关 DL 驱动的卵巢癌数据分析的几个重要见解：- 大多数研究 71%（96 项中的 68 项）侧重于检测和诊断，而没有研究解决 OC 的预测和预防。- 分析主要基于非多元人群的样本（75%（72/96 项研究）），仅限于某个地理位置或国家。- 只有很小一部分研究（仅 33%（32/96）进行了综合分析，其中大多数使用同质数据（临床或组学）。- 值得注意的是，只有 8.3%（8/96）的研究使用外部和多样化的数据集验证了他们的模型，突出了增强模型验证的必要性，以及 - 将 AIA 纳入癌症数据分析还处于非常早期的阶段；只有 2.1%（2/96）通过可解释性明确解决了 AIA。</paragraph>
...
摘要：<paragraph>可解釋性是深度學習中長期存在的挑戰，特別是在醫療保健等高風險領域。常見的可解釋性方法會強調驅動 AI 模型決策的影像區域。然而，人類在傳達解釋時，極度依賴語言，不只說明「在哪裡」，還說明「是什麼」。此外，大多數的可解釋性方法都專注於解釋個別的 AI 預測，而不是描述 AI 模型一般使用的特徵。後者對於模型和資料集稽核特別有用，甚至在 AI 逐漸用於新穎任務時，還能產生知識。在此，我們提出一個可解釋性策略，它使用視覺語言模型來識別視覺分類任務的基於語言的描述符。透過利用影像和文字之間預先訓練的聯合嵌入空間，我們的做法將新的分類任務估計為一個線性單字組合，結果是每個單字的權重，表示它與基於視覺的分類器的對齊方式。我們使用兩個醫學影像分類任務評估我們的做法，結果發現，儘管缺乏特定領域語言的訓練，產生的描述符在很大程度上與臨床知識一致。然而，我們的做法也識別出所使用的公開資料集中「捷徑連線」的可能性。為了達到可解釋性的功能性測量，我們進行了一項試驗性讀者研究，發現 AI 識別的單字能讓非專家人類在非平凡的層級上執行專門的醫療任務。總而言之，我們的結果強調了使用多模態基礎模型來提供直觀、基於語言的視覺任務解釋的潛力。</paragraph>
...
摘要：<paragraph>使用醫療影像訓練的人工智慧 (AI) 模型，用於臨床任務時，常會展現出在不同子群間效能差異的形式偏見。由於現實世界中的醫療影像資料並非所有偏見來源都容易辨識，因此全面評估這些偏見如何在模型中編碼，以及偏見緩解方法如何改善效能差異，是一項挑戰。在本文中，我們介紹一個新穎的分析架構，用於系統且客觀地探討醫療影像中的偏見對 AI 模型的影響。我們開發並測試了這個架構，用於進行受控的電腦模擬試驗，以評估醫療影像 AI 中的偏見，使用一種工具來產生具有已知疾病影響和偏見來源的合成磁共振影像。可行性透過使用三個反事實偏見情境來衡量模擬偏見效應對卷積神經網路 (CNN) 分類器和三個偏見緩解策略的效能影響，來展示。分析顯示，當 CNN 在合成資料集上訓練時，模擬偏見導致預期的子群效能差異。此外，重新加權被認為是此設定中最成功的偏見緩解策略，我們展示了解釋性 AI 方法如何協助調查使用此架構的模型中偏見的表現。開發公平的 AI 模型是一項相當大的挑戰，因為醫療影像資料集中可能存在許多且通常未知的偏見來源。在這項工作中，我們提出一個新穎的方法，用於客觀地研究偏見和緩解策略對深度學習管線的影響，這可以支援穩健且負責任的臨床 AI 開發。</paragraph>
...
摘要：機器學習提供巨大潛力，可用於自動預測中風後症狀及其對復健的反應。此項工作的重大挑戰包括神經影像資料的維度非常高、可用於學習的資料集規模相對較小，以及如何有效結合神經影像和表格資料（例如人口統計資訊和臨床特徵）。本文根據兩項策略評估多種解決方案。第一項是使用總結 MRI 掃描的 2D 影像。第二項是選擇改善分類精準度的關鍵特徵。此外，我們引入一種新穎的方法，即在結合從 MRI 中萃取的感興趣區域以及表格資料的符號表示的影像上訓練卷積神經網路 (CNN)。我們評估一系列 CNN 架構（2D 和 3D），這些架構在 MRI 和表格資料的不同表示上受過訓練，以預測中風後口語圖片描述能力的複合測量是否在失語症或非失語症範圍內。MRI 和表格資料來自 758 位參與 PLORAS 研究的英語中風倖存者。僅針對病灶大小的基線邏輯迴歸分類準確度為 0.678，當依序加入初始症狀嚴重程度和復原時間後，則上升至 0.757 和 0.813。在 2D 殘差神經網路中從每個 MRI 掃描中萃取 8 個感興趣區域，並與病灶大小、初始嚴重程度和復原時間結合時，觀察到最高的分類準確度 0.854。我們的研究結果說明了如何將影像和表格資料結合，以獲得高於中風後分類準確度，即使在機器學習術語中，資料集很小的時候也是如此。最後，我們提出如何改善目前的模型，以使用來自醫院掃描儀的影像，達到更高的準確度。
...
摘要：可解釋人工智慧 (XAI) 已成為處理任務關鍵應用程式時的一項基本需求，確保所使用的黑箱 AI 模型具有透明度和可解釋性。XAI 的重要性涵蓋從醫療保健到金融的各個領域，在這些領域中，了解深度學習演算法的決策制定過程至關重要。大多數基於 AI 的電腦視覺模型通常是黑箱；因此，在影像處理中提供深度神經網路的可解釋性對於它們在醫學影像分析、自動駕駛和遙測應用中的廣泛採用和部署至關重要。最近，已經引入了幾種用於影像分類任務的 XAI 方法。相反地，影像分割在可解釋性的背景下受到的關注相對較少，儘管它是電腦視覺應用中的一項基本任務，尤其是在遙測中。只有部分研究提出用於影像分割的基於梯度的 XAI 演算法。本文採用最近的無梯度 Sobol XAI 方法進行語意分割。為了衡量 Sobol 方法在分割方面的效能，我們提出了一種基於可學習雜訊模型的定量 XAI 評估方法。此模型的主要目標是在解釋圖上產生雜訊，其中較高的誘導雜訊表示較低的準確度，反之亦然。進行基準分析以評估和比較三種 XAI 方法的效能，包括 Seg-Grad-CAM、Seg-Grad-CAM++ 和 Seg-Sobol，並使用所提出的基於雜訊的評估技術。這構成了使用高解析度衛星影像執行和評估 XAI 方法的首次嘗試。
...
摘要：大型語言模型在多個領域中大量湧現，且時間短暫。然而，由於事實性、相干性和幻覺等問題，醫學和醫療保健領域對其採用持猶豫態度。由於醫療保健的風險很高，許多研究人員甚至警告在這些問題得到解決之前不要使用它。在醫療保健中實施和部署 LLM 的關鍵是讓這些模型值得信賴、透明（儘可能）且易於理解。在本文中，我們描述了創建可靠、值得信賴和無偏見模型的關鍵要素，作為其在醫療保健中採用的必要條件。具體來說，我們專注於在醫療保健背景下對幻覺的量化、驗證和緩解。最後，我們討論了 LLM 在醫療保健中的未來可能是什麼樣子。
...
摘要：人工智慧（AI）發展快速，目前已準備在各種應用程式中部署，例如自動系統、醫療診斷和自然語言處理。早期採用 AI 技術於真實世界應用程式並非沒有問題，特別是神經網路，它可能不穩定且容易受到對抗性範例影響。長期而言，需要開發適當的安全保證技術，以減少由於可避免的系統故障造成的潛在危害並確保可信度。本文著重於認證和可解釋性，概述了為確保 AI 決策安全而開發的技術，並討論未來的挑戰。
...
摘要：歐洲議會議會研究服務總局已為歐洲議會議員準備了一份報告，其中列舉了人工智慧 (AI) 在醫學和醫療保健中的七項主要風險：因 AI 錯誤而導致患者受到傷害、誤用醫療 AI 工具、AI 中的偏差和延續現有的不平等、缺乏透明度、隱私和安全問題、問責制方面的差距以及實施障礙。
  在本研究中，我們提出了 AI 系統可能實施的十四項功能性要求，以降低與其醫療目的相關的風險：AI 護照、使用者管理、法規檢查、僅限學術用途免責聲明、資料品質評估、臨床醫師雙重檢查、持續效能評估、稽核記錄、持續可用性測試、回顧回溯/模擬案例、偏差檢查、可解釋 AI、加密和使用經過現場測試的函式庫以及語意互操作性。
  我們在此的目的是提供技術解決方案的具體高階規格，以確保持續良好的效能和使用 AI 系統，以符合未來歐盟法規架構，讓患者受益。
...
摘要：人工智慧技術可被用於分類病患的生理活動並預測生命徵象，以進行遠距病患監測。基於非線性模型（如深度學習模型）的回歸分析由於其黑箱特性而具有有限的可解釋性。這可能需要決策者根據非線性模型結果盲目地做出重大決定，尤其是在醫療保健應用中。在非侵入式監測中，來自追蹤感測器的病患資料及其易感臨床屬性作為輸入特徵，用於預測未來的生命徵象。解釋各種特徵對監測應用整體輸出的貢獻對於臨床醫生的決策至關重要。在本研究中，提出了一個可解釋人工智慧（QXAI）架構，用於監督學習方法中的回歸和分類任務，具有事後模型可解釋性和內在可解釋性。這是通過利用 Shapley 值概念並將注意力機制納入深度學習模型來實現的。我們採用人工神經網路 (ANN) 和基於注意力的雙向 LSTM (BiLSTM) 模型，根據感測器資料預測心率和分類生理活動。深度學習模型在預測和分類任務中都取得了最先進的成果。對輸入資料進行全局解釋和局部解釋，以了解各種病患資料的特徵貢獻。所提出的 QXAI 架構使用 PPG-DaLiA 資料來預測心率，並使用行動健康 (MHEALTH) 資料根據感測器資料來分類生理活動。蒙地卡羅近似應用於該架構，以克服 Shapley 值計算所需的時間複雜度和高運算能力需求。
...
摘要：在可解釋人工智慧 (XAI) 研究中，主要的重點在於為專家和從業人員解釋模型。模型不可知論和局部解釋方法在許多應用中被認為是可解釋且足夠的。然而，在醫療保健等領域，最終使用者是沒有 AI 或領域專業知識的患者，因此迫切需要更易於理解並建立對模型運作的信任的模型解釋。我們假設生成具有敘述性、特定於患者和全局（模型整體）的模型解釋將有助於提高可理解性並促進決策制定。我們使用決策樹模型來測試這一點，為被確定為患有冠狀動脈疾病高風險的患者生成局部和全局解釋。這些解釋會提供給非專家使用者。我們發現對特定類型的解釋有強烈的個人偏好。大多數參與者偏好全局解釋，而較少數的參與者偏好局部解釋。基於這些參與者的心理模型的任務評估提供了有價值的回饋，以增強敘述性全局解釋。這反過來指導了既值得信賴又可操作的健康資訊系統的設計。
...
摘要：可解釋 AI 在醫學影像中至關重要。在具有挑戰性的神經科學領域中，視覺主題呈現出高度的複雜性，特別是在三維空間內。神經科學的應用，包括從 MRI 中識別腦溝特徵，由於專家之間不同的註釋協定和複雜的三維大腦功能，面臨著重大的障礙。因此，傳統的可解釋性方法無法有效驗證和評估這些網路。為了解決這個問題，我們首先提出了一個數學公式，描述了不同電腦視覺任務中各種類型的解釋需求，根據驗證協定的可靠性，將其分類為自解釋、半解釋、非解釋和新模式學習應用。關於這個數學公式，我們提出了一個 3D 可解釋性架構，旨在驗證深度學習網路在偵測腦回旁溝（大腦解剖學特徵）中的輸出。這個架構整合了局部 3D 解釋、透過降維的全局解釋、串聯的全局解釋和統計形狀特徵，揭示了模式學習的新見解。我們在具有挑戰性的 TOP-OSLO 資料集上訓練和測試了兩個先進的 3D 深度學習網路，顯著提高了腦溝偵測的準確度，特別是在左半球。在使用此資料集的不同註釋協定進行評估時，我們強調了在我們提出的 3D 架構中，公正的註釋過程對於實現精確預測和有效模式學習至關重要。所提出的架構不僅註釋了可變腦溝，還揭示了隱藏的 AI 知識，有望推進我們對大腦解剖和功能的理解。
...
摘要：電子健康紀錄 (EHR) 和例行文件編寫實務在病患的日常照護中扮演至關重要的角色，提供關於健康、診斷和治療的完整紀錄。然而，複雜且冗長的電子健康紀錄敘述會讓醫療保健提供者不堪負荷，冒著診斷不準確的風險。儘管大型語言模型 (LLM) 已展現其在各種語言任務中的潛力，但其在醫療領域的應用需要確保將診斷錯誤降到最低，並防止病患受到傷害。在本文中，我們概述了一種創新的方法，用以提升大型語言模型在自動化診斷生成領域的熟練度，這方法是透過整合一個醫學知識圖譜 (KG) 和一個新穎的圖譜模型：Dr.Knows，其靈感來自於臨床診斷推理程序。我們從美國國家醫學圖書館的統一醫學語言系統 (UMLS) 中衍生出知識圖譜，這是一個強大的生物醫學知識儲存庫。我們的這種方法否定了預先訓練的需要，而改用知識圖譜作為輔助工具，協助解釋和總結複雜的醫學概念。使用真實世界的醫院資料集，我們的實驗結果證明了將大型語言模型與知識圖譜結合的提議方法有潛力提升自動化診斷生成的準確度。更重要的是，我們的這種方法提供了一條可解釋的診斷途徑，讓我們更接近實現人工智慧增強的診斷決策支援系統。
...
摘要：現有的用於診斷膝骨關節炎 (OA) 的人工智慧 (AI) 模型由於缺乏透明度和可解釋性而面臨批評，儘管它們達到了與醫療專家相似的表現。這種不透明性使得它們難以在臨床實務中獲得信任。最近，可解釋人工智慧 (XAI) 已成為一種專業技術，它可以通過揭示預測的來源來提供對模型預測的信心，從而促進在醫療保健中使用 AI 系統。本文提供了用於膝骨關節炎診斷的 XAI 技術的首次調查。XAI 技術從兩個角度進行探討：資料可解釋性和模型可解釋性。本文的目的是提供有價值的見解，了解 XAI 對更可靠的膝骨關節炎診斷方法的潛力，並鼓勵在臨床實務中採用它。
...
摘要：最近在醫療保健中的人工智慧應用進步神速，在診斷和疾病預後方面超越人類表現的潛力令人難以置信。然而，隨著人工智慧模型日益複雜，人們開始關注它們的不透明性、潛在偏差和對可解釋性的需求。為了確保人工智慧系統的信任度和可靠性，尤其是在臨床風險預測模型中，可解釋性變得至關重要。可解釋性通常是指人工智慧系統能夠對其決策邏輯或決策本身提供穩健的解釋，以便人類利益相關者理解。在臨床風險預測中，可解釋性的其他方面，例如公平性、偏差、信任和透明度，也代表了超越可解釋性的重要概念。在這篇評論中，我們探討了這些概念之間的關係，因為它們經常被一起使用或互換使用。這篇評論還討論了最近在開發可解釋臨床風險預測模型方面取得的進展，強調了在臨床實務中多種常見模式中進行定量和臨床評估和驗證的重要性。它強調了外部驗證和結合多種可解釋性方法以增強信任度和公平性的必要性。採用嚴謹的測試，例如使用具有已知生成因素的合成資料集，可以進一步提高可解釋性方法的可靠性。開放獲取和程式碼共享資源對於透明度和可複製性至關重要，有助於可解釋性研究的發展和可信度。儘管存在挑戰，但一種從頭到尾的可解釋性方法在臨床風險預測中，結合了從臨床醫生到開發人員的利益相關者，對於成功至關重要。
...
摘要：儘管在醫學和醫療保健方面的人工智慧 (AI) 有了重大的進展，但 AI 技術的部署和採用在實際的臨床實務中仍然有限。近年來，人們對於與醫療 AI 相關的技術、臨床、倫理和法律風險提出了疑慮。為了增加實際世界的採用率，醫療 AI 工具必須獲得患者、臨床醫師、醫療保健組織和主管機關的信任和接受。本研究描述了 FUTURE-AI 指南，這是第一個國際共識架構，用於指導醫療保健中值得信賴的 AI 工具的開發和部署。FUTURE-AI 聯盟成立於 2021 年，目前由來自 51 個國家的 118 位跨領域專家組成，代表了所有洲，包括 AI 科學家、臨床醫師、倫理學家和社會科學家。在兩年的時間裡，該聯盟透過一個反覆運算的過程定義了值得信賴的 AI 指導原則和最佳實務，包括深入的文獻回顧、修改後的德爾菲調查和線上共識會議。FUTURE-AI 架構是根據醫療保健中值得信賴的 AI 的 6 項指導原則建立的，即公平性、普遍性、可追溯性、可用性、穩健性和可解釋性。透過共識，定義了一組 28 項最佳實務，涵蓋技術、臨床、法律和社會倫理層面。這些建議涵蓋了醫療 AI 的整個生命週期，從設計、開發和驗證到法規、部署和監控。FUTURE-AI 是一個基於風險、不假設的指南，它提供了一個結構化的方法來建構醫療 AI 工具，這些工具將在實際實務中受到信任、部署和採用。鼓勵研究人員在概念驗證階段考慮這些建議，以促進未來醫療 AI 向臨床實務的轉化。
...
摘要：醫療領域的人工智慧在醫療影像、病人照護和其他領域的新興應用已取得重大進展。雖然這些應用已在回顧性研究中證明是成功的，但實際應用者卻很少。醫療 AI 領域面臨著各種挑戰，包括建立使用者信任、遵守法規和使用資料符合道德。可解釋 AI (XAI) 旨在讓人們了解 AI 並信任其結果。本文根據近年發表的 198 篇文章的代表性樣本，對醫療決策支援的 XAI 解決方案的最新發展進行文獻回顧。相關文章的系統性綜合得出了幾個發現。(1) 這些解決方案大多採用與模型無關的 XAI 技術，(2) 深度學習模型的使用多於其他類型的機器學習模型，(3) 可解釋性用於促進信任，但很少有研究報告醫師參與迴圈，(4) 視覺和互動式使用者介面在理解系統的解釋和建議方面更有用。需要與醫療和 AI 專家合作進行更多研究，這可以指導設計、實施和評估 XAI 解決方案在醫療中的適當架構的發展。
...
摘要：體外受精是治療不孕症最廣泛的方法之一。其中一個主要挑戰是評估和選擇胚胎進行植入，這是一個具有很大的臨床間和臨床內變異性的過程。基於深度學習的方法正受到關注，但它們的不透明性質會影響它們在臨床環境中的接受度，而在臨床環境中，決策制定中的透明度至關重要。在本文中，我們分析了 AI 輔助胚胎分析模型的可解釋性的現有工作，並找出其局限性。我們還討論了如何將這些模型作為決策支持系統整合到臨床環境中，同時考慮臨床醫生和患者的需求。最後，我們提出了提高可解釋性和可信度的準則，推進這項技術邁向既定的臨床實踐。
...
摘要：在需求工程 (RE) 领域中，可解释人工智能 (XAI) 在将人工智能支持系统与用户需求、社会期望和监管标准相结合中日益重要的意义获得了认可。一般来说，可解释性已成为影响系统质量的重要非功能性需求。然而，可解释性和性能之间的权衡挑战了可解释性的假定积极影响。如果满足可解释性的要求需要降低系统性能，则必须仔细考虑这些质量方面中的哪一个优先，以及如何在它们之间进行折衷。在本文中，我们批判性地研究了所谓的权衡。我们认为，以一种微妙的方式来处理这个问题是最好的，这种方式结合了资源可用性、领域特征和风险考虑。通过为未来的研究和最佳实践提供基础，这项工作旨在推进人工智能的 RE 领域。
...
摘要：在需求工程（RE）领域内，可解释人工智能（XAI）在将人工智能支持系统与用户需求、社会期望和监管标准相一致方面日益显着的重要性已得到认可。一般来说，可解释性已成为影响系统质量的重要非功能性需求。然而，可解释性和性能之间的权衡挑战了可解释性的正面影响。如果满足可解释性的要求需要降低系统性能，那么必须仔细考虑这些质量方面中哪一方面优先，以及如何在它们之间进行权衡。在本文中，我们批判性地考察了所谓的权衡。我们认为，最好以一种微妙的方式来处理它，这种方式结合了资源可用性、领域特征和风险考虑。通过为未来的研究和最佳实践提供基础，这项工作旨在推进人工智能的再利用领域。
...
摘要：<paragraph>本文批判性地評估了歐盟委員會提出的 AI 法案，對高風險 AI 系統風險管理和風險可接受性的方法，這些系統對基本權利和安全構成風險。該法案旨在以相應的法規負擔促進「值得信賴」的 AI。其有關風險可接受性的條款要求將高風險系統的殘餘風險減少或消除「在可能的範圍內」，並考慮「技術水準」。此標準，特別是如果狹義解釋的話，是不可行的，既不能促進相應的法規負擔，也不能促進可信度。相反，議會對風險管理條款的最新修正草案引入了「合理性」、「成本效益分析」，且更透明地說明了風險可接受性判斷的價值觀和背景性質。本文認為，議會的方法更可行，且能更好地平衡相稱性和可信度的目標。本文說明風險可接受性判斷中的合理性將需要什麼，並根據過失法和歐洲醫療器械法規中的原則進行說明。本文主張，風險可接受性判斷的方法需要穩固的公民合法性基礎：包括監管機構的詳細指導或參與，以及受影響利益相關者的有意義投入。</paragraph>
...
摘要：可解釋人工智慧 (XAI) 是機器學習中快速進展的領域，旨在解開複雜模型的預測。XAI 特別需要在敏感應用中，例如在醫療保健中，當診斷、建議和治療選擇可能依賴於人工智慧系統做出的決策時。AI 方法也已廣泛用於老化研究，特別是在開發生物時鐘模型和識別老化和與年齡相關疾病的生物標誌方面。然而，這裡 XAI 的潛力有待充分認識。我們討論了 XAI 在開發「老化時鐘」中的應用，並根據對特定生理系統的關注對文獻進行了全面的分析。
...
摘要：部分原型模型是可解釋的設計影像分類器，也是黑箱 AI 的有前途的替代方案。本文探討可解釋機器學習的適用性和潛力，特別是 PIP-Net，用於實際醫療影像資料的自動診斷支援。PIP-Net 學習人類可理解的原型影像部分，我們評估其準確度和可解釋性，用於骨折偵測和皮膚癌診斷。我們發現 PIP-Net 的決策制定過程符合醫療分類標準，同時僅提供影像層級類別標籤。由於 PIP-Net 的原型無監督預訓練，因此可以輕鬆識別資料品質問題，例如 X 光中的不需要文字或標記錯誤。此外，我們首次展示人類可以透過直接停用不需要的原型手動修正 PIP-Net 的推理。我們得出結論，部分原型模型由於其可解釋性和進階模型除錯的潛力，對於醫療應用很有前途。
...
摘要：可解釋 AI (XAI) 是機器學習研究中日益重要的領域，旨在使黑盒模型透明且可解釋。在本文中，我們提出了一種新的 XAI 方法，它使用條件特徵置換生成的所謂反事實路徑。該演算法透過識別最影響模型預測變化的特徵順序置換來衡量特徵重要性。它特別適合根據包含領域知識的知識圖譜中的反事實路徑來產生解釋。反事實路徑在解釋和視覺化黑盒模型中為當前的 XAI 方法引入了額外的圖形維度。使用合成和醫療資料的實驗證明了我們方法的實用適用性。
...
摘要：人工智能 (AI) 中的可解释性领域见证了越来越多的研究和日益增长的学术兴趣。然而，在解释机器学习算法结果时缺乏人性化和个体化的解释，极大地阻碍了临床医生在研究和临床实践中接受这些方法。为了解决这个问题，我们的研究使用反事实解释来探索“假设”情景在医学研究中的适用性。我们的目标是扩展我们对用于诊断小儿后颅窝脑肿瘤的磁共振成像 (MRI) 特征的理解，超越现有的界限。在我们的案例研究中，提出的概念提供了一种新颖的方法来检查替代决策制定方案，这些方案提供个性化和特定于上下文的见解，从而能够验证预测并阐明在不同情况下出现的差异。此外，我们探索了反事实用于数据扩充的可能性，并评估了它们作为我们医学研究案例中替代方法的可行性。结果证明了使用反事实解释来增强对临床研究中人工智能驱动方法的接受度的巨大潜力。
...
摘要：人工智慧領域的最新進展，促成了各種由人工智慧驅動的失智症評估的發展，可用於在失智症的早期階段識別患者。這可以徹底改變失智症的照護環境。醫療社群必須了解各種人工智慧評估，並根據其在失智症患者 (PwD) 早期識別方面的有效性、效率、實用性、可靠性和準確性程度來選擇它們。另一方面，人工智慧開發人員也應該了解各種非人工智慧評估以及最近開發的人工智慧評估。因此，這篇論文既適合臨床醫師和人工智慧工程師閱讀，它填補了文獻中在解釋現有失智症識別解決方案給臨床醫師、以及使用技術和最廣泛的失智症資料集給人工智慧工程師方面的空白。它遵循對人工智慧和非人工智慧失智症評估論文的回顧，提供有關各種失智症評估的寶貴資訊，供人工智慧和醫療社群使用。討論和結論重點說明了最顯著的研究方向和現有解決方案的成熟度。
...
摘要：<paragraph>可解释性对人工智能 (AI) 技术构成重大挑战。目前关于可解释人工智能 (XAI) 的研究缺乏提取学习任务全局知识的效率，因此存在诸如不精确显着性、上下文感知缺失和含义模糊等缺陷。在本文中，我们提出了类关联嵌入 (CAE) 方法来解决这些问题。我们采用编码器-解码器架构来嵌入样本特征，并将它们同时分离为与类相关的和与个体相关的风格向量。将给定样本的个体风格代码与另一个类的类风格代码重新组合，得到一个合成的样本，该样本保留了个人特征，但更改了类分配，遵循循环对抗学习策略。类关联嵌入将所有实例的全局类相关特征提取到一个统一的域中，并在类之间有很好的分离。然后可以提取不同类之间的转换规则，并进一步应用于单个实例。然后，我们提出了一个主动 XAI 框架，该框架沿引导路径操作某个样本的类风格向量朝向反类，从而产生一系列具有相同个人特征的反例合成样本。将这些反事实样本与原始样本进行比较，可以直观地说明分类任务的本质。我们采用该框架进行医学图像分类任务，结果表明，与现有方法相比，可以获得更精确的显着性图，并具有强大的上下文感知表示。此外，可以通过遍历类风格空间中的路径直接可视化疾病病理。</paragraph>
...
摘要：提供基於機器學習的 AI 預測的高品質解釋是一項具有挑戰性和複雜性的任務。要順利進行，它需要以下因素：選擇適當的解釋概括性/特殊性層級；考量解釋受益者對所考慮的 AI 任務的熟悉程度的假設；參考促成決策的特定元素；利用額外的知識（例如專家證據），這些知識可能不是預測過程的一部分；並提供支持負面假設的證據。最後，系統需要以明確可解讀且可能令人信服的方式制定解釋。考量這些因素，ANTIDOTE 促成了可解釋 AI 的整合願景，其中深度學習過程的低層級特徵與人類論證能力的更高層級架構結合。ANTIDOTE 將利用深度學習和論證中的跨領域能力，來支持更廣泛且創新的可解釋 AI 觀點，其中對臨床病例審議的高品質解釋需求至關重要。作為該專案的第一個成果，我們發布了 Antidote CasiMedicos 資料集，以利於對一般可解釋 AI 以及特別是醫療領域中的論證進行研究。
...
摘要：近年來，圖神經網路的進展迅速，在藥物發現、醫療診斷和推薦系統方面都有許多新發展。儘管進展顯著，但許多網路都是「黑盒子」，對於網路在學習什麼「具體」內容了解甚少。許多高風險應用，例如藥物發現，需要來自模型的人類可理解的解釋，以便使用者可以識別錯誤並發現新知識。因此，可解釋 AI 演算法的開發對於我們獲取 AI 的好處至關重要。
我們提出了一種稱為 eXplainable Insight (XInsight) 的 GNN 可解釋性演算法，它使用 GFlowNets 產生模型解釋的分布。由於 GFlowNets 根據獎勵生成機率成比例的物件，因此與僅學習最大獎勵範例的先前方法相比，XInsight 可以產生多樣化的解釋集合。我們透過為在兩個圖形分類任務上訓練的 GNN 產生解釋來展示 XInsight：使用 MUTAG 資料集對致突變化合物進行分類，並使用我們已開放原始碼的合成資料集對非環狀圖形進行分類。我們透過使用 QSAR 建模分析產生的化合物來展示 XInsight 解釋的效用，我們發現 XInsight 產生的化合物會根據親脂性進行分類，這是致突變性的已知相關性。我們的結果表明，XInsight 產生了解釋的分布，揭示了模型所展示的底層關係。它們還強調了產生多樣化解釋集合的重要性，因為它使我們能夠發現模型中的隱藏關係，並為進一步分析提供有價值的指導。
...
摘要：我們提出並實作一個可解釋機器學習分類模型，用於基於表達布林公式的可解釋人工智慧 (XAI)。潛在應用包括信用評分和醫療狀況診斷。布林公式定義一個規則，具有可調整的複雜性（或可解釋性），根據該規則對輸入資料進行分類。此類公式可以包含任何可應用於一個或多個布林變數的運算子，因此與更嚴格的基於規則和基於樹的方法相比，提供了更高的表達力。分類器使用原生局部最佳化技術進行訓練，有效地搜尋可行公式的空間。淺層規則可以用快速的整數線性規劃 (ILP) 或二次無約束二元最佳化 (QUBO) 求解器確定，這些求解器可能由特殊用途硬體或量子裝置提供支援。我們結合原生局部最佳化器的表達力和效率，以及這些裝置的快速運算，透過執行最佳化完整布林公式子樹的非局部移動來實現。我們提供廣泛的數值基準測試結果，其中包含在眾所周知的公開資料集上針對幾個基準的特色。根據結果，我們發現原生局部規則分類器通常與其他分類器具有競爭力。新增非局部移動可使用較少的反覆運算來達成類似的結果，因此使用專用或量子硬體可以透過快速提出非局部移動來加速。
...
摘要：為了應對心理健康問題的全球挑戰，我們提出了一個基於邏輯神經網路 (LNN) 的神經符號 AI 方法，用於診斷心理疾病。由於心理疾病缺乏有效的治療覆蓋範圍，因此需要一種 AI 解決方案來協助治療師進行診斷。然而，目前的類神經網路模型缺乏可解釋性，治療師可能不會信任它們。LNN 是一種遞迴神經網路架構，結合了神經網路的學習能力和基於經典邏輯的 AI 的推理能力。所提出的系統使用來自臨床訪談的輸入謂詞來輸出心理疾病類別，並使用不同的謂詞剪枝技術來實現可擴充性和更高的分數。此外，我們提供了一個見解提取方法來幫助治療師進行診斷。所提出的系統解決了當前神經網路模型缺乏可解釋性的問題，並為心理疾病診斷提供了一個更值得信賴的解決方案。
...
摘要：隨著機器學習模型在醫療診斷中日益普及，可解釋性和透明度的需求變得至關重要。XAI 文藝復興標誌著該領域的重大轉變，旨在重新定義醫療診斷模型的可解釋性。本文探討了可解釋 AI (XAI) 領域內的創新方法和方法論，這些方法和方法論正在徹底改變醫療診斷模型的可解釋性。通過闡明基礎決策制定過程，XAI 技術使醫療保健專業人員能夠理解、信任並有效地利用這些模型進行準確可靠的醫療診斷。這篇評論重點介紹了 XAI 在醫療診斷方面的關鍵進展及其轉變醫療保健領域的潛力，最終改善患者的治療效果並促進對 AI 驅動診斷系統的信任。
...
摘要：在以高度連結性和流動性為特徵的環境中，加上心血管疾病激增，透過遠端監控心血管健康狀況來減少醫療保健費用的必要性變得更加明顯。準確檢測和分類心律不整對於診斷患有心臟不規則的人至關重要。本研究強調在家中環境中使用心電圖 (ECG) 測量進行即時心律不整檢測的可行性。本論文提出心律不整檢測的新應用，利用尖端的 You-Only-Look-Once (YOLO)v8 演算法對單導程 ECG 訊號進行分類。我們引入一個新的損失修改 YOLOv8 模型，針對 MIT-BIH 心律不整資料集進行微調，實現即時連續監控。獲得的結果證實了我們方法的有效性，該模型在 NVIDIA Tesla V100 上實現了 99.5% 的平均準確度和 0.992 mAP@50，以及 0.002 秒的快速檢測時間。我們的研究說明了即時心律不整檢測的潛力，使用戶能夠在家中舒適地視覺化解讀模型輸出。此外，本研究為擴展到能夠部署在醫療保健領域的即時可解釋 AI (XAI) 模型奠定了基礎，從而顯著推進了醫療保健解決方案的領域。
...
摘要：乳癌 (BC) 仍然是一個重大的健康威脅，目前尚無長期的治癒方法。早期發現至關重要，但乳房攝影的判讀卻受到高假陽性和假陰性的阻礙。隨著乳癌的發生率預計將超過肺癌，改善早期檢測方法至關重要。熱像攝影使用高解析度紅外線相機，特別是在結合人工智慧 (AI) 時，提供了希望。這項工作提出了一個基於注意力的卷積神經網路進行分割，在乳癌檢測和分類中提供了更快的速度和更高的精度。該系統增強影像並使用可解釋的 AI 執行癌症分割。我們提出了一個基於Transformer注意力的卷積架構 (UNet) 進行故障識別，並採用梯度加權類激活映射 (Grad-CAM) 來分析 UNet 架構中偏見和弱點的區域，並結合 IRT 影像。與現有的深度學習框架相比，我們提出的框架的優越性得到了證實。
...
摘要：憂鬱症是最普遍且嚴重的精神疾病，它會造成嚴重的財務和社會影響。憂鬱症的偵測對於早期介入以減輕這些後果至關重要。如此高風險的決定本質上需要可解釋性。雖然一些憂鬱症偵測研究試圖根據重要性評分或注意力權重來解釋決策，但這些解釋與基於憂鬱症狀的臨床憂鬱症診斷標準不一致。為了填補這個缺口，我們遵循計算設計科學範例來開發一個新穎的多尺度時間原型網路 (MSTPNet)。MSTPNet 創新地偵測和解釋憂鬱症狀以及它們持續多久。使用大型資料集進行的廣泛實證分析顯示，MSTPNet 以 0.851 的 F1 分數優於最先進的憂鬱症偵測方法。此結果還揭示了調查方法中未注意到的新症狀，例如分享對不同生活的仰慕。我們進一步進行了一項使用者研究，以證明其在可解釋性方面優於基準。本研究透過一個新穎的可解釋深度學習模型，為社群媒體中的憂鬱症偵測提供了 IS 文獻。在實務上，我們提出的方法可以實作在社群媒體平台中，以提供個人化的線上資源給偵測出的憂鬱症患者。
...
摘要：電子健康紀錄 (EHR) 作為預見人工智慧 (AI) 驅動的醫療保健轉型中一個重要的資料來源。
然而，反映在 EHR 備註中的臨床偏見會導致 AI 模型繼承並放大這些偏見，進而讓健康差距持續存在。本研究探討了 EHR 備註中污名化語言 (SL) 對死亡率預測的影響，使用基於 Transformer 的深度學習模型和可解釋 AI (XAI) 技術。我們的研究結果表明，由臨床醫生撰寫的 SL 會對 AI 效能產生負面影響，特別是對黑人患者而言，突顯 SL 是 AI 模型開發中種族差異的來源。為了探索一種在營運上有效的方法來減輕 SL 的影響，我們透過臨床醫生的協作網路探討 SL 產生的模式，找出核心臨床醫生對 AI 模型中的種族差異具有較強的影響。我們發現，移除由核心臨床醫生撰寫的 SL 是比消除資料全集中所有 SL 更有效率的偏見降低策略。本研究為負責任的 AI 開發提供可行的見解，並有助於了解臨床醫生的行為和醫療保健中的 EHR 備註撰寫。
...
摘要：當代透過 AI 的自動化需要大量幕後的人力，這往往既被隱形化又領取低薪。由於包含標籤和維護工作的隱形勞動是當代 AI 系統中不可或缺的一部分，因此讓使用者了解其角色仍然很重要。我們建議這可以透過可解釋 AI（XAI）設計來完成，特別是女性主義交叉 XAI。我們提出源自女性主義交叉研究的製圖方法，用以描繪 AI 的系統觀點，並納入與隱形勞動相關的 AI 向度。
...
摘要：虛擬心理健康助理 (VMHA) 不斷進步，以支援每年有 6,000 萬人次初級保健就診和 600 萬人次急診室 (ER) 就診的負擔過重的全球醫療保健系統。這些系統是由臨床心理學家、精神科醫師和人工智慧 (AI) 研究人員為認知行為療法 (CBT) 所建置。目前，VMHA 的角色是透過資訊提供情緒支持，較少著重於與患者建立反思性的對話。需要一種更全面、安全且可解釋的方法來建置負責任的 VMHA，以提出後續問題或提供見解豐富的回應。這項調查提供了對現有心理健康對話代理的系統性批判性回顧，接著提出對 VMHA 改進的新見解，包括脈絡知識、資料集，以及它們在臨床決策支援中浮現的角色。我們也提供新的方向，以可解釋性、安全性，以及整體可信度來豐富 VMHA 的使用者體驗。最後，我們提供評估指標和實務考量，讓 VMHA 超越現有文獻，在主動溝通中建立 VMHA 與患者之間的信任。
...
摘要：XAI 指的是建立 AI 應用程式的技術與方法，協助最終使用者詮釋 AI 模型的輸出與預測。黑箱 AI 應用程式在高風險決策情境中，例如醫療領域，已增加了透明度與可解釋性的需求，因為錯誤的預測可能會造成嚴重的後果。模型可解釋性和可詮釋性對於 AI 模型在醫療實務中的成功部署至關重要。AI 應用程式的基礎推理需要對臨床醫生透明，才能獲得他們的信任。本文提供了 XAI 面向與挑戰在醫療領域中的系統性回顧。本研究的主要目標是回顧各種 XAI 方法、它們的挑戰，以及在醫療保健領域中相關的機器學習模型。這些方法分為六類討論：面向特徵的方法、全局方法、概念模型、代理模型、局部像素化方法，以及以人為中心的方法。最重要的是，本文探討了 XAI 在醫療保健問題中的角色，以釐清其在安全關鍵應用中的必要性。本文旨在透過回顧相關的實驗結果，建立對醫療保健領域中與 XAI 相關應用程式的全面理解。為了促進未來的研究以填補研究差距，本文從不同的觀點探討了 XAI 模型的重要性及其限制。
...
摘要：最先進的機器學習模型經常會學習到訓練資料中內嵌的虛假相關性。當這些模型被部署到高風險決策制定中時，例如皮膚癌偵測等醫學應用，就會產生風險。為了解決這個問題，我們提出 Reveal to Revise (R2R)，一個涵蓋了整個可解釋人工智慧 (XAI) 生命週期的架構，讓從業人員能夠透過最少的人為互動，反覆找出、減輕和（重新）評估虛假的模型行為。在第一步（1）中，R2R 透過找出歸因值中的異常值，或是透過檢查模型學習到的潛在概念，來揭露模型的弱點。其次（2），偵測負責的偽影並在輸入資料中進行空間定位，然後利用這個來（3）修正模型行為。具體來說，我們應用 RRR、CDEP 和 ClArC 的方法來進行模型修正，並（4）重新評估模型的效能和對偽影的剩餘敏感性。我們使用兩個醫學基準資料集進行黑色素瘤偵測和骨齡估計，將我們的 R2R 架構應用到 VGG、ResNet 和 EfficientNet 架構中，從而揭露和修正真正的資料內在偽影，以及受控設定中的合成變體。為了完成 XAI 生命週期，我們展示多個 R2R 迭代來減輕不同的偏差。程式碼可在 https://github.com/maxdreyer/Reveal2Revise 取得。
...
摘要：可解釋人工智慧 (XAI) 領域近年來已大幅進展，但進展主要在於電腦視覺和自然語言處理。對於時間序列，其輸入通常難以解釋，僅有有限的研究可用於 XAI。在這項工作中，我們提出了一個虛擬檢查層，將時間序列轉換為可解釋的表示，並允許透過區域 XAI 方法（例如逐層相關性傳播 (LRP)）將相關性歸因傳播到這個表示。藉此，我們將 XAI 方法家族的適用性擴展到輸入僅在轉換後才可解釋的網域（例如語音）。在此，我們專注於傅立葉轉換，它顯著應用於時間序列和 LRP 的解釋，並將我們的稱為 DFT-LRP。我們在各種時間序列分類設定中展示了 DFT-LRP 的效用，例如音訊和電子健康記錄。我們展示了 DFT-LRP 如何揭示在不同網域（例如時間與頻率網域）中訓練的模型的分類策略的差異，或有助於發現模型如何作用於資料中的虛假關聯。
...
摘要：解釋深度學習模型預測的能力對於最終使用者來說是一項重要的功能，它可以利用人工智慧 (AI) 的力量來進行醫療決策過程，這通常被認為是不透明且難以理解的。在本文中，我們採用最先進的可解釋人工智慧 (XAI) 方法來解釋黑箱 AI 模型在甲狀腺結節診斷應用中的預測。我們提出新的基於統計的 XAI 方法，即核密度估計和密度圖，來解釋未檢測到結節的情況。XAI 方法的性能被視為定性和定量比較，作為改進資料品質和模型性能的回饋。最後，我們調查評估醫生和患者對 XAI 解釋模型對甲狀腺結節影像決定的信任度。
...
摘要：醫療設備和人工智慧系統快速轉化醫療保健條款。同時，由於其本質，醫療設備中或作為醫療設備的人工智慧可能會遭受網路攻擊，導致患者安全和安全風險。本章節分為三部分。第一部分從設定場景開始，我們在其中說明網路安全在醫療保健中的角色。然後，我們簡要定義當我們談論被視為醫療設備本身或支援醫療設備的人工智慧時所指的內容。為了說明此類醫療設備帶來的風險，我們提供了三個範例：資料集中毒、社會工程以及資料或原始碼萃取。在第二部分，本文概述了歐盟的監管架構，與確保醫療設備中或作為醫療設備的人工智慧的網路安全相關（MDR、NIS 指令、網路安全法案、GDPR、AI 法案提案和 NIS 2 指令提案）。最後，本文的第三部分探討了源自歐盟監管架構的潛在挑戰。特別是，我們展望源自兩項立法提案的挑戰，以及它們與現有關於人工智慧醫療設備網路安全的立法之間的互動。它們被結構化為以下問題的答案：(1) 人工智慧法案將如何與 MDR 就網路安全和安全要求進行互動？(2) 我們應如何解釋 NIS 2 指令提案和 MDR 中的事件通知要求？(3) 關鍵基礎設施演變術語的後果是什麼？
[這是草稿章節。最終版本將出現在 Barry Solaiman 和 I. Glenn Cohen 編輯的《健康、人工智慧與法律研究手冊》中，即將於 2023 年由 Edward Elgar Publishing Ltd 出版]
...
摘要：阿茲海默症（AD）是一種進行性神經退化性疾病，也是導致失智症的主要原因。早期診斷對於患者從潛在的干預和治療中受益至關重要。由於視網膜與大腦有解剖學上的連接，因此假設視網膜是 AD 檢測的診斷部位。為此開發的 AI 模型尚未對決策提供合理的解釋，也沒有推斷出疾病進展的階段。沿著這個方向，我們提出了一個新的與模型無關的可解釋 AI 框架，稱為顆粒神經元級別解釋器 (LAVA)，這是一個解釋原型，可以探查卷積神經網路 (CNN) 模型的中間層，直接從視網膜影像評估 AD 連續體，而無需縱向或臨床評估。此方法用於驗證視網膜血管作為阿茲海默症 (AD) 評估的生物標記和診斷方式。英國生物資料庫的認知測試和血管形態特徵表明，LAVA 在識別整個進程連續體中的 AD 階段方面顯示出強大的前景和有效性。
...
摘要：許多可解釋的 AI (XAI) 已開發出視覺化，但它們通常需要使用者進一步推理才能解釋。我們主張 XAI 應支援圖解和演繹推理，以便 AI 執行假設產生和評估，以縮小可解釋性差距。我們提出圖表化以 i) 執行皮爾士演繹推理，ii) 遵循領域慣例，以及 iii) 以視覺或口頭方式用圖表說明。我們實作了 DiagramNet，用於臨床應用程式，以根據心臟聽診預測心臟診斷，並以基於形狀的雜音圖表說明。在建模研究中，我們發現 DiagramNet 不僅提供了真實的雜音形狀說明，而且比基準模型具有更好的預測效能。我們進一步在與醫學生的定性使用者研究中，展示了圖解說明的可解釋性和可信度，顯示出以臨床相關的圖解說明比技術顯著性圖說明更受青睞。這項工作有助於提供以使用者為中心的 XAI 的領域慣例演繹說明。
...
摘要：皮膚癌是人類最普遍的癌症類型之一。它的辨識主要是透過視覺，從臨床篩檢開始，接著是皮膚鏡檢查、組織病理學評估和檢體收集。深度卷積神經網路 (CNN) 會針對分類的細緻物件執行高度區隔且可能通用的任務。本研究提出一個新穎的多類別預測架構，它會根據 ViT 和 ViTGAN 對皮膚病灶進行分類。基於視覺Transformer的 GAN（生成對抗網路）用於處理類別不平衡。這個架構包含四個主要階段：ViTGAN、影像處理和可解釋 AI。階段 1 包含產生合成影像，以平衡資料集中的所有類別。階段 2 包含套用不同的資料擴充技術和形態運算，以增加資料的大小。階段 3 和 4 包含開發一個適用於邊緣運算系統的 ViT 模型，它可以識別模式，並從影像中可見的使用者皮膚中分類皮膚病灶。在階段 3，在使用 ViT 將病灶分類到想要的類別後，我們將使用可解釋 AI (XAI)，它會產生更可解釋的結果（使用啟動圖等），同時確保高度的預測準確度。皮膚疾病的即時影像可以用相機或行動應用程式由醫生或患者拍攝，以執行早期檢查並確定皮膚病灶的原因。整個架構會與現有的皮膚病灶偵測架構進行比較。
...
摘要：<paragraph>在高風險環境（例如醫療保健）中部署人工智慧 (AI) 時，提供可解釋性/說明性或允許精細錯誤分析的方法至關重要。許多最近的可解釋性/說明性和精細錯誤分析方法使用概念，這些概念是對人類具有語義意義的元標籤。然而，只有少數資料集包含概念層級的元標籤，而且這些元標籤大多與不需要領域專業知識的自然影像相關。專注於與單一疾病（例如黑色素瘤）相關的元標籤的醫學密集標註資料集。在皮膚科中，皮膚疾病使用既定的臨床詞彙來描述，讓臨床醫生可以彼此描述身體檢查結果。為了提供由領域專家密集標註的醫療資料集，其中標註對多種疾病過程有用，我們開發了 SkinCon：由皮膚科醫生密集標註的皮膚疾病資料集。SkinCon 包含來自 Fitzpatrick 17k 資料集的 3230 張影像，密集標註了 48 個臨床概念，其中 22 個至少有 50 張影像代表該概念。所使用的概念是由兩位皮膚科醫生在考慮用於描述皮膚病灶的臨床描述詞彙後選擇的。範例包括「斑塊」、「鱗屑」和「糜爛」。相同的概念也用於標註來自 Diverse Dermatology Images 資料集的 656 張皮膚疾病影像，提供了具有多樣化膚色的額外外部資料集。我們回顧了 SkinCon 資料集的潛在應用，例如探測模型、基於概念的說明和概念瓶頸。此外，我們使用 SkinCon 來展示這兩個使用案例：使用概念偵錯現有皮膚科 AI 模型的錯誤，以及使用事後概念瓶頸模型開發可解釋的模型。</paragraph>
...
摘要：不安分的多臂老虎機（RMAB）是一種流行的決策理論框架，已被用於模擬公共衛生、野生動物保育、通訊系統等領域的真實世界順序決策問題。已部署的 RMAB 系統通常分兩個階段運作：第一階段預測定義 RMAB 實例的未知參數，第二階段採用最佳化演算法來解決已建構的 RMAB 實例。在這項工作中，我們提供並分析了在公共衛生領域部署 RMAB 系統的首次同類型結果，旨在改善孕產婦和兒童健康。我們的分析重點在於了解預測準確度與已部署 RMAB 系統的整體效能之間的關係。這對於決定投資於改善預測準確度以提升最終系統效能的價值至關重要，而且有助於診斷、監控已部署的 RMAB 系統。使用我們已部署 RMAB 系統的真實世界資料，我們證明整體預測準確度的改善甚至可能伴隨著 RMAB 系統效能的下降——廣泛投資資源以改善整體預測準確度可能不會產生預期的結果。在此之後，我們開發了以決策為中心的評估指標來評估預測組成，並證明它更能（在經驗和理論上）說明已部署 RMAB 系統的整體效能。
...
摘要：可解釋 AI 包含開發機制，允許決策系統與人類互動，讓後者的決策變得容易理解。這在醫療領域等敏感情境中特別重要。我們提出一個使用案例研究，用於皮膚病灶診斷，說明如何為執業醫師提供有關最先進深度神經網路分類器決策的說明，該分類器經過訓練，可根據範例描述皮膚病灶。我們的架構包含一個訓練過的分類器，說明模組在該分類器上運作。後者能夠為分類診斷提供執業醫師範例和反例，因此允許醫師與自動診斷系統互動。範例是透過對抗自動編碼器產生的。我們說明了系統在代表性範例中的行為。
...
摘要：人工智慧（AI）應用中的演算法公平性對於建立一個更美好的社會至關重要。作為社會機制的基礎公理，公平性包含多個面向。儘管機器學習（ML）社群已將焦點放在交叉性作為統計均等性的問題上，特別是在歧視問題上，但新興的文獻探討了另一個面向——單調性。根據領域專家，單調性在許多與公平性相關的領域中扮演著至關重要的角色，其中違反單調性可能會誤導人類決策並導致災難性的後果。在本文中，我們首先系統性地評估應用單調神經加法模型（MNAM）的重要性，該模型使用公平感知的 ML 演算法來強制執行個人和成對的單調性原則，以確保 AI 倫理和社會的公平性。我們透過理論推理、模擬和廣泛的實證分析的混合方法發現，在所有公平領域（包括犯罪學、教育、醫療保健和金融）中，考量單調性公理至關重要。我們的研究有助於 AI 倫理、可解釋 AI（XAI）和人機互動（HCI）介面上的跨領域研究。透過證明不符合單調性的災難性後果，我們探討了單調性需求在 AI 應用中的重要性。此外，我們證明 MNAM 是一種有效的公平感知 ML 方法，透過施加單調性限制來整合人類智慧。
...
摘要：<paragraph>在安全关键型应用中，例如法律或医学领域，解释 AI 模型的预测至关重要。对预测的一种解释形式是提取基本原理，即实例的特征子集，该子集导致模型对该实例给出其预测。例如，子短语 ``他偷了手机'' 可能是 ``盗窃'' 预测的提取基本原理。以前关于生成提取基本原理的工作通常采用两阶段模型：选择器（选择最重要的特征（即基本原理））和预测器（仅基于所选特征进行预测）。这些工作的缺点之一是学习选择特征的主要信号来自将预测器给出的答案与真实答案进行比较。在这项工作中，我们建议通过信息校准方法从预测器中提取更多信息。更准确地说，我们联合训练两个模型：一个是解决手头任务的典型神经模型，以准确但黑盒的方式，另一个是选择器预测器模型，它还为其预测产生基本原理。第一个模型用作第二个模型的指导。我们使用对抗技术来校准两个模型提取的信息，使得它们之间的差异是遗漏或过度选择的特征的指示器。此外，对于自然语言任务，我们提出了一种基于语言模型的正则化器来鼓励提取流畅的基本原理。情感分析任务、仇恨言论识别任务以及法律领域三个任务的实验结果表明了我们基本原理提取方法的有效性。</paragraph>
...
摘要：近期獲認證的人工智慧 (AI) 工具在醫療保健領域的激增，讓圍繞著該技術採用所展開的辯論再度浮上檯面。其中一項辯論的主題是可解釋 AI (XAI) 及其讓 AI 裝置更具透明度和可信度的承諾。一些活躍於醫療 AI 領域的聲音表達了對可解釋 AI 技術的可靠性，特別是特徵歸因方法的疑慮，並質疑其在準則和標準中的使用與納入。儘管有合理的疑慮，我們主張，對事後局部可解釋性方法的可行性所提出的現有批評，將問題過度概括至特定於影像資料，因而捨棄了有用的部分。我們先將問題特徵化為解釋與人類理解之間缺乏語義匹配。為了了解特徵重要性何時能被可靠地使用，我們引入了低階與高階特徵特徵重要性的區別。我們主張，對於低階特徵具備明確語義的資料類型，例如電子健康紀錄 (EHR) 等表格資料，語義匹配是可以達成的，因此特徵歸因方法仍能以有意義且有用的方式被採用。最後，我們勾勒出一個程序來測試語義匹配是否已達成。
...
摘要：儘管人工智慧（AI）的機器學習（ML）模型在醫療領域取得高成效，但並非沒有錯誤。賦予臨床醫師辨識錯誤模型建議的能力對於建立對醫療 AI 的信任至關重要。可解釋 AI（XAI）旨在透過釐清 AI 推論來滿足此項需求，以協助最終使用者。最近，多項針對生物醫學影像的研究獲得了令人滿意的成果。然而，使用表格資料的模型的解決方案並不足以滿足臨床醫師的需求。本文提出了一種方法，以協助臨床醫師辨識使用表格資料訓練的 ML 模型的失敗。我們的做法建立在三個主要支柱上：透過運用臨床脈絡潛在空間分解特徵集、評估整體解釋的臨床關聯性，以及基於潛在空間相似性（LSS）的局部解釋。我們在由感染引起的早產兒疾病的 ML 辨識上展示了我們的做法。由於模型失敗而產生的死亡風險、終身殘疾和抗生素抗藥性是此領域中一個開放的研究問題。我們透過方法辨識出兩個模型的錯誤分類案例。透過將局部解釋脈絡化，我們的解決方案為臨床醫師提供可行的見解，以支持他們自主做出明智的最終決定。
...
摘要：<paragraph>我們正見證著一個 AI 經濟和社會的出現，其中 AI 技術正日益影響著醫療保健、商業、交通和日常生活的許多方面。許多成功案例表明，AI 系統甚至超越了人類專家的準確性。然而，AI 系統可能會產生錯誤，可能會表現出偏見，可能會對數據中的噪聲敏感，而且常常缺乏技術和司法透明度，從而導致信任度下降和採用它們時面臨挑戰。這些最近的缺點和擔憂已在科學和一般媒體中得到記錄，例如自動駕駛汽車事故、醫療保健中的偏見、有色人種的招聘和面部識別系統、看似正確的醫療決策後來被發現是出於錯誤的原因等。這導致出現許多政府和監管舉措，要求值得信賴且合乎道德的 AI 提供準確性和穩健性、某種形式的可解釋性、人類控制和監督、消除偏見、司法透明度和安全性。提供可信賴的 AI 系統時面臨的挑戰激勵了對可解釋 AI 系統 (XAI) 的深入研究。XAI 的目的是提供人類可以理解的資訊，說明 AI 系統如何做出決策。在本文中，我們首先簡要總結當前的 XAI 工作，然後質疑準確性與可解釋性相互排斥且僅專注於深度學習的最新論點。然後，我們提出我們的建議，以在高風險可信賴 AI 系統交付的整個生命週期中使用 XAI，例如開發、驗證和認證，以及可信賴的生產和維護。</paragraph>
...
摘要：深度神经網路 (DNN) 偵測資料中的模式，並已在許多電腦視覺應用中展現出多功能性和強大效能。然而，DNN 本身容易犯下違反簡單常識概念的明顯錯誤，並且在使用明確知識來引導其搜尋和決策制定方面受到限制。儘管整體 DNN 效能指標可能良好，但這些明顯的錯誤加上缺乏可解釋性，已阻止其廣泛採用於諸如醫學影像分析等關鍵任務。本文的目的是介紹 SimpleMind，一個專注於醫學影像理解的認知 AI 開源軟體架構。它允許建立一個知識庫，以直觀的人類可讀形式描述影像物件預期的特徵和關係。SimpleMind 架構透過以下方式為 DNN 帶來思考：(1) 提供方法以知識庫推論影像內容，例如空間推理和條件推理，以檢查 DNN 輸出；(2) 以通用軟體代理的形式應用程序知識，將其串連在一起以完成影像前處理、DNN 預測和結果後處理，以及 (3) 執行所有知識庫參數的自動共同最佳化，以適應代理到特定問題。SimpleMind 能夠對多個偵測到的物件進行推理以確保一致性，並提供 DNN 輸出之間的交叉檢查。這種機器推理透過可解釋模型和可解釋決策改善 DNN 的可靠性和可信度。提供的範例應用程式展示了 SimpleMind 如何透過將深度神經網路嵌入認知 AI 架構中來支援和改善它們。
...
摘要：深度學習方法在視覺感知任務上的進展在過去十年中取得驚人的成長，廣泛應用於從自動駕駛到臨床決策支援系統等眾多應用領域。儘管這些基於深度學習的模型效能令人印象深刻，但它們在決策過程中仍然相當不透明，這使得它們在人類關鍵任務中的部署成為一項風險極高的工作。這反過來又使得理解這些模型所做的決策對於它們的可靠部署至關重要。可解釋人工智慧 (XAI) 方法試圖通過為這些黑箱深度學習方法提供解釋來解決這個問題。在本文中，我們對電腦視覺中的基於歸因的 XAI 方法進行了全面的調查，並回顧了基於梯度、基於擾動和基於對比的 XAI 方法的現有文獻，並對開發和評估強大的 XAI 方法的關鍵挑戰提供了見解。
...
這些創新轉化為實際應用程式的速度落後於
這個快速的步伐。特別是安全關鍵應用程式，
依賴於嚴格的法規和道德要求，需要加以注意，
而且仍然是辯論的活躍領域。可解釋 AI (XAI) 和
隱私保護機器學習 (PPML) 都是重要的研究領域，
旨在減輕 DL 中普遍存在的資料密集黑盒模型的一些缺點。儘管
在各自的領域中進行了熱烈的研究活動，但尚未關注
它們的互動。這項工作是第一個研究
...
解釋的影響。研究結果表明，透過引入隱私，
解釋會產生不可忽視的變化。除了報告
PPML 對 XAI 的個別影響之外，本文還對
在實際應用中選擇技術提供了明確的建議。透過揭示
這些關鍵技術的相互依賴性，這項工作是
朝著克服安全關鍵領域中實用 AI 剩餘障礙邁出的第一步。
...
摘要：肺結核（TB）是一種傳染性細菌疾病，是造成死亡的重要原因，特別是在低收入國家，據估計在 2020 年全球報告了 1000 萬例新病例。儘管肺結核是可以治療的，但未遵守藥物治療方案是發病率和死亡率的重要原因。因此，主動找出有中斷用藥方案風險的患者，可以採取糾正措施以減輕不良後果。使用極端不遵守的代理測量和來自印度四個邦近 700,000 名患者的數據集，我們制定並解決了基於自訂排名指標的非遵守早期預測機器學習 (ML) 問題。我們訓練 ML 模型並針對基準進行評估，在考量全國大規模未來部署的情況下，比基於規則的基準高出約 100%，比隨機分類器高出約 214%。在這個過程中，我們處理了各種問題，包括資料品質、高基數分類資料、低目標發生率、分布轉移、不同群組之間的差異、演算法公平性，以及對穩健性和可解釋性的需求。我們的研究結果表明，不遵守治療的患者的風險分層是一種可行的、可大規模部署的 ML 解決方案。作為印度中央肺結核部門的官方 AI 合作夥伴，我們正在與多個城市和州級試點計畫合作，目標是全印度部署。
...
摘要：可解釋人工智慧 (XAI) 透過提升使用者對機器信任度，轉化人工智慧 (AI) 領域。隨著連網裝置數量持續增加，物聯網 (IoT) 市場所需具備使用者信任度。然而，現有文獻仍缺乏針對物聯網使用 XAI 的系統化且全面的調查研究。為彌補此一缺失，本文探討 XAI 架構，並專注於其特性和對物聯網的支援。我們說明廣泛使用的物聯網應用 XAI 服務，例如安全性強化、醫療物聯網 (IoMT)、工業物聯網 (IIoT) 和城市物聯網 (IoCT)。我們也建議在這些應用中透過適當範例，選擇 XAI 模型在物聯網系統上的實作，並總結未來研究的主要推論。此外，我們提出邊緣 XAI 架構的尖端發展，以及第六代 (6G) 通訊服務對物聯網應用支援，並附上主要推論。簡而言之，本文構成針對未來物聯網使用案例需求量身打造的 XAI 基礎架構發展之首份整體彙編。
...
摘要：每年约有 125 万美国人因烧伤接受治疗。精确的烧伤分类是医疗人工智能领域的一个重要方面。在这项工作中，我们提出一个可解释的人在回路框架，用于改进烧伤超声分类模型。我们的框架利用基于 LIME 分类解释器的解释系统来证实和整合烧伤专家的知识——提出新特征并确保模型的有效性。使用此框架，我们发现 B 型模式超声分类器可以通过提供纹理特征来增强。更具体地说，我们确认基于超声帧的灰度共生矩阵 (GLCM) 的纹理特征可以提高深度学习烧伤深度分类器的准确性。我们在猪受试者的真实数据上测试了我们的假设。我们展示了烧伤深度分类的准确性改进——从约 88% 提高到约 94%——一旦根据我们的框架进行修改。
...
摘要：在醫療保健中，AI 系統支援臨床醫生和患者進行診斷、治療和監控，但許多系統的解釋性不足，對實際應用仍然構成挑戰。克服這個障礙是可解釋 AI (XAI) 的目標。然而，解釋可以被不同地感知，因此無法為所有人解決黑盒問題。以人為中心的 AI 領域通過將 AI 適應用戶來解決這個問題。我們提出了一個以用戶為中心的角色概念來評估 XAI，並使用它來調查行動健康壓力監控應用程式中各種解釋樣式和內容的最終使用者偏好。我們的線上調查結果顯示，使用者的年齡層和個性，以及解釋類型，會影響解釋偏好，這表示這些是 XAI 設計的基本特徵。我們將結果歸納為三個原型使用者角色：權力型、休閒型和注重隱私的使用者。我們的見解使互動式、以人為中心的 XAI 更接近實際應用。
...
摘要：青光眼是所有視力缺陷中導致部分或完全失明的第二大原因，主要發生在因焦慮或憂鬱症導致眼壓過高，進而損害視神經並造成視力併發症。傳統的青光眼篩檢是一個耗時的過程，需要醫療專業人員持續關注，即使如此，由於時間限制和壓力，他們仍時常無法正確分類，導致治療錯誤。已做出許多努力來自動化整個青光眼分類程序，然而，這些現有模型通常具有黑盒特性，使用戶無法了解預測背後的主要原因，因此醫療從業人員通常無法依賴這些系統。在本文中，我們在比較了各種預先訓練的模型後，提出了一個轉移學習模型，能夠以 94.71% 的準確率對青光眼進行分類。此外，我們利用了局部的可解釋模型不可知解釋 (LIME)，在我們的系統中引入了可解釋性。此項改進使醫療專業人員能夠獲得重要且全面的資訊，以協助他們做出判斷。它也降低了傳統深度學習模型的不透明性和脆弱性。
...
摘要：深度學習在偵測物體和分類影像方面展現出絕佳的效能，為分析醫學影像帶來極大的希望。要將深度學習的成功轉移到醫學影像，其中醫師需要了解其背後機制，就必須具備解釋和說明神經網路預測結果的能力。深度神經網路的可解釋性通常仰賴估計輸入特徵（例如像素）對於結果（例如類別機率）的重要性。然而，已經開發出許多重要性估計器（也稱為顯著度圖），但目前尚不清楚哪一種對於醫學影像應用較為相關。在目前的研究中，我們使用三種不同的評估指標，探討了多種重要性估計器在解釋卷積深度網路對電腦斷層掃描（CT）影像進行分類的效能。首先，以模型為中心的保真度衡量在特定輸入受到擾動時，模型精確度的下降程度。其次，在像素層級上，使用受試者工作特性（ROC）曲線衡量重要性評分與專家定義的分割遮罩之間的一致性。第三，我們使用 Dice 相似性係數（DSC）衡量基於 XRAI 的圖與分割遮罩之間的區域重疊程度。整體而言，兩個版本的 SmoothGrad 在保真度和 ROC 排名中名列前茅，而積分梯度和 SmoothGrad 則在 DSC 評估中表現出色。有趣的是，以模型為中心（保真度）和以人為中心（ROC 和 DSC）的評估之間存在著重大的差異。分割遮罩中蘊含的專家期望和直覺並不一定與模型得出預測結果的方式一致。了解可解釋性中的這種差異將有助於利用深度學習在醫學上的力量。
...
摘要：深度學習方法，特別是卷積神經網路，已成為醫學影像運算任務中強大的工具。雖然這些複雜的模型提供了極佳的效能，但其黑箱性質可能會阻礙在高風險決策中採用於現實世界。在本文中，我們提出一個互動式系統來利用最先進的可解釋性技術，以協助放射科醫師進行乳癌篩檢。我們的系統將深度學習模型整合到放射科醫師的工作流程中，並提供新穎的互動方式，以促進對模型決策過程的理解。此外，我們證明我們的系統可以逐漸利用使用者的互動，以提供更細緻的可解釋性報告，而標記工作量卻很小。由於所採用的可解釋性技術的通用性質，我們的系統與領域無關，可用於許多不同的醫學影像運算任務，提供了一個新穎的觀點，說明我們如何利用視覺分析將原本靜態的可解釋性技術轉換為擴充人類決策制定並促進採用醫療 AI。
...
摘要：近年來，人工智慧（AI）已成為醫療保健應用程式中不可或缺的組成部分，無論是臨床或遠距醫療，但效能最佳的 AI 系統往往過於複雜，無法自行解釋。可解釋 AI（XAI）技術被定義為揭示系統預測和決策背後的推理，在處理敏感且個人的健康資料時，它們變得更加重要。值得注意的是，XAI 尚未在不同的研究領域和資料類型中獲得相同的關注，特別是在醫療保健方面。特別是，許多臨床和遠距醫療應用程式分別基於表格和時間序列資料，而 XAI 通常不會在這些資料類型上進行分析，而電腦視覺和自然語言處理（NLP）則是參考應用程式。為了概述最適合醫療保健領域中表格和時間序列資料的 XAI 方法，本文回顧了過去 5 年的文獻，說明所產生的解釋類型以及評估其相關性和品質所做的努力。具體來說，我們將臨床驗證、一致性評估、客觀且標準化的品質評估，以及以人為中心的品質評估，視為確保對最終使用者提供有效解釋的主要特徵。最後，我們強調了該領域的主要研究挑戰，以及現有 XAI 方法的限制。
...
摘要：<paragraph>目的：本研究評估了一種市售的可解釋 AI 演算法對增強臨床醫師在胸部 X 光片 (CXR) 中辨識肺癌的能力的影響。
設計：這項回顧性研究評估了 11 位臨床醫師在有和沒有市售 AI 演算法（紅點，Behold.ai）協助下從胸部 X 光片中偵測肺癌的表現，該演算法會預測 CXR 中疑似肺癌。臨床醫師的表現是根據臨床確診的診斷進行評估。
背景：這項研究分析了來自 NHS 醫院的匿名患者資料；該資料集包含 400 張在 2020 年接受 CXR 的成年患者（18 歲及以上）的胸部 X 光片，以及對應的臨床文字報告。
參與者：由 11 位臨床醫師（顧問放射科醫師、放射科醫師受訓人員和報告放射技師）組成的讀者小組參與了這項研究。
主要結果指標：臨床醫師在有和沒有 AI 輸入的情況下，在 CXR 中偵測肺癌的整體準確度、敏感度、特異度和精確度。臨床醫師之間的一致性比率和執行標準差也在有和沒有 AI 輸入的情況下進行評估。
結果：臨床醫師使用 AI 演算法可改善肺腫瘤偵測的整體表現，整體增加 17.4% 的肺癌在 CXR 中被辨識出來，否則這些肺癌將會被遺漏，整體增加較小腫瘤的偵測率，分別增加 24% 和 13% 的第一期和第二期肺癌的偵測率，並標準化臨床醫師的表現。
結論：這項研究顯示 AI 演算法在改善早期肺癌診斷和透過整體改善讀者表現來促進健康公平性方面具有極大的臨床應用前景，同時不會影響下游影像資源。</paragraph>
...
摘要：缺乏足够大的开放医疗数据库是人工智能驱动的医疗保健中最大的挑战之一。使用生成对抗网络 (GAN) 创建的合成数据似乎是缓解隐私政策问题的良好解决方案。另一种治疗方法是在多个医疗机构之间分散协议，而无需交换本地数据样本。在本文中，我们探索了集中和分散设置中的无条件和条件 GAN。集中设置模仿了对大但高度不平衡的皮肤损伤数据集的研究，而分散设置模拟了三个机构的更现实的医院场景。我们根据保真度、多样性、训练速度以及在生成合成数据上训练的分类器的预测能力评估了模型的性能。此外，我们通过探索潜在空间和嵌入投影提供了可解释性，重点关注全局和局部解释。计算实图像与其在潜在空间中的投影之间的距离证明了训练过的 GAN 的真实性和泛化性，这是此类应用程序中的主要关注点之一。进行的研究的开源代码可在
\url{https://github.com/aidotse/stylegan2-ada-pytorch} 公开获得。
...
摘要：在醫院的世界裡有許多複雜的組合問題，解決這些問題對於提升病患滿意度和照護品質非常重要。醫療保健中的問題很複雜，因為要解決它們必須考慮到許多限制和不同類型的資源。此外，必須在短時間內評估解決方案，以確保在實際情況中能使用。我們計畫為這類問題提出解決方案，包括擴充已測試的解決方案，以及透過建模為新問題找出解決方案，同時考量文獻資料並在有資料時使用真實資料。解決這類問題很重要，但由於歐盟執委會制定了《一般資料保護規範》，規定每個人都有權要求說明 AI 做出的決定，因此在未發展出可解釋性方法之前，基於 AI 的解決方案（例如基於答案集程式設計的解決方案）的使用將受到限制。因此，研究的另一部分將致力於研究並提出新的方法來解釋所獲得的解決方案。
...
摘要：在現代動態不斷發展的社會中，越來越多的人罹患慢性且嚴重的疾病，而醫生和患者需要特殊且精密的醫療保健支持。因此，傑出的醫療保健利益相關者已意識到開發此類服務的重要性，以讓患者的生活更輕鬆。此類支持需要收集大量的患者複雜資料，例如臨床、環境、營養、日常活動、來自智慧穿戴式裝置的各種資料、來自配備感測器的服裝的資料等。整體患者資料必須適當地彙總、處理、分析並提供給醫生和照護者，以建議適當的治療和措施，以改善患者的健康相關參數和整體健康狀況。進階的人工智慧技術提供了分析此類大資料、使用它們並衍生新知識以支持個人化醫療決策的機會。基於進階機器學習、聯邦學習、遷移學習、可解釋人工智慧等新方法為未來更優質地使用健康和醫療資料開闢了新途徑。在本文中，我們將提出一些關鍵面向和特徵範例，說明人工智慧方法在個人化醫療決策中的應用領域。
...
摘要：<paragraph>持續監控已訓練的機器學習模型，以確定何時應該信任其預測，以及何時不應信任其預測，對於安全部署模型至關重要。此類框架應具有高性能、可解釋性、事後性和可操作性。我們提出 TRUST-LAPSE，這是一個用於持續模型監控的「不信任」評分框架。我們使用一系列潛在空間嵌入來評估每個輸入樣本的模型預測的可信度。具體來說，(a) 我們的潛在空間不信任分數使用潛在空間中的距離度量（馬氏距離）和相似度量（餘弦相似度）來估計不信任，(b) 我們的順序不信任分數確定在非參數、基於滑動視窗演算法中的過去輸入表示序列上的相關性偏差，以進行可操作的持續監控。我們透過兩項下游任務來評估 TRUST-LAPSE：(1) 分布式位移輸入偵測，以及 (2) 資料漂移偵測。我們在不同的領域（音訊和視覺）中使用公開資料集進行評估，並進一步在具有挑戰性的真實世界腦電圖 (EEG) 資料集上對我們的做法進行基準測試，以進行癲癇偵測。我們的潛在空間不信任分數取得了最先進的結果，AUROC 為 84.1（視覺）、73.9（音訊）和 77.1（臨床 EEG），優於基準線超過 10 個百分點。我們揭露了流行基準線中的嚴重失敗，這些基準線對輸入語義內容不敏感，使其不適合於真實世界的模型監控。我們表明，我們的順序不信任分數達到了很高的漂移偵測率；所有領域中超過 90% 的串流顯示小於 20% 的誤差。透過廣泛的定性和定量評估，我們表明我們的信任分數更強大，並且提供可解釋性，以便於實務採用。</paragraph>
...
摘要：機器學習的普及增加了不公平模型在高風險應用中部署的風險，例如司法系統、藥物/疫苗設計和醫療診斷。儘管有有效的方法可以從頭訓練公平的模型，但如何自動揭示和解釋訓練模型的不公平性仍然是一項具有挑戰性的任務。以可解釋的方式揭示機器學習模型的不公平性是邁向公平且值得信賴的人工智慧的關鍵一步。在本文中，我們系統性地解決了通過挖掘可解釋證據 (RUMIE) 來揭示不公平模型的新任務。關鍵思想是以一群模型最歧視的資料實例的形式找到確鑿證據。為了使證據可解釋，我們還找到了一組人類可以理解的關鍵屬性和決策規則，這些屬性和決策規則的特徵在於被歧視的資料實例，並將它們與其他未被歧視的資料區分開來。正如在許多真實世界資料集上的大量實驗所證明的那樣，我們的模型找到了高度可解釋且確鑿的證據來有效揭示訓練模型的不公平性。此外，它比所有基線方法的可擴充性都要高得多。
...
摘要：解釋性人工智慧（XAI）在安全關鍵系統中是必要的，例如在臨床診斷中，因為有致命的決策風險。然而，目前 XAI 較像是一個方法的鬆散集合，而不是一個定義良好的程序。在這項工作中，我們詳細說明了 XAI 中最大的子群解釋性機器學習（IML）與古典統計之間的概念相似性。基於這些相似性，我們提出了沿著統計程序的 IML 形式化。採用這種統計觀點使我們能夠將機器學習模型和 IML 方法解釋為複雜的統計工具。基於這種解釋，我們推論出三個關鍵問題，我們將其視為 IML 在安全關鍵環境中成功和採用的關鍵。透過制定這些問題，我們進一步旨在引發一場關於 IML 與古典統計有何區別的討論，以及我們的觀點對該領域的未來有何影響。
...
摘要：人工智慧已無所不在於各學科和領域，
生物醫學影像和訊號處理也不例外。對此主題日益增長且廣泛的興趣已觸發了龐大的研究活動，這反映在指數級的研究工作中。透過研究大量且多樣的生物醫學數據，機器和深度學習模型已徹底改變了建模、分割、配準、分類和合成等各種任務，其表現優於傳統技術。然而，將結果轉化為生物學/臨床可解釋資訊的難度，阻礙了它們在該領域的充分利用。可解釋 AI (XAI) 嘗試透過提供讓模型可解釋並提供說明的方法，來填補這種轉譯差距。迄今為止已提出不同的解決方案，並越來越受到社群的關注。本文旨在提供生物醫學數據處理中 XAI 的概觀，並指出 IEEE 訊號處理雜誌即將於 2022 年 3 月發表的生物醫學影像和訊號處理深度學習特刊。
...
摘要：在使用醫療影像進行診斷時，無論是臨床醫師或人工智慧 (AI) 系統，影像品質都很重要。當影像品質不佳時，產生影像的醫療檢查通常需要重新執行。遠距醫療中常見的問題是，只有在患者離開診所後才會標示品質問題，這表示他們必須回來重新進行檢查。這對居住在偏遠地區的人來說尤其困難，而這些人占 Portal Telemedicina 的患者很大一部分，Portal Telemedicina 是一家位於巴西的數位醫療保健組織。在本文中，我們報告關於以下事項的進行中工作：(i) 開發一種 AI 系統，用於標示和解釋低品質醫療影像，並提供即時說明，(ii) 一項訪談研究，以了解在 OurCompany 使用 AI 系統的利害關係人對說明的需求，以及 (iii) 一項縱向使用者研究設計，用於檢查在我們的診所中，將說明納入技術人員工作流程的影響。據我們所知，這將是第一個針對評估 XAI 方法對最終使用者（使用 AI 系統但沒有特定 AI 專業知識的利害關係人）的影響的縱向研究。我們歡迎對我們的實驗設定提供回饋和建議。
