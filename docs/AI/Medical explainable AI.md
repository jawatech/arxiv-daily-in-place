...
摘要：<paragraph>近年來 AI 的突破性進展，有望從根本上提升我們對醫療保健的研究和理解。開發整合式多對多架構，利用多重資料模式輸入，進行多重醫療任務的分析建模，對於統一理解現代醫學至關重要。在這項工作中，我們介紹 M3H，一個可解釋的多模式多任務機器學習醫療保健架構，它整合了廣泛醫療任務類別和機器學習問題類別中來自不同多模式輸入的學習成果。這個架構的模組化設計確保了資料處理、任務定義和快速模型建構的可概化性，適用於臨床和營運醫療保健環境。我們透過驗證從四種模式（表格、時間序列、語言和視覺）訓練的模型，在四個機器學習問題類別中執行 41 項醫療任務，來評估 M3H 架構。我們的結果顯示，M3H 持續產生多任務模型，在來自 16 個醫療部門的 37 種疾病診斷、三個醫院營運預測和一個患者表型分析任務中，表現優於標準的單一任務模型（1.1- 37.2%）。涵蓋了機器學習問題類別的監督二元分類、多類分類、回歸和聚類。此外，這個架構引入了一種新穎的注意力機制，用於平衡自我開發（專注於學習來源任務）和交叉探索（鼓勵從其他任務中學習）。此外，M3H 提供了可解釋性的見解，說明額外任務的聯合學習如何影響來源任務的學習，使用建議的 TIM 分數，揭示任務相互依賴的動態。其適應性架構促进了定制和集成，使其成为未来 AI 驱动的医疗保健系统的强大且可扩展的候选解决方案。</paragraph>
...
摘要：本文探討了應用群體結構多代理系統 (MAS) 來建立醫療必要性，這是一個涉及系統性審查患者特定醫療結構化和非結構化資料，並對照臨床指南的過程。我們透過將此複雜任務分解成較小、更易於管理的子任務來解決這個問題。每個子任務都由一個專門的 AI 代理處理。我們對各種提示策略對這些代理的影響進行系統性研究，並對不同的大型語言模型 (LLM) 進行基準測試，以確定它們完成這些任務的準確性。此外，我們探討了這些代理如何提供可解釋性，從而增強系統內的信任和透明度。
...
摘要：人工智慧 (AI) 的普遍整合在涉及 AI 啟用系統的事故中引入了責任和責任歸屬的複雜挑戰。這些系統的互連性、AI 引發事故的道德考量，加上 AI 技術的不確定性以及相應法規的缺失，使得傳統的責任歸屬具有挑戰性。為此，本研究提出了一種計算反射平衡 (CRE) 方法，以建立一個對所有利害關係人而言既一致又合乎道德的責任歸屬架構。計算方法提供了一種結構化的分析，克服了概念方法在處理動態和多面向情境時的限制，展示了架構在責任歸屬過程中可解釋性、一致性和適應性屬性。我們探討了與平衡計算中索賠相關的初始啟動層級的關鍵作用。使用 AI 輔助醫療決策支援系統作為案例研究，我們說明了不同的初始化如何導致不同的責任分配。該架構提供了對 AI 引發事故中問責制的寶貴見解，透過持續監控、修正和反思，促進永續且具復原力的系統開發。
...
摘要：人工智慧透過預測模型協助醫療保健專業人員，大幅轉變臨床決策制定。本研究探討在醫療保健中採用人工智慧應用時，公平性和可解釋性的重要需求，以確保不同病患人口統計資料能獲得公平的結果。我們專注於敗血症相關死亡率的預測模型，提出一個方法，可以學習效能最佳化的預測模型，然後採用遷移學習程序來產生一個具有更佳公平性的模型。我們的這個方法也引進一種新的基於置換的特性重要性演算法，目標在於闡明每個特性在增強預測公平性方面的貢獻。我們的建議方法與現有的可解釋性方法不同，現有的方法專注於解釋特性對預測效能的貢獻，而我們的建議方法獨特地彌補了理解每個特性如何對公平性做出貢獻的鴻溝。這項進展至關重要，因為敗血症的死亡率很高，且在三分之一的醫院死亡案例中扮演了角色。我們的這個方法不僅有助於識別和減輕預測模型中的偏誤，也透過改善模型預測的透明度和公平性來促進醫療保健利害關係人之間的信任，進而為更公平且值得信賴的醫療保健提供做出貢獻。
...
摘要：<paragraph>憂鬱症是現今社會中重大的議題。根據世界衛生組織（WHO）的統計，2023 年有超過 2.8 億人飽受憂鬱症所苦。這是一個龐大的數字，若不加以重視，這個數字將會快速增加。約有 48.9 億人使用社群媒體。人們在 Twitter、Facebook、Reddit、Instagram 等平台上表達自己的感受和情緒。這些平台包含有價值的資訊，可用於研究目的。在各種社群媒體平台上進行了大量的研究。然而，在這些努力中仍然存在某些限制。特別是，先前的研究僅專注於偵測推文中的憂鬱症和憂鬱症的嚴重程度。此外，資料集標籤中也存在不準確的情況。在這項研究工作中，根據語彙標籤，使用來自 Twitter 資料庫的推文預測了五種類型的憂鬱症（躁鬱症、重度憂鬱症、精神病性憂鬱症、非典型憂鬱症和產後憂鬱症）。可解釋的 AI 用於提供推理，重點說明推文中代表憂鬱症類型的部分。使用來自 Transformer 的雙向編碼器表示（BERT）進行特徵萃取和訓練。使用機器學習和深度學習方法來訓練模型。BERT 模型呈現出最有希望的結果，整體準確率達到 0.96。</paragraph>
...
摘要：深度學習正在徹底轉變醫學影像和放射學領域，能夠識別醫學影像中的病理，包括電腦斷層掃描 (CT) 和 X 光掃描。然而，深度學習模型的效能，特別是在分割任務中，經常受到廣泛註解資料集需求的限制。為了應對這項挑戰，透過可解釋 AI 和反事實解釋的產生，探索了弱監督語意分割的能力。這項研究的範圍是開發一種新穎的反事實內插方法 (COIN)，透過使用生成模型將預測分類標籤從異常翻轉為正常。例如，如果分類器將輸入醫學影像 X 視為異常，表示有病理存在，生成模型旨在內插異常區域，從而逆轉分類器的原始預測標籤。這種方法使我們能夠產生病理的精確分割，而不需要依賴於預先存在的分割遮罩。至關重要的是，利用影像層級標籤，這比建立詳細的分割遮罩容易取得得多。該方法的有效性透過分割愛沙尼亞塔爾圖大學醫院取得的 CT 影像中的合成目標和實際腎臟腫瘤來證明。研究結果表明，COIN 遠遠優於已建立的歸因方法，例如 RISE、ScoreCAM 和 LayerCAM，以及 Singla 等人提出的替代反事實解釋方法。這些證據表明，COIN 是 CT 影像中腫瘤語意分割的一種有前途的方法，並在醫療保健中讓深度學習應用程式更容易使用和更有效邁進了一步，其中註解資料很稀少。
...
摘要：甲狀腺癌是一種日益嚴重的全球健康問題，需要先進的診斷方法。本篇評論探討了人工智慧與放射特徵在甲狀腺癌診斷中的應用。在符合 PRISMA 指南的情況下，對多個資料庫進行了回顧，時間截至 2023 年 10 月。結合多個關鍵字，發現了與甲狀腺癌及相關主題相關的英文學術出版品。在移除 109 個重複項後，原始搜尋結果回傳了 267 篇論文。在根據預先設定的標準審查了摘要和標題後，淘汰了 124 篇文章，並根據標準挑選出相關研究。在進行全面分析後，另外排除了六項研究。在納入的 28 項研究中，結合超音波 (US) 影像的放射特徵分析證明了其在診斷甲狀腺癌方面的有效性。研究結果不一，有些研究提出了優於現狀的新策略。文獻強調了人工智慧模型面臨的各種挑戰，包括可解釋性問題、資料集限制和操作員依賴性。28 項納入研究的綜合發現提到，需要標準化工作和前瞻性多中心研究來解決這些問題。此外，還找出克服這些障礙的方法，例如可解釋人工智慧技術和個人化醫療技術的進步。本篇評論重點在於人工智慧和放射特徵如何轉化甲狀腺癌的診斷和治療。儘管有挑戰，但未來對跨領域合作、臨床適用性驗證和演算法改進的研究，有潛力改善甲狀腺癌治療的患者預後和診斷精準度。
...
摘要：近年來，乳癌盛行率迅速增加，使其成為全球主要的死亡原因之一。在所有癌症中，乳癌迄今為止最為常見。手動診斷此疾病需要大量的時間和專業知識。由於乳癌檢測是一個耗時的過程，因此透過建立機器預測可以協助防止其進一步擴散。機器學習和可解釋 AI 在分類中至關重要，因為它們不僅提供準確的預測，還能洞悉模型如何做出決策，有助於理解和信賴分類結果。在本研究中，我們使用主要資料集（達卡醫學院醫院的 500 名患者）評估和比較五種不同機器學習方法的分類準確度、精確度、召回率和 F-1 分數。五種不同的監督式機器學習技術，包括決策樹、隨機森林、邏輯迴歸、樸素貝氏和 XGBoost，已用於在我們的資料集上取得最佳結果。此外，本研究將 SHAP 分析應用於 XGBoost 模型，以解釋模型的預測並了解每個特徵對模型輸出的影響。我們比較了幾種演算法對資料進行分類的準確度，並與該領域的其他文獻進行對比。經過最後評估，本研究發現 XGBoost 達到了最佳模型準確度，為 97%。
...
摘要：深度學習 (DL) 用於從乳房攝影
圖像診斷乳癌的模型通常以「黑盒子」運作，這使得醫療保健
專業人員難以信任和理解他們的決策過程。本
研究提出一個整合架構，結合卷積神經網路
(CNN) 和可解釋人工智慧 (XAI)，以使用 CBIS-DDSM 資料集增強
乳癌的診斷。方法包含一個精細的資料前處理管線和進階資料擴充技術
來對抗資料集限制，並採用預先訓練的網路（如 VGG-16、Inception-V3 和 ResNet）進行遷移學習。我們
研究的一個重點是評估 XAI 在解釋模型
預測中的有效性，重點是利用豪斯多夫測度來評估 AI 生成的解釋和專家註解
之間的對齊程度。這種方法對於 XAI 在促進 AI 輔助診斷中的可信度
和道德公平性至關重要。我們研究的發現說明了 CNN 和 XAI 在推進
乳癌診斷方法中的有效協作，從而促進了先進 AI 技術在臨床環境中的更順暢
整合。透過增強 AI 驅動決策的可解釋性，這項工作為 AI 系統和醫療從業人員之間
的更佳協作奠定了基礎，最終豐富了患者照護。此外，我們研究的影響力遠遠超出了
目前的技術。它鼓勵進一步研究如何結合多模式資料並改善 AI 解釋以滿足
臨床實務的需求。
...
摘要：以人为本的可解释 AI（HCXAI）倡导将社会面向整合到 AI 解释中。HCXAI 话语的核心是社会透明度 (ST) 框架，旨在让 AI 系统的社会组织背景对用户来说触手可及。在这项工作中，我们建议扩展 ST 框架以解决大型语言模型 (LLM) 中社会错误归因的风险，特别是在心理健康等敏感领域。事实上，LLM 具有惊人的角色和人物模拟能力，可能导致设计者的意图与用户对社会属性的认知之间出现不匹配，从而有引发情感操纵和危险行为、认知不公正和不合理信任的风险。为了解决这些问题，我们建议用第五个“W 问题”来增强 ST 框架，以澄清设计者和用户分配给 LLM 的具体社会归因。这一补充旨在弥合 LLM 能力与用户认知之间的差距，促进基于 LLM 的技术的道德负责开发和使用。
...
摘要：<paragraph>背景：氣胸是一種因肺部和胸壁之間異常的空氣聚集而引起的急性胸腔疾病。為了解決深度學習 (DL) 模型經常出現的不透明性，已導入可解釋人工智慧 (XAI) 方法，以概述與 DL 模型所做的氣胸診斷相關的區域。然而，這些解釋有時會與實際病灶區域有所出入，這凸顯了進一步改進的必要性。方法：我們提出一個模板引導方法，將氣胸的臨床知識納入 XAI 方法產生的模型解釋中，從而提升這些解釋的品質。利用放射科醫師建立的一個病灶描繪，我們的做法首先產生一個模板，表示氣胸可能發生的區域。這個模板接著會疊加在模型解釋上，以濾除超出模板邊界的無關解釋。為了驗證其效能，我們對三個 XAI 方法進行了比較分析，在兩個真實世界資料集中解釋兩個 DL 模型時，分別採用和不採用我們的模板引導。結果：所提出的方法在建立於三個 XAI 方法、兩個 DL 模型和兩個資料集的十二個基準情境中，始終優於基準 XAI 方法。在比較模型解釋和真實病灶區域時，透過基準效能的效能提升計算出的平均增量百分比，在聯合交集 (IoU) 中為 97.8%，在 Dice 相似性係數 (DSC) 中為 94.1%。結論：在氣胸診斷的背景下，我們提出了一個模板引導方法，用於改善 AI 解釋。我們預期我們的模板引導將透過整合臨床領域專業知識，為闡明 AI 模型建立一種新的方法。</paragraph>
...
摘要：<paragraph>在當前的機器翻譯 (MT) 領域中，Transformer 架構脫穎而出，成為黃金標準，特別是對於高資源語言對。本研究深入探討其對低資源語言對的功效，包括英語↔愛爾蘭語和英語↔馬拉提語語言對。值得注意的是，該研究確定了最佳超參數和子詞模型類型，以顯著提升 Transformer 模型對低資源語言對的翻譯品質。
低資源語言的平行資料集的稀缺性可能會阻礙機器翻譯的發展。為了解決這個問題，開發了 gaHealth，這是愛爾蘭語的第一個雙語健康資料語料庫。專注於健康領域，使用這個領域內資料集開發的模型在 BLEU 分數方面表現出非常顯著的進步，與 LoResMT2021 共享任務的模型相比。隨後使用多維品質指標錯誤分類法進行的人工評估，展示了 Transformer 系統在減少準確性和流暢性錯誤方面的優異性能，與基於 RNN 的對應系統相比。
此外，本論文介紹了 adaptNMT 和 adaptMLLM，這兩個開放原始碼應用程式簡化了神經機器翻譯模型的開發、微調和部署。這些工具大幅簡化了設定和評估流程，讓機器翻譯更容易讓開發人員和翻譯人員使用。值得注意的是，adaptNMT 以 OpenNMT 生態系統為基礎，通過強調模型開發的環境足跡，促進環保的自然語言處理研究。adaptMLLM 對 MLLM 的微調證明了翻譯性能的進步，針對兩個低資源語言對：英語↔愛爾蘭語和英語↔馬拉提語，與 LoResMT2021 共享任務的基準線相比。</paragraph>
...
摘要：糖尿病 (DM) 會導致患者出現血管併發症。
視網膜影像和血管反映身體的微血管和巨血管健康狀況。可透過這些影像和血管診斷糖尿病併發症，包括糖尿病視網膜病變 (DR)、神經病變、腎病變和動脈粥樣硬化性心血管疾病，以及預測心血管事件的風險。使用數位化視網膜影像進行高通量 DR 偵測的人工智慧 (AI) 系統已經在臨床上採用。除了 DR 篩檢外，AI 整合也極具潛力，可解決與糖尿病患者整體照護相關的挑戰。在這項研究中，我們旨在全面檢視與糖尿病診斷、預後和管理相關的視網膜影像 AI 應用研究文獻。我們將說明整體 AI 輔助糖尿病照護的發現，包括但不限於 DR 篩檢，並討論實施此類系統的障礙，包括倫理、資料隱私、公平存取和可解釋性等問題。透過評估患者的健康狀態，並預測未來心血管併發症的風險，AI 輔助視網膜影像分析有潛力成為糖尿病患者現代化個人化醫療的主要工具。
...
摘要：本研究從多個利益相關者的角度探討不同人工智慧 (AI) 應用在教育上的可接受性，包括學生、老師和家長。承認 AI 在教育上的轉型潛力，它解決了與資料隱私、AI 代理、透明度、可解釋性和 AI 的道德部署相關的疑慮。透過小插曲方法，參與者被呈現四種場景，其中 AI 的代理、透明度、可解釋性和隱私受到操縱。在每個場景之後，參與者完成了一項調查，調查捕捉了他們對 AI 的整體效用、個人效用、公正性、信心、風險以及在有機會的情況下使用每個場景的 AI 的意圖的看法。收集的資料包括 1198 位多利益相關者參與者的最終樣本，透過合作夥伴機構和社群媒體活動進行分發，並專注於對四個 AI 使用案例的個別回應。對資料的調解分析表明，對 AI 的接受度和信任在利益相關者群體之間有顯著差異。我們發現，AI 代理、透明度和可解釋性的高低層級之間的關鍵調解因素，以及使用不同教育 AI 的意圖，包括感知的整體效用、公正性和信心。該研究強調，在教育中接受 AI 是一個微妙且多方面的問題，需要仔細考慮具體的 AI 應用及其特徵，以及不同的利益相關者的看法。
...
摘要：基於可穿戴單導程心電圖 (ECG) 設備的遠程患者監測，對於早期偵測心臟疾病具有顯著的潛力，特別是結合人工智慧 (AI) 方法進行自動化心臟疾病偵測時。過去已有研究應用基於深度學習的 AI 方法進行心臟疾病偵測。然而，由於目前圍繞許多 AI 演算法的黑箱觀念，這些模型尚未廣泛被接受為臨床診斷的可靠輔助工具。特別需要找出有助於做出準確診斷的 ECG 訊號關鍵特徵，進而提升模型的可解釋性。在目前的研究中，我們開發一種視覺轉換器方法，根據單導程 ECG 資料辨識心房顫動。我們也開發一種殘差網路 (ResNet) 方法，與視覺轉換器方法進行比較。這些模型應用於 Chapman-Shaoxing 資料集，以分類心房顫動，以及另一種常見心律不整竇性心動過緩和正常竇性心律的心跳。這些模型能夠找出決定最終分類的心跳關鍵區域，並強調 P 波和 T 波，以及心跳持續時間和訊號振幅在區分正常竇性心律、心房顫動和竇性心動過緩的重要性。
...
摘要：本文介紹了一種用於憂鬱症偵測和治療的新穎範例，它採用先進的大型語言模型 (LLM)：生成式預訓練Transformer 4 (GPT-4)、Llama 2 聊天機器人，以及 Gemini。這些 LLM 經過微調，並具備專門提示，用於診斷、解釋和建議憂鬱症的治療介入措施。一種獨特的小樣本提示方法增強了模型根據 DSM-5 標準分析和解釋憂鬱症狀的能力。在互動階段，這些模型會參與同理心對話管理，從 PsychDB 和認知行為療法 (CBT) 指南等資源中汲取，促進與經歷重度憂鬱症的個人進行支持性互動。此外，本研究引入了 Illuminate 資料庫，其中包含了各種 CBT 模組，有助於提供個人化的治療建議。本研究使用 F1 分數、精確度、召回率、餘弦相似度和面向召回率的理解評估 (ROUGE) 等指標，在不同的測試集中評估 LLM 的效能，證明了它們的有效性。這種綜合方法將尖端的 AI 與既定的心理方法相結合，為心理保健提供了新的可能性，並展示了 LLM 在革新憂鬱症診斷和治療策略方面的潛力。
...
摘要：<paragraph>AI 系統的說明很少能滿足受演算法決策（ADM）影響的人們的資訊需求。這種傳達資訊與受影響的利害關係人重視的資訊之間的落差，可能會阻礙理解和遵守 AI 法規等法規架構。為了彌補這個落差，我們提出了「XAI 新手問題庫」：一個受影響的利害關係人資訊需求目錄，涵蓋兩個 ADM 使用案例（就業預測和健康監控），涵蓋資料、系統內容、系統使用和系統規格等類別。在訪談研究中收集資訊需求，參與者在提出詢問後收到說明作為回應。參與者進一步報告了他們的理解和決策信心，顯示雖然在收到說明後信心往往會增加，但參與者也遇到了理解上的挑戰，例如無法說明為什麼他們的理解感覺不完整。說明進一步影響了參與者對系統風險和好處的看法，他們會根據使用案例確認或改變這些看法。當風險被認為很高時，參與者表示特別有興趣了解意圖的說明，例如為什麼以及為了什麼目的建立一個系統。透過這項工作，我們旨在透過提供受影響的利害關係人在決定採用 ADM 系統時相關的資訊和挑戰的概覽，來支援將受影響的利害關係人納入可解釋性。最後，我們透過總結我們的發現，列出六個關鍵影響，為未來針對受影響的利害關係人受眾的說明設計提供資訊。</paragraph>
...
摘要：<paragraph>人工智慧（AI）的快速演進，特別是在大型語言模型（LLM）和生成式 AI 領域中，為各個領域的應用開啟了新途徑，但其在商業教育中的角色仍未被充分探討。本研究引入了第一個基準，用於評估七個主要 LLM 的效能，包括 OpenAI 的模型（GPT-3.5 Turbo、GPT-4 和 GPT-4 Turbo）、Google 的模型（PaLM 2、Gemini 1.0 Pro）和 Anthropic 的模型（Claude 2 和 Claude 2.1），這些模型在 GMAT 上的表現，GMAT 是研究所商學課程入學考試中的一項關鍵考試。我們的分析顯示，大多數 LLM 的表現都優於人類應試者，其中 GPT-4 Turbo 不僅優於其他模型，更超越了頂尖商學院中研究所學生的平均成績。透過案例研究，本研究探討了 GPT-4 Turbo 解釋答案、評估回應、找出錯誤、調整說明和產生替代情境的的能力。與前代版本相比，最新的 LLM 版本 GPT-4 Turbo、Claude 2.1 和 Gemini 1.0 Pro 在推理任務中表現出顯著的改善，突顯了它們在複雜問題解決方面的潛力。儘管 AI 在教育、評量和輔導方面的應用前景明確，但仍有挑戰存在。我們的研究不僅闡明了 LLM 的學術潛力，也強調了在教育中謹慎開發和應用 AI 的必要性。隨著 AI 技術的進步，建立 AI 互動的架構和協定、驗證 AI 生成的內容的準確性、確保全球各地學習者都能使用，以及創造一個 AI 支援人類專業知識的教育環境至關重要。本研究為進一步探討負責任地使用 AI 以豐富教育體驗並改善考試準備和評量方法奠定了基礎。</paragraph>
...
摘要：<paragraph>預測加護病房 (ICU) 病患的院內死亡率是臨床最終結果的關鍵。AI 已展現出優異的準確性，但缺乏可解釋性。為了解決這個問題，本文提出了一個可解釋的多模式死亡率預測器 (X-MMP)，透過多模式 ICU 資料，提供一個有效且可解釋的 AI 解決方案來預測院內死亡率。我們在架構中採用多模式學習，它可以接收臨床資料的異質輸入並做出決策。此外，我們引入了一個可解釋的方法，即層級傳播到 Transformer，作為 LRP 方法到 Transformer 的適當延伸，對多模式輸入產生解釋，並揭示歸因於預測的顯著特徵。此外，可以視覺化每個模式對臨床結果的貢獻，協助臨床醫生了解決策背後的推理。我們根據 MIMIC-III 和 MIMIC-III 波形資料庫匹配子集建構了一個多模式資料集。在基準資料集上的全面實驗證明，我們提出的架構可以實現合理的解釋，並具有競爭力的預測準確性。特別是，我們的架構可以輕鬆轉移到其他臨床任務，這有助於發現醫療保健研究中的關鍵因素。</paragraph>
...
摘要：<paragraph>在過去十年間，人工智慧 (AI) 在病理學中的方法已大幅進步。然而，由於許多挑戰，包括將研究結果轉化為臨床診斷產品的技術和法規障礙，以及缺乏標準化介面，導致整合到常規臨床實務中進展緩慢。開放且與供應商無關的 EMPAIA 計畫應對了這些挑戰。在此，我們概述了 EMPAIA 的成就和所學到的教訓。EMPAIA 整合了病理學 AI 生態系統的各種利害關係人，例如病理學家、電腦科學家和產業。在密切合作下，我們制定了技術互操作性標準、AI 測試和產品開發建議，以及可解釋性方法。我們實作了模組化且開放原始碼的 EMPAIA 平臺，並成功整合了來自 8 家不同供應商的 14 個基於 AI 的影像分析應用程式，展示了不同的應用程式如何使用單一標準化介面。我們優先考慮需求，並評估了 AI 在歐洲和亞洲 14 個不同病理實驗室中的實際臨床應用。除了技術開發之外，我們還為所有利害關係人建立了一個論壇，以分享數位病理學和 AI 的資訊和經驗。商業、臨床和學術利害關係人現在可以採用 EMPAIA 的常見開放原始碼介面，為大規模標準化和流程簡化提供了獨特的機會。需要進一步的努力，才能有效且廣泛地在常規實驗室使用中建立 AI 輔助。為此，已成立非營利協會 EMPAIA International，以持續標準化並支援廣泛實作和倡導 AI 輔助的數位病理學未來。</paragraph>
...
摘要：反事實解釋 (CE) 技術作為一種向與 AI 系統互動的使用者提供見解的方法而備受關注。儘管在醫學影像和自動駕駛等領域廣泛研究，但圖形反事實解釋 (GCE) 方法相對而言仍未得到充分探索。GCE 生成一個與原始圖形相似的新的圖形，其不同的結果基於底層預測模型。在這些 GCE 技術中，儘管在其他領域（例如藝術風格和自然語言建模）中展示了令人印象深刻的成就，但那些植根於生成機制的技術卻受到的關注相對較少。生成解釋器的偏好源於它們在推理過程中生成反事實實例的能力，利用輸入圖形的自主獲取的擾動。基於以上依據，我們的研究引入了 RSGG-CE，一種用於反事實解釋的新型魯棒隨機圖形生成器，能夠從考慮部分有序生成序列的學習潛在空間中產生反事實範例。此外，我們進行定量和定性分析，以比較 RSGG-CE 的性能與 SoA 生成解釋器，突顯其產生合理解釋候選項的能力增強。
...
摘要：可解釋 AI 的動機之一，是讓人類在使用和部署 AI 模型時，能做出更好、更明智的決策。但需要仔細評估，才能評量這個期望是否已實現。目前的評估主要關注解釋的演算法屬性，而涉及人類受試者的評估，通常採用主觀問題來測試人類對解釋實用性的看法，而沒有根據客觀的指標和測量。在這項工作中，我們評估解釋是否能在機器學習模型開發的實際場景中，改善人類的決策制定。我們進行了一項混合方法的使用者研究，其中涉及影像資料，以評估 SmoothGrad、GradCAM 和預言解釋所產生的顯著性圖，在兩個任務中的表現：模型選擇和反事實模擬。令人驚訝的是，我們沒有發現證據顯示，當使用者獲得任何顯著性圖（即使是設計成易於理解且高度指示答案的合成預言解釋）時，這些任務會有顯著的改善。儘管如此，解釋確實有助於使用者更準確地描述模型。這些發現表明，對於基於顯著性的解釋的實用性和可能產生的誤解，應持謹慎態度。
...
摘要：可說明性和安全性建立信任。這些需要一個模型來展示一致性和可靠性。為了實現這些目標，有必要使用和分析資料和知識，並使用與 AI 應用相關的統計和符號 AI 方法 - 單獨使用任何一種方法都不會成功。因此，我們主張並試圖證明 NeuroSymbolic AI 方法更適合將 AI 變成一個值得信賴的 AI 系統。我們提出 CREST 架構，展示了一致性、可靠性、使用者層級的可說明性以及安全性是如何建立在 NeuroSymbolic 方法上的，而這些方法使用資料和知識來支援關鍵應用程式的需求，例如健康和福祉。本文專注於大型語言模型 (LLM)，因為它們是 CREST 架構中所選的 AI 系統。LLM 由於其在處理廣泛自然語言處理 (NLP) 場景方面的多功能性，因此獲得了研究人員的極大關注。例如，ChatGPT 和 Google 的 MedPaLM 已成為提供一般和健康相關問題資訊的高度有前途的平台。儘管如此，這些模型仍然是黑盒子，儘管它們結合了人類回饋和指令引導的調整。例如，儘管制定了安全防護措施，ChatGPT 仍會產生不安全的回應。CREST 提出了一種可行的途徑，在 NeuroSymbolic 架構中利用程序性和基於圖表的知識，以闡明與 LLM 相關的挑戰。
...
摘要：本研究調查了在 COVID-19 大流行期間及之後，已部署的人工智慧 (AI) 模型在預測死亡率方面的效能、可解釋性和穩健性。作為此類研究中的第一個，我們發現貝氏神經網路 (BNN) 和智慧型訓練技術，讓我們的模型在資料大幅變動的情況下，仍能維持效能。我們的結果強調了開發穩健 AI 模型的重要性，此類模型即使在具挑戰性的情況下，也能與臨床醫師的預測相符或超越。我們對模型可解釋性的探討顯示，隨機模型會產生更多元且個人化的解釋，從而突顯了 AI 模型在現實世界的臨床環境中提供詳細且個人化見解的需求。此外，我們強調了量化 AI 模型中不確定性的重要性，這使臨床醫師能夠根據可靠的預測做出更明智的決策。我們的研究主張在醫療保健的 AI 研究中優先實施科學，並確保 AI 解決方案在現實世界的臨床環境中實用、有益且永續。透過解決醫療保健環境中獨特的挑戰和複雜性，研究人員可以開發出能有效改善臨床實務和患者預後的 AI 模型。
...
摘要：肺癌是英國 21% 癌症死亡的原因，五年存活率受到癌症發現階段的很大影響。最近的研究展示了人工智慧方法在從例行掃描中準確及早診斷肺癌的能力。然而，這個證據尚未轉化為臨床實務，其中一個障礙是缺少可解釋的模型。本研究調查變異自動編碼器 (VAE) 的應用，這是一種生成式人工智慧模型，用於肺癌病灶。提出的模型在從 LIDC-IDRI 公共資料集中的 3D 電腦斷層掃描中提取的病灶上進行訓練。VAE 產生的 2D 切片的潛在向量表示透過聚類進行探索以證明其品質，並用於肺癌診斷的多層感知器分類器模型，最佳模型達到了 AUC 0.98 和 93.1% 精確度的最先進指標。聚類分析顯示，VAE 潛在空間根據有意義的特徵成分（包括腫瘤大小、形狀、患者和惡性類別）將惡性和良性病灶的資料集分開。我們還包括標準高斯 VAE (GVAE) 和較新的 Dirichlet VAE (DirVAE) 的比較分析，後者以 Dirichlet 分配取代先驗，以鼓勵具有解開特徵表示的更可解釋的潛在空間。最後，我們展示了潛在空間橫越與臨床有意義的特徵變化相應的潛力。
...
摘要：現有的工具用於解釋影像分類器的輸出可分為白盒，依賴於存取模型內部，以及黑盒，不論模型為何。隨著 AI 在醫療領域的使用增加，可解釋性工具的使用也隨之增加。現有的醫療影像解釋工作著重於白盒工具，例如 gradcam。然而，切換到黑盒工具有明顯的優點，包括與任何分類器一起使用，以及廣泛的黑盒工具選擇。在標準影像上，黑盒工具與白盒一樣精確。在本文中，我們比較了幾個黑盒方法在腦癌 MRI 資料集上與 gradcam 的效能。我們證明了大多數的黑盒工具不適合解釋醫學影像分類，並對其缺點的原因進行詳細分析。我們也顯示了一個黑盒工具，一個因果可解釋性為基礎的 rex，其效能與 \gradcam 一樣好。
...
摘要：人工智慧開發社群日益使用 Hugging Face 等託管中介機構，提供使用者上傳的模型和訓練資料的簡易存取。這些模型市集降低了數十萬使用者的技術部署障礙，但卻可能被用於許多潛在有害和非法的方式。在本文中，我們說明人工智慧系統既可以「包含」內容，也可以是開放式工具，這兩種方式呈現迄今為止最棘手的平台治理挑戰。我們提供 Hugging Face、GitHub 和 Civitai 等三個說明性平台上數起事件的案例研究，探討模型市集如何控管模型。根據此分析，我們概述產業為回應控管需求所發展的重要（但仍有限）實務：授權、存取和使用限制、自動化內容控管和開放式政策發展。雖然現有的政策挑戰相當龐大，我們仍提出一些想法，說明平台如何能更有效地動員資源，作為謹慎、公平且適度的法規存取點。
...
摘要：<paragraph>背景和目標：通過提取這些信息，基於機器或深度學習 (ML/DL) 的自主數據分析工具可以協助臨床醫生和癌症研究人員從複雜數據集中發現模式和關係。最近已經發布了許多基於 DL 的卵巢癌 (OC) 數據分析。這些分析在癌症的各個方面（例如，它們涉及的子領域和癌症類型）和數據分析特徵方面千差萬別。然而，目前缺乏對這些分析在這些特徵和 AI 保證 (AIA) 方面的全面理解。這篇系統性回顧旨在通過檢查現有文獻並明確關注關鍵特徵和 AI 保證觀點，來填補這一空白。方法：使用 PRISMA 框架在三個期刊數據庫中進行全面搜索。分析僅包括 2015 年至 2023 年間發表在同行評審期刊中的研究。結果：在審查中，總共檢查了 96 項由 DL 驅動的分析。研究結果揭示了關於由 DL 驅動的卵巢癌數據分析的幾個重要見解：- 大多數研究 71%（96 項中的 68 項）側重於檢測和診斷，而沒有研究涉及 OC 的預測和預防。- 這些分析主要基於非多元化人群的樣本（75%（96 項研究中的 72 項）），僅限於地理位置或國家。- 只有少部分研究（僅 33%（96 項中的 32 項））進行了綜合分析，其中大多數使用同質數據（臨床或組學）。- 值得注意的是，僅有 8.3%（96 項中的 8 項）的研究使用外部和多樣化的數據集驗證了他們的模型，這突顯了增強模型驗證的必要性，- 將 AIA 納入癌症數據分析仍處於非常早期的階段；只有 2.1%（96 項中的 2 項）通過可解釋性明確解決了 AIA。</paragraph>
...
摘要：<paragraph>可解釋性是深度學習中一個長期的挑戰，特別是在醫療保健等高風險領域。常見的可解釋性方法會強調驅動 AI 模型決策的影像區域。然而，人類在傳達解釋時，很大程度上依賴語言，不僅說明「在哪裡」，還說明「是什麼」。此外，大多數的可解釋性方法專注於解釋個別 AI 預測，而不是描述 AI 模型一般使用的特徵。後者對於模型和資料集稽核特別有用，甚至可能在 AI 日益用於新任務時產生知識。在此，我們提出一個可解釋性策略，使用視覺語言模型來識別視覺分類任務的基於語言的描述符。透過利用影像和文字之間預先訓練的聯合嵌入空間，我們的方法將新的分類任務估計為一個線性詞語組合，每個詞語都有對應的權重，表示其與基於視覺的分類器的對齊方式。我們使用兩個醫學影像分類任務評估我們的方法，發現產生的描述符在很大程度上與臨床知識一致，儘管缺乏特定領域的語言訓練。然而，我們的方法也識別出所使用公共資料集中「捷徑連線」的可能性。為了獲得可解釋性的功能性衡量標準，我們進行了一項試驗性讀者研究，發現 AI 識別的詞語可以使非專家人類在非平凡的層級上執行專門的醫療任務。總之，我們的結果強調了使用多模態基礎模型來提供直觀、基於語言的視覺任務解釋的潛力。</paragraph>
...
摘要：<paragraph>使用醫學影像訓練的人工智慧 (AI) 模型，用於臨床任務時，常會表現出偏見，例如在不同子群間的效能差異。由於並非所有來自真實世界醫學影像資料的偏見來源都容易辨識，因此要全面評估這些偏見如何編碼在模型中，以及偏見緩解方法在改善效能差異上的能力，是一項挑戰。在本文中，我們介紹了一種創新的分析架構，用於系統化且客觀地探討醫學影像中偏見對 AI 模型的影響。我們開發並測試了這個架構，用於執行受控的電腦模擬試驗，以評估醫學影像 AI 中的偏見，使用一個工具來產生具有已知疾病影響和偏見來源的合成磁共振影像。可行性透過使用三個反事實偏見情境來展示，以測量模擬偏見效應對卷積神經網路 (CNN) 分類器和三個偏見緩解策略的功效的影響。分析顯示，當 CNN 在合成資料集上訓練時，模擬偏見會導致預期的子群效能差異。此外，重新加權被認為是針對此設定最成功的偏見緩解策略，我們展示了可解釋的 AI 方法如何協助調查模型中偏見的表現，使用這個架構。開發公平的 AI 模型是一項相當大的挑戰，因為在醫學影像資料集中可能存在許多且通常未知的偏見來源。在這項工作中，我們提出了一種創新的方法，用於客觀地研究偏見和緩解策略對深度學習管線的影響，這可以支援穩健且負責任的臨床 AI 的開發。</paragraph>
...
摘要：機器學習為自動預測中風後症狀及其對復健的反應提供了巨大的潛力。此項任務的主要挑戰包括神經影像資料的維度非常高、可用於學習的資料集相對較小，以及如何有效結合神經影像和表格資料（例如人口統計資料和臨床特徵）。本文根據兩種策略評估了多種解決方案。第一個是使用總結 MRI 掃描的 2D 影像。第二個是選擇可提升分類精確度的關鍵特徵。此外，我們引進了一種新穎的方法，即在結合從 MRI 中提取的感興趣區域與表格資料的符號表示的影像上訓練卷積神經網路 (CNN)。我們評估了一系列 CNN 架構（2D 和 3D），這些架構在 MRI 和表格資料的不同表示上進行訓練，以預測中風後口述圖片描述能力的綜合測量是否在失語或非失語範圍內。MRI 和表格資料是從 758 名參與 PLORAS 研究的英語中風倖存者中取得的。僅針對病灶大小的基準邏輯迴歸分類準確度為 0.678，在依序加入初始症狀嚴重程度和復原時間後，上升至 0.757 和 0.813。在從每個 MRI 掃描中提取 8 個感興趣區域，並在 2D 殘差神經網路中與病灶大小、初始嚴重程度和復原時間結合時，觀察到最高的分類準確度 0.854。我們的研究結果證明了如何將影像和表格資料結合，以獲得高於中風後分類準確度，即使在機器學習術語中資料集很小的情況下也是如此。我們最後提出如何改進目前的模型，以使用來自醫院掃描儀的影像來達到更高的準確度。
...
摘要：可解釋人工智慧 (XAI) 已成為處理任務關鍵型應用程式時的一項基本需求，確保所採用黑箱 AI 模型的透明度和可解釋性。XAI 的重要性涵蓋各種領域，從醫療保健到金融，在這些領域中，了解深度學習演算法的決策制定過程至關重要。大多數基於 AI 的電腦視覺模型通常都是黑箱；因此，在影像處理中提供深度神經網路的可解釋性對於其在醫學影像分析、自動駕駛和遙測應用中的廣泛採用和部署至關重要。最近，已經引入了幾種用於影像分類任務的 XAI 方法。相反地，影像分割在可解釋性方面受到的關注相對較少，儘管它是電腦視覺應用中的一項基本任務，特別是在遙測方面。只有部分研究提出用於影像分割的基於梯度的 XAI 演算法。本文採用最近的無梯度 Sobol XAI 方法進行語意分割。為了衡量 Sobol 方法在分割中的效能，我們提出了一種基於可學習雜訊模型的定量 XAI 評估方法。此模型的主要目的是在解釋圖上產生雜訊，其中較高的誘發雜訊表示低準確度，反之亦然。進行基準分析以評估和比較三種 XAI 方法的效能，包括使用所提出的基於雜訊的評估技術的 Seg-Grad-CAM、Seg-Grad-CAM++ 和 Seg-Sobol。這是使用高解析度衛星影像執行和評估 XAI 方法的首次嘗試。
...
摘要：大型語言模型在短時間內已在多個領域中大量擴散。然而，由於事實性、連貫性和幻覺等問題，在醫療保健領域中仍猶豫是否採用它們。由於醫療保健的風險很高，許多研究人員甚至警告在這些問題解決之前不要使用它。在醫療保健中實施和部署 LLM 的關鍵在於使這些模型值得信賴、透明（儘可能）且可解釋。在本文中，我們描述了建立可靠、值得信賴且無偏見模型的關鍵要素，作為其在醫療保健中採用的必要條件。具體來說，我們專注於在醫療保健背景下對幻覺進行量化、驗證和緩解。最後，我們討論了 LLM 在醫療保健中的未來可能是什麼樣子。
...
摘要：人工智慧 (AI) 以快速的速度進步，現在已準備部署在各種應用程式中，例如自動系統、醫療診斷和自然語言處理。早期採用 AI 技術於真實世界應用並非沒有問題，特別是對神經網路而言，它可能不穩定且容易受到對抗範例的影響。從長遠來看，需要開發適當的安全保證技術，以減少由於可避免的系統故障而造成的潛在危害，並確保可信度。本文著重於認證和可解釋性，概述了已開發用於確保 AI 決策安全性的技術，並討論未來的挑戰。
...
摘要：歐洲議會議會研究服務總局已為歐洲議會議員準備了一份報告，其中列舉了人工智能（AI）在醫學和醫療保健中的七項主要風險：因 AI 錯誤而造成的患者傷害、醫療 AI 工具的誤用、AI 中的偏見和現有不平等現象的延續、缺乏透明度、隱私和安全問題、問責制差距以及實施障礙。
  在這項研究中，我們提出了十四項 AI 系統可能實施的功能性要求，以降低其醫療目的相關的風險：AI 護照、使用者管理、法規檢查、僅限學術用途免責聲明、資料品質評估、臨床醫生雙重檢查、持續效能評估、稽核追蹤、持續可用性測試、回顧性/模擬案例審查、偏見檢查、可解釋 AI、加密和使用經過現場測試的程式庫，以及語義互操作性。
  我們在此的目的是提供具體的高層級技術解決方案規格，以確保持續良好的效能，並使用 AI 系統使患者受益，以符合未來的歐盟法規架構。
...
摘要：人工智慧技術可用於分類病患的生理活動，並預測遠距監控病患的重要生命徵象。基於非線性模型（如深度學習模型）的回歸分析，由於其黑箱性質，可解釋性有限。這可能需要決策者根據非線性模型結果盲目地做出決策，特別是在醫療保健應用中。在非侵入性監控中，追蹤感測器與其易感臨床屬性的病患資料，作為預測未來生命徵象的輸入特徵。解釋各種特徵對監控應用程式整體輸出的貢獻，對於臨床醫師的決策至關重要。本研究提出一個可解釋人工智慧定量分析 (QXAI) 架構，具有事後模型可解釋性與內在可解釋性，用於監督式學習方法中的回歸與分類任務。這是透過利用 Shapley 值概念，並將注意力機制納入深度學習模型中來實現的。我們採用人工神經網路 (ANN) 和基於注意力的雙向 LSTM (BiLSTM) 模型，根據感測器資料預測心率和分類生理活動。深度學習模型在預測和分類任務中均達到最先進的結果。對輸入資料進行整體解釋和局部解釋，以了解各種病患資料的特徵貢獻。所提出的 QXAI 架構使用 PPG-DaLiA 資料評估，以預測心率，並使用行動健康 (MHEALTH) 資料根據感測器資料分類生理活動。蒙地卡羅逼近法應用於架構中，以克服 Shapley 值計算所需的時間複雜度和高運算能力需求。
...
摘要：在可解釋的人工智慧 (XAI) 研究中，主要的重點在於為專家和從業者詮釋模型。模型不可知論和局部解釋方法在許多應用中被認為是可解釋且充分的。然而，在醫療保健等領域，最終使用者是沒有 AI 或領域專業知識的患者，因此迫切需要更易於理解且能建立對模型運作的信任的模型解釋。我們假設產生敘述性、特定於患者且全局（模型的整體）的模型解釋將能讓使用者更易於理解並能做出決策。我們使用決策樹模型測試這一點，為被認定有高冠狀動脈心臟病風險的患者產生局部和全局解釋。這些解釋會提供給非專家使用者。我們發現針對特定類型的解釋有強烈的個人偏好。大多數參與者偏好全局解釋，而較小的一群則偏好局部解釋。基於任務的參與者心智模型評估提供了有價值的回饋，以增強敘述性全局解釋。這反過來引導了能同時值得信賴且可行的健康資訊系統的設計。
...
摘要：<paragraph>在医学影像中，可解释人工智能至关重要。在神经科学的挑战性领域中，视觉主题呈现出高度复杂性，尤其是在三维空间中。神经科学的应用涉及从 MRI 中识别脑沟特征，由于专家之间不同的注释协议和大脑复杂的 3D 功能，因此面临着重大障碍。因此，传统的可解释性方法在有效验证和评估这些网络方面存在不足。为了解决这个问题，我们首先提出一个数学公式，描述了不同计算机视觉任务中各种类别的解释需求，根据验证协议的可靠性，将其分类为自解释、半解释、不可解释和新模式学习应用程序。针对这一数学公式，我们提出了一个 3D 可解释性框架，旨在验证深度学习网络在检测旁扣带回沟（一种基本的大脑解剖特征）中的输出。该框架集成了局部 3D 解释、通过降维的全局解释、串联的全局解释和统计形状特征，揭示了模式学习的新见解。我们在具有挑战性的 TOP-OSLO 数据集上训练并测试了两个先进的 3D 深度学习网络，显着提高了脑沟检测的准确性，尤其是在左半球。在使用该数据集的不同注释协议进行评估时，我们强调了无偏注释过程在我们的 3D 框架中实现精确预测和有效模式学习的关键作用。所提出的框架不仅注释了可变脑沟，还揭示了隐藏的人工智能知识，有望促进我们对大脑解剖和功能的理解。</paragraph>
...
摘要：電子健康紀錄 (EHR) 和例行文件記錄實務在病患的日常照護中扮演著至關重要的角色，提供完整的健康、診斷和治療紀錄。然而，複雜且冗長的 EHR 敘述會讓醫療保健提供者不堪負荷，導致診斷不準確的風險。雖然大型語言模型 (LLM) 已在各種語言任務中展現其潛力，但其在醫療保健領域的應用需要確保將診斷錯誤降到最低並防止病患受到傷害。在本文中，我們概述了一種創新的方法，透過整合醫學知識圖譜 (KG) 和一種新的圖譜模型：Dr.Knows 來擴充 LLM 在自動化診斷產生方面的能力，靈感來自於臨床診斷推理流程。我們從美國國家醫學圖書館的統一醫學語言系統 (UMLS) 中衍生出 KG，這是一個強大的生物醫學知識儲存庫。我們的方法否定了預先訓練的需要，而是利用 KG 作為輔助工具，協助解釋和總結複雜的醫療概念。使用真實世界的醫院資料集，我們的實驗結果證明了將 LLM 與 KG 結合的建議方法有潛力提升自動化診斷產生的準確性。更重要的是，我們的做法提供了一條可解釋的診斷途徑，讓我們更接近實現 AI 擴增的診斷決策支援系統。
...
摘要：現有的用於診斷膝骨關節炎 (OA) 的人工智慧 (AI) 模型因缺乏透明度和可解釋性而受到批評，儘管它們達到了類似醫療專家的表現。這種不透明性使得它們難以在臨床實務中獲得信任。最近，可解釋人工智慧 (XAI) 已成為一種專門技術，它能透過揭示預測的推導方式來提供對模型預測的信心，從而促進 AI 系統在醫療保健中的使用。本文介紹了用於膝骨關節炎診斷的第一個 XAI 技術調查。XAI 技術從兩個角度進行探討：資料可解釋性和模型可解釋性。本文的目的是提供有價值的見解，了解 XAI 在更可靠的膝骨關節炎診斷方法中的潛力，並鼓勵在臨床實務中採用 XAI。
...
摘要：近期在醫療保健中的人工智慧應用進展顯著，在診斷和疾病預後方面超越人類表現的潛力令人難以置信。然而，隨著人工智慧模型的複雜性日益增加，對於其不透明性、潛在偏差和可解釋性的需求也引發了擔憂。為了確保人工智慧系統的信任度和可靠性，特別是在臨床風險預測模型中，可解釋性至關重要。可解釋性通常被稱為人工智慧系統提供其決策邏輯或決策本身的強健解釋給人類利害關係人的能力。在臨床風險預測中，可解釋性的其他面向，如公平性、偏差、信任和透明度，也代表了超越可解釋性的重要概念。在此回顧中，我們探討這些概念之間的關係，因為它們通常一起或交替使用。本回顧還討論了開發可解釋臨床風險預測模型的最新進展，強調了在臨床實務中多種常見模式的量化和臨床評估與驗證的重要性。它強調了外部驗證和結合多樣的可解釋性方法以增強信任和公平性的必要性。採用嚴謹的測試，例如使用具有已知生成因子的合成資料集，可以進一步提高可解釋性方法的可靠性。開放取用和程式碼共享資源對於透明度和可複製性至關重要，有助於可解釋性研究的成長和可信度。儘管存在挑戰，但臨床風險預測中可解釋性的端到端方法，結合從臨床醫生到開發人員的利害關係人，對於成功至關重要。
...
摘要：儘管人工智慧 (AI) 在醫學和醫療保健方面有重大的進展，但 AI 技術在現實世界的臨床實務中部署和採用仍然有限。近年來，人們對醫療 AI 相關的技術、臨床、倫理和法律風險提出了疑慮。為了增加現實世界的採用率，醫療 AI 工具必須受到患者、臨床醫生、醫療機構和主管機關的信賴和接受。本研究將 FUTURE-AI 指南描述為指導醫療保健領域中值得信賴的 AI 工具開發和部署的第一個國際共識架構。FUTURE-AI 聯盟成立於 2021 年，目前由來自 51 個國家/地區的 118 位跨領域專家組成，代表了所有洲，包括 AI 科學家、臨床醫生、倫理學家和社會科學家。在兩年的時間裡，該聯盟透過一個反覆運算的過程，包括深入的文獻回顧、修改後的德爾菲調查和線上共識會議，定義了值得信賴的 AI 的指導原則和最佳實務。FUTURE-AI 架構建立在醫療保健中值得信賴的 AI 的 6 個指導原則之上，即公平性、普遍性、可追溯性、可用性、穩健性和可解釋性。透過共識，定義了一組 28 項最佳實務，涵蓋技術、臨床、法律和社會倫理層面。建議涵蓋了醫療 AI 的整個生命週期，從設計、開發和驗證到法規、部署和監控。FUTURE-AI 是一個基於風險、無假設的指南，提供了一個結構化的方法來建構醫療 AI 工具，這些工具將在現實世界的實務中受到信賴、部署和採用。鼓勵研究人員在概念驗證階段考慮這些建議，以促進醫療 AI 未來向臨床實務的轉化。
...
摘要：人工智慧在醫學方面已取得顯著進展，在醫學影像、病人照護和其他領域都有新興應用。儘管這些應用已在回顧性研究中證實成功，但實際應用者卻很少。醫學人工智慧領域面臨各種挑戰，包括建立使用者信任、遵守法規、以符合道德的方式使用資料。可解釋人工智慧 (XAI) 旨在讓人類了解人工智慧並信任其結果。本文根據近年發表的 198 篇文章的代表性樣本，探討醫學決策支援的 XAI 解決方案的最新發展。相關文章的系統性綜合產生了多項發現。(1) 這些解決方案大多採用與模型無關的 XAI 技術，(2) 深度學習模型的使用多於其他類型的機器學習模型，(3) 可解釋性用於促進信任，但很少有研究報告醫師參與其中，(4) 視覺化且互動式的使用者介面有助於理解系統的解釋和建議。需要更多醫學和人工智慧專家合作進行研究，以指導設計、實作和評估醫學中 XAI 解決方案的適當架構。
...
摘要：體外受精是治療不孕症最廣泛的方法之一。其主要挑戰之一是評估和選擇胚胎進行植入，此過程具有很大的臨床間和臨床內變異性。基於深度學習的方法正備受關注，但其不透明的本質會影響其在臨床環境中的接受度，而透明度在決策制定中至關重要。在本文中，我們分析了 AI 輔助胚胎分析模型的可解釋性方面的現有工作，並找出其限制。我們還討論了如何將這些模型作為決策支援系統整合到臨床環境中，同時考慮臨床醫生和患者的需求。最後，我們提出了提高可解釋性和可信度的準則，推進這項技術朝著既定的臨床實務邁進。
...
摘要：在需求工程 (RE) 領域中，可解釋人工智慧 (XAI) 在將 AI 支援系統與使用者需求、社會期望和法規標準相符上日益重要的意義已獲得認可。一般而言，可解釋性已成為影響系統品質的重要非功能性需求。然而，可解釋性和效能之間假定的取捨挑戰了可解釋性的假定正面影響。如果滿足可解釋性的需求需要降低系統效能，那麼必須仔細考慮這些品質面向中哪一個優先，以及如何在它們之間取得平衡。在本文中，我們批判性地探討了假設的取捨。我們主張以一種微妙的方式來處理它，這種方式結合了資源可用性、領域特性和風險考量。透過為未來的研究和最佳實務提供基礎，這項工作旨在推動 RE for AI 的領域。
...
摘要：在需求工程（RE）領域中，可解釋的人工智慧（XAI）在將 AI 支援系統與使用者需求、社會期望和法規標準相符時的重要性日益提升，已獲得認可。一般而言，可解釋性已成為影響系統品質的重要非功能需求。然而，可解釋性與效能之間的假設取捨挑戰了可解釋性的假設正面影響。如果滿足可解釋性的需求需要降低系統效能，則必須仔細考慮這些品質面向中哪一個優先，以及如何在它們之間做出妥協。在本文中，我們批判性地探討了假設的取捨。我們主張最好以細緻的方式來探討，這包含了資源可用性、領域特性和風險考量。透過提供未來研究和最佳實務的基礎，這項工作旨在推動 AI 的 RE 領域。
...
摘要：本論文批判性地評估了歐盟委員會提出的 AI 法案，在風險管理和風險可接受性方面，針對對基本權利和安全構成風險的高風險 AI 系統所採取的方法。該法案旨在以適當的監管負擔促進「值得信賴」的 AI。其關於風險可接受性的條款要求高風險系統的殘餘風險應「盡可能」降低或消除，並考慮「最新技術」。此項標準，特別是如果狹義解釋的話，是不可行的，而且既無法促進適當的監管負擔，也無法促進值得信賴。相比之下，議會最近針對風險管理條款提出的修正草案引入了「合理性」、成本效益分析，並且更透明地說明了風險可接受性判斷的價值觀和情境性質。本文認為議會的方法更可行，而且更能平衡相稱性和值得信賴的目標。本文說明風險可接受性判斷中的合理性將會帶來什麼，並根據疏忽法和歐洲醫療器材法規中的原則進行說明。本文主張，風險可接受性判斷的方法需要穩固的公民合法性基礎：包括來自監管機構的詳細指導或參與，以及受影響利益相關者的有意義投入。
...
摘要：可解釋人工智慧 (XAI) 是機器學習中快速進展的領域，旨在解開複雜模型的預測。XAI 在敏感應用中特別需要，例如在醫療保健中，當診斷、建議和治療選擇可能依賴於人工智慧系統做出的決策時。AI 方法也已廣泛用於老化研究中，特別是在開發生物時鐘模型和識別老化和與年齡相關疾病的生物標誌物方面。然而，XAI 在此處的潛力仍有待充分發揮。我們討論了 XAI 在開發「老化時鐘」中的應用，並對按特定生理系統的焦點分類的文獻進行了全面的分析。
...
摘要：部分原型模型是可解釋的設計圖像分類器，也是黑箱 AI 的一個有前景的替代方案。本文探討了解釋性機器學習的適用性和潛力，特別是 PIP-Net，用於對真實世界醫學影像資料進行自動化診斷支援。PIP-Net 學習人類可理解的原型圖像部分，我們評估其準確性和可解釋性，用於骨折檢測和皮膚癌診斷。我們發現 PIP-Net 的決策制定過程符合醫療分類標準，而僅提供影像層級的類別標籤。由於 PIP-Net 的原型無監督預訓練，資料品質問題，例如 X 光中的不需要文字或標籤錯誤，可以很容易地被識別出來。此外，我們首次展示人類可以通過直接停用不需要的原型來手動校正 PIP-Net 的推論。我們得出結論，部分原型模型由於其可解釋性和高級模型除錯的潛力，對於醫療應用來說是有前景的。
...
摘要：可解釋 AI (XAI) 是機器學習研究中日益重要的領域，旨在使黑箱模型透明且可解釋。在本文中，我們提出了一種新的 XAI 方法，它使用由特徵條件排列產生的所謂反事實路徑。該演算法透過辨識影響模型預測變化最大的特徵序列排列來衡量特徵重要性。它特別適用於根據結合領域知識的知識圖譜中的反事實路徑產生解釋。反事實路徑為目前的 XAI 方法引入了額外的圖形維度，用於解釋和視覺化黑箱模型。使用合成資料和醫療資料進行的實驗證明了我們方法的實際應用性。
...
摘要：人工智能 (AI) 中的可解釋性領域見證了越來越多的研究和越來越多的學術興趣。然而，在解釋機器學習演算法的結果時，缺乏對人類友善和個別的解釋，顯著阻礙了臨床醫生在研究和臨床實務中接受這些方法。為了解決這個問題，我們的研究使用反事實解釋來探討「如果？」情境在醫學研究中的適用性。我們的目標是擴展我們對磁共振造影 (MRI) 特徵的理解，這些特徵用於診斷小兒後顱窩腦腫瘤，並超越現有的界限。在我們的案例研究中，提出的概念提供了一種新的方式來檢查替代決策制定情境，這些情境提供個性化和特定於情境的見解，使我們能夠驗證預測並釐清在不同情況下的變異。此外，我們探討了反事實用於資料擴充的潛在用途，並評估其作為我們醫學研究案例中替代方法的可行性。結果證明了使用反事實解釋來增強臨床研究中 AI 驅動方法的接受度的潛在可能性。
...
摘要：目前人工智能領域的進展已導致開發出各種由 AI 提供支援的失智症評估，可用於識別處於失智症早期階段的患者。它可以徹底改變失智症照護環境。醫療界必須了解各種 AI 評估，並在考慮其有效性、效率、實用性、可靠性和準確性程度的情況下選擇它們，以早期識別失智症患者 (PwD)。另一方面，AI 開發人員也應了解各種非 AI 評估以及最近開發的 AI 評估。因此，這篇臨床醫生和 AI 工程師都可以閱讀的論文填補了文獻中說明現有失智症識別解決方案的空白，以及向 AI 工程師說明所使用的技術和最廣泛的失智症資料集。它遵循對 AI 和非 AI 失智症評估論文的回顧，以提供有關各種失智症評估的寶貴資訊，供 AI 和醫療界參考。討論和結論突出了最顯著的研究方向和現有解決方案的成熟度。
...
摘要：<paragraph>可解釋性對人工智慧 (AI) 技術提出重大挑戰。目前對可解釋 AI (XAI) 的研究缺乏提取學習任務的全局知識的效率，因此存在不精確顯著性、上下文感知缺失和含義模糊等缺陷。在本文中，我們提出類別關聯嵌入 (CAE) 方法來解決這些問題。我們採用編碼器-解碼器架構來嵌入樣本特徵，並同時將它們分為與類別相關和與個體相關的樣式向量。將給定樣本的個體樣式代碼與另一個樣本的類別樣式代碼重新組合，會產生一個合成樣本，保留個體特徵，但改變類別分配，遵循循環對抗學習策略。類別關聯嵌入將所有實例的全局類別相關特徵提取到一個統一的域中，並在類別之間有良好的分離。然後可以提取不同類別之間的轉換規則，並進一步應用於個別實例。然後，我們提出一個主動 XAI 框架，它沿著引導路徑操縱特定樣本的類別樣式向量，朝著反類別前進，從而產生一系列具有相同個體特徵的反例合成樣本。將這些反事實樣本與原始樣本進行比較，可以直觀地說明分類任務的本質。我們採用該框架對醫學影像分類任務，結果表明，與現有方法相比，可以獲得更精確的顯著性圖，並具有強大的上下文感知表示。此外，疾病病理學可以通過遍歷類別樣式空間中的路徑直接視覺化。</paragraph>
...
摘要：提供基於機器學習的 AI 預測的高品質解釋是一項具有挑戰性和複雜性的任務。要順利進行，需要以下因素：選擇適當的解釋普遍性/特殊性層級；考量解釋受益者對所考慮的 AI 任務的熟悉程度的假設；參考促成決策的特定元素；利用額外的知識（例如專家證據），這些知識可能不屬於預測程序的一部分；並提供支持負面假設的證據。最後，系統需要以清晰可解讀且可能令人信服的方式表述解釋。基於這些考量，ANTIDOTE 培育了可解釋 AI 的整合願景，其中深度學習程序的低階特徵與人類論證能力特有的高階架構相結合。ANTIDOTE 將利用深度學習和論證的跨領域能力來支持更廣泛且創新的可解釋 AI 觀點，其中對於臨床案例審議的高品質解釋需求至關重要。作為專案的第一個成果，我們發布了 Antidote CasiMedicos 資料集，以利於研究一般可解釋 AI，特別是醫療領域的論證。
...
摘要：近年來，圖神經網路的進展迅速，在藥物發現、醫療診斷和推薦系統等方面都有許多新的發展。雖然這項進展很重要，但許多網路都是「黑盒子」，對於網路究竟在學習什麼鮮少了解。許多高風險的應用，例如藥物發現，需要模型提供人類可以理解的解釋，以便使用者能夠辨識錯誤並發現新知識。因此，可解釋 AI 演算法的開發對於我們獲取 AI 的好處至關重要。
我們提出一個用於 GNN 的可解釋性演算法，稱為 eXplainable Insight (XInsight)，它使用 GFlowNets 產生模型解釋的分布。由於 GFlowNets 會產生機率與獎勵成正比的物件，與僅學習最大獎勵樣本的先前方法相比，XInsight 可以產生多樣化的解釋。我們透過產生在兩個圖形分類任務中訓練的 GNN 的解釋來展示 XInsight：使用 MUTAG 資料集分類致突變化合物，並使用我們已開放原始碼的合成資料集分類非環狀圖形。我們透過使用 QSAR 建模分析產生的化合物來展示 XInsight 解釋的效用，我們發現 XInsight 產生了按親脂性（已知的致突變性相關因素）分群的化合物。我們的結果顯示 XInsight 產生了解釋的分布，揭示了模型所展示的底層關係。它們也突顯了產生多樣化解釋集的重要性，因為它使我們能夠發現模型中的隱藏關係，並為進一步的分析提供有價值的指導。
...
摘要：我們提議並實作一個可解釋機器學習分類模型，用於基於表達式布林公式的可解釋 AI (XAI)。潛在應用包括信用評分和醫療狀況診斷。布林公式定義了一個具有可調整複雜度（或可解釋性）的規則，根據該規則對輸入資料進行分類。此類公式可以包含任何可套用於一個或多個布林變數的運算子，因此與更嚴格的基於規則和基於樹狀結構的方法相比，提供了更高的表達力。使用原生局部最佳化技術訓練分類器，有效地搜尋可行公式的空間。淺層規則可以用快速的整數線性規劃 (ILP) 或二次無約束二元最佳化 (QUBO) 求解器來確定，這些求解器可能由特殊用途硬體或量子裝置提供支援。我們結合了原生局部最佳化器的表達力和效率，以及這些裝置的快速運算，方法是執行非局部移動，對完整布林公式的子樹進行最佳化。我們提供廣泛的數值基準測試結果，其中包含幾個在眾所周知的公共資料集上的基線。根據結果，我們發現原生局部規則分類器通常與其他分類器具有競爭力。加入非局部移動以更少的反覆運算達到類似的結果，因此使用專用或量子硬體可以透過快速提出非局部移動來加速。
...
摘要：為了應對心理健康問題的全球挑戰，我們提出了一種基於邏輯神經網路 (LNN) 的神經符號 AI 方法來診斷心理疾病。由於缺乏針對心理疾病的有效治療，因此需要一種 AI 解決方案來協助治療師進行診斷。然而，當前的神經網路模型缺乏可解釋性，治療師可能無法信賴。LNN 是一種遞迴神經網路架構，它結合了神經網路的學習能力和基於經典邏輯的 AI 的推理能力。所提出的系統使用臨床訪談中的輸入謂詞來輸出心理疾病類別，並使用不同的謂詞剪枝技術來實現可擴充性和更高的分數。此外，我們提供了一種見解提取方法來幫助治療師進行診斷。所提出的系統解決了當前神經網路模型缺乏可解釋性的問題，並為心理疾病診斷提供了一個更值得信賴的解決方案。
...
摘要：隨著機器學習模型在醫療診斷中日益普及，對於可解釋性和透明度的需求變得至關重要。XAI 文藝復興標誌著該領域的重大轉變，旨在重新定義醫療診斷模型的可解釋性。本文探討了可解釋 AI (XAI) 領域內的創新方法和方法論，這些方法和方法論正在徹底改變醫療診斷模型的可解釋性。通過闡明基礎決策制定過程，XAI 技術使醫療保健專業人員能夠理解、信任並有效利用這些模型進行準確且可靠的醫療診斷。本綜述重點介紹了 XAI 在醫療診斷方面的關鍵進展及其轉變醫療保健領域的潛力，最終改善患者的治療結果並培養對 AI 驅動的診斷系統的信任。
...
摘要：在連通性和行動性日益提升的環境中，加上心血管疾病的激增，透過遠端監控心血管健康來減少醫療保健支出的必要性變得更加明顯。準確檢測和分類心律不整對於診斷患有心臟不規則的人至關重要。本研究強調了在家中環境中使用心電圖 (ECG) 測量進行實時心律不整檢測的可行性。本論文提出了心律不整檢測的新應用，利用尖端的 You-Only-Look-Once (YOLO)v8 演算法來分類單導程 ECG 訊號。我們引入了一個新穎的損失修改 YOLOv8 模型，在 MIT-BIH 心律不整資料集上進行微調，以實現實時連續監控。獲得的結果證實了我們方法的有效性，該模型在 NVIDIA Tesla V100 上達到了 99.5% 的平均準確度和 0.992 mAP@50，以及 0.002 秒的快速檢測時間。我們的調查例證了實時心律不整檢測的潛力，使用戶能夠在舒適的家中視覺化地解釋模型輸出。此外，本研究為擴展到能夠部署在醫療保健領域的實時可解釋 AI (XAI) 模型奠定了基礎，從而顯著推進了醫療保健解決方案的領域。
...
摘要：乳癌 (BC) 仍然是一個重大的健康威脅，目前尚無長期治癒方法。早期檢測至關重要，但乳房攝影的判讀受到高假陽性和假陰性的阻礙。隨著乳癌發生率預計將超過肺癌，改善早期檢測方法至關重要。熱成像使用高解析度紅外線相機，特別是與人工智能 (AI) 結合使用時，提供了希望。這項工作提出了一個基於注意力的卷積神經網路用於分割，在乳癌檢測和分類中提供了更高的速度和精確度。該系統增強影像並執行可解釋的 AI 癌症分割。我們提出了一個基於 Transformer 注意力的卷積架構 (UNet) 用於故障識別，並採用梯度加權類激活映射 (Grad-CAM) 來分析 UNet 架構中偏見和弱點的區域，並使用 IRT 影像。與現有的深度學習框架相比，我們提出的框架的優越性得到了證實。
...
摘要：憂鬱症是最普遍且嚴重的精神疾病，會造成嚴重的財務和社會後果。憂鬱症偵測是早期介入以減輕這些後果的關鍵。如此高風險的決定本質上需要可解釋性。儘管一些憂鬱症偵測研究嘗試根據重要性分數或注意力權重來解釋決定，但這些解釋與基於憂鬱症狀的臨床憂鬱症診斷標準不一致。為了填補這個缺口，我們遵循計算設計科學範例來開發一個新穎的多尺度時間原型網路 (MSTPNet)。MSTPNet 創新地偵測和解釋憂鬱症狀以及它們持續的時間。使用大規模資料集進行的廣泛實證分析顯示，MSTPNet 以 F1 分數 0.851 優於最先進的憂鬱症偵測方法。此結果也揭示了調查方法中未註明的新的症狀，例如分享對不同生活的欽佩。我們進一步進行使用者研究，以證明其在可解釋性方面優於基準。本研究以一個新穎的可解釋深度學習模型為憂鬱症偵測在社群媒體中做出了貢獻。在實務上，我們提出的方法可以在社群媒體平台中實作，以提供個人化的線上資源給偵測出的憂鬱症患者。
...
摘要：電子健康紀錄 (EHR) 是用於預想中由人工智慧 (AI) 推動的醫療保健轉型的基本資料來源。
然而，反映在 EHR 備註中的臨床偏見可能導致 AI 模型繼承並擴大這些偏見，使健康差異永存。這項研究調查了 EHR 備註中污名化語言 (SL) 對使用基於 Transformer 的深度學習模型和可解釋 AI (XAI) 技術進行死亡率預測的影響。我們的發現證明了由臨床醫生撰寫的 SL 對 AI 效能產生不利影響，特別是對黑人患者而言，強調 SL 是 AI 模型開發中種族差異的來源。為了探索一種操作上有效的方法來減輕 SL 的影響，我們透過臨床醫生的協作網路調查 SL 產生的模式，找出核心臨床醫生對 AI 模型中的種族差異有較大的影響。我們發現移除由核心臨床醫生撰寫的 SL 是比消除資料全集中所有 SL 更有效率的偏見降低策略。這項研究為負責任的 AI 開發提供了可行的見解，並有助於了解臨床醫生行為和醫療保健中的 EHR 備註撰寫。
...
摘要：當代透過 AI 進行的自動化，需要大量的幕後人力，這些人力常常被隱形化且低薪。由於包含標記和維護工作的隱形勞動，是當代 AI 系統中不可或缺的一部分，因此讓使用者了解其角色仍然很重要。我們建議透過可解釋 AI (XAI) 設計來達成，特別是女性主義的交叉 XAI。我們提出源自女性主義交叉研究的製圖法，以提出 AI 的系統觀點，並納入與隱形勞動相關的 AI 向度。
...
摘要：虛擬心理健康助理 (VMHA) 持續進步，以支援每年有 6000 萬人次初級保健就診和 600 萬人次急診室 (ER) 就診的負擔過重的全球醫療保健系統。這些系統是由臨床心理學家、精神科醫師和人工智慧 (AI) 研究人員為認知行為療法 (CBT) 而建構。目前，VMHA 的角色是透過資訊提供情緒支持，較少著重於與患者進行反思對話。需要一種更全面、安全且可解釋的方法來建構負責任的 VMHA，以提出後續問題或提供充分資訊的回應。這項調查提供了對現有心理健康對話代理的系統性批判性回顧，接著深入探討如何透過脈絡知識、資料集和 VMHA 在臨床決策支援中新興的角色來改善 VMHA。我們也提供了新的方向，以透過可解釋性、安全性與整體可信度來豐富 VMHA 的使用者體驗。最後，我們提供了超出目前文獻的評估指標和實務考量，以在 VMHA 和患者的積極溝通中建立信任。
...
摘要：XAI 指的是建立 AI 應用程式的技術和方法，協助最終使用者解讀 AI 模型的輸出和預測。在高風險決策情況下，例如醫療領域，黑箱 AI 應用程式增加了透明度和可解釋性的需求，因為錯誤的預測可能會造成嚴重的後果。模型可解釋性和可詮釋性對於 AI 模型在醫療實務中的成功部署至關重要。AI 應用程式的底層推論需要對臨床醫生透明，才能獲得他們的信任。本文提供了醫療保健領域中 XAI 面向和挑戰的系統性回顧。本研究的主要目標是回顧各種 XAI 方法、它們的挑戰，以及醫療保健中相關的機器學習模型。這些方法分為六類進行討論：面向特徵的方法、全域方法、概念模型、代理模型、局部基於像素的方法和以人為中心的方法。最重要的是，本文探討了 XAI 在醫療保健問題中的角色，以釐清其在安全關鍵應用中的必要性。本文旨在透過回顧相關的實驗結果，建立對醫療保健領域中 XAI 相關應用程式的全面理解。為了促進未來研究填補研究差距，本文從不同的觀點探討了 XAI 模型的重要性及其限制。
...
摘要：最先进的機器學習模型通常會學習訓練資料中內嵌的虛假關聯性。這在將這些模型部署於高風險決策制定時會造成風險，例如皮膚癌偵測等醫療應用。為了解決此問題，我們提出 Reveal to Revise (R2R)，一個涵蓋整個可解釋人工智慧 (XAI) 生命週期的架構，讓從業人員能夠反覆找出、減輕和（重新）評估虛假的模型行為，且只需最少的人力互動。在第一步 (1)，R2R 透過找出歸因值中的異常值，或透過檢查模型學習到的潛在概念，來揭露模型的弱點。其次 (2)，偵測出負責的偽影並在輸入資料中進行空間定位，然後利用它來 (3) 修訂模型行為。具體來說，我們應用 RRR、CDEP 和 ClArC 的方法來進行模型修正，並 (4)（重新）評估模型的效能和對偽影的剩餘敏感度。使用兩個用於黑色素瘤偵測和骨齡估計的醫療基準資料集，我們將 R2R 架構應用於 VGG、ResNet 和 EfficientNet 架構，並藉此揭露和修正真實的資料集內在偽影，以及受控設定中的合成變異。完成 XAI 生命週期，我們示範多重 R2R 迭代以減輕不同的偏誤。程式碼可於 https://github.com/maxdreyer/Reveal2Revise 取得。
...
摘要：可解釋人工智慧 (XAI) 領域近年來進展顯著，但進展主要在於電腦視覺和自然語言處理。對於時間序列，輸入通常無法解釋，因此只有有限的研究可用於 XAI。在這項工作中，我們提出了一個虛擬檢查層，它將時間序列轉換為可解釋的表示，並允許通過局部 XAI 方法（例如逐層相關性傳播 (LRP)）將相關性歸因傳播到此表示。這樣，我們將 XAI 方法家族的適用性擴展到在轉換後才能解釋輸入的領域（例如語音）。在這裡，我們專注於傅立葉轉換，它主要應用於時間序列和 LRP 的解釋，並將我們的稱之為 DFT-LRP。我們展示了 DFT-LRP 在各種時間序列分類設定（例如音訊和電子健康記錄）中的有用性。我們展示了 DFT-LRP 如何揭示在不同領域（例如時間域與頻率域）訓練的模型的分類策略的差異，或有助於發現模型如何作用於資料中的虛假相關性。
...
摘要：解釋深度學習模型預測結果的能力對最終使用者來說非常重要，可以利用人工智慧 (AI) 的力量來進行醫療決策，這通常被認為是不透明且難以理解的。在本文中，我們採用最先進的可解釋人工智慧 (XAI) 方法來解釋黑箱 AI 模型在甲狀腺結節診斷應用中的預測結果。我們提出新的基於統計的 XAI 方法，即核密度估計和密度圖，來解釋未檢測到結節的情況。XAI 方法的效能會在定性和定量比較下被視為回饋，用於改善資料品質和模型效能。最後，我們進行調查，以評估醫生和患者對 XAI 對模型在甲狀腺結節影像上決策的解釋的信任度。
...
摘要：<paragraph>醫療設備和人工智慧系統迅速改變醫療保健的提供方式。同時，由於其本質，醫療設備中或作為醫療設備的人工智慧可能會遭受網路攻擊，進而導致患者安全和安全風險。本書章節分為三部分。第一部分從設定場景開始，說明網路安全在醫療保健中的角色。然後，當我們談論被視為醫療設備本身或支援醫療設備的人工智慧時，我們簡要定義我們的說法。為了說明此類醫療設備帶來的風險，我們提供了三個範例：資料集中毒、社會工程和資料或原始碼萃取。在第二部分，本文概述了歐盟的監管架構，與確保醫療設備中或作為醫療設備的人工智慧的網路安全相關（MDR、NIS 指令、網路安全法、GDPR、AI 法案提案和 NIS 2 指令提案）。最後，本文的第三部分探討了源自歐盟監管架構的潛在挑戰。特別是，我們展望源自兩個立法提案的挑戰，以及它們與現有關於人工智慧醫療設備網路安全的立法之間的互動。它們被建構成為下列問題的答案：(1) 人工智慧法案如何與 MDR 互動，關於網路安全和安全要求？(2) 我們應如何詮釋 NIS 2 指令提案和 MDR 的事件通知要求？(3) 關鍵基礎設施演進術語的後果是什麼？
[這是草稿章節。最終版本將收錄在 Barry Solaiman 和 I. Glenn Cohen 編輯的《健康、人工智慧和法律研究手冊》中，預計於 2023 年由 Edward Elgar Publishing Ltd 出版]</paragraph>
...
摘要：阿茲海默症 (AD) 是一種進行性神經退化疾病，也是導致失智症的主因。早期診斷對患者接受潛在干預和治療至關重要。由於視網膜與大腦具有解剖學上的連接，因此被假設為 AD 檢測的診斷部位。為此目的而開發的 AI 模型尚未對決策提供合理的解釋，也沒有推論出疾病進程的階段。沿著這個方向，我們提出了一個新的模型不可知可解釋 AI 框架，稱為顆粒神經元級解釋器 (LAVA)，一個解釋原型，它探查卷積神經網路 (CNN) 模型的中間層，以直接從視網膜影像評估 AD 連續體，而無需縱向或臨床評估。這種方法用於驗證視網膜血管作為阿茲海默症 (AD) 評估的生物標記和診斷方式。英國生物樣本庫的認知測試和血管形態特徵表明，LAVA 在識別 AD 階段的進程連續體方面顯示出強大的前景和有效性。
...
摘要：許多視覺化已開發用於可解釋 AI (XAI)，但它們通常需要使用者進一步推理才能解釋。我們主張 XAI 應支援圖解和演繹推理，以便 AI 執行假設產生和評估，以縮小可解釋性差距。我們提出圖表化以 i) 執行皮爾士演繹-演繹推理，ii) 遵循領域慣例，以及 iii) 以視覺或口頭方式用圖表解釋。我們為臨床應用實作 DiagramNet，以從心臟聽診預測心臟診斷，並用基於形狀的雜音圖表解釋。在建模研究中，我們發現 DiagramNet 不僅提供忠實的雜音形狀解釋，而且比基準模型具有更好的預測效能。我們進一步在與醫學生的定性使用者研究中展示圖解解釋的可解釋性和可信度，顯示臨床相關的圖解解釋優於技術顯著性圖解釋。這項工作有助於提供以使用者為中心的 XAI 的領域慣例演繹解釋。
...
摘要：皮膚癌是人類最常見的癌症類型之一。它的診斷主要依賴視覺，從臨床篩檢開始，接著進行皮膚鏡檢查、組織學評估和檢體收集。深度卷積神經網路 (CNN) 針對已分類的細緻物件執行高度區隔且潛在通用的任務。本研究提出一個新的多類別預測架構，用於根據 ViT 和 ViTGAN 對皮膚病灶進行分類。基於視覺轉換器的 GAN（生成對抗網路）用於解決類別不平衡問題。該架構包含四個主要階段：ViTGAN、影像處理和可解釋 AI。第 1 階段包含生成合成影像，以平衡資料集中的所有類別。第 2 階段包含應用不同的資料擴充技術和形態運算，以增加資料量。第 3 和第 4 階段涉及開發適用於邊緣運算系統的 ViT 模型，該模型可以識別圖案並對影像中可見使用者皮膚上的皮膚病灶進行分類。在第 3 階段，在使用 ViT 將病灶分類到所需的類別後，我們將使用可解釋 AI (XAI)，這將導致更具可解釋性的結果（使用活化映射等），同時確保高預測準確度。皮膚病的即時影像可以使用行動應用程式的相機由醫生或患者擷取，以進行早期檢查並確定皮膚病灶的原因。將整個架構與現有的皮膚病灶檢測架構進行比較。
...
摘要：<paragraph>在高風險的環境中部署人工智慧（AI），例如醫療保健，提供可解釋性/說明性或允許精細錯誤分析的方法至關重要。許多近期的可解釋性/說明性和精細錯誤分析方法使用概念，這些概念是對人類具有語義意義的元標籤。然而，只有少數幾個資料集包含概念層級的元標籤，而這些元標籤大多與不需要領域專業知識的自然影像相關。針對醫療的密集標註資料集著重於與單一疾病（例如黑色素瘤）相關的元標籤。在皮膚科中，皮膚疾病使用既定的臨床詞彙來描述，讓臨床醫生可以彼此描述身體檢查結果。為了提供由領域專家密集標註的醫療資料集，其標註對多種疾病過程都有用，我們開發了 SkinCon：由皮膚科醫生密集標註的皮膚疾病資料集。SkinCon 包含來自 Fitzpatrick 17k 資料集的 3230 張影像，並密集標註了 48 個臨床概念，其中 22 個概念至少有 50 張影像代表該概念。使用的概念是由兩位皮膚科醫生選定的，他們考慮了用於描述皮膚病變的臨床描述詞。範例包括「斑塊」、「鱗屑」和「侵蝕」。相同的概念也用於標註來自 Diverse Dermatology Images 資料集的 656 張皮膚疾病影像，提供了具有多樣化膚色表示的額外外部資料集。我們檢視了 SkinCon 資料集的潛在應用，例如探測模型、基於概念的說明和概念瓶頸。此外，我們使用 SkinCon 來展示這兩個使用案例：使用概念偵錯現有皮膚科 AI 模型的錯誤，以及使用事後概念瓶頸模型開發可解釋的模型。</paragraph>
...
摘要：不安分的多臂老虎機（RMAB）是一個流行的決策理論框架，
它已被用於模擬公共衛生、野生動物保護、通信系統等領域中的現實世界順序決策問題。
已部署的 RMAB 系統通常分兩個階段運行：第一階段預測定義 RMAB 實例的未知參數，第二階段採用優化演算法來解決已建構的 RMAB 實例。
在本文中，我們提供並分析了在公共衛生領域首次部署 RMAB 系統的結果，旨在改善孕產婦和兒童健康。我們的分析重點在於了解預測準確度與已部署 RMAB 系統的整體效能之間的關係。這對於確定投資於提高預測準確度以改善最終系統效能的價值至關重要，並且對於診斷、監控已部署的 RMAB 系統很有用。
使用我們已部署的 RMAB 系統中的真實世界資料，我們證明了整體預測準確度的提高甚至可能伴隨著 RMAB 系統效能的下降——廣泛投入資源來提高整體預測準確度可能無法產生預期的結果。在此之後，我們開發了以決策為中心的評估指標來評估預測組成，並表明它更能（無論是經驗上還是理論上）解釋已部署 RMAB 系統的整體效能。
...
摘要：可解释 AI 是开发机制，允许决策系统与人类交互，从而使前者的决策易于理解。这在敏感环境中尤为重要，例如在医学领域。我们提出了一个使用案例研究，用于皮肤病变诊断，说明如何为从业者提供有关最新深度神经网络分类器决策的解释，该分类器经过训练可以根据示例表征皮肤病变。我们的框架包括一个训练有素的分类器，解释模块在其上运行。后者能够为分类诊断提供示例和反例，从而允许医生与自动诊断系统进行交互。示例是通过对抗性自动编码器生成的。我们说明了系统在代表性示例上的行为。
...
摘要：人工智慧（AI）應用中的演算法公平性對於一個更美好的社會至關重要。公平性作為社會機制的基礎公理，包含多個面向。儘管機器學習（ML）社群專注於交叉性作為統計平權的問題，特別是在歧視問題上，但新興的文獻探討了另一個面向——單調性。根據領域專家，單調性在許多與公平性相關的領域中扮演至關重要的角色，其中違反會誤導人類決策並導致災難性的後果。在本文中，我們首先系統性地評估應用單調神經加法模型（MNAM）的重要性，該模型使用公平感知 ML 演算法來強制執行個人和成對單調性原則，以確保 AI 道德和社會的公平性。我們透過理論推理、模擬和廣泛的實證分析的混合方法發現，在所有公平性領域中考慮單調性公理至關重要，包括犯罪學、教育、醫療保健和金融。我們的研究有助於 AI 道德、可解釋 AI（XAI）和人機互動（HCI）介面中的跨學科研究。透過證明不符合單調性的災難性後果，我們探討了 AI 應用中單調性要求的重要性。此外，我們證明 MNAM 是一項有效的公平感知 ML 方法，透過施加單調性限制來整合人類智慧。
...
摘要：<paragraph>在安全關鍵的應用程式中，例如法律或醫療領域，解釋 AI 模型的預測至關重要。一種預測的解釋形式是提取式依據，亦即一個實例中引導模型對該實例做出預測的子集合特徵。例如，子短語「他偷了手機」可以是「竊盜」預測的提取式依據。先前關於產生提取式依據的研究通常採用兩階段模型：一個選擇器，用於選擇最重要的特徵（即依據），接著是一個預測器，用於根據所選特徵做出預測。這些研究的一個缺點是，學習選擇特徵的主要訊號來自於將預測器給出的答案與真實答案進行比較。在這項研究中，我們建議透過資訊校正方法從預測器中提取更多資訊。更精確地說，我們聯合訓練兩個模型：一個是典型的解決手邊任務的神經模型，但以準確但黑箱的方式進行，另一個是選擇器預測器模型，它還為其預測提供了依據。第一個模型用作第二個模型的指南。我們使用對抗技術來校正兩個模型提取的資訊，使得它們之間的差異成為遺漏或過度選擇的特徵的指標。此外，對於自然語言任務，我們提出了一個基於語言模型的正則化器，以鼓勵提取流暢的依據。在情緒分析任務、仇恨言論辨識任務以及法律領域的三項任務上的實驗結果顯示了我們在依據提取方面的做法的有效性。</paragraph>
...
摘要：最近通過認證的人工智慧 (AI) 工具在醫療保健領域的激增，重新引發了有關採用這項技術的辯論。其中一個辯論主題涉及可解釋 AI (XAI) 及其讓 AI 裝置更透明和值得信賴的承諾。一些活躍於醫療 AI 領域的聲音對可解釋 AI 技術的可靠性，特別是特徵歸因方法表示擔憂，並質疑其在準則和標準中的使用和納入。儘管有合理的疑慮，我們認為對事後局部可解釋性方法可行性的現有批評，將問題泛化到特定於影像資料的問題，因而因噎廢食。我們首先將問題描述為解釋與人類理解之間缺乏語義匹配。為了了解何時可以可靠地使用特徵重要性，我們區分了低層級和高層級特徵的特徵重要性。我們認為，對於低層級特徵具有明確語義的資料類型，例如電子健康記錄 (EHR) 等表格資料，可以獲得語義匹配，因此特徵歸因方法仍然可以有意義且有用的方式使用。最後，我們概述了一個程序來測試是否已達成語義匹配。
...
摘要：儘管人工智慧（AI）的機器學習（ML）模型在醫療領域中表現優異，但並非沒有錯誤。賦予臨床醫生識別錯誤模型建議的能力對於建立對醫療 AI 的信任至關重要。可解釋 AI（XAI）旨在通過釐清 AI 推理來滿足此需求，以支援最終使用者。最近，幾項關於生物醫學影像的研究取得了可喜的成果。儘管如此，使用表格資料的模型解決方案還不足以滿足臨床醫生的需求。本文提出了一種方法，以支援臨床醫生識別使用表格資料訓練的 ML 模型的失敗。我們的方法建立在三個主要支柱上：利用臨床背景潛在空間分解特徵集、評估整體解釋的臨床關聯性，以及基於潛在空間相似度 (LSS) 的局部解釋。我們在 ML 基於感染所致早產兒疾病的識別上展示了我們的技術。在這個領域中，因模型失敗而導致的死亡風險、終身殘疾和抗生素抗藥性是一個公開的研究問題。我們的方法成功識別出兩個模型的錯誤分類案例。透過將局部解釋脈絡化，我們的解決方案為臨床醫生提供了可行的見解，以支援他們自主做出明智的最終決定。
...
摘要：<paragraph>我們見證了 AI 經濟與社會的興起，其中 AI 技術日益影響醫療保健、商業、運輸和日常生活中的許多面向。許多成功案例中，AI 系統甚至超越了人類專家的準確度。然而，AI 系統可能會產生錯誤，可能表現出偏見，可能對資料中的雜訊敏感，而且通常缺乏技術和司法透明度，導致信任度降低，並對其採用造成挑戰。這些最近的缺點和疑慮已記錄在科學和一般媒體中，例如自動駕駛汽車事故、醫療保健、聘雇和有色人種的臉部辨識系統中的偏見、看似正確的醫療決策後來發現是基於錯誤的原因而做出等。這導致許多政府和法規倡議要求值得信賴且符合道德的 AI，以提供準確性和穩健性、某種形式的可解釋性、人為控制和監督、消除偏見、司法透明度和安全性。提供可信賴的 AI 系統的挑戰促使對可解釋 AI 系統 (XAI) 進行深入研究。XAI 的目標是提供人類可理解的資訊，說明 AI 系統如何做出決策。在本文中，我們首先簡要總結目前的 XAI 工作，然後挑戰準確性與可解釋性互斥且僅專注於深度學習的近期論點。然後，我們提出我們對在高風險可信賴 AI 系統交付的完整生命週期中使用 XAI 的建議，例如開發、驗證和認證，以及可信賴的生產和維護。</paragraph>
...
摘要：深度神经网络 (DNN) 可侦测资料中的模式，并在许多计算机视觉应用中展现其多功能性和强大效能。但 DNN 容易出现违反基本常识概念的明显错误，且无法使用明确知识来引导其搜寻和决策制定能力有限。虽然整体 DNN 效能指标可能良好，但这些明显的错误加上缺乏可解释性，已阻碍其广泛用于关键任务，例如医学影像分析。本文的目的是介绍 SimpleMind，一个专注于医学影像理解的认知 AI 开源软件框架。它允许建立知识库，以直觉且人类可读的形式描述影像对象预期的特性和关系。SimpleMind 框架透过下列方式为 DNN 带来思考能力：(1) 提供方法运用知识库对影像内容进行推理，例如空间推理和条件推理，以检查 DNN 输出；(2) 应用流程知识，以通用软件代理的形式，将它们串连在一起以完成影像前处理、DNN 预测和结果后处理；(3) 对所有知识库参数进行自动共同优化，以使代理适应具体问题。SimpleMind 能对多个侦测到的对象进行推理以确保一致性，并在 DNN 输出之间提供交叉检查。这种机器推理透过可解释的模型和可解释的决策，提升 DNN 的可靠性和可信度。本文提供了范例应用，说明 SimpleMind 如何透过将深度神经网络嵌入在认知 AI 框架中，提供支持并对其进行改善。
...
摘要：在過去十年中，基於深度學習方法的視覺感知任務有了驚人的進展，並在從自動駕駛到臨床決策支援系統等眾多應用領域中得到廣泛採用。儘管這些基於深度學習的模型表現令人印象深刻，但它們在決策過程中仍然相當不透明，這使得它們在人類關鍵任務中的部署成為一項冒險的事業。這反過來又使得理解這些模型所做的決策對於它們的可靠部署至關重要。可解釋 AI (XAI) 方法試圖通過為這種黑箱深度學習方法提供解釋來解決這個問題。在本文中，我們對計算機視覺中的基於歸因的 XAI 方法進行了全面調查，並回顧了基於梯度、基於擾動和基於對比的 XAI 的現有文獻，並提供了關於開發和評估穩健 XAI 方法的關鍵挑戰的見解。
...
摘要：自10年代中期以来，深度学习（DL）时代持续至今，每年都会带来新的最高级和创新。然而，这些创新转化为实际应用的速度落后于这一快节奏。特别是安全关键型应用程序，其基础是严格的法规和道德要求，需要予以关注，并且仍然是争论的活跃领域。可解释AI（XAI）和隐私保护机器学习（PPML）都是至关重要的研究领域，旨在减轻DL中流行的数据饥渴黑盒模型的一些缺点。尽管在各自领域的研究活动活跃，但尚未关注它们之间的交互。这项工作是第一个研究隐私学习技术对基于DL的模型生成的解释的影响。在涵盖多个域的各种图像和时间序列数据集以及不同的隐私技术、XAI方法和模型架构的广泛实验分析中，研究了隐私训练对生成解释的影响。研究结果表明，通过引入隐私，解释发生了不可忽略的变化。除了报告PPML对XAI的个别影响外，本文还对实际应用中技术的选用给出了明确的建议。通过揭示这些关键技术的相互依存性，这项工作是克服安全关键领域中实用AI的剩余障碍的第一步。
...
摘要：肺結核（TB）是一種傳染性細菌疾病，是造成死亡的主因，特別是在低收入國家，估計在 2020 年全球通報了一千萬例新病例。雖然肺結核可以治療，但未遵守藥物治療計畫是造成發病率和死亡率高的主因。因此，主動找出有中斷藥物治療計畫風險的患者，可以採取矯正措施來減輕不良後果。我們使用極端不遵守治療的代理衡量指標和來自印度四個邦近 70 萬名患者的資料集，制定並解決機器學習 (ML) 問題，根據自訂的基於排名指標來預測不遵守治療。我們訓練 ML 模型並根據基準進行評估，相較於基於規則的基準，提升了約 100%，相較於隨機分類器，提升了約 214%，考量了全國性的未來大規模部署。在此過程中，我們處理了各種問題，包括資料品質、高基數類別資料、低目標患病率、分佈轉移、不同群組間的變異、演算法公平性，以及對健壯性和可解釋性的需求。我們的研究結果顯示，不遵守治療患者的風險分層是一種可行的、可大規模部署的 ML 解決方案。作為印度中央肺結核部門的官方 AI 合作夥伴，我們正與多個城市和邦級試點計畫合作，目標是全印度部署。
...
摘要：可解釋人工智慧 (XAI) 透過提升使用者對機器信任感，轉化人工智慧 (AI) 領域。隨著連網裝置數量持續增加，物聯網 (IoT) 市場需要獲得使用者的信任。然而，現有文獻仍缺乏有關 XAI 在物聯網中使用的系統性且全面的調查研究。為了彌補這個缺口，我們在本文中探討 XAI 架構，重點在於其特性和對物聯網的支援。我們說明廣泛使用的 XAI 服務，用於物聯網應用，例如安全性提升、醫療物聯網 (IoMT)、工業物聯網 (IIoT) 和城市物聯網 (IoCT)。我們也建議在這些應用中，透過適當的範例在物聯網系統中實作 XAI 模型，並總結未來工作的關鍵推論。此外，我們展示邊緣 XAI 結構的尖端發展，以及第六代 (6G) 通訊服務對物聯網應用的支援，並附上關鍵推論。簡而言之，本文構成第一個針對 XAI-based 架構發展的整體彙編，專門針對未來物聯網使用案例的需求。
...
摘要：美國每年約有 125 萬人因燒傷而接受治療。精確的燒傷分類是醫療 AI 領域中一個重要的面向。在這項工作中，我們提出一個可解釋的人工迴路架構，用於改善燒傷超音波分類模型。我們的架構利用基於 LIME 分類說明器說明系統，來驗證和整合燒傷專家的知識，建議新功能並確保模型的有效性。使用此架構，我們發現 B 型超音波分類器可以透過提供紋理特徵來增強。更具體地說，我們確認基於超音波影像灰階共生矩陣 (GLCM) 的紋理特徵，可以提高轉移學習燒傷深度分類器的準確度。我們在豬隻受試者的真實資料上測試我們的假設。我們展示了燒傷深度分類的準確度提升，從約 88% 提升至約 94%，一旦根據我們的架構進行修改。
...
摘要：在醫療保健領域，AI 系統支援臨床醫師和患者進行診斷、治療和監控，但許多系統的解釋性不佳，對於實際應用而言仍然具有挑戰性。克服這個障礙是可解釋 AI (XAI) 的目標。然而，解釋可以被不同地感知，因此無法為所有人解決黑盒子問題。以人為中心的 AI 領域透過調整 AI 來適應使用者來處理這個問題。我們提出以使用者為中心的角色概念來評估 XAI，並使用它來調查行動健康壓力監控應用程式中各種解釋風格和內容的最終使用者偏好。我們的線上調查結果顯示使用者的背景資料和個性，以及解釋類型，會影響解釋偏好，這表示這些是 XAI 設計的基本特徵。我們將結果歸納為三種原型使用者角色：重視權力、休閒和隱私的使用者。我們的見解讓互動式、以人為中心的 XAI 更接近實際應用。
...
摘要：青光眼是所有視覺缺陷中導致部分或完全失明的第二大原因，主要是由於焦慮或憂鬱導致眼睛壓力過大，損害視神經並造成視力問題。傳統的青光眼篩檢是一個耗時的過程，需要醫療專業人員持續的關注，即使如此，由於時間限制和壓力，他們有時無法正確分類，導致治療錯誤。已經做出許多努力來自動化整個青光眼分類程序，然而，這些現有模型通常具有黑盒特性，這會讓使用者無法了解預測背後的主要原因，因此醫療從業人員通常無法依賴這些系統。在本文中，我們在比較了各種預訓練模型後，提出了一個能夠以 94.71% 的準確度對青光眼進行分類的遷移學習模型。此外，我們利用了局部可解釋模型不可知解釋（LIME），這為我們的系統引入了可解釋性。這項改進使醫療專業人員能夠獲得重要且全面的資訊，以幫助他們做出判斷。它也降低了傳統深度學習模型的不透明性和脆弱性。
...
摘要：深度學習在偵測物件和分類影像方面展現了極佳的效能，確保了分析醫學影像的絕佳前景。
將深度學習的成功轉化為醫學影像，其中醫生需要了解基礎的過程，需要有能力詮釋和說明神經網路的預測。深度神經網路的可解釋性通常依賴於估計輸入特徵（例如像素）相對於結果（例如類別機率）的重要性。然而，已經開發了許多重要性估計器（也稱為顯著性圖），目前尚不清楚哪些對於醫學影像應用更相關。在目前的工作中，我們使用三個不同的評估指標，探討了幾個重要性估計器在說明卷積深度網路對電腦斷層掃描（CT）影像進行分類的效能。首先，以模型為中心的保真度衡量在特定輸入受到擾動時模型準確度的下降。其次，在像素層級上，使用受試者操作特性（ROC）曲線來衡量重要性分數和專家定義的分割遮罩之間的一致性。第三，我們使用 Dice 相似性係數（DSC）來衡量 XRAI-based 地圖和分割遮罩之間的區域重疊。總體而言，兩個版本的 SmoothGrad 在保真度和 ROC 排名中名列前茅，而 Integrated Gradients 和 SmoothGrad 在 DSC 評估中表現出色。有趣的是，以模型為中心的（保真度）和以人為中心的（ROC 和 DSC）評估之間存在著嚴重的差異。分割地圖中嵌入的專家期望和直覺並不一定與模型如何得出其預測相符。了解可解釋性的這種差異將有助於利用深度學習在醫學上的力量。
...
摘要：深度學習方法，特別是卷積神經網路，已成為醫學影像運算任務中強大的工具。雖然這些複雜的模型提供了極佳的效能，但其黑箱性質可能會阻礙在高風險決策中採用於真實世界。在本文中，我們提出一個互動系統，利用最先進的可解釋性技術，協助放射科醫師進行乳癌篩檢。我們的系統將深度學習模型整合到放射科醫師的工作流程中，並提供新穎的互動方式，以促進對模型決策過程的理解。此外，我們證明我們的系統可以逐漸利用使用者互動，以提供更細緻的可解釋性報告，而標記工作量很小。由於所採用的可解釋性技術的通用性質，我們的系統與領域無關，可用於許多不同的醫學影像運算任務，對我們如何利用視覺分析將原本靜態的可解釋性技術轉換為擴增人類決策制定並促進採用醫療 AI，提出了一個新觀點。
...
摘要：如今，人工智能 (AI) 已成為醫療保健應用程式，無論是臨床或遠程醫療，的基本組成部分，但效能最佳的 AI 系統通常過於複雜，無法自行解釋。可解釋 AI (XAI) 技術被定義為揭示系統預測和決策背後的推理，且在處理敏感且個人的健康資料時，其變得更加重要。值得注意的是，XAI 並未在不同的研究領域和資料類型中獲得相同的關注，特別是在醫療保健方面。具體來說，許多臨床和遠程醫療應用分別基於表格和時間序列資料，而 XAI 通常不會在這些資料類型上進行分析，而電腦視覺和自然語言處理 (NLP) 是參考應用程式。為了概述最適合醫療保健領域中表格和時間序列資料的 XAI 方法，本文回顧了過去 5 年的文獻，說明產生的解釋類型以及評估其相關性和品質所付出的努力。具體來說，我們將臨床驗證、一致性評估、客觀且標準化的品質評估以及以人為中心的品質評估確定為確保對最終使用者提供有效解釋的主要特徵。最後，我們重點說明了該領域的主要研究挑戰以及現有 XAI 方法的限制。
...
摘要：<paragraph>目的：本研究評估了一種市售的可解釋 AI 演算法，用於增強臨床醫生在胸部 X 光片 (CXR) 上識別肺癌的能力。
設計：這項回顧性研究評估了 11 位臨床醫生從胸部 X 光片檢測肺癌的表現，有和沒有市售 AI 演算法（紅點，Behold.ai）的協助，該演算法會從 CXR 預測疑似肺癌。臨床醫生表現與臨床確診結果進行比對評估。
背景：本研究分析了英國國家醫療服務體系 (NHS) 醫院的匿名患者資料；資料集包含 400 張成人患者（18 歲以上）的胸部 X 光片，他們在 2020 年進行了 CXR 檢查，並有對應的臨床文字報告。
參與者：由 11 位臨床醫生（顧問放射科醫生、放射科醫生受訓人員和報告放射技師）組成的讀者小組參與了這項研究。
主要結果指標：臨床醫生在 CXR 上檢測肺癌的整體準確度、敏感度、特異度和精準度，有和沒有 AI 輸入。臨床醫生之間的一致性比率和效能標準差也進行了評估，有和沒有 AI 輸入。
結果：臨床醫生使用 AI 演算法，改善了肺部腫瘤檢測的整體表現，整體增加了 17.4% 的肺癌在 CXR 上被識別，否則這些肺癌將會被遺漏，整體增加了較小腫瘤的檢測率，第一期和第二期肺癌的檢測率分別增加了 24% 和 13%，並標準化了臨床醫生的表現。
結論：本研究顯示 AI 演算法在臨床應用中極具前景，可以透過整體改善讀者表現來促進早期肺癌診斷和促進健康公平，同時不影響下游影像資源。</paragraph>
...
摘要：缺乏足够大的開放醫療資料庫是 AI 驅動的醫療保健中最大的挑戰之一。使用生成對抗網路 (GAN) 所建立的合成資料，似乎可以緩解隱私政策的問題，是一個很好的解決方案。另一種治療方法是在多個醫療機構中採用分散式協定，而不交換本地的資料樣本。在本文中，我們探討了集中式和分散式設定中的無條件和條件 GAN。集中式設定模擬了大型但高度不平衡的皮膚病變資料集的研究，而分散式設定則模擬了三個機構中更實際的醫院場景。我們在保真度、多樣性、訓練速度和在生成的合成資料上訓練的分類器的預測能力方面，評估了模型的效能。此外，我們透過探索潛在空間和嵌入投影來提供可解釋性，重點放在全局和局部解釋上。計算出的真實影像與其在潛在空間中的投影之間的距離，證明了經過訓練的 GAN 的真實性和概化性，這類型的應用程式中，這是其中一項主要考量。已進行研究的開放原始碼可在
...
摘要：在醫院的世界裡有許多複雜的組合問題，解決這些問題對於提升病患滿意度與照護品質非常重要。醫療保健中的問題很複雜，因為要解決這些問題必須考量到許多限制與不同類型的資源。此外，必須在短時間內評估解決方案，以確保在實際情況中可用。我們計畫提出解決這些問題的方案，包括擴展已經測試過的方案，以及為新問題建模解決方案，考量文獻，並在可行的情況下使用實際資料。解決這些問題很重要，但由於歐盟執委會制定一般資料保護法規，規定每個人都有權要求 AI 做出的決定提供說明，因此在未發展出可解釋性方法的情況下，基於 AI 的求解器（例如基於答案設定程式設計的求解器）的使用將受到限制。因此，研究的另一部分將致力於研究並提出新的方法，說明所獲得的解決方案。
...
摘要：在現代動態不斷發展的社會中，越來越多的人罹患慢性及重症疾病，醫生和患者需要特別且精密的醫療和健康支持。因此，傑出的醫療保健利害關係人已認知到此類服務開發的重要性，以讓患者的生活更輕鬆。此類支持需要收集大量的患者複雜資料，例如臨床、環境、營養、日常活動、各種智慧型穿戴式裝置資料、配備感測器的服裝資料等。整體患者資料必須妥善彙整、處理、分析，並呈現給醫生和照護人員，以建議適當的治療和行動，以改善患者的健康相關參數和整體健康狀況。進階的人工智慧技術提供機會來分析這些大數據，使用它們，並衍生新的知識以支持個人化的醫療決策。基於進階機器學習、聯盟學習、遷移學習、可解釋人工智慧等新方法為未來更優質地使用健康和醫療資料開啟了新途徑。在本文中，我們將介紹一系列人工智慧方法在個人化醫療決策應用領域的一些關鍵面向和特徵範例。
...
摘要：<paragraph>持續監控訓練過的機器學習模型，以確定何時應信任或不信任其預測，對於安全部署而言至關重要。這樣的框架應該具有高性能、可解釋性、事後性和可操作性。我們提出 TRUST-LAPSE，這是一個用於持續模型監控的「不信任」評分框架。我們使用一系列潛在空間嵌入來評估每個輸入範例的模型預測的可信度。具體來說，(a) 我們的潛在空間不信任分數使用潛在空間中的距離度量（馬氏距離）和相似度量（餘弦相似度）來估計不信任，(b) 我們的順序不信任分數確定在過去輸入表示序列中相關性的偏差，用於可操作的持續監控的非參數滑動視窗演算法。我們透過兩個下游任務評估 TRUST-LAPSE：(1) 分佈式偏移輸入偵測，以及 (2) 資料漂移偵測。我們使用公開資料集評估各種領域（音訊和視覺），並進一步針對具有挑戰性的真實世界腦電圖 (EEG) 資料集評估我們的做法，以進行癲癇偵測。我們的潛在空間不信任分數達到了最先進的結果，AUROC 分別為 84.1（視覺）、73.9（音訊）和 77.1（臨床 EEG），優於基線超過 10 個百分點。我們揭露了流行基線中的嚴重失敗，這些基線對輸入語義內容不敏感，使它們不適合於現實世界的模型監控。我們表明，我們的順序不信任分數達到了很高的漂移偵測率；超過 90% 的串流顯示所有領域的誤差小於 20%。透過廣泛的定性和定量評估，我們表明我們的信任分數更健全，並提供可解釋性，以便於實務採用。</paragraph>
...
摘要：機器學習的普及提高了不公平模型在高風險應用程式中部署的風險，例如司法系統、藥物/疫苗設計和醫療診斷。儘管有從頭訓練公平模型的有效方法，但如何自動揭露和解釋訓練模型的不公平性仍然是一項具有挑戰性的任務。以可解釋的方式揭露機器學習模型的不公平性是邁向公平且值得信賴的人工智慧的關鍵一步。在本文中，我們系統性地解決了通過挖掘可解釋證據（RUMIE）來揭露不公平模型的新任務。其關鍵思想是以一組受模型最大程度區別的資料實例的形式，找到確鑿的證據。為了使證據具有可解釋性，我們還找到了一組人類可理解的關鍵屬性和決策規則，它們表徵了被區別的資料實例，並將它們與其他未被區別的資料區分開來。正如在許多真實世界資料集上進行的廣泛實驗所證明的那樣，我們的模型找到了高度可解釋且確鑿的證據，以有效地揭示訓練模型的不公平性。此外，它比所有基線方法的可擴充性都要高得多。
...
摘要：可解釋 AI (XAI) 在安全關鍵系統中是必要的，例如在臨床診斷中，因為致命決策的風險很高。然而，目前 XAI 就像是一堆鬆散的方法，而不是一個定義良好的流程。在這項工作中，我們闡述了 XAI 最大子群體，可解釋機器學習 (IML) 與古典統計之間的概念相似性。基於這些相似性，我們提出 IML 的形式化，類似於統計流程。採用這種統計觀點讓我們能夠將機器學習模型和 IML 方法解釋為複雜的統計工具。基於這種解釋，我們推論出三個關鍵問題，我們認為這對 IML 在安全關鍵設定中的成功和採用至關重要。透過制定這些問題，我們進一步旨在引發一場討論，說明 IML 與古典統計之間的區別，以及我們的觀點對該領域的未來有何影響。
...
摘要：人工智能已普及到各個學科和領域，生物醫學影像和訊號處理也不例外。對此主題日益增長且廣泛的興趣引發了大量的研究活動，這反映在指數級的研究工作中。透過研究大量且多樣化的生物醫學資料，機器和深度學習模型徹底改變了各種任務，例如建模、分割、配準、分類和合成，其表現優於傳統技術。然而，難以將結果轉化為生物學/臨床可解釋的資訊，這阻礙了它們在該領域的充分利用。可解釋 AI (XAI) 嘗試透過提供方法來使模型可解釋並提供說明，從而填補這種轉譯差距。迄今為止已提出不同的解決方案，並越來越引起社群的興趣。本文旨在提供生物醫學資料處理中 XAI 的概述，並指出 IEEE 訊號處理雜誌中即將推出的生物醫學影像和訊號處理深度學習特刊，該特刊將於 2022 年 3 月出刊。
...
摘要：在使用醫學影像進行診斷時，無論是由臨床醫師或人工智慧 (AI) 系統，影像品質都很重要。當影像品質不佳時，產生該影像的醫療檢查通常需要重新執行。在遠距醫療中，一個常見的問題是，只有在患者離開診所後才會標示出品質問題，這表示他們必須回來重新進行檢查。這對居住在偏遠地區的人來說尤其困難，而他們佔巴西一家數位醫療保健組織 Portal Telemedicina 患者的很大一部分。在本文中，我們報告了關於 (i) 開發一種 AI 系統，用於標示和說明低品質醫學影像的即時工作，(ii) 一項訪談研究，以了解在 OurCompany 使用 AI 系統的利害關係人對說明的需求，以及 (iii) 一項縱向使用者研究設計，用於檢查在我們的診所中，說明會對技術人員的工作流程產生什麼影響。據我們所知，這將是第一個評估 XAI 方法對最終使用者（使用 AI 系統但沒有 AI 專業知識的利害關係人）的影響的縱向研究。我們歡迎對我們的實驗設置提出回饋和建議。
