...
摘要：<paragraph>最近的人工智慧突破，將從根本上提升我們對醫療保健的研究和理解。開發一個整合性的多對多架構，利用多種資料模式輸入，用於多種醫療任務的分析建模，對於統一理解現代醫學至關重要。在這項工作中，我們介紹 M3H，一個可解釋的多模式多任務機器學習醫療保健架構，它整合了廣泛醫療任務類別和機器學習問題類別中來自不同多模式輸入的學習。這個架構的模組化設計確保了其資料處理、任務定義和快速模型建模的可概化性，適用於臨床和運作醫療保健環境。我們透過驗證從四種模式（表格、時間序列、語言和視覺）訓練的模型，在四種機器學習問題類別中執行 41 項醫療任務，來評估 M3H 架構。我們的結果表明，M3H 持續產生多任務模型，其表現優於正規單任務模型（1.1-37.2%），涵蓋來自 16 個醫療部門的 37 種疾病診斷、三個醫院運作預測和一個患者表型化任務：跨越監督二元分類、多類分類、回歸和聚類的機器學習問題類別。此外，這個架構引入了一種新的注意力機制，以平衡自我開發（專注於學習來源任務）和交叉探索（鼓勵從其他任務中學習）。此外，M3H 提供了可解釋性的見解，說明額外任務的聯合學習如何透過提議的 TIM 分數影響來源任務的學習，進而闡明任務相互依賴的動態。其可適應的架構促進了自訂化和整合，使其成為未來人工智慧驅動醫療保健系統的強大且可擴充的候選解決方案。</paragraph>
...
摘要：本文探討了應用群體結構多主體系統 (MAS) 來建立醫療必要性，這是一個涉及根據臨床指南對特定患者的醫療結構化和非結構化資料進行系統性審查的流程。我們透過將此複雜任務分解為較小、更易於管理的子任務來解決此問題。每個子任務都由一個專門的 AI 主體處理。我們對各種提示策略對這些主體的影響進行系統性研究，並對不同的大型語言模型 (LLM) 進行基準測試，以確定它們在完成這些任務時的準確性。此外，我們探討了這些主體如何提供可解釋性，從而增強系統內的信任和透明度。
...
摘要：人工智慧 (AI) 的普遍整合在涉及 AI 啟用系統的事件中，引入了責任和問責的複雜挑戰。這些系統的互連性、AI 引發事件的道德考量，加上 AI 技術的不確定性和缺乏相應的法規，使得傳統責任歸屬具有挑戰性。為此，本研究提出一個運算反思均衡 (CRE) 的方法，為所有利害關係人建立一個連貫且在道德上可接受的責任歸屬架構。運算方法提供了一個結構化的分析，克服了概念方法在處理動態且多面向情境時的限制，展示了該架構在責任歸屬過程中可解釋性、連貫性和適應性等特性。我們探討了與均衡運算中索賠相關的初始啟動層級的關鍵角色。使用 AI 輔助醫療決策支援系統作為案例研究，我們說明了不同的初始化如何導致不同的責任分配。該架構提供了對 AI 引發事件中問責制的寶貴見解，透過持續監控、修訂和反思，促進永續且有韌性的系統的發展。
...
摘要：人工智慧透過預測模型協助醫療保健專業人員，大幅轉變臨床決策。本研究探討在醫療保健中使用 AI 應用程式時，公平性和可解釋性的關鍵需求，以確保不同病患人口統計資料的結果公平。我們專注於敗血症相關死亡率的預測模型，提出一個方法來學習效能最佳化的預測模型，然後採用轉移學習的過程，產生一個公平性更好的模型。我們的模型也引進一個新的基於置換特徵重要性演算法，旨在闡明每個特徵在提升預測公平性中的貢獻。與現有的可解釋性方法專注於解釋特徵對預測效能的貢獻不同，我們提出的方法獨特地彌補了理解每個特徵如何對公平性做出貢獻的差距。這項進展至關重要，因為敗血症的死亡率很高，而且在三分之一的醫院死亡中扮演角色。我們的模型不僅有助於找出並減輕預測模型中的偏誤，也透過改善模型預測的透明度和公平性，培養醫療保健利害關係人之間的信任，進而促成更公平、更值得信賴的醫療保健服務。
...
摘要：<paragraph>憂鬱症是現今社會中一個重要的議題。根據世界衛生組織（WHO）的統計，在2023年，超過2.8億人正與憂鬱症搏鬥。這是一個龐大的數字；如果我們不嚴肅以待，這些數字將會快速增加。大約有48.9億人是社群媒體的使用者。人們在推特、臉書、Reddit、Instagram等平台上表達自己的感受與情緒。這些平台包含了有價值的資訊，可用於研究目的。許多研究已經在各種社群媒體平台上進行。然而，這些努力中仍然存在著一些限制。特別是，先前的研究僅專注於在推文中偵測憂鬱症和憂鬱症的嚴重程度。此外，在資料集標記中也存在著不準確的情況。在這項研究中，我們使用基於詞彙標記的推特資料庫中的推文，預測了五種類型的憂鬱症（躁鬱症、重度憂鬱症、精神病性憂鬱症、非典型憂鬱症和產後憂鬱症）。我們使用可解釋的AI，透過強調代表憂鬱症類型的推文部分，來提供推理。我們使用轉換器中的雙向編碼器表示（BERT）進行特徵萃取和訓練。我們使用機器學習和深度學習方法來訓練模型。BERT模型呈現出最有希望的結果，達到了0.96的整體準確度。</paragraph>
...
摘要：深度學習大幅改變了醫學影像和放射科領域，能夠辨識醫學影像中的病理，包括電腦斷層掃描 (CT) 和 X 光掃描。然而，深度學習模型的效能，特別是在區塊化任務中，通常受到廣泛註解資料集需求的限制。為了應對這項挑戰，透過可解釋 AI 和反事實解釋的產生，探索了弱監督語意區塊化的功能。這項研究的範圍是開發一種新穎的反事實內插方法 (COIN)，該方法透過使用生成模型將預測的分類標籤從異常翻轉為正常。例如，如果分類器將輸入醫學影像 X 視為異常，表示存在病理，則生成模型旨在內插異常區域，從而逆轉分類器的原始預測標籤。這種方法使我們能夠為病理產生精確的區塊化，而無需依賴預先存在的區塊化遮罩。至關重要的是，利用影像級別標籤，其比建立詳細的區塊化遮罩容易取得。該方法的有效性透過區塊化合成目標和愛沙尼亞塔爾圖大學醫院取得的 CT 影像中的實際腎臟腫瘤得到證明。研究結果表明，COIN 遠遠優於已建立的歸因方法，例如 RISE、ScoreCAM 和 LayerCAM，以及 Singla 等人提出的另一種反事實解釋方法。這些證據表明，COIN 是一種有前景的 CT 影像中腫瘤語意區塊化方法，並代表了在醫療保健領域中使深度學習應用更易於取得且更有效的一大進步，在醫療保健領域中，註解資料很稀少。
...
摘要：甲狀腺癌是一個日益嚴重的全球健康問題，需要進階的診斷方法。本篇綜述探討了人工智慧和放射特徵在甲狀腺癌診斷中的應用。在符合 PRISMA 指南的前提下，我們檢視了多個資料庫，截止時間為 2023 年 10 月。透過結合多個關鍵字，我們發現了與甲狀腺癌和相關主題相關的英文學術出版品。在移除 109 篇重複文獻後，原始搜尋結果回傳了 267 篇論文。在根據預先確定的標準檢視了 124 篇論文的摘要和標題後，我們篩選出相關的研究。在進行全面分析後，我們額外排除六篇研究。在 28 篇納入的研究中，結合超音波 (US) 影像的放射特徵分析證實了其在甲狀腺癌診斷中的有效性。我們注意到各種結果，其中一些研究提出了優於現狀的新策略。文獻強調了人工智慧模型面臨的各種挑戰，包括可解釋性問題、資料集限制和操作員依賴性。28 篇納入研究的綜合發現提到，需要標準化工作和前瞻性多中心研究來解決這些問題。此外，我們也找出克服這些障礙的方法，例如可解釋人工智慧技術和個人化醫療技術的進步。本綜述重點探討人工智慧和放射特徵如何轉變甲狀腺癌的診斷和治療。儘管存在挑戰，未來針對多學科合作、臨床適用性驗證和演算法改進的研究，仍有潛力改善甲狀腺癌治療的患者預後和診斷精準度。
...
摘要：乳癌近年來盛行率急速上升，使其成為全球主要的死亡原因之一，在所有癌症中，乳癌更是遙遙領先。手動診斷此疾病需要大量時間和專業知識。由於乳癌的檢測過程耗時，透過建立機器學習預測模型可以協助預防其進一步擴散。機器學習和可解釋人工智慧在分類中至關重要，因為它們不僅提供準確的預測，還能深入了解模型如何做出決策，有助於理解和信任分類結果。在這項研究中，我們評估並比較了五種不同的機器學習方法的分類準確度、精確度、召回率和 F-1 分數，使用了一個主要資料集（來自達卡醫學院附設醫院的 500 位患者）。已經使用了五種不同的監督式機器學習技術，包括決策樹、隨機森林、邏輯迴歸、樸素貝氏和 XGBoost，以在我們的資料集上獲得最佳結果。此外，本研究將 SHAP 分析應用於 XGBoost 模型，以解釋模型的預測並了解每個特徵對模型輸出的影響。我們比較了幾種演算法對資料進行分類的準確度，並與該領域的其他文獻進行了對比。經過最後評估，本研究發現 XGBoost 達到了最佳模型準確度，為 97%。
...
摘要：深度學習 (DL) 模型用於從乳房攝影圖像診斷乳癌時，通常會像「黑盒子」一樣運作，這使得醫療保健專業人員難以信賴和理解其決策過程。本研究提出一個整合架構，結合卷積神經網路 (CNN) 和可解釋人工智慧 (XAI)，以使用 CBIS-DDSM 資料集增強乳癌診斷。該方法包含一個精密的資料前處理管道和進階資料擴充技術，以對抗資料集限制，並使用預先訓練的網路（例如 VGG-16、Inception-V3 和 ResNet）進行遷移學習。我們研究的重點是評估 XAI 在解釋模型預測中的有效性，重點在於利用豪斯多夫測度來評估 AI 生成的解釋和專家註解之間的對齊程度。這種方法對於 XAI 在促進 AI 輔助診斷中的可信度和倫理公平性至關重要。我們研究的發現說明了 CNN 和 XAI 在推進乳癌診斷方法中的有效協作，從而促進先進 AI 技術在臨床環境中的更順暢整合。透過增強 AI 驅動決策的可解釋性，這項工作為 AI 系統和醫療從業人員之間的改進協作奠定了基礎，最終豐富了患者照護。此外，我們研究的影響遠遠超出了目前的技術。它鼓勵進一步研究如何結合多模式資料和改進 AI 解釋以滿足臨床實務的需求。
...
摘要：以人为本的可解释人工智能 (HCXAI) 倡导将社会层面整合到人工智能解释中。HCXAI 话语的核心是社会透明度 (ST) 框架，旨在让用户能够了解人工智能系统的社会组织背景。在这项工作中，我们建议扩展 ST 框架以解决大型语言模型 (LLM) 中社会错误归因的风险，尤其是在心理健康等敏感领域。事实上，LLM 能够出色地模拟角色和角色，这可能导致设计者的意图与用户对社会属性的感知之间出现错配，从而有引发情感操纵和危险行为、认识不公正和不合理信任的风险。为了解决这些问题，我们建议用第五个“W 问题”来增强 ST 框架，以阐明设计者和用户赋予 LLM 的具体社会属性。此补充旨在弥合 LLM 能力和用户感知之间的差距，促进基于 LLM 的技术的道德负责开发和使用。
...
摘要：<paragraph>背景：氣胸是一種由肺部和胸壁之間異常空氣聚集所引起的急性胸腔疾病。為了解決深度學習 (DL) 模型經常出現的不透明性，已引入可解釋人工智能 (XAI) 方法來概述與 DL 模型做出的氣胸診斷相關的區域。然而，這些解釋有時會偏離實際病灶區域，凸顯了進一步改進的必要性。方法：我們提出了一種模板引導方法，將氣胸的臨床知識整合到 XAI 方法生成的模型解釋中，從而提高這些解釋的品質。利用放射科醫師建立的一個病灶描繪，我們的做法首先產生一個模板，代表氣胸可能發生區域。然後將此模板疊加在模型解釋上，以過濾掉超出模板邊界的無關解釋。為了驗證其效能，我們對三個 XAI 方法在解釋兩個真實世界資料集中的兩個 DL 模型時，進行了有和沒有我們的模板指導的比較分析。結果：所提出的方法在建立在三個 XAI 方法、兩個 DL 模型和兩個資料集上的十二個基準情境中，持續改善基準 XAI 方法。在比較模型解釋和真實病灶區域時，透過基準效能的效能改進計算的平均增量百分比，在交集並集 (IoU) 中為 97.8%，在骰子相似性系數 (DSC) 中為 94.1%。結論：在氣胸診斷的背景下，我們提出了一種模板引導的方法來改善 AI 解釋。我們預期我們的模板指導將透過整合臨床領域專業知識，為闡明 AI 模型建立一種新的方法。</paragraph>
...
摘要：<paragraph>在當前的機器翻譯 (MT) 領域中，Transformer 架構以其黃金標準的地位脫穎而出，特別是對於高資源語言對。本研究探討其對低資源語言對的效用，包括英語$\leftrightarrow$愛爾蘭語和英語$\leftrightarrow$馬拉地語語言對。值得注意的是，本研究識別出最佳超參數和子詞模型類型，以顯著提升 Transformer 模型對低資源語言對的翻譯品質。
  低資源語言的平行資料集的稀少性可能會阻礙 MT 的發展。為了解決此問題，開發了 gaHealth，這是愛爾蘭語的第一個健康資料雙語語料庫。專注於健康領域，使用此領域內資料集開發的模型在 BLEU 分數方面表現出非常顯著的提升，與 LoResMT2021 共享任務中的模型相比更是如此。隨後使用多維品質指標錯誤分類法進行的人工評估顯示，與基於 RNN 的對應模型相比，Transformer 系統在減少準確性和流暢性錯誤方面表現出卓越的效能。
  此外，本論文介紹了 adaptNMT 和 adaptMLLM，這兩個開源應用程式專門用於神經機器翻譯模型的開發、微調和部署。這些工具大幅簡化了設定和評估流程，讓開發人員和翻譯人員都能更輕鬆地使用 MT。值得注意的是，建立在 OpenNMT 生態系統中的 adaptNMT 通過強調模型開發的環境足跡，促進了生態友善的自然語言處理研究。與 LoResMT2021 共享任務中的基準線相比，adaptMLLM 對 MLLM 的微調展示了英語$\leftrightarrow$愛爾蘭語和英語$\leftrightarrow$馬拉地語這兩個低資源語言對的翻譯效能提升。</paragraph>
...
摘要：糖尿病 (DM) 使患者易患血管并发症。
视网膜图像和脉管系统反映了人体的微观和宏观血管健康状况。它们可用于诊断 DM 并发症，包括糖尿病视网膜病变 (DR)、神经病变、肾病和动脉粥样硬化性心血管疾病，以及预测心血管事件的风险。为使用数字化视网膜图像的高通量 DR 检测而开发的人工智能 (AI) 启用系统已在临床上得到采用。除了 DR 筛查外，AI 集成还具有巨大的潜力来应对与 DM 患者整体护理相关的挑战。在这项工作中，我们的目标是全面审查有关基于视网膜图像的 AI 应用的研究文献，这些图像与 DM 诊断、预后和管理相关。我们将描述整体 AI 辅助糖尿病护理的发现，包括但不限于 DR 筛查，并讨论实施此类系统的障碍，包括涉及伦理、数据隐私、公平获取和可解释性的问题。通过评估患者的健康状况与 DM 并发症以及未来心血管并发症的风险预后，AI 辅助视网膜图像分析有可能成为 DM 患者现代个性化医疗的核心工具。
...
摘要：本研究從多個利益相關者的觀點探討了在教育中使用不同的人工智慧 (AI) 應用程式的可接受性，包括學生、教師和家長。承認 AI 在教育中的轉型潛力，它解決了與資料隱私、AI 代理、透明度、可解釋性和 AI 的道德部署相關的問題。透過小插曲方法，向參與者展示了四種場景，其中 AI 的代理、透明度、可解釋性和隱私受到操縱。在每個場景之後，參與者完成了一項調查，調查捕捉了他們對 AI 的整體效用、個人效用、公正性、信心、風險以及在有機會的情況下使用每個場景的 AI 的意圖的看法。收集的資料包含 1198 位多重利益相關者參與者的最終樣本，透過合作機構和社群媒體活動進行分發，並專注於對四個 AI 使用案例的個別回應。對資料的調解分析表明，對 AI 的接受度和信任在利益相關者群體之間存在顯著差異。我們發現，AI 的代理、透明度和可解釋性水平高低之間的關鍵調解因素，以及使用不同教育 AI 的意圖，包括感知到的整體效用、公正性和信心。該研究強調，在教育中接受 AI 是個微妙且多方面的問題，除了對不同利益相關者的看法之外，還需要仔細考慮具體的 AI 應用程式及其特徵。
...
摘要：遠端病患監測基於可穿戴單導程心電圖 (ECG) 裝置，對於早期偵測心臟疾病具有重大潛力，特別是與人工智慧 (AI) 方法結合用於自動化心臟疾病偵測時。先前已有研究應用基於深度學習的 AI 方法進行心臟疾病偵測。然而，這些模型尚未被廣泛接受為臨床診斷的可靠輔助工具，部分原因在於圍繞許多 AI 演算法的當前黑箱感知。特別需要找出 ECG 訊號中有助於做出準確診斷的主要特徵，藉此提升模型的可解釋性。在本研究中，我們開發一種視覺轉換器方法，基於單導程 ECG 資料辨識心房顫動。殘差網路 (ResNet) 方法也經過開發，用於與視覺轉換器方法進行比較。這些模型應用於 Chapman-Shaoxing 資料集，以分類心房顫動，以及另一種常見心律不整，竇性心動過緩，以及正常竇性心律心跳。這些模型可以辨識決定最終分類結果的心跳關鍵區域，並強調 P 波和 T 波，以及心跳持續時間和訊號振幅在區分正常竇性心律與心房顫動和竇性心動過緩中的重要性。
...
摘要：本文介紹了一種用於憂鬱症偵測和治療的新穎典範，利用先進的大語言模型 (LLM)：生成式預訓練變壓器 4 (GPT-4)、Llama 2 chat 和 Gemini。這些 LLM 經過微調，具備專業提示，可診斷、解釋並建議憂鬱症的治療介入措施。一種獨特的少次提示方法增強了模型根據 DSM-5 標準分析和解釋憂鬱症狀的能力。在互動階段，這些模型從事同理心對話管理，利用 PsychDB 和認知行為療法 (CBT) 指南等資源，促進與經歷重度憂鬱症的個人進行支持性互動。此外，本研究引入了 Illuminate 資料庫，其中豐富了各種 CBT 模組，有助於個人化治療建議。本研究使用 F1 分數、準確度、召回率、餘弦相似度和面向召回率的評估輔助工具 (ROUGE) 等指標評估 LLM 效能，跨不同測試集證明其有效性。這種綜合方法結合了尖端的 AI 與既定的心理方法，在心理保健中提供了新的可能性，並展示了 LLM 在革新憂鬱症診斷和治療策略方面的潛力。
...
摘要：<paragraph>AI 系統的說明很少會提到受演算法決策 (ADM) 影響的人們的資訊需求。傳達資訊與受影響利害關係人重視的資訊之間的差距，可能會阻礙理解和遵守法規架構，例如 AI 法案。為了解決這個差距，我們提出了「XAI 初學者問題庫」：受影響利害關係人在兩個 ADM 使用案例（就業預測和健康監控）中的資訊需求目錄，涵蓋資料、系統脈絡、系統使用和系統規格等類別。在訪談研究中收集了資訊需求，參與者在詢問後收到了說明。參與者進一步報告了他們的理解和決策信心，顯示雖然在收到說明後信心往往會增加，但參與者也遇到了理解上的挑戰，例如無法說明為什麼他們的理解感覺不完整。說明進一步影響了參與者對系統風險和好處的看法，他們會根據使用案例確認或改變這些看法。當風險被認為很高時，參與者表示特別有興趣了解意圖的說明，例如為什麼和為了什麼目的而建立一個系統。透過這項工作，我們旨在透過在決定採用 ADM 系統時提供相關資訊和挑戰的概覽，來支援受影響利害關係人參與可解釋性。最後，我們在六項關鍵影響的清單中總結我們的發現，這些影響會告知未來針對受影響利害關係人受眾的說明設計。</paragraph>
...
摘要：<paragraph>人工智慧 (AI) 的快速演進，特別是在大型語言模型 (LLM) 和生成式 AI 的領域中，為各種領域的應用開啟了新的途徑，但它在商業教育中的角色仍未被充分探討。本研究引入了第一個基準，用來評估七種主要 LLM 的表現，包括 OpenAI 的模型 (GPT-3.5 Turbo、GPT-4 和 GPT-4 Turbo)、Google 的模型 (PaLM 2、Gemini 1.0 Pro) 和 Anthropic 的模型 (Claude 2 和 Claude 2.1)，這些模型在 GMAT 中的表現，而 GMAT 是研究所商學課程入學過程中的一項關鍵考試。我們的分析顯示，大多數 LLM 的表現都優於人類考生，其中 GPT-4 Turbo 不僅優於其他模型，還超越了頂尖商學院中研究所學生的平均分數。透過個案研究，本研究探討了 GPT-4 Turbo 在解釋答案、評量回應、找出錯誤、調整說明和產生替代方案的能力。最新的 LLM 版本，GPT-4 Turbo、Claude 2.1 和 Gemini 1.0 Pro，在推理任務方面與其前身相比有了顯著的進步，強調了它們在解決複雜問題方面的潛力。儘管 AI 在教育、評量和輔導方面的承諾很明確，但挑戰仍然存在。我們的研究不僅闡明了 LLM 的學術潛力，也強調了在教育中謹慎開發和應用 AI 的必要性。隨著 AI 技術的進步，建立 AI 互動的架構和協定、驗證 AI 生成的內容的準確性、確保全球各地不同的學習者都能使用，並創造一個 AI 支持人類專業知識的教育環境至關重要。本研究為進一步探索負責任地使用 AI 來豐富教育體驗並改善考試準備和評量方法奠定了基礎。</paragraph>
...
摘要：<paragraph>預測加護病房 (ICU) 病患的院內死亡率對於最終的臨床結果至關重要。AI 已展現出優異的準確性，但缺乏可解釋性。為了解決這個問題，本文提出了一個可解釋的多模態死亡率預測器 (X-MMP)，接近一個有效且可解釋的 AI 解决方案，以透過多模態 ICU 資料預測院內死亡率。我們在架構中採用多模態學習，可以接收來自臨床資料的異質輸入並做出決策。此外，我們引入了一個可解釋的方法，即層級傳播到 Transformer，作為 LRP 方法到 Transformer 的適當延伸，對多模態輸入產生解釋，並揭示歸因於預測的顯著特徵。此外，可以視覺化每個模態對臨床結果的貢獻，協助臨床醫生了解決策背後的推理。我們基於 MIMIC-III 和 MIMIC-III 波形資料庫配對子集建構了一個多模態資料集。在基準資料集上的全面實驗證明，我們提出的架構可以在具有競爭力的預測準確度下實現合理的解釋。特別是，我們的架構可以輕鬆轉移到其他臨床任務，這有助於發現醫療保健研究中的關鍵因素。</paragraph>
...
摘要：在過去十年中，人工智慧（AI）在病理學中的方法已大幅進步。然而，由於許多挑戰，包括將研究結果轉化為臨床診斷產品中的技術和法規障礙，以及缺乏標準化介面，導致整合到常規臨床實務緩慢。開放且與供應商無關的 EMPAIA 計畫應對了這些挑戰。在此，我們提供 EMPAIA 成就和經驗教訓的概觀。EMPAIA 整合了病理學 AI 生態系統的各個利害關係人，即病理學家、電腦科學家和產業。在密切合作下，我們制定了技術互操作性標準、AI 測試和產品開發建議，以及可解釋性方法。我們實作了模組化和開放原始碼的 EMPAIA 平臺，並成功整合了來自 8 家不同供應商的 14 個基於 AI 的影像分析應用程式，展示了不同的應用程式如何使用單一標準化介面。我們優先考慮需求，並評估了 AI 在歐洲和亞洲 14 個不同病理實驗室中的實際臨床應用。除了技術開發外，我們還為所有利害關係人建立了一個論壇，以分享數位病理學和 AI 的資訊和經驗。商業、臨床和學術利害關係人現在可以採用 EMPAIA 的常見開放原始碼介面，為大規模標準化和簡化流程提供獨特機會。需要進一步的努力才能有效且廣泛地在常規實驗室使用中建立 AI 協助。為此，已成立非營利協會 EMPAIA International，作為永續基礎設施，以持續標準化和支援廣泛實作和倡導 AI 輔助數位病理學的未來。
...
摘要：反事實解釋 (CE) 技術因其作為提供洞察力給與 AI 系統互動的使用者之手段而備受關注。儘管在醫學影像和自動駕駛等領域廣泛研究，但圖形反事實解釋 (GCE) 方法相對較少被探討。GCE 會產生一個與原始圖形相似的圖形，但其不同的結果建立在底層預測模型上。在這些 GCE 技術中，儘管在其他領域（例如藝術風格和自然語言建模）中展現出令人印象深刻的成就，但植基於生成機制的技術卻相對較少受到研究。生成式解釋器的偏好源於它們在推理期間生成反事實實例的能力，利用輸入圖形的自主獲取擾動。基於上述依據，我們的研究引入了 RSGG-CE，一種用於反事實解釋的新型穩健隨機圖形生成器，能夠從學習到的潛在空間中產生反事實範例，並考慮部分排序的生成序列。此外，我們進行定量和定性分析，以比較 RSGG-CE 的效能與 SoA 生成式解釋器，強調其產生合理反事實候選項的能力提升。
...
摘要：可解釋 AI 的其中一個動機，是讓人類在使用和部署 AI 模型時，能夠做出更好且更明智的決策。但需要謹慎評估，才能判斷這個期望是否已實現。目前的評估主要關注解釋的演算法特性，而涉及人類受試者的評估，則經常使用主觀問題來測試人類對解釋實用性的認知，而沒有以客觀的指標和測量為依據。在這項研究中，我們評估解釋是否能在機器學習模型開發的實際場景中，改善人類的決策制定。我們進行了一項涉及影像資料的混合方法使用者研究，以評估 SmoothGrad、GradCAM 和一個神諭解釋所產生的顯著性圖，在兩個任務中的表現：模型選擇和反事實模擬。令我們驚訝的是，我們沒有發現任何證據顯示，當使用者獲得任何顯著性圖時，這些任務有顯著的改善，即使是設計為易於理解且高度指示答案的合成神諭解釋也是如此。儘管如此，解釋確實有助於使用者更準確地描述模型。這些發現表明，對於基於顯著性的解釋的實用性和潛在誤解，需要謹慎看待。
...
摘要：可解釋性和安全性會產生信任。這些需要模型展現一致性和可靠性。要達成這些目標，必須使用和分析資料和知識，以及與人工智慧應用相關的統計和符號人工智慧方法，單獨使用其中一種方法無法達成。因此，我們主張並試圖證明，神經符號人工智慧方法更適合用於打造值得信賴的人工智慧系統。我們提出 CREST 架構，說明一致性、可靠性、使用者層級的可解釋性和安全性如何建構在神經符號方法上，這些方法使用資料和知識來支援關鍵應用（例如健康和福祉）的要求。本文重點關注大型語言模型 (LLM)，視其為 CREST 架構內選擇的人工智慧系統。LLM 因應對廣泛自然語言處理 (NLP) 情境的多功能性而獲得研究人員的極大關注。例如，ChatGPT 和 Google 的 MedPaLM 已成為提供一般資訊和健康相關查詢的高度有前途的平台。儘管如此，這些模型仍然是黑盒子，儘管已納入人類回饋和指導式調整。例如，ChatGPT 儘管建立了安全防護措施，仍可能產生不安全的回應。CREST 提出了一種可行的途徑，在神經符號架構中利用程序和圖形知識，以闡明與 LLM 相關的挑戰。
...
摘要：本研究調查了在 COVID-19 大流行期間及之後部署的人工智慧 (AI) 模型在預測死亡率方面的效能、可解釋性和穩健性。作為同類研究中的首例，我們發現貝氏神經網路 (BNN) 和智慧訓練技術使我們的模型能夠在資料大幅變動的情況下維持效能。我們的結果強調了開發穩健的 AI 模型的重要性，即使在具挑戰性的情況下，這些模型也能夠匹配或超越臨床醫師的預測。我們對模型可解釋性的探索顯示，隨機模型會產生更多樣化且個人化的解釋，從而突顯了 AI 模型在現實世界的臨床環境中提供詳細且個別見解的需求。此外，我們強調了量化 AI 模型中不確定性的重要性，這使臨床醫師能夠根據可靠的預測做出更明智的決策。我們的研究主張在醫療保健的 AI 研究中優先實施科學，並確保 AI 解決方案在現實世界的臨床環境中切實可行、有益且永續。透過解決醫療保健環境中獨特的挑戰和複雜性，研究人員可以開發出有效改善臨床實務和患者預後的 AI 模型。
...
摘要：肺癌在英國造成 21% 的癌症死亡，而五年存活率很大程度取決於癌症被發現的期別。最近的研究已證明 AI 方法有能力準確且早期診斷常規掃描中的肺癌。然而，此證據尚未轉化為臨床實務，其中一個障礙是缺乏可解釋的模型。本研究探討將變異自動編碼器 (VAE)，一種生成式 AI 模型，應用於肺癌病灶。建議的模型在從 LIDC-IDRI 公共資料集中的 3D CT 掃描中萃取的病灶上進行訓練。透過分群探索 VAE 產生的 2D 切片的潛在向量表示，以證明其品質，並用於肺癌診斷的 MLP 分類器模型，最佳模型達到了 0.98 的 AUC 和 93.1% 準確度的最先進指標。分群分析顯示，VAE 潛在空間根據有意義的特徵成分（包括腫瘤大小、形狀、患者和惡性類別）將惡性和良性病灶的資料集分開。我們還包括標準高斯 VAE (GVAE) 和較新的 Dirichlet VAE (DirVAE) 的比較分析，後者以 Dirichlet 分配取代先驗，以促進更具可解釋性的潛在空間，並具有解開的特徵表示。最後，我們展示了潛在空間橫越對應於臨床有意義特徵變化的潛力。
...
摘要：現有的工具用於說明影像分類器的輸出，可分為白盒，依賴於模型內部的存取，以及黑盒，與模型無關。隨著 AI 在醫療領域的使用增加，可解釋性工具的使用也隨之增加。現有醫學影像說明的工作重點在於白盒工具，例如 gradcam。然而，切換到黑盒工具有明顯的優點，包括能夠與任何分類器一起使用，以及廣泛的黑盒工具可供選擇。在標準影像上，黑盒工具與白盒一樣精確。在本文中，我們比較了幾種黑盒方法在腦癌 MRI 資料集上與 gradcam 的效能。我們證明了大多數的黑盒工具不適合用於說明醫學影像分類，並對其缺點的原因進行了詳細分析。我們還表明，一種黑盒工具，基於因果可解釋性的 rex，其效能與 \gradcam 一樣好。
...
摘要：人工智慧發展社群日益善用 Hugging Face 等託管中介，提供使用者輕鬆存取使用者上傳的模型和訓練資料。這些模型市集降低了數十萬使用者的技術部署門檻，但可能會被用於許多潛在有害和非法的用途。在本文中，我們說明了人工智慧系統（既可以「包含」內容，也可以是開放式工具）呈現迄今為止最棘手的平台治理挑戰的方式。我們提供了 Hugging Face、GitHub 和 Civitai 這三個說明性平台上數起事件的案例研究，以探討模型市集如何審核模型。根據此分析，我們概述了產業為回應審核需求而發展的重要（但仍有限）實務：授權、存取和使用限制、自動化內容審核和開放政策制定。雖然當前的政策挑戰相當可觀，我們最後提出了一些想法，說明平台如何能更好地動員資源，作為審慎、公平且適度的法規存取點。
...
摘要：背景和目標：通過提取這些資訊，機器或深度學習 (ML/DL) 基於自主資料分析工具可以協助臨床醫生和癌症研究人員從複雜的資料集中發現模式和關係。許多基於 DL 的卵巢癌 (OC) 資料分析最近已發表。這些分析在癌症的各個方面（例如，他們探討的子領域和癌症類型）和資料分析特徵方面高度多樣化。然而，目前缺乏對這些分析在這些特徵和 AI 保證 (AIA) 方面的全面理解。這篇系統性回顧旨在透過檢視現有文獻並找出使用 DL 分析 OC 資料的重要面向來填補這個缺口，明確地著重於關鍵特徵和 AI 保證觀點。方法：PRISMA 架構用於在三個期刊資料庫中進行全面搜尋。分析僅納入 2015 年至 2023 年間發表於同行評審期刊的研究。結果：在回顧中，總共檢視了 96 篇由 DL 驅動的分析。研究結果揭露了幾個關於由 DL 驅動的卵巢癌資料分析的重要見解：- 大多數研究 71%（96 篇中有 68 篇）專注於偵測和診斷，而沒有研究探討 OC 的預測和預防。- 這些分析主要基於來自非多元族群的樣本（75%（96 篇研究中有 72 篇）），僅限於某個地理位置或國家。- 只有小部分研究（僅 33%（96 篇中有 32 篇））執行整合分析，其中大多數使用同質資料（臨床或組學）。- 值得注意的是，僅有 8.3%（96 篇中有 8 篇）的研究使用外部和多元資料集驗證其模型，突顯了加強模型驗證的必要性，- 將 AIA 納入癌症資料分析仍處於非常早期的階段；只有 2.1%（96 篇中有 2 篇）透過可解釋性明確地探討 AIA。
...
摘要：可解釋性是深度學習中長久以來的挑戰，尤其是在醫療保健等高風險領域。常見的可解釋性方法會強調驅動 AI 模型決策的影像區域。然而，人類高度依賴語言來傳達不僅「在哪裡」，還有「是什麼」的解釋。此外，大多數可解釋性方法專注於解釋個別的 AI 預測，而不是描述 AI 模型一般使用的特徵。後者對於模型和資料集稽核特別有用，甚至在 AI  zuneh用於新穎任務時，還能產生知識。在此，我們提出一個可解釋性策略，該策略使用視覺語言模型來識別視覺分類任務的基於語言的描述符。透過利用影像和文字之間的預先訓練聯合嵌入空間，我們的做法將新的分類任務估計為一個線性文字組合，產生每個文字的權重，以表示其與基於視覺的分類器的一致性。我們使用兩個醫學影像分類任務來評估我們的做法，我們發現，儘管缺乏特定領域的語言訓練，但產生的描述符在很大程度上與臨床知識一致。然而，我們的做法也識別出所使用的公共資料集中「捷徑連線」的可能性。朝向可解釋性的功能性衡量標準，我們進行了一項試驗性讀者研究，發現 AI 識別的文字可以讓非專家人類在非平凡的層級上執行專業的醫療任務。總之，我們的結果強調了使用多模態基礎模型來提供直觀的、基於語言的視覺任務解釋的潛力。
...
摘要：<paragraph>使用醫療影像訓練用於臨床任務的人工智慧 (AI) 模型，通常會表現出在不同子群體之間效能差異的形式偏見。由於並非所有真實世界醫療影像資料中的偏見來源都容易辨識，因此全面評估這些偏見如何編碼在模型中，以及有能力的偏見減緩方法如何改善效能差異是一項挑戰。在本文中，我們介紹了一個系統且客觀地探討醫療影像中偏見對 AI 模型影響的新穎分析架構。我們開發並測試了此架構，以進行受控的電腦模擬試驗，使用一個產生具有已知疾病影響和偏見來源的合成磁共振影像的工具來評估醫療影像 AI 中的偏見。可行性透過使用三個反事實偏見情境來衡量模擬偏見影響對卷積神經網路 (CNN) 分類器的影響，以及三個偏見減緩策略的效能來展示。分析顯示，當 CNN 在合成資料集上訓練時，模擬偏見會導致預期的子群體效能差異。此外，重新加權被認為是針對此設定最成功的偏見減緩策略，我們展示了可解釋的 AI 方法如何協助使用此架構探討模型中偏見的表現。由於醫療影像資料集中可能存在許多且通常未知的偏見來源，因此開發公平的 AI 模型是一項相當大的挑戰。在這項工作中，我們提出了一種客觀研究偏見影響和深度學習管線中減緩策略的新穎方法，這可以支援健全且負責任的臨床 AI 的開發。</paragraph>
...
摘要：機器學習提供極大的潛力，可自動預測中風後症狀及其對復健的反應。這項努力的主要挑戰包括神經影像數據的維度非常高、可供學習的數據集大小相對較小，以及如何有效結合神經影像和表格數據（例如人口統計資訊和臨床特徵）。本文根據兩種策略評估了多種解決方案。第一個是使用總結 MRI 掃描的 2D 影像。第二個是選擇可提高分類準確度的關鍵特徵。此外，我們介紹了一種新方法，即在結合從 MRI 中提取的感興趣區域與表格數據的符號表示的影像上訓練卷積神經網路 (CNN)。我們評估了一系列 CNN 架構（2D 和 3D），這些架構在 MRI 和表格數據的不同表示上訓練，以預測中風後口述圖片描述能力的綜合測量是否在失語症或非失語症範圍內。MRI 和表格數據來自 758 位參與 PLORAS 研究的英語中風倖存者。僅針對病灶大小的基準邏輯迴歸分類準確度為 0.678，當依序加入初始症狀嚴重度和恢復時間後，準確度上升至 0.757 和 0.813。當從每次 MRI 掃描中提取 8 個感興趣區域，並在 2D 殘差神經網路中與病灶大小、初始嚴重度和恢復時間結合時，觀察到最高的分類準確度 0.854。我們的研究結果展示了如何將影像和表格數據結合，以獲得高於中風後的分類準確度，即使在機器學習術語中，數據集很小。最後，我們提出如何改進目前的模型，以使用來自醫院掃描儀的影像，達到更高的準確度。
...
摘要：可解釋人工智慧 (XAI) 已成為處理任務關鍵應用程式時的一項基本需求，確保採用黑盒子 AI 模型的透明度和可解釋性。XAI 的重要性涵蓋從醫療保健到金融的各種領域，在這些領域中，瞭解深度學習演算法的決策制定過程至關重要。大多數基於 AI 的電腦視覺模型通常都是黑盒子；因此，在影像處理中提供深度神經網路的可解釋性對於其在醫學影像分析、自動駕駛和遙測應用中的廣泛採用和部署至關重要。最近，已經引入了幾種用於影像分類任務的 XAI 方法。相反地，影像分割在可解釋性的背景下受到的關注相對較少，儘管它是電腦視覺應用中的一項基本任務，特別是在遙測中。只有少數研究提出用於影像分割的基於梯度的 XAI 演算法。本文採用了最近的無梯度 Sobol XAI 方法進行語意分割。為了衡量 Sobol 方法在分割中的效能，我們提出了一種基於可學習雜訊模型的量化 XAI 評估方法。此模型的主要目的是在解釋圖上產生雜訊，其中較高的誘發雜訊表示較低的準確度，反之亦然。進行基準分析以評估和比較三種 XAI 方法的效能，包括 Seg-Grad-CAM、Seg-Grad-CAM++ 和 Seg-Sobol，並使用提出的基於雜訊的評估技術。這構成了使用高解析度衛星影像執行和評估 XAI 方法的首次嘗試。
...
摘要：大型語言模型在短時間內已在多個領域中大量增加。然而，由於事實性、連貫性和幻覺等問題，醫學和醫療保健領域對其採用持猶豫態度。鑑於醫療保健的高風險性質，許多研究人員甚至警告不要使用它，直到這些問題得到解決。在醫療保健中實施和部署 LLM 的關鍵是讓這些模型值得信賴、透明（盡可能透明）且可解釋。在本文中，我們描述了在醫療保健中採用可靠、值得信賴且無偏見的模型的主要要素，作為其必要條件。具體來說，我們專注於量化、驗證和減輕醫療保健背景下的幻覺。最後，我們討論了 LLM 在醫療保健中的未來可能是什麼樣子。
...
摘要：人工智慧 (AI) 以快速的速度進步，目前已準備部署在各種應用程式中，例如自主系統、醫療診斷和自然語言處理。早期採用 AI 技術於實際應用並非沒有問題，特別是對神經網路而言，神經網路可能不穩定且容易受到對抗範例的影響。長期而言，需要開發適當的安全保證技術，以減少因可避免的系統故障而造成的潛在危害並確保可信賴性。本論文專注於認證和可解釋性，提供了已開發用於確保 AI 決策安全性的技術概觀，並討論未來的挑戰。
...
摘要：歐洲議會議會研究服務總局已向歐洲議會議員提交了一份報告，其中列舉了人工智慧 (AI) 在醫療和保健方面的七項主要風險：由於 AI 錯誤造成的患者傷害、醫療 AI 工具的誤用、AI 中的偏差和現有不平等的延續、缺乏透明度、隱私和安全問題、問責制方面的差距，以及實施障礙。
  在研究中，我們提出了十四項功能性要求，AI 系統可以實施這些要求來降低與其醫療目的相關的風險：AI 護照、使用者管理、法規檢查、僅限學術用途免責聲明、資料品質評估、臨床醫師雙重檢查、持續效能評估、稽核追蹤、持續可用性測試、回顧/模擬案例、偏差檢查、可解釋 AI、加密和使用經過現場測試的函式庫，以及語義互操作性。
  我們在此的用意是提供技術解決方案的具體高階規格，以確保持續良好的效能和使用 AI 系統，以符合未來歐盟法規架構，造福患者。
...
摘要：人工智能技術可應用於分類病患的生理活動，並預測遠距病患監測的重要生命徵象。基於深度學習模型等非線性模型的回歸分析，由於其黑箱特性，可解釋性有限。這可能需要決策者根據非線性模型結果盲目地做出信仰上的飛躍，特別是在醫療保健應用中。在非侵入式監控中，來自追蹤感測器和其易感臨床特性的病患資料，作為預測未來生命徵象的輸入特徵。解釋各種特徵對監控應用整體輸出的貢獻，對於臨床醫師的決策至關重要。在本研究中，提出了一個可解釋 AI 定量分析 (QXAI) 架構，具有事後模型可解釋性和內在可解釋性，適用於監督式學習方法中的回歸和分類任務。這是透過利用 Shapley 值概念，並將注意力機制納入深度學習模型中來實現的。我們採用人工神經網路 (ANN) 和基於注意力的雙向 LSTM (BiLSTM) 模型，根據感測器資料預測心率和分類生理活動。深度學習模型在預測和分類任務中都達到了最先進的結果。對輸入資料進行了全局解釋和局部解釋，以了解各種病患資料的特徵貢獻。所提出的 QXAI 架構使用 PPG-DaLiA 資料評估，以預測心率，並使用行動健康 (MHEALTH) 資料根據感測器資料分類生理活動。蒙地卡羅近似應用於架構，以克服 Shapley 值計算所需的時間複雜度和高運算能力需求。
...
摘要：在可解释人工智能 (XAI) 研究中，主要重点一直是为专家和从业人员解释模型。模型不可知和局部解释方法在许多应用中被认为是可解释且足够的。然而，在医疗保健等领域，最终用户是没有任何人工智能或领域专业知识的患者，因此迫切需要更易于理解和灌输对模型操作的信任的模型解释。我们假设生成叙述性的、特定于患者的和全局的（模型的整体）模型解释将能够更好地理解并能够做出决策。我们使用决策树模型来测试这一点，为被确定为患有冠状动脉疾病高风险的患者生成局部和全局解释。这些解释被呈现给非专家用户。我们发现个人对特定类型的解释有强烈的偏好。大多数参与者更喜欢全局解释，而较小的一组更喜欢局部解释。基于任务的评估这些参与者的心理模型提供了有价值的反馈，以增强叙述性的全局解释。这反过来又指导了既值得信赖又可操作的健康信息学系统的设计。
...
摘要：可解釋 AI 在醫學影像中至關重要。在神經科學這個充滿挑戰的領域中，視覺主題呈現出高度複雜性，特別是在三維空間中。神經科學的應用涉及從 MRI 中識別腦溝特徵，由於專家之間不同的註解協定和腦部複雜的三維功能，因此面臨著重大的障礙。因此，傳統的可解釋性方法無法有效驗證和評估這些網路。為了解決這個問題，我們首先提出一個數學公式，描述了各種電腦視覺任務中不同類別的解釋需求，根據驗證協定的可靠性，將其分類為自解釋、半解釋、非解釋和新模式學習應用。關於這個數學公式，我們提出了一個 3D 可解釋性架構，旨在驗證深度學習網路在偵測腦回旁溝（一個重要的腦部解剖特徵）的輸出。該架構整合了局部 3D 解釋、透過降維的全局解釋、串聯的全局解釋和統計形狀特徵，揭示了模式學習的新見解。我們在具有挑戰性的 TOP-OSLO 資料集上訓練和測試了兩個先進的 3D 深度學習網路，顯著提高了腦溝偵測的準確度，特別是在左半球。在使用此資料集的不同註解協定進行評估時，我們強調了無偏註解流程在我們提出的 3D 架構中實現精確預測和有效模式學習的關鍵作用。所提出的架構不僅註解了可變的腦溝，還揭示了隱藏的 AI 知識，有望促進我們對腦部解剖和功能的理解。
...
摘要：電子病歷 (EHR) 和例行文件記錄實務在病患的日常照護中扮演著至關重要的角色，提供全面的健康、診斷和治療記錄。然而，複雜且冗長的電子病歷敘述會讓醫療保健提供者不堪負荷，進而導致診斷不準確。儘管大型語言模型 (LLM) 已展現其在各種語言任務中的潛力，但它們在醫療保健領域的應用需要確保將診斷錯誤降到最低，並防止對病患造成傷害。在本文中，我們概述了一種創新的方法，用於提升大型語言模型在自動診斷生成領域的能力，並透過整合一個醫療知識圖譜 (KG) 和一個新穎的圖模型：Dr.Knows，而達成，其靈感來自於臨床診斷推理過程。我們從美國國家醫學圖書館的統一醫學語言系統 (UMLS) 中衍生出知識圖譜，這是一個強大的生物醫學知識儲存庫。我們的這項方法否定了預訓練的必要性，而是將知識圖譜作為輔助工具，用於協助解釋和總結複雜的醫療概念。利用真實世界的醫院資料集，我們的實驗結果證明了將大型語言模型與知識圖譜結合的建議方法，具有提升自動診斷生成精準度的潛力。更重要的是，我們的這項方法提供了一條可解釋的診斷途徑，讓我們更接近實現人工智慧增強診斷決策支援系統。
...
摘要：現有的用於診斷膝關節骨關節炎 (OA) 的人工智慧 (AI) 模型，儘管達到了醫療專家般的表現，但仍因缺乏透明度和可解釋性而受到批評。這種不透明性使它們難以在臨床實務中受到信任。最近，可解釋的人工智慧 (XAI) 已成為一種專業技術，它能透過揭示預測的推導方式來提供對模型預測的信心，從而促進 AI 系統在醫療保健中的使用。本文提供了用於膝關節骨關節炎診斷的 XAI 技術的首次調查。XAI 技術從兩個角度進行討論：資料可解釋性和模型可解釋性。本文的目的是提供有價值的見解，了解 XAI 對更可靠的膝關節骨關節炎診斷方法的潛力，並鼓勵在臨床實務中採用它。
...
摘要：在醫療保健中，AI 應用程式的最新進展已展現出驚人的前景，在診斷和疾病預後方面超越人類的表現。然而，隨著 AI 模型越來越複雜，人們開始關注其不透明性、潛在偏見以及對可解釋性的需求。為了確保 AI 系統的信任度和可靠性，特別是在臨床風險預測模型中，可解釋性變得至關重要。可解釋性通常是指 AI 系統提供其決策邏輯或決策本身的健全解釋的能力，以供人類利益相關者理解。在臨床風險預測中，可解釋性的其他方面，如公平性、偏見、信任度和透明度，也代表了超越可解釋性的重要概念。在本綜述中，我們探討了這些概念之間的關係，因為它們通常一起使用或可以互換。本綜述還討論了為臨床風險預測開發可解釋模型的最新進展，強調了在臨床實踐中對多種常見模式進行定量和臨床評估和驗證的重要性。它強調了外部驗證和結合多種可解釋性方法以增強信任度和公平性的必要性。採用嚴格的測試，例如使用具有已知生成因子的合成數據集，可以進一步提高可解釋性方法的可靠性。開放獲取和程式碼共享資源對於透明度和可複製性至關重要，有助於可解釋性研究的發展和可信度。儘管存在挑戰，但從臨床醫生到開發人員，採用端到端的方法來解釋臨床風險預測對於成功至關重要。
...
摘要：儘管在醫學和保健方面的人工智慧 (AI) 有重大的進展，但 AI 技術的部署和採用在現實世界的臨床實務中仍然有限。近年來，人們對醫療 AI 相關的技術、臨床、倫理和法律風險提出了疑慮。為了增加現實世界的採用率，醫療 AI 工具必須獲得患者、臨床醫生、醫療機構和當局的信任和接受。這項工作將 FUTURE-AI 指南描述為指導醫療保健中可信賴 AI 工具開發和部署的第一個國際共識架構。FUTURE-AI 聯盟成立於 2021 年，目前由來自 51 個國家/地區的 118 位跨領域專家組成，代表了所有大陸，包括 AI 科學家、臨床醫生、倫理學家和社會科學家。在兩年的時間裡，該聯盟通過一個反覆運算的過程定義了可信賴 AI 的指導原則和最佳實務，包括深入的文獻回顧、修改後的德爾菲調查和線上共識會議。FUTURE-AI 架構建立在醫療保健中可信賴 AI 的 6 項指導原則之上，即公平性、普遍性、可追溯性、可用性、穩健性和可解釋性。通過共識，定義了一組 28 項最佳實務，涵蓋技術、臨床、法律和社會倫理層面。建議涵蓋了醫療 AI 的整個生命週期，從設計、開發和驗證到法規、部署和監控。FUTURE-AI 是一個基於風險、無假設的指南，它提供了一個結構化的方法來建構醫療 AI 工具，這些工具將在現實世界的實務中受到信任、部署和採用。鼓勵研究人員在概念驗證階段考慮建議，以促進未來醫療 AI 向臨床實務的轉化。
...
摘要：在医学領域，人工智慧已取得顯著進展，在醫學影像、病人照護和其他領域都有新興應用。雖然這些應用已在回顧性研究中被證實是成功的，但實際上應用它們的卻很少。醫學人工智慧領域面臨各種挑戰，包括建立使用者信任、遵守法規、合乎道德地使用資料。可解釋人工智慧 (XAI) 旨在讓人們了解人工智慧並相信其結果。本文根據近年來發表的 198 篇文章的代表性樣本，對用於醫療決策支援的 XAI 解決方案的最新發展進行文獻回顧。對相關文章的系統性綜合產生了若干研究結果：(1) 這些解決方案大多採用與模型無關的 XAI 技術，(2) 深度學習模型比其他類型的機器學習模型使用得更多，(3) 可解釋性被用於促進信任，但很少有研究報告醫生參與迴路，(4) 視覺和互動式使用者介面在理解系統的解釋和建議方面更有用。需要醫學和人工智慧專家合作進行更多研究，這可以指導在醫學中設計、實施和評估 XAI 解決方案的合適架構的開發。
...
摘要：試管受精是治療不孕症最廣泛的療法之一。其主要挑戰之一是胚胎評估與選擇，這是植入過程的一部分，且在臨床醫師之間存在很大的差異性。基於深度學習的方法正受到關注，但其不透明的本質會降低其在臨床環境中的接受度，因為在臨床環境中，決策透明度是關鍵。在本文中，我們分析了 AI 輔助胚胎分析模型的可解釋性的現有研究，並找出其限制。我們也討論了這些模型如何作為決策支援系統整合到臨床環境中，並考量臨床醫師和患者的需求。最後，我們提出了一些準則，以提高可解釋性和可信度，推動這項技術朝向既定的臨床實務邁進。
...
摘要：在需求工程 (RE) 領域中，可解釋人工智慧 (XAI) 在將 AI 支援的系統與使用者需求、社會期望和法規標準對齊方面的重要性日益提升，已獲得認可。一般來說，可解釋性已成為影響系統品質的重要非功能性需求。然而，可解釋性和效能之間的假定權衡挑戰了可解釋性的假定正面影響。如果滿足可解釋性需求需要降低系統效能，則必須仔細考慮這些品質面向中哪一項優先，以及如何在它們之間折衷。在本文中，我們批判性地檢視了假定的權衡。我們主張以細緻的方式來處理這項權衡，其中納入資源可用性、網域特性和風險考量。透過提供未來研究和最佳實務的基礎，這項工作旨在推動 RE for AI 領域的進展。
...
摘要：在需求工程 (RE) 領域中，可解釋人工智慧 (XAI) 在將 AI 支援系統與使用者需求、社會期望和法規標準對齊方面的重要性日益提升，已獲得認可。一般而言，可解釋性已成為影響系統品質的重要非功能性需求。然而，可解釋性和效能之間的假定權衡挑戰了可解釋性的假定正面影響。如果滿足可解釋性的需求需要降低系統效能，則必須仔細考慮這些品質面向中哪一個優先，以及如何在它們之間折衷。在本文中，我們批判性地審查了假定的權衡。我們主張以細緻的方式來探討它，其中包含資源可用性、領域特性和風險考量。透過提供未來研究和最佳實務的基礎，這項工作旨在推動 AI 的 RE 領域。
...
摘要：本文嚴格評估了歐盟委員會提出的 AI 法案，針對高風險 AI 系統的風險管理和風險可接受性方法，這些系統對基本權利和安全構成風險。該法案旨在以相稱的監管負擔促進「可信賴」的 AI。其關於風險可接受性的條款要求盡量減少或消除高風險系統的殘餘風險，同時考慮「最先進技術」。此準則，特別是如果狹義解釋，是不可行的，既不能促進相稱的監管負擔，也不能促進可信賴性。相比之下，議會最近對風險管理條款的修正案草案引入了「合理性」、「成本效益分析」，並更透明地說明了風險可接受性判斷的價值觀和背景性質。本文認為，議會的方法更可行，並且能更好地平衡相稱性和可信賴性的目標。它解釋了風險可接受性判斷中的合理性會是什麼，並根據過失法和歐洲醫療器材法規中的原則進行說明。它主張風險可接受性判斷的方法需要穩固的公民合法性基礎：包括監管機構的詳細指導或參與，以及受影響利益相關者的有意義意見。
...
摘要：可解釋人工智慧（XAI）是機器學習中快速發展的領域，旨在解開複雜模型的預測。XAI 在敏感應用中特別需要，例如在醫療保健中，當診斷、建議和治療選擇可能依賴於人工智慧系統做出的決策時。人工智慧方法也已廣泛用於老化研究，特別是在開發生物時鐘模型和識別老化和與年齡相關疾病的生物標記方面。然而，這裡 XAI 的潛力有待充分認識。我們討論了 XAI 在開發「老化時鐘」中的應用，並對按特定生理系統的重點分類的文獻進行了全面分析。
...
摘要：部分原型模型是可解釋設計的影像分類器，也是黑箱 AI 的替代方案。本文探討可解釋機器學習的適用性和潛力，特別是 PIP-Net，用於對真實世界醫學影像資料進行自動診斷支援。PIP-Net 學習人類可理解的原型影像部分，我們評估其準確性和可解釋性，用於骨折檢測和皮膚癌診斷。我們發現 PIP-Net 的決策制定過程符合醫學分類標準，同時僅提供影像層級類別標籤。由於 PIP-Net 的原型無監督預訓練，資料品質問題（例如 X 光中的不需要文字或標籤錯誤）可以輕鬆辨識。此外，我們首次證明人類可以透過直接停用不需要的原型，手動修正 PIP-Net 的推理。我們得出結論，部分原型模型由於其可解釋性和進階模型除錯的潛力，對於醫學應用而言很有前景。
...
摘要：可解釋 AI (XAI) 是機器學習研究中日益重要的領域，旨在使黑箱模型透明且可解釋。在本文中，我們提出了一種使用條件特徵置換所產生的所謂反事實路徑的新 XAI 方法。該演算法透過識別影響模型預測變化的特徵順序置換來衡量特徵重要性。它特別適用於根據包含領域知識的知識圖譜中的反事實路徑產生解釋。反事實路徑在解釋和視覺化黑箱模型時，為目前的 XAI 方法引入了額外的圖形維度。使用合成和醫療資料進行的實驗證明了我們方法的實際適用性。
...
摘要：人工智能（AI）的可解釋性領域見證了越來越多的研究和學術興趣的增加。然而，在解釋機器學習演算法的結果時缺乏人性化和個別的解釋，這顯著地阻礙了臨床醫生在研究和臨床實務中接受這些方法。為了解決這個問題，我們的研究使用反事實解釋來探討「如果？」情境在醫學研究中的適用性。我們的目標是擴展我們對用於診斷小兒後顱窩腦瘤的磁共振造影（MRI）特徵的理解，超越現有的界限。在我們的案例研究中，所提出的概念提供了一種新的方法來檢驗替代決策情境，這些情境提供個性化和特定於情境的見解，使我們能夠驗證預測並釐清在不同情況下的差異。此外，我們探討了反事實資料擴充的潛在用途，並評估它們作為我們醫學研究案例中替代方法的可行性。結果證明了使用反事實解釋來增強臨床研究中 AI 驅動方法的可接受性的潛力。
...
摘要：目前人工智能領域的進展已導致開發出各種由 AI 支援的失智症評估，可用於識別失智症早期階段的患者。它可以革新失智症照護環境。醫療界必須了解各種 AI 評估，並根據其有效性、效率、實用性、可靠性和準確性，針對失智症患者 (PwD) 的早期識別進行評估。另一方面，AI 開發人員應了解各種非 AI 評估以及最近開發的 AI 評估。因此，這篇論文可供臨床醫生和 AI 工程師閱讀，填補了文獻中對臨床醫生說明現有失智症識別解決方案、以及對 AI 工程師說明所用技術和最廣泛的失智症數據集的空白。它遵循對 AI 和非 AI 失智症評估論文的回顧，為 AI 和醫療界提供有關各種失智症評估的寶貴資訊。討論和結論突出了最顯著的研究方向和現有解決方案的成熟度。
...
摘要：<paragraph>可解釋性對人工智慧 (AI) 技術構成重大挑戰。當前關於可解釋 AI (XAI) 的研究缺乏提取學習任務全局知識的效率，因此存在不精確顯著性、上下文感知缺失和含義模糊等缺陷。在本文中，我們提出類別關聯嵌入 (CAE) 方法來解決這些問題。我們採用編碼器-解碼器架構來嵌入樣本特徵，並同時將它們分為類別相關和個體相關的樣式向量。將給定樣本的個體樣式代碼與另一類別的類別樣式代碼重新組合，會產生一個合成樣本，它保留了個體特徵，但改變了類別分配，遵循循環對抗學習策略。類別關聯嵌入將所有實例的全局類別相關特徵提取到一個統一的域中，並且類別之間有很好的分離。然後可以提取不同類別之間的轉換規則，並進一步應用於個別實例。然後，我們提出一個主動 XAI 框架，它沿著引導路徑操縱特定樣本的類別樣式向量朝向反類別，從而產生一系列具有相同個體特徵的反例合成樣本。將這些反事實樣本與原始樣本進行比較，可以直觀地說明分類任務的本質。我們採用該框架進行醫學影像分類任務，這表明與現有方法相比，可以獲得更精確的顯著性圖，並具有強大的上下文感知表示。此外，疾病病理學可以通過在類別樣式空間中遍歷路徑來直接視覺化。</paragraph>
...
摘要：提供基於機器學習的人工智慧預測的高品質解釋，是一項具有挑戰性且複雜的任務。要順利進行，需要具備以下因素：選擇適當的解釋概括性/特殊性層級；考量解釋受益者對所考慮的人工智慧任務的熟悉度假設；參照促成決策的特定元素；利用額外的知識（例如專家證據），這些知識可能不屬於預測流程的一部分；並提供支持否定假設的證據。最後，系統需要以清晰可解，且可能具有說服力的方式制定解釋。考量這些因素後，ANTIDOTE 促進了可解釋人工智慧的整合願景，其中深度學習流程的低階特徵與人類論證能力的高階架構結合。ANTIDOTE 將利用深度學習和論證的跨領域能力，來支援更廣泛且創新的可解釋人工智慧觀點，其中對於臨床案例審議的高品質解釋需求至關重要。作為該專案的第一個成果，我們發布了 Antidote CasiMedicos 資料集，以利於一般可解釋人工智慧的研究，以及特別是醫療領域的論證研究。
...
摘要：<paragraph>近年来，图神经网络的进展迅速，在药物发现、医疗诊断和推荐系统方面都有许多新的进展。虽然这一进展意义重大，但许多网络都是“黑匣子”，对网络到底学习了“什么”知之甚少。许多高风险应用，如药物发现，需要模型提供人类可理解的解释，以便用户能够识别错误并发现新知识。因此，可解释 AI 算法的开发对于我们充分利用 AI 的优势至关重要。
  我们提出了一种用于 GNN 的可解释性算法，称为 eXplainable Insight (XInsight)，它使用 GFlowNets 生成模型解释的分布。由于 GFlowNets 生成具有与奖励成正比的概率的对象，因此与仅学习最大奖励样本的先前方法相比，XInsight 可以生成多种解释。我们通过生成对在两个图分类任务上训练的 GNN 的解释来演示 XInsight：使用 MUTAG 数据集对诱变化合物进行分类，并使用我们开源的合成数据集对无环图进行分类。我们通过使用 QSAR 建模分析生成的化合物来展示 XInsight 解释的效用，我们发现 XInsight 生成的化合物按亲脂性聚类，亲脂性是诱变性的已知相关因素。我们的结果表明，XInsight 生成了一个解释分布，揭示了模型所展示的底层关系。它们还强调了生成多种解释的重要性，因为它使我们能够发现模型中的隐藏关系，并为进一步分析提供有价值的指导。</paragraph>
...
摘要：我們提出並實作了一個可解釋機器學習分類模型，用於基於表達式布林公式的可解釋 AI (XAI)。潛在應用包括信用評分和醫療狀況診斷。布林公式定義一個具有可調整複雜度（或可解釋性）的規則，根據此規則對輸入資料進行分類。此類公式可以包含任何可應用於一個或多個布林變數的運算子，因此與更嚴格的基於規則和基於樹的方法相比，提供了更高的表達能力。分類器使用原生局部最佳化技術進行訓練，有效地搜尋可行公式的空間。淺層規則可以用快速的整數線性規劃 (ILP) 或二次無約束二元最佳化 (QUBO) 求解器來確定，這些求解器可能由特殊用途硬體或量子裝置提供支援。我們透過執行在完整布林公式的子樹上最佳化的非局部移動，將原生局部最佳化器的表達能力與效率與這些裝置的快速運作結合起來。我們提供了廣泛的數值基準測試結果，其中包含在眾所周知的公開資料集上的幾個基線。根據結果，我們發現原生局部規則分類器通常與其他分類器具有競爭力。非局部移動的加入以較少的反覆運算次數達到類似的結果，因此使用專用或量子硬體可以透過快速提出非局部移動來加速。
...
摘要：因應全球心理健康問題的挑戰，我們提出一個基於邏輯神經網路 (LNN) 的神經符號 AI 方法，用於診斷心理疾病。由於心理疾病缺乏有效的治療覆蓋率，因此需要一種 AI 解決方案，可以協助治療師進行診斷。然而，目前的類神經網路模型缺乏可解釋性，治療師可能不會信任它們。LNN 是一種遞迴神經網路架構，它結合了神經網路的學習能力和基於經典邏輯的 AI 的推理能力。所提出的系統使用臨床訪談中的輸入謂詞來輸出心理疾病類別，並使用不同的謂詞剪枝技術來實現可擴充性和更高的分數。此外，我們提供了一個見解提取方法來協助治療師進行診斷。所提出的系統解決了當前類神經網路模型缺乏可解釋性的問題，並為心理疾病診斷提供了一個更值得信賴的解決方案。
...
摘要：隨著機器學習模型在醫療診斷中越來越普遍，對可解釋性和透明度的需求變得至關重要。XAI 文藝復興標誌著該領域的重大轉變，旨在重新定義醫療診斷模型的可解釋性。本文探討了解釋性 AI (XAI) 領域內的創新方法和方法論，這些方法和方法論正在革新醫療診斷模型的可解釋性。通過闡明基礎決策制定過程，XAI 技術讓醫療保健專業人員能夠理解、信任並有效地利用這些模型進行準確且可靠的醫療診斷。本綜述重點介紹了醫療診斷中 XAI 的關鍵進展及其轉變醫療保健領域的潛力，最終改善患者的治療結果並建立對 AI 驅動診斷系統的信任。
...
摘要：在以高度连通性和移动性为特征的格局中，加上心血管疾病的激增，通过远程监测心血管健康来减少医疗保健支出的必要性变得更加明显。准确检测和分类心律不齐对于诊断患有心脏不规则的人至关重要。本研究强调了在家中使用心电图 (ECG) 测量进行实时心律不齐检测的可行性。本文提出了心律不齐检测的新应用，利用尖端的 You-Only-Look-Once (YOLO)v8 算法对单导联 ECG 信号进行分类。我们引入了一个新颖的损失修改 YOLOv8 模型，该模型针对 MIT-BIH 心律不齐数据集进行了微调，从而实现了实时的连续监测。获得的结果证实了我们方法的有效性，该模型在 NVIDIA Tesla V100 上实现了 99.5% 的平均准确率和 0.992 mAP@50，以及 0.002 秒的快速检测时间。我们的研究例证了实时心律不齐检测的潜力，使用户能够在舒适的家中直观地解释模型输出。此外，本研究为扩展到能够在医疗保健领域部署的实时可解释人工智能 (XAI) 模型奠定了基础，从而极大地推进了医疗保健解决方案的领域。
...
摘要：乳癌 (BC) 仍然是重大的健康威脅，目前尚無長期可用的療法。早期偵測至關重要，但乳房攝影檢查的解釋受到高偽陽性和偽陰性的阻礙。由於預計乳癌的發生率將超過肺癌，因此改善早期偵測方法至關重要。熱像攝影使用高解析度紅外線相機，特別是與人工智慧 (AI) 結合使用時，提供了希望。這項工作提出了一個基於注意力的卷積神經網路用於分割，在乳癌檢測和分類中提供了更高的速度和精確度。該系統增強影像並執行可解釋的 AI 癌症分割。我們提出了一個基於 transformer 注意力的卷積架構 (UNet) 用於故障識別，並採用梯度加權類別激活對應 (Grad-CAM) 來分析 UNet 架構中偏見和弱點的區域，使用 IRT 影像。與現有的深度學習框架相比，我們提出的框架的優越性得到了證實。
...
摘要：憂鬱症是最普遍且嚴重的精神疾病，會造成嚴重的財務與社會影響。憂鬱症偵測對於早期介入以減輕這些後果至關重要。如此重大的決定本質上需要可解釋性。雖然一些憂鬱症偵測研究嘗試根據重要性分數或注意力權重來解釋這個決定，但這些解釋與基於憂鬱症狀的臨床憂鬱症診斷標準不符。為了填補這個缺口，我們遵循計算設計科學範例來開發一個新穎的多尺度時間原型網路 (MSTPNet)。MSTPNet 創新地偵測和解釋憂鬱症狀以及它們持續多久。使用大規模資料集進行的廣泛實證分析顯示，MSTPNet 以 0.851 的 F1 分數優於最先進的憂鬱症偵測方法。此結果還揭示了在調查方法中未註明的新的症狀，例如分享對不同生活的欽佩。我們進一步進行使用者研究以證明其在可解釋性方面優於基準。本研究以一個新穎的可解釋深度學習模型為憂鬱症偵測在社群媒體中做出貢獻。在實務上，我們提出的方法可以實作在社群媒體平台中，以提供個人化的線上資源給被偵測出憂鬱症的患者。
...
摘要：電子健康紀錄 (EHR) 用作醫療保健中預期的由人工智慧 (AI) 推動的轉型的基本資料來源。
然而，反映在 EHR 記錄中的臨床偏見可能會導致 AI 模型繼承並擴大這些偏見，進而導致健康差距持續存在。本研究使用基於 Transformer 的深度學習模型和可解釋 AI (XAI) 技術，探討 EHR 記錄中污名化語言 (SL) 對死亡率預測的影響。我們的研究結果表明，臨床醫生所寫的 SL 會對 AI 效能產生負面影響，特別是對黑人患者而言，突顯 SL 是 AI 模型開發中種族差異的來源。為了探索一種在運作上有效率的方法來減輕 SL 的影響，我們透過臨床醫生的協作網路探討 SL 產生的模式，並找出核心臨床醫生對 AI 模型中的種族差異有較大的影響。我們發現，移除核心臨床醫生所寫的 SL 是一種比消除資料集中所有 SL 更有效率的偏見減少策略。本研究提供了可操作的見解，以利負責任的 AI 開發，並有助於了解臨床醫生的行為和撰寫醫療保健中 EHR 記錄的方式。
...
摘要：當代透過 AI 的自動化包含大量幕後的人力，這通常既看不見又低薪。由於看不見的勞動，包括標籤和維護工作，是當代 AI 系統不可或缺的一部分，因此讓使用者了解它的角色仍然很重要。我們建議這可以透過可解釋的 AI (XAI) 設計來完成，特別是女性主義交叉 XAI。我們提出源自女性主義交叉研究的製圖法，以提出 AI 的系統觀點，並納入與看不見的勞動相關的 AI 向度。
...
摘要：虛擬心理健康助理 (VMHA) 持續進步，以支援每年有 6,000 萬人次初級保健就診和 600 萬人次急診室 (ER) 就診的全球醫療保健系統。這些系統由臨床心理學家、精神科醫師和人工智慧 (AI) 研究人員為認知行為療法 (CBT) 所建置。目前，VMHA 的角色是透過資訊提供情緒支持，較少著重於與患者建立反思性對話。需要更全面、安全且可解釋的方法來建置負責任的 VMHA，以提出後續問題或提供見多識廣的回應。這項調查提供對現有心理健康對話代理進行系統性批判性探討，接著提出運用脈絡知識、資料集改善 VMHA 的新見解，以及其在臨床決策支援中浮現的角色。我們也提供新的方向，以可解釋性、安全性與整體可信度豐富 VMHA 的使用者體驗。最後，我們提供評估指標和實務考量，供 VMHA 在現有文獻之外使用，以在 VMHA 與患者的積極溝通中建立信任。
...
摘要：XAI 指協助最終使用者詮釋 AI 模型的輸出和預測的技術和方法。在高風險決策制定情況下，例如醫療領域，黑箱 AI 應用程式增加了透明度和可解釋性的需求，因為錯誤的預測可能會產生嚴重的後果。模型的可解釋性和可詮釋性對於在醫療實務中成功部署 AI 模型至關重要。AI 應用程式的基礎推理需要對臨床醫生透明，才能獲得他們的信任。本文提出了對醫療保健領域中 XAI 方面和挑戰的系統性回顧。本研究的主要目標是回顧各種 XAI 方法、其挑戰，以及與醫療保健相關的機器學習模型。這些方法分為六類討論：以特徵為導向的方法、整體方法、概念模型、代理模型、局部基於像素的方法和以人為中心的方法。最重要的是，本文探討了 XAI 在醫療保健問題中的角色，以釐清其在安全關鍵應用中的必要性。本文旨在透過回顧相關的實驗結果，建立對醫療保健領域中與 XAI 相關應用程式的全面理解。為了促進未來研究以填補研究空白，本文調查了 XAI 模型從不同觀點來看的重要性及其限制。
...
摘要：<paragraph>最先進的機器學習模型通常會學習到訓練資料中內嵌的虛假關聯性。這在將這些模型部署於高風險決策制定（例如皮膚癌偵測等醫療應用）時會造成風險。為了解決這個問題，我們提出「揭露以修正」（R2R），一個包含了整個可解釋人工智慧（XAI）生命週期的架構，讓從業人員能夠透過最少的人為互動，反覆地找出、減輕和（重新）評估虛假的模型行為。在第一步（1），R2R 透過找出歸因中的異常值或透過檢查模型學習到的潛在概念，揭露模型的弱點。其次（2），找出並在輸入資料中空間定位負責的偽影，然後利用它（3）修正模型行為。具體來說，我們應用 RRR、CDEP 和 ClArC 的方法來進行模型修正，並（4）重新評估模型的效能和對偽影的剩餘敏感度。我們使用兩個醫療基準資料集（黑色素瘤偵測和骨齡估計），將我們的 R2R 架構應用於 VGG、ResNet 和 EfficientNet 架構，並藉此揭露和修正真正的資料內在偽影，以及受控設定中的合成變體。完成 XAI 生命週期後，我們示範了多個 R2R 迭代，以減輕不同的偏差。程式碼可在 https://github.com/maxdreyer/Reveal2Revise 取得。</paragraph>
...
摘要：可解释人工智能 (XAI) 领域近年来取得了巨大进步，但进展主要在计算机视觉和自然语言处理方面。对于时间序列，输入通常不可解释，只有有限的研究可用于 XAI。在这项工作中，我们提出了一个虚拟检查层，将时间序列转换为可解释表示，并允许通过局部 XAI 方法（如逐层相关性传播 (LRP)）将相关性归因传播到此表示。通过这种方式，我们将 XAI 方法族应用于输入仅在转换后才可解释的域（例如语音）。在此，我们专注于傅立叶变换，该变换主要应用于时间序列和 LRP 的解释，并将其方法称为 DFT-LRP。我们在各种时间序列分类设置（如音频和电子健康记录）中展示了 DFT-LRP 的有用性。我们展示了 DFT-LRP 如何揭示在不同域（例如时间域与频域）中训练的模型的分类策略的差异，或有助于发现模型如何作用于数据中的虚假相关性。
...
摘要：解釋深度學習模型預測的能力，對於最終使用者來說，是一項重要的功能，可以利用人工智慧 (AI) 的力量，進行醫療決策的過程，這通常被視為不透明且難以理解。在本文中，我們應用最先進的可解釋人工智慧 (XAI) 方法，來解釋黑箱 AI 模型在甲狀腺結節診斷應用中的預測。我們提出新的基於統計的 XAI 方法，即核密度估計和密度圖，來解釋未檢測到結節的情況。XAI 方法的效能會在質性和量化的比較下被考慮，作為改善數據品質和模型效能的回饋。最後，我們進行調查，以評估醫生和患者對 XAI 解釋模型對甲狀腺結節影像決策的信任度。
...
摘要：醫療裝置和人工智慧系統迅速改變醫療保健的提供方式。同時，由於其性質，醫療裝置中或做為醫療裝置的人工智慧可能會遭受網路攻擊，導致病患安全和安全風險。本章節分為三部分。第一部分從設定場景開始，說明網路安全在醫療保健中的角色。接著，我們簡要定義我們在討論時所指涉的人工智慧，其本身被視為醫療裝置或支援醫療裝置。為了說明此類醫療裝置構成的風險，我們提供三個範例：資料集中毒、社會工程和資料或原始碼萃取。在第二部分，本文提供歐盟監管架構的概觀，與確保人工智慧做為或在醫療裝置中的網路安全有關（醫療裝置法規、網路與資訊安全指令、網路安全法、一般資料保護規範、人工智慧法提案和網路與資訊安全 2 指令提案）。最後，本文的第三部分探討源自歐盟監管架構的可能挑戰。特別是，我們展望源自兩個立法提案的挑戰，以及它們與現有關於人工智慧醫療裝置網路安全的立法互動。它們被架構為對以下問題的回答：(1) 人工智慧法將如何與醫療裝置法規在網路安全和安全要求方面互動？(2) 我們應如何詮釋網路與資訊安全 2 指令提案和醫療裝置法規中的事故通知要求？(3) 關鍵基礎設施演進條款的後果是什麼？[這是草稿章節。最終版本將刊載於由 Barry Solaiman 和 I. Glenn Cohen 編輯的《健康、人工智慧與法律研究手冊》中，愛德華埃爾加出版社有限公司，2023 年出版]
...
摘要：阿茲海默症 (AD) 是一種進行性神經退化性疾病，也是失智症的主要原因。早期診斷對於患者受益於潛在的干預和治療至關重要。由於視網膜與大腦的解剖連接，因此假設視網膜是 AD 檢測的診斷部位。為此目的開發的 AI 模型尚未對決策提供合理的解釋，也沒有推斷出疾病進展的階段。沿著這個方向，我們提出了一個新的模型不可知可解釋 AI 框架，稱為顆粒神經元級別解釋器 (LAVA)，這是一個解釋原型，它探查卷積神經網路 (CNN) 模型的中間層，以直接從視網膜影像評估 AD 連續體，而無需縱向或臨床評估。此方法用於驗證視網膜血管作為阿茲海默症 (AD) 評估的生物標記和診斷方式。英國生物銀行的認知測試和血管形態特徵表明，LAVA 在識別進展連續體中的 AD 階段方面顯示出強大的前景和有效性。
...
摘要：針對可解釋 AI (XAI) 已開發出許多視覺化，但使用者通常需要進一步推論才能解讀。我們認為 XAI 應支援圖解和演繹推理，讓 AI 執行假設產生和評估，以縮小可解釋性差距。我們提出圖解化，以 i) 執行皮爾士演繹推理，ii) 遵循領域慣例，以及 iii) 以視覺或口頭方式透過圖表說明。我們針對臨床應用實作 DiagramNet，以從心臟聽診預測心臟診斷，並以基於形狀的雜音圖表說明。在建模研究中，我們發現 DiagramNet 不僅提供忠實的雜音形狀說明，且預測效能優於基準模型。我們在與醫學生的定性使用者研究中進一步展示圖解說明的可解釋性和可信度，顯示臨床相關的圖解說明優於技術顯著性地圖說明。這項工作有助於提供以使用者為中心的 XAI 的領域慣例演繹說明。
...
摘要：皮膚癌是人類最普遍的癌症類型之一。它主要是透過視覺辨識，從臨床篩檢開始，再進行皮膚鏡檢查、組織學評估和標本收集。深度卷積神經網路 (CNN) 針對分類的細粒度物件執行高度分離且潛在通用的任務。本研究提出一個新的多類別預測架構，基於 ViT 和 ViTGAN 對皮膚病灶進行分類。基於視覺轉換器的 GAN（生成對抗網路）用於解決類別不平衡問題。該架構包含四個主要階段：ViTGAN、影像處理和可解釋 AI。階段 1 包括產生合成影像，以平衡資料集中的所有類別。階段 2 包括應用不同的資料擴充技術和形態運算，以增加資料大小。階段 3 和 4 涉及開發一個適用於邊緣運算系統的 ViT 模型，該模型可以識別模式並對影像中可見使用者皮膚上的皮膚病灶進行分類。在階段 3 中，在使用 ViT 將病灶分類到所需的類別後，我們將使用可解釋 AI (XAI)，透過使用啟用映射等方式，產生更具可解釋性的結果，同時確保高預測準確度。皮膚疾病的即時影像可以由醫生或患者使用行動應用程式的相機拍攝，以進行早期檢查並確定皮膚病灶的原因。整個架構與現有的皮膚病灶偵測架構進行比較。
...
摘要：對於人工智慧 (AI) 在高風險環境（例如醫療保健）中的部署，提供可解釋性/可說明性或允許精細錯誤分析的方法至關重要。許多用於可解釋性/可說明性和精細錯誤分析的近期方法使用概念，這些概念是對人類具有語義意義的元標籤。然而，只有少數數據集包含概念級別的元標籤，而且這些元標籤大多與不需要領域專業知識的自然影像相關。專注於與單一疾病（例如黑色素瘤）相關的元標籤的醫學密集標記數據集。在皮膚科中，皮膚疾病使用既定的臨床詞彙進行描述，允許臨床醫生彼此描述身體檢查結果。為了提供由領域專家密集標記的醫療數據集，其中標記可用於多種疾病過程，我們開發了 SkinCon：由皮膚科醫生密集標記的皮膚疾病數據集。SkinCon 包含來自 Fitzpatrick 17k 數據集的 3230 張影像，並使用 48 個臨床概念進行密集標記，其中 22 個概念至少有 50 張影像代表該概念。所使用的概念是由兩位皮膚科醫生考慮用於描述皮膚病變的臨床描述詞而選擇的。範例包括「斑塊」、「鱗屑」和「糜爛」。相同的概念也用於標記來自 Diverse Dermatology Images 數據集的 656 張皮膚疾病影像，提供了具有多樣化膚色的額外外部數據集。我們回顧了 SkinCon 數據集的潛在應用，例如探測模型、基於概念的說明和概念瓶頸。此外，我們使用 SkinCon 來展示其中兩個用例：使用概念偵錯現有皮膚科 AI 模型的錯誤，以及使用事後概念瓶頸模型開發可解釋模型。
...
摘要：不安分的多臂強盜 (RMAB) 是一個流行的決策理論架構，已被用於模擬公共衛生、野生動物保育、通訊系統等領域中真實世界的順序決策問題。已部署的 RMAB 系統通常分兩個階段運作：第一個階段預測定義 RMAB 範例的未知參數，而第二個階段則採用最佳化演算法來解決已建構的 RMAB 範例。
在這項工作中，我們提供並分析了在公共衛生領域中首次部署 RMAB 系統的結果，目標在於改善孕產婦和兒童健康。我們的分析著重於了解預測準確度與已部署 RMAB 系統的整體效能之間的關係。這對於決定投資於改善預測準確度以提升最終系統效能的價值至關重要，且有助於診斷、監控已部署的 RMAB 系統。
使用來自我們已部署 RMAB 系統的真實世界資料，我們證明了整體預測準確度的提升甚至可能伴隨著 RMAB 系統效能的下降——廣泛投入資源來改善整體預測準確度可能無法產生預期的結果。在此之後，我們開發了以決策為重點的評估指標來評估預測組成，並證明其更能（無論在經驗上或理論上）說明已部署 RMAB 系統的整體效能。
...
摘要：可解釋 AI 在於開發機制，讓決策系統與人類之間得以互動，藉由讓前者的決策變得易於理解。這在敏感的脈絡中特別重要，例如在醫療領域。我們提出一個使用案例研究，針對皮膚病灶診斷，說明如何為從範例中訓練的最新深度神經網路分類器提供決策說明。我們的架構包含一個訓練好的分類器，說明模組會在該分類器上執行。後者能夠為分類診斷提供範例和反例，因此讓醫師能夠與自動診斷系統互動。範例是透過對抗式自動編碼器產生的。我們在具代表性的範例中說明系統的行為。
...
摘要：<paragraph>人工智慧（AI）應用中的演算法公平性對於建立更美好的社會至關重要。公平性作為社會機制的基礎公理，包含多個面向。儘管機器學習（ML）社群將焦點放在交叉性作為統計同質性的問題上，特別是在歧視議題上，但新興的文獻探討了另一個面向——單調性。根據領域專家，單調性在許多與公平性相關的領域中扮演至關重要的角色，而違反單調性可能會誤導人類決策並導致災難性的後果。在本文中，我們首先系統性地評估應用單調神經加法模型（MNAM）的重要性，該模型使用公平感知 ML 演算法來強制執行個人和成對單調性原則，以確保 AI 倫理和社會的公平性。我們透過理論推理、模擬和廣泛的實證分析的混合方法發現，在所有公平性領域（包括犯罪學、教育、醫療保健和金融）中，考量單調性公理至關重要。我們的研究有助於在 AI 倫理、可解釋 AI（XAI）和人機互動（HCI）的介面中進行跨領域研究。透過證明不符合單調性的災難性後果，我們探討了單調性需求在 AI 應用中的重要性。此外，我們證明 MNAM 是一種有效的公平感知 ML 方法，透過施加單調性限制來整合人類智慧。</paragraph>
...
摘要：<paragraph>在安全關鍵應用程式中，例如法律或醫療領域，解釋 AI 模型的預測至關重要。一種預測解釋形式是萃取依據，也就是一個實例的特徵子集，引導模型對該實例做出預測。例如，子詞組「他偷了手機」可以是「偷竊」預測的萃取依據。先前關於產生萃取依據的研究通常採用兩階段模型：一個選擇器，選擇最重要的特徵（也就是依據），接著是一個預測器，根據選定的特徵做出預測。這些研究的一個缺點是，學習選擇特徵的主要訊號來自於預測器給出的答案與真實答案的比較。在這項研究中，我們提議透過資訊校正方法，從預測器中擠出更多資訊。更精確地說，我們聯合訓練兩個模型：一個是典型的類神經網路模型，以準確但黑箱的方式解決手邊的任務，另一個是選擇器預測器模型，它會額外為其預測產生一個依據。第一個模型用作第二個模型的指南。我們使用對抗技術校正兩個模型萃取的資訊，使得它們之間的差異成為遺漏或過度選擇特徵的指標。此外，對於自然語言任務，我們提出一個基於語言模型的正則化器，以鼓勵萃取流暢的依據。情緒分析任務、仇恨言論辨識任務以及法律領域三個任務的實驗結果顯示，我們的依據萃取方法非常有效。</paragraph>
...
摘要：近期通過認證的人工智慧 (AI) 醫療保健工具大幅增加，讓這項技術的採用議題再度引發爭論。這場辯論的其中一條主軸，在於可解釋 AI (XAI) 及其承諾，要讓 AI 裝置更具透明度和可信度。在醫療 AI 領域中，有些活躍的聲音對可解釋 AI 技術的可靠性表達了疑慮，特別是特徵歸因法，並質疑其使用及納入準則和標準的做法。儘管有合理的疑慮，我們主張，對於事後局部可解釋性方法的可行性，現有的批評過於概括了僅限於影像資料的問題，因而將好壞一概否定。我們從將問題描述為解釋與人類理解之間缺乏語義對應開始。為了了解特徵重要性何時可以可靠地使用，我們區分了低階特徵與高階特徵的特徵重要性。我們主張，對於低階特徵具有明確語義的資料類型（例如電子病歷 (EHR) 等表格資料），可以獲得語義對應，因此特徵歸因法仍可被用於有意義且有用的方式。最後，我們勾勒出一個程序，用於測試是否已達成語義對應。
...
摘要：儘管人工智慧（AI）的機器學習（ML）模型在醫學領域表現優異，但並非沒有錯誤。賦予臨床醫師辨識錯誤模型建議的能力對於建立對醫療 AI 的信任至關重要。可解釋 AI（XAI）旨在透過釐清 AI 推論來滿足此項需求，以支援最終使用者。近期針對生物醫學影像的幾項研究已取得令人滿意的結果。儘管如此，使用表格資料的模型解決方案仍不足以滿足臨床醫師的需求。本文提出了一種方法，以協助臨床醫師辨識使用表格資料訓練的 ML 模型的失敗之處。我們的做法建立在三個主要支柱上：利用臨床背景潛在空間分解特徵集、評估整體解釋的臨床關聯性，以及基於潛在空間相似性（LSS）的局部解釋。我們在 ML 基於感染所造成的早產兒疾病識別上驗證了我們的做法。由於模型失敗，死亡風險、終身殘疾和抗生素抗藥性是此領域中一個公開的研究問題。我們的方法成功辨識出兩個模型的錯誤分類案例。我們的解決方案透過將局部解釋脈絡化，為臨床醫師提供可行的見解，以支援他們自主做出明智的最終決定。
...
摘要：<paragraph>我們正見證人工智慧經濟與社會的興起，其中人工智慧技術日益影響醫療保健、商業、運輸和日常生活中的許多面向。許多成功案例已報導出人工智慧系統甚至超越了人類專家的準確度。然而，人工智慧系統可能會產生錯誤、展現偏見、可能對資料中的雜訊敏感，而且往往缺乏技術和司法透明度，導致信任度降低，並在採用時面臨挑戰。這些最近的缺點和疑慮已記錄在科學文獻中，也記錄在一般媒體中，例如自動駕駛汽車事故、醫療保健中的偏見、有色人種的雇用和人臉辨識系統、看似正確的醫療決策後來發現是出於錯誤的原因等。這導致許多政府和法規倡議要求可信賴且符合道德的人工智慧，以提供準確性和穩健性、某種形式的可解釋性、人類控制和監督、消除偏見、司法透明度和安全性。提供可信賴人工智慧系統的挑戰激勵了對可解釋人工智慧系統 (XAI) 的深入研究。XAI 的目標是提供人類可以理解的資訊，說明人工智慧系統如何做出決策。在本文中，我們首先簡要總結目前的 XAI 工作，然後挑戰準確性與可解釋性互斥且僅專注於深度學習的最新論點。然後，我們提出我們的建議，說明在高風險可信賴人工智慧系統交付的完整生命週期中使用 XAI，例如開發、驗證和認證，以及可信賴的生產和維護。</paragraph>
...
摘要：深度神经網路 (DNN) 可偵測資料中的模式，並已在許多電腦視覺應用中展現出多功能性和強大的效能。
然而，單獨的 DNN 容易犯下違反簡單常識概念的明顯錯誤，且在使用明確知識來引導其搜尋和決策制定方面受到限制。雖然整體 DNN 效能指標可能良好，但這些明顯的錯誤加上缺乏可解釋性，已阻礙其廣泛採用於醫療影像分析等關鍵任務。本文的目的是介紹 SimpleMind，一個專注於醫療影像理解的認知 AI 開源軟體架構。它允許建立一個知識庫，以直觀的人類可讀形式描述影像物件預期的特徵和關係。SimpleMind 架構透過以下方式為 DNN 帶來思考能力：(1) 提供推理知識庫中關於影像內容的方法，例如空間推理和條件推理，以檢查 DNN 輸出；(2) 應用流程知識，以通用軟體代理的形式，將它們串連在一起以完成影像前處理、DNN 預測和結果後處理，以及 (3) 執行所有知識庫參數的自動共同最佳化，以適應代理到特定問題。SimpleMind 能對多個偵測到的物件進行推理以確保一致性，並在 DNN 輸出之間提供交叉檢查。這種機器推理透過可解釋的模型和可解釋的決策來提升 DNN 的可靠性和可信度。提供的範例應用程式展示了 SimpleMind 如何透過將深度神經網路嵌入在認知 AI 架構中來支援和改善它們。
...
摘要：在過去十年中，基於深度學習方法的視覺感知任務取得了驚人的進展，在從自動駕駛到臨床決策支援系統的眾多應用領域中得到廣泛採用。儘管這些基於深度學習的模型表現出色，但在決策過程中仍相當不透明，這使得它們在人類關鍵任務中的部署成為一項有風險的事業。這反過來又使得理解這些模型做出的決策對於它們的可靠部署至關重要。可解釋 AI (XAI) 方法試圖通過為此類黑盒深度學習方法提供解釋來解決這個問題。在本文中，我們對計算機視覺中的基於歸因的 XAI 方法進行了全面的調查，並回顧了基於梯度、基於擾動和基於對比的 XAI 的現有文獻，並提供了關於開發和評估穩健 XAI 方法的關鍵挑戰的見解。
...
摘要：自 10 年代中期以來，深度學習 (DL) 的時代持續至今，每年都帶來新的最高級和創新。儘管如此，這些創新轉化為實際應用程式的速度仍落後於這個快速步伐。特別是安全關鍵應用程式，其基礎在於嚴格的法規和道德要求，需要加以重視，並且仍然是積極的辯論領域。可解釋 AI (XAI) 和隱私保護機器學習 (PPML) 都是至關重要的研究領域，旨在減輕 DL 中盛行的資料密集黑盒模型的一些缺點。儘管在各自的領域中進行了熱烈的研究活動，但對於它們的互動尚未引起注意。這項工作是第一個研究私密學習技術對基於 DL 的模型所產生的解釋的影響。在涵蓋多個領域的各種影像和時間序列資料集，以及不同的私密技術、XAI 方法和模型架構的廣泛實驗分析中，研究了私密訓練對產生的解釋的影響。研究結果表明，透過引入私密性，解釋會產生不可忽視的變化。除了報告 PPML 對 XAI 的個別影響之外，本文還針對實際應用中技術的選擇提供了明確的建議。透過揭示這些關鍵技術的相互依賴性，這項工作是克服安全關鍵領域中實用 AI 所剩障礙的第一步。
...
摘要：肺結核（TB）是一種感染性細菌疾病，是造成死亡的主要原因，特別是在低收入國家，據估計在 2020 年全球通報了 1000 萬例新病例。雖然肺結核可以治療，但未遵守藥物治療方案是造成發病率和死亡率的主要原因。因此，主動找出有中斷藥物治療方案風險的患者，有助於採取矯正措施以減輕不良後果。我們使用極端不遵守治療的代理測量值，以及來自印度四個邦的近 70 萬名患者的數據集，根據自訂的基於排名指標制定並解決機器學習 (ML) 問題，以早期預測不遵守治療。我們訓練 ML 模型並根據基準進行評估，在考量全國範圍的大規模未來部署後，獲得比基於規則的基準高出約 100%，比隨機分類器高出約 214% 的提升。我們在過程中處理各種問題，包括資料品質、高基數類別資料、低目標患病率、分佈轉移、不同群組間的差異、演算法公平性，以及對穩健性和可解釋性的需求。我們的研究結果表明，不遵守治療患者的風險分層是一種可行的、可大規模部署的 ML 解決方案。作為印度中央肺結核部門的官方 AI 合作夥伴，我們正在與多個城市和州級試點計畫合作，目標是全印度部署。
...
摘要：可解釋人工智慧 (XAI) 藉由提升終端使用者對機器信任度，轉變了人工智慧 (AI) 領域。
隨著連網裝置數量持續增加，物聯網 (IoT) 市場需要對終端使用者而言是值得信賴的。
然而，現有文獻仍缺乏關於 XAI 在物聯網中使用的系統化且全面的調查工作。
為了彌補這個不足，我們在這篇論文中探討 XAI 架構，重點在於它們的特性和對物聯網的支持。
我們說明了廣泛使用的 XAI 服務，用於物聯網應用，例如安全性增強、醫療物聯網 (IoMT)、工業物聯網 (IIoT) 和城市物聯網 (IoCT)。
我們也建議在這些應用中透過適當的範例在物聯網系統上實作 XAI 模型的選擇，並總結出未來工作的關鍵推論。
此外，我們展示了邊緣 XAI 結構的尖端發展，以及對物聯網應用第六代 (6G) 通訊服務的支持，以及關鍵推論。
簡而言之，這篇論文構成了第一個針對未來物聯網用例需求量身打造的 XAI 基礎架構發展的整體彙編。
...
摘要：美國每年約有 125 萬人因燒傷接受治療。精確的燒傷分類是醫療 AI 領域的重要面向。在這項工作中，我們提出一個可解釋的人在迴路架構，用於改善燒傷超音波分類模型。我們的架構利用基於 LIME 分類說明器的說明系統，來驗證和整合燒傷專家的知識，建議新的特徵並確保模型的有效性。使用這個架構，我們發現 B 模式超音波分類器可以透過提供紋理特徵來增強。更具體地說，我們確認基於超音波影像灰階共生矩陣 (GLCM) 的紋理特徵可以提高轉移學習燒傷深度分類器的準確度。我們在豬隻受試者的真實資料上測試我們的假設。我們顯示在根據我們的架構修改後，燒傷深度分類的準確度從約 88% 提升到約 94%。
...
摘要：在醫療保健中，AI 系統支援臨床醫生和病人在診斷、治療和監控方面，但許多系統的解釋能力不佳，這對實際應用而言仍然是一項挑戰。克服這個障礙是可解釋 AI (XAI) 的目標。然而，每個人對解釋的理解可能不同，因此無法為所有人解決黑盒子問題。以人為本的 AI 領域透過調整 AI 來適應使用者，來處理這個問題。我們提出一個以使用者為中心的假想角色概念來評估 XAI，並使用它來調查行動健康壓力監控應用程式中，終端使用者對於各種解釋風格和內容的偏好。我們的線上調查結果顯示，使用者的個人資料和人格特質，以及解釋的類型，都會影響解釋偏好，這表示這些都是 XAI 設計的基本特徵。我們將結果歸納為三個典型的使用者假想角色：注重權力、休閒和隱私的使用者。我們的見解讓互動式、以人為本的 XAI 更接近實際應用。
...
摘要：青光眼是造成部分或完全失明的原因中排名第二的因素，在所有視覺缺陷中，主要是因為焦慮或憂鬱症導致眼睛壓力過大，進而損害視神經並造成視力上的併發症。傳統的青光眼篩檢是一個耗時的過程，需要醫療專業人員持續的關注，即便如此，由於時間的限制和壓力，他們時常無法正確分類，導致錯誤的治療。已經有許多努力嘗試自動化整個青光眼分類的程序，然而，這些現有的模型通常具有黑盒特性，使用戶無法理解預測背後的主要原因，因此醫療從業人員通常無法依賴這些系統。在本文中，我們在比較過各種預先訓練的模型後，提出了一個能夠以 94.71% 的準確度分類青光眼的遷移學習模型。此外，我們利用了局部可解釋模型不可知解釋（LIME），在我們的系統中引入了可解釋性。這項改進使醫療專業人員能夠獲得重要且全面的資訊，以協助他們做出判斷。它也降低了傳統深度學習模型的不透明性和脆弱性。
...
摘要：深度學習在偵測物件和分類影像方面展現出絕佳的效能，確保了分析醫學影像的絕佳前景。
將深度學習的成功轉化為醫學影像，其中醫師需要理解其背後流程，需要有能力詮釋和說明神經網路的預測。深度神經網路的可詮釋性通常仰賴於估計輸入特徵（例如，像素）相對於結果（例如，類別機率）的重要性。然而，已開發出許多重要性估計器（也稱為顯著性圖），目前尚不清楚哪些對於醫學影像應用較為相關。在目前的研究中，我們使用三個不同的評估指標，探討了數個重要性估計器在說明卷積深度網路對電腦斷層掃描 (CT) 影像進行分類的效能。首先，以模型為中心的保真度衡量在某些輸入受到擾動時，模型精確度的下降程度。其次，透過受試者操作特徵 (ROC) 曲線，在像素層級衡量重要性評分與專家定義的分割遮罩之間的一致性。第三，我們透過 Dice 相似性係數 (DSC) 衡量 XRAI-based 地圖與分割遮罩之間的區域重疊程度。整體而言，SmoothGrad 的兩個版本在保真度和 ROC 排名中名列前茅，而積分梯度和 SmoothGrad 則在 DSC 評估中表現優異。有趣的是，以模型為中心（保真度）和以人為中心（ROC 和 DSC）的評估之間存在關鍵差異。分割遮罩中內嵌的專家期望和直覺不一定與模型得出預測的方式一致。了解可詮釋性的這種差異將有助於利用深度學習在醫學上的力量。
...
摘要：深度學習方法，特別是卷積神經網路，已成為醫學影像運算任務中強大的工具。雖然這些複雜的模型提供了卓越的效能，但其黑箱性質可能會阻礙在高風險決策中採用於現實世界。在本文中，我們提出一個互動系統，以利用最先進的可解釋性技術，協助放射科醫師進行乳癌篩檢。我們的系統將深度學習模型整合到放射科醫師的工作流程中，並提供新穎的互動方式，以促進對模型決策過程的理解。此外，我們證明我們的系統可以逐步利用使用者互動，以提供更細緻的可解釋性報告，而標記工作量很小。由於所採用的可解釋性技術的通用性質，我們的系統與領域無關，可用於許多不同的醫學影像運算任務，提供了一個新的觀點，說明我們如何利用視覺分析將原本靜態的可解釋性技術轉換為擴充人類決策制定和促進採用醫療 AI。
...
摘要：如今，人工智能 (AI) 已成為醫療保健應用程式中不可或缺的組成部分，包括臨床和遠距醫療，但效能最佳的 AI 系統通常過於複雜，無法自行解釋。可解釋 AI (XAI) 技術旨在揭示系統預測和決策背後的推理，在處理敏感且個人的健康資料時，它們變得更加重要。值得注意的是，XAI 並未在不同的研究領域和資料類型中獲得相同的關注，特別是在醫療保健方面。特別是，許多臨床和遠距醫療應用程式分別基於表格資料和時間序列資料，而 XAI 並未針對這些資料類型進行分析，而電腦視覺和自然語言處理 (NLP) 則為參考應用程式。為了概述最適合醫療保健領域中表格資料和時間序列資料的 XAI 方法，本文回顧了過去 5 年的文獻，說明所產生的解釋類型以及評估其相關性和品質所付出的努力。具體來說，我們將臨床驗證、一致性評估、客觀且標準化的品質評估以及以人為中心的品質評估確定為確保對最終使用者進行有效解釋的主要特徵。最後，我們強調了該領域的主要研究挑戰以及現有 XAI 方法的局限性。
...
摘要：<paragraph>目標：本研究評估了市售可解釋 AI 演算法在提升臨床醫師識別胸部 X 光 (CXR) 上肺癌的能力方面所產生的影響。
設計：這項回溯性研究評估了 11 位臨床醫師在偵測胸部 X 光中的肺癌的表現，是否使用市售 AI 演算法（紅點，Behold.ai）的協助，該演算法可預測 CXR 中疑似肺癌。臨床醫師的表現根據臨床確診結果進行評估。
背景：這項研究分析了 NHS 醫院的匿名患者資料；該資料集包含 400 張成人患者（18 歲以上）的胸部 X 光，這些患者在 2020 年接受了 CXR 檢查，並有對應的臨床文字報告。
參與者：由 11 位臨床醫師（顧問放射科醫師、放射科醫師受訓人員和報告放射技師）組成的讀者小組參與了這項研究。
主要結果指標：臨床醫師在有和沒有 AI 輸入的情況下，偵測 CXR 上肺癌的整體準確度、敏感度、特異度和精確度。臨床醫師之間的一致性比率和效能標準差也在有和沒有 AI 輸入的情況下進行評估。
結果：臨床醫師使用 AI 演算法可改善肺部腫瘤偵測的整體表現，整體上增加了 17.4% 的肺癌在 CXR 上被識別出來，否則這些肺癌可能會被遺漏，整體上增加了較小腫瘤的偵測率，第一期和第二期肺癌的偵測率分別增加了 24% 和 13%，並標準化了臨床醫師的表現。
結論：這項研究顯示 AI 演算法在臨床實用性方面極具前景，可透過整體改善讀者的表現來提升早期肺癌診斷並促進健康公平，而不會影響下游影像資源。</paragraph>
...
摘要：缺乏足够大的开放式医疗数据库是 AI 驱动的医疗保健中最大的挑战之一。使用生成对抗网络 (GAN) 创建的合成数据似乎是缓解隐私政策问题的良好解决方案。另一种治疗方法是在不交换本地数据样本的情况下，跨多个医疗机构分散协议。在本文中，我们探索了集中式和分散式设置中的无条件和条件 GAN。集中式设置模仿了对大但高度不平衡的皮肤病变数据集的研究，而分散式设置则模拟了具有三个机构的更真实的医院场景。我们评估了模型在保真度、多样性、训练速度和基于生成合成数据训练的分类器的预测能力方面的性能。此外，我们还通过探索潜在空间和嵌入投影来提供可解释性，重点关注全局和局部解释。计算真实图像与其在潜在空间中的投影之间的距离证明了训练过的 GAN 的真实性和泛化性，这是此类应用程序中的主要关注点之一。进行的研究的开源代码可公开获得，网址为 \url{https://github.com/aidotse/stylegan2-ada-pytorch}。
...
摘要：在醫院的世界中，有許多複雜的組合問題，而解決這些問題對於提高病患滿意度和提供的照護品質非常重要。醫療保健中的問題很複雜，因為要解決這些問題，必須考慮到許多限制和不同類型的資源。此外，必須在短時間內評估解決方案，以確保在實際情況中可以使用。我們計畫提出解決這類問題的方案，既擴充已經測試過的方案，也透過模擬解決新問題，同時考慮文獻並在有現成資料時使用。解決這類問題很重要，但自從歐盟執委會制定《一般資料保護規範》，規定每個人都有權要求解釋 AI 做出的決定，因此在未發展出可解釋性方法之前，基於 AI 的求解器（例如基於答案集程式設計的求解器）的使用將受到限制。因此，研究的另一部分將致力於研究並提出新的方法來解釋所獲得的解決方案。
...
摘要：在當今現代社會中，越來越多人飽受慢性及重症疾病所苦，醫師與病患需要特殊且精密的醫療保健服務。因此，重要的醫療保健利益相關者已經了解到開發此類服務的重要性，讓病患的生活更輕鬆。此類支持需要收集大量的病患複雜資料，例如臨床、環境、營養、日常生活、來自智慧穿戴裝置的各種資料、來自配備感測器的衣物的資料等。整體病患資料必須適當地彙整、處理、分析並提供給醫師和照護者，以便建議適當的治療方式和行動，以改善病患的健康相關參數和整體健康狀況。進階的人工智慧技術提供機會分析此類大數據、使用它們，並衍生新的知識來支持個人化的醫療決策。像基於進階機器學習、聯邦學習、遷移學習、可解釋人工智慧的新方法為未來更優質地使用健康和醫療資料開啟了新途徑。在本文中，我們將提供一些關鍵面向和特徵範例，說明在個人化醫療決策中應用一系列人工智慧方法的領域。
...
摘要：<paragraph>持續監控訓練過的機器學習模型，以確定何時應信任或不信任其預測，對於安全部署至關重要。這樣的框架應具備高性能、可解釋性、事後性和可操作性。我們提出 TRUST-LAPSE，一個用於持續模型監控的「不信任」評分框架。我們使用一系列潛在空間嵌入來評估每個輸入範例模型預測的可信度。具體來說，(a) 我們的潛在空間不信任分數使用潛在空間中的距離度量（馬氏距離）和相似度量（餘弦相似度）來估計不信任，(b) 我們的順序不信任分數確定在非參數、基於滑動視窗演算法中過去輸入表示順序上的相關性偏差，以進行可操作的持續監控。我們透過兩個下游任務評估 TRUST-LAPSE：(1) 分布式轉移輸入偵測，以及 (2) 資料漂移偵測。我們使用公開資料集評估各種領域（音訊和視覺），並進一步針對具有挑戰性的真實世界腦電圖 (EEG) 資料集進行基準測試，以進行癲癇偵測。我們的潛在空間不信任分數達到最先進的結果，AUROC 為 84.1（視覺）、73.9（音訊）和 77.1（臨床 EEG），比基準高出 10 分以上。我們揭露了熱門基準中對輸入語義內容不敏感的關鍵失敗，使其不適合用於真實世界模型監控。我們展示了我們的順序不信任分數達到了很高的漂移偵測率；所有領域超過 90% 的串流顯示小於 20% 的誤差。透過廣泛的定性和定量評估，我們展示了我們的不信任分數更為強健，並提供可解釋性，以便於實務採用。</paragraph>
...
摘要：機器學習的普及增加了不公平模型在高風險應用程式中部署的風險，例如司法系統、藥物/疫苗設計和醫療診斷。儘管有有效的方法可以從頭開始訓練公平模型，但如何自動揭露和解釋訓練模型的不公平性仍然是一項具有挑戰性的任務。以可解釋的方式揭示機器學習模型的不公平性是邁向公平且值得信賴的人工智慧的關鍵一步。在本文中，我們系統性地應對了通過挖掘可解釋證據 (RUMIE) 來揭示不公平模型的新任務。關鍵思想是以一組數據實例的形式找到確鑿的證據，這些實例受到模型的最大歧視。為了使證據可解釋，我們還找到了一組人類可以理解的關鍵屬性和決策規則，這些屬性和決策規則描述了受到歧視的數據實例，並將其與其他未受到歧視的數據區分開來。正如在許多真實世界數據集上進行的廣泛實驗所證明的那樣，我們的模型找到了高度可解釋且確鑿的證據，可以有效揭示訓練模型的不公平性。此外，它比所有基線方法的可擴展性都要高得多。
...
摘要：可解釋 AI (XAI) 在安全關鍵系統中是必要的，例如在臨床診斷中，因為有做出致命決策的高風險。然而，目前 XAI 仍像是一組鬆散的方法，而不是一個定義明確的流程。在這項工作中，我們闡述了 XAI 中最大的子群體，可解釋機器學習 (IML) 與古典統計之間的概念相似性。基於這些相似性，我們提出了 IML 的形式化，類似於統計流程。採用這種統計觀點，讓我們能夠將機器學習模型和 IML 方法解釋為精密的統計工具。基於這種解釋，我們推論出三個關鍵問題，我們將其確定為 IML 在安全關鍵設定中成功和採用的關鍵。透過制定這些問題，我們進一步旨在激發關於 IML 與古典統計之間的區別，以及我們的觀點對該領域未來的影響的討論。
...
摘要：人工智慧已普及於各個領域，生物醫學影像與訊號處理也不例外。對於此主題日益增長且廣泛的興趣，已觸發了大量的研究活動，這反映在指數級的研究成果中。透過研究大量的且多樣化的生物醫學資料，機器與深度學習模型已革新了各種任務，例如建模、分割、配準、分類與合成，其表現優於傳統技術。然而，將結果轉換為生物學/臨床可解釋資訊的難度，阻礙了其在該領域的充分運用。可解釋人工智慧 (XAI) 嘗試透過提供方法來使模型可解釋並提供解釋，以填補這種轉譯差距。迄今已提出不同的解決方案，並獲得社群越來越濃厚的興趣。本文旨在提供生物醫學資料處理中 XAI 的概觀，並指出 2022 年 3 月將出現在 IEEE 訊號處理雜誌的生物醫學影像與訊號處理深度學習專題。
...
摘要：在使用醫學影像進行診斷時，無論是臨床醫師或人工智慧 (AI) 系統，影像品質都很重要。當影像品質不佳時，產生該影像的醫療檢查通常需要重新執行。在遠距醫療中，一個常見的問題是，只有在患者離開診所後才會標示出品質問題，這表示他們必須回來重新檢查。這對居住在偏遠地區的人來說特別困難，而這些人佔巴西一家數位醫療保健組織 Portal Telemedicina 的患者中很大一部分。在本文中，我們報告關於 (i) 開發一個 AI 系統以標示和說明低品質醫學影像的進行中工作，(ii) 一項訪談研究，以了解在 OurCompany 使用 AI 系統的利害關係人對說明的需求，以及 (iii) 一個縱向使用者研究設計，以檢視在我們的診所中，說明對技術人員工作流程的影響。據我們所知，這將是第一個評估 XAI 方法對最終使用者（使用 AI 系統但沒有 AI 專門知識的利害關係人）影響的縱向研究。我們歡迎對我們的實驗設置提供回饋和建議。
