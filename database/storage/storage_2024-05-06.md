# arxiv-daily
 Automated deployment @ 2024-05-06 09:31:04 Asia/Taipei
...
摘要：專有的語言模型，例如 GPT-4，通常用於評估各種語言模型的回應品質。然而，包括透明度、可控性和經濟實惠性的問題強烈地促使開發專門用於評估的開源語言模型。另一方面，現有的開放式評估器語言模型表現出嚴重的缺點：1) 它們發出的分數與人類分配的分數有顯著差異，以及 2) 它們缺乏執行直接評估和成對排名這兩種最普遍的評估形式的靈活性。此外，它們不具備根據自訂評估標準進行評估的能力，而是專注於一般屬性，例如有幫助性和無害性。為了解決這些問題，我們引入了 Prometheus 2，它比其前身更強大的評估器語言模型，它密切反映了人類和 GPT-4 的判斷。此外，它能夠處理與使用者定義的評估標準分組的直接評估和成對排名格式。在四個直接評估基準和四個成對排名基準上，Prometheus 2 在所有測試的開放式評估器語言模型中與人類和專有語言模型評審的分數相關性和一致性最高。我們的模型、程式碼和資料都公開於 https://github.com/prometheus-eval/prometheus-eval。
...
摘要：大型語言模型 (LLM) 已被證明能夠執行長期機器人任務的高階規劃，但現有方法需要存取預先定義的技能庫 (例如，挑選、放置、拉動、推動、導航)。然而，LLM 規劃並未說明如何設計或學習這些行為，這在長期設定中仍然具有挑戰性。此外，對於許多感興趣的任務，機器人需要能夠以細緻的方式調整其行為，這需要代理能夠修改低階控制動作。我們是否可以改用 LLM 的網路規模知識來制定高階政策，引導強化學習 (RL) 政策有效率地解決機器人控制任務，而不需要預先設定的技能組？在本文中，我們提出 Plan-Seq-Learn (PSL)：一種模組化方法，使用動作規劃來彌補抽象語言和學習低階控制之間的差距，從頭開始解決長期機器人任務。我們證明 PSL 在超過 25 個具有挑戰性的機器人任務中取得最先進的結果，最多有 10 個階段。PSL 從原始視覺輸入中解決長期任務，橫跨四個基準，成功率超過 85%，優於基於語言、傳統和端到端的方法。影片結果和程式碼請參閱 https://mihdalal.github.io/planseqlearn/
...
摘要：概念瓶頸模型 (CBM) 將影像分類建立在人類可理解的概念上，以允許可解釋的模型決策。至關重要的是，CBM 設計本質上允許人類介入，專家使用者可以修改潛在錯誤對齊的概念選擇，以可解釋的方式影響模型的決策行為。然而，現有的方法通常需要對每張影像進行大量的人工介入才能達到強大的效能，在取得人工回饋成本高昂的情況下，這會造成實際挑戰。在本文中，我們發現這顯著地受到介入期間概念獨立處理的驅動，其中一個概念的改變並不會影響模型最終決策中其他概念的使用。為了解決這個問題，我們引入了可訓練的概念介入重新對齊模組，它利用概念關係在介入後重新對齊概念指派。在標準的真實世界基準中，我們發現概念重新對齊可以顯著改善介入效能；顯著減少達到目標分類效能或概念預測準確度所需的介入次數。此外，它可以輕鬆整合到現有的基於概念的架構中，而不需要更改模型本身。這種降低的人機協作成本對於在資源受限的環境中提升 CBM 的可行性至關重要。
...
摘要：對齊是一種標準程序，用於微調預先訓練的大型語言模型 (LLM)，以遵循自然語言指令並作為有用的 AI 助理。然而，我們觀察到，傳統的對齊過程無法增強 LLM 的事實準確性，並且經常導致產生更多虛假事實（即幻覺）。在本文中，我們研究如何讓 LLM 對齊過程更具事實性，首先找出導致對齊步驟中產生幻覺的因素：監督微調 (SFT) 和強化學習 (RL)。特別是，我們發現訓練 LLM 學習新知識或不熟悉的文本會助長幻覺。這使得 SFT 的事實性降低，因為它訓練的是對 LLM 來說可能是新穎的人類標記數據。此外，標準 RL 中使用的獎勵函數也可能助長幻覺，因為它引導 LLM 對各種指令提供更多有用的回應，通常偏好更長且更詳細的回應。基於這些觀察，我們提出事實感知對齊，它包含事實感知 SFT 和通過直接偏好優化的事實感知 RL。實驗表明，我們提出的事實感知對齊引導 LLM 輸出更多的事實回應，同時保持遵循指令的能力。
...
摘要：深度學習分類和基礎模型對未見數據的概化仍然知之甚少。如何評估網路在小樣本學習、分布外概化和領域適應精神下適應其輸入空間的新版本或擴充版本的能力？網路的哪幾層最有可能概化得最好？我們提供了一種新的方法來評估網路表示取樣領域的能力，無論網路是否已針對領域中的所有類別進行訓練。我們的做法如下：在特定領域對視覺分類的最新預訓練模型進行微調後，我們評估它們在該領域相關但不同的變異中資料上的效能。概化能力被量化為來自中間層未見資料的潛在嵌入函數，適用於無監督和監督設定。在網路的所有階段進行作業，我們發現 (i) 高分類準確度並不表示高概化能力；以及 (ii) 模型中的較深層並非總是概化得最好，這對剪枝有影響。由於在不同資料集觀察到的趨勢在很大程度上是一致的，因此我們得出結論，我們的做法揭示了模型不同層的內在概化能力（函數）。
...
摘要：已提出各種對齊語言模型的方法，包括監督微調、RLHF 和直接最佳化方法，例如 DPO。
儘管 DPO 因其直接的訓練過程和競爭力的結果而迅速獲得普及，但仍存在一個開放的問題，即是否仍然有使用判別器（例如獎勵模型）來評估回應的實際優勢。我們提出 D2PO，判別器引導的 DPO，一種在整個學習過程中收集偏好的線上設定方法。
當我們收集黃金偏好時，我們不僅使用它們來訓練我們的政策，還使用它們來訓練一個區別性的回應評估模型，以對更多合成數據進行銀標籤處理以進行政策訓練。我們在各種任務中探索這種方法，包括現實的聊天設定，我們發現與具有相同數據預算的 DPO 相比，我們的做法會產生更高品質的輸出，並且在偏好數據需求方面更有效率。此外，我們展示了銀標籤最有助益的條件：它在使用 DPO 訓練政策時最有效，優於傳統的 PPO，並且受益於保持判別器與政策模型分開。
...
摘要：傳統上，自然語言處理 (NLP) 模型經常使用由語言專家建立的一組豐富特徵，例如語義表示。然而，在大語言模型 (LLM) 的時代，越來越多的任務轉變為通用、端對端的序列生成問題。在本文中，我們探討以下問題：在 LLM 的時代，語義表示扮演什麼角色？具體來說，我們探討抽象意義表示 (AMR) 在五項不同的 NLP 任務中的效果。我們提出了一個由 AMR 驅動的思考鏈提示方法，我們稱之為 AMRCoT，並發現它通常比有幫助的情況下更損害效能。為了探討 AMR 在這些任務中可能提供的內容，我們進行了一系列的分析實驗。我們發現很難預測 AMR 可能在哪些輸入範例中提供幫助或造成損害，但錯誤往往出現在多字詞表達、命名實體，以及 LLM 必須將其在 AMR 上的推理與其預測聯繫起來的最後推論步驟。我們建議在 LLM 的語義表示的未來工作中專注於這些領域。我們的程式碼：https://github.com/causalNLP/amr_llm。
...
摘要：儘管大多數關於可控文本生成的相關研究都專注於引導基礎語言模型，但新興的指令微調和提示範例提供了一個可控性的替代方法。我們編制並發布 ConGenBench，一個由 17 個不同的可控生成任務組成的測試平台，使用其子集來評量 9 個不同基線和方法在指令微調語言模型上的效能。令人驚訝的是，我們發現基於提示的方法在多數資料集和任務中優於可控文本生成方法，突顯了針對特定指令微調語言模型進行可控文本生成研究的必要性。基於提示的方法在多數文體任務中與人類表現相符，但在結構任務中表現落後，突顯了研究更多元約束和更具挑戰性的文體任務的必要性。為了促進此類研究，我們提供了一個演算法，它僅使用任務資料集和具備情境內功能的大語言模型來自動生成約束資料集。此方法消除了領域對預先策劃的約束資料集的依賴性，因此大幅擴展了未來可以研究的約束範圍。
...
摘要：近年來見證了大量大型多模態模型 (LMM) 有效解決單一影像視覺語言任務。然而，它們解決多影像視覺語言任務的能力仍有待改進。現有的多影像 LMM（例如 OpenFlamingo、Emu、Idefics 等）大多透過預先訓練數億個來自網路的雜訊交錯影像文字資料來獲得多影像能力，既不有效率也不有效。在本文中，我們旨在透過使用學術級資源進行指令調整來建構強大的多影像 LMM。因此，我們精心建構了包含來自 14 個多影像資料集的 721K 個實例的 Mantis-Instruct。我們設計 Mantis-Instruct 以涵蓋不同的多影像技能，例如共同參考、推理、比較、時間理解。我們將 Mantis-Instruct 與幾個單一影像視覺語言資料集結合，以訓練我們的模型 Mantis 處理任何交錯的影像文字輸入。我們在五個多影像基準和八個單一影像基準上評估訓練後的 Mantis。儘管僅需要學術級資源（即在 16xA100-40G 上使用 36 小時），Mantis-8B 仍可以在所有多影像基準上達到最先進的效能，並以平均 9 個絕對點數擊敗現有的最佳多影像 LMM Idefics2-8B。我們觀察到 Mantis 在內部和外部評估基準上的表現同樣出色。我們進一步在單一影像基準上評估 Mantis，並證明 Mantis 可以維持與 CogVLM 和 Emu2 相當的強大單一影像效能。我們的結果特別令人鼓舞，因為它表明低成本指令調整在建構多影像 LMM 方面確實比密集預先訓練有效得多。
...
摘要：與人類價值觀和偏好一致的大語言模型 (LLM) 對於讓它們變得有幫助且安全至關重要。然而，建立用於執行一致性的有效工具可能具有挑戰性，特別是對於通常包含數十億或數百億個參數的規模最大且最具能力的 LLM。我們建立了 NeMo-Aligner，這是一個模型一致性工具包，可以有效擴展到使用數百個 GPU 進行訓練。NeMo-Aligner 隨附了針對主要模型一致性範例的高度優化且可擴展的實作，例如：人類回饋強化學習 (RLHF)、直接偏好最佳化 (DPO)、SteerLM 和自玩微調 (SPIN)。此外，我們的工具包支援在參數有效微調 (PEFT) 設定中執行大部分一致性技術。NeMo-Aligner 被設計成可擴充的，允許以最小的工作量支援其他一致性技術。它採用 Apache 2.0 授權開放原始碼，我們邀請社群在 https://github.com/NVIDIA/NeMo-Aligner 中做出貢獻
...
摘要：大型视觉语言模型 (VLM) 已在需要对文字图像和文本进行细致理解的任务中展示了强大的推理能力，例如视觉问答或视觉蕴涵。然而，当模型面对包含隐喻或幽默等比喻现象的图像和标题时，对这些模型的能力几乎没有探索，而这些现象的含义通常是隐含的。为了弥补这一差距，我们提出了一个新任务和一个高质量数据集：具有文本解释的视觉比喻语言理解 (V-FLUTE)。我们将视觉比喻语言理解问题构建为一个可解释的视觉蕴涵任务，其中模型必须预测图像（前提）是否蕴涵一个主张（假设），并用文本解释来证明预测的标签。使用人机协作框架，我们构建了一个高质量数据集 V-FLUTE，其中包含 6,027 个<图像、主张、标签、解释>实例，涵盖五种不同的多模态比喻现象：隐喻、明喻、习语、讽刺和幽默。比喻现象可以出现在图像、标题或两者中。我们进一步进行自动和人工评估，以评估当前 VLM 在理解比喻现象方面的能力。
...
摘要：聊天機器人，例如 GPT-4 和 ChatGPT，目前服務於數百萬用戶。儘管它們被廣泛使用，但仍然缺乏公開的資料集，展示這些工具如何實際上被一群用戶使用。為了彌補這個差距，我們為線上使用者免費提供 ChatGPT，以換取他們積極、自願選擇匿名收集他們的聊天記錄和請求標頭。由此，我們編制了 WildChat，一個包含 100 萬使用者-ChatGPT 對話的語料庫，其中包含超過 250 萬個互動回合。我們將 WildChat 與其他流行的使用者-聊天機器人互動資料集進行比較，發現我們的資料集提供了最多樣化的使用者提示，包含最多的語言，並為研究人員提供了最豐富的潛在有毒使用案例。除了帶有時間戳記的聊天記錄外，我們還使用人口統計資料豐富了資料集，包括州、國家和雜湊 IP 地址，以及請求標頭。這種擴充允許對不同地理區域和時間維度中的使用者行為進行更詳細的分析。最後，由於它涵蓋了廣泛的用例，我們展示了該資料集在微調遵循指令的模型中的潛在效用。WildChat 在 https://wildchat.allen.ai 下根據 AI2 ImpACT 許可證發布。
...
摘要：AI 基礎模型在各種應用中獲得關注，包括放射學等醫療領域。然而，醫療基礎模型通常在有限的任務上進行測試，這讓它們的泛化性和偏差未被探索。我們展示了 RayDINO，這是一個大型視覺編碼器，通過 873k 胸部 X 光片的自我監督進行訓練。我們將 RayDINO 與之前最先進的模型進行比較，涵蓋了從分類和密集分割到文本生成的九項放射學任務，並對我們模型的人口、年齡和性別偏差進行了深入分析。我們的研究結果表明，自我監督允許以患者為中心的 AI 在臨床工作流程中證明其有用性，並全面解釋 X 光片。通過 RayDINO 和特定於任務的小型適配器，我們達到了最先進的結果，並改進了對未見人群的泛化，同時減輕了偏差，說明了基礎模型的真正前景：通用性和魯棒性。
...
摘要：預先訓練好的對比視覺語言模型已展現出在各種任務中的顯著效能。然而，它們通常在預先訓練期間未充分呈現類別的精細訓練資料集上遇到困難，這使得適應變得必要。最近的研究顯示，利用來自網路規模資料庫的樣本進行檢索增強適應，特別是在低資料量模式中，已展現出有前景的結果。儘管有經驗上的成功，但瞭解檢索如何影響視覺語言模型的適應仍然是一個開放的研究問題。在這項工作中，我們採用反思觀點，透過提出系統性研究來瞭解檢索增強適應中關鍵組成的角色。我們揭示了單模態和跨模態檢索的新見解，並強調了邏輯集合對有效適應的關鍵作用。我們進一步提出了直接支援我們經驗觀察的理論基礎。
...
摘要：<paragraph>此論文介紹了 UQA，這是一個針對問題解答和文字理解的新型資料集，適用於烏爾都語，這是一種擁有超過 7000 萬名母語人士的低資源語言。UQA 是透過翻譯史丹佛問題解答資料集 (SQuAD2.0) 而產生的，這是一個大型的英語 QA 資料集，使用一種稱為 EATS（封閉到錨點、翻譯、搜尋）的技術，它保留了翻譯語境段落中的答案跨度。本文描述了在兩個候選者中選擇和評估最佳翻譯模型的過程：Google 翻譯和 Seamless M4T。本文還在 UQA 上對幾個最先進的多語言 QA 模型進行了基準測試，包括 mBERT、XLM-RoBERTa 和 mT5，並報告了有希望的結果。對於 XLM-RoBERTa-XL，我們的 F1 分數為 85.99，EM 為 74.56。UQA 是用於開發和測試烏爾都語多語言 NLP 系統以及增強現有模型的跨語言可傳遞性的寶貴資源。此外，本文展示了 EATS 在為其他語言和領域建立高品質資料集方面的有效性。UQA 資料集和程式碼可在 www.github.com/sameearif/UQA 公開取得。</paragraph>
...
摘要：在本文中，我們討論了將計算創意 (CC) 與大型語言和視覺模型 (LLVM) 的研究相結合的方法，以解決這些模型的一個關鍵限制，即創造性問題解決。我們提出初步實驗，展示如何將 CC 原則應用於通過增強提示來解決這個限制。通過這項工作，我們希望促進在計算創意背景下討論 ML 演算法，以解決 LLVMs 中的創造性問題。我們的程式碼位於：
https://github.com/lnairGT/creative-problem-solving-LLMs
...
摘要：本文探討使用大型語言模型 (LLM) 自動化硬體描述碼的生成，旨在探索其在支援和強化高效能神經型態運算架構開發的潛力。建立在我們先前的研究基礎上，我們使用 OpenAI 的 ChatGPT4 和自然語言提示來合成一個可程式化遞迴脈衝神經網路的 RTL Verilog 模組，同時也產生測試平台來評估系統的正確性。結果設計在三個案例研究中獲得驗證，包括獨佔或、IRIS 花朵分類和 MNIST 手寫數字分類，準確度高達 96.6%。為了驗證其可合成性和可實作性，該設計在現場可程式閘陣列上製作原型，並使用開源電子設計自動化流程在 SkyWater 130 奈米技術上實作。此外，我們已將其提交給 Tiny Tapeout 6 晶片製造計畫，以進一步評估系統未來在晶片上的效能。
...
摘要：大型 2D 視覺語言模型 (2D-LLM) 透過使用簡單的投影儀將大型語言模型 (LLM) 與影像連結起來，因而獲得了顯著的關注。受到其成功的啟發，大型 3D 點雲語言模型 (3D-LLM) 也將點雲整合到 LLM 中。然而，將點雲直接與 LLM 對齊需要昂貴的訓練成本，通常在 A100 上需要數百個 GPU 小時，這阻礙了 3D-LLM 的發展。在本文中，我們介紹 MiniGPT-3D，這是一個高效且強大的 3D-LLM，在一個 RTX 3090 上僅訓練 27 小時就達到了多個 SOTA 結果。具體來說，我們建議使用 2D-LLM 的 2D 先驗將 3D 點雲與 LLM 對齊，這可以利用 2D 和 3D 視覺資訊之間的相似性。我們介紹了一種新穎的四階段訓練策略，以串聯的方式進行模態對齊，以及一個混合查詢專家模組，以高效率自適應聚合特徵。此外，我們利用參數高效的微調方法 LoRA 和 Norm 微調，僅產生 47.8M 個可學習參數，這比現有方法少了 260 倍。大量的實驗表明，MiniGPT-3D 在 3D 物件分類和字幕任務上達到了 SOTA，訓練成本顯著降低。值得注意的是，與 ShapeLLM-13B 相比，MiniGPT-3D 在具有挑戰性的物件字幕任務上獲得了 GPT-4 評分 8.12 的提升，而後者在 8 個 A800 上共耗費了 160 個 GPU 小時。我們是第一個探索高效的 3D-LLM 的人，為社群提供了新的見解。程式碼和權重可在 https://github.com/TangYuan96/MiniGPT-3D 取得。
...
摘要：經食道心臟超音波 (TEE) 在心臟病學的診斷和介入性治療中扮演著關鍵的角色。然而，由於影像擷取和解讀的複雜性，要有效運用它需要廣泛的訓練。為了提升新手超音波檢查員的效率並減少掃描擷取的變異性，我們提出了一種新的超音波 (US) 導航輔助方法，它基於對比學習作為目標條件強化學習 (GCRL)。我們使用一種新的對比病人批次處理方法 (CPB) 和資料擴充對比損失來擴充先前的架構，我們證明兩者對於確保對不同病人的解剖變異進行概化至關重要。所提出的架構能透過單一模型導航到標準診斷和複雜的介入性影像。我們的技術是使用 789 名病人的大型資料集開發的，在 140 名病人的測試資料集上，位置的平均誤差為 6.56 毫米，角度為 9.36 度，與針對個別影像訓練的模型相比，具有競爭力或更優異。此外，我們定量驗證了我們的方法導航到介入性影像（例如左心耳 (LAA) 封閉中使用的左心耳附屬物 (LAA) 影像）的能力。我們的做法有望在經食道超音波檢查中提供有價值的指導，有助於提升心臟超音波從業者的技能習得。
...
摘要：自然語言解釋已成為評估可解釋和多步驟自然語言推理 (NLI) 模型的代理。然而，評估 NLI 解釋的有效性具有挑戰性，因為它通常涉及適當數據集的群眾外包，這是一個耗時且容易出現邏輯錯誤的過程。為了解決現有的限制，本文探討了通過整合大型語言模型 (LLM) 和定理證明器 (TP) 來驗證和改進自然語言解釋。具體來說，我們提出了一個名為 Explanation-Refiner 的神經符號框架，它使用 LLM 擴充了一個 TP，以生成和形式化解釋性句子，並提出 NLI 的潛在推理策略。反過來，TP 被用於提供對解釋邏輯有效性的正式保證，並為後續改進提供反饋。我們展示了 Explanation-Refiner 如何被聯合用於評估解釋性推理、自動形式化和最先進 LLM 的錯誤校正機制，以及自動提高不同領域中人類註釋解釋的質量。
...
摘要：在口語中，發音精準度降低的情況很常見，但對於對話來說，其音響特性和語用功能的研究卻很少。我們在此嘗試彌補這個差距。這份技術報告包含從期刊文章（Ward 等人，2024 年，已提交）中遺漏的內容。具體來說，我們在此報告 1) 關於標註感知到的簡化的經驗教訓，2) 與朗讀的語音不同，對話中簡化的相關性包括高音高、寬音高範圍和強度，以及 3) 一個用於預測對話中簡化的基準模型，使用簡單的音響/韻律特徵，與英語的人類感知相關性達到 0.24，與西班牙語相關性達到 0.17。我們還提供了英語中簡化其他可能的語用功能的範例，以及各種討論、觀察和推測
...
摘要：大型機器（例如粒子加速器）通常由經驗豐富的操作員團隊運行。對於粒子加速器，這些操作員具備加速器物理和構成機器技術的適當背景知識。由於機器複雜，機器的特定子系統由專家負責，操作員可以向他們求助。在這項工作中，推理和動作 (ReAct) 提示範例用於將開放權重大型語言模型 (LLM) 與高級機器控制系統框架和其他工具（例如電子日誌或機器設計文件）結合在一起。藉此實作多專家檢索增強生成 (RAG) 系統，協助操作員執行知識檢索任務，在需要時直接與機器互動，或撰寫高級控制系統腳本。這種專家知識和機器互動的整合可以簡化和加速新舊人類操作員的機器操作任務。
...
摘要：彌合大型語言模型的英文和非英文表現之間的重大差距是一項巨大的挑戰。雖然一些先前的研究嘗試使用翻譯的訓練資料來縮小這個差距，但最近提出的問題比對方法利用了模型的英文專業知識，以最少使用昂貴且容易出錯的翻譯來改善多語言表現。在本文中，我們探討了這種方法可以廣泛應用的程度，方法是檢視它在可執行程式碼推理和常識推理中的影響。我們還探討如何使用代理調整有效率地將這種方法應用於極大型語言模型。多語言推理基準 mGSM、mSVAMP 和 xCSQA 的實驗結果證明，問題比對方法可用於提升各種推理情境、模型系列和規模的多語言表現。例如，當應用於 LLaMA2 模型時，我們的模型即使在 70B 模型中，也能讓 mGSM 的平均準確度提升 12.2%。為了了解其成功的機制，我們分析了表徵空間、思考鏈和翻譯資料規模，揭示了問題翻譯訓練如何強化 LLM 內部的語言比對並形塑其工作模式。
...
摘要：本研究引入了一種創新的 AI 驅動精準農業系統，利用 YOLOv8 進行疾病識別，並利用檢索增強生成 (RAG) 進行情境感知診斷。專注於解決影響卡納塔克邦咖啡生產部門的疾病挑戰，該系統將先進的物體檢測技術與語言模型相結合，以解決與大型語言模型 (LLM) 相關的固有約束。我們的技術不僅解決了 LLM 中的幻覺問題，還引入了動態疾病識別和補救策略。實時監控、協作式數據集擴展和組織參與確保了系統在多樣化農業環境中的適應性。建議系統的效果超越了自動化，旨在確保糧食供應、保護生計並促進環保農業實務。通過促進精確的疾病識別，該系統有助於實現可持續和注重環境的農業，減少對農藥的依賴。展望未來，該項目預計 RAG 集成的物體檢測系統將持續發展，強調可擴展性、可靠性和可用性。本研究致力於成為農業正面變革的燈塔，與全球在可持續和技術增強食品生產方面的努力保持一致。
...
摘要：對遞迴尖峰神經網路 (RSNN) 進行編程以穩健地執行多時間尺度運算仍然是一項艱難的挑戰。為了解決這個問題，我們展示了向量符號架構 (VSA) 提供的分布式方法如何使用高維隨機向量作為表示的最小單位，可以利用來將穩健的多時間尺度動態嵌入到基於吸引子的 RSNN 中。我們通過疊加對稱自聯想權重矩陣和非對稱轉換項將有限狀態機嵌入到 RSNN 動態中。轉換項由輸入的 VSA 結合和狀態之間的異聯想外部乘積形成。我們的做法通過具有高度非理想權重的模擬進行驗證；實驗性的閉環憶阻器硬體設置；以及在 Loihi 2 上，它可以無縫地擴展到大型狀態機。這項工作證明了 VSA 表示對於將穩健運算與遞迴動態嵌入到神經形態硬體中是有效的，而不需要參數微調或重要的特定於平台的最佳化。這推動了 VSA 成為神經形態硬體中認知演算法的高階表示不變抽象語言。
...
摘要：大型語言模型 (LLM) 已成為各種自然語言任務和一系列應用領域中強大的支援工具。最近的研究專注於探索它們在資料註解方面的能力。本文提供了對十二項研究的比較概述，這些研究探討了 LLM 在標籤資料方面的潛力。儘管這些模型展示了有望降低成本和節省時間的優點，但仍存在相當大的限制，例如代表性、偏見、對提示變化的敏感性以及偏好英語語言。利用這些研究的見解，我們的實證分析進一步探討了人類和 GPT 生成的意見分佈在四個主觀資料集之間的一致性。與探討代表性的研究相反，我們的研究方法直接從 GPT 獲得意見分佈。因此，我們的分析支持少數在評估資料註解任務時考慮不同觀點的研究，並強調需要朝這個方向進行進一步的研究。
...
摘要：本文探討使用混合 CTC/注意力編碼器-解碼器模型，並使用中間 CTC (InterCTC) 訓練愛爾蘭語 (蓋爾語) 低資源語音辨識 (ASR) 和方言辨識 (DID)。將結果與目前訓練 ASR (TDNN-HMM) 和 DID (ECAPA-TDNN) 的最佳表現模型進行比較。最初使用 Conformer 編碼器建立最佳 InterCTC 設定。然後使用此設定訓練具有 E-branchformer 編碼器的模型，並比較兩種架構的效能。採用多任務微調方法進行語言模型 (LM) 淺層融合。實驗顯示，與基準 ECAPA-TDNN 相比，DID 準確度提升了 10.8%，而 WER 效能接近 TDNN-HMM 模型。這種多任務方法成為愛爾蘭語低資源 ASR 和 DID 的一個有前途的策略。
...
摘要：非自迴歸 (NAR) 語言模型以其在神經機器翻譯 (NMT) 中的低延遲而聞名。然而，由於解碼空間大且難以準確捕捉目標詞之間的依賴關係，因此 NAR 和自迴歸模型之間存在效能差距。更複雜的是，為 NAR 模型準備適當的訓練資料是一項非平凡的任務，通常會加劇暴露偏差。為了應對這些挑戰，我們將強化學習 (RL) 應用於 Levenshtein Transformer，這是一個基於編輯的 NAR 代表模型，證明使用自生資料的 RL 可以增強基於編輯的 NAR 模型的效能。我們探討兩種 RL 方法：逐步獎勵最大化和情節獎勵最大化。我們討論這兩種方法各自的優缺點，並以經驗驗證它們。此外，我們以實驗方式探討溫度設定對效能的影響，確認適當溫度設定對 NAR 模型訓練的重要性。
...
摘要：許多研究顯示現有的臉部辨識系統 (FRS)，包括商業系統，常常因為資料代表性不足而對某些種族展現偏見。在這項工作中，我們探討使用合成臉部影像產生方法進行種族改變和膚色修改，以增加資料集的多樣性。我們首先建構一個平衡的臉部影像資料集，代表三種種族：亞洲人、黑人和印度人，再進行詳細分析。然後我們利用現有的生成對抗網路 (GAN) 影像到影像轉譯和流形學習模型來將種族從一種改變為另一種。進一步進行系統分析，使用個別類型角 (ITA) 研究逼真的膚色表現，以評估此類資料集是否適合 FRS。此外，我們也使用現有的臉部影像品質評估 (FIQA) 方法分析品質特徵。然後我們使用四個不同的系統提供一個整體的 FRS 效能分析。我們的發現為未來的研究工作鋪路，包括：(i) 開發特定種族和一般 (任何到任何) 種族改變模型，(ii) 擴充此類方法以建立具有多樣化膚色的資料庫，(iii) 建立代表各種種族的資料集，而這進一步有助於在處理隱私問題的同時減輕偏見。
...
摘要：本文介紹了一種針對自動駕駛量身打造的軌跡預測模型，著重於在動態交通場景中捕捉複雜的交互作用，而不依賴於高解析度地圖。該模型稱為 MFTraj，利用歷史軌跡數據與一種新穎的動態幾何圖形行為感知模組。在核心部分，一個適應性結構感知互動圖形卷積網路捕捉道路使用者的位置和行為特徵，保留時空複雜性。該模型透過線性注意力機制增強，實現了運算效率並減少了參數開銷。在 Argoverse、NGSIM、HighD 和 MoCAD 資料集上的評估突顯了 MFTraj 的穩健性和適應性，即使在資料不足的場景中，在不需要 HD 地圖或向量地圖等額外資訊的情況下，也優於許多基準。重要的是，即使在缺少大量資料的場景中，它仍能維持競爭力的效能，與現有最先進的模型不相上下。結果和方法表明自動駕駛軌跡預測有了顯著的進步，為更安全、更有效率的自動系統鋪平了道路。
...
摘要：自然語言推論 (NLI)，也稱為識別文本蘊涵 (RTE)，是自然語言理解的重要面向。
現在大多數研究都使用機器學習和深度學習來執行特定資料集上的這項任務，表示他們的解決方案既無法解釋，也不明確。
為了滿足 RTE 可解釋方法的需求，我們提出一個創新的管道，其基礎是將文字翻譯成抽象意義表示 (AMR) 圖形。我們使用預先訓練好的 AMR 分析器來執行這項任務。接著，我們將 AMR 圖形翻譯成命題邏輯，並使用 SAT 求解器進行自動推理。在文字中，常識通常會暗示前提和主張之間存在蘊涵（或矛盾）關係，但由於使用了不同的措辭，因此無法從其邏輯表示中辨識出來。為了解決這個問題，我們引進放鬆方法來允許替換或遺忘一些命題。我們的實驗結果顯示，這個管道在四個 RTE 資料集上表現良好。
...
摘要：提示工程對於發揮大型語言模型 (LLM) 的潛力至關重要，特別是在使用專業術語和措辭的醫療領域。然而，提示工程在醫療領域的功效仍有待探討。在這項工作中，回顧了 114 篇最近的研究（2022-2024 年），這些研究將提示工程應用於醫學，涵蓋提示學習 (PL)、提示調整 (PT) 和提示設計 (PD)。PD 最為普遍（78 篇文章）。在 12 篇論文中，PD、PL 和 PT 術語可以互換使用。ChatGPT 是使用最廣泛的 LLM，有七篇論文使用它來處理敏感的臨床數據。思想鏈成為最常見的提示工程技術。雖然 PL 和 PT 文章通常提供了一個評估基於提示的方法的基準，但 64% 的 PD 研究缺乏與提示無關的基準。我們提供了表格和圖形，總結了現有工作，並報告了指導未來研究貢獻的建議。
...
摘要：我們提出 TRAMBA，一種混合Transformer和 Mamba 架構，適用於行動和穿戴式平台的聲學和骨傳導語音增強。骨傳導語音增強在行動和穿戴式平台中不切實際的原因有幾個：(i) 資料收集需要大量人力，導致稀缺；(ii) 記憶體佔用量達數百 MB 的最先進模型與更適合於資源受限系統的方法之間存在效能差距。為了讓 TRAMBA 適應基於振動的感測模式，我們使用廣泛可用的音訊語音資料集對 TRAMBA 進行預訓練。然後，使用者使用少量骨傳導資料進行微調。TRAMBA 在 PESQ 中比最先進的 GAN 高出 7.3%，在 STOI 中高出 1.8%，且記憶體佔用量小一個數量級，推理速度提高多達 465 倍。我們將 TRAMBA 整合到實際系統中，並表明 TRAMBA (i) 透過減少資料取樣和傳輸，將穿戴式裝置的電池續航力提高多達 160%；(ii) 在嘈雜環境中產生比無線語音更高的音質；(iii) 記憶體佔用量小於 20.0 MB。
...
摘要：大型語言模型 (LLM) 在各種任務中都取得了顯著的成功，但它們仍然容易受到對抗性攻擊，特別是記錄良好的「越獄」攻擊。最近，貪婪坐標梯度 (GCG) 攻擊已證明了利用此漏洞的功效，方法是透過結合梯度啟發法和貪婪搜尋來最佳化對抗性提示。然而，此攻擊的效率已成為攻擊過程中的一個瓶頸。為了減輕此限制，我們在本文中透過最佳化鏡頭重新思考對抗性提示的產生，目標是穩定最佳化過程並利用先前反覆運算的更多啟發式見解。具體來說，我們引入了\textbf{M}omentum \textbf{A}ccelerated G\textbf{C}G (\textbf{MAC}) 攻擊，其中將動量項納入梯度啟發法中。實驗結果展示了 MAP 在對齊語言模型的基於梯度的攻擊中所取得的顯著增強。我們的程式碼可在 https://github.com/weizeming/momentum-attack-llm 取得。
...
摘要：論證結構學習 (ASL) 涉及預測論證之間的關係。由於它可以為文件建構結構以利於理解，因此已廣泛應用於許多領域（醫療、商業和科學領域）。儘管 ASL 被廣泛使用，但它仍然是一項具有挑戰性的任務，因為它涉及檢查潛在非結構化話語中句子之間的複雜關係。為了解決這個問題，我們開發了一種簡單但有效的方法，稱為 ASL 任務的雙塔多尺度卷積神經網路 (DMON)。具體來說，我們將論證組織成一個關係矩陣，該矩陣與論證嵌入一起形成一個關係張量，並設計一種機制來捕捉與上下文論證的關係。在三個不同領域的論證挖掘資料集上的實驗結果表明，我們的框架優於最先進的模型。程式碼可在 https://github.com/VRCMF/DMON.git 取得。
...
摘要：強化學習政策通常由黑盒子神經網路表示，它們不可解釋且不適合於安全至上的領域。為了解決這兩個問題，我們提出受約束的正規化流動政策，作為可解釋且安全且建構政策模型。我們針對具有即時安全約束的強化學習問題實現了安全性，我們可以透過分析建構一個正規化流動來利用領域知識，以確保約束滿足。正規化流動對應於作用樣本上的一系列可解釋轉換，每個轉換確保相對於特定約束的一致性。我們的實驗揭示了可解釋性以外的好處，在於更容易的學習目標，並在整個學習過程中維持約束滿足。我們的做法利用了獎勵工程上的約束，同時提供了增強的可解釋性、安全性，以及在不依賴於複雜獎勵函數的情況下向代理提供領域知識的直接方法。
...
摘要：邊緣運算讓人工智慧和機器學習模型能夠部署在邊緣裝置上，在邊緣裝置上，它們可以從在地資料中學習，並協作形成一個全球模型。聯邦學習 (FL) 是一種分散式機器學習技術，在維護資料隱私的同時促進這個過程。然而，FL 也面臨挑戰，例如在資源受限的裝置上，高運算和通訊成本，以及由於邊緣用戶端資料異質性和存在分布外資料，而導致的概化效能不佳。在本文中，我們提出梯度同質性引導的聯邦稀疏訓練 (FedSGC)，這是一種新的方法，將動態稀疏訓練和梯度同質性檢查整合到聯邦學習架構中，以解決這些問題。我們的方法利用了這樣一個想法：與相對於全球模型具有衝突方向的梯度相關的神經元，包含與其他用戶端無關或概化程度較低的信息，並且可以在稀疏訓練過程中被剪枝。相反，與一致方向相關的神經元可以優先增長。這樣，FedSGC 可以大幅減少在地運算和通訊開銷，同時增強 FL 的概化能力。我們在具有挑戰性的非獨立同分布設定上評估我們的方法，並展示它在各種場景中以最先進的 FL 方法獲得競爭優勢的準確性，同時將運算和通訊成本降至最低。
...
摘要：從有雜訊的標籤訓練深度神經網路 (DNN) 是一項重要且具挑戰性的任務。然而，現有的方法大多著重於損壞的標籤，而忽略了內在資料結構的重要性。為了彌補有雜訊的標籤與資料之間的差距，我們受到物理學中位能概念的啟發，提出了一個基於位能的混合模型 (PEMM)，用於雜訊標籤學習。我們創新了一個基於距離的分類器，其類別中心具有位能正則化。將我們提出的分類器嵌入現有的深度學習主幹，我們可以獲得具有更好特徵表示的穩健網路。它們可以保留資料中的內在結構，從而產生優異的雜訊容忍度。我們進行了大量的實驗，以分析我們提出的模型在多個真實世界資料集上的效率。定量結果表明，它可以達到最先進的效能。
...
摘要：自訓練是深度學習的一種強大方法。關鍵過程是找到一個擬標籤進行建模。然而，先前的自訓練演算法會受到硬標籤帶來的過度自信問題，即使是一些與信心相關的正則化項也無法全面捕捉不確定性。因此，我們提出一個新的自訓練架構來結合模型和資料集的不確定性資訊。具體來說，我們建議使用期望最大化 (EM) 來平滑標籤並全面估計不確定性資訊。我們進一步設計一個基礎提取網路，從資料集估計初始基礎。獲得的不確定性基礎可以根據不確定性資訊進行過濾。然後可以將其轉換為真正的硬標籤，以在重新訓練過程中反覆更新模型和基礎。在影像分類和語意分割上的實驗顯示了我們的演算法在不同的資料集上比有信心感知的自訓練演算法有 1-3 個百分點的優勢。
...
摘要：本文介紹了 TartuNLP 團隊在 2024 年 EvaLatin 共享任務中提交的歷史拉丁語文本的情緒極性偵測。我們的系統依賴於兩種不同的方法來註釋監督式學習的訓練資料：1) 採用組織者提供的極性詞彙表來建立基於啟發法的標籤，以及 2) 使用 GPT4 產生標籤。我們使用適配器架構進行了參數有效的微調，並針對訓練語言和任務適配器進行了單語和跨語言知識轉移的實驗。我們使用 LLM 生成的標籤所提交的內容在情緒極性偵測任務中獲得了總體第一名。我們的結果表明，基於 LLM 的註釋在拉丁語文本中顯示出有希望的結果。
...
摘要：異常偵測 (AD) 是一個在工業環境中經常需要的關鍵程序。異常可以表示系統內潛在的問題，進而提示進一步調查。工業程序旨在盡可能簡化作業，包含最終產品的生產，使異常偵測成為達成此目標的必要手段。傳統的異常偵測方法通常將觀測值分類為正常或異常，而不會提供這些分類背後的原因。因此，鑑於工業 5.0 的出現，一個更理想的方法是提供可解釋的結果，讓使用者了解結果背後的理由。本文提出 ExIFFI 的第一個工業應用，ExIFFI 是一個最近開發的方法，專注於為延伸隔離森林 (EIF) 異常偵測方法產生快速且有效的解釋。ExIFFI 在兩個公開可用的工業資料集上進行測試，證明其在解釋和運算效率方面優於其他最先進的可解釋異常偵測模型。
...
摘要：<paragraph>在實時 X 光影像擷取中精準偵測和追蹤導管等裝置是心血管內介入性治療的基本先決條件。這些資訊可用於程序指導，例如引導支架放置。為了確保程序安全性和有效性，追蹤過程中需要極高的穩健性，不能發生故障。為此，需要有效應對挑戰，例如：對比劑或其他外部裝置或導線遮蔽裝置、視野或擷取角度改變，以及由於心臟和呼吸運動導致的持續移動。為了克服上述挑戰，我們提出了一種新方法，從超過 1600 萬個介入性 X 光影像序列資料的極大資料群集中學習時空特徵，並使用影像序列資料的自監督。我們的做法基於遮罩影像建模技術，該技術利用基於插值的重建來學習精細的幀間時態對應。編碼在結果模型中的特徵會在後續進行微調。我們的做法達到了最先進的效能，特別是與經過高度最佳化的參考解決方案（使用多階段特徵融合、多任務和流動正規化）相比，具有更高的穩健性。實驗表明，我們的方法將最大追蹤誤差降低了 66.31%，相較於參考解決方案（使用流動正規化時為 23.20%）；在 42 幀/秒（在 GPU 上）的 3 倍推理速度下，成功率達到 97.95%。這些結果鼓勵在介入性影像分析的各種其他任務中使用我們的做法，這些任務需要有效理解時空語義。</paragraph>
...
摘要：本研究探討了主觀賦予意義的假設構成意識經驗的現象內容。亦即，現象內容具有語意性。這種形式的主觀意義表現為感覺的內在且非表徵性的特徵。經驗上，主觀意義普遍存在於意識經驗中。我們指出現象學研究提供了支持此論點的證據。此外，此意義概念與弗雷格在形上學和語言哲學中所指的「意義」密切相關。它也與皮爾斯的符號學中的「詮釋項」相符。我們討論了弗雷格的意義如何也能延伸至意識的原始感受。意義和指涉在現象經驗中都扮演著角色。此外，在心物關係的脈絡中，我們提供了與個人心智表徵相關的主觀意義的形式化。透過辨識物理和心智領域之間精確的對應，我們主張語法和語義結構超越語言，並在這些領域中實現。形式上，意義是一種關係屬性，透過一個對應實現，此對應在適當的語義空間中詮釋形式系統的語法結構。此對應在心智領域中的影像與經驗相關，因此構成感覺的現象內容。我們以這可能對基於經驗的意識理論造成的影響作為結論。
...
摘要：積極參與對話是建立共識的關鍵，
因為理解是由生產者和接收者共同調整的。
旁聽者被剝奪了執行基礎行為的特權，只能猜測預期的含義。儘管如此，數據生成和註解、
NLP 對話模型的建模、訓練和評估依賴於旁聽範式。有多少基礎過程被
因此沒收？正如我們所展示的，有證據表明不可能使用數據驅動學習
模型正確地建模人類元溝通行為。在本文中，我們討論了這個問題，並提供了一個初步分析
關於人類請求澄清決策的可變性。最重要的是，我們希望將這個話題帶回社區的餐桌上，
鼓勵討論僅「聆聽」的模型的後果。
...
摘要：現有建立基於來源資訊尋求對話資料集的方法，通常成本高昂且難以實作，因為它們完全依賴人工註解。我們建議結合大型語言模型 (LLM) 提示和人類專業知識，以更有效率且可靠地產生資料。我們使用 LLM 產生來模擬兩個角色，而不是勞力密集的綠野仙蹤 (WOZ) 方法，其中兩個註解者從頭開始產生對話，扮演代理和使用者的角色。然後，註解者驗證輸出，並使用歸因資料擴充它。我們透過建構 MISeD（會議資訊尋求對話資料集）來展示我們的這項方法，這是第一個專注於會議紀錄的資訊尋求對話資料集。使用 MISeD 微調的模型在我們的測試集上展現出優異效能，以及在新的完全手動 WOZ 測試集和現有的基於查詢的摘要基準上，這表示我們方法的效用。
...
摘要：單目深度估計演算法發展的主要障礙在於難以取得與收集到的 RGB 影像相符的高品質深度資料。收集此資料耗時且昂貴，而且即使是現代感測器收集的資料也有範圍或解析度限制，且容易出現不一致性和雜訊。為了解決此問題，我們提出使用 3D 合成環境和 CycleGAN 領域轉移進行模擬資料生成的方法。我們將此資料生成方法與廣泛使用的 NYUDepth V2 資料集進行比較，方法是使用不同的真實和模擬資料訓練組來訓練基於 DenseDepth 結構的深度估計模型。我們在 Husky 機器人新收集的影像和 LiDAR 深度資料上評估模型的效能，以驗證此方法的可概化性，並顯示 GAN 轉換資料可以用作真實世界資料的有效替代方案，特別是在深度估計方面。
...
摘要：在追求精進全自動駕駛的精確感知模型時，持續的線上模型訓練變得至關重要。車輛網路中的聯合學習 (FL) 提供了一種有效率的模型訓練機制，同時保留原始感測資料的完整性。然而，FL 在非相同分佈資料（例如，數量偏差）上會遇到困難，導致模型訓練期間次佳的收斂率。在先前的研究中，我們引入了 FedLA，這是一種創新的標籤感知聚合方法，用於解決 FL 在一般場景中的資料異質性問題。
  在本文中，我們引入了 FedProx+LA，這是一種新穎的 FL 方法，建立在現有技術 FedProx 和 FedLA 的基礎上，用於解決資料異質性問題，特別針對車輛網路量身打造。我們評估了 FedProx+LA 在連續線上物件偵測模型訓練中的效能。透過與傳統方法和現有技術進行比較分析，我們的研究結果揭示了 FedProx+LA 具有優異的收斂率。值得注意的是，如果標籤分佈非常異質，與基線方法相比，我們的 FedProx+LA 方法顯示出偵測效能的顯著提升，也優於我們先前的 FedLA 方法。此外，與基線方法相比，FedLA 和 FedProx+LA 都將收斂速度提升了 30%。
...
摘要：圖形Transformer，由於其全局注意力機制，已成為處理圖形結構化數據的新工具。眾所周知，全局注意力機制在完全連接的圖形中考慮了更廣泛的感受野，這導致許多人相信可以從所有節點中提取有用的信息。在本文中，我們挑戰了這一觀點：全局化屬性是否總能使圖形Transformer受益？我們通過提供經驗證據和理論分析揭示了圖形Transformer中的過度全局化問題，即當前的注意力機制過度關注那些遠程節點，而實際上包含大部分有用信息的近節點則相對較弱。然後，我們提出了一個新的雙級全局圖形Transformer，並採用協作訓練（CoBFormer），包括集群間和集群內Transformer，以防止過度全局化問題，同時保持從遠程節點提取有價值信息的的能力。此外，提出了協作訓練，以在理論保證下提高模型的泛化能力。在各種圖形上的大量實驗很好地驗證了我們提出的 CoBFormer 的有效性。
...
摘要：<paragraph>舉發對於確保公共和私人部門的透明度和責任制至關重要。然而，（潛在的）舉發者在匿名舉報時，常常會害怕或面臨報復。他們揭露的具體內容和獨特的寫作風格可能會再次將他們識別為消息來源。歐盟舉報者保護指令等法律措施的範圍和效力有限。因此，計算機方法對於防止再次識別是鼓勵舉報者站出來的重要的補充工具。然而，目前的文本清理工具採用一刀切的方法，對匿名性的看法過於狹隘。它們旨在通過將典型的風險較高的詞彙（例如人名和其他 NE 標籤）及其組合替換為佔位符來降低識別風險。然而，這種方法對於舉報場景來說是不夠的，因為它忽視了文本特徵中進一步的重新識別可能性，包括寫作風格。因此，我們提出、實施並評估了一種新的分類和緩解策略，用於重寫文本，其中涉及舉報者對風險和效用的評估。我們的原型工具在詞彙/術語層級半自動評估風險，並應用風險適應匿名化技術來產生語法不連貫但適當清理的文本。然後，我們使用一個經過微調的 LLM 來進行改寫，以使文本連貫且風格中立。我們使用歐洲人權法院的案例和真實舉報者證詞的摘錄來評估我們工具的有效性，並使用流行的 IMDb62 電影評論數據集統計衡量對作者歸因 (AA) 攻擊的保護和效用損失。我們的模型可以將 AA 準確率從 98.81% 大幅降低到 31.22%，同時保留高達 73.1% 的原始內容語義。</paragraph>
...
摘要：聯合學習 (FL) 是一種分散式學習範例，使各方能夠在保持其資料機密性的同時協作訓練模型。在自動駕駛中，它具有降低資料儲存成本、減少頻寬需求和加速學習的潛力。然而，FL 容易受到中毒攻擊。在本文中，我們介紹了針對自動駕駛中回歸任務量身定制的兩種新型中毒攻擊：FLStealth 和 Off-Track Attack (OTA)。FLStealth 是一種非目標攻擊，旨在提供會惡化整體模型效能但看起來良性的模型更新。另一方面，OTA 是一種目標攻擊，目標是改變整體模型在暴露於特定觸發器時的行為。我們透過針對車輛軌跡預測任務進行全面實驗來證明我們攻擊的有效性。特別是，我們表明在五種不同的非目標攻擊中，FLStealth 在繞過伺服器採用的考慮防禦方面最為成功。對於 OTA，我們證明了常見防禦策略無法減輕攻擊，突顯了在 FL 中針對自動駕駛的目標攻擊迫切需要新的防禦機制。
...
摘要：通訊瓶頸會阻礙分散式神經網路訓練的可擴充性，特別是在分散式記憶體運算叢集上。為了大幅減少這種通訊開銷，我們引入了 AB 訓練，一種新穎的資料平行訓練方法，它能將權重矩陣分解成低階表示，並利用獨立的基於群組的訓練。這種方法持續在多種擴充場景中將網路流量減少 50%，增加了在受通訊限制的系統上進行訓練的潛力。我們的這種方法在較小規模下展現了正則化的效果，導致像 VGG16 這樣的模型的泛化能力得到改善，同時在 CIFAR-10 上的訓練中達到了驚人的 44.14 : 1 壓縮率，並維持了有競爭力的準確度。儘管有希望，我們的實驗表明，即使在低階訓練模式中，大批次效應仍然是一個挑戰。
...
摘要：從單一 RGB 影像重建手部網格是一項具有挑戰性的任務，因為手部經常被物體遮擋。大多數先前的研究嘗試引入更多額外的資訊，並採用注意力機制來改善 3D 重建結果，但這會增加計算複雜性。此觀察促使我們提出一個新的簡潔架構，同時提高計算效率。在這項工作中，我們提出一個簡單且有效的 3D 手部網格重建網路 HandSSCA，它率先將狀態空間模型納入手部姿勢估計領域。在網路中，我們設計了一個新穎的狀態空間通道注意力模組，它擴展了有效感測場，提取空間維度中的手部特徵，並增強通道維度中的手部區域特徵。此設計有助於重建完整且詳細的手部網格。在具有挑戰性的手部物體遮擋（例如 FREIHAND、DEXYCB 和 HO3D）中進行的廣泛實驗證明，我們提出的 HandSSCA 在維持最小參數計數的同時，達到了最先進的效能。
...
摘要：模擬土壤反射光譜對於土壤-植物輻射模型和訓練機器學習模型非常有價值，但由於土壤結構與其組成成分之間的複雜關係，因此很困難。為了解決這個問題，開發了一個基於土壤屬性輸入的土壤反射光譜模擬的完全數據驅動土壤光學生成模型 (SOGM)。該模型在一個包含來自 17 個數據集的近 180,000 個土壤光譜-屬性對的廣泛數據集上進行訓練。它從描述土壤屬性及其值的基於文本的輸入中生成土壤反射光譜，而不是僅二進制向量格式中的數值和標籤。生成模型可以根據不完整的輸入屬性集模擬輸出光譜。SOGM 基於去噪擴散概率模型 (DDPM)。還構建了兩個額外的子模型來補充 SOGM：一個光譜填充模型，可以填補小於全可見近紅外範圍 (VIS-NIR；400 至 2499 nm) 的光譜的空白，以及一個濕土光譜模型，可以估計水含量對乾燥光譜的影響由 SOGM 預測的光譜。SOGM 通過與 Helios 3D 植物建模軟體相結合進行擴展，這允許生成模擬土壤和植物場景的合成航拍圖像。它還可以輕鬆地與用於遙感研究的土壤植物輻射模型（如 PROSAIL）集成。SOGM 在未包含在模型訓練中的新數據集上的測試結果證明，該模型可以根據可用的屬性輸入生成合理的土壤反射光譜。所提出的模型可在以下位置公開訪問：
https://github.com/GEMINI-Breeding/SOGM_soil_spectra_simulation。
...
摘要：<paragraph>由於城市私人汽車擁有量激增，導致停車難問題日益嚴重，這使得在城市規劃和管理中，有效預測停車位可用性變得十分必要。然而，現有的預測方法存在預測準確度低的問題，因為它們缺乏與停車量相關的時空關聯性特徵，並且忽略了特定區域內相似停車場之間的流動模式和關聯性。為了應對這些挑戰，本研究提出了一個停車位可用性預測框架，該框架將時空深度學習與多源數據融合相結合，涵蓋了來自多個來源（例如地鐵、公車、計程車服務）的交通需求數據和停車場數據。該框架基於 Transformer 作為時空深度學習模型，並利用 K 均值聚類來建立停車場群集區域，從連接到停車場的各種交通模式（即地鐵、公車、線上叫車服務和計程車）中提取和整合交通需求特徵。使用真實世界的經驗數據驗證了所提出的方法與用於預測停車位可用性的不同機器學習、深度學習和傳統統計模型相比的有效性。實驗結果表明，使用所提出的管線，開發的 Transformer 模型在各種指標方面優於其他模型，例如均方誤差 (MSE)、平均絕對誤差 (MAE) 和平均絕對百分比誤差 (MAPE)。通過將多源需求數據與時空深度學習技術相融合，這種方法提供了開發停車位可用性預測系統的潛力，該系統可為駕駛者和城市規劃者提供更準確和及時的資訊，從而促進更有效率和可持續的城市流動性。</paragraph>
...
摘要：機器學習中泛化的目標是在各種未知任務和領域中獲得優異的效能。最近，自我監督式學習 (SSL) 被視為達成此目標的有效方法。它可以從未標記的資料中學習高品質的表徵，並在多個下游任務中獲得有前景的經驗效能。現有的 SSL 方法主要從兩個方面約束泛化：(i) 大規模訓練資料，以及 (ii) 學習任務層級的共用知識。然而，這些方法缺乏在學習目標中對 SSL 泛化的明確建模，而對 SSL 泛化的理論理解仍然有限。這可能會導致 SSL 模型在資料稀少的情況下過度擬合，並在現實世界中泛化不良，使得難以達到真正的泛化。為了解決這些問題，我們提供了 SSL 中泛化的理論定義，並定義了一個 $\sigma$-測量來幫助量化它。基於此見解，我們將泛化明確建模到自我監督式學習中，並進一步提出一個新穎的 SSL 架構，稱為 GeSSL。它根據 $\sigma$-測量引入了自我激勵的目標，使模型能夠找到朝向泛化的最佳更新方向。廣泛的理論和經驗評估證明了所提出的 GeSSL 的優異效能。
...
摘要：<paragraph>深度學習的最新進展已展現出卓越的效能，可與各種監督式電腦視覺任務的人類能力相媲美。然而，在模型訓練之前，普遍假設擁有涵蓋所有類別的廣泛訓練資料集通常與實際情況不同，而實際情況中，新類別的資料可用性有限才是常態。挑戰在於將少數樣本的新類別無縫整合到訓練資料中，要求模型靈活容納這些新增資料，同時不影響其在基本類別上的效能。為了解決這個問題，研究社群已在少樣本類別增量學習 (FSCIL) 的領域中提出多種解決方案。在本研究中，我們提出一個創新的 FSCIL 架構，它利用語言正規化器和子空間正規化器。在基本訓練期間，語言正規化器有助於納入從視覺語言模型中萃取的語義資訊。子空間正規化器有助於促進模型在增量訓練期間獲取基本類別中影像和文字語義之間的細微關聯。我們提出的架構不僅能讓模型採用資料有限的新類別，還能確保基本類別的效能。為了證實我們方法的效力，我們對三個不同的 FSCIL 基準進行全面的實驗，我們的架構在其中獲得了最先進的效能。</paragraph>
...
摘要：學習解決車輛路線問題 (VRP) 已備受關注。然而，大多數神經求解器僅在特定問題上獨立地建構和訓練，這使得它們較不通用且不實用。在本文中，我們旨在開發一個統一的神經求解器，可以同時應對一系列 VRP 變體。具體來說，我們提出了一個具有專家混合 (MVMoE) 的多任務車輛路線求解器，它極大地增強了模型容量，而不會成比例地增加計算。我們進一步為 MVMoE 開發了一個階層式閘控機制，在經驗效能和計算複雜度之間取得了良好的平衡。在實驗中，我們的模型顯著提升了 10 個未見 VRP 變體的零次學習泛化效能，並在少次學習設定和真實世界基準實例中展示了良好的結果。我們進一步提供了關於 MoE 配置在解決 VRP 中的影響的廣泛研究。令人驚訝的是，階層式閘控可以實現更好的分佈外泛化效能。原始碼可在以下網址取得：https://github.com/RoyalSkye/Routing-MVMoE。
...
摘要：儘管預先訓練好的語言模型在基於提示的少次學習中展現出極大的彈性和多功能性，但它們卻有參數規模過於龐大且推論適用性有限的問題。最近的研究建議，將 PLM 用作資料集產生器，並訓練一個微小的特定於任務的模型以達成有效率的推論。然而，它們在各種領域的適用性有限，因為它們傾向於產生特定於領域的資料集。在這項工作中，我們提出了一種新的方法，用於普遍領域概化，它會產生一個與目標領域無關的資料集。這允許將微小任務模型概化到任何與標籤空間相同的領域，進而提升資料集產生範例的真實世界適用性。我們的實驗指出，所提出的方法在使用比 PLM 小好幾個數量級的參數集時，就能在各種領域中達成概化能力。
...
摘要：<paragraph>最近的基于 Transformer 的 ASR 模型已将词错误率 (WER) 降至 4% 以下，超越了人类注释员的准确性，但它们需要大量的服务器资源，从而导致巨大的碳足迹。ASR 的传统基于服务器的架构还存在隐私问题，以及由于网络依赖性导致的可靠性和延迟问题。相比之下，设备端 (边缘) ASR 增强了隐私，提高了性能，并通过有效平衡特定应用程序的能耗和准确性来促进可持续性。本研究探讨了量化、内存需求和能耗对 NVIDIA Jetson Orin Nano 上各种 ASR 模型推理性能的影响。通过分析使用 FP32、FP16 和 INT8 量化在干净和嘈杂数据集上的模型的 WER 和转录速度，我们重点介绍了准确性、速度、量化、能效和内存需求之间的关键权衡。我们发现，对于不同的模型，将精度从 fp32 转换为 fp16 可将音频转录的能耗减半，同时性能下降很小。较大的模型大小和参数数量既不能保证更好的抗噪性，也不能预测给定转录负载的能耗。这些以及其他一些发现为在能耗和内存受限的环境中优化 ASR 系统提供了新的见解，这对于开发高效的设备端 ASR 解决方案至关重要。本文中复制结果所需的代码和输入数据是开源的，可在 [https://github.com/zzadiues3338/ASR-energy-jetson] 上获得。</paragraph>
...
摘要：Igbo 語言正面臨瀕臨滅絕的風險，正如 2025 年聯合國教科文組織的研究所示。這突顯了為 Igbo 語言開發語言技術以促進溝通、學習和保存的必要性。為 Igbo 語言創建強大、有影響力和廣泛採用的語言技術，必須納入語言的多方言性質。實現方言感知語言技術的主要障礙是缺乏全面的方言數據集。為了解決這個問題，我們提出了 IgboAPI 數據集，這是一個多方言 Igbo-English 詞典數據集，旨在增強 Igbo 方言的表示。此外，我們通過兩項不同的研究來說明 IgboAPI 數據集的實用性：一項專注於 Igbo 語義詞彙，另一項專注於機器翻譯。在語義詞彙項目中，我們成功地為 Igbo 語義標記器建立了一個初始的 Igbo 語義詞彙，而在機器翻譯研究中，我們證明了通過使用 IgboAPI 數據集微調現有的機器翻譯系統，我們可以顯著提高它們處理句子中方言變化的能力。
...
摘要：儘管大型語言模型 (LLM) 在文字理解和生成方面取得了顯著的成功，但它們在文字分群任務中的潛力仍未被充分探索。我們觀察到功能強大的閉源 LLM 可以提供實體集合的高品質分群，但由於需要大量的運算能力和相關成本，因此無法擴展。因此，我們提出了 CACTUS（使用擴增三元組損失的上下文感知分群），這是一種系統化方法，它利用開源 LLM 來有效率且有效地監督實體子集的分群，特別是專注於基於文字的實體。現有的文字分群方法無法有效擷取實體子集提供的上下文。此外，儘管有幾種基於語言建模的分群方法，但很少有方法是針對監督分群任務而設計的。本文介紹了一種新穎的方法，透過可擴展的實體間注意力機制擷取上下文，來使用 LLM 分群實體子集。我們提出了一個新穎的擴增三元組損失函數，專門針對監督分群，它解決了將三元組損失直接應用於此問題的固有挑戰。此外，我們引入了一個基於文字擴充技術的自我監督分群任務，以改善我們模型的泛化能力。為了評估，我們從閉源 LLM 收集地面真實分群，並在監督分群架構下將此知識轉移到開源 LLM，允許更快速且更便宜的開源模型執行相同的任務。在各種電子商務查詢和產品分群資料集上的實驗表明，我們提出的方法在各種外部分群評估指標下，明顯優於現有的無監督和監督基準。
...
摘要：神經崩潰 (NC) 是深度神經網路 (DNN) 在訓練終端階段的一種簡單且對稱的現象，其中最後一層的特徵會崩潰成它們的類別平均值，並形成一個與分類器向量對齊的單純等角緊密框架。然而，最後一層特徵與資料和訓練期間的中間層的關係仍未探討。為此，我們描述了 ResNet 中間層的幾何形狀，並提出了一個新的猜想，即漸進前饋崩潰 (PFC)，聲稱崩潰的程度會在 DNN 的前向傳播過程中增加。根據權重衰減的 ResNet 近似於在終端階段的 Wasserstein 空間中的測地線曲線，我們為訓練良好的 ResNet 推導了一個透明模型。PFC 的指標確實在各種資料集上隨著深度單調遞減。我們提出了一個新的替代模型，多層無約束特徵模型 (MUFM)，通過最優傳輸正則化器連接中間層。MUFM 的最優解與 NC 不一致，但相對於輸入資料更集中。總的來說，這項研究將 NC 擴展到 PFC 以模擬中間層的崩潰現象及其對輸入資料的依賴性，為分類問題中 ResNet 的理論理解提供了啟示。
...
摘要：大型語言模型 (LLM) 讓滿足資訊需求有了新方法。儘管在將其應用於文件排名和簡短文字產生等設定方面已取得長足進展，但它們在撰寫完整、準確且可驗證的長篇報告方面仍面臨難題。具備這些特質的報告對於滿足使用者的複雜、細緻或多面向資訊需求是必要的。在本觀點論文中，我們彙整了業界和學術界以及各種相關研究領域的意見，以提出我們對自動報告產生的願景，以及更重要的是，一個可以評估此類報告的彈性架構。與其他摘要任務不同，自動報告產生始於對資訊需求的詳細說明，說明報告的必要背景、需求和範圍。此外，產生的報告應完整、準確且可驗證。這些在許多分析報告撰寫設定中理想（如果不是必需的話）的特質，需要重新思考如何建構和評估展現這些特質的系統。為了促進建構這些系統的新努力，我們提出一個評估架構，該架構汲取了各種評估中的構想。為了測試完整性和準確性，該架構使用以問答形式表達的資訊片段，這些片段必須是任何高品質產出報告的一部分。此外，評估引文（將報告中提出的主張對應到其來源文件）可確保可驗證性。
...
摘要：<paragraph>設計偏好引導（PE）方法，可以在冷啟動設定中快速確定使用者的首要項目偏好，這對於建立有效且個人化的對話式推薦（ConvRec）系統是一項關鍵挑戰。雖然大型語言模型（LLM）構成了一種新穎的技術，可以實現完全自然語言（NL）PE 對話，我們假設單體 LLM NL-PE 方法缺乏多輪、決策理論推理，而這對於有效平衡 NL 探索和利用使用者的偏好以針對任意項目集是必要的。相比之下，傳統貝氏最佳化 PE 方法定義了理論上最佳的 PE 策略，但無法使用 NL 項目描述或產生 NL 查詢，不切實際地假設使用者可以用直接的項目評分和比較來表達偏好。為了克服這兩種方法的限制，我們在貝氏最佳化（BO）架構中制定 NL-PE，該架構旨在產生 NL 查詢，主動引發自然語言回饋以減少項目效用的不確定性，進而找出最佳推薦。我們在一個新穎的 NL-PE 演算法 PEBOL 中展示我們的架構，它使用使用者偏好語句和 NL 項目描述之間的自然語言推論（NLI），以維持偏好信念和 BO 策略，例如湯普森抽樣（TS）和上置信界（UCB），以引導 LLM 查詢產生。我們在受控實驗中對我們的這些方法進行數值評估，發現與單體 GPT-3.5 相比，儘管依賴於一個小得多的 400M 參數 NLI 模型進行偏好推論，但 PEBOL 在 10 輪冷啟動 NL-PE 對話後在 MAP@10 中實現了高達 131% 的改進。</paragraph>
...
摘要：<paragraph>本文介紹 TVB-HKSL-News，這是一個從電視新聞節目中收集超過 7 個月的香港手語 (HKSL) 新資料集。收集此資料集是為了豐富 HKSL 的資源，並支援大詞彙量連續手語辨識 (SLR) 和翻譯 (SLT) 的研究。它包含兩位手語者的 16.07 小時手語影片，詞彙量為 6,515 個手語符號 (用於 SLR) 和 2,850 個中文字或 18K 個中文詞彙 (用於 SLT)。一位手語者有 11.66 小時的手語影片，另一位有 4.41 小時。建立此資料集的其中一個目標是支援調查對於單一位手語者，如果給予 (相對) 大量訓練資料，連續手語辨識/翻譯的表現有多好，這可能會導致新的建模方法的發展。此外，大部分資料收集管線都是自動化的，幾乎沒有人工介入；我們相信我們的收集方法可以擴展，以便在未來輕鬆地收集更多手語資料，用於任何手語的 SLT，只要有此類手語翻譯影片。我們也在資料集上執行 SOTA SLR/SLT 模型，並取得 34.08% 的基準 SLR 詞彙錯誤率和 23.58 的基準 SLT BLEU-4 分數，作為未來資料集研究的基準。</paragraph>
...
摘要：多語言資訊檢索 (MLIR) 考慮了針對以可能與這些語言不同的語言表達的查詢，對多種語言的文件進行排名的問題。最近的研究發現，諸如組合代表單一文件語言的已排序清單或使用多語言預訓練語言模型等方法顯示出偏好一種語言勝過其他語言。這導致對不同語言的文件進行系統性的不公平處理。這項工作提出了一個語言公平性指標，以使用 Kruskal-Wallis 檢定透過統計等價測試來評估不同語言的文件是否得到公平排名。與群體公平性中的大多數先前工作相反，我們不認為任何語言都是不受保護的群體。因此，我們提出的指標 PEER（EqualExpected Rank 的機率）是第一個公平性指標，專門用於捕捉 MLIR 系統的語言公平性。我們展示了 PEER 在人工排序清單上的行為。我們還對兩個公開基準上的真實 MLIR 系統進行評估，並顯示 PEER 分數與 MLIR 公平性的先前分析結果一致。我們的實作與 ir-measures 相容，且可以在 http://github.com/hltcoe/peer_measure 取得。
...
摘要：最近跨語言資訊檢索 (CLIR) 的工作（其中查詢和文件使用不同的語言）顯示了 Translate-Distill 框架的好處，該框架使用翻譯和精餾訓練跨語言神經雙編碼器模型。然而，Translate-Distill 只支援單一文件語言。多語言資訊檢索 (MLIR) 對多語言文件集合進行排名，其訓練難度高於 CLIR，因為模型必須為不同語言的文件指定可比較的相關性分數。這項工作擴充了 Translate-Distill，並為 MLIR 提出多語言 Translate-Distill (MTD)。我們顯示使用 MTD 訓練的 ColBERT-X 模型優於使用多語言 Translate-Train（這是以前最先進的訓練方法）訓練的對應模型，在 nDCG@20 中高出 5% 至 25%，在 MAP 中高出 15% 至 45%。我們還顯示該模型對於訓練批次中語言的混合方式具有穩健性。我們的實作可在 GitHub 上取得。
...
摘要：PLAID 是一種高效的 ColBERT 後期互動雙編碼器實作，它使用預訓練語言模型進行排名，在單語、跨語言和多語言檢索中持續達成最先進的效能。PLAID 與 ColBERT 的不同之處在於將詞彙指派給群集，並將這些詞彙表示為群集質心加上壓縮的殘差向量。雖然 PLAID 在批次實驗中很有效，但它的效能會在串流設定中下降，因為文件會隨著時間而陸續抵達，而新詞彙的表示可能會因為用於選擇群集質心的早期詞彙而建模不佳。PLAID 串流階層索引在數 TB 的時間文字上執行（PLAID SHIRTTT）使用基於階層分片的多分段增量索引來解決這個問題。在 ClueWeb09 和多語言 NeuCLIR 彙編上的實驗展示了這種方法的有效性，分別適用於 ColBERT 架構迄今索引的最大彙編和多語言設定。
...
摘要：大型語言模型 (LLM) 已在各種領域展現出驚人的潛力，但它們常常缺乏存取和推理特定領域知識和工具的能力。在本文中，我們介紹了 CACTUS（化學代理連接工具使用到科學），一種基於 LLM 的代理，它整合了化學資訊學工具，以實現化學和分子發現中的先進推理和問題解決。我們使用一組多樣化的開源 LLM，包括 Gemma-7b、Falcon-7b、MPT-7b、Llama2-7b 和 Mistral-7b，對 CACTUS 的效能進行評估，基準是數千個化學問題。我們的結果表明，CACTUS 明顯優於基準 LLM，其中 Gemma-7b 和 Mistral-7b 模型無論使用何種提示策略，都能達到最高的準確度。此外，我們探討了特定領域提示和硬體配置對模型效能的影響，強調了提示工程的重要性以及在消費者級硬體上部署較小模型的潛力，而準確度不會顯著下降。透過結合開源 LLM 的認知能力和特定領域工具，CACTUS 能協助研究人員執行分子性質預測、相似性搜尋和藥物相似性評估等任務。此外，CACTUS 代表了化學資訊學領域的一個重要里程碑，為從事化學和分子發現的研究人員提供了一個適應性工具。透過整合開源 LLM 的優勢和特定領域工具，CACTUS 有可能加速科學進步，並開啟探索新穎、有效和安全的治療候選物、催化劑和材料的新領域。此外，CACTUS 能與自動化實驗平台整合，並在真實時間做出資料驅動決策，為自主發現開啟了新的可能性。
...
摘要：一對一輔導被廣泛認為是一種有效的教學方法，條件是輔導員有資格。然而，對合格輔導員的高需求仍然是一個挑戰，這通常需要對新手輔導員（即受訓者）進行培訓以確保輔導有效。研究表明，提供及時的解釋性回饋可以促進受訓者的培訓過程。然而，由於由人類專家評估受訓者表現的耗時性質，這提出了挑戰。受到大型語言模型 (LLM) 最近進展的啟發，我們的研究採用 GPT-4 模型來建立一個解釋性回饋系統。此系統以二進位形式（即正確/不正確）識別受訓者的反應，並自動提供基於範本的回饋，其中 GPT-4 模型適當地重新表述了反應。我們對來自受訓者的 410 個反應進行了研究，涵蓋三個培訓課程：給予有效的讚美、對錯誤的反應以及確定學生的知識。我們的研究結果表明：1) 使用少次嘗試的方法，GPT-4 模型有效地從三個培訓課程中識別出受訓者的正確/不正確反應，平均 F1 分數為 0.84，AUC 分數為 0.85；2) 使用少次嘗試的方法，GPT-4 模型巧妙地將受訓者不正確的反應重新表述為所需的反應，達到了與人類專家相當的表現。
...
摘要：Whisper 是一款多任务且多语言的语音模型，涵盖 99 种语言。
它在所涵盖语言的一个子集中产生了值得称道的自动语音识别 (ASR) 结果，但该模型在数量不小的代表性不足的语言上仍然表现不佳，这个问题在较小的模型版本中会更加严重。在这项工作中，我们检查了它的局限性，展示了说话者相关（性别、年龄）和模型相关（资源丰富性和模型大小）偏差的存在。尽管如此，我们表明只有模型相关的偏差被量化放大，影响了更多资源匮乏的语言和较小的模型。在寻找更好的压缩方法时，我们提出了 DistilWhisper，这是一种能够弥合这些语言的 ASR 中的性能差距的方法，同时保留多任务和多语言功能的优势。我们的方法涉及两个关键策略：使用特定语言的专家对 whisper-small 进行轻量级模块化 ASR 微调，以及从 whisper-large-v2 中进行知识蒸馏。这种双重方法使我们能够有效提升 ASR 性能，同时保持从多任务和多语言预训练中继承的鲁棒性。结果表明，与标准微调或 LoRA 适配器相比，我们的方法更有效，提升了目标语言在域内和域外测试集中的性能，同时在推理时仅引入了可忽略的参数开销。
...
摘要：在本研究中，我們引入生成式製造系統 (GMS) 作為一種新穎的方法，用於有效管理和協調自主製造資產，從而增強其應變能力和靈活性，以滿足廣泛的生產目標和人類偏好。與傳統的顯式建模不同，GMS 採用生成式 AI，包括擴散模型和 ChatGPT，從設想的未來進行隱式學習，標誌著從模型最優決策轉向訓練取樣決策制定。透過整合生成式 AI，GMS 能夠透過與人類的互動式對話進行複雜的決策制定，允許製造資產生成多個高品質的整體決策，並能根據人類的回饋進行反覆改進。實證研究結果展示了 GMS 在系統韌性和對不確定性的應變能力方面有顯著的提升，決策時間從數秒縮短至毫秒。這項研究強調了生成式解決方案中固有的創造力和多樣性，透過無縫且持續的人機互動，促進以人為中心的決策制定。
...
摘要：不安分的 Multi-Armed Bandits (RMAB) 在建模具有即时激活约束的顺序决策问题中发挥着核心作用，该约束规定在任何决策时期最多可以激活 B 个臂。每个不安分的臂都具有一个状态，该状态根据马尔可夫决策过程独立演变，无论是否被激活。在本文中，我们考虑在具有未知转移函数和对抗性奖励的情景 RMAB 中学习的任务，这些奖励可能会在不同情景中任意改变。此外，我们考虑了一个具有挑战性但自然的赌徒反馈设置，其中只有已激活臂的对抗性奖励才会向决策者 (DM) 透露。DM 的目标是在学习过程中最大化其对抗性总奖励，同时必须在每个决策时期满足即时激活约束。我们开发了一种新颖的强化学习算法，该算法有两个关键贡献者：一个新颖的偏差对抗性奖励估计器，用于处理赌徒反馈和未知转移，以及一个低复杂度指数策略，用于满足即时激活约束。我们展示了算法的 $\tilde{\mathcal{O}}(H\sqrt{T})$ 后悔界限，其中 $T$ 是情景数，$H$ 是情景长度。据我们所知，这是第一个算法，可以确保在我们的考虑的具有挑战性的设置中对抗性 RMAB 的 $\tilde{\mathcal{O}}(\sqrt{T})$ 后悔。
...
摘要：本研究引入了一個系統性的架構，以比較大型語言模型 (LLM) 在各種化學資訊學任務中微調的功效。採用統一的訓練方法，我們評估了三個著名的模型 (RoBERTa、BART 和 LLaMA) 使用簡化分子輸入線條輸入系統 (SMILES) 作為通用分子表示格式預測分子特性的能力。我們的比較分析涉及預訓練這些模型的 18 種配置，具有不同的參數大小和資料集規模，然後在 DeepChem 的六個基準任務上對它們進行微調。我們在所有模型中保持一致的訓練環境，以確保可靠的比較。這種方法使我們能夠評估模型類型、大小和訓練資料集大小對模型效能的影響。具體來說，我們發現基於 LLaMA 的模型通常提供最低的驗證損失，表明它們在任務和規模上的適應性更強。然而，我們觀察到，絕對驗證損失並非模型效能的決定性指標——與先前的研究相矛盾——至少對於微調任務而言：相反，模型大小發揮著至關重要的作用。通過嚴格的複製和驗證，包括多個訓練和微調週期，我們的研究不僅描述了每種類型模型的優點和缺點，還提供了一種用於選擇最適合特定化學資訊學應用程式的強健方法論。本研究強調了在部署 AI 進行分子性質預測時考慮模型架構和資料集特徵的重要性，為在藥物發現和相關領域更明智和有效地利用 AI 鋪平了道路。
...
摘要：同理心需要換位思考：同理心的回應要求一個人推理他人經歷過什麼，並用語言傳達這種理解。然而，大多數對同理心的 NLP 方法並沒有明確地建模這個對齊過程。在此，我們引入一種新的方法來識別同理心言語中的對齊，這種方法以評估理論為基礎。我們引入了一個新的數據集，其中包含對某人經歷的不同類型評估的 9.2K 以上的跨度級別註釋，以及說話者和觀察者言語之間的 3K 以上的同理心對齊。通過計算實驗，我們表明這些評估和對齊可以被準確地識別出來。在超過 920 萬 Reddit 對話的實驗中，我們發現評估捕捉到了有意義的行為群組，但大多數回應的對齊程度很低。然而，我們發現心理健康專業人員參與了更多具有同理心的對齊。
...
摘要：沟通被定义为「谁对谁说什么，有什么效果。」沟通者的讯息会产生下游接收者的效果，也就是行为。接收者的行为，作为讯息的下游效果，会传递出丰富的讯号。即使在传递了关于讯息的讯号后，行为资料在训练大型语言模型时仍常被忽略。我们证明在接收者的行为上训练 LLM 实际上可以帮助改善其内容理解能力。具体来说，我们证明训练 LLM 来预测按赞和留言的接收者行为，可以改善 LLM 在各种下游内容理解任务上的表现。我们在 23 个基准数据集上对 40 个视频和图像理解任务展示了这种性能提升，在零次学习和微调设置中都优于许多监督基准。此外，由于接收者的行为（例如按赞和留言）在互联网上是默认收集的，并且不需要任何人工注释即可发挥作用，因此我们在训练此资料后获得的性能提升基本上是免费的。我们发布了从多个平台收集的 750,000 张图像和视频的经过整理的接收者行为留言和按赞，以及我们的指令微调资料。
...
摘要：我們提出 EchoScene，一種互動且可控的生成模型，可在場景圖上生成 3D 室內場景。EchoScene 利用雙分支擴散模型，可動態適應場景圖。現有方法由於節點數量不同、邊緣組合多樣以及操作器引發的節點邊緣操作，難以處理場景圖。EchoScene 通過將每個節點與去噪程序關聯起來，並啟用協作資訊交換，增強可控且一致的生成，並意識到全局約束。這是透過形狀和佈局分支中的資訊迴音方案實現的。在每個去噪步驟中，所有程序都會與資訊交換單元共享其去噪資料，該單元使用圖形卷積結合這些更新。該方案確保去噪程序受到場景圖整體理解的影響，從而促進生成全局一致的場景。產生的場景可以在推理期間透過編輯輸入場景圖和對擴散模型中的雜訊進行採樣來操作。廣泛的實驗驗證了我們的做法，它保持了場景的可控性，並在生成保真度方面超越了以前的方法。此外，生成的場景品質很高，因此可直接相容於現成的紋理生成。程式碼和訓練模型是開源的。
...
摘要：<paragraph>背景與目的：確定缺血性中風的血栓栓塞來源對於治療和二次預防至關重要，但通常無法確定。本研究描述了數字病理學中栓塞的自我監督深度學習方法，用於根據組織病理學圖像對缺血性中風血塊來源進行分類。方法：該數據集包括來自 STRIP AI Kaggle 挑戰賽的全幻燈片圖像 (WSI)，由機械血栓切除術後缺血性中風患者取出的血塊組成。使用遷移學習和自我監督預訓練開發了基於 Transformer 的深度學習模型，用於對 WSI 進行分類。自定義包括注意池化層、加權損失函數和閾值優化。測試並比較了各種模型架構，並主要使用加權對數損失對模型性能進行評估。結果：該模型在交叉驗證中實現了 0.662 的對數損失分數，在測試集中實現了 0.659。比較了不同的模型主幹，其中 swin_large_patch4_window12_384 表現出更高的性能。採用了血塊來源分類的閾值技術來平衡假陽性和假陰性。結論：該研究展示了基於 Transformer 的深度學習模型在從組織病理學圖像中識別缺血性中風血塊來源方面的功效程度，並強調了專門針對血栓 WSI 調整建模技術的必要性。需要進一步研究以提高模型性能、可解釋性、驗證其有效性。未來的改進可能包括整合更大的患者群體、先進的預處理策略，並探索集成多模態方法以提高診斷準確性。</paragraph>
...
摘要：視覺Transformer徹底改變了電腦視覺，但它們的運算需求對訓練和部署提出了挑戰。本文介紹了 LOTUS（具有超稀疏性的樂透Transformer），這是一種新方法，利用數據樂透選票選擇和稀疏性剪枝來加速視覺Transformer訓練，同時保持準確性。我們的做法專注於識別和利用最有資訊性的數據子集，並消除多餘的模型參數來優化訓練過程。透過廣泛的實驗，我們證明了 LOTUS 在顯著降低運算需求的情況下實現快速收斂和高準確性的有效性。這項工作突顯了結合數據選擇和稀疏性技術以進行有效視覺Transformer訓練的潛力，為該領域的進一步研究和開發打開了大門。
...
摘要：<paragraph>在過去十年中，與其他應用領域類似，社交媒體內容已被證明在災害信息學中非常有效。然而，由於數據的非結構化性質，社交媒體內容中的災害分析面臨著許多挑戰。為了充分探索社交媒體內容在災害信息學中的潛力，訪問相關內容和正確的地理位置信息非常關鍵。在本文中，我們提出了應對這些挑戰的三步驟解決方案。首先，所提出的解決方案旨在將社交媒體貼文分類為相關和不相關的貼文，然後通過命名實體識別 (NER) 分析從貼文的文字中自動提取位置信息。最後，為了快速分析大量社交媒體貼文中涵蓋的主題，我們執行主題建模，生成一個頂級關鍵字清單，重點說明推文中討論的問題。對於推文相關分類 (RCTP)，我們提出了一個基於優點的融合框架，結合了四種不同模型的功能，即 BERT、RoBERTa、Distil BERT 和 ALBERT，在基準數據集上獲得了 0.933 的最高 F1 分數。對於從推文文字中提取位置 (LETT)，我們在 NER 框架中評估了四個模型，即 BERT、RoBERTa、Distil BERTA 和 Electra，獲得了 0.960 的最高 F1 分數。對於主題建模，我們使用了 BERTopic 函式庫來發現相關推文中的隱藏主題模式。所提出的端到端解決方案的所有組成的實驗結果非常令人鼓舞，並暗示了社交媒體內容和 NLP 在災害管理中的潛力。</paragraph>
...
摘要：大型語言模型顯得相當有創意，在創意任務中經常表現得與一般人一樣出色。然而，對於 LLM 創意的研究僅專注於「產品」，很少關注創意「過程」。人類創意的過程分析通常需要手動編碼的類別或利用反應時間，這不適用於 LLM。我們提供了一種自動化方法，用於描述人類和 LLM 如何在替代用途任務中探索語義空間，並與流暢口語任務中的行為形成對比。我們使用句子嵌入來識別回應類別並計算語義相似性，我們使用這些相似性來生成跳躍輪廓。我們的結果證實了人類早期工作的報告，即持久（在少數語義空間中深入搜尋）和靈活（在多個語義空間中廣泛搜尋）的途徑都可以通往創造力，這兩種途徑都可以帶來類似的創造力分數。發現 LLM 偏向於持久或靈活的途徑，這在不同的任務中有所不同。儘管 LLM 作為一個族群符合人類的輪廓，但它們與創造力的關係卻不同，其中較靈活的模型在創造力方面得分更高。我們的資料集和腳本可以在 \href{https://github.com/surabhisnath/Creative_Process}{GitHub} 上找到。
...
摘要：<paragraph>在極低功耗裝置上的機器學習應用程式，通常稱為微型機器學習 (TinyML)，承諾提供更智慧且更緊密的連結世界。然而，目前的 TinyML 研究進展受到相關資料集大小和品質的限制。為了應對這項挑戰，我們推出 Wake Vision，一個針對人員偵測量身打造的大規模、多樣化資料集，這項任務是 TinyML 視覺感測的法規任務。Wake Vision 包含超過 600 萬張影像，與先前的標準相比，增加了百倍，而且經過徹底的品質過濾。使用 Wake Vision 進行訓練，可將準確度提高 2.41%，與既定的基準相比。除了資料集之外，我們還提供五個詳細的基準組，用於評估模型在測試資料的特定區段上的效能，例如不同的光線條件、與相機的距離，以及受試者的特徵。這些新穎的細緻基準有助於評估模型品質，在僅關注整體準確度時，這些挑戰性的真實世界場景通常會被忽略。透過在基準上評估 MobileNetV2 TinyML 模型，我們發現輸入解析度在偵測遠距離受試者時扮演比模型寬度更重要的角色，而且由於資料集品質的關係，量化對模型穩健性的影響很小。這些發現強調了詳細評估的重要性，以找出模型開發的必要因素。資料集、基準組、程式碼和模型在 CC-BY 4.0 授權下公開提供，可以使用於商業用例。</paragraph>
...
摘要：傳統語言模型以自迴歸的方式運作，亦即一次預測一個符號。模型規模的快速爆炸導致高推論時間。在這項工作中，我們提出 DynaMo，一組多符號預測語言模型，可減少網路推論時間。我們的模型基於其對預測聯合機率分佈的信心，動態預測多個符號。我們提出一個輕量級技術來訓練這些模型，利用傳統自迴歸對應項的權重。此外，我們提出創新的方法來增強估計的聯合機率以提高文字生成品質，即共現加權遮罩和自適應閾值。我們也提出系統性的定性和定量方法來嚴格測試非自迴歸生成的生成文字品質。我們套件中的其中一個模型 DynaMo-7.3B-T3，在與基準線 (Pythia-6.9B) 相同品質的生成文字中，僅以 5.87% 和 2.67% 的參數和訓練時間開銷，達到 2.57 倍的加速。
...
摘要：視覺語言模型 (VLM) 近期浮現並受到關注，原因在於它們能理解影像和文字資料的雙重模式。LLaVA、ChatGPT-4 和 Gemini 等 VLM 近期在任務上展現驚人的表現，例如自然影像標題、視覺問答 (VQA) 和空間推理。此外，Meta AI 的通用分割模型 Segment Anything Model (SAM) 在從未預見的影像中分離物件方面展現前所未有的表現。由於醫學專家、生物學家和材料科學家例行檢查顯微鏡或醫學影像，並結合標題、文獻或報告形式的文字資訊，並得出極為重要且有價值的結論，因此無庸置疑地，在這些影像上測試 VLM 和 SAM 等基礎模型的表現至關重要。在此研究中，我們要求 ChatGPT、LLaVA、Gemini 和 SAM 對各種顯微鏡影像進行分類、分割、計數和 VQA 任務。我們觀察到，ChatGPT 和 Gemini 能夠令人印象深刻地理解顯微鏡影像中的視覺特徵，而 SAM 則相當有能力從一般意義上分離人工製品。然而，表現並未接近領域專家 - 模型很容易受到影像中雜質、缺陷、人工製品重疊和多樣性的影響。
...
摘要：自動化軟體測試是軟體開發過程中不可或缺的一環，它能簡化工作流程並確保產品可靠性。在此背景下，視覺測試（特別是關於使用者介面 (UI) 和使用者體驗 (UX) 驗證）是決定整體軟體品質的關鍵因素之一。儘管如此，傳統方法（例如逐像素比較和基於區域的視覺變更偵測）在捕捉背景相似性、細微變更，以及理解 UI 元素之間的空間關係方面仍有不足。在本文中，我們介紹了一種用於軟體測試自動化中的視覺變更偵測的新穎圖形化方法。透過利用機器學習模型，我們的這種方法可以從軟體截圖中準確識別 UI 控制項，並建構一個圖形，以表示控制項之間的背景和空間關係。然後，這些資訊會用於在不同版本的軟體截圖中找到 UI 控制項之間的對應關係。產生的圖形囊括了 UI 的複雜佈局和底層背景關係，提供了一個整體且具備背景感知能力的模型。最後，這個模型會用於偵測和突顯 UI 中的視覺回歸。在不同資料集上進行的全面實驗顯示，我們的變更偵測器可以在各種簡單和複雜的測試情境中準確偵測視覺軟體變更。此外，在更複雜的測試情境中，它在逐像素比較和基於區域的基準測試中表現出色，大幅勝出。這項工作不僅有助於推進視覺變更偵測的進展，也具有實際意義，為真實世界的軟體測試自動化挑戰提供了一個穩健的解決方案，進而提升可靠性，並確保軟體介面的無縫演進。
...
摘要：多選題 (MCQ) 因為在管理和評分上的效率，是一種評量學生知識的熱門方法。
撰寫高品質的數學多選題是一個需要大量人力且耗時的工作，要求教育者制定精確的題幹和合理的干擾選項。
大型語言模型 (LLM) 的近期進展激發了自動化多選題製作的興趣，但仍有挑戰存在於確保數學正確性和解決學生的錯誤。
本文介紹了一個原型工具，旨在促進 LLM 與教育者之間的協作，以簡化數學多選題的產生過程。
我們進行了一項涉及數學教育者的試驗研究，以探討該工具如何幫助他們簡化製作高品質數學多選題的過程。
我們發現，雖然 LLM 可以產生結構良好的題幹，但它們產生干擾選項的能力，以捕捉常見的學生錯誤和誤解是有限的。
儘管如此，人機協作有潛力提高多選題產生的效率和效能。
...
摘要：偽科學、邏輯謬誤和陰謀論等不合理信念的盛行，對社會構成重大障礙，並有散布錯誤資訊的風險。本研究利用既定的心理測量評估，探討大型語言模型 (LLM) 相對於一般人在偵測普遍邏輯陷阱的能力。我們進行一項哲學探討，將人類的理性與 LLM 的理性並置。此外，我們提出利用 LLM 來對抗誤解的方法，並借鑒認知失調理論和訊息精緻可能性理論等心理說服模型。透過這項努力，我們強調 LLM 作為個人化錯誤資訊破解代理的潛力。
...
摘要：在本文中，我們提出了 Sim-Grasp，一個強大的 6-DOF 雙指抓取系統，它整合了先進的語言模型，以增強在混亂環境中的物體操作。我們引入了 Sim-Grasp-Dataset，其中包含 500 個場景中的 1,550 個物體，並有 790 萬個註解標籤，並開發了 Sim-GraspNet 從點雲生成抓取姿勢。Sim-Grasp-Polices 對單一物體的抓取成功率達到 97.14%，對等級 1-2 和等級 3-4 物體的混合混亂場景的抓取成功率分別達到 87.43% 和 83.33%。通過整合語言模型來透過文字和方框提示進行目標識別，Sim-Grasp 能夠進行物體不可知和目標挑選，推動了智慧型機器人系統的界限。
...
摘要：分散式多智能體學習 (DML) 能在保留資料隱私的同時進行協作模型訓練。然而，智能體資源（運算、通訊和任務大小）的內在異質性可能會導致訓練時間大幅變異。這種異質性會造成瓶頸，由於落後效應而延長整體訓練時間，並可能浪費較快智能體的備用資源。為了在異質環境中將訓練時間最小化，我們提出分散式多智能體學習的通訊效率訓練工作負載平衡 (ComDML)，它透過分散式方法平衡智能體之間的工作負載。ComDML 採用局部損失分割訓練，能進行並行更新，其中較慢的智能體會將部分工作負載卸載給較快的智能體。為了將整體訓練時間最小化，ComDML 優化工作負載平衡，同時考量智能體的通訊和運算能力，這取決於整數規劃。開發了動態分散式配對排程器來有效配對智能體並確定最佳卸載量。我們證明在 ComDML 中，對於凸函數和非凸函數，較慢和較快的智能體模型都會收斂。此外，在熱門資料集（CIFAR-10、CIFAR-100 和 CINIC-10）及其非 I.I.D. 變體上進行的廣泛實驗結果，以及 ResNet-56 和 ResNet-110 等大型模型，證明與現有方法相比，ComDML 可以顯著減少整體訓練時間，同時維持模型準確度。ComDML 在異質環境中展現出穩健性，且可以無縫整合隱私措施以增強資料保護。
...
摘要：<paragraph>我們提出 WIBA，這是一個新穎的架構和方法組，可全面了解不同脈絡中的「論點是什麼」。我們的做法開發了一個綜合架構，用於偵測：(a) 論點是否存在，(b) 論點的主題，以及 (c) 論點的立場，並正確考量三項任務之間的邏輯依賴性。我們的演算法利用大型語言模型的微調和提示工程。我們評估我們的做法，並顯示它在所有三項能力中都有良好的表現。首先，我們開發並發布一個論點偵測模型，它可以將一段文字分類為論點，在三個不同的基準資料集上的 F1 分數介於 79% 到 86% 之間。其次，我們發布一個語言模型，它可以識別句子中被論證的主題，無論是隱含的還是明確的，平均相似度分數為 71%，比目前的簡化方法高出近 40%。最後，我們開發了一種論點立場分類的方法，並評估我們做法的能力，顯示它在三個不同的基準資料集上達到了 71% 到 78% 之間的分類 F1 分數。我們的評估顯示，WIBA 允許全面了解在不同脈絡中大型語料庫中的「論點是什麼」，這對語言學、溝通以及社會和電腦科學中的許多應用程式來說都是核心興趣。為了促進取得本著作中概述的進展，我們將 WIBA 作為一個免費開放存取平台 (wiba.dev) 發布。</paragraph>
...
摘要：客戶服務是公司與客戶互動的方式。它可以極大地提升整體客戶滿意度。然而，高品質的服務可能會變得昂貴，這會帶來一個誘因，讓服務盡可能地具有成本效益，並促使大多數公司利用 AI 驅動的助理或「聊天機器人」。另一方面，客戶仍然渴望人與人的互動，特別是在遇到糾紛和帳單支付等複雜情況和敏感話題時。
這提高了客戶服務人員的標準。他們需要準確地了解客戶的問題或疑慮，找出可接受且可行的解決方案（並符合公司的政策），同時還能一次處理多個對話。
在這項工作中，我們將「Ask Me Anything」（AMA）作為一種附加功能，引入到面向客服人員的客戶服務介面中。AMA 讓客服人員可以在處理客戶對話時，依據需求向大型語言模型（LLM）提問——LLM 會即時提供準確的回應，減少客服人員在不同情境間切換的次數。在我們的內部實驗中，我們發現使用 AMA 的客服人員，與使用傳統搜尋體驗的客服人員相比，在包含搜尋的對話中平均減少了約 10% 的秒數，這相當於每年節省數百萬美元。使用 AMA 功能的客服人員有將近 80% 的時間給予正面的回饋，這證明了其作為 AI 輔助功能在客戶服務中的實用性。
...
摘要：大型文本到图像模型可以根据文本提示生成高质量且多样的图像，已展现出非凡的成功。这些模型最终的目标是创建复杂的场景，而解决多主题生成挑战是实现这一目标的关键步骤。然而，现有的最先进的扩散模型在生成涉及多个主题的图像时面临困难。当出现包含多个主题的提示时，这些模型可能会省略一些主题或将它们合并在一起。为了应对这一挑战，我们提出了一种基于指导原则的新方法。我们允许扩散模型最初提出一个布局，然后我们重新排列布局网格。这是通过强制交叉注意力图 (XAM) 遵守建议的蒙版并通过将像素从潜在地图迁移到我们确定的新位置来实现的。我们引入了新的损失项，旨在减少 XAM 熵以更清晰地定义主题的空间，减少 XAM 之间的重叠，并确保 XAM 与其各自的蒙版对齐。我们将我们的方法与几种替代方法进行对比，并表明它在各种文本提示中更忠实地捕捉了所需的理念。
...
摘要：隨著近期大型語言模型等重量級模型的新興多模式工作負載，對硬體的運算和記憶體需求大幅增加。為了滿足這些日益增長的需求，設計可擴充的硬體架構已成為一個關鍵問題。在最近的解決方案中，基於 2.5D 矽介面層多晶片模組 (MCM) 的 AI 加速器由於其在低工程成本和可組合性方面的顯著優勢，已被積極探索為一個有前途的可擴充解決方案。然而，先前的 MCM 加速器基於具有固定資料流的同質架構，由於其有限的工作負載適應性，在高度異質的多模式工作負載中遇到了重大挑戰。因此，在這項工作中，我們探討了異質資料流 MCM AI 加速器的機會。我們發現，在異質資料流 MCM AI 加速器上安排多模式工作負載是一個重要且具有挑戰性的問題，因為其重要性和規模，即使對於 6x6 晶片組上的單一模型案例，也達到 O(10^18) 的規模。我們開發了一組啟發法來導航龐大的排程空間，並將它們編入一個排程器，其中包含先進的技術，例如晶片間流水線。我們對資料中心多租戶和 AR/VR 使用案例的十個多模式工作負載場景的評估顯示了我們方法的有效性，與同質基準相比，對於各自的應用程式設定，平均節省了 35.3% 和 31.4% 的能耗延遲乘積 (EDP)。
...
摘要：<paragraph>傳統的人類回饋強化學習 (RLHF) 方法依賴於參數模型，例如 Bradley-Terry 模型，在捕捉人類偏好的非傳遞性和非理性方面存在不足。最近的進展表明，直接使用偏好機率可以更準確地反映人類偏好，實現更靈活、更準確的語言模型對齊。在本文中，我們提出了一種基於自玩的方法進行語言模型對齊，將問題視為一個恆和雙人遊戲，旨在識別納什均衡策略。我們的做法稱為「自玩偏好最佳化」(SPPO)，透過反覆的策略更新來逼近納什均衡，並享有理論上的收斂保證。我們的做法可以有效地增加所選回應的對數似然度，並減少被拒絕回應的對數似然度，這是對稱成對損失（例如直接偏好最佳化 (DPO) 和身分偏好最佳化 (IPO)）無法輕易實現的。在我們的實驗中，僅使用 UltraFeedback 資料集中的 60k 個提示（沒有回應），且沒有任何提示擴充，透過利用僅有 0.4B 參數的預訓練偏好模型 PairRM，SPPO 可以從微調 Mistral-7B-Instruct-v0.2 中獲得模型，在 AlpacaEval 2.0 上對抗 GPT-4-Turbo 時，達到 28.53% 的最先進長度控制勝率。它還在 MT-Bench 和 Open LLM Leaderboard 上優於（反覆）DPO 和 IPO。值得注意的是，SPPO 的強勁效能是在沒有來自 GPT-4 或其他更強大的語言模型的額外外部監督（例如回應、偏好等）的情況下實現的。</paragraph>
...
摘要：本研究針對最新的大型語言模型 Llama-3 提出一個目標模型編輯分析。我們探討了熱門模型編輯技術的功效，包括 ROME、MEMIT 和 EMMET，這些技術是為精確的層級介入而設計的。我們透過評估來找出針對編輯最有效的層級，評估涵蓋了透過三種不同策略執行多達 4096 次編輯：循序編輯、批次編輯，以及我們稱為循序批次編輯的混合方法。我們的發現顯示，與循序使用較小的編輯批次來執行相同次數的編輯相比，增加編輯批次大小可能會更顯著地降低模型效能。因此，我們認為循序模型編輯是擴充模型編輯方法的重要組成部分，而未來的研究應著重於結合批次和循序編輯的兩種方法。此觀察結果顯示，目前推動使用較大編輯批次大小的模型編輯方法存在潛在限制，我們希望它能為未來優化批次大小和模型編輯效能的研究鋪路。
...
摘要：語義文本關聯性是語義相似性的更廣泛概念。它衡量兩段文本傳達類似意義或主題的程度，或共享相關概念或上下文。這種相關性的概念可以應用於各種應用程式，例如文件分群和摘要。SemEval-2024 中的共用任務 SemRel-2024，旨在透過提供包含阿拉伯語在內的十四種語言和方言的資料集，縮小語義關聯性任務的差距。本文報告我們參與 A 軌道（阿爾及利亞和摩洛哥方言）和 B 軌道（現代標準阿拉伯語）的結果。BERT 基礎模型經過擴充和微調，以進行監督軌道 (A) 中的回歸評分，而 BERT 基礎餘弦相似度則用於非監督軌道 (B)。我們的系統在 SemRel-2024 中的 MSA 排名第 1，Spearman 相關係數得分為 0.49。我們在摩洛哥排名第 5，在阿爾及利亞排名第 12，得分分別為 0.83 和 0.53。
...
摘要：對於長篇文件摘要，語篇結構對於辨別文本的關鍵內容和句子之間重要性層級的差異非常重要。不幸的是，將語篇結構理論 (RST) 整合到長篇文件摘要的參數有效微調策略中仍未被探討。因此，本文介紹 RST-LoRA，並提出四種 RST 感知變體，以將 RST 明確納入 LoRA 模型中。我們的實證評估表明，整合語篇關係的類型和不確定性可以互補地增強 LoRA 在摘要任務中的性能。此外，我們引入的效能最佳變體優於香草 LoRA 和全參數微調模型，正如多項自動和人工評估所證實的那樣，甚至超越了先前的最先進方法。
...
摘要：在不確定的環境中安全地規劃，代理人必須在實用性與安全限制之間取得平衡。安全的規劃問題可以建模為受機會約束的部分可觀察馬可夫決策過程 (CC-POMDP)，而解決方案通常使用昂貴的展開或啟發法來估計最佳值和動作選擇策略。這項工作引入了受約束零策略反覆運算演算法，它透過學習最佳值和策略的神經網路近似值來解決信念空間中的 CC-POMDP，並使用額外的網路頭來估計給定信念下的失敗機率。這個失敗機率會在線上蒙地卡羅樹狀搜尋 (MCTS) 期間引導安全的動作選擇。為了避免過度強調基於失敗估計的搜尋，我們引入了 $\Delta$-MCTS，它使用適應性共形推論在規劃期間更新失敗閾值。這個方法在一個安全關鍵的 POMDP 基準、一個飛機防撞系統和安全的 CO$_2$ 儲存永續性問題上進行了測試。結果顯示，透過將安全限制與目標分開，我們可以在不最佳化獎勵與成本之間的平衡下，達成目標安全等級。
...
摘要：最近的研究透過訓練後量化或低位元權重表示，為大型語言模型 (LLM) 引進了有效的壓縮技術。儘管量化權重提供了儲存效率，並允許更快的推論，但現有研究表明量化可能會損害 LLM 的效能並加劇偏差。本研究探討了量化模型的信心和校準，考慮了語言模型類型和規模等因素作為量化損失的影響因素。首先，我們揭示了使用 GPTQ 將權重量化到 4 位元會導致對真實標籤的信心下降，在不同的語言模型中觀察到的影響各不相同。其次，我們觀察到不同規模對信心影響的波動。最後，我們根據信心等級提出了量化損失的解釋，表明量化對原本完整模型展現低信心等級的樣本產生了不成比例的影響。
...
不正確的輸出，可能會誤導依賴它們的使用者，彷彿它們是正確的。為了減少這種過度依賴，有人呼籲 LLM 向最終使用者傳達它們的不確定性。然而，很少有實證研究探討使用者如何感知和根據 LLM 的不確定性表達採取行動。我們透過大規模、預先註冊的人類受試者實驗 (N=404) 來探討這個問題，其中參與者回答醫療問題，而有或沒有存取虛構的 LLM 注入式搜尋引擎的回應。
使用行為和自我報告的測量，我們探討不同的自然語言不確定性表達如何影響參與者的依賴、
信任和整體任務表現。我們發現第一人稱表達
（例如，「我不確定，但...」）降低參與者對系統的信心和同意系統答案的傾向，同時提高參與者的準確性。探索性分析表明，這種增加可以歸因於對不正確答案的過度依賴減少（但並未完全消除）。雖然我們觀察到從一般觀點表達的不確定性（例如，「這並不清楚，但...」）有類似的影響，但這些影響較弱且沒有
統計顯著性。我們的研究結果表明，使用自然語言的不確定性表達可能是減少對 LLM 過度依賴的有效方法，但使用的精確語言很重要。這突顯了在大規模部署 LLM 之前進行使用者測試的重要性。</paragraph>
...
摘要：AI 基礎模型在各種應用中獲得廣泛採用，包括放射學等醫療領域。然而，醫療基礎模型通常在有限的任務上進行測試，讓它們的泛化能力和偏差無法被探索。我們提出 RayDINO，一個通過 873k 胸部 X 光片上的自我監督訓練的大型視覺編碼器。我們將 RayDINO 與九項放射學任務中以前最先進的模型進行比較，從分類和密集分割到文本生成，並對我們模型的人口、年齡和性別偏差進行深入分析。我們的研究結果表明，自我監督允許以患者為中心的 AI 在臨床工作流程中證明其有用性，並全面解釋 X 光片。使用 RayDINO 和小型特定任務適配器，我們達到了最先進的結果，並改進了對未見人群的泛化，同時減輕了偏差，說明了基礎模型的真正前景：通用性和穩健性。
...
摘要：論證結構學習~(ASL)需要預測論證之間的關係。由於它可以建構文件以利於理解，因此已廣泛應用於許多領域~(醫療、商業和科學領域)。儘管其廣泛應用，ASL 仍然是一項具有挑戰性的任務，因為它涉及檢查潛在非結構化論述中句子之間的複雜關係。為了解決這個問題，我們開發了一種名為雙塔多尺度卷積神經網路~(DMON)的簡單但有效的方法，用於 ASL 任務。具體來說，我們將論證組織成一個關係矩陣，該矩陣與論證嵌入一起形成一個關係張量，並設計一種機制來捕獲與上下文相關論證的關係。在三個不同領域的論證挖掘資料集上的實驗結果表明，我們的框架優於最先進的模型。程式碼可在 https://github.com/VRCMF/DMON.git 取得。
...
摘要：視覺語言模型 (VLM) 近期浮現並獲得矚目，因為它們能夠理解影像和文字資料的雙重模式。VLM 如 LLaVA、ChatGPT-4 和 Gemini 近期在自然影像標題、視覺問答 (VQA) 和空間推理等任務中展現出令人印象深刻的表現。此外，Meta AI 的通用分割模型 Segment Anything Model (SAM) 在從未預見過的影像中分離物體方面展現出前所未見的表現。由於醫學專家、生物學家和材料科學家會例行性地檢查顯微鏡或醫學影像，並結合標題、文獻或報告等文字資訊，並得出極為重要且有價值的結論，因此在這些影像上測試 VLM 和 SAM 等基礎模型的表現無疑至關重要。在此研究中，我們讓 ChatGPT、LLaVA、Gemini 和 SAM 執行各種顯微鏡影像的分類、分割、計數和 VQA 任務。我們觀察到 ChatGPT 和 Gemini 能夠令人印象深刻地理解顯微鏡影像中的視覺特徵，而 SAM 則相當有能力從一般意義上分離出人工製品。然而，表現並未接近領域專家 - 這些模型很容易受到影像中雜質、缺陷、人工製品重疊和多樣性的影響。
...
摘要：<paragraph>廣泛部署的大型語言模型 (LLM) 可以產生令人信服但
不正確的輸出，可能會誤導依賴它們的使用者，彷彿它們是正確的。為了減少這種過度依賴，有人呼籲 LLM 向最終使用者傳達它們的不確定性。然而，很少有實證研究探討使用者如何感知和根據 LLM 的不確定性表達採取行動。我們透過一項大規模、預先註冊的人類主體實驗 (N=404) 來探討這個問題，在這個實驗中，參與者回答醫療問題，有或沒有存取虛構的 LLM 注入式搜尋引擎的回應。
使用行為和自我報告的測量，我們探討不同的自然語言不確定性表達如何影響參與者的依賴、
信任和整體任務表現。我們發現第一人稱表達（例如，「我不確定，但...」）會降低參與者對系統的信心和同意系統答案的傾向，同時提高參與者的準確性。探索性分析表明，這種增加可以歸因於減少（但並未完全消除）對不正確答案的過度依賴。雖然我們觀察到從一般觀點表達的不確定性（例如，「這不清楚，但...」）有類似的效果，但這些效果較弱且在統計上不顯著。我們的研究結果表明，使用自然語言的不確定性表達可能是減少對 LLM 過度依賴的有效方法，但使用的精確語言很重要。這凸顯了在大規模部署 LLM 之前進行使用者測試的重要性。</paragraph>
...
摘要：超音波機器人日益被用於醫療診斷和早期疾病篩檢。然而，目前的超音波機器人缺乏理解人類意圖和指令的智慧，阻礙了自主超音波掃描。為了解決這個問題，我們提出了一個新穎的超音波具身智慧系統，它為超音波機器人配備了大型語言模型 (LLM) 和領域知識，從而提高了超音波機器人的效率。具體來說，我們首先設計了一個超音波操作知識資料庫，將超音波掃描的專業知識新增到 LLM 中，使 LLM 能夠執行精確的動作規劃。此外，我們根據「思考-觀察-執行」提示工程設計了一個動態超音波掃描策略，允許 LLM 在掃描過程中動態調整動作規劃策略。廣泛的實驗表明，我們的系統顯著提高了超音波掃描效率和品質，並能從口語指令中獲得。自主醫療掃描技術的這一進步有助於非侵入性診斷和簡化的醫療工作流程。
...
摘要：大型語言模型 (LLM) 在許多數學推理基準測試中獲得了令人印象深刻的成功。然而，越來越令人擔憂的是，其中一些表現實際上反映了資料集污染，其中與基準問題非常相似的資料會洩漏到訓練資料中，而不是真正的推理能力。為了嚴謹地調查此主張，我們委託了小學數學 1000 (GSM1k)。GSM1k 的設計目的是反映已建立的 GSM8k 基準的風格和複雜性，這是衡量小學數學推理的黃金標準。我們確保這兩個基準在人類解決率、解決步驟數、答案大小等重要指標上具有可比性。在評估 GSM1k 上的領先開源和閉源 LLM 時，我們觀察到準確度下降多達 13%，其中幾種類型的模型（例如 Phi 和 Mistral）顯示出幾乎所有模型規模的系統性過度擬合的證據。與此同時，許多模型，尤其是前沿模型（例如 Gemini/GPT/Claude）顯示出最小的過度擬合跡象。進一步的分析表明模型產生 GSM8k 範例的機率與其在 GSM8k 和 GSM1k 之間的效能差距之間存在正相關關係（Spearman 的 r^2=0.32），這表明許多模型可能已經部分記憶了 GSM8k。
...
摘要：語音情緒辨識 (SER) 因其在各種領域廣泛的應用而備受關注，包括人機互動、虛擬助理和心理健康協助。然而，現有的 SER 方法經常忽略預訓練語音辨識任務和下游 SER 任務之間的資訊落差，導致次佳的效能。此外，當前的方法需要大量時間針對每個特定語音資料集進行微調，例如 IEMOCAP，這限制了它們在具有大量雜訊資料的真實世界情境中的效能。為了解決這些問題，我們提出了一個基於主動學習 (AL) 的 SER 微調框架，稱為 \textsc{After}，它利用任務適應預訓練 (TAPT) 和 AL 方法來提升效能和效率。具體來說，我們首先使用 TAPT 來最小化預訓練語音辨識任務和下游語音情緒辨識任務之間的資訊落差。然後，採用 AL 方法來反覆選擇一組資訊量最大且最多樣化的樣本進行微調，從而減少時間消耗。實驗證明，我們提出的方法 \textsc{After}，僅使用 20% 的樣本，就能將準確度提升 8.45%，並將時間消耗減少 79%。\textsc{After} 的額外延伸和消融研究進一步確認了其在各種真實世界情境中的效能和適用性。我們的原始程式碼可在 Github 上取得，以供複製。(https://github.com/Clearloveyuan/AFTER)
...
摘要：每年，植物寄生線蟲，植物病原體的主要群組之一，會造成全球作物大量損失。為了減輕線蟲造成的作物產量損失，有效的線蟲監控方法對於植物和作物疾病管理至關重要。在其他方面，有效的線蟲檢測有助於醫學研究和藥物發現，因為線蟲是模式生物。隨著電腦技術的快速發展，電腦視覺技術為量化線蟲或線蟲感染提供了一個可行的解決方案。在本文中，我們調查並分類了通過深度學習模型進行線蟲檢測的研究和可用數據集。為了促進相關研究的進展，本調查介紹了潛在的最新物體檢測模型、訓練技術、優化技術和深度學習初學者的評估指標。此外，七個最先進的物體檢測模型在三個公共數據集和植物寄生線蟲的 AgriNema 數據集上得到驗證，以構建線蟲檢測基準。
...
摘要：<paragraph>醫學與人工智慧（AI）工程代表兩個截然不同的領域，各自都有數十年的已發表歷史。隨著這些歷史而來的是一套術語，其應用方式有其特定方式。然而，當兩個具有重疊術語的不同領域開始合作時，可能會發生溝通不良和誤解。本敘述性回顧旨在為這些術語提供歷史背景，強調在醫學 AI 背景中使用這些術語時明確性的重要性，並提供解決方案來減輕來自任一領域的讀者的誤解。透過審查歷史文件，包括文章、寫作指南和教科書，本回顧追溯了資料集及其影響術語的不同演變。最初，探討了「驗證」一詞在醫學和 AI 背景中不一致的解釋。然後對用於 AI 評估的資料集進行分類，即隨機拆分、交叉驗證、時間、地理、內部和外部集。這些資料集的準確且標準化的描述對於證明 AI 應用在醫學中的穩健性和可概化性至關重要。本回顧釐清現有文獻，以全面了解這些分類及其在 AI 評估中的含義。然後，本回顧找出經常被誤解的術語，並提出務實的解決方案來減輕術語混淆。這些解決方案包括使用標準化的術語，例如「訓練集」、「驗證（或調整）集」和「測試集」，以及在每個醫學 AI 研究出版物中明確定義資料集拆分術語。本回顧旨在提高醫學 AI 中溝通的精確性，從而促進這個跨領域領域更有效和透明的研究方法。</paragraph>
...
摘要：了解在医学诊断中影像中呈现的病情的严重程度至关重要，这作为临床评估、治疗以及评估纵向进展的关键指南。本文提出了 Con-PrO：一种使用对比学习集成偏好优化的医学影像严重程度评估的新颖表征学习方法。与最大化类别之间距离的传统对比学习方法不同，ConPrO 将距离偏好知识注入到潜在向量中，在不同严重程度类别和正常类别之间。我们系统地检查了我们框架的关键组件，以阐明对比预测任务如何获取有价值的表征。我们表明，我们的表征学习框架在特征空间中提供了有价值的严重程度排序，同时在分类任务中优于以前最先进的方法。与有监督和自监督基线相比，我们分别实现了 6% 和 20% 的相对改进。此外，我们对严重程度指标和偏好比较在医学领域的相关应用进行了讨论。
...
摘要：在胸部 X 光 (CXR) 影像分析領域中，放射科醫師會仔細檢查各個區域，並在報告中記錄觀察結果。CXR 診斷中錯誤的盛行，特別是在經驗不足的放射科醫師和醫院住院醫師中，這強調了了解放射科醫師的意圖和對應的關注區域的重要性。這種理解對於糾正錯誤至關重要，它引導放射科醫師關注正確的區域，特別是在診斷胸部 X 光異常時。為了回應這個必要性，我們提出了一個創新的系統，旨在識別放射科醫師在報告中表達的主要意圖和 CXR 影像中對應的關注區域。此系統旨在闡明放射科醫師文字發現背後的視覺背景，並有可能糾正經驗較少的從業人員所犯的錯誤，並引導他們關注精確的區域。重要的是，所提出的系統可以幫助為醫院中經驗不足的放射科醫師或初級住院醫師提供建設性的回饋，彌補面對面溝通的差距。此系統代表了一種有價值的工具，用於提高診斷準確性，並在醫療界內促進持續學習。
...
摘要：<paragraph>建立多感官 AI 系統，從多種感官輸入中學習，例如文字、語音、影片、真實世界感測器、穿戴式裝置和醫療資料，在許多科學領域中極具影響力，並帶來實際效益，例如支援人類健康和福祉、支援多媒體內容處理，以及增強真實世界中的自主代理人。透過綜合一系列理論架構和應用領域，本論文旨在推進多感官 AI 的機器學習基礎。在第一部分中，我們提出一個理論架構，形式化模態如何相互作用以產生任務的新資訊。這些互動是所有多模態問題中的基本建構塊，而它們的量化讓使用者能夠了解其多模態資料集、設計原則化方法來學習這些互動，並分析其模型是否成功學習。在第二部分中，我們研究實用多模態基礎模型的設計，這些模型概括許多模態和任務，這表示朝向將大型語言模型基礎化到真實世界的感官模態邁進一步。我們介紹 MultiBench，一個統一的大規模基準，涵蓋各種模態、任務和研究領域，接著是跨模態注意力和多模態Transformer架構，這些架構現在支撐許多當今的多模態基礎模型。在 MultiBench 上調整這些架構的規模，能夠建立通用多感官 AI 系統，我們討論我們在應用這些模型以對情感運算、心理健康、癌症預後和機器人技術產生真實世界影響方面的合作努力。最後，我們透過討論未來的研究如何利用這些想法朝向更通用、互動且安全的 AI 結論本論文。</paragraph>
...
摘要：<paragraph>近期 AI 的突破性進展，將徹底提升我們對醫療保健的研究與理解。開發一個整合式多對多架構，利用多重資料模式輸入，進行多重醫療任務的分析建模，對於統一理解現代醫學至關重要。在此研究中，我們介紹 M3H，一個可解釋的多模態多任務機器學習醫療保健架構，它整合了廣泛醫療任務類別和機器學習問題類別中，來自各種多模態輸入的學習成果。這個架構的模組化設計確保了其資料處理、任務定義和快速模型原型製作的可概化性，適用於臨床和營運醫療保健環境。我們透過驗證從四種模式（表格、時間序列、語言和視覺）訓練的模型，在四個機器學習問題類別中執行 41 項醫療任務，來評估 M3H 架構。我們的結果證明，M3H 持續產生多任務模型，其表現優於標準單任務模型（1.1-37.2%），涵蓋來自 16 個醫療部門的 37 種疾病診斷、三種醫院營運預測和一項患者表型任務：橫跨監督二元分類、多類別分類、回歸和聚類的 ML 問題類別。此外，這個架構還導入了一種新穎的注意力機制，以平衡自我開發（專注於學習來源任務）和交叉探索（鼓勵從其他任務中學習）。此外，M3H 提供了可解釋性的見解，說明額外任務的聯合學習如何透過提議的 TIM 分數影響來源任務的學習，進而闡明任務相互依賴性的動態。其適應性架構促進了客製化和整合，使其成為未來 AI 驅動醫療保健系統中強固且可擴充的候選解決方案。</paragraph>
...
摘要：器官分割是醫學影像的基本任務，對於許多臨床自動化管道很有用。通常，該過程涉及分割整個體積，當感興趣的點受到限制時，這可能是沒有必要的。在這些情況下，可以使用分類器來代替分割。然而，分類器的上下文大小和速度之間存在固有的權衡。為了解決這個問題，我們提出了一種新方法，它採用了在整個視野中使用稀疏採樣的數據選擇策略，而無需圖像重新採樣。這種稀疏採樣策略使得可以實時將體素分類到多個器官中，而無需使用加速器。儘管我們的模型是一個獨立的分類器，但它可以通過查詢任何解析度的網格位置來生成完整的分割。我們已經將我們的模型與現有的分割技術進行了比較，證明了它在醫學影像實際應用中具有優越的執行時間的潛力。
...
摘要：量子機器學習 (QML) 是量子運算一個有前景的早期使用案例。在過去五年中，從理論研究和數值模擬到概念驗證，已取得進展。在當代量子裝置上展示的使用案例包括對醫學影像和 Iris 資料集中的項目進行分類、對手寫影像進行分類和產生、毒性篩選以及學習機率分佈。QML 的潛在優點包括更快的訓練和識別經典中找不到的特徵圖。儘管如此，這些範例缺乏商業開發的規模，而且可能需要好幾年 QML 演算法才能取代經典的解決方案，但 QML 是一個令人興奮的領域。
這篇文章是寫給那些已經對量子運算有扎實知識，現在希望對術語和一些經典機器學習應用程式獲得基本概觀，準備好學習量子機器學習的人。讀者已經理解了相關的相關線性代數，包括希爾伯特空間，一個具有內積的向量空間。
...
摘要：聯盟學習 (FL) 允許多個注重隱私的應用程式利用其資料集建構全球模型，而無需揭露資訊。其中一個領域是醫療保健，在這個領域中，各個孤島合作以生成具有更佳精確度和概括性的全球預測器。然而，內在的挑戰在於醫療資料的高度異質性，這需要採用精密的技術進行評估和補償。本文全面探討了 FL 環境中異質性的數學形式化和分類法，重點在於醫療資料的複雜性。特別是，我們針對最流行的 FL 演算法進行評估和比較，以了解它們應對基於數量、特徵和標籤分佈的異質性的能力。目標是提供 FL 系統中資料異質性對醫療保健網路影響的量化評估，以及 FL 演算法選擇的指南。我們的研究超越現有研究，針對醫療資料使用案例所帶來的獨特挑戰，對七種最常見的 FL 演算法進行基準測試。本文針對一組由不同聯盟醫院孤島收集的表格臨床報告，預測中風復發的風險：此情境中經常遇到的資料異質性及其對 FL 性能的影響。
...
摘要：本文结合 Struts 和 Hibernate 两种架构，使用 DAO（数据访问对象）来存储和访问数据。然后建立一套适用于深度网络的双模态湿度医学图像库，并提出了一种基于图像的双模态医学图像辅助诊断方法。通过各种特征提取方法的测试，曲线产品下的最优操作特征（AUROC）为 0.9985，召回率为 0.9814，准确率为 0.9833。该方法可应用于临床诊断，是一种实用方法。任何门诊医生均可通过系统快速注册，或登录平台上传图像以获取更准确的图像。通过系统，各门诊医师可快速注册或登录平台进行图像上传，从而获取更准确的图像。图像分割可指导临床科室医生。然后对图像进行分析，确定肿瘤的部位和性质，以便进行针对性治疗。
...
摘要：<paragraph>在各種醫療應用中追求卓越對 AI 構成相當大的挑戰，需要進階推理、取得最新的醫療知識以及理解複雜的多模態資料。Gemini 模型具備多模態和長語境推理的強大一般功能，為醫學領域帶來令人興奮的可能性。我們以 Gemini 的這些核心優勢為基礎，引入了 Med-Gemini，這是一個專精於醫學、能夠無縫使用網路搜尋且能使用自訂編碼器有效調整為新模態的高度有能多模態模型系列。我們在 14 個醫療基準上評估 Med-Gemini，在其中 10 個基準上建立了新的最先進 (SoTA) 效能，並在每次可進行直接比較的基準上都超越 GPT-4 模型系列，而且差距通常很大。在熱門的 MedQA (USMLE) 基準上，我們效能表現最佳的 Med-Gemini 模型使用創新的不確定性引導搜尋策略，達到了 91.1% 精確度的 SoTA 效能。在包括 NEJM 影像挑戰和 MMMU（健康與醫學）在內的 7 個多模態基準上，Med-Gemini 比 GPT-4V 提升了平均 44.5% 的相對幅度。我們透過在從長去識別化健康記錄和醫療影片問答中進行大海撈針檢索任務上達到的 SoTA 效能，證明了 Med-Gemini 的長語境功能的有效性，超越了僅使用語境學習的先前客製化方法。最後，Med-Gemini 的效能顯示出實際用途，在醫療文字摘要等任務上超越了人類專家，同時展現了在多模態醫療對話、醫學研究和教育方面具有令人期待的潛力。綜合來說，我們的結果為 Med-Gemini 的潛力提供了令人信服的證據，儘管在這個安全至上的領域中進行實際部署之前，進一步的嚴謹評估至關重要。</paragraph>
...
摘要：對稱群 $S_{n}$ 在大量的量子系統中表現為量子態的某些特性的不變性，相對於對量子位元進行置換。$S_{n}$ 的子群在許多其他脈絡中出現，用於描述經典影像相對於空間轉換（例如反射或旋轉）的標籤對稱性。在這項工作中，我們具備幾何量子機器學習的形式主義，提出了遵循 $S_{n}$ 及其子群的不變量子卷積神經網路 (EQCNN) 架構。我們證明了仔細選擇像素到量子位元嵌入順序可以簡化 $S_{n}$ 小子群的 EQCNN 建構。我們對應於全置換群 $S_{n}$ 的新穎 EQCNN 架構是透過應用所有可能的 QCNN，機率相等，這也可以視為量子神經網路中的中斷策略。對於 $S_{n}$ 的子群，我們使用 MNIST 資料集的數值結果顯示，分類準確度優於非不變 QCNN。$S_{n}$ 不變 QCNN 架構顯示出明顯優於非不變 QCNN 的訓練和測試效能，用於連接和非連接圖的分類。在使用足夠大量的資料訓練時，$S_{n}$ 不變 QCNN 顯示出比 $S_{n}$ 不變 QNN 更好的平均效能。這些結果有助於在置換對稱系統中建構強大的量子機器學習架構。
...
摘要：準確呈現醫療資訊對於病患安全至關重要，然而人工智慧 (AI) 系統，例如大型語言模型 (LLM)，在無錯誤的臨床文字詮釋上遭遇挑戰。這篇論文提出了一個提交給 MEDIQA-CORR 2024 共享任務（Ben Abacha 等人，2024a）的新穎方法，專注於自動校正臨床筆記中的單字錯誤。與依賴廣泛通用資料的 LLM 不同，我們的做法強調從可用的臨床文字資料中萃取與脈絡相關的資訊。利用萃取式和抽象式問答方法的整合，我們建構了一個具有特定領域特徵工程的監督式學習架構。我們的做法整合了領域專業知識，以提升錯誤校正的準確度。透過整合領域專業知識和優先考量有意義的資訊萃取，我們的做法強調了以人為中心的策略在醫療保健中採用 AI 的重要性。
...
摘要：本文探討了群體結構多主體系統 (MAS) 在建立醫療必要性中的應用，這是一個涉及針對臨床指引系統性審查特定患者醫療結構化和非結構化資料的流程。我們透過將其分解成較小、更易於管理的子任務來處理這項複雜任務。每個子任務都由一個專業的 AI 主體處理。我們對各種提示策略對這些主體的影響進行系統性研究，並對不同的大型語言模型 (LLM) 進行基準測試，以確定它們在完成這些任務時的準確性。此外，我們探討了這些主體如何提供可解釋性，從而增強系統內的信任和透明度。
...
摘要：<paragraph>現有的基於 X 射線的預訓練視覺模型通常在相對小規模的數據集（少於 50 萬個樣本）上進行，且解析度有限（例如，224 $\times$ 224）。然而，自監督預訓練大模型成功的關鍵在於海量的訓練數據，而在 X 射線圖像領域中保持高解析度是有效解決各種疑難雜症的保證。在本文中，我們通過提出我們新收集的大規模數據集上的第一個高解析度（1280 $\times$ 1280）基於 X 射線的預訓練基礎視覺模型來解決這些問題，該數據集包含超過 100 萬張 X 射線圖像。我們的模型遵循掩蔽自動編碼器框架，該框架採用掩蔽處理（以高比率）後的標記作為輸入，並通過 Transformer 編碼器-解碼器網路重建掩蔽的圖像塊。更重要的是，我們引入了一種新穎的上下文感知掩蔽策略，該策略利用胸部輪廓作為自適應掩蔽操作的邊界。我們在兩項下游任務（包括 X 射線報告生成和疾病識別）上驗證了我們模型的有效性。大量實驗表明，我們的預訓練醫學基礎視覺模型在下游基準數據集上實現了可比甚至新的最先進性能。本文的源代碼和預訓練模型將在 https://github.com/Event-AHU/Medical_Image_Analysis 上發布。</paragraph>
...
摘要：放射學報告生成 (R2Gen) 展示了多模態大型語言模型 (MLLM) 如何自動化建立準確且連貫的放射學報告。現有方法經常在基於文字的報告中出現幻覺細節，而這些細節並不能準確反映影像內容。為了減輕這種情況，我們引入了一種新策略，SERPENT-VLM（使用視覺語言模型的自精煉放射報告生成），它透過將自精煉機制整合到 MLLM 架構中來改進 R2Gen 任務。我們採用了一種獨特的自監督損失，它利用池化影像表示和生成的放射學文字的上下文表示之間的相似性，以及標準因果語言模型目標，來精煉影像文字表示。這允許模型透過給定影像和生成的文字之間的動態互動來審查和校準生成的文字，從而減少幻覺並持續增強細緻的報告生成。SERPENT-VLM 優於現有的基準，例如 LLaVA-Med、BiomedGPT 等，在 IU X 光和情境中放射學物件 (ROCO) 資料集上實現了 SoTA 效能，並證明對雜訊影像具有穩健性。定性案例研究強調了朝向更精密的 R2Gen MLLM 架構的重大進展，為在醫學影像領域進一步研究自監督精煉開闢了道路。
...
摘要：卷積神經網路 (CNN) 已廣泛用於醫學影像分割任務，展現出有前景的效能。然而，卷積架構中固有的歸納偏差限制了它們建模長程依賴性和空間關聯性的能力。儘管最近的Transformer基礎架構透過利用自我注意機制來編碼長程依賴性和學習表達式表示，來解決這些限制，但它們通常難以提取低階特徵，且高度依賴資料可用性。這促使我們開發 GLIMS，一種資料有效率的注意力引導混合體積分割網路。GLIMS 利用膨脹特徵聚合器卷積區塊 (DACB) 來有效擷取局部和全域特徵關聯性。此外，結合的 Swin Transformer 基礎瓶頸橋接了局部和全域特徵，以提升模型的健壯性。此外，GLIMS 透過通道和空間感知注意力區塊 (CSAB) 採用注意力引導分割方法，以定位表達式特徵，進行細粒度邊界分割。在神經膠質母細胞瘤和多器官電腦斷層掃描分割任務上的量化和定性結果證明了 GLIMS 在複雜性和準確性方面的有效性。GLIMS 在 BraTS2021 和 BTCV 資料集上展現出傑出的效能，超越了 Swin UNETR 的效能。值得注意的是，GLIMS 以大幅減少的可訓練參數數量達到了此高效能。具體來說，GLIMS 有 47.16M 個可訓練參數和 72.30G FLOP，而 Swin UNETR 有 61.98M 個可訓練參數和 394.84G FLOP。程式碼已公開在 https://github.com/yaziciz/GLIMS。
...
摘要：多模態融合專注於整合來自多種模態的資訊，目標是進行更準確的預測，這在包括自動駕駛和醫療診斷在內的廣泛場景中已取得顯著進展。然而，多模態融合的可靠性在很大程度上仍未得到探索，尤其是在低品質資料設定下。本文調查了在野外進行多模態融合的常見挑戰和最新進展，並以全面的分類法呈現。從以資料為中心的觀點來看，我們找出多模態融合在低品質資料上所面臨的四項主要挑戰，即 (1) 受到異質性雜訊污染的雜訊多模態資料，(2) 有些模態遺失的不完整多模態資料，(3) 不同模態的品質或屬性有顯著差異的不平衡多模態資料，以及 (4) 各個模態的品質會隨著不同樣本而動態變化的品質變化多模態資料。這個新的分類法將使研究人員能夠了解該領域的現況，並找出幾個潛在的方向。我們也針對此領域的開放性問題提供討論，並提出有趣的未來研究方向。
...
摘要：<paragraph>人工智慧（AI）整合在醫療保健中，展現出能提升營運效率和健康成果的轉變潛力。大型語言模型（LLM），例如 ChatGPT，已展現其在支援醫療決策方面的能力。將 LLM 嵌入醫療系統正成為醫療保健發展中一個有前景的趨勢。ChatGPT 解決急診部門分流問題的潛力已受到檢視，但鮮少有研究探討其在門診部門的應用。本研究專注於簡化工作流程和提升門診分流的效率，特別旨在評估 ChatGPT 在門診指導中所提供回應的一致性，包括版本內回應分析和版本間比較。對於版本內，結果顯示 ChatGPT-4.0 的內部回應一致性顯著高於 ChatGPT-3.5（p=0.03），且兩者在頂尖建議中都具有一致性（4.0 為 71.2%，3.5 為 59.6%）。然而，版本間一致性相對較低（平均一致性分數=1.43/3，中位數=1），表示兩個版本之間的建議匹配很少。此外，在比較中只有 50% 的頂尖建議完全匹配。有趣的是，ChatGPT-3.5 的回應比 ChatGPT-4.0 的回應更可能是完整的（p=0.02），這表示兩個版本之間在資訊處理和回應產生方面可能有差異。這些發現提供了對 AI 輔助門診作業的見解，同時也促進了探索 LLM 在醫療保健利用中的潛力和限制。未來的研究可能會專注於仔細最佳化 LLM 和 AI 整合在醫療保健系統中，根據人體工學和人為因素原則，精準地與有效門診分流的特定需求保持一致。</paragraph>
...
摘要：本論文介紹我們團隊參與 MEDIQA-ClinicalNLP2024 共享任務 B 的成果。我們提出了一種診斷臨床皮膚科病例的新方法，方法是整合大型多模態模型，特別是在檢索器和重新排名器架構下利用 GPT-4V 的功能。我們的研究表明，GPT-4V 作為檢索代理時，使用皮膚科影像和簡短的病歷，85% 的時間可以準確檢索正確的皮膚狀況。此外，我們憑經驗表明，樸素的思考鏈 (CoT) 適用於檢索，而基於醫療指南的 CoT 則適用於準確的皮膚科診斷。此外，我們引入了多代理對話 (MAC) 框架，並展示了其優於最佳 CoT 策略的卓越性能和潛力。實驗表明，將樸素的 CoT 用於檢索，將多代理對話用於基於批判的診斷，GPT-4V 可以對皮膚科疾病進行早期準確的診斷。這項工作的意義在於改進診斷工作流程、支持皮膚科教育，以及透過提供可擴充、可存取且準確的診斷工具來提升病患照護。
...
摘要：細粒度異常細胞偵測對於臨床診斷和病理研究至關重要。單細胞定序資料為此任務提供了空前的機會。然而，當前的異常偵測方法難以處理多樣本和多領域單細胞定序資料中普遍存在的領域轉移，導致次佳效能。此外，這些方法無法將異常細胞區分為病理上不同的亞型。為了解決此問題，我們提出 ACSleuth，一個新的重建偏差引導生成框架，將異常細胞的偵測、領域適應和細粒度註解整合到一個方法論上的一致工作流程中。值得注意的是，我們提出第一個理論分析，使用生成模型輸出的重建偏差來進行異常偵測，以取代領域轉移。此分析讓我們得以在 ACSleuth 中開發一個新穎且優越的最大平均差異異常評分器。在各種單細胞資料和其他類型表格資料上的廣泛基準測試證明了 ACSleuth 在多樣本和多領域環境中識別和對異常進行亞型分類的優越性，優於最先進的方法。我們的程式碼可於 https://github.com/Catchxu/ACsleuth 取得。
...
摘要：多年来，多模态移动感应已广泛用于对健康、福祉、行为和背景进行推断。然而，阻碍此类模型在现实世界场景中广泛部署的一个重大挑战是分布偏移的问题。这是训练集中数据的分布与现实世界（部署环境）中的数据的分布不同的现象。虽然在计算机视觉和自然语言处理中得到了广泛的探索，并且虽然移动传感方面的先前研究简要地解决了这一问题，但目前的工作主要集中于处理单一数据模式（如音频或加速度计读数）的模型，因此，在处理多模态传感器数据时几乎没有无监督域自适应的研究。为了解决这一差距，我们对域对抗神经网络 (DANN) 进行了广泛的实验，表明它们可以有效地处理多模态传感器数据中的分布偏移。此外，我们提出了一种对 DANN 的新改进，称为 M3BAT，用于多模态移动传感的无监督域自适应，并采用多分支对抗训练，以在域自适应期间考虑传感器数据的多分模态性。通过对两个多模态移动传感数据集、三个推理任务和 14 个源目标域对（包括回归和分类）进行的广泛实验，我们证明了我们的方法在看不见的域上表现有效。与将源域中训练的模型直接部署到目标域相比，该模型在分类任务上的性能提高了高达 12% AUC（接收器工作特征曲线下的面积），在回归任务上的性能提高了高达 0.13 MAE（平均绝对误差）。
...
摘要：社交焦慮症在現代社會中是一個普遍的挑戰，影響著個人和職業領域的人們。如果置之不理，這種狀況可能會造成重大的負面後果，影響社交互動和表現。進一步了解其多樣的身體和情緒症狀對於全面的診斷和量身打造的治療干預至關重要。本研究分析了梅約診所收集的社交焦慮症狀的患病率和頻率，利用一個龐大的 Reddit 資料集探討了各種人類經驗，該資料集專門針對此議題。利用這些平台，本研究旨在提取見解並檢視與社交焦慮症相關的一系列身體和情緒症狀。本研究維護資料集中使用者的嚴格匿名性，以維護道德考量。透過採用一種新穎的方法，本研究利用 BART 為基礎的多標籤零次分類來識別和衡量症狀的患病率和重要性，並以每個考慮症狀的機率分數的形式呈現。結果揭示了獨特的模式：「顫抖」出現作為一種普遍的身體症狀，而「害怕受到負面評斷」等情緒症狀則表現出高頻率。這些發現提供了對社交焦慮症多面向特性的見解，有助於臨床實務和針對其不同表現量身打造的干預措施。
...
摘要：在這項工作中，我們提出了一個不確定性量化框架的新應用，稱為放射治療劑量預測領域中的深度證據學習。使用開放知識規劃挑戰資料集的醫學影像，我們發現這個模型可以有效地用於產生不確定性估計，這些估計繼承了網路訓練完成後預測誤差相關性。這是在重新制定原始損失函數以實現穩定實現之後才實現的。我們發現 (i) 認識論不確定性與預測誤差高度相關，各種關聯指標與蒙特卡洛輟學和深度集合方法相當或更強，(ii) 中位數誤差隨不確定性閾值變化，對於深度證據學習中的認識論不確定性來說更線性相對於這其他兩個傳統框架，表明對模型誤差的靈敏度更均勻校準，(iii) 相對於認識論不確定性，隨機不確定性在對 CT 強度添加高斯噪聲後，其分佈發生了更顯著的變化，這與其解釋為反映數據噪聲相符。總之，我們的結果表明深度證據學習是一種有前途的方法，它可以在放射治療劑量預測中賦予深度學習模型統計穩健性。為了增強其臨床相關性，我們展示了如何使用這樣的模型來構造預測劑量體積直方圖的置信區間。
...
摘要：人工智慧（AI）的普遍整合在涉及 AI 啟用系統的事故中引入了責任和問責制的複雜挑戰。這些系統的互連性、AI 引發事件的倫理問題，加上 AI 技術的不確定性和缺乏相應法規，使得傳統責任歸屬具有挑戰性。為此，本研究提出了一種計算反思平衡（CRE）方法，為所有利害關係人建立一個連貫且在道德上可接受的責任歸屬架構。計算方法提供了一種結構化的分析，克服了概念方法在處理動態和多方面的場景時所存在的限制，展示了責任歸屬過程中該架構的可解釋性、相干性和適應性特性。我們探討了與平衡計算中索賠相關的初始激活層級的關鍵作用。使用 AI 輔助醫療決策支援系統作為案例研究，我們說明了不同的初始化如何導致不同的責任分配。該架構提供了對 AI 引發事件中問責制的寶貴見解，透過持續監控、修訂和反思，促進了一個永續且有韌性的系統的發展。
...
摘要：對於在開放世界中安全地部署機器學習模型而言，對分布外 (OOD) 樣本的穩健性至關重要。最近的研究重點在於設計評分函數以量化 OOD 不確定性。由於 OOD 樣本通常在前期不可用，因此為這些評分函數設定適當的 OOD 檢測閾值是一項挑戰。通常，閾值設定為達到所需的真陽性率 (TPR)，例如 95% TPR。然而，這可能導致非常高的假陽性率 (FPR)，範圍從 60% 到 96%，如 Open-OOD 基準中所觀察到的。在安全關鍵的實際應用中，例如醫療診斷，在動態處理各種 OOD 樣本時控制 FPR 至關重要。為了應對這些挑戰，我們提出了一個以數學為基礎的 OOD 檢測框架，該框架利用專家反饋來動態「安全地」更新閾值。我們提供了理論結果，表明它保證始終滿足 FPR 約束，同時最大限度地減少人工反饋的使用。我們框架的另一個關鍵特徵是它可以與任何用於 OOD 不確定性量化的評分函數配合使用。在合成和基準 OOD 資料集上對我們系統的經驗評估表明，我們的演算法可以在最大化 TPR 的同時將 FPR 維持在最多 5%。
...
摘要：乳房攝影檢查影像上的惡性病灶偵測對於早期乳癌診斷至關重要。在臨床實務中，影像會從兩個不同角度取得，而放射科醫師可以充分利用來自兩個視角的資訊，同時定位同一個病灶。然而，對於自動偵測方法而言，這樣的資訊融合仍然是一項挑戰。在本文中，我們提出一個稱為 MAMM-Net 的新模型，它允許同時處理兩個乳房攝影視角，不僅在物件層級共享資訊（如現有研究所示），也同時在特徵層級共享資訊。MAMM-Net 的關鍵元件是融合層，它基於可變形注意力，旨在提高偵測精確度，同時保持高召回率。我們的實驗顯示，與先前的最先進模型相比，在公共 DDSM 資料集上具有優異的效能，同時引入了新的有用特徵，例如像素級病灶註解和病灶惡性分類。
...
摘要：本報告列出 13 種功能性條件，以計算方式兌現，據稱這些條件構成有意識的價值經驗。這些條件從現有的經驗和理論文獻中提取，包括動物知覺、醫療疾病、麻醉劑、哲學、演化、神經科學和人工智慧。
...
摘要：大型語言模型 (LLM) 用於協助臨床醫師，已引起極大的關注。現有作品主要採用封閉式問答任務，並提供選項供評估。然而，在實際的臨床場景中，許多臨床決策（例如治療建議）都涉及回答沒有預設選項的開放式問題。同時，現有研究主要使用準確度來評估模型效能。在本文中，我們全面評比醫療保健中的各種 LLM，以清楚了解其優缺點。我們的評比包含七項任務和十三個資料集，涵蓋醫療語言生成、理解和推理。我們在零次學習和少次學習 (即 1、3、5 次學習) 設定下，對現有的十六個醫療保健 LLM 進行詳細評估。我們報告了五項指標的結果（即匹配、忠實度、全面性、概括性和穩健性），這些指標對於獲得臨床使用者的信任至關重要。我們進一步邀請醫學專家進行人工評估。
...
摘要：社群媒體平台在全球廣泛採用，不僅提升了
使用者的連結性和溝通，也成為傳播健康相關資訊的重要管道，進而將社群
媒體資料建立為公共衛生研究中無價的有機資料資源。
電子菸或電子香菸使用在美國和其他國家流行激增，導致電子菸和電子菸相關
肺部傷害 (EVALI) 爆發，導致 2019 年住院和死亡，凸顯出了解電子菸行為和發展有效
戒菸策略的迫切性。在本研究中，我們從 Reddit 上的一個電子菸子社群中提取一個樣本資料集，以分析使用者的戒菸意圖。
利用大型語言模型，包括最新的 GPT-4 和傳統的 BERT 基於語言模型的句子級別戒菸意圖
預測任務，本研究將這些模型的結果與人工標註進行比較。值得注意的是，與人工評估者相比，GPT-4 模型在遵守標註準則和流程方面表現出優異的一致性，展示了檢測人工評估者可能忽視的細微使用者戒菸意圖的先進能力。這些初步發現強調了 GPT-4 在增強社群媒體資料分析的準確性和可靠性方面的潛力，特別是在識別可能逃過人工檢測的使用者細微意圖方面。
...
摘要：大型語言模型 (LLM) 在處理臨床文字摘要任務方面展現出令人期待的潛力。在此研究中，我們證明一個小型開源 LLM 可以經過有效訓練，從門診病患與醫師對話中產生高品質的臨床筆記。我們透過針對 LLaMA-2 130 億個參數模型進行全面的領域和任務特定適應流程來達成此目標。此流程包含持續的預訓練、監督微調以及來自 AI 和人類回饋的強化學習。我們引進一種增強的方法，稱為 DistillDirect，用於執行策略強化學習，由 Gemini Pro 擔任教師模型。我們產生的模型 LLaMA-Clinic 能夠產生品質與醫師撰寫的臨床筆記相當的臨床筆記。在盲測醫師閱讀者研究中，大多數 (90.4%) 的個別評估將 LLaMA-Clinic 產生的筆記評為「可接受」或更高，涵蓋所有三個標準：真實世界準備度、完整度和準確度。值得注意的是，在更具挑戰性的「評估和計畫」部分，LLaMA-Clinic 在真實世界準備度方面得分較高 (4.2/5)，高於醫師撰寫的筆記 (4.1/5)。此外，我們在公共臨床筆記資料集中（例如 ACI-BENCH）發現了警告。我們強調未來臨床筆記產生任務中的關鍵考量，強調預先定義最佳實務筆記格式的重要性。總的來說，我們的研究證明了訓練較小、開源的 LLM 以協助臨床文件處理的潛力和可行性，並利用醫療保健機構取得病歷和領域專業知識的優勢。我們已公開我們新建立的合成診所對話筆記資料集和醫師回饋資料集，以促進此領域的未來研究。
...
摘要：最近，基於深度學習的語言模型大幅提升了文本轉 SQL 的任務，在醫學領域中擷取病歷記錄方面有令人期待的應用。此類應用中一個顯著的挑戰是辨別無法回答的查詢。透過微調模型，我們展示了將病歷查詢轉換為 SQL 查詢的可行性。此外，我們引入了一種基於熵的方法來識別和過濾無法回答的結果。我們進一步透過基於對數機率分配過濾低信心的 SQL 來提升結果品質，同時透過在實際資料庫上執行查詢來減輕語法和架構錯誤。我們透過實驗驗證，我們的模型可以過濾無法回答的問題，即使無法存取模型的參數，也能廣泛使用，並且可以有效地用於實務中。
...
摘要：大型語言模型 (LLM) 與醫療保健的整合承諾將轉變醫療診斷、研究和患者照護。然而，醫療 LLM 的進展面臨障礙，例如複雜的訓練需求、嚴格的評估要求，以及限制學術探索的專有模型的支配地位。透明、全面的 LLM 資源存取對於推進該領域、促進可複製性，並鼓勵醫療保健 AI 的創新至關重要。我們提出 Hippocrates，一個專門為醫療領域開發的開源 LLM 框架。與之前的努力截然不同，它提供對其訓練資料集、程式碼庫、檢查點和評估協定的無限制存取。這種開放式方法旨在激勵協作研究，讓社群可以在透明的生態系統中建立、改進和嚴格評估醫療 LLM。此外，我們介紹 Hippo，一個針對醫療領域量身打造的 7B 模型系列，透過持續預訓練、指令調整，以及來自人類和 AI 回饋的強化學習，從 Mistral 和 LLaMA2 微調而成。我們的模型以大幅度優於現有的開放式醫療 LLM 模型，甚至超越具有 70B 參數的模型。透過 Hippocrates，我們渴望釋放 LLM 的全部潛力，不僅促進醫療知識和患者照護，也民主化醫療保健中 AI 研究的益處，讓它們在全球各地都能使用。
...
摘要：使用生成模型的弱监督医学影像分割 (MIS) 对于临床诊断至关重要。然而，分割结果的准确性通常受到监督不足和医学影像的复杂性限制。现有模型也仅提供单一结果，不允许测量不确定性。在本文中，我们介绍了 DiffSeg，这是一种基于扩散差的皮肤病变分割模型，它利用扩散模型原理从具有不同语义信息的图像中提取基于噪声的特征。通过辨别这些噪声特征之间的差异，该模型识别出患病区域。此外，它的多输出能力模仿了医生的注释行为，促进了分割结果一致性和模糊性的可视化。此外，它使用广义能量距离 (GED) 量化输出不确定性，有助于医生解释和决策。最后，该模型通过密集条件随机场 (DenseCRF) 算法整合输出，通过考虑像素间相关性来细化分割边界，从而提高准确性并优化分割结果。我们在 ISIC 2018 Challenge 数据集上展示了 DiffSeg 的有效性，其性能优于最先进的基于 U-Net 的方法。
...
摘要：自動視網膜層分段與醫學影像，例如光學相干斷層掃描 (OCT) 影像，是診斷眼科疾病的重要工具。然而，由於影像中對比度低和血流雜訊，要實現精確分段具有挑戰性。此外，演算法應輕量化，才能部署於實際臨床應用。因此，需要設計一個輕量化網路，以實現視網膜層分段的高效能。在本文中，我們提出 LightReSeg，用於視網膜層分段，可應用於 OCT 影像。具體來說，我們的做法遵循編碼器-解碼器結構，其中編碼器部分採用多尺度特徵提取和 Transformer 區塊，以充分利用所有尺度的特徵圖的語義資訊，並使特徵具有更好的全局推理能力，而解碼器部分，我們設計了一個多尺度非對稱注意力 (MAA) 模組，以保留每個編碼器尺度的語義資訊。實驗表明，與目前最先進的方法 TransUnet 相比，我們的做法在我們收集的資料集和另外兩個公開資料集上實現了更好的分段效能，只有 330 萬個參數，而 TransUnet 有 10570 萬個參數。
...
摘要：儘管深度學習在醫學影像分析中取得顯著的成功，
由於缺乏用於監督的高品質標籤影像，醫學影像分割仍然具有挑戰性。此外，自然影像和醫學影像之間的顯著領域差距，特別是超音波影像，阻礙了在自然影像上訓練的模型微調到手邊的任務。在這項工作中，我們解決了分割模型在低資料模式下的效能下降問題，並提出了一種無提示分割方法，利用分割基礎模型分割抽象形狀的能力。我們透過我們新穎的提示點生成演算法來做到這一點，該演算法使用粗略的語意分割遮罩作為輸入，並將零次提示可用的基礎模型作為最佳化目標。我們在超音波影像的分割發現任務（病理異常）上展示了我們的方法。我們的方法的優點在小規模肌肉骨骼超音波影像資料集的低資料模式實驗中得到不同程度的驗證，隨著訓練集大小的減少，效能提升幅度越大。
...
摘要：電子健康紀錄（EHR）儘管對醫療保健從業者來說是一大福音，但每天都變得越來越複雜和冗長。在這些冗長的電子病歷中篩選既費力又會成為醫患互動中的一個負擔。已經提出了一些方法來幫助緩解這個普遍存在的問題，無論是通過摘要還是分段，然而，過去只有少數方法真正有幫助。隨著自動化方法的興起，機器學習 (ML) 在解決識別電子病歷中相關部分的任務中顯示出了前景。然而，大多數機器學習方法依賴於標記數據，這在醫療保健中很難獲得。另一方面，大型語言模型 (LLM) 在自然語言處理 (NLP) 中表現出了令人印象深刻的壯舉，而且是以零次學習的方式進行的，即沒有任何標記數據。為此，我們建議使用 LLM 來識別相關的章節標題。我們發現 GPT-4 可以有效地解決零次學習和少次學習設置中的任務，並且比最先進的方法分段效果好得多。此外，我們還註釋了一個更難的現實世界數據集，並發現 GPT-4 難以表現良好，這暗示了進一步的研究和更困難的基準。
...
摘要：大型語言模型 (LLM) 中的提示洩漏會造成重大的安全和隱私威脅，特別是在檢索增強生成 (RAG) 系統中。然而，多輪 LLM 互動中的洩漏以及緩解策略尚未以標準化方式進行研究。本文探討了 LLM 在 4 個不同領域和 10 個封閉和開放原始碼 LLM 中針對提示洩漏的漏洞。我們獨特的多輪威脅模型利用了 LLM 的阿諛奉承效應，我們的分析剖析了 LLM 回應中的任務指令和知識洩漏。在多輪設定中，我們的威脅模型將平均攻擊成功率 (ASR) 提高到 86.2%，包括 GPT-4 和 claude-1.3 的 99% 洩漏。我們發現，像 Gemini 這樣的某些黑盒 LLM 在不同領域表現出不同的洩漏敏感性 - 與醫療領域相比，它們更有可能在新聞領域洩漏上下文知識。我們的實驗測量了 6 種黑盒防禦策略的具體效果，包括 RAG 場景中的查詢重寫器。我們提出的多層防禦組合對黑盒 LLM 仍有 5.3% 的 ASR，這表示 LLM 安全研究有待加強和未來的方向。
...
摘要：工業系統中的異常偵測對於預防設備故障、確保風險識別，以及維持整體系統效率至關重要。傳統監控方法通常依賴於固定閾值和經驗法則，這些方法可能不夠靈敏，無法偵測系統健康狀況的細微變化，並預測即將發生的故障。為了解決這個限制，本文提出一個新的基於注意力的卷積自編碼器 (ABCD)，用於風險偵測，並將風險值映射到維護計畫中。ABCD 從真實世界工業冷卻系統的歷史資料中學習導電性的正常行為，並重建輸入資料，識別出偏離預期模式的異常。此架構還採用校正技術來確保其預測的可靠性。評估結果表明，與沒有注意力的情況相比，ABCD 中的注意力機制使效能提升了 57.4%，並將誤報減少了 9.37%。此方法可以有效偵測風險，風險優先順序映射到維護中，為冷卻系統設計人員和服務人員提供有價值的見解。0.03% 的校正誤差表示模型經過良好校正，並增強了模型的可信度，進而能夠針對維護策略做出明智的決策。
...
摘要：序列建模是跨越各種領域中至關重要的一個區域，包括自然語言處理 (NLP)、語音辨識、時間序列預測、音樂生成和生物資訊學。遞迴神經網路 (RNN) 和長短期記憶網路 (LSTM) 在歷史上一直主導序列建模任務，例如機器翻譯、命名實體辨識 (NER) 等。然而，由於Transformer的效能優異，因此推動了此範例的轉變。然而，Transformer有 $O(N^2)$ 注意力複雜度以及處理歸納偏誤的挑戰。已經提出多種變異來解決這些問題，這些變異使用頻譜網路或卷積，並在各種任務中表現良好。然而，它們在處理長序列時仍有困難。狀態空間模型 (SSM) 已成為此脈絡中序列建模範例中很有前途的替代方案，特別是隨著 S4 及其變體（例如 S4nd、Hippo、Hyena、Diagnol State Spaces (DSS)、Gated State Spaces (GSS)、Linear Recurrent Unit (LRU)、Liquid-S4、Mamba 等）的出現。在此調查中，我們根據三個範例對基礎 SSM 進行分類，即閘控架構、結構架構和遞迴架構。此調查也重點介紹 SSM 在各個領域的各種應用，例如視覺、影片、音訊、語音、語言（特別是長序列建模）、醫療（包括基因組學）、化學（例如藥物設計）、推薦系統和時間序列分析，包括表格資料。此外，我們整合了 SSM 在基準資料集上的效能，例如 Long Range Arena (LRA)、WikiText、Glue、Pile、ImageNet、Kinetics-400、sstv2，以及影片資料集，例如 Breakfast、COIN、LVU 和各種時間序列資料集。Mamba-360 工作的專案網頁可在這個網頁上取得。\url{https://github.com/badripatro/mamba360}。
...
摘要：<paragraph>雖然融合多視角乳房攝影的資訊在提高乳癌偵測準確度上扮演重要的角色，但開發基於多視角乳房攝影的電腦輔助診斷 (CAD) 計畫仍面臨挑戰，且目前尚未有此類 CAD 計畫用於臨床實務。為了克服這些挑戰，我們探討一種基於對比語言影像預訓練 (CLIP) 的新方法，這在各種醫學影像任務中引起了興趣。透過解決 (1) 有效調整單視角 CLIP 以進行多視角特徵融合，以及 (2) 有效微調這個參數密集的模型，且樣本和運算資源有限的挑戰，我們引入了 Mammo-CLIP，這是第一個處理多視角乳房攝影和對應簡短文字的多模態架構。Mammo-CLIP 使用早期特徵融合策略，以學習從左右乳房的 CC 和 MLO 視角取得的四張乳房攝影中，多視角之間的關係。為了提升學習效率，將即插即用適配器加入 CLIP 影像和文字編碼器中，以微調參數，並將更新限制在約 1% 的參數。對於架構評估，我們回溯性地組裝了兩個資料集。第一個資料集包含 470 個惡性案例和 479 個良性案例，用於少樣本微調，以及透過 5 倍交叉驗證對提議的 Mammo-CLIP 進行內部評估。第二個資料集包含 60 個惡性案例和 294 個良性案例，用於測試 Mammo-CLIP 的泛化性。研究結果顯示，Mammo-CLIP 在兩個資料集上的 AUC 都優於最先進的跨視角轉換器 (0.841 對 0.817，0.837 對 0.807)。它也比先前的兩種基於 CLIP 的方法高出 20.3% 和 14.3%。這項研究突顯了應用微調視覺語言模型，以開發下一代基於影像和文字的乳癌 CAD 計畫的潛力。</paragraph>
...
摘要：大型語言模型，例如 GPT-4 和 Med-PaLM，在臨床任務上表現令人印象深刻；然而，它們需要存取運算資源，是閉源的，而且無法部署在裝置上。中型模型，例如 BioGPT-large、BioMedLM、LLaMA 2 和 Mistral 7B 避免了這些缺點，但它們在臨床任務上的能力尚未得到充分研究。為了幫助評估它們在臨床上的使用潛力，並幫助研究人員決定應使用哪種模型，我們比較了它們在兩個臨床問答 (QA) 任務上的表現：MedQA 和消費者查詢回答。我們發現 Mistral 7B 是表現最好的模型，在所有基準測試中獲勝，並且優於專門針對生物醫學領域訓練的模型。儘管 Mistral 7B 的 MedQA 分數為 63.0%，接近原始的 Med-PaLM，而且它通常可以對消費者健康查詢產生合理的回應，但仍有改進空間。本研究首次對開放原始碼中型模型在臨床任務上的表現進行了正面評估。
...
摘要：反射式共軛顯微鏡 (RCM) 是一種非侵入性影像技術，用於生物醫學研究和臨床皮膚科。它提供皮膚和表層組織的虛擬高解析度影像，減少了進行實體切片的需要。RCM 使用雷射光源照射組織，擷取反射光以產生各種深度微觀結構的詳細影像。最近的研究探討了 AI 和機器學習，特別是 CNN，用於分析 RCM 影像。我們的研究提出了一個基於紋理特徵的分割策略，以識別臨床顯著區域，讓皮膚科醫師能夠有效解讀影像並提升診斷信心。這種方法有望推進皮膚科診斷和治療。
...
摘要：由於基礎設施的可及性有限，遠端關鍵基礎設施的健康監控是一項複雜且昂貴的活動。檢查無人機是無所不在的資產，透過改善可及性來增強關鍵基礎設施的可靠性。然而，由於惡劣的操作環境，監控其健康狀況以確保檢查操作成功至關重要。電池是決定檢查無人機整體可靠性的關鍵組件，並透過適當的健康管理方法，有助於可靠且穩健的檢查。在此背景下，本文提出了一種新的混合機率方法，用於預測 Li-Po 電池的電池放電結束 (EOD) 電壓。這種混合化是在錯誤修正組態中實現的，它結合了基於物理的放電和機率錯誤修正模型，以量化隨機和認識論的不確定性。混合機率方法的效能經過實證評估，評估資料集包含在不同負載條件下的 EOD 電壓。該資料集來自在不同飛行中操作的實際檢查無人機，重點是離岸風力渦輪機檢查。所提出的方法已使用不同的機率方法進行測試，並顯示出與最佳機率方法相比，機率準確度提高了 14.8%。此外，隨機和認識論的不確定性提供了穩健的估計，以增強對電池健康狀態的診斷。
...
摘要：<paragraph>我們提出一個基於機器學習的異常偵測產品，AI 偵測與回應 (AIDR)，可以即時監控沃爾瑪的業務和系統健康狀況。在超過 3 個月的驗證期間，此產品從超過 3000 個模型提供預測給超過 25 個應用程式、平台和營運團隊，涵蓋 63% 的重大事件，並將平均偵測時間 (MTTD) 縮短超過 7 分鐘。與先前的異常偵測方法不同，我們的解決方案利用統計、機器學習和深度學習模型，同時持續納入基於規則的靜態閥值，以納入特定領域的知識。單變數和多變數機器學習模型皆透過分散式服務進行部署和維護，以確保可擴充性和高可用性。AIDR 有一個回饋迴路，可以透過漂移偵測演算法和客戶回饋的組合評估模型品質。它還提供自我引導功能和可自訂性。AIDR 已在各種內部團隊中獲得成功，偵測時間縮短，且誤報比先前的做法更少。隨著我們向前邁進，我們的目標是擴展事件涵蓋範圍和預防措施，減少雜訊，並進一步與根本原因建議 (RCR) 整合，以實現端到端的 AIDR 體驗。</paragraph>
...
摘要：人工智能應用已在疫情管理中展現潛力，且已廣泛用於協助辨識、分類和診斷醫學影像。為因應猴痘 (Mpox) 全球爆發，HeHealth.ai 團隊運用現有工具篩檢性傳染疾病，藉由人工智能方法開發出針對有症狀 Mpox 的數位篩檢測試。在 Mpox 全球爆發之前，該團隊開發了一款智慧型手機應用程式，應用程式使用者可以使用自己的智慧型手機相機拍攝自己的陰莖照片，以篩檢有症狀的性傳染病。人工智能模型最初是使用 5000 個案例開發，並使用改良的卷積神經網路來輸出視覺可診斷陰莖病理的預測分數，包括梅毒、單純疱疹病毒和人類乳突病毒。從 2022 年 6 月到 2022 年 10 月，總計約有 22,000 名使用者下載 HeHealth 應用程式，且約有 21,000 張影像已使用 HeHealth AI 技術進行分析。接著，我們從 2022 年 7 月開始進行形成性研究、利害關係人參與、快速統合影像、驗證研究以及工具實作。從 2022 年 7 月到 2022 年 10 月，總計有 1000 張與 Mpox 相關的影像用於訓練 Mpox 症狀檢查工具。我們的數位症狀檢查工具顯示出 87% 的準確度可判定 Mpox，以及 90% 的準確度可排除有症狀的 Mpox。已識別出的數個障礙包括應用程式使用者資料隱私和安全問題、訓練 AI 工具的初始資料不足，以及輸入資料的潛在概括性。我們提出數項建議，以協助其他人在緊急情況下啟動類似專案，包括讓廣泛的利害關係人參與、組成跨領域團隊、優先考量實用主義，以及大資料實際上是由小資料組成這個概念。
...
摘要：臨床試驗配對是找出患者可能有資格參加的試驗的任務。通常，這項任務需要大量人力，且需要詳細驗證病患的電子健康紀錄 (EHR)，並對照臨床試驗的嚴格納入和排除標準。這個程序手動且耗時，而且難以擴大規模，導致許多患者錯失潛在的治療選項。大型語言模型 (LLM) 的最新進展讓自動化病患試驗配對成為可能，這在多項同時進行的研究中得到證明。然而，目前的做法僅限於受限且通常是合成式的資料集，無法充分反映真實世界醫療資料中遇到的複雜性。在本研究中，我們提出第一個端到端的臨床試驗配對大規模實證評估，使用真實世界的 EHR。我們的研究展示了 LLM 能準確地將病患與適當的臨床試驗配對的能力。我們使用專有的 LLM 進行實驗，包括 GPT-4 和 GPT-3.5，以及我們自訂微調的模型 OncoLLM，並顯示 OncoLLM 儘管規模小很多，不僅優於 GPT-3.5，還達到合格醫師的表現。所有實驗都針對真實世界的 EHR 進行，其中包括臨床筆記和來自美國單一癌症中心的可用臨床試驗。
...
摘要：在都市環境中，綠地空間的稀缺是一個重大的挑戰。這會造成多重的不良影響，衝擊市民的健康和福祉。小規模的介入，例如袖珍公園，是一個可行的解決方案，但會帶來多重限制，包括特定區域的設計和實施。在這項研究中，我們利用生成式 AI 的能力進行多尺度的介入規劃，專注於基於自然的解決方案。透過利用影像轉影像和影像修復演算法，我們提出了一種方法來解決都市地區的綠地空間不足問題。我們專注於塞薩洛尼基的兩條小巷，那裡缺乏綠意，我們展示了我們的方法在視覺化 NBS 介入方面的效力。我們的發現強調了新興技術在塑造未來都市介入規劃流程方面具有變革的潛力。
...
摘要：在應用於臨床領域的自然語言處理中，利用大型語言模型已成為臨床筆記中錯誤偵測和修正的有前途途徑，這項任務知識密集，且標註資料稀少。本文提出 MedReAct'N'MedReFlex，它利用一組四個基於 LLM 的醫療代理。MedReAct 代理透過觀察、分析和採取行動來啟動流程，產生軌跡以引導搜尋，目標是找出臨床筆記中的潛在錯誤。隨後，MedEval 代理使用五個評估器來評估目標錯誤和建議的修正。在 MedReAct 的行動證明不足的情況下，MedReFlex 代理會介入，從事反思分析並提出替代策略。最後，MedFinalParser 代理會格式化最終輸出，在確保錯誤修正過程的完整性的同時，保留原始樣式。我們方法的一個核心組成部分是基於我們的 ClinicalCorp 語料庫的 RAG 管道。在其他包含臨床指南和資訊的眾所周知來源中，我們預處理並釋出開源 MedWiki 資料集，以供臨床 RAG 應用。我們的結果證明了我們的 RAG 方法的核心作用，並透過 MedReAct'N'MedReFlex 架構利用了 ClinicalCorp。它在 MEDIQA-CORR 2024 最終排行榜上獲得第九名。
...
摘要：反事實範例對於探索機器學習模型的決策邊界和確定特徵歸因很有用。我們如何應用基於反事實的方法來分析和解釋 LLM？我們找出以下關鍵挑戰。首先，產生的文字反事實應對使用者而言有意義且可讀，因此可以在心理上進行比較以得出結論。其次，為了使解決方案可擴展到長篇文字，使用者應具備工具，以便根據各種粒度層級的擾動建立反事實批次，並互動分析結果。在本文中，我們解決上述挑戰，並貢獻 1) 一種新的演算法，用於透過移除和替換不同粒度中的文字片段來產生批次的完整且有意義的文字反事實，以及 2) LLM 分析器，這是一個互動式視覺化工具，可協助使用者透過互動式檢查和彙總有意義的反事實來了解 LLM 的行為。我們透過使用來自醫療、法律、金融、教育和新聞資料集的 1,000 個樣本，根據其產生的反事實的語法正確性來評估所提出的演算法。在我們的實驗中，97.2% 的反事實在語法上是正確的。透過使用案例、使用者研究和專家的回饋，我們展示了所提出的互動式視覺化工具的實用性和可用性。
...
摘要：本研究的目的是預測一個人是否會完成戒毒和戒酒的復健計畫，以及一個人參與的次數。本研究的基礎是從物質濫用和心理健康服務管理局 (SAMHSA) 取得的人口統計資料，這些資料來自奧克拉荷馬州戒毒和戒酒復健中心，包含入院和出院資料。人口統計資料高度分類，導致使用二進位編碼，並使用各種公平性措施來減輕九個人口統計變數的偏差。使用支援向量機在各種參數範圍內比較線性、多項式、sigmoid 和徑向基函數等核方法，以找出最佳值。然後將這些方法與決策樹、隨機森林和神經網路等方法進行比較。使用分類資料的合成少數過採樣技術標稱 (SMOTEN) 來平衡資料，並對遺失資料進行插補。然後對九個偏差變數進行交叉分析以減輕偏差，並整合雙重和三重交互作用，以使用機率來查看最差情況比率公平性緩解。在二元和多類別情境中，都探討了不同的影響、統計同等性差異、條件統計同等性比率、人口同等性、人口同等性比率、均等機會、均等機會比率、均等機會和均等機會比率。
...
摘要：<paragraph>醫療視覺語言預訓練 (Med-VLP) 建立了醫療影像的視覺內容與相關文字描述之間的連結。現有的 Med-VLP 方法主要關注描繪單一身體部位的 2D 影像，特別是胸部 X 光。在本文中，我們透過使用 CT 影像和報告的多模態資料集，將 Med-VLP 的範圍擴展到涵蓋 3D 影像，特別針對全身情境。與 2D 對應項相比，3D VLP 必須有效擷取 3D 影像中顯著稀疏表示中的基本語義。在本文中，我們介紹了 CT-GLIP（使用 CT 掃描的基礎語言影像預訓練），這是一種創新的方法，用於建構器官層級的影像文字對，以增強多模態對比學習，將基礎視覺特徵與精確的診斷文字對齊。此外，我們還開發了一個異常字典，以使用不同的對比對增強對比學習。我們的模型訓練於一個多模態 CT 資料集，該資料集包含來自 104 個器官的 17,702 名患者的 44,011 個器官層級視覺文字對，顯示它可以使用自然語言以零次學習的方式識別器官和異常。CT-GLIP 的效能經過一個包含 1,130 名患者的獨立測試集驗證，著重於 7 個器官中最頻繁的 16 種異常。實驗結果顯示，我們的模型在零次學習和微調情境中，使用 CNN 和 ViT 架構，都優於標準 CLIP 架構。</paragraph>
...
摘要：多模態醫學影像在臨床診斷和研究中扮演著關鍵角色，因為它結合了來自各種影像模式的資訊，以提供對基礎病理學的更全面了解。最近，基於深度學習的多模態融合技術已成為改進醫學影像分類的強大工具。本篇評論提供了對基於深度學習的多模態融合在醫學分類任務中的發展的深入分析。我們探討了流行臨床模式之間的互補關係，並概述了多模態分類網路的三種主要融合架構：輸入融合、中間融合（包含單層融合、階層融合和基於注意力的融合）和輸出融合。透過評估這些融合技術的效能，我們深入了解不同網路架構在各種多模態融合場景和應用領域中的適用性。此外，我們深入探討與網路架構選擇、處理不完整多模態資料管理和多模態融合的潛在限制相關的挑戰。最後，我們重點介紹了基於 Transformer 的多模態融合技術的未來前景，並針對這個快速發展的領域提出未來研究建議。
...
摘要：改善醫療保健的未來，從更深入了解醫院當前的實際做法開始。這激勵了從患者資料中發現典型照護路徑的目標。透過分群，可以揭示出同質的照護路徑群組。以時間戳記事件序列表示的照護路徑分群的難處，在於定義語義上適當的指標和分群演算法。
  在本文中，我們將兩種針對時間序列開發的方法調整為時間序列：drop-DTW 指標和 DBA 方法，用於建構平均時間序列。然後將這些方法應用於分群演算法，以提出針對計時序列的原始且完善的分群演算法。
  此方法已在合成和實際使用案例中進行實驗和評估。
...
摘要：醫療視覺語言預訓練已成為一種有前途的方法，可用於學習醫療影像與文字的領域一般表徵。然而，目前利用醫療影像與文字之間的整體和局部對齊演算法可能會受到醫療資料中冗餘資訊的影響。為了解決這個問題，我們提出了一個針對胸部 X 光的基礎知識增強型醫療視覺語言預訓練 (GK-MVLP) 架構。在此架構中，醫療知識透過使用基於 Transformer 的基礎知識增強型模組，將知識基礎建立在適當的解剖區域上，以在解剖區域層級的視覺特徵和醫療知識的紋理特徵之間進行細緻對齊。GK-MVLP 的效能與最先進的技術相當，甚至超越了最先進的技術，包括下游胸部 X 光疾病分類、疾病定位、報告產生和醫療視覺問答任務。我們的結果顯示了納入基礎機制以消除偏誤和改善胸部 X 光影像與放射科報告之間對齊的優點。
...
摘要：近年來，從對話資料自動偵測憂鬱症已引起極大的興趣。由人類控制的虛擬代理人所進行的訪談，DAIC-WOZ 資料集已被廣泛用於這項任務。最近的研究報告指出，將訪談者的提示納入模型時，效能有所提升。在這項工作中，我們假設這種提升可能主要歸因於這些提示中存在的偏差，而非所提出的架構和方法。透過消融實驗和定性分析，我們發現使用訪談者提示的模型會專注於訪談中的特定區域，其中詢問了關於過去心理健康問題的經驗，並將這些問題用作區辨性捷徑來偵測憂鬱症患者。相反地，使用參與者回應的模型會從整個訪談中收集證據。最後，為了強調這種偏差的程度，我們透過刻意利用它來達到 0.90 的 F1 分數，這是迄今為止僅使用文字資訊在這個資料集上報告的最高結果。我們的發現強調在將訪談者的提示納入模型時需要謹慎，因為它們可能會無意間學會利用有針對性的提示，而不是學會描述真正顯示患者心理健康狀況的語言和行為。
...
摘要：基礎模型在推進醫療領域方面已變得極其有價值。
儘管它們很有前景，但戰略性地部署 LLM 以便在複雜的醫療任務中有效使用仍然是一個未解決的問題。我們的創新框架，醫療決策代理 (MDAgents) 旨在通過自動分配 LLM 的有效協作結構來解決這個差距。分配的單獨或小組協作結構會根據手邊的醫療任務的複雜性進行調整，模擬真實世界的醫療決策制定流程。我們使用最先進的 LLM 評估我們的框架和基準方法，並在多項具有挑戰性的醫療基準測試中進行評估：MedQA、MedMCQA、PubMedQA、DDXPlus、PMC-VQA、Path-VQA 和 MedVidQA，在需要理解多模式醫療推理的 7 個基準測試中取得了 5 項最佳表現。消融研究表明，MDAgents 在調整協作代理的數量以優化效率和準確性方面表現出色，展示了其在各種場景中的穩健性。我們還探討了小組共識的動態，深入了解協作代理如何在複雜的臨床團隊動態中表現。我們的程式碼可以在 https://github.com/mitmedialab/MDAgents 中找到。
...
摘要：鼻部細胞學是一種診斷鼻炎和過敏的新型且有效率的臨床技術，但由於細胞計數耗時，因此並未廣泛使用；這就是為什麼 AI 輔助計數可能是此技術擴散的轉捩點。在本文中，我們展示了第一個鼻細胞學領域影像的資料集：NCD（鼻細胞學資料集），旨在訓練和部署物件偵測模型，以在臨床實務中支援醫師和生物學家。鼻黏膜中細胞類型的實際分佈已被複製，從臨床病患的玻片中取樣影像，並手動註解每個在玻片上發現的細胞。對應的物件偵測任務呈現出與嚴重的類別不平衡相關的非平凡問題，涉及最稀有的細胞類型。這項工作透過提出一種新穎的機器學習方法來協助鼻黏膜細胞的自動偵測和分類，為一些公開挑戰做出貢獻：DETR 和 YOLO 模型在偵測細胞並正確分類方面表現良好，顯示出加速鼻科專家工作的巨大潛力。
...
摘要：<paragraph>合成資料生成提供了一個有希望的解決方案，透過生成真實的去識別化資料來提升電子健康紀錄 (EHR) 的效用。然而，現有的文獻主要關注合成健康資料的品質，忽略了下游預測中公平性的關鍵面向。因此，在合成 EHR 上訓練的模型因在目標任務中產生有偏見的結果而受到批評。這些偏見可能來自特徵之間的虛假關聯，或模型無法準確表示子群組。為了解決這些問題，我們提出了偏見轉換生成對抗網路 (Bt-GAN)，一種專門為醫療保健領域設計的基於 GAN 的合成資料產生器。為了應對虛假關聯 (i)，我們提出了一個資訊受限資料生成程序，使產生器能夠根據明確定義的演算法公平性概念學習公平的確定性轉換。為了克服捕捉精確子群組表示 (ii) 的挑戰，我們透過基於分數的加權抽樣來激勵產生器保留子群組密度。這種方法迫使產生器從資料流形中代表性不足的區域中學習。我們使用 MIMIC-III 資料庫進行了大量的實驗。我們的結果表明，Bt-GAN 達到了 SOTA 準確度，同時顯著改善了公平性並最小化了偏見放大。我們還進行了深入的可解釋性分析，以提供額外的證據來支持我們研究的有效性。總之，我們的研究引入了一種新穎且專業的方法來解決醫療保健領域中合成資料生成的限制。透過納入公平性考量並利用 GAN 等先進技術，我們為醫療保健應用中更可靠且無偏見的預測鋪平了道路。</paragraph>
...
摘要：这项工作是受深度神经网络的最新发展启发，特别是 Transformer 架构，它是 ChatGPT 等应用程序的基础，以及在移动设备上执行推理的需要。专注于新兴的 Transformer（特别是具有计算高效的 Swin 类架构的 Transformer）和基于 Transformer 的大型模型（例如 Stable Diffusion 和 LLM），我们观察到计算算子之间的布局转换会导致这些应用程序显著变慢。本文介绍了 SmartMem，这是一个用于消除大多数布局转换的综合框架，其思想是多个算子可以通过仔细选择布局和操作实现来使用相同的张量布局。我们的方法基于将算子分类为四组，并考虑算子之间的生产者-消费者边缘的组合。我们开发了一组用于搜索此类布局的方法。我们工作的另一个组成部分是为移动设备中常见的 2.5 维内存开发高效的内存布局。我们的实验结果表明，SmartMem 在 18 个不同的神经网络（包括 CNN、具有局部和全局注意力的 Transformer 以及 LLM）上优于 5 个最先进的 DNN 执行框架。特别是，与 DNNFusion 相比，SmartMem 平均提速 2.8 倍，并且平均比 TVM 和 MNN 分别提速 6.9 倍和 7.9 倍。
...
摘要：深度學習的興起在電腦視覺、自然語言處理和醫學影像等領域標誌著顯著進展，這主要通過為特定任務調整預訓練模型來實現。傳統的微調方法涉及調整所有參數，由於高計算和記憶體需求而面臨挑戰。這導致了參數高效微調 (PEFT) 技術的發展，該技術有選擇地更新參數以平衡計算效率和性能。本綜述探討了 PEFT 方法，對各種策略進行了詳細比較，重點介紹了跨越不同領域的應用，包括文本生成、醫學影像、蛋白質建模和語音合成。通過評估 PEFT 方法在降低計算負載、加快訓練和降低記憶體使用方面的有效性，本文有助於使深度學習更易於訪問和適應，促進其更廣泛的應用並鼓勵模型優化的創新。最終，本文旨在為 PEFT 不斷變化的格局提供見解，指導研究人員和從業者克服傳統微調方法的局限性。
...
摘要：從 RGB 影片中可以提取出微妙的週期性訊號，例如血容積脈搏和呼吸，這使得低成本的非接觸式健康監測成為可能。遠端脈搏估計的進展（或遠端光電容積描記術 (rPPG)）目前是由深度學習解決方案推動的。然而，現代方法是在具有來自接觸式 PPG 感測器實地真相的基準資料集上訓練和評估的。我們提出了第一個非對比無監督學習架構，用於訊號回歸，以減輕對標籤影片資料的需求。我們的架構在週期性和有限頻寬的最小假設下，直接從未標籤的影片中發現血容積脈搏。我們發現，鼓勵在正常生理頻寬限制和功率譜批次中的變異中產生稀疏功率譜就足以學習週期性訊號的視覺特徵。我們執行了第一個實驗，利用未標籤的影片資料（並非特別為 rPPG 建立）來訓練穩健的脈搏率估計器。鑑於有限的歸納偏誤，我們成功地將相同的方法應用於基於相機的呼吸，方法是改變目標訊號的頻寬限制。這表明該方法對於從不同領域中無監督學習頻寬限制的準週期性訊號而言已經夠通用。此外，我們表明該架構對於在單一受試者的未標籤影片上對模型進行微調是有效的，從而允許個性化和自適應訊號回歸器。
...
摘要：聯邦學習 (FL) 已成為一種重要的隱私保護技術，用於啟用機密臨床機器學習等用例。FL 通過聚合由擁有數據的遠端裝置訓練的模型來運作。因此，FL 能夠使用來自大量學習者的群眾外包數據訓練強大的全球模型，同時不損害其隱私。然而，在產生全球模型時，聚合伺服器是單點故障。此外，當數據在所有遠端裝置上不是獨立同分布 (非 IID 數據) 時，模型的效能會受到影響。這會導致聚合截然不同的模型，在某些情況下可能會將效能降低多達 50%。在本文中，我們尋求在保留 FL 優點的同時解決上述問題。我們提出多聯盟學習：一個分散式 FL 架構，旨在處理非 IID 數據。與傳統 FL 不同，多聯盟學習將並行維護多個模型 (而不是單一全球模型)，以在數據非 IID 時協助收斂。在轉移學習的幫助下，學習者可以收斂到較少的模型。為了增加適應性，學習者可以選擇從其同儕聚合哪些更新。
...
摘要：這項研究探討了一種基於大型語言模型 (LLM) 的新穎資料擴增方法，用於預測已停用的美國執照醫學考試 (USMLE) 多重選擇題 (MCQ) 在 BEA 2024 共享任務中的題目難度和反應時間。我們的做法是使用零次學習 LLM（Falcon、Meditron、Mistral）的答案擴增資料集，並使用基於六種替代特徵組合的轉換器模型。結果表明預測題目難度更具挑戰性。值得注意的是，我們表現最好的方法始終包含題目文字，並受益於 LLM 答案的多變性，突顯了 LLM 在改善醫學許可考試中自動化評估的潛力。我們提供我們的程式碼 https://github.com/ana-rogoz/BEA-2024。
...
摘要：電池健康監控和預測在電動交通時代至關重要，對安全、永續性和經濟層面有重大影響。現有研究通常專注於預測準確度，但往往忽略會阻礙技術在實際應用中部署的實際因素。在本文中，我們探討這些實際考量，並基於貝氏神經網路開發用於預測電池使用壽命的模型。我們的模型使用與電池健康相關的感測器資料，並針對模型的每個參數套用分佈，而非單一數值。這讓模型能夠捕捉電池健康的內在隨機性和不確定性，不僅能進行準確預測，還能量化不確定性。我們進行了一項實驗研究，並展示了我們提出的模型的有效性，預測錯誤率平均為 13.9%，而某些測試電池的錯誤率低至 2.9%。此外，所有預測都包含可量化的確定性，從電池的初始階段到中期階段，確定性提升了 66%。這項研究對電池技術具有實際價值，並有助於加速產業中技術的採用。
...
摘要：大型語言模型 (LLM) 的進步鼓勵其在醫療保健領域的採用，在該領域中，重要的臨床資訊通常包含在非結構化筆記中。癌症分期狀態可在臨床報告中取得，但需要自然語言處理才能從非結構化文字中提取狀態。隨著以臨床為導向的 LLM 的進步，在不投入大量精力訓練演算法的情況下，提取此類狀態是很有希望的。預先訓練的 LLM 的提示方法引發了模型的推理過程，例如思考鏈，可能有助於提高生成回應的可信度。使用自我一致性進一步改善模型效能，但通常會導致跨多個推理路徑產生不一致的生成。在本研究中，我們提出了一個整體推理方法，目的是提高模型生成的相容性。使用開放式臨床大型語言模型從真實世界的病理報告中確定病理癌症分期，我們表明整體推理方法能夠提高 LLM 在確定癌症分期時的相容性和效能，從而證明了在可靠性和可信度至關重要的臨床或其他領域使用這些模型的潛力。
...
摘要：人工智慧透過預測建模協助醫療專業人員，大幅轉變臨床決策制定。本研究探討醫療保健中 AI 應用對於公平性和可解釋性的關鍵需求，以確保不同患者人口統計資料的公平結果。透過專注於敗血症相關死亡率的預測建模，我們提出了一種方法，該方法學習效能最佳化的預測模型，然後採用遷移學習程序來產生具有更佳公平性的模型。我們的模型還引入了創新的基於置換的特徵重要性演算法，旨在闡明每個特徵在增強預測公平性方面的貢獻。與現有的可解釋性方法專注於解釋特徵對預測效能的貢獻不同，我們提出的方法獨特地彌補了理解每個特徵如何促進公平性的差距。鑑於敗血症顯著的死亡率及其在醫院死亡中所占的三分之一比例，這項進展至關重要。我們的模型不僅有助於識別和減輕預測模型中的偏差，還透過提高模型預測的透明度和公平性來促進醫療保健利害關係人之間的信任，從而促進更公平、更值得信賴的醫療保健服務。
...
摘要：帕金森氏症在全球排名第二的神經退化性疾病。本研究旨在開發一個利用混合現實功能追蹤和評估眼球運動的系統。在本文中，我們提出了一個醫療場景，並概述了一個應用程式的開發，該應用程式旨在透過混合現實技術擷取眼球追蹤訊號，以評估神經退化性疾病。此外，我們還介紹了一個從眼球注視分析中提取臨床相關特徵的管道，從醫療角度描述了所提出系統的能力。該研究涉及一組健康對照個體和患有帕金森氏症的患者，展示了所提出的技術在非侵入性監控眼球運動模式以診斷神經退化性疾病方面可行性和潛力。
臨床相關性 - 緊急需要開發一種非侵入性生物標記，用於帕金森氏症的準確檢測疾病發作。這將允許在最早的階段及時引入神經保護治療，並能夠持續監控干預結果。檢測眼球運動中細微變化的能力允許早期診斷，在出現更明顯的症狀之前提供重要的干預窗口。眼球追蹤提供客觀且可量化的生物標記，確保對疾病進展和認知功能進行可靠評估。使用混合現實眼鏡的眼球注視分析是無線的，方便在家庭和醫院環境中進行評估。該方法的優點是利用不需要額外專用附件的硬體，從而能夠透過個人眼鏡進行檢查。
...
摘要：視覺任務適應已被證明可用於使用可學習的專門層或代碼將預先訓練好的視覺轉換器 (ViT) 適應到一般下游視覺任務。然而，目前還沒有大規模基準來充分探討視覺任務適應對現實且重要的醫療領域的影響，特別是跨越不同的醫療視覺方式，例如彩色圖像、X 光和電腦斷層掃描。為了縮小這個差距，我們提出了 Med-VTAB，這是一個大規模的醫療視覺任務適應基準，包含 168 萬張針對不同器官、方式和適應方法的醫療影像。基於 Med-VTAB，我們探討了與可調整參數有關的醫療提示調整的縮放定律，以及使用非醫療/醫療預訓練權重進行醫療視覺適應的概括性。此外，我們研究了患者 ID 異常分佈對醫療視覺適應的影響，這是一個真實且具挑戰性的場景。此外，Med-VTAB 的結果表明，單一的預先訓練模型不足以進行醫療任務適應。因此，我們引入了 GMoE-Adapter，這是一種透過閘控混合專家適配器結合醫療和一般預訓練權重的創新方法，在醫療視覺任務適應中取得了最先進的成果。
...
摘要：憂鬱症是現今社會重要的議題。根據世界衛生組織 (WHO) 的資料，2023 年有超過 2.8 億人正與憂鬱症奮戰。這是一個龐大的數字；若不認真看待，這些數字將會快速增加。約有 48.9 億人是社群媒體使用者。人們在 Twitter、Facebook、Reddit、Instagram 等平台上表達自己的感受與情緒。這些平台包含有價值的資訊，可用於研究目的。已在各個社群媒體平台上進行大量研究。然而，這些研究仍存在一定的限制。特別是，先前的研究僅專注於偵測推文中的憂鬱症和憂鬱症的嚴重程度。此外，在資料集標籤中存在不準確的情況。在這項研究中，使用 Twitter 資料庫中的推文預測五種類型的憂鬱症（雙極性、重度、精神病性、非典型和產後），其基礎為詞彙標籤。可解釋的 AI 用於提供推理，方法是重點標示出代表憂鬱症類型的推文部分。使用 Transformer 的雙向編碼器表示法 (BERT) 進行特徵萃取和訓練。使用機器學習和深度學習方法訓練模型。BERT 模型呈現最令人滿意的結果，達到 0.96 的整體準確度。
...
摘要：深度學習顯著地改變了醫學影像和放射線學領域，讓病理學可以在醫學影像中進行辨識，包括電腦斷層掃描 (CT) 和 X 光掃描。然而，深度學習模型的效能，特別是在分割任務中，通常受到廣泛註解資料集需求的限制。為了應對這項挑戰，透過可解釋 AI 和反事實解釋的產生，探索了弱監督語意分割的能力。本研究的範圍是開發一種新穎的反事實內插方法 (COIN)，透過使用生成模型將預測的分類標籤從異常翻轉為正常。例如，如果分類器將輸入醫學影像 X 視為異常，表示存在病理，生成模型的目標是內插異常區域，從而逆轉分類器的原始預測標籤。此方法使我們能夠產生病理的精確分割，而不需要依賴於預先存在的分割遮罩。至關重要的是，使用影像層級標籤，這比建立詳細的分割遮罩容易得多。該方法的有效性透過分割愛沙尼亞塔爾圖大學醫院取得的 CT 影像中的合成目標和實際腎臟腫瘤來證明。研究結果表明，COIN 遠遠超過已建立的歸因方法，例如 RISE、ScoreCAM 和 LayerCAM，以及 Singla 等人提出的另一種反事實解釋方法。此證據表明，COIN 是一種很有前景的 CT 影像中腫瘤語意分割方法，並在醫療保健中讓深度學習應用程式更易於使用和更有效率，而標註資料很少。
...
摘要：影像重建是每種醫學影像方法的必要步驟，
包括光聲斷層掃描 (PAT)，這是一種有前途的影像模式，
結合了超音波和光學影像方法的優點。使用傳統方法重建 PAT 影像會產生
粗糙的偽影，特別是當直接應用於稀疏的 PAT 資料時。近年來，生成對抗網路 (GAN) 已展現出
在影像生成和轉換方面的強大效能，使其成為應用於重建任務的明智選擇。在本研究中，我們提出了一個端對端的
方法，稱為 DensePANet，以解決從稀疏資料重建 PAT 影像的問題。所提出的模型在其產生器中採用 UNet 的新穎修改，稱為 FD-UNet++，它大幅改善了重建
效能。我們在各種體內和模擬資料集上評估此方法。定量和定性結果顯示我們的模型優於其他流行的深度學習技術。
...
摘要：本研究提出一個基於 Transformer 架構和自注意力機制的 Multitrans 多模式融合框架。此架構結合非對比電腦斷層掃描 (NCCT) 影像和接受中風治療患者的出院診斷報告的研究，使用基於 Transformer 架構方法的各種方法來預測中風治療的功能性結果。結果顯示，單模式文字分類的表現顯著優於單模式影像分類，但多模式組合的效果優於任何單一模式。儘管 Transformer 模型僅在影像資料上表現較差，但與臨床元診斷資訊結合時，兩者都能學習到更好的互補資訊，並對準確預測中風治療效果做出良好的貢獻。
...
摘要：全球糖尿病盛行率不断上升，强调了糖尿病管理的需求。最近的研究重点关注糖尿病管理中的数字生物标记，在计算框架和使用个性化葡萄糖指标的非侵入性监测技术方面进行创新。然而，它们主要关注胰岛素剂量和特定的葡萄糖值，或对整体血糖控制的关注有限。这在扩大数字生物标记在糖尿病管理中整体血糖控制范围方面留下了空白。为了解决这样的研究空白，我们提出了 GluMarker——一个端到端框架，用于使用更广泛的因素来源对数字生物标记进行建模，以预测血糖控制。通过评估和改进各种机器学习基线，GluMarker 在预测次日血糖控制方面在 Anderson 的数据集上取得了最先进的成果。此外，我们的研究确定了次日血糖控制预测的关键数字生物标记。这些确定的生物标记有助于阐明影响血糖管理的日常因素，为糖尿病护理提供重要的见解。
...
摘要：在大數據時代，整合多樣化的數據模式會帶來重大的挑戰，特別是在醫療保健等複雜領域。本文介紹了一個新的多模態數據融合流程模型，用於資料探勘，整合嵌入式和資料探勘的跨產業標準流程，以及現有的資料融合資訊群組模型。我們的模型旨在降低運算成本、複雜性和偏差，同時提高效率和可靠性。我們還提出了「解開糾纏的稠密融合」，一種新穎的嵌入式融合方法，旨在最佳化互惠資訊並促進稠密模式間特徵互動，從而最小化冗餘資訊。我們透過三個使用案例來展示模型的效能：使用視網膜影像和患者元資料預測糖尿病視網膜病變、使用衛星影像、網際網路和人口普查資料預測家庭暴力，以及從放射線影像和臨床筆記中識別臨床和人口特徵。該模型在糖尿病視網膜病變預測中達到 0.92 的巨觀 F1 分數、在家庭暴力預測中達到 0.854 的 R 平方和 24.868 的 sMAPE，以及在放射學分析中分別達到 0.92 和 0.99 的巨觀 AUC，用於疾病預測和性別分類。這些結果強調了資料融合資料探勘模型在多模態資料處理中具有顯著影響的潛力，促進其在多元化、資源受限的環境中採用。
...
摘要：藥物推薦系統旨在提供個人化的藥物建議，這些建議與個別患者的需求緊密結合。先前的研究主要集中於開發藥物嵌入，取得顯著進展。然而，這些方法通常無法準確反映個別患者的病歷，這主要是由於難以區分各種患者狀況，以及無法在特定狀況與適當藥物之間建立精確的關聯性。為了回應這些問題，我們引入了 DisMed，這是一個專注於患者狀況以增強個人化的模型。DisMed 採用因果推論來辨別明確且可量化的因果關係。然後，它深入檢查患者狀況，識別並適應這些狀況不斷變化的細微差別，並將它們直接對應到相應的藥物。此外，DisMed 利用來自多個患者就診的數據來提出藥物組合。對真實世界數據集的全面測試表明，DisMed 不僅改善了患者病歷的客製化，而且在精準度和安全性方面都超越了領先的模型。
...
摘要：多巴胺轉運體 (DAT) 影像常被用於監控帕金森氏症 (PD)，其中紋狀體 DAT 攝取量用於評估 PD 嚴重程度。然而，DAT 影像成本高，且有輻射暴露風險，一般診所無法使用。最近，黑質區域的 MRI 區塊被提議為較安全且容易的替代方案。本文提出一個對稱回歸器，用於從黑質 MRI 區塊預測 DAT 攝取量。考量左右黑質之間的對稱性，所提出的回歸器結合配對輸入輸出模型，同時預測左右紋狀體的 DAT 攝取量。此外，它採用對稱損失，對左右預測之間的差異施加約束，類似於兩側 DAT 攝取量之間的高度相關性。此外，我們提出一個對稱蒙地卡羅 (MC) 中斷法，用於提供 DAT 攝取預測的豐富不確定性估計，它利用了上述對稱性。我們在 734 個黑質區塊上評估所提出的方法，結果顯示與標準回歸器相比，對稱回歸器的效能顯著提升，同時提供更好的可解釋性和特徵表徵。對稱 MC 中斷法也提供了精確的不確定性範圍，且有很高的機率將真實 DAT 攝取量包含在範圍內。
...
摘要：醫學影像分割通常需要廣泛的密集註解才能進行模型訓練，這既耗時又需要技能。為了減輕這種負擔，已引入基於範例的醫學影像分割方法，僅使用一張註解影像就能進行有效的訓練。在本文中，我們介紹了一個新的基於範例的醫學影像分割（CMEMS）的跨模型互學習框架，它利用兩個模型從未標記的資料中相互挖掘隱含資訊，並具有多種粒度。CMEMS 可以消除確認偏誤，並透過在模型之間的不同粒度上強制一致性，來進行協作訓練以學習互補資訊。具體來說，基於跨模型影像擾動的互學習是透過使用弱擾動影像來產生高置信度的偽標籤，監督跨模型對強擾動影像的預測。此方法能夠共同追求影像粒度上的預測一致性。此外，基於跨模型多級特徵擾動的互學習是透過讓偽標籤監督來自具有不同解析度的擾動多級特徵的預測，這可以擴展擾動空間並增強我們框架的穩健性。CMEMS 使用範例資料、合成資料和未標記資料以端到端的方式進行聯合訓練。在兩個醫學影像資料集上的實驗結果表明，所提出的 CMEMS 在極其有限的監督下優於最先進的分割方法。
...
摘要：聯邦學習 (FL) 已成為一種有前途的隱私保護機器學習方法，特別是在醫療保健等敏感領域。在此背景下，TRUSTroke 項目旨在利用 FL 協助臨床醫生預測缺血性中風。本文概述了 TRUSTroke FL 網路基礎架構。提出的架構採用客戶端伺服器模型，並配備中央參數伺服器 (PS)。我們為客戶端節點引入了基於 Docker 的設計，為在臨床環境中實施 FL 流程提供了一個靈活的解決方案。分析了不同通訊協定 (HTTP 或 MQTT) 對 FL 網路運作的影響，並選擇了適用於 FL 場景的 MQTT。還提出了一個控制平面來支援 FL 流程所需的主要運作。本文最後分析了 FL 架構的安全層面，探討潛在威脅並提出緩解策略以提高可信度。
...
摘要：機器去學習是更新機器學習模型的過程，用於移除特定訓練資料範例的資訊，以符合資料保護法規，允許個人要求移除其個人資料。儘管最近開發出許多去學習演算法，但這些演算法的可靠評估仍是一個開放的研究問題。在這項工作中，我們專注於基於成員推論攻擊 (MIA) 的評估，這是評估去學習演算法最常見的方法之一，並解決現有評估指標缺乏可靠性的各種缺點。具體而言，我們提出一個博弈論架構，將評估過程形式化為去學習演算法和 MIA 對手的博弈，透過 MIA 對手的功能來衡量去學習演算法的資料移除效能。透過仔細設計這個博弈，我們證明從博弈中推導出的自然評估指標享有現有評估指標無法滿足的可證明保證。此外，我們提出一個實用且有效的演算法來估計從博弈中推導出的評估指標，並透過理論分析和實證實驗證明其有效性。這項工作提出一個新穎且可靠的方法來實證評估去學習演算法，為開發更有效的去學習技術鋪路。
...
缺乏結構化的輸出和醫師互動性，無法產出清晰且臨床上
有意義的報告。我們的技術導入提示導引方法，使用預先訓練的大語言模型 (LLM) 來產生結構化的胸部 X 光報告。
首先，我們會識別胸部 X 光中的解剖區域，以產生以關鍵視覺元素為中心的焦點句子，進而建立以解剖為基礎的句子，作為結構化報告的基礎。我們也會將偵測到的解剖轉換成文字提示，傳達解剖理解給 LLM。
此外，臨床脈絡提示引導 LLM 強調互動性和臨床需求。透過整合以解剖為中心的句子和解剖/臨床提示，預先訓練的 LLM 可以產生結構化的胸部 X 光報告，針對提示的解剖區域和臨床脈絡進行調整。我們使用語言生成和臨床有效性指標進行評估，證明了強大的效能。
...
摘要：隨著全球人口老化，慢性腎臟疾病 (CKD) 的發生率正在上升。CKD 通常在晚期才會出現症狀，這對醫療保健系統和患者的生活品質造成重大負擔。本研究開發了一個可解釋的機器學習系統，用於預測有心血管風險患者的 CKD，利用病史和實驗室數據。隨機森林模型達到了 88.2% 的最高敏感度。該研究引入了一個全面的可解釋性框架，超越了傳統的特徵重要性方法，結合了全局和局部解釋、偏差檢查、生物醫學相關性和安全性評估。在全局解釋中識別出的關鍵預測特徵是使用糖尿病和 ACEI/ARB 藥物，以及初始 eGFR 值。局部解釋通過反事實解釋提供了模型見解，這與系統的其他部分一致。在進行偏差檢查後，發現初始 eGFR 值和 CKD 預測表現出一些偏差，但沒有發現顯著的性別偏差。通過範圍規則提取的模型邏輯被證實與現有的醫學文獻一致。安全性評估測試了潛在的危險案例，並確認模型行為安全。該系統增強了模型的可解釋性、可靠性和問責制，促進了它在醫療保健環境中的潛在整合和對即將出台的法規標準的遵守，並顯示出在醫療保健機器學習中更廣泛應用的前景。
...
摘要：肺部感染區域分割對於評估肺部疾病的嚴重程度至關重要。然而，現有的影像文字多模態方法通常依賴於模型訓練的人工密集型註解，在時間和專業知識方面構成挑戰。為了解決這個問題，我們提出了一個新穎的屬性知識引導框架，用於無監督肺部感染區域分割（AKGNet），該框架僅基於影像文字資料進行分割，而無需任何遮罩註解。AKGNet 同時促進文本屬性知識學習、屬性影像交叉注意力融合和基於高信心的偽標籤探索。它可以在嵌入空間中學習統計資訊並捕捉影像和文本屬性之間的空間相關性，反覆調整遮罩以增強分割。具體來說，我們通過提取屬性知識並將其納入特徵表示來引入文本屬性知識學習模組，使模型能夠學習統計資訊並適應不同的屬性。此外，我們設計了一個屬性影像交叉注意力模組，通過計算嵌入空間中屬性與影像之間的相關性來捕捉空間依賴性資訊，從而選擇性地關注相關區域，同時過濾無關區域。最後，採用自訓練遮罩改進過程，使用高信心的預測生成偽標籤，以反覆增強遮罩和分割。在基準醫學影像資料集上的實驗結果表明，與無監督情境中的最先進分割技術相比，我們的模型具有卓越的效能。
...
摘要：光達 (LiDAR) 與物聯網 (IoT) 技術的整合為城市安全與行人福祉的公共衛生資訊學提供了轉型契機。本文提出一個利用這些技術的創新架構，用於增強 3D 物件偵測與城市交通情境中的活動分類。透過採用架高的光達，我們取得詳細的 3D 點雲資料，進而能精確監控行人活動。為了解決城市資料短缺的問題，我們透過 Blender 中模擬的交通環境建立一個專門的資料集，以利進行目標模型訓練。我們的方法採用修改過的點體素區域卷積神經網路 (PV-RCNN) 進行穩健的 3D 偵測，並採用 PointNet 分類行人活動，這能大幅受益於城市交通管理與公共衛生，因為它能提供行人行為的見解並促進更安全的城市環境。我們的雙模型方法不僅能增強城市交通管理，還能透過提供行人行為的見解並促進更安全的城市環境，為公共衛生做出顯著貢獻。
...
摘要：糖尿病患人數不斷增加，是當今社會的嚴重問題，對民眾健康與國家財政支出造成極大的負面影響。由於糖尿病可能發展成潛在的嚴重併發症，糖尿病患者的早期血糖預測對於及時醫療介入至關重要。現有的血糖預測方法通常利用患者的個人資料（例如年齡、性別、種族）和生理參數（例如血壓、心率）作為血糖預測的參考特徵，這不可避免地導致隱私保護問題。此外，這些模型通常專注於長期（以月為基礎）或短期（以分鐘為基礎）的預測。長期預測方法通常不準確，因為外部的不確定性會對血糖值產生很大的影響，而短期預測則無法提供及時的醫療指導。基於上述問題，我們提出 CrossGP，一個新穎的機器學習框架，用於跨日血糖預測，僅基於患者的外部活動，不涉及任何生理參數。同時，我們實作了三個基線模型進行比較。在 Anderson 資料集上的大量實驗有力地證明了 CrossGP 的優越效能，並證明了其在未來實際應用中的潛力。
...
摘要：近期，原型學習已出現在半監督醫療影像分割中，並取得顯著的成效。然而，標籤資料的稀少性限制了先前方法中原型的表現力，這可能會阻礙原型在類別嵌入中的完整表示。為了解決這個問題，我們提出了混合原型一致性學習 (MPCL) 架構，其中包括一個平均教師和一個輔助網路。平均教師會為標籤和未標籤資料產生原型，而輔助網路會為由 CutMix 處理的混合資料產生額外的原型。透過原型融合，混合原型為標籤和未標籤原型提供額外的語義資訊。每個類別的高品質全球原型是透過融合兩個增強的原型形成的，並最佳化一致性學習中使用的隱藏嵌入分佈。在左心房和 B 型主動脈剝離資料集上的廣泛實驗證明了 MPCL 優於先前的最新技術，證實了我們架構的有效性。程式碼將很快釋出。
...
摘要：重組腺相關病毒 (rAAV) 載體徹底改變了基因治療，但它們廣泛的嗜性和次優的轉導效率限制了它們的臨床應用。為了克服這些限制，研究人員專注於設計和篩選衣殼庫，以識別改良的載體。然而，龐大的序列空間和有限的資源在識別可行的衣殼變體方面提出了挑戰。在本研究中，我們提出了一個端到端的擴散模型來產生具有增強活力的衣殼序列。使用公開的 AAV2 資料，我們生成了 38,000 個多樣的 AAV2 病毒蛋白 (VP) 序列，並評估了 8,000 個病毒選擇。結果證明了我們的模型優於傳統方法。此外，在沒有 AAV9 衣殼資料的情況下，除了野生型序列之外，我們使用相同的模型直接生成了許多具有多達 9 個突變的可行序列。我們將剩下的 30,000 個樣本轉移到 AAV9 結構域。此外，我們對 AAV9 VP 高變異區域 VI 和 V 進行了誘變，有助於 AAV9 VP 序列的持續改進。這項研究代表了 rAAV 載體設計和功能驗證的重大進展，為基因治療應用中提高特異性和轉導效率提供了創新的解決方案。
...
摘要：自然語言處理 (NLP) 領域隨著深度學習技術的快速發展取得了顯著進展。文本情緒分析的研究方向之一是醫療文本的情緒分析，這在臨床診斷應用中具有巨大的潛力。然而，醫療領域目前缺乏足夠的文本數據集，而情緒分析的有效性受到不同模型設計方法的很大影響，這提出了挑戰。因此，本文重點關注醫療領域，使用來自 Transformer（BERT）的雙向編碼器表示作為基本的預訓練模型，並在輸出層對卷積神經網路（CNN）、全連接網路（FCN）和圖形卷積網路（GCN）等模組進行實驗。在 METS-CoV 數據集上進行了實驗和分析，以探索整合不同的深度學習網路後訓練的效能。結果表明，當與 BERT 等預訓練模型結合使用時，CNN 模型在較小的醫療文本數據集上訓練時優於其他網路。本研究強調了模型選擇在醫療領域實現有效情緒分析中的重要性，並為未來研究開發更有效的模型架構提供了參考。
...
摘要：影像辨識技術高度依賴大量標籤資料，特別是在醫療情境中。處理取得標籤資料相關挑戰，已導致自監督式學習和半監督式學習的顯著地位，特別是在標註資料有限的情況下。在本文中，我們提出一個創新的方法，將自監督式學習整合到半監督式模型中，以增強醫療影像辨識。我們的做法從使用 BYOL 方法對未標籤資料進行預訓練開始。隨後，我們合併偽標籤和標籤資料集，以建構一個神經網路分類器，並透過反覆微調來改善它。在三個不同資料集上的實驗結果證明，我們的做法最佳化地利用未標籤資料，在醫療影像辨識的準確度方面優於現有方法。
...
摘要：可信度是安全應用不透明深度學習模型於高風險領域（如醫學）的主要先決條件。了解決策制定過程不僅有助於培養信任，還可能揭示複雜模型中先前未知的決策標準，這可能有助於推進醫學研究的狀態。從黑盒模型中發現與決策相關的概念是一項特別具有挑戰性的任務。本研究提出透過潛在擴散對抗事實軌跡（CDCT）進行概念發現，這是一個用於概念發現的新穎三步驟架構，它利用了擴散模型的優越影像合成能力。在第一步中，CDCT 使用潛在擴散模型（LDM）來產生對抗事實軌跡資料集。此資料集用於使用變異自動編碼器（VAE）衍生分類相關概念的解糾纏表示。最後，將搜尋演算法應用於解糾纏潛在空間中，以識別相關概念。將 CDCT 應用於在最大的公開皮膚病變資料集上訓練的分類器，不僅揭示了幾個偏差的存在，還揭示了有意義的生物標記。此外，在 CDCT 中產生的對抗事實比先前建立的最新方法產生的對抗事實顯示出更好的 FID 分數，同時資源效率高出 12 倍。無監督概念發現對於可信賴 AI 的應用以及各種領域中人類知識的進一步發展具有巨大的潛力。CDCT 代表了朝這個方向邁出的進一步一步。
...
摘要：異常偵測在風力渦輪機的預測維護領域中扮演著至關重要的角色，然而不同演算法的比較是一個困難的任務，因為特定領域的公開資料集非常稀少。許多不同方法的比較，不是使用由許多不同領域資料組成的基準、無法存取的資料，就是使用少數幾個公開可用的資料集，而這些資料集缺乏有關故障的詳細資訊。此外，許多出版物強調了幾個故障偵測成功的案例研究。透過這篇論文，我們公佈了一個高品質的資料集，其中包含來自 3 個不同風場的 36 個風力渦輪機的資料，以及迄今為止我們所知最詳細的任何公開風力渦輪機資料集的故障資訊。新的資料集包含 89 年的風力渦輪機實際運作資料，分佈在 44 個標記時間範圍內，這些時間範圍導致故障，以及 51 個代表正常行為的時間序列。此外，透過每個資料點的基於渦輪機狀態的標籤，確保了訓練資料的品質。此外，我們提出了一種新的評分方法，稱為 CARE（覆蓋率、準確性、可靠性和早期性），它利用資料集中存在的資訊深度來識別一個良好的全方位異常偵測模型。此評分考量了異常偵測效能、適當辨識正常行為的能力，以及在儘早偵測異常的同時，盡可能減少誤報的能力。
...
摘要：<paragraph>最近，隨著健康意識的提升，新穎的方法讓個人能夠在家中監控睡眠。利用睡眠聲音相較於智慧手錶等傳統方法，具有非侵入性且能夠偵測各種生理活動的優點。本研究旨在建構一個基於機器學習的睡眠評估模型，提供基於證據的評估，例如因睡眠開始時頻繁移動而導致的睡眠品質不佳。萃取睡眠聲音事件、使用 VAE 導出潛在表徵、使用 GMM 進行分群，以及訓練 LSTM 以進行主觀睡眠評估，在區分睡眠滿意度方面達到了 94.8% 的高準確度。此外，TimeSHAP 揭示了不同個人在影響力大的聲音事件類型和時間上的差異。</paragraph>
...
摘要：大腦網路呈現出層級組織，這是一個複雜性，對現有的深度學習模型構成挑戰，這些模型通常被建構為扁平分類器，導致難以解釋和「黑盒子」問題。為了彌合這個差距，我們提出了一種新穎的架構：一種由弱監督和新興語言 (EL) 框架通知的符號自動編碼器。此模型透過產生階層群集和對應的影像，超越傳統的扁平分類器，隨後透過符號句子表示，以改善對階層組織資料（例如內在腦網路）的臨床可解釋性，這些資料可以用靜止狀態功能性磁振造影影像來表徵。我們的創新包括一個廣義的階層損失函數，旨在確保句子和影像都能準確反映功能性腦網路的階層結構。這使我們能夠從更廣泛的觀點對功能性腦網路進行建模，深入到更細微的細節。此外，我們引入了一種定量方法來評估這些符號表示的階層一致性。我們的定性分析顯示，我們的模型成功地生成了階層組織、臨床上可解釋的影像，這項發現得到了我們的定量評估的支援。我們發現，我們效能最好的損失函數在識別與腦網路對應的影像時，會產生超過 97% 的階層一致性。這種方法不僅提升了深度學習模型在神經影像分析中的可解釋性，也代表了在建模腦網路複雜的階層性質方面邁出了一大步。
...
摘要：<paragraph>最近的人工智能突破有望从根本上提升我们对医疗保健的研究和理解。开发一个集成的多对多框架，利用多种数据模式输入来对多种医疗任务进行分析建模，对于统一理解现代医学至关重要。在这项工作中，我们引入了 M3H，这是一个可解释的多模态多任务机器学习医疗保健框架，它整合了来自广泛医疗任务类别和机器学习问题类别的不同多模态输入的学习。该框架的模块化设计确保了其可推广的数据处理、任务定义和快速模型原型制作，适用于临床和运营医疗保健环境。我们通过验证在 4 个机器学习问题类别中对 41 个医疗任务进行训练的四种模式（表格、时间序列、语言和视觉）的模型来评估 M3H 框架。我们的结果表明，M3H 始终产生多任务模型，其优于来自 16 个医疗部门的 37 种疾病诊断、三个医院运营预测和一项患者表型任务的规范单任务模型（1.1- 37.2%）：跨越监督二元分类、多类分类、回归和聚类的 ML 问题类别。此外，该框架引入了一种新颖的注意力机制来平衡自我利用（专注于学习源任务）和交叉探索（鼓励从其他任务中学习）。此外，M3H 提供了可解释性见解，说明附加任务的联合学习如何使用提议的 TIM 分数影响源任务的学习，从而阐明任务相互依赖的动态。其适应性架构促进了定制和集成，使其成为未来人工智能驱动的医疗保健系统的强大且可扩展的候选解决方案。</paragraph>
...
摘要：本文探讨了群體結構多代理系統 (MAS) 在建立醫療必要性中的應用，這是一個涉及系統性審查患者特定醫療結構化和非結構化數據與臨床指南的過程。我們透過將此複雜任務分解成較小、更易於管理的子任務來處理它。每個子任務都由一個專門的 AI 代理處理。我們對各種提示策略對這些代理的影響進行系統性研究，並對不同的大型語言模型 (LLM) 進行基準測試，以確定它們在完成這些任務中的準確性。此外，我們研究了這些代理如何提供可解釋性，從而增強系統內的信任和透明度。
...
摘要：人工智慧 (AI) 的普遍整合在涉及 AI 啟用系統的事故中引入了責任和問責制的複雜挑戰。這些系統的互連性、AI 引發事件的道德考量，加上 AI 技術的不確定性和缺乏相應的法規，使得傳統責任歸屬具有挑戰性。為此，這項工作提出了一種計算反思平衡 (CRE) 方法，為所有利害關係人建立一個連貫且在道德上可接受的責任歸屬架構。計算方法提供了一個結構化的分析，克服了概念性方法在處理動態和多方面的場景時的限制，展示了該架構在責任歸屬過程中可解釋性、連貫性和適應性等特性。我們探討了與平衡計算中的索賠相關的初始啟動層級的關鍵作用。使用 AI 輔助醫療決策支援系統作為案例研究，我們說明了不同的初始化如何導致不同的責任分配。該架構提供了對 AI 引發事件中問責制的寶貴見解，透過持續監控、修訂和反思，促進永續且有韌性的系統開發。
...
摘要：人工智慧透過預測模型協助醫療保健專業人員，大幅改變臨床決策制定。本研究探討醫療保健中 AI 應用對公平性和可解釋性的關鍵需求，以確保不同患者人口統計資料的公平結果。透過專注於敗血症相關死亡率的預測模型，我們提出一個方法，學習一個效能最佳化的預測模型，然後採用遷移學習流程來產生一個具有更佳公平性的模型。我們的模型也引進一個新的基於置換的特徵重要性演算法，旨在闡明每個特徵在增強預測公平性上的貢獻。與現有的可解釋性方法（專注於說明特徵對預測效能的貢獻）不同，我們提出的方法獨特地彌補了在理解每個特徵如何有助於公平性上的差距。這項進展至關重要，因為敗血症的死亡率很高，且在三分之一的醫院死亡中扮演角色。我們的模型不僅有助於辨識和減輕預測模型中的偏誤，也透過改善模型預測的透明度和公平性來促進醫療保健利害關係人之間的信任，進而促成更公平且值得信賴的醫療保健提供。
...
摘要：<paragraph>憂鬱症在現今是一個重要的議題。根據世界衛生組織 (WHO) 的資料，2023 年有超過 2.8 億人正與憂鬱症奮戰。這是一個龐大的數字；如果沒有認真看待，這些數字將會快速增加。約有 48.9 億人是社群媒體的使用者。人們在 Twitter、Facebook、Reddit、Instagram 等平台上表達自己的感受和情緒。這些平台包含有價值的資訊，可用於研究目的。已經在各種社群媒體平台上進行了大量的研究。然而，這些努力中仍然存在某些限制。特別是，先前的研究僅專注於偵測推文中的憂鬱症和憂鬱症的嚴重程度。此外，資料集標籤中也存在不準確的情況。在這項研究工作中，使用基於詞彙標籤的 Twitter 資料庫中的推文預測了五種類型的憂鬱症（躁鬱症、重度憂鬱症、精神病性憂鬱症、非典型憂鬱症和產後憂鬱症）。可解釋的 AI 用於透過突顯代表憂鬱症類型的推文部分來提供推理。使用來自 Transformer 的雙向編碼器表示 (BERT) 進行特徵萃取和訓練。使用機器學習和深度學習方法來訓練模型。BERT 模型呈現出最有希望的結果，達到 0.96 的整體準確度。</paragraph>
...
摘要：深度學習正在顯著地轉化醫療影像和放射學領域，能夠識別醫療影像中的病理，包括電腦斷層掃描 (CT) 和 X 光掃描。然而，深度學習模型的效能，特別是在分割任務中，通常受到廣泛註解資料集需求的限制。為了解決此挑戰，透過可解釋人工智慧和反事實解釋的生成，探索了弱監督語意分割的能力。本研究的範圍是開發一種新穎的反事實內插方法 (COIN)，透過使用生成模型將預測的分類標籤從異常翻轉為正常。例如，如果分類器將輸入的醫療影像 X 視為異常，表示存在病理，生成模型的目標是內插異常區域，從而逆轉分類器的原始預測標籤。此方法使我們能夠產生病理的精確分割，而無需依賴於預先存在的分割遮罩。至關重要的是，利用影像層級標籤，與建立詳細的分割遮罩相比，這些標籤更容易取得。該方法的有效性透過分割愛沙尼亞塔爾圖大學醫院取得的 CT 影像中的合成目標和實際腎臟腫瘤來證明。研究結果表明，COIN 遠遠優於已建立的歸因方法，例如 RISE、ScoreCAM 和 LayerCAM，以及 Singla 等人提出的另一種反事實解釋方法。此證據表明，COIN 是一種用於 CT 影像中腫瘤語意分割的有前途的方法，並在醫療保健中讓深度學習應用程式更易於使用和更有效率方面邁進了一步，而醫療保健中註解資料很稀少。
...
摘要：甲狀腺癌是日益受到全球關注的健康問題，需要先進的診斷方法。本文回顧了人工智慧和放射特徵分析在甲狀腺癌診斷中的應用。根據 PRISMA 指南，我們回顧了多個資料庫，直到 2023 年 10 月。我們結合多個關鍵字，發現了與甲狀腺癌和相關主題相關的英文學術出版品。在移除 109 個重複資料後，原始搜尋共回傳 267 篇論文。在根據預先確定的標準審查了 124 篇論文的摘要和標題後，我們選出了相關研究。在進行全面分析後，我們又排除了六項研究。在納入的 28 項研究中，結合超音波 (US) 影像的放射特徵分析，證明了其在診斷甲狀腺癌方面的有效性。研究結果不一，有些研究提出了優於現狀的新策略。文獻強調了人工智慧模型面臨的各種挑戰，包括可解釋性問題、資料集限制和操作員依賴性。28 項納入研究的綜合發現提到，需要標準化工作和前瞻性多中心研究來解決這些問題。此外，我們也找出了克服這些障礙的方法，例如可解釋人工智慧技術和個人化醫療技術的進步。本回顧重點探討了人工智慧和放射特徵分析如何轉變甲狀腺癌的診斷和治療。儘管面臨挑戰，未來對多學科合作、臨床適用性驗證和演算法改進的研究，仍有可能改善甲狀腺癌治療的患者預後和診斷精準度。
...
摘要：近年來，乳癌的盛行率急速上升，成為全球主要的死亡原因之一。在所有癌症中，乳癌是迄今為止最常見的。手動診斷此疾病需要大量時間和專業知識。由於乳癌的檢測過程耗時，因此透過建立機器學習模型，可以協助預防其進一步擴散。機器學習和可解釋 AI 在分類中至關重要，因為它們不僅提供準確的預測，還能洞察模型如何做出決策，有助於理解和信賴分類結果。在本研究中，我們評估並比較了五種不同機器學習方法的分類準確率、精確度、召回率和 F1 分數，並使用了一個主要資料集（達卡醫學院醫院的 500 名患者）。我們使用了五種不同的監督式機器學習技術，包括決策樹、隨機森林、邏輯斯迴歸、樸素貝氏和 XGBoost，以在我們的資料集上取得最佳結果。此外，本研究將 SHAP 分析應用於 XGBoost 模型，以解釋模型的預測並了解每個特徵對模型輸出的影響。我們比較了幾種演算法對資料進行分類的準確度，並與該領域的其他文獻進行了對比。經過最終評估，本研究發現 XGBoost 達到了最佳模型準確率，為 97%。
...
摘要：深度學習 (DL) 模型用於診斷乳房 X 光檢查中的乳癌，通常以「黑盒子」的方式運作，這讓醫療保健專業人員難以信賴和理解他們的決策過程。本研究提出一個整合架構，結合卷積神經網路 (CNN) 和可解釋人工智慧 (XAI)，以增強使用 CBIS-DDSM 資料集診斷乳癌。該方法包含一個精細的資料前處理管道和先進的資料擴充技術，以對抗資料集限制，並使用預訓練網路（例如 VGG-16、Inception-V3 和 ResNet）進行遷移學習。本研究的重點是評估 XAI 在解釋模型預測方面的有效性，重點利用豪斯多夫測度定量評估 AI 生成的解釋與專家註解之間的一致性。此方法對於 XAI 在促進 AI 輔助診斷中的可信度和道德公平性至關重要。我們研究的結果說明了 CNN 和 XAI 在推進乳癌診斷方法方面的有效協作，從而促進了先進 AI 技術在臨床環境中的更無縫整合。透過增強 AI 驅動決策的可解釋性，這項工作為 AI 系統和醫療從業人員之間的改善協作奠定了基礎，最終豐富了患者照護。此外，我們研究的影響遠遠超出了當前的研究方法。它鼓勵進一步研究如何結合多模式資料並改善 AI 解釋，以滿足臨床實務的需求。
...
摘要：以人为本的可解释人工智能 (HCXAI) 倡导将社会层面融入人工智能解释中。HCXAI 话语的核心是社会透明度 (ST) 框架，旨在让用户能够了解人工智能系统的社会组织背景。在这项工作中，我们建议扩展 ST 框架以解决大型语言模型 (LLM) 中社会错误归因的风险，尤其是在心理健康等敏感领域。事实上，LLM 具有惊人的角色和人格模拟能力，可能会导致设计者的意图与用户对社会属性的认知之间的不匹配，从而有可能助长情感操纵和危险行为、认知不公正以及不合理的信任。为了解决这些问题，我们建议用第五个“W 问题”来增强 ST 框架，以阐明设计者和用户赋予 LLM 的具体社会属性。这一补充旨在弥合 LLM 能力与用户认知之间的差距，促进基于 LLM 的技术的道德负责开发和使用。
...
摘要：<paragraph>背景：氣胸是一種因肺部和胸壁之間異常空氣聚集所引起的急性胸腔疾病。為了處理經常與深度學習 (DL) 模型相關的不透明性，已引入可解釋人工智慧 (XAI) 方法來概述與 DL 模型所做的氣胸診斷相關的區域。然而，這些解釋有時會偏離實際病灶區域，突顯出進一步改進的必要性。方法：我們提出一個範本引導方法，將氣胸的臨床知識納入 XAI 方法產生的模型解釋中，從而提高這些解釋的品質。利用放射科醫師建立的一個病灶描繪，我們的做法首先產生一個範本，代表氣胸發生的潛在區域。然後將此範本疊加在模型解釋上，以濾除範本界線外的多餘解釋。為了驗證其效力，我們在兩個真實世界資料集中解釋兩個 DL 模型時，對三種 XAI 方法進行了有和沒有我們範本指導的比較分析。結果：所提出的方法在建構於三種 XAI 方法、兩個 DL 模型和兩個資料集上的十二個基準情境中，持續改善基線 XAI 方法。在比較模型解釋和真實病灶區域時，透過基線效能的效能改進所計算出的平均增量百分比，在聯合交集 (IoU) 中為 97.8%，在骰子相似係數 (DSC) 中為 94.1%。結論：在氣胸診斷的背景下，我們提出了一個範本引導的方法來改善 AI 解釋。我們預期我們的範本指導將透過整合臨床領域專業知識，為闡明 AI 模型建立一個新的方法。</paragraph>
...
摘要：<paragraph>在當前的機器翻譯 (MT) 領域中，Transformer 架構脫穎而出，成為黃金標準，特別是對於高資源語言對。本研究探討其對低資源語言對的效能，包括英語$\leftrightarrow$愛爾蘭語和英語$\leftrightarrow$馬拉地語語言對。值得注意的是，本研究找出最佳的超參數和子詞模型類型，以顯著提升 Transformer 模型在低資源語言對的翻譯品質。
  低資源語言的平行資料集稀少，可能會阻礙 MT 的發展。為了解決這個問題，開發了 gaHealth，這是第一個愛爾蘭語的健康資料雙語語料庫。專注於健康領域，使用這個領域內資料集開發的模型，與 LoResMT2021 共享任務的模型相比，在 BLEU 分數上表現出非常顯著的提升。後續使用多維品質指標錯誤分類法進行的人工評估，展示了 Transformer 系統在減少準確度和流暢度錯誤方面的優異表現，與基於 RNN 的對照組相比。
  此外，本論文介紹了 adaptNMT 和 adaptMLLM，這兩個開放原始碼應用程式簡化了神經機器翻譯模型的開發、微調和部署。這些工具大幅簡化了設定和評估流程，讓開發人員和翻譯人員都能更輕鬆地使用 MT。值得注意的是，adaptNMT 基於 OpenNMT 生態系統，透過強調模型開發的環境足跡，促進環保的自然語言處理研究。adaptMLLM 微調 MLLM，證明了在翻譯效能方面有進步，針對兩個低資源語言對：英語$\leftrightarrow$愛爾蘭語和英語$\leftrightarrow$馬拉地語，與 LoResMT2021 共享任務的基準線相比。</paragraph>
...
摘要：糖尿病（DM）會使患者容易出現血管併發症。
視網膜影像和血管反映身體的微血管和巨血管健康狀況。它們可用於診斷糖尿病併發症，包括糖尿病視網膜病變（DR）、神經病變、腎病變和動脈粥樣硬化性心血管疾病，以及預測心血管事件的風險。使用數位化視網膜影像進行 DR 高通量偵測的人工智慧（AI）啟用系統已在臨床上採用。除了 DR 篩檢之外，AI 整合也極具潛力，可以解決與糖尿病患者整體照護相關的挑戰。在這項工作中，我們的目標是全面檢視關於以視網膜影像為基礎的 AI 應用於糖尿病診斷、預後和管理的研究文獻。我們將描述整體 AI 輔助糖尿病照護的發現，包括但不限於 DR 篩檢，並討論實施此類系統的障礙，包括涉及倫理、資料隱私、公平取得和可解釋性的問題。透過評估患者的健康狀態，並針對未來的心血管併發症進行風險預後，AI 輔助視網膜影像分析有潛力成為糖尿病患者現代化個人化醫療的中心工具。
...
摘要：本研究從多個利益相關者的角度探討了在教育中使用不同的人工智慧 (AI) 應用程式的可接受性，包括學生、教師和家長。承認 AI 在教育中具有變革潛力，它解決了與資料隱私、AI 機構、透明度、可解釋性以及 AI 的道德部署相關的問題。透過簡要說明的方法，參與者被提供四個場景，其中 AI 的機構、透明度、可解釋性和隱私被操縱。在每個場景之後，參與者完成了一項調查，調查捕捉了他們對 AI 的整體效用、個別效用、公正性、信心、風險以及在有機會時使用每個場景 AI 的意圖的看法。收集的資料包含 1198 位多重利益相關者參與者的最終樣本，透過合作機構和社群媒體活動進行分發，並專注於對四個 AI 使用案例的個別回應。對資料的調解分析表明，對 AI 的接受度和信任在利益相關者群體中差異很大。我們發現，AI 機構、透明度和可解釋性的高低層級之間的主要調解者，以及使用不同教育 AI 的意圖，包括感知到的整體效用、公正性和信心。這項研究強調，在教育中接受 AI 是一個微妙且多方面的問題，除了不同的利益相關者的看法之外，還需要仔細考慮具體的 AI 應用程式及其特徵。
...
摘要：遠距病患監測基於可穿戴單導程心電圖 (ECG) 裝置，對於早期偵測心臟疾病有顯著的潛力，特別是結合人工智慧 (AI) 方法進行自動化心臟疾病偵測。先前已有研究應用基於深度學習的 AI 方法進行心臟疾病偵測。然而，這些模型尚未被廣泛接受為臨床診斷的可靠輔助工具，部分原因是目前許多 AI 演算法周圍的黑盒子感知。特別需要找出有助於做出準確診斷的 ECG 訊號關鍵特徵，進而增強模型的可解釋性。在本研究中，我們開發一種視覺轉換器方法，根據單導程 ECG 資料找出心房顫動。殘差網路 (ResNet) 方法也開發出來，與視覺轉換器方法進行比較。這些模型應用於 Chapman-Shaoxing 資料集，以分類心房顫動，以及另一種常見心律不整，竇性心動過緩，和正常竇性心律的心跳。這些模型能夠找出決定最終分類的心跳關鍵區域，並強調 P 波和 T 波，以及心跳持續時間和訊號振幅在區別正常竇性心律、心房顫動和竇性心動過緩的重要性。
...
摘要：本論文介紹了一種使用先進大型語言模型 (LLM) 進行憂鬱症偵測和治療的新穎典範：生成式預訓練轉換器 4 (GPT-4)、Llama 2 聊天和 Gemini。這些 LLM 經過微調，具備診斷、解釋和建議憂鬱症治療介入措施的專業提示。一種獨特的少量提示方法增強了模型根據 DSM-5 標準分析和解釋憂鬱症狀的能力。在互動階段，這些模型從事同理心對話管理，利用 PsychDB 和認知行為療法 (CBT) 指南等資源，促進與經歷重度憂鬱症的個人進行支持性互動。此外，本研究引入了 Illuminate 資料庫，其中包含了各種 CBT 模組，有助於個性化治療建議。本研究使用 F1 分數、精確度、召回率、餘弦相似度和面向召回率的 Gisting 評估替身 (ROUGE) 等指標，評估不同測試集中的 LLM 效能，證明了其有效性。這種全面性方法將尖端的 AI 與既定的心理方法相結合，為心理保健提供了新的可能性，並展示了 LLM 在革新憂鬱症診斷和治療策略方面的潛力。
...
摘要：<paragraph>AI 系統的說明很少會說明受演算法決策 (ADM) 影響的人們的資訊需求。傳達的資訊與受影響的利害關係人重視的資訊之間的差距會阻礙對 AI 法規等法規架構的理解與遵守。為了解決這個差距，我們提出了「XAI 新手問題庫」：一個受影響的利害關係人資訊需求目錄，包含兩個 ADM 使用案例（就業預測和健康監控），涵蓋資料、系統脈絡、系統使用和系統規格等類別。資訊需求是透過訪談研究收集，參與者在提出詢問後會收到說明。參與者進一步回報他們的理解和決策信心，顯示雖然在收到說明後信心往往會增加，但參與者也遇到了理解上的挑戰，例如無法說明為什麼他們的理解感覺不完整。說明進一步影響了參與者對系統風險和好處的看法，他們會根據使用案例確認或更改看法。當風險被視為很高時，參與者表示特別有興趣了解意圖的說明，例如為什麼以及為了什麼目的而建立系統。透過這項工作，我們旨在透過在利害關係人決定採用 ADM 系統時提供相關資訊和挑戰的概觀，來支持將受影響的利害關係人納入可解釋性中。最後，我們將我們的發現總結成一份包含六項關鍵影響的清單，為針對受影響的利害關係人受眾的未來說明設計提供資訊。</paragraph>
...
摘要：人工智慧 (AI) 的快速演進，特別是在大型語言模型 (LLM) 和生成式 AI 的領域，已為各種領域的應用開啟了新的管道，但其在商業教育中的作用仍未受到充分探討。本研究首次引進基準，用於評估七種主要 LLM 的效能，包括 OpenAI 的模型 (GPT-3.5 Turbo、GPT-4 和 GPT-4 Turbo)、Google 的模型 (PaLM 2、Gemini 1.0 Pro) 和 Anthropic 的模型 (Claude 2 和 Claude 2.1)，這些模型都用於 GMAT，這是研究所商學課程入學程序中的關鍵考試。我們的分析顯示，大多數 LLM 的表現都優於人類考生，而 GPT-4 Turbo 不僅優於其他模型，更超越了頂尖商學院研究所學生的平均分數。透過案例研究，本研究探討了 GPT-4 Turbo 解釋答案、評估回應、找出錯誤、調整說明和產生替代方案的能力。最新的 LLM 版本，GPT-4 Turbo、Claude 2.1 和 Gemini 1.0 Pro，在推理任務方面相較於前一代版本有顯著的進步，強調了它們在複雜問題解決方面的潛力。雖然 AI 在教育、評量和輔導方面的承諾很明確，但挑戰依然存在。我們的研究不僅闡明了 LLM 的學術潛力，也強調了在教育中謹慎發展和應用 AI 的必要性。隨著 AI 技術的進步，建立 AI 互動的架構和協定、驗證 AI 所產生內容的準確性、確保全球各地不同的學習者都能使用，以及創造一個 AI 支援人類專業知識的教育環境至關重要。本研究為進一步探討負責任地使用 AI 以豐富教育體驗並改善考試準備和評量方法奠定了基礎。
...
摘要：預測加護病房 (ICU) 病患的院內死亡率是最終臨床結果的關鍵。AI 已展現出優異的準確性，但缺乏可解釋性。為了解決這個問題，本文提出一個可解釋的多模式死亡率預測器 (X-MMP)，透過多模式 ICU 資料，提供一個有效且可解釋的 AI 解決方案來預測院內死亡率。我們在架構中採用多模式學習，它可以接收來自臨床資料的異質性輸入並做出決策。此外，我們引入一種可解釋的方法，即層級傳播到轉換器，作為 LRP 方法到轉換器的適當延伸，對多模式輸入產生解釋，並揭示歸因於預測的顯著特徵。此外，每個模式對臨床結果的貢獻都可以視覺化，協助臨床醫師了解決策背後的推理。我們根據 MIMIC-III 和 MIMIC-III 波形資料庫配對子集建構一個多模式資料集。在基準資料集上的全面實驗證明，我們提出的架構可以在具有競爭力的預測準確度下實現合理的解釋。特別是，我們的架構可以輕鬆轉移到其他臨床任務，這有助於發現醫療保健研究中的關鍵因素。
...
摘要：在過去十年中，病理學中的人工智慧 (AI) 方法已大幅進步。然而，由於將研究結果轉化為臨床診斷產品的技術和法規障礙以及缺乏標準化介面等眾多挑戰，整合到常規臨床實務的進度緩慢。開放且與供應商無關的 EMPAIA 計畫解決了這些挑戰。在此，我們概述了 EMPAIA 的成就和所學到的教訓。EMPAIA 整合了病理學 AI 生態系統的各種利害關係人，即病理學家、電腦科學家和產業。在密切合作下，我們制定了技術互通性標準、AI 測試和產品開發建議，以及可解釋性方法。我們實作了模組化且開放原始碼的 EMPAIA 平台，並成功整合了來自 8 家不同供應商的 14 個基於 AI 的影像分析應用程式，展示了不同的應用程式如何使用單一的標準化介面。我們優先考慮需求，並在歐洲和亞洲的 14 個不同的病理實驗室評估了 AI 在實際臨床環境中的使用情況。除了技術開發外，我們還為所有利害關係人建立了一個論壇，以分享有關數位病理學和 AI 的資訊和經驗。商業、臨床和學術利害關係人現在可以採用 EMPAIA 的常見開放原始碼介面，為大規模標準化和流程簡化提供了獨特的機會。需要進一步的努力才能在常規實驗室使用中有效且廣泛地建立 AI 協助。為此，已成立非營利協會 EMPAIA International，作為一個永續基礎架構，以持續標準化和支援廣泛實作，並倡導 AI 協助的數位病理學未來。
...
摘要：反事實解釋 (CE) 技術作為一種手段，能為與 AI 系統互動的使用者提供見解，因而備受關注。儘管在醫學影像和自動駕駛車輛等領域廣泛研究，但圖形反事實解釋 (GCE) 方法相對而言仍未得到充分探討。GCE 會產生一個與原始圖形相似的圖形，但其不同的結果基於底層預測模型。在這些 GCE 技術中，儘管在藝術風格和自然語言建模等其他領域展示了令人印象深刻的成就，但植根於生成機制的技術卻受到的探討相對較少。人們偏好生成式解釋器，原因在於它們能夠在推理期間生成反事實實例，利用輸入圖形的自主獲取擾動。基於上述依據，我們的研究引入了 RSGG-CE，這是一種用於反事實解釋的新型穩健隨機圖形生成器，能夠從學習到的潛在空間中產生反事實範例，並考慮部分有序的生成順序。此外，我們進行定量和定性分析，以比較 RSGG-CE 與 SoA 生成式解釋器的效能，強調其產生合理解釋反事實候選項的能力有所提升。
...
摘要：可解釋 AI 的動機之一是讓人類在使用和部署 AI 模型時做出更好、更明智的決策。但需要仔細評估以評估是否已實現此預期。目前的評估主要集中在解釋的演算法特性，而涉及人類受試者的評估通常會使用主觀問題來測試人類對解釋有用性的認知，而沒有依據客觀指標和測量結果。在這項工作中，我們評估了解釋是否能在機器學習模型開發的實際情況中改善人類的決策制定。我們進行了一項涉及影像資料的混合方法使用者研究，以評估 SmoothGrad、GradCAM 和一個神諭解釋在兩個任務中產生的顯著性圖：模型選擇和反事實模擬。令人驚訝的是，我們沒有發現任何證據顯示當使用者獲得任何顯著性圖時，這些任務有顯著改善，即使是設計成易於理解且高度指示答案的合成神諭解釋也是如此。儘管如此，解釋確實有助於使用者更準確地描述模型。這些發現表明對基於顯著性的解釋中實用性和誤解的可能性持謹慎態度。
...
摘要：可解釋性和安全性培養出信任。這些需要模型表現出一致性和可靠性。為了達成這些，必須使用和分析資料和知識，並搭配與 AI 應用相關的統計和符號 AI 方法，單獨使用一種方法無法達成。因此，我們主張並試圖證明神經符號 AI 方法更適合讓 AI 成為值得信賴的 AI 系統。我們提出 CREST 架構，說明一致性、可靠性、使用者層級的可解釋性，以及安全性是如何建立在神經符號方法之上，這些方法使用資料和知識來支援關鍵應用程式的需求，例如健康和福祉。本文專注於大型語言模型 (LLM)，視其為 CREST 架構內所選的 AI 系統。由於 LLM 在處理廣泛的自然語言處理 (NLP) 場景方面具有多功能性，因此獲得研究人員的大量關注。例如，ChatGPT 和 Google 的 MedPaLM 已成為提供一般和健康相關查詢資訊的極具前景的平台。儘管如此，這些模型仍然是黑盒子，儘管納入了人類回饋和指令引導調整。例如，儘管制定了安全防護措施，但 ChatGPT 仍可能產生不安全的回應。CREST 提出了一種可行的途徑，在神經符號架構內利用程序和圖形知識，以闡明與 LLM 相關的挑戰。
...
摘要：這項研究調查了已部署人工智慧 (AI) 模型在預測 COVID-19 大流行期間及之後死亡率方面的效能、可解釋性和穩健性。作為同類研究中的第一個，我們發現貝氏神經網路 (BNN) 和智慧型訓練技術讓我們的模型在資料大幅變動的情況下仍能維持效能。我們的結果強調了開發穩健的 AI 模型的重要性，即使在具挑戰性的條件下，這些模型也能夠與臨床醫師的預測相符或超越臨床醫師的預測。我們對模型可解釋性的探討顯示，隨機模型會產生更多元且個人化的解釋，因此突顯了 AI 模型在實際臨床環境中提供詳細且個別見解的必要性。此外，我們強調了量化 AI 模型中不確定性的重要性，這讓臨床醫師能夠根據可靠的預測做出更明智的決策。我們的研究主張在醫療保健的 AI 研究中優先實施科學，並確保 AI 解決方案在實際臨床環境中實用、有益且永續。透過解決醫療保健環境中獨特的挑戰和複雜性，研究人員可以開發出有效改善臨床實務和患者預後的 AI 模型。
...
摘要：肺癌占英國癌症死亡人數的 21%，五年存活率深受癌症發現時的期別影響。最近的研究已證明，人工智慧方法有能力準確及早從例行掃描中診斷出肺癌。然而，此證據並未轉化為臨床實務，其中一項障礙是缺乏可解釋的模型。本研究探討變異自動編碼器 (VAE) 的應用，這是一種生成式人工智慧模型，用於肺癌病灶。建議的模型根據從 LIDC-IDRI 公共資料集中的 3D 電腦斷層掃描中提取的病灶進行訓練。透過聚類探討 VAE 產生的 2D 切片的潛在向量表示，以證明其品質，並用於肺癌診斷的多層感知機分類器模型，最佳模型達到了 AUC 0.98 和 93.1% 準確度的最佳指標。聚類分析顯示，VAE 潛在空間根據有意義的特徵組成（包括腫瘤大小、形狀、患者和惡性類別）將惡性和良性病灶的資料集分開。我們還包括標準高斯 VAE (GVAE) 和較新的 Dirichlet VAE (DirVAE) 的比較分析，後者以 Dirichlet 分布取代先驗，以鼓勵具有解開特徵表示的更具可解釋性的潛在空間。最後，我們展示了與臨床有意義的特徵變化相應的潛在空間橫越的潛力。
...
摘要：現有的圖像分類器輸出說明工具可分為依賴訪問模型內部結構的白盒，以及與模型無關的黑盒。隨著 AI 在醫療領域的使用日益廣泛，可解釋性工具的使用也隨之增加。現有醫療影像說明的研究重點在於白盒工具，例如 gradcam。然而，轉換為黑盒工具有明顯的優點，包括可與任何分類器一起使用，以及可選擇廣泛的黑盒工具。在標準影像上，黑盒工具與白盒一樣精確。在本文中，我們比較了幾種黑盒方法在腦癌 MRI 資料集上與 gradcam 的效能。我們證明了大多數黑盒工具不適合用於說明醫療影像分類，並詳細分析了其缺點的原因。我們還展示了一種黑盒工具，一種因果可解釋性為基礎的 rex，其效能與 \gradcam 一樣好。
...
摘要：AI 開發社群日益利用 Hugging Face 等託管中介機構，提供使用者上傳的模型和訓練資料的簡易存取。這些模型市集降低了數十萬使用者的技術部署障礙，但卻可能被用於許多潛在有害且非法的用途。在本文中，我們說明 AI 系統（可以「包含」內容並成為開放式工具）呈現迄今為止最棘手的平台治理挑戰的方式。我們提供 Hugging Face、GitHub 和 Civitai 等三個說明性平台上數起事件的案例研究，以探討模型市集如何審核模型。根據此分析，我們概述產業為回應審核需求而發展的重要（但仍受限）實務：授權、存取和使用限制、自動化內容審核和開放政策發展。儘管當前的政策挑戰相當可觀，我們最後提出一些想法，說明平台如何能更好地動員資源，作為謹慎、公平且適度的法規存取點。
...
摘要：<paragraph>背景與目標：透過萃取這些資訊，機器或深度學習 (ML/DL) 為基礎的自動化資料分析工具，可以協助臨床醫師和癌症研究人員從複雜的資料集中發現模式和關聯性。許多基於深度學習的卵巢癌 (OC) 資料分析近期已發表。這些分析在癌症的各個面向（例如，他們探討的子領域和癌症類型）和資料分析特徵方面，有高度的多樣性。然而，目前對於這些分析在這些特徵和 AI 保證 (AIA) 方面的全面理解仍有所欠缺。這篇系統性回顧旨在透過檢視現有文獻並找出使用深度學習進行卵巢癌資料分析的重要面向，明確地關注關鍵特徵和 AI 保證觀點，來填補這個缺口。方法：使用 PRISMA 架構在三個期刊資料庫中進行全面的搜尋。分析中僅納入 2015 年至 2023 年間發表於同行評審期刊的研究。結果：在回顧中，共檢視了 96 項由深度學習驅動的分析。研究結果揭露了關於由深度學習驅動的卵巢癌資料分析的幾個重要見解：- 大多數研究 71%（96 項中有 68 項）專注於偵測和診斷，而沒有研究探討卵巢癌的預測和預防。- 這些分析主要根據來自非多元族群的樣本（75%（96 項研究中的 72 項）），僅限於某個地理位置或國家。- 只有少部分研究（僅 33%（96 項中的 32 項））執行整合分析，其中大多數使用同質資料（臨床或組學）。- 特別值得注意的是，僅有 8.3%（96 項中的 8 項）的研究使用外部和多元的資料集驗證其模型，突顯了增強模型驗證的必要性，以及- 將 AIA 納入癌症資料分析仍處於非常早期的階段；僅有 2.1%（96 項中的 2 項）透過可解釋性明確地探討 AIA。</paragraph>
...
摘要：<paragraph>可解釋性是深度學習中長久以來的挑戰，尤其是在醫療保健等高風險領域。常見的可解釋性方法會強調驅動 AI 模型決策的影像區域。然而，人類在傳達解釋時極度依賴語言，不只說明「在哪裡」，還說明「是什麼」。此外，大多數可解釋性方法專注於解釋個別 AI 預測，而不是描述 AI 模型普遍使用的特徵。後者對於模型和資料集稽核特別有用，甚至在 AI  zuneh用於新穎任務時也能產生知識。在此，我們提出一個可解釋性策略，使用視覺語言模型來識別視覺分類任務的語言描述符。透過利用影像和文字之間預先訓練的聯合嵌入空間，我們的做法將新的分類任務估計為字詞的線性組合，為每個字詞產生一個權重，表示其與基於視覺的分類器的對齊方式。我們使用兩個醫學影像分類任務來評估我們的做法，發現產生的描述符在很大程度上與臨床知識一致，儘管缺乏特定領域的語言訓練。然而，我們的做法也發現了所用公共資料集中「捷徑連線」的可能性。為了達到可解釋性的功能性衡量，我們進行了一項試驗性讀者研究，發現 AI 識別的字詞可以讓非專家人類在非平凡的層級上執行專業的醫療任務。總之，我們的結果強調了使用多模態基礎模型來提供直觀、基於語言的視覺任務解釋的潛力。</paragraph>
...
摘要：人工智慧（AI）模型使用醫療影像訓練臨床任務時，常會展現出偏見，表現為不同子群體之間的表現差異。由於現實世界醫療影像資料中並非所有偏見來源都容易辨識，因此全面評估偏見如何編碼到模型中，以及偏見緩解方法在改善表現差異方面的能力是一項挑戰。在本文中，我們提出了一個新穎的分析架構，用於系統且客觀地調查醫療影像中的偏見對 AI 模型的影響。我們開發並測試了這個架構，用於執行受控的電腦模擬試驗，以評估醫療影像 AI 中的偏見，使用一種工具來生成具有已知疾病影響和偏見來源的合成磁共振影像。透過使用三個反事實偏見情境來衡量模擬偏見影響對卷積神經網路（CNN）分類器的影響，以及三個偏見緩解策略的效能，展示了可行性。分析顯示，當 CNN 在合成資料集上訓練時，模擬偏見會導致預期的子群體表現差異。此外，重新加權被認為是針對此設定最成功的偏見緩解策略，我們展示了解釋性 AI 方法如何協助調查使用此架構的模型中偏見的表現。由於醫療影像資料集中可能存在許多且通常未知的偏見來源，因此開發公平的 AI 模型是一項重大挑戰。在這項工作中，我們提出了一種新穎的方法，用於客觀地研究偏見和緩解策略對深度學習管線的影響，這可以支援開發穩健且負責任的臨床 AI。
...
摘要：機器學習提供巨大的潛力，用於自動預測中風後症狀及其對康復的反應。這項努力的主要挑戰包括神經影像資料的維度非常高、可用於學習的資料集規模相對較小，以及如何有效地結合神經影像和表格資料（例如人口統計資訊和臨床特徵）。本文根據兩種策略評估了多種解決方案。第一個是使用概括 MRI 掃描的 2D 影像。第二個是選擇能改善分類準確度的關鍵特徵。此外，我們引進了訓練卷積神經網路 (CNN) 的新方法，使用結合從 MRI 中萃取的感興趣區域與表格資料的符號表示的影像。我們評估了一系列 CNN 架構（2D 和 3D），這些架構在 MRI 和表格資料的不同表示上受過訓練，以預測中風後口述圖片描述能力的複合測量是否在失語症或非失語症範圍內。MRI 和表格資料是從 758 位參與 PLORAS 研究的英語中風倖存者取得。僅針對病灶大小的基準邏輯迴歸分類準確度為 0.678，當陸續加入初始症狀嚴重程度和恢復時間後，提高到 0.757 和 0.813。在 2D 殘差神經網路中從每個 MRI 掃描中萃取 8 個感興趣區域，並與病灶大小、初始嚴重程度和恢復時間結合時，觀察到最高的分類準確度 0.854。我們的研究結果展示了如何結合影像和表格資料以獲得高於中風後的分類準確度，即使在機器學習術語中資料集很小也是如此。我們最後提出如何改善目前的模型，以使用來自醫院掃描器的影像，達到更高層級的準確度。
...
摘要：可解釋人工智慧 (XAI) 已成為處理任務關鍵應用程式時的一項基本需求，確保所使用黑盒子 AI 模型的透明度和可解釋性。XAI 的重要性涵蓋各種領域，從醫療保健到金融，其中了解深度學習演算法的決策制定過程至關重要。大多數基於 AI 的電腦視覺模型通常是黑盒子；因此，在影像處理中提供深度神經網路的可解釋性對於其在醫學影像分析、自動駕駛和遙測應用中的廣泛採用和部署至關重要。最近，已引入幾種用於影像分類任務的 XAI 方法。相反地，影像分割在可解釋性的背景下受到的關注相對較少，儘管它是電腦視覺應用中的一項基本任務，特別是在遙測中。只有少數研究提出用於影像分割的基於梯度的 XAI 演算法。本文採用最近的無梯度 Sobol XAI 方法進行語意分割。為了衡量 Sobol 方法在分割中的效能，我們提出了一種基於可學習雜訊模型的定量 XAI 評估方法。此模型的主要目標是在解釋圖上產生雜訊，其中較高的誘發雜訊表示較低準確度，反之亦然。進行基準分析以評估和比較三種 XAI 方法的效能，包括 Seg-Grad-CAM、Seg-Grad-CAM++ 和 Seg-Sobol，使用提出的基於雜訊的評估技術。這構成了使用高解析度衛星影像執行和評估 XAI 方法的首次嘗試。
...
摘要：大型語言模型在多個領域中快速擴散。然而，由於事實性、連貫性和幻覺等問題，醫療和保健領域對其採用持猶豫態度。鑑於醫療保健的高風險性質，許多研究人員甚至警告在這些問題得到解決之前不要使用它。在醫療保健中實施和部署 LLM 的關鍵是使這些模型值得信賴、透明（儘可能透明）且可解釋。在本文中，我們描述了創建可靠、值得信賴和無偏見模型的關鍵要素，作為其在醫療保健中採用的必要條件。具體來說，我們專注於量化、驗證和減輕醫療保健背景下的幻覺。最後，我們討論了 LLM 在醫療保健中的未來可能是什麼樣子。
...
摘要：人工智慧（AI）進展神速，目前已準備部署在各式各樣的應用程式中，例如自動系統、醫療診斷和自然語言處理。早期採用 AI 技術於實際應用並非沒有問題，特別是神經網路，它可能不穩定且容易受到對抗性範例的影響。從長遠來看，需要開發適當的安全保證技術，以減少因可避免的系統故障而造成的潛在危害，並確保可信度。本文著重於認證和可解釋性，概述了已開發出來用於確保 AI 決策安全的技術，並探討未來的挑戰。
...
摘要：歐洲議會議會研究服務總司為歐洲議會議員準備了一份報告，其中列舉了人工智能 (AI) 在醫學和醫療保健中的七項主要風險：由於 AI 錯誤造成的患者傷害、醫療 AI 工具的誤用、AI 中的偏見和既有失衡的延續、缺乏透明度、隱私和安全問題、問責制差距以及實施障礙。
  在本研究中，我們提出了 AI 系統可能實施的十四項功能性要求，以降低與其醫療目的相關的風險：AI 護照、使用者管理、法規檢查、僅限學術用途免責聲明、資料品質評估、臨床醫師雙重檢查、持續效能評估、稽核追蹤、持續可用性測試、回顧性/模擬案例審查、偏見檢查、可解釋 AI、加密和使用經過實地測試的程式庫以及語意互操作性。
  我們在此的目的是提供技術解決方案的具體高階規格，以確保持續良好的效能，並使用 AI 系統讓患者受益，以符合未來的歐盟法規架構。
...
摘要：人工智慧技術可被用於分類病患的身體活動並預測遠距病患監控的重要生命徵象。基於非線性模型（例如深度學習模型）的回歸分析，由於其黑盒性質，解釋力有限。這可能需要決策者根據非線性模型結果做出盲目的信仰飛躍，特別是在醫療保健應用中。在非侵入性監控中，來自追蹤感測器和其易感臨床屬性的病患資料，作為預測未來生命徵象的輸入特徵。解釋各種特徵對監控應用程式整體輸出的貢獻，對於臨床醫師的決策制定至關重要。在本研究中，提出了一個用於量化分析（QXAI）框架的可解釋 AI，它具有事後模型可解釋性和內在可解釋性，適用於監督式學習方法中的回歸和分類任務。這透過利用 Shapley 值概念並在深度學習模型中納入注意力機制來達成。我們採用人工神經網路（ANN）和基於注意力的雙向 LSTM（BiLSTM）模型，根據感測器資料預測心率和分類身體活動。深度學習模型在預測和分類任務中都達到了最先進的結果。對輸入資料進行全局解釋和局部解釋，以了解各種病患資料的特徵貢獻。所提出的 QXAI 框架使用 PPG-DaLiA 資料來預測心率，並使用行動健康（MHEALTH）資料來根據感測器資料分類身體活動。蒙地卡羅近似法被應用於該框架，以克服 Shapley 值計算所需的時間複雜度和高運算能力需求。
...
摘要：在可解釋人工智慧 (XAI) 研究中，主要重點在於為專家和從業人員詮釋模型。模型不可知論和局部解釋方法被認為在許多應用中具有可解釋性和充分性。然而，在醫療保健等領域，最終使用者是沒有 AI 或領域專業知識的患者，因此迫切需要更易於理解且能建立對模型運作的信任的模型解釋。我們假設生成具有敘述性、針對患者且全面的（模型整體）模型解釋將有助於更好的理解力和決策制定。我們使用決策樹模型來測試這一點，為被認定有冠狀動脈心臟病高風險的患者產生局部和全面解釋。這些解釋會提供給非專家使用者。我們發現對特定類型的解釋有強烈的個人偏好。大多數參與者偏好全面解釋，而較少數的參與者偏好局部解釋。基於任務的這些參與者的心理模型評估提供了有價值的回饋，以增強敘述性全面解釋。這反過來指導設計既值得信賴又可操作的健康資訊系統。
...
摘要：<paragraph>可解釋 AI 在醫學影像中至關重要。在具有挑戰性的神經科學領域中，視覺主題呈現高度的複雜性，特別是在三維空間中。神經科學的應用，包括從 MRI 中識別腦溝特徵，由於專家之間不同的註解協定和腦部複雜的三維功能，面臨重大的障礙。因此，傳統的可解釋性方法無法有效驗證和評估這些網路。為了解決這個問題，我們首先提出一個數學公式，描述了各種電腦視覺任務中不同類型的解釋需求，根據驗證協定的可靠性，將其分類為自解釋、半解釋、非解釋和新模式學習應用。關於這個數學公式，我們提出了一個 3D 可解釋性框架，旨在驗證深度學習網路在偵測腦回旁溝（一個重要的腦部解剖特徵）的輸出。該框架整合了局部 3D 解釋、透過降維的整體解釋、串聯的整體解釋和統計形狀特徵，揭示了模式學習的新見解。我們在具有挑戰性的 TOP-OSLO 資料集上訓練並測試了兩個先進的 3D 深度學習網路，大幅提升了腦溝偵測的準確度，特別是在左半球。在使用此資料集的不同註解協定進行評估期間，我們強調了無偏註解程序在我們提出的 3D 框架中實現精確預測和有效模式學習的關鍵作用。所提出的框架不僅註解了可變腦溝，還揭示了隱藏的 AI 知識，有望促進我們對腦部解剖和功能的理解。</paragraph>
...
摘要：電子健康紀錄 (EHR) 和例行文件編寫實務在病患的日常照護中扮演至關重要的角色，提供全面的健康、診斷和治療紀錄。然而，複雜且冗長的 EHR 敘述會讓醫療保健提供者不堪負荷，進而導致診斷不準確。儘管大型語言模型 (LLM) 已展現其在各種語言任務中的潛力，其在醫療保健領域的應用需要確保將診斷錯誤降到最低並防止病患受到傷害。在本文中，我們概述了一種創新的方法來增強 LLM 在自動化診斷生成領域的熟練度，並透過整合醫學知識圖譜 (KG) 和一種新穎的圖譜模型：Dr.Knows（靈感來自臨床診斷推理過程）來達成。我們從美國國家醫學圖書館的統一醫學語言系統 (UMLS) 中衍生出 KG，這是一個強大的生物醫學知識儲存庫。我們的這種方法否定了預訓練的必要性，而是將 KG 作為輔助工具，協助解釋和總結複雜的醫學概念。使用真實世界的醫院資料集，我們的實驗結果證明，將 LLM 與 KG 結合的建議方法有潛力改善自動化診斷生成的準確性。更重要的是，我們的這種方法提供了一條可解釋的診斷途徑，讓我們更接近實現 AI 擴增的診斷決策支援系統。
...
摘要：現有的用於診斷膝骨關節炎 (OA) 的人工智慧 (AI) 模型因其缺乏透明度和可解釋性而受到批評，儘管它們達到了與醫學專家相似的效能。這種不透明性使得它們難以在臨床實務中獲得信任。最近，可解釋的人工智慧 (XAI) 已成為一種專門技術，它能透過揭示預測的推導方式來提供對模型預測的信心，從而促進在醫療保健中使用 AI 系統。本文提供了用於膝骨關節炎診斷的 XAI 技術的第一份調查。XAI 技術從兩個角度進行討論：資料可解釋性和模型可解釋性。本文的目的是提供有價值的見解，以了解 XAI 在更可靠的膝骨關節炎診斷方法中的潛力，並鼓勵在臨床實務中採用它。
...
摘要：隨著 AI 應用於醫療保健的最新進展，在診斷和疾病預後方面超越人類表現的潛力令人難以置信。然而，隨著 AI 模型越來越複雜，人們開始關注其不透明性、潛在偏差以及對可解釋性的需求。為了確保 AI 系統的信任度和可靠性，特別是在臨床風險預測模型中，可解釋性變得至關重要。可解釋性通常是指 AI 系統向人類利益相關者提供其決策邏輯或決策本身的穩健解釋的能力。在臨床風險預測中，可解釋性的其他方面，例如公平性、偏差、信任和透明度，也代表了超越可解釋性的重要概念。在本綜述中，我們探討了這些概念之間的關係，因為它們經常一起或交替使用。本綜述還討論了開發用於臨床風險預測的可解釋模型的最新進展，強調了在臨床實踐中跨多種常見模式進行定量和臨床評估和驗證的重要性。它強調了外部驗證和結合多樣的可解釋性方法以增強信任和公平性的必要性。採用嚴格的測試，例如使用具有已知生成因素的合成數據集，可以進一步提高可解釋性方法的可靠性。開放獲取和程式碼共享資源對於透明度和可複製性至關重要，能夠促進可解釋性研究的發展和可信度。儘管存在挑戰，但一種從臨床醫生到開發人員都納入利益相關者的端到端可解釋性方法對於臨床風險預測的成功至關重要。
...
摘要：儘管在醫學和醫療保健方面的人工智慧 (AI) 有重大進展，但 AI 技術的部署和採用在現實世界的臨床實務中仍然有限。近年來，人們對與醫療 AI 相關的技術、臨床、倫理和法律風險提出了疑慮。為了增加現實世界的採用率，醫療 AI 工具必須得到患者、臨床醫生、醫療機構和主管機關的信任和接受。這項工作將 FUTURE-AI 指南描述為指導醫療保健中可信賴 AI 工具開發和部署的第一個國際共識架構。FUTURE-AI 聯盟成立於 2021 年，目前由來自 51 個國家/地區的 118 位跨領域專家組成，代表所有大陸，包括 AI 科學家、臨床醫生、倫理學家和社會科學家。在兩年的時間裡，該聯盟通過一個反覆運算的流程定義了可信賴 AI 的指導原則和最佳實務，包括深入的文獻回顧、修正的德爾菲調查和線上共識會議。FUTURE-AI 架構建立在醫療保健中可信賴 AI 的 6 項指導原則之上，即公平性、普遍性、可追溯性、可用性、穩健性和可解釋性。通過共識，定義了一組 28 項最佳實務，涵蓋技術、臨床、法律和社會倫理層面。這些建議涵蓋了醫療 AI 的整個生命週期，從設計、開發和驗證到法規、部署和監控。FUTURE-AI 是一個基於風險且沒有假設的指南，提供了一個結構化的方法來建構醫療 AI 工具，這些工具將在現實世界的實務中獲得信任、部署和採用。鼓勵研究人員在概念驗證階段考慮這些建議，以促進未來醫療 AI 向臨床實務的轉化。
...
摘要：人工智慧在醫學上已經取得重大進展，在醫學影像、病人照護和其他領域都有新興的應用。雖然這些應用在回顧性研究中已被證實是成功的，但實際上應用它們的卻很少。醫學人工智慧領域面臨各種挑戰，包括建立使用者信任、遵守法規、合乎道德地使用資料。可解釋人工智慧 (XAI) 的目標是讓人們理解人工智慧並信任其結果。本文根據近年來發表的 198 篇文章的代表性樣本，對用於醫療決策支援的 XAI 解決方案的最新發展進行文獻回顧。對相關文章的系統性綜合整理得出以下幾項發現。(1) 這些解決方案大多使用與模型無關的 XAI 技術，(2) 深度學習模型的使用多於其他類型的機器學習模型，(3) 可解釋性被用於促進信任，但很少有研究報告醫師參與其中，(4) 視覺化且互動式的使用者介面在理解系統的說明和建議方面更有用。需要更多醫學和人工智慧專家合作進行研究，以指導設計、實作和評估醫學中 XAI 解決方案的合適架構。
...
摘要：體外受精是治療不孕症最廣泛的方法之一。其主要挑戰之一是評估和選擇用於植入的胚胎，這個過程具有很大的臨床醫生之間和臨床醫生內部的變異性。基於深度學習的方法正受到關注，但它們的不透明性質會損害它們在臨床環境中的接受度，在臨床環境中，決策制定中的透明度是關鍵。在本文中，我們分析了 AI 輔助胚胎分析模型的可解釋性的現有工作，找出其中的局限性。我們還討論了如何將這些模型整合到臨床環境中作為決策支持系統，同時考慮臨床醫生和患者的需求。最後，我們提出了提高可解釋性和可信度的準則，推進這項技術朝著既定的臨床實踐邁進。
...
摘要：在需求工程 (RE) 領域中，可解釋人工智慧 (XAI) 在將 AI 支援系統與使用者需求、社會期望和法規標準一致化的重要性日益增加，已獲得認可。一般來說，可解釋性已成為影響系統品質的重要非功能需求。然而，可解釋性和效能之間的假定取捨挑戰了可解釋性的假定正面影響。如果滿足可解釋性的需求需要降低系統效能，那麼必須仔細考慮哪一個品質面向優先，以及如何在它們之間折衷。在本文中，我們批判性地檢視了假定的取捨。我們主張，以一種微妙的方式來處理這件事，這種方式納入了資源可用性、領域特性和風險考量。透過為未來的研究和最佳實務提供基礎，這項工作旨在推進 AI 的 RE 領域。
...
摘要：在需求工程（RE）领域，可解释人工智能（XAI）在将 AI 支持的系统与用户需求、社会期望和监管标准相一致方面的重要性日益提高，已获得认可。一般而言，可解释性已成为影响系统质量的重要非功能性需求。然而，可解释性和性能之间的权衡挑战了可解释性的正面影响。如果满足可解释性的要求需要降低系统性能，那么必须仔细考虑这些质量方面中的哪一个优先，以及如何在它们之间进行权衡。在本文中，我们批判性地研究了所谓的权衡。我们认为，最好以一种细致入微的方式来处理它，其中包含资源可用性、域特征和风险考虑。通过为未来的研究和最佳实践提供基础，这项工作旨在推进人工智能的 RE 领域。
...
摘要：本文批判性地評估了歐盟委員會提出的 AI 法案在風險管理和高風險 AI 系統的風險可接受性方面的作法，這些系統對基本權利和安全構成風險。該法案旨在以相稱的監管負擔來促進「值得信賴」的 AI。其關於風險可接受性的條款要求將高風險系統的殘餘風險降低或消除「儘可能地」，並考慮「最新技術」。這個標準，特別是如果狹義地解釋，是不可行的，既無法促進相稱的監管負擔，也無法促進可信賴性。相比之下，議會對風險管理條款提出的最新修正草案引入了「合理性」、成本效益分析，並且更透明地說明了風險可接受性判斷的價值觀和背景性質。本文認為，議會的作法更可行，並且能更好地平衡相稱性和可信賴性的目標。本文說明了風險可接受性判斷中的合理性將需要什麼，並援引了過失法和歐洲醫療器材法規中的原則。本文主張，風險可接受性判斷的作法需要穩固的公民合法性基礎：包括來自監管機構的詳細指導或參與，以及受影響利益相關者的有意義意見。
...
摘要：可解釋人工智慧 (XAI) 是機器學習中進展迅速的領域，旨在解開複雜模型的預測。XAI 在敏感應用中特別需要，例如在醫療保健中，當診斷、建議和治療選擇可能依賴於人工智慧系統做出的決策時。人工智能方法也已廣泛用於衰老研究，特別是在開發生物時鐘模型和識別衰老和年齡相關疾病的生物標記方面。然而，這裡 XAI 的潛力尚未得到充分重視。我們討論了 XAI 在開發「衰老時鐘」中的應用，並根據對特定生理系統的關注對文獻進行了全面的分析。
...
摘要：零件原型模型是可解釋的圖像分類器，也是黑箱 AI 的有希望的替代品。本文探討了解釋性機器學習（特別是 PIP-Net）在實際醫療影像資料中自動診斷支援的適用性和潛力。PIP-Net 學習人類可理解的原型影像部分，我們評估其在骨折偵測和皮膚癌診斷的準確性和可解釋性。我們發現 PIP-Net 的決策制定過程符合醫療分類標準，同時僅提供影像層級的類別標籤。由於 PIP-Net 的原型無監督預訓練，因此可以輕鬆識別資料品質問題，例如 X 光中的不需要文字或標籤錯誤。此外，我們首次展示人類可以透過直接停用不需要的原型來手動修正 PIP-Net 的推理。我們得出結論，零件原型模型由於其可解釋性和進階模型除錯的潛力，對於醫療應用很有希望。
...
摘要：可解釋 AI (XAI) 是機器學習研究中越來越重要的領域，其目標是讓黑盒模型透明且可解釋。在本文中，我們提出 XAI 的一種新方法，它使用條件排列特徵所產生的反事實路徑。此演算法透過找出最影響模型預測變化的特徵順序排列，來衡量特徵重要性。它特別適合根據包含領域知識的知識圖譜中的反事實路徑產生解釋。反事實路徑為目前的 XAI 方法引入一個額外的圖形維度，用於解釋和視覺化黑盒模型。使用合成資料和醫療資料進行的實驗證明了我們方法的實際適用性。
...
摘要：在人工智能 (AI) 的可解釋性領域中，見證了越來越多的研究和學術興趣。然而，在解釋機器學習演算法結果時，缺乏人性化且個別化的詮釋，顯著阻礙了臨床醫生在研究和臨床實務中接受這些方法。為了解決這個問題，我們的研究使用反事實解釋來探討「如果？」情境在醫學研究中的適用性。我們的目標是擴展我們對磁共振造影 (MRI) 特徵的理解，這些特徵用於診斷小兒後顱窩腦瘤，超越現有的界限。在我們的案例研究中，所提出的概念提供了一種新穎的方法來檢視替代決策情境，提供個人化且特定於情境的見解，從而驗證預測並釐清在不同情況下的差異。此外，我們探討了反事實用於資料擴充的潛在用途，並評估它們作為我們醫學研究案例中一種替代方法的可行性。結果證明了使用反事實解釋來增強臨床研究中以 AI 為主的接受度的潛力。
...
摘要：人工智能領域的進展，導致各種類型由 AI 驅動的失智症評估方式的開發，可運用於辨識失智症早期患者。它可以徹底改變失智症照護的設定。醫療界務必了解各種 AI 評估方式，並考慮其有效性、效率、實用性、可靠性及準確性，以利於對失智症患者 (PwD) 進行早期辨識。另一方面，AI 開發人員應了解各種非 AI 評估方式，以及最近開發的 AI 評估方式。因此，這篇論文可供臨床醫師和 AI 工程師閱讀，彌補文獻中對失智症辨識現有解決方案的說明，以及對 AI 工程師說明所使用的技術和最廣泛的失智症資料集。它遵循探討失智症 AI 和非 AI 評估方式的論文，提供關於各種失智症評估方式的寶貴資訊，供 AI 和醫療界使用。討論與結論重點說明最顯著的研究方向和現有解決方案的成熟度。
...
摘要：可解釋性對人工智慧 (AI) 技術構成一項重大挑戰。目前關於可解釋 AI (XAI) 的研究缺乏提取學習任務的整體知識的效率，因此存在不精確顯著性、與情境無關的缺失和含糊意義等缺陷。在本文中，我們提出類別關聯嵌入 (CAE) 方法來解決這些問題。我們採用編碼器-解碼器架構來嵌入樣本特徵，並同時將它們分為與類別相關和與個體相關的樣式向量。將給定樣本的個體樣式代碼與另一個類別的類別樣式代碼重新組合，會產生一個合成樣本，該樣本保留了個體特徵，但改變了類別分配，遵循循環對抗學習策略。類別關聯嵌入將所有實例的整體類別相關特徵提煉到一個統一的領域中，並在類別之間有良好的區分。然後可以提取不同類別之間的轉換規則，並進一步應用於個別實例。然後，我們提出一個主動 XAI 框架，該框架沿著引導路徑操縱特定樣本的類別樣式向量朝向反類別，從而產生一系列具有相同個體特徵的相反示例合成樣本。將這些反事實樣本與原始樣本進行比較，可以對分類任務的性質提供一個整體、直觀的說明。我們採用該框架對醫學影像分類任務，結果表明，與現有方法相比，可以獲得更精確的顯著性圖，並具有強大的與情境相關的表示。此外，疾病病理學可以直接通過在類別樣式空間中遍歷路徑來視覺化。
...
摘要：提供基於機器學習的 AI 預測的高品質解釋是一項具有挑戰性和複雜性的任務。要順利進行，它需要具備以下條件：選擇適當層級的解釋概括性/特定性；考慮對解釋受益人熟悉 AI 任務的假設；參考促成決策的特定元素；利用額外的知識（例如專家證據），這些知識可能不是預測過程的一部分；並提供支持否定假設的證據。最後，系統需要以清晰可解且可能令人信服的方式表述解釋。基於這些考量，ANTIDOTE 促進了可解釋 AI 的整合願景，其中深度學習過程的低階特徵與人類論證能力的高階方案相結合。ANTIDOTE 將利用深度學習和論證的跨領域能力，支持更廣泛且創新的可解釋 AI 觀點，其中對於臨床案例審議的高品質解釋需求至關重要。作為專案的第一個成果，我們發布了 Antidote CasiMedicos 資料集，以利於研究一般可解釋 AI，以及特別是醫療領域的論證。
...
摘要：<paragraph>近年來，圖神經網路的進展迅速，在藥物發現、醫療診斷和推薦系統方面有許多新發展。雖然進展顯著，但許多網路都是「黑盒子」，對於網路確切學習的「內容」知之甚少。許多高風險應用，例如藥物發現，需要模型提供人類可以理解的解釋，以便使用者能辨識錯誤並發現新知識。因此，可解釋 AI 演算法的開發對我們收穫 AI 的好處至關重要。
  我們提出一個稱為 eXplainable Insight (XInsight) 的 GNN 可解釋性演算法，它使用 GFlowNets 產生模型解釋的分配。由於 GFlowNets 會產生機率與獎勵成正比的物件，因此與僅學習最大獎勵樣本的先前方法相比，XInsight 可以產生多樣化的解釋集。我們透過產生在兩個圖形分類任務中訓練的 GNN 解釋來說明 XInsight：使用 MUTAG 資料集分類致突變化合物，並使用我們已開源的合成資料集分類非環圖。我們透過使用 QSAR 建模分析產生的化合物來顯示 XInsight 解釋的效用，我們發現 XInsight 會產生按親脂性（致突變性的已知相關因素）分群的化合物。我們的結果顯示，XInsight 會產生一系列解釋，揭示模型所展示的底層關係。它們也強調產生多樣化解釋集的重要性，因為它使我們能夠發現模型中的隱藏關係，並為進一步分析提供有價值的指導。</paragraph>
...
摘要：我們提出並實作了一個基於表達式布林公式的可解釋機器學習分類模型，用於可解釋人工智慧 (XAI)。潛在應用包括信用評分和醫療狀況診斷。布林公式定義了一個具有可調整複雜性（或可解釋性）的規則，根據該規則對輸入資料進行分類。此類公式可以包含任何可應用於一個或多個布林變數的運算子，因此與更嚴格的基於規則和基於樹狀的方法相比，提供了更高的表達性。分類器使用原生的局部最佳化技術進行訓練，有效地搜尋可行公式的空間。淺規則可以由快速的整數線性規劃 (ILP) 或二次無約束二元最佳化 (QUBO) 求解器確定，這些求解器可能由特殊用途硬體或量子裝置提供支援。我們透過執行最佳化完整布林公式子樹的非局部移動，結合了原生局部最佳化器的表達性和效率，以及這些裝置的快速運作。我們提供了廣泛的數值基準測試結果，其中包含了幾個在眾所周知的公開資料集上的基準。根據結果，我們發現原生局部規則分類器通常與其他分類器具有競爭力。非局部移動的加入以較少的反覆運算次數達到了類似的結果，因此使用專用或量子硬體可以透過快速提出非局部移動來加速。
...
摘要：為了解決心理健康問題的全球挑戰，我們提出了一種基於邏輯神經網路 (LNN) 的神經符號 AI 方法來診斷心理疾病。由於缺乏對心理疾病的有效治療覆蓋，因此需要一種 AI 解決方案來協助治療師進行診斷。然而，目前的類神經網路模型缺乏可解釋性，治療師可能不信任它們。LNN 是一種遞迴神經網路架構，它結合了神經網路的學習能力和基於經典邏輯的 AI 的推理能力。所提出的系統使用臨床訪談中的輸入謂詞來輸出心理疾病類別，並使用不同的謂詞剪枝技術來實現可擴充性和更高的分數。此外，我們提供了一個見解提取方法來幫助治療師進行診斷。所提出的系統解決了當前神經網路模型缺乏可解釋性的問題，並為心理疾病診斷提供了一個更值得信賴的解決方案。
...
摘要：隨著機器學習模型在醫療診斷中日益普及，對可解釋性和透明度的需求變得至關重要。XAI 復興標誌著該領域的重大轉變，旨在重新定義醫療診斷模型的可解釋性。本文探討了可解釋 AI (XAI) 領域內的創新方法和方法論，這些方法和方法論正在革新醫療診斷模型的可解釋性。通過闡明基礎決策制定過程，XAI 技術使醫療保健專業人員能夠理解、信任並有效地利用這些模型進行準確可靠的醫療診斷。本綜述重點介紹了 XAI 在醫療診斷方面的關鍵進展及其轉變醫療保健領域的潛力，最終改善患者的治療效果並增強對 AI 驅動的診斷系統的信任。
...
摘要：在以高度連通性和流動性為特徵的環境中，再加上心血管疾病的激增，透過遠端監測心血管健康來減少醫療保健支出的必要性變得更加明顯。準確檢測和分類心律不整對於診斷患有心臟不規則的人至關重要。本研究強調了在家中環境中使用心電圖 (ECG) 測量進行實時心律不整檢測的可行性。本論文提出心律不整檢測的新應用，利用尖端的 You-Only-Look-Once (YOLO)v8 演算法對單導程 ECG 訊號進行分類。我們引入了一個新的損失修改 YOLOv8 模型，在 MIT-BIH 心律不整資料集上進行微調，實現實時連續監測。獲得的結果證實了我們方法的有效性，該模型在 NVIDIA Tesla V100 上實現了 99.5% 的平均準確度和 0.992 mAP@50，以及 0.002 秒的快速檢測時間。我們的研究說明了實時心律不整檢測的潛力，使用戶能夠在家中舒適地視覺化解讀模型輸出。此外，本研究為擴展到實時可解釋 AI (XAI) 模型奠定了基礎，該模型能夠部署在醫療保健領域，從而顯著推進醫療保健解決方案的領域。
...
摘要：乳癌 (BC) 仍然是一個重大的健康威脅，目前沒有長期可用的治療方法。早期偵測至關重要，但乳房攝影的判讀受到高偽陽性和偽陰性的阻礙。隨著乳癌發生率預計將超過肺癌，改善早期偵測方法至關重要。熱像攝影使用高解析度紅外線相機，具有前景，特別是在與人工智慧 (AI) 結合使用時。這項工作提出了一個基於注意力的卷積神經網路用於分割，提供更快的速度和更精確的乳癌偵測和分類。該系統增強影像並執行可解釋的 AI 癌症分割。我們提出了一個基於Transformer注意力的卷積架構 (UNet) 進行故障識別，並採用梯度加權類別激活映射 (Grad-CAM) 來分析 UNet 架構中 IRT 影像的偏差和弱點區域。與現有的深度學習框架相比，我們提出的框架的優越性得到證實。
...
摘要：<paragraph>憂鬱症是最普遍且嚴重的精神疾病，會造成嚴重的財務和社會後果。憂鬱症偵測對於早期介入以減輕這些後果至關重要。如此重大的決定本質上需要可解釋性。雖然一些憂鬱症偵測研究嘗試根據重要性分數或注意力權重來解釋決策，但這些解釋與基於憂鬱症狀的臨床憂鬱症診斷標準不一致。為了填補這個缺口，我們遵循計算設計科學範例來開發一種新穎的多尺度時間原型網路 (MSTPNet)。MSTPNet 創新地偵測和解釋憂鬱症狀以及它們持續多久。使用大規模資料集進行的廣泛實證分析顯示，MSTPNet 以 0.851 的 F1 分數優於最先進的憂鬱症偵測方法。這個結果也揭示了問卷調查方法中未注意到的新症狀，例如分享對不同生活的欽佩。我們進一步進行使用者研究以證明其在可解釋性方面優於基準。本研究透過一種新穎的可解釋深度學習模型為社交媒體中的憂鬱症偵測貢獻了 IS 文獻。在實務中，我們提出的方法可以實作在社交媒體平台中，以提供個人化的線上資源給偵測到的憂鬱症患者。</paragraph>
...
摘要：電子健康紀錄 (EHR) 是預想中人工智慧 (AI) 驅動的醫療保健轉型的基本資料來源。
然而，反映在 EHR 備註中的臨床偏見可能導致 AI 模型繼承並放大這些偏見，進而使健康差距持續存在。
本研究調查了 EHR 備註中污名化語言 (SL) 對使用基於 Transformer 的深度學習模型和可解釋 AI (XAI) 技術預測死亡率的影響。
我們的研究結果表明，由臨床醫生撰寫的 SL 對 AI 效能產生負面影響，特別是對黑人患者而言，凸顯了 SL 是 AI 模型開發中種族差異的來源。
為了探索一種運作上有效率的方法來減輕 SL 的影響，我們透過臨床醫生的協作網路調查 SL 產生的模式，並找出核心臨床醫生對 AI 模型中的種族差異影響較大。
我們發現，移除由核心臨床醫生撰寫的 SL 是一種比消除資料庫中所有 SL 更有效率的偏見減少策略。
本研究為負責任的 AI 開發提供了可行的見解，並有助於了解臨床醫生的行為和醫療保健中的 EHR 備註撰寫。
...
摘要：當代透過 AI 的自動化，需要大量的幕後人工勞動，而這些勞動通常既被隱形化，又領取低薪。由於包括標記和維護工作在內的隱形勞動是當代 AI 系統的組成部分，因此讓使用者了解其角色仍然很重要。我們建議透過可解釋 AI (XAI) 設計來達成此目的，特別是女性主義的交叉 XAI。我們提出源自女性主義交叉研究的製圖法，以描繪出 AI 的系統觀點，並包含與隱形勞動相關的 AI 向度。
...
摘要：虛擬心理健康助理 (VMHA) 持續進步，以支援每年有 6,000 萬人次初級照護就診和 600 萬人次急診室 (ER) 就診的負擔過重的全球醫療保健系統。這些系統是由臨床心理學家、精神科醫師和人工智慧 (AI) 研究人員為認知行為療法 (CBT) 所建置。目前，VMHA 的角色是透過資訊提供情緒支持，較少著重於與患者進行反思性對話。需要更全面、安全且可解釋的方法來建置負責任的 VMHA，以提出後續問題或提供見多識廣的回應。這項調查提供了對現有心理健康對話代理的系統性批判性回顧，接著深入探討了 VMHA 在脈絡知識、資料集和其在臨床決策支援中新興角色的改進。我們也提供了新的方向，以透過可解釋性、安全性，以及健全的信賴度來豐富 VMHA 的使用者體驗。最後，我們提供了評量指標和實務考量，以超越現有文獻，在 VMHA 和患者的積極溝通中建立信賴。
...
摘要：可解釋人工智慧（XAI）指的是建構人工智慧應用程式的技術與方法，可協助最終使用者詮釋人工智慧模型的輸出與預測。在高風險決策情境中，例如醫療領域，黑箱人工智慧應用程式已增加透明度與可解釋性的需求，因為錯誤預測可能會造成嚴重後果。模型可解釋性與可詮釋性是人工智慧模型在醫療實務中成功部署的關鍵。人工智慧應用的基本推理需要對臨床醫師透明，才能獲得他們的信任。本文提出醫療領域中 XAI 面向與挑戰的系統性探討。本研究的主要目標是探討各種 XAI 方法、其挑戰，以及相關的醫療保健機器學習模型。這些方法分為六類討論：以特徵為導向的方法、全局方法、概念模型、代理模型、局部像素為基礎的方法，以及以人為中心的方法。最重要的是，本文探討 XAI 在醫療保健問題中的角色，以釐清其在安全關鍵應用中的必要性。本文旨在透過探討相關實驗結果，建立對醫療保健領域中 XAI 相關應用的全面理解。為了促進未來研究填補研究缺口，本文探討 XAI 模型從不同觀點的重要性及其限制。
...
摘要：最先进的机器学习模型通常会学习训练数据中嵌入的虚假相关性。在将这些模型部署到高风险决策制定中时会带来风险，例如在皮肤癌检测等医疗应用中。为了解决这个问题，我们提出了 Reveal to Revise (R2R)，一个包含整个可解释人工智能 (XAI) 生命周期的框架，使从业者能够通过最少的人工交互迭代识别、缓解和（重新）评估虚假的模型行为。在第一步（1）中，R2R 通过找出归因中的异常值或通过检查模型学习的潜在概念来揭示模型弱点。其次（2），检测负责的伪像并在输入数据中进行空间定位，然后利用它（3）修改模型行为。具体来说，我们应用 RRR、CDEP 和 ClArC 的方法进行模型校正，以及（4）重新评估模型的性能和对伪像的剩余敏感性。使用两个用于黑色素瘤检测和骨龄估计的医学基准数据集，我们将我们的 R2R 框架应用于 VGG、ResNet 和 EfficientNet 架构，从而揭示和纠正了真实的数据集内在伪像，以及受控环境中的合成变体。完成 XAI 生命周期，我们演示了多个 R2R 迭代以减轻不同的偏差。代码可在 https://github.com/maxdreyer/Reveal2Revise 中获得。
...
摘要：可解釋人工智慧 (XAI) 領域近年來進展神速，但進展主要是在電腦視覺和自然語言處理方面。對於時間序列，其輸入通常難以解釋，僅有有限的研究探討 XAI。在本文中，我們提出一個虛擬檢查層，將時間序列轉換為可解釋的表示，並允許透過局部 XAI 方法（例如逐層相關性傳播 (LRP)）將相關性歸因傳播到此表示。這樣一來，我們將 XAI 方法家族的適用性擴展到輸入僅在轉換後才能解釋的領域（例如語音）。在此，我們專注於傅立葉轉換，它廣泛應用於時間序列和 LRP 的解釋中，並將我們的方法稱為 DFT-LRP。我們展示了 DFT-LRP 在各種時間序列分類設定中的效用，例如音訊和電子健康紀錄。我們展示 DFT-LRP 如何揭示在不同領域（例如時間域與頻率域）訓練的模型的分類策略差異，或有助於發現模型如何處理資料中的虛假相關性。
...
摘要：解釋深度學習模型預測的能力給最終使用者是一個重要的功能，可用於在醫療決策過程中運用人工智慧 (AI) 的力量，這通常被認為是不透明且難以理解的。在本文中，我們應用最先進的可解釋人工智慧 (XAI) 方法來解釋黑盒 AI 模型在甲狀腺結節診斷應用中的預測。我們提出新的基於統計的 XAI 方法，即核密度估計和密度圖，來解釋未檢測到結節的情況。XAI 方法的效能會在質量和數量比較下被視為回饋，以改善資料品質和模型效能。最後，我們進行調查以評估醫生和患者對 XAI 對甲狀腺結節影像模型決策的解釋的信任度。
...
摘要：<paragraph>醫療設備和人工智慧系統迅速改變醫療保健的提供方式。同時，由於其性質，作為醫療設備中或作為醫療設備的人工智慧可能會暴露於網路攻擊，導致患者安全和安全風險。本章節分為三個部分。第一部分從設定場景開始，我們在其中說明網路安全在醫療保健中的作用。然後，我們簡要定義我們在談論被視為醫療設備本身或支援醫療設備的人工智慧時所指涉的內容。為了說明此類醫療設備帶來的風險，我們提供了三個範例：資料集中毒、社會工程和資料或原始碼萃取。在第二部分，本文概述了歐盟的監管架構，與確保人工智慧作為或在醫療設備中的網路安全有關（醫療器材法規、網路與資訊安全指令、網路安全法、一般資料保護規範、人工智慧法提案和網路與資訊安全 2 指令提案）。最後，本文的第三部分探討了源自歐盟監管架構的潛在挑戰。特別是，我們展望源自兩項立法提案的挑戰，以及它們與現有關於人工智慧醫療設備網路安全的立法之間的互動。它們被結構化為以下問題的答案：(1) 人工智慧法將如何與醫療器材法規在網路安全和安全要求方面互動？(2) 我們應如何詮釋網路與資訊安全 2 指令提案和醫療器材法規中的事件通知要求？(3) 關鍵基礎設施演變條款的後果是什麼？
[這是草稿章節。最終版本將出現在由 Barry Solaiman 和 I. Glenn Cohen 編輯的《健康、人工智慧與法律研究手冊》中，預計於 2023 年由 Edward Elgar Publishing Ltd 出版]</paragraph>
...
摘要：阿茲海默症 (AD) 是一種進行性神經退化性疾病，也是失智症的主要原因。早期診斷對於患者受益於潛在干預和治療至關重要。由於視網膜與大腦的解剖連接，因此假設視網膜是 AD 檢測的診斷部位。為此開發的 AI 模型尚未對決策提供合理的解釋，也沒有推論出疾病進展的階段。沿著這個方向，我們提出了一個新的與模型無關的可解釋 AI 框架，稱為顆粒神經元級別解釋器 (LAVA)，這是一個解釋原型，它探測卷積神經網路 (CNN) 模型的中間層，以直接從視網膜影像評估 AD 連續體，而無需縱向或臨床評估。此方法用於驗證視網膜血管作為阿茲海默症 (AD) 評估的生物標記和診斷方式。英國生物樣本庫的認知測試和血管形態特徵表明，LAVA 在識別整個進程連續體中的 AD 階段方面顯示出強大的前景和有效性。
...
摘要：<paragraph>許多視覺化技術已被開發用於可解釋 AI (XAI)，但它們通常需要使用者進一步推論才能解釋。我們認為 XAI 應支援圖解和演繹推理，讓 AI 執行假設生成和評估，以縮小可解釋性差距。我們提出圖表化，以 i) 執行皮爾士演繹-演繹推理，ii) 遵循領域慣例，以及 iii) 以視覺或口語方式透過圖表解釋。我們實作了 DiagramNet，用於臨床應用程式，以從心臟聽診預測心臟診斷，並以基於形狀的雜音圖表進行解釋。在建模研究中，我們發現 DiagramNet 不僅提供忠實的雜音形狀解釋，而且比基準模型具有更好的預測效能。我們進一步在與醫學生的定性使用者研究中展示了圖解解釋的可解釋性和可信度，顯示出以臨床相關的圖解解釋優於技術顯著性圖解釋。這項工作有助於提供以使用者為中心的 XAI 的領域慣例演繹解釋。</paragraph>
...
摘要：皮膚癌是人類最常見的癌症形式之一。它的主要識別方式是視覺，從臨床篩檢開始，並持續進行皮膚鏡檢查、組織學評估和標本收集。深度卷積神經網路 (CNN) 對分類的細緻物件執行高度區隔且可能是通用的任務。本研究提出一個新的多類別預測架構，它根據 ViT 和 ViTGAN 對皮膚病灶進行分類。基於視覺轉換器的 GAN（生成對抗網路）用於解決類別不平衡。該架構包含四個主要階段：ViTGAN、影像處理和可解釋 AI。第 1 階段包括產生合成影像，以平衡資料集中的所有類別。第 2 階段包括應用不同的資料擴充技術和形態運算，以增加資料量。第 3 和第 4 階段涉及開發一個適用於邊緣運算系統的 ViT 模型，該模型可以識別模式並分類影像中可見於使用者皮膚上的皮膚病灶。在第 3 階段，在使用 ViT 將病灶分類到所需的類別後，我們將使用可解釋 AI (XAI)，它會導致更具可解釋性的結果（使用激活映射等），同時確保高度的預測準確度。皮膚疾病的即時影像可以用相機拍攝，由醫生或患者使用行動應用程式進行早期檢查並確定皮膚病灶的原因。整個架構與現有的皮膚病灶偵測架構進行比較。
...
摘要：<paragraph>在高風險環境中部署人工智慧 (AI) 時，例如醫療保健，提供可解釋性/說明性或允許精細錯誤分析的方法至關重要。許多最近的可解釋性/說明性和精細錯誤分析方法使用概念，這些概念是對人類具有語義意義的元標籤。然而，只有少數數據集包含概念級元標籤，而且這些元標籤大多與不需要領域專業知識的自然影像有關。專注於與單一疾病（例如黑色素瘤）相關的元標籤的醫學密集標註數據集。在皮膚科中，皮膚疾病使用既定的臨床詞彙來描述，讓臨床醫生可以彼此描述身體檢查結果。為了提供由領域專家密集標註的醫學數據集，其中標註對多種疾病過程有用，我們開發了 SkinCon：一個由皮膚科醫生密集標註的皮膚疾病數據集。SkinCon 包含來自 Fitzpatrick 17k 數據集的 3230 張影像，密集標註了 48 個臨床概念，其中 22 個概念至少有 50 張影像代表該概念。所使用的概念是由兩位皮膚科醫生考慮用於描述皮膚病灶的臨床描述詞而選擇的。範例包括「斑塊」、「鱗屑」和「糜爛」。相同的概念也用於標註來自 Diverse Dermatology Images 數據集的 656 張皮膚疾病影像，提供了一個具有多樣化膚色表現的額外外部數據集。我們回顧了 SkinCon 數據集的潛在應用，例如探測模型、基於概念的說明和概念瓶頸。此外，我們使用 SkinCon 來展示這兩個用例：使用概念除錯現有皮膚科 AI 模型的錯誤，以及使用事後概念瓶頸模型開發可解釋的模型。</paragraph>
...
摘要：不安的多臂強盜 (RMAB) 是一個流行的決策理論架構，已被用於模擬公共衛生、野生動物保育、通訊系統等領域中實際的順序決策問題。已部署的 RMAB 系統通常分兩個階段運作：第一個階段預測定義 RMAB 範例的未知參數，而第二個階段採用最佳化演算法來解決已建構的 RMAB 範例。在本文中，我們提供並分析了在公共衛生領域中首次部署 RMAB 系統的結果，旨在改善孕產婦和兒童的健康。我們的分析著重於了解預測準確度和已部署 RMAB 系統的整體效能之間的關係。這對於決定投資於改善預測準確度以提升最終系統效能的價值至關重要，並且有助於診斷和監控已部署的 RMAB 系統。使用我們已部署的 RMAB 系統中的實際資料，我們證明了整體預測準確度的提升甚至可能伴隨著 RMAB 系統效能的下降——廣泛投資資源以提升整體預測準確度可能無法產生預期的結果。在此之後，我們開發了以決策為中心的評估指標來評估預測元件，並表明它更能（在經驗上和理論上）解釋已部署 RMAB 系統的整體效能。
...
摘要：可解釋 AI 是開發機制，允許決策系統和人類互動，讓前者的決策易於理解。這在敏感的脈絡中特別重要，例如在醫療領域。我們提出一個使用案例研究，用於皮膚病變診斷，說明如何向執業醫師提供對最先進深度神經網路分類器決策的解釋，該分類器經過訓練，可以根據範例來描述皮膚病變。我們的架構包含一個訓練好的分類器，解釋模組在該分類器上執行。後者能夠為分類診斷提供正例和反例，從而讓醫師可以與自動診斷系統互動。正例是透過對抗式自動編碼器產生的。我們說明系統在代表性範例中的行為。
...
摘要：<paragraph>人工智慧 (AI) 應用中的演算法公平性對於一個更好的社會至關重要。公平性作為社會機制的基礎公理，包含多個面向。雖然機器學習 (ML) 社群已將焦點放在交叉性作為統計均等問題，特別是在歧視議題上，但新興的文獻探討了另一個面向——單調性。根據領域專業知識，單調性在許多與公平性相關的領域中扮演至關重要的角色，違反單調性可能會誤導人類決策並導致災難性的後果。在本文中，我們首先系統性地評估應用單調神經加法模型 (MNAM) 的重要性，該模型使用公平感知 ML 演算法來強制執行個人和成對單調性原則，以確保 AI 倫理和社會的公平性。我們透過理論推論、模擬和廣泛實證分析的混合方法發現，在所有公平性領域中，包括犯罪學、教育、醫療保健和金融，考量單調性公理至關重要。我們的研究有助於 AI 倫理、可解釋 AI (XAI) 和人機互動 (HCI) 介面的跨領域研究。透過證明不符合單調性的災難性後果，我們探討了單調性需求在 AI 應用中的重要性。此外，我們證明 MNAM 透過施加單調性限制並整合人類智慧，是一種有效的公平感知 ML 方法。</paragraph>
...
摘要：<paragraph>在安全關鍵應用程式中，例如法律或醫療領域，解釋 AI 模型的預測至關重要。對預測的一種解釋形式是萃取依據，亦即引導模型對該實例做出預測的實例特徵子集。例如，子詞組「他偷了手機」可以是「竊盜」預測的萃取依據。之前關於產生萃取依據的研究通常採用兩階段模型：一個選擇器，用於選擇最重要的特徵（即依據），接著是一個預測器，它根據所選特徵做出預測。這些研究的一個缺點是，學習選擇特徵的主要訊號來自將預測器給出的答案與真實答案進行比較。在這項研究中，我們建議透過資訊校正方法從預測器中擠出更多資訊。更精確地說，我們聯合訓練兩個模型：一個是典型的類神經網路模型，它以準確但黑箱的方式解決手邊的任務，另一個是選擇器預測器模型，它另外為其預測產生依據。第一個模型用作第二個模型的指南。我們使用對抗技術校正兩個模型萃取的資訊，使得它們之間的差異成為遺漏或過度選擇的特徵的指標。此外，對於自然語言任務，我們提出一個基於語言模型的正則化器，以鼓勵萃取流暢的依據。情緒分析任務、仇恨言論辨識任務以及法律領域三個任務的實驗結果顯示了我們在依據萃取方面的作法之有效性。</paragraph>
...
摘要：最近認證的人工智慧（AI）醫療保健工具激增，重新引發了關於採用這項技術的爭論。其中一個爭論主題涉及可解釋 AI（XAI）及其承諾讓 AI 裝置更透明且值得信賴。一些活躍於醫療 AI 領域的聲音表達了對可解釋 AI 技術可靠性的疑慮，特別是特徵歸因方法，質疑其在準則和標準中的使用和納入。儘管有合理的疑慮，我們主張對事後局部可解釋性方法的可行性提出現有批評，會因為將特定於影像資料的問題概括化而因噎廢食。我們從將問題描述為解釋與人類理解之間缺乏語義匹配開始。為了了解何時可以可靠地使用特徵重要性，我們在低階和高階特徵的特徵重要性之間引入區別。我們主張對於低階特徵具有明確語義的資料類型，例如電子健康記錄（EHR）等表格資料，可以取得語義匹配，因此特徵歸因方法仍可以有意義且有用的方式使用。最後，我們勾勒出一個程序來測試是否已達成語義匹配。
...
摘要：儘管人工智慧 (AI) 的機器學習 (ML) 模型在醫療領域有傑出的表現，但它們並非沒有錯誤。賦予臨床醫師識別錯誤模型建議的能力對於建立對醫療 AI 的信任至關重要。可解釋 AI (XAI) 旨在透過釐清 AI 推論來支援最終使用者，以滿足此項需求。最近，生物醫學影像的幾項研究獲得了令人滿意的結果。儘管如此，對於使用表格資料的模型而言，其解決方案仍不足以滿足臨床醫師的需求。本文提出了一種方法，以協助臨床醫師找出使用表格資料訓練的 ML 模型的失敗之處。我們的方法建立在三個主要支柱上：利用臨床背景潛在空間分解特徵集、評估全局解釋的臨床關聯性，以及基於潛在空間相似性 (LSS) 的局部解釋。我們在因感染而導致早產兒發病的 ML 辨識上展示了我們的做法。由於模型失敗而導致的死亡風險、終身殘疾和抗生素抗藥性是這個領域中一個公開的研究問題。我們的方法成功找出兩個模型的錯誤分類案例。透過將局部解釋脈絡化，我們的解決方案可為臨床醫師提供可行的見解，以支持他們做出明智的最終決策的自主性。
...
摘要：<paragraph>我們見證了 AI 經濟與社會的興起，其中 AI 技術對醫療保健、商業、運輸和日常生活的許多方面產生越來越大的影響。已經報導了許多 AI 系統甚至超越人類專家準確性的成功案例。然而，AI 系統可能會產生錯誤，可能表現出偏見，可能對資料中的雜訊敏感，而且經常缺乏技術和司法透明度，導致信任度下降和採用上的挑戰。這些最近的缺點和疑慮已在科學和一般媒體中記錄下來，例如自動駕駛汽車事故、醫療保健、招聘和有色人種的面部辨識系統中的偏見、看似正確的醫療決策後來發現是出於錯誤的原因等。這導致許多政府和監管倡議的出現，要求可信賴且符合道德的 AI 提供準確性和穩健性、某種形式的可解釋性、人類控制和監督、消除偏見、司法透明度和安全性。提供可信賴 AI 系統的挑戰促使對可解釋 AI 系統 (XAI) 進行深入研究。XAI 的目標是提供人類可以理解的資訊，說明 AI 系統如何做出決策。在本文中，我們首先簡要總結目前的 XAI 工作，然後挑戰最近關於準確性與可解釋性相互排斥且僅專注於深度學習的論點。然後，我們提出在高風險可信賴 AI 系統交付的全生命週期中使用 XAI 的建議，例如開發、驗證和認證，以及可信賴的生產和維護。</paragraph>
...
摘要：深度神经网络 (DNN) 可检测数据中的模式，并在许多计算机视觉应用中展现出多功能性和强大的性能。然而，DNN 本身容易犯违反简单常识概念的明显错误，并且在利用明确知识来指导其搜索和决策的能力方面受到限制。虽然整体 DNN 性能指标可能不错，但这些明显的错误加上缺乏可解释性，已阻碍了其在关键任务（如医学图像分析）中的广泛采用。本文的目的是介绍 SimpleMind，一个专注于医学图像理解的认知 AI 开源软件框架。它允许创建知识库，以直观的人类可读形式描述图像对象之间的预期特征和关系。SimpleMind 框架通过以下方式为 DNN 带来了思考能力：(1) 提供推理方法，利用知识库对图像内容进行推理，例如空间推理和条件推理，以检查 DNN 输出；(2) 应用过程知识，以通用软件代理的形式，将这些代理链接在一起以完成图像预处理、DNN 预测和结果后处理；以及 (3) 对所有知识库参数进行自动联合优化，以使代理适应具体问题。SimpleMind 支持对多个检测到的对象进行推理以确保一致性，从而在 DNN 输出之间提供交叉检查。这种机器推理通过可解释的模型和可解释的决策提高了 DNN 的可靠性和可信度。提供的示例应用程序演示了 SimpleMind 如何通过将深度神经网络嵌入到认知 AI 框架中来支持和改进它们。
...
摘要：在過去十年中，基於深度學習方法的視覺感知任務取得了驚人的進展，在從自動駕駛到臨床決策支援系統的眾多應用領域中得到廣泛採用。儘管這些基於深度學習的模型表現出色，但它們在決策過程中仍然相當不透明，這使得它們在人類關鍵任務中的部署成為一項有風險的嘗試。這反過來又使得理解這些模型做出的決策對於它們的可靠部署至關重要。可解釋 AI (XAI) 方法試圖通過為這種黑盒深度學習方法提供解釋來解決這個問題。在本文中，我們對計算機視覺中的基於歸因的 XAI 方法進行了全面調查，並回顧了現有的基於梯度、基於擾動和基於對比的 XAI 方法文獻，並對開發和評估健壯的 XAI 方法的關鍵挑戰提供見解。
...
摘要：自 10 年代中期以來，深度學習 (DL) 時代持續至今，每年都帶來新的最高級和創新。儘管如此，這些創新轉化為實際應用的速度仍落後於這種快速步伐。特別是安全關鍵應用程式，其基礎是嚴格的法規和道德要求，需要加以注意，並且仍然是爭論的活躍領域。可解釋 AI (XAI) 和隱私保護機器學習 (PPML) 都是至關重要的研究領域，旨在減輕 DL 中盛行的資料密集型黑盒模型的一些缺點。儘管在各自領域中進行了熱烈的研究活動，但尚未關注它們的互動。這項工作是第一個探討私人學習技術對基於 DL 的模型所產生解釋的影響。在涵蓋來自多個領域的各種影像和時間序列資料集，以及不同的隱私技術、XAI 方法和模型架構的廣泛實驗分析中，研究了私人訓練對產生的解釋的影響。研究結果表明，透過引入隱私，解釋會產生不可忽視的變化。除了報告 PPML 對 XAI 的個別影響之外，本文還對實際應用中技術的選擇提供了明確的建議。透過揭示這些關鍵技術的相互依存性，這項工作是朝著克服安全關鍵領域中實用 AI 的剩餘障礙邁出的第一步。
...
摘要：肺結核（TB）是一種由細菌感染引起的疾病，是導致死亡的主因，特別是在低收入國家，在 2020 年全球約有 1000 萬例新通報病例。雖然肺結核可以治療，但未遵守藥物治療方案是導致發病率和死亡率的重要原因。因此，主動找出有中斷藥物治療方案風險的患者，可以採取矯正措施來減輕不良後果。使用極端不遵守治療的代理測量值和來自印度四個邦約 70 萬名患者的資料集，我們制定並解決了基於自訂排名指標的非遵守治療的早期預測機器學習 (ML) 問題。我們訓練 ML 模型並針對基準進行評估，在考量全國範圍的大規模未來部署後，比基於規則的基準提升約 100%，比隨機分類器提升約 214%。我們在這個過程中處理了各種問題，包括資料品質、高基數分類資料、低目標患病率、分佈轉移、不同群組的差異、演算法公平性，以及對健壯性和可解釋性的需求。我們的研究結果表明，非遵守治療患者的風險分層是一種可行的、可大規模部署的 ML 解決方案。作為印度中央肺結核部門的官方 AI 合作夥伴，我們正在與多個城市和州級試點計畫合作，目標是全印度部署。
...
摘要：可解釋人工智慧 (XAI) 透過提升使用者對機器信任，轉變人工智慧 (AI) 領域。隨著連網裝置數量持續增加，物聯網 (IoT) 市場需要讓使用者信賴。然而，現有文獻仍缺乏系統性和全面的調查工作，探討 XAI 在物聯網中的應用。為了解決這個問題，我們在這篇論文中探討 XAI 架構，重點關注其特性與對物聯網的支援。我們說明廣泛使用的 XAI 服務，應用於物聯網應用程式，例如安全性提升、醫療物聯網 (IoMT)、工業物聯網 (IIoT) 和城市物聯網 (IoCT)。我們也建議在這些應用程式中，透過適當的範例，在物聯網系統中實作 XAI 模型，並總結出對未來工作的關鍵推論。此外，我們展示邊緣 XAI 架構的尖端發展，以及第六代 (6G) 通訊服務對物聯網應用程式的支援，並提出關鍵推論。簡而言之，這篇論文構成第一個針對 XAI 基礎架構發展的整體彙編，專為未來物聯網使用案例的需求量身打造。
...
摘要：每年在美国大约有 125 万人因烧伤接受治疗。精确的烧伤分类是医学人工智能领域的一个重要方面。在这项工作中，我们提出了一种可解释的人在回路框架，用于改进烧伤超声分类模型。我们的框架利用基于 LIME 分类解释器的解释系统来证实和整合烧伤专家的知识——提出新特征并确保模型的有效性。使用此框架，我们发现可以通过提供纹理特征来增强 B 模式超声分类器。更具体地说，我们确认基于超声帧的灰度共生矩阵 (GLCM) 的纹理特征可以提高转移学习烧伤深度分类器的准确性。我们在猪受试者的真实数据上测试了我们的假设。我们展示了烧伤深度分类的准确性提高——从约 88% 提高到约 94%——一旦根据我们的框架修改。
...
摘要：在醫療保健中，AI 系統支援臨床醫生和患者進行診斷、治療和監測，但許多系統的解釋能力不佳，對實際應用來說仍然具有挑戰性。克服此障礙是可解釋 AI (XAI) 的目標。然而，每個人對解釋的理解可能不同，因此無法為所有人解決黑盒問題。以人為中心的 AI 領域會透過調整 AI 以適應使用者來處理此問題。我們提出以使用者為中心的擬人概念來評估 XAI，並使用它來調查行動健康壓力監測應用程式中，最終使用者對各種解釋風格和內容的偏好。我們的線上調查結果顯示，使用者的背景資料和個性，以及解釋類型，會影響解釋偏好，這表示這些是 XAI 設計的基本特徵。我們將結果歸納為三種典型的使用者擬人：權力導向、休閒導向和注重隱私的使用者。我們的見解讓互動式、以人為中心的 XAI 更接近實際應用。
...
摘要：青光眼是造成部分或完全失明的原因中排名第二，主要發生在眼睛因焦慮或憂鬱症造成過度壓力，進而損害視神經並導致視力出現問題。傳統青光眼篩檢是一項耗時的程序，需要醫療專業人員持續關注，即便如此，偶爾仍會因時間限制和壓力而無法正確分類，導致治療錯誤。已經有許多努力試圖自動化整個青光眼分類程序，然而，這些現有模型通常具有黑箱特性，使用戶無法了解預測背後的主要原因，因此醫療從業人員通常無法依賴這些系統。在本文中，我們在比較過各種預先訓練的模型後，提出了一個能夠以 94.71% 的準確度分類青光眼的轉移學習模型。此外，我們利用了局部可解釋模型不可知解釋 (LIME)，為我們的系統引入了可解釋性。這項改良讓醫療專業人員能夠取得重要且全面的資訊，協助他們做出判斷。它也降低了傳統深度學習模型的不透明性和脆弱性。
...
摘要：深度學習在偵測物件和分類影像上已展現卓越的表現，確保了分析醫學影像的一大承諾。
將深度學習的成功轉化為醫學影像，其中醫生需要了解其背後過程，需要有能力詮釋和說明神經網路的預測。深度神經網路的可解釋性通常依賴於估計輸入特徵（例如，像素）相對於結果（例如，類別機率）的重要性。然而，已經開發出許多重要性估計器（也稱為顯著性圖），但目前尚不清楚哪一種與醫學影像應用較為相關。在目前的研究中，我們使用三個不同的評估指標，探討了幾個重要性估計器在解釋卷積深度網路對電腦斷層掃描 (CT) 影像分類的效能。首先，模型中心保真度衡量在某些輸入受到擾動時模型準確度的下降。其次，透過接受者操作特徵 (ROC) 曲線在像素層級衡量重要性評分與專家定義的分割遮罩之間的一致性。第三，我們透過 Dice 相似係數 (DSC) 衡量基於 XRAI 的圖與分割遮罩之間的區域重疊。整體而言，兩個版本的 SmoothGrad 在保真度和 ROC 排名中名列前茅，而整合梯度和 SmoothGrad 則在 DSC 評估中表現出色。有趣的是，模型中心（保真度）和以人為中心（ROC 和 DSC）評估之間存在著關鍵差異。分割圖中嵌入的專家期望和直覺並不一定與模型如何得出其預測相符。了解這種可解釋性的差異將有助於利用深度學習在醫學上的力量。
...
摘要：深度學習方法，尤其是卷積神經網路，已成為醫學影像運算任務中強大的工具。雖然這些複雜模型提供了優異的效能，但其黑箱性質可能會阻礙在高風險決策制定中採用於現實世界。在本文中，我們提出一個互動系統，利用最先進的可解釋性技術來協助放射科醫師進行乳癌篩檢。我們的系統將深度學習模型整合到放射科醫師的工作流程中，並提供新穎的互動方式，以促進對模型決策過程的理解。此外，我們證明我們的系統可以逐步利用使用者互動，提供更細緻的可解釋性報告，而標記成本很低。由於所採用的可解釋性技術的通用性質，我們的系統與領域無關，可用於許多不同的醫學影像運算任務，提供了一個新的觀點，說明我們如何利用視覺分析將原本靜態的可解釋性技術轉變為擴增人類決策制定和促進採用醫學人工智慧。
...
摘要：如今，人工智能 (AI) 已成为医疗保健应用的基础组件，包括临床和远程医疗，但性能最佳的人工智能系统通常过于复杂，无法自我解释。可解释人工智能 (XAI) 技术被定义为揭示系统预测和决策背后的推理，在处理敏感和个人健康数据时，它们变得更加关键。值得注意的是，XAI 并未在不同的研究领域和数据类型中引起相同的关注，尤其是在医疗保健领域。特别是，许多临床和远程医疗应用分别基于表格数据和时间序列数据，而 XAI 并不常见于这些数据类型，而计算机视觉和自然语言处理 (NLP) 是参考应用。为了概述最适合医疗保健领域表格数据和时间序列数据 XAI 方法，本文回顾了最近 5 年的文献，说明了生成解释的类型以及为评估其相关性和质量所做的努力。具体来说，我们将临床验证、一致性评估、客观和标准化质量评估以及以人为本的质量评估确定为确保最终用户有效解释的关键特征。最后，我们重点介绍了该领域的的主要研究挑战以及现有 XAI 方法的局限性。
...
摘要：<paragraph>目標：本研究評估了市售可解釋的人工智慧演算法在增強臨床醫師辨識胸部 X 光片 (CXR) 上肺癌的能力方面的影響。
設計：這項回溯性研究評估了 11 位臨床醫師在偵測胸部 X 光片的肺癌表現，有無市售人工智慧演算法 (紅點，Behold.ai) 的協助，該演算法可預測 CXR 上疑似肺癌。臨床醫師的表現與臨床確診進行評估。
設定：本研究分析了英國國家醫療服務體系 (NHS) 醫院的匿名患者資料；資料集包含 400 張成人患者 (18 歲以上) 的胸部 X 光片（2020 年執行 CXR），以及對應的臨床文字報告。
參與者：由 11 位臨床醫師（顧問放射科醫師、放射科醫師受訓者和報告放射技師）組成的讀者小組參與本研究。
主要結果指標：臨床醫師在 CXR 上偵測肺癌的整體準確度、敏感度、特異度和精確度，有無人工智慧輸入。臨床醫師之間的同意率和表現標準差也進行評估，有無人工智慧輸入。
結果：臨床醫師使用人工智慧演算法可改善肺部腫瘤偵測的整體表現，整體增加 17.4% 的肺癌在 CXR 上被辨識出來，否則將會遺漏，整體增加較小腫瘤的偵測，分別增加 24% 和 13% 的第一期和第二期肺癌偵測，並標準化臨床醫師的表現。
結論：本研究顯示人工智慧演算法在改善早期肺癌診斷和透過整體改善讀者的表現促進健康公平性方面具有很大的臨床應用前景，且不影響下游影像資源。</paragraph>
...
摘要：在 AI 驅動的醫療保健中，缺乏足夠大的開放式醫療資料庫是最大的挑戰之一。使用生成對抗網路 (GAN) 所建立的合成資料，看來是能減輕隱私政策問題的良好解決方案。另一種治療方式是跨越多個醫療機構的分散式協定，而不交換本地的資料樣本。在本文中，我們在集中式和分散式設定中探索了無條件和條件 GAN。集中式設定模擬了大型但高度不平衡的皮膚病變資料集的研究，而分散式設定模擬了具有三個機構的更實際的醫院場景。我們根據保真度、多樣性、訓練速度和在生成的合成資料上訓練的分類器的預測能力來評估模型的效能。此外，我們透過探索潛在空間和嵌入投影來提供可解釋性，重點放在全局和局部解釋上。計算出的真實影像與其在潛在空間中的投影之間的距離，證明了訓練過的 GAN 的真實性和概化性，這是此類應用程式的主要考量之一。已公開執行研究的開源程式碼於
\url{https://github.com/aidotse/stylegan2-ada-pytorch}。
...
摘要：在醫院的世界裡有許多複雜的組合問題，而解決這些問題對於提升病患滿意度和照護品質非常重要。醫療保健中的問題很複雜，因為要解決它們，必須考慮到各種限制和不同類型的資源。此外，必須在短時間內評估解決方案，以確保在實際情況中可用。我們計畫提出解決這類問題的方法，包括擴充已經測試過的方法，以及針對新問題建模解決方案，同時考慮文獻並在有資料時使用實際資料。解決這類問題非常重要，但由於歐盟委員會制定了《一般資料保護規範》，規定每個人都有權要求說明 AI 做出的決定，因此在未發展出可解釋性方法的情況下，基於 AI 的求解器（例如基於答案集程式設計的求解器）的使用將受到限制。因此，研究的另一部分將致力於研究並提出說明所獲得解決方案的新方法。
...
摘要：在現代動態且持續發展的社會中，越來越多的人飽受慢性且嚴重的疾病所苦，而醫生和患者需要特別且精密的醫療保健支持。因此，傑出的醫療保健利害關係人已意識到開發此類服務的重要性，以讓患者的生活更輕鬆。此類支持需要收集大量的患者複雜資料，例如臨床、環境、營養、日常活動、來自智慧穿戴式裝置的各種資料、來自配備感測器的服裝的資料等。全面的患者資料必須適當彙總、處理、分析，並提供給醫生和照護者，以便建議適當的治療和措施，以改善患者與健康相關的參數和整體健康狀況。先進的人工智慧技術提供了分析此類大資料、使用它們並衍生新知識的機會，以支持個人化的醫療決策。基於先進機器學習、聯邦學習、遷移學習、可解釋人工智慧等新方法為未來更優質地使用健康和醫療資料開闢了新途徑。在本文中，我們將提出一些關鍵面向和特徵範例，說明人工智慧方法在個人化醫療決策方面的應用範圍。
...
摘要：<paragraph>持續監控已訓練的 ML 模型，以確定何時應信任其預測，何時不應信任，這對於安全部署至關重要。此類架構應具有高性能、可解釋性、事後性和可操作性。我們提出 TRUST-LAPSE，一種用於持續模型監控的「不信任」評分架構。我們使用一系列潛在空間嵌入來評估每個輸入範例模型預測的可信度。具體來說，(a) 我們的潛在空間不信任評分使用潛在空間中的距離度量（馬氏距離）和相似性度量（餘弦相似性）估計不信任，(b) 我們的順序不信任評分確定過去輸入表示順序中相關性的偏差，採用非參數、基於滑動視窗的演算法進行可操作的持續監控。我們透過兩個下游任務評估 TRUST-LAPSE：(1) 分佈式偏移輸入偵測，以及 (2) 資料漂移偵測。我們使用公開資料集評估各種領域（音訊和視覺），並進一步在具有挑戰性的真實世界腦電圖 (EEG) 資料集上對癲癇偵測方法進行基準測試。我們的潛在空間不信任評分獲得最先進的結果，AUROC 分別為 84.1（視覺）、73.9（音訊）和 77.1（臨床 EEG），比基準高出 10 分以上。我們揭露了流行基準中的嚴重缺陷，這些缺陷對輸入語義內容不敏感，使其不適合進行真實世界的模型監控。我們表明，我們的順序不信任評分可實現高漂移偵測率；所有領域中超過 90% 的串流顯示 < 20% 的誤差。透過廣泛的定性和定量評估，我們表明我們的誤信評分更可靠，並提供可解釋性，以便輕鬆採用實務。</paragraph>
...
摘要：機器學習的普及提高了不公平模型在高風險應用程式（例如司法系統、藥物/疫苗設計和醫療診斷）中部署的風險。儘管有從頭訓練公平模型的有效方法，但如何自動揭露和解釋訓練模型的不公平性仍然是一項具有挑戰性的任務。以可解釋的方式揭露機器學習模型的不公平性是邁向公平且值得信賴的 AI 的關鍵一步。在本文中，我們系統性地解決了通過挖掘可解釋證據 (RUMIE) 來揭露不公平模型的新任務。其關鍵思想是以一群模型最常區分的資料實例形式找到確鑿證據。為了使證據可解釋，我們還找到了一組人類可理解的關鍵屬性和決策規則，這些屬性和決策規則描述了被區分的資料實例，並將其與其他未被區分的資料區分開來。正如在許多實際資料集上進行的廣泛實驗所證明的，我們的模型找到了高度可解釋且確鑿的證據，以有效揭露訓練模型的不公平性。此外，它比所有基準方法的可擴展性高得多。
...
摘要：可解釋 AI (XAI) 在安全關鍵系統中是必要的，例如在臨床診斷中，因為存在致命決策的高風險。然而，目前 XAI 像是方法的鬆散集合，而不是定義良好的流程。在這項工作中，我們闡述了 XAI 最大子群體可解釋機器學習 (IML) 與古典統計之間的概念相似性。根據這些相似性，我們提出了 IML 的形式化，沿著統計過程的線條。採用這種統計觀點讓我們能夠將機器學習模型和 IML 方法解釋為複雜的統計工具。根據這種解釋，我們推論出三個關鍵問題，我們將其視為 IML 在安全關鍵設置中成功和採用的關鍵。通過制定這些問題，我們進一步旨在引發關於 IML 與古典統計之間的區別以及我們的觀點對該領域未來的影響的討論。
...
摘要：人工智慧已廣泛應用於各學科和領域，
生物醫學影像和訊號處理也不例外。對此主題日益增長且廣泛的興趣引發了大量的研究活動，這反映在指數級的研究成果中。透過研究龐大且多元的生物醫學資料，機器和深度學習模型已徹底改變了建模、分割、配準、分類和合成等各種任務，其效能優於傳統技術。然而，難以將結果轉化為生物學/臨床可解讀的資訊，這阻礙了它們在該領域的充分應用。可解釋人工智慧 (XAI) 嘗試透過提供方法來讓模型可解讀並提供說明，以填補此轉譯鴻溝。迄今為止已提出不同的解決方案，並獲得社群日益增長的興趣。本文旨在提供生物醫學資料處理中 XAI 的概觀，並指出 IEEE Signal Processing Magazine 即將於 2022 年 3 月發行的生物醫學影像和訊號處理深度學習特刊。
...
摘要：在使用醫學影像進行診斷時，無論是臨床醫生或人工智慧 (AI) 系統，影像的高品質都非常重要。當影像品質不佳時，產生該影像的醫療檢查通常需要重新進行。在遠距醫療中，一個常見的問題是，品質問題只會在病患離開診所後才被標示出來，這表示他們必須回來重新進行檢查。這對居住在偏遠地區的人來說特別困難，而這些人佔了巴西一家數位醫療保健組織 Portal Telemedicina 的病患中很大一部分。在本文中，我們報告了關於 (i) 開發一種 AI 系統用於標示和解釋低品質醫療影像的正在進行中的工作、(ii) 一項訪談研究，以了解在 OurCompany 使用 AI 系統的利害關係人的解釋需求，以及 (iii) 一項縱向使用者研究設計，用於檢查在我們的診所中，將解釋納入技術人員的工作流程中會產生的影響。據我們所知，這將是第一個關於評估 XAI 方法對最終使用者（使用 AI 系統但沒有特定 AI 專業知識的利害關係人）的影響的縱向研究。我們歡迎對我們的實驗設置提供意見回饋和建議。
