# arxiv-daily
 Automated deployment @ 2024-05-04 20:53:16 Asia/Taipei
...
摘要：專有的 LM（例如 GPT-4）通常用於評估各種 LM 的回應品質。然而，包括透明度、可控性和負擔能力在內的疑慮強烈促使開發專門用於評估的開源 LM。另一方面，現有的開放式評估器 LM 表現出嚴重的缺點：1) 它們發出的分數與人類指定的顯著不同，2) 它們缺乏執行直接評估和成對排名（兩種最普遍的評估形式）的彈性。此外，它們不具備根據自訂評估標準進行評估的能力，而是專注於一般屬性，例如有益性和無害性。為了解決這些問題，我們引入了 Prometheus 2，這是一款比其前身更強大的評估器 LM，它密切反映了人類和 GPT-4 的判斷。此外，它能夠處理直接評估和成對排名格式，並與使用者定義的評估標準分組。在四個直接評估基準和四個成對排名基準中，Prometheus 2 在所有受測的開放式評估器 LM 中，與人類和專有 LM 評審的分數相關性和一致性最高。我們的模型、程式碼和資料都公開於 https://github.com/prometheus-eval/prometheus-eval。
...
摘要：大型語言模型 (LLM) 已被證明有能力執行長期機器人任務的高層級規劃，但現有方法需要存取預先定義的技能庫 (例如：拾取、放置、拉動、推動、導航)。然而，LLM 規劃並未說明如何設計或學習這些行為，這在長期設定中仍然具有挑戰性。此外，對於許多感興趣的任務，機器人需要能夠以細微的方式調整其行為，這需要代理能夠修改低層級控制動作。我們是否能改用 LLM 中的網際網路規模知識來制定高層級政策，指導強化學習 (RL) 政策以有效解決機器人控制任務，而不需要預先確定的技能組？在本文中，我們提出 Plan-Seq-Learn (PSL)：一種模組化方法，它使用運動規劃來彌合抽象語言和學習的低層級控制之間的差距，以從頭開始解決長期機器人任務。我們證明 PSL 在超過 25 項具有挑戰性的機器人任務上取得了最先進的成果，其中最多有 10 個階段。PSL 從原始視覺輸入中解決長期任務，涵蓋四個基準，成功率超過 85%，優於基於語言、傳統和端到端的方法。影片結果和程式碼請見 https://mihdalal.github.io/planseqlearn/
...
摘要：概念瓶頸模型 (CBM) 將影像分類建立在人類可理解的概念上，以允許模型決策具有可解釋性。
至關重要的是，CBM 設計本質上允許人類介入，其中專家使用者有能力修改潛在的錯誤概念選項，以可解釋的方式影響模型的決策行為。然而，現有方法通常需要大量的每張影像人類介入才能達到強勁的表現，在取得人類回饋很昂貴的情況下，這會造成實際上的挑戰。在本文中，我們發現這顯著地受到介入過程中概念的獨立處理所驅動，其中一個概念的改變並不會影響模型最終決策中其他概念的使用。為了解決這個問題，我們引入一個可訓練的概念介入重新調整模組，它利用概念關係在介入後重新調整概念分配。在標準的真實世界基準中，我們發現概念重新調整可以顯著地改善介入效果；顯著地減少達到目標分類效能或概念預測精確度所需的介入次數。此外，它可以輕鬆地整合到現有的基於概念的架構中，而不需要改變模型本身。這種降低的人類模型協作成本對於在資源受限的環境中提升 CBM 的可行性至關重要。
...
摘要：對齊是一種微調預訓練大型語言模型 (LLM) 的標準程序，用於遵循自然語言指令並作為有用的 AI 助理。然而，我們觀察到，傳統的對齊過程無法增強 LLM 的事實準確性，而且經常導致產生更多虛假事實（即幻覺）。在本文中，我們探討如何使 LLM 對齊過程更具事實性，首先找出導致對齊步驟中產生幻覺的因素：監督微調 (SFT) 和強化學習 (RL)。特別是，我們發現訓練 LLM 學習新知識或不熟悉的文本會助長幻覺。這使得 SFT 較不具事實性，因為它訓練的是人類標記的數據，這些數據對 LLM 來說可能是新穎的。此外，標準 RL 中使用的獎勵函數也會助長幻覺，因為它引導 LLM 對各種指令提供更多有用的回應，通常偏好較長且更詳細的回應。基於這些觀察，我們提出了具有事實意識的對齊，包括具有事實意識的 SFT 和透過直接偏好最佳化而具有的事實意識 RL。實驗顯示，我們提出的具有事實意識的對齊引導 LLM 輸出更多的事實回應，同時維持遵循指令的能力。
...
摘要：對於深度學習分類和基礎模型，對未見資料的概化仍然知之甚少。如何評估網路在小樣本學習、分佈外概化和領域適應精神下適應其輸入空間的新版本或擴充版本的能力？網路的哪些層可能最能概化？我們提供一種新的方法來評估網路表示取樣領域的能力，無論網路是否已針對該領域中的所有類別進行訓練。我們的做法如下：在特定領域對視覺分類的最新預訓練模型進行微調後，我們評估它們在該領域相關但不同的變異資料上的效能。概化能力被量化為來自中介層的未見資料潛在嵌入函數，適用於無監督和監督設定。在網路的所有階段中，我們發現 (i) 高分類準確度並不意味著高可概化性；以及 (ii) 模型中的較深層並非總是能最佳概化，這對剪枝有影響。由於在各個資料集觀察到的趨勢在很大程度上是一致的，我們得出結論，我們的做法揭示了模型不同層的內在概化能力（函數）。
...
摘要：已經提出了各種對齊語言模型的方法，包括監督微調、RLHF 和直接最佳化方法，例如 DPO。
儘管 DPO 因其簡潔的訓練過程和競爭力的結果而迅速獲得歡迎，但仍有一個未解決的問題，即是否仍有使用判別器（例如獎勵模型）來評估回應的實際優點。我們提出了 D2PO，判別器引導的 DPO，一種在整個學習過程中收集偏好的線上設定方法。
當我們收集黃金偏好的同時，我們不僅使用它們來訓練我們的政策，還要訓練一個判別式回應評估模型，以對更多合成資料進行白銀標籤，以便進行政策訓練。我們在各種任務中探索這種方法，包括現實的聊天設定，我們發現與具有相同資料預算的 DPO 相比，我們的做法會產生更高品質的輸出，並且在偏好資料需求方面更有效率。此外，我們展示了白銀標籤最有助益的條件：它在使用 DPO 訓練政策時最有效，優於傳統的 PPO，並且受益於維持一個與政策模型分開的判別器。
...
摘要：傳統上，自然語言處理 (NLP) 模型通常使用由語言專業知識建立的豐富功能集，例如語義表示。然而，在大語言模型 (LLM) 的時代，越來越多的任務轉變為通用的端到端序列生成問題。在本文中，我們探討這個問題：在 LLM 時代，語義表徵扮演什麼角色？具體來說，我們探討抽象意義表徵 (AMR) 對五項不同的 NLP 任務的影響。我們提出一個 AMR 驅動的思考鏈提示方法，我們稱之為 AMRCoT，並發現它通常會損害效能，而不是幫助它。為了探討 AMR 在這些任務上可能提供的功能，我們進行一系列的分析實驗。我們發現很難預測 AMR 可能有助於或損害哪些輸入範例，但錯誤往往會出現在多字詞表達式、命名實體以及 LLM 必須將其對 AMR 的推理與其預測聯繫起來的最後推論步驟中。我們建議在未來針對 LLM 的語義表徵工作中專注於這些領域。我們的程式碼：
https://github.com/causalNLP/amr_llm。
...
摘要：儘管大多數可控文字生成的研究都專注於引導基礎語言模型，新興的指令調整和提示範例提供了另一種可控性的方法。我們編制並發布了 ConGenBench，一個包含 17 項不同可控生成任務的測試平台，使用其中一個子集來評量 9 種不同基準和方法在指令調整語言模型上的效能。令人驚訝的是，我們發現基於提示的方法在多數資料集和任務上都優於可控文字生成方法，這凸顯了針對特定指令調整語言模型的可控文字生成研究需求。基於提示的方法在多數風格化任務上與人類表現相符，但在結構化任務上則落後，這凸顯了研究更多樣化的限制和更具挑戰性的風格化任務的需求。為了促進此類研究，我們提供了一種演算法，它僅使用任務資料集和具備情境能力的大型語言模型來自動產生限制資料集。此方法消除了對預先整理限制資料集的依賴，因而大幅擴展了未來可以研究的限制範圍。
...
摘要：近年來，出現了許多大型多模態模型 (LMM) 來有效解決單一影像視覺語言任務。然而，它們解決多影像視覺語言任務的能力仍有待改進。現有的多影像 LMM（例如 OpenFlamingo、Emu、Idefics 等）大多透過預先在網路上數億個雜訊交錯影像文字資料上進行訓練來獲得其多影像能力，既不有效率也不有效。在本文中，我們旨在透過使用學術級別資源的指令調整來建立強大的多影像 LMM。因此，我們精心建構了 Mantis-Instruct，其中包含來自 14 個多影像資料集的 721K 個實例。我們設計 Mantis-Instruct 來涵蓋不同的多影像技能，例如共同參考、推理、比較、時間理解。我們將 Mantis-Instruct 與幾個單一影像視覺語言資料集結合起來，訓練我們的模型 Mantis 來處理任何交錯的影像文字輸入。我們在五個多影像基準和八個單一影像基準上評估訓練後的 Mantis。儘管僅需要學術級別的資源（即 16xA100-40G 上的 36 小時），Mantis-8B 仍可以在所有多影像基準上達到最先進的效能，並以平均 9 個絕對點數擊敗現有的最佳多影像 LMM Idefics2-8B。我們觀察到 Mantis 在保留和排除評估基準上的表現同樣良好。我們進一步在單一影像基準上評估 Mantis，並證明 Mantis 可以維持與 CogVLM 和 Emu2 相當的強大單一影像效能。我們的結果特別令人鼓舞，因為它表明低成本指令調整在建構多影像 LMM 方面確實比密集預訓練有效得多。
...
摘要：將大型語言模型 (LLM) 與人類價值觀和偏好保持一致，對於讓它們發揮幫助和安全的作用至關重要。然而，建立有效率的工具來執行對齊可能會是一項挑戰，特別是對於包含數十億或數百億個參數、最大且最具能力的 LLM。我們建立了 NeMo-Aligner，這是一個模型對齊工具包，可以有效率地擴展到使用數百個 GPU 來進行訓練。NeMo-Aligner 隨附針對模型對齊的主要範例進行高度最佳化且可擴展的實作，例如：人類回饋強化學習 (RLHF)、直接偏好最佳化 (DPO)、SteerLM 和自玩微調 (SPIN)。此外，我們的工具包支援在參數有效微調 (PEFT) 設定中執行大部分對齊技術。NeMo-Aligner 旨在具備可擴充性，允許以最小的工作量支援其他對齊技術。它採用 Apache 2.0 授權開源，我們歡迎社群在 https://github.com/NVIDIA/NeMo-Aligner 提供貢獻
...
摘要：大型视觉语言模型 (VLM) 已在需要对文字图像和文本进行细致理解的任务中展现出强大的推理能力，例如视觉问答或视觉蕴涵。然而，当模型面对包含隐喻或幽默等比喻现象的图像和标题时，对于这些模型的能力尚未有太多探讨，而这些现象的含义通常是隐含的。为了弥补这一差距，我们提出了一个新任务和一个高质量数据集：具有文本解释的视觉比喻语言理解 (V-FLUTE)。我们将视觉比喻语言理解问题构建为一个可解释的视觉蕴涵任务，其中模型必须预测图像（前提）是否蕴涵一个主张（假设），并用文本解释为预测的标签提供依据。使用人机协作框架，我们构建了一个高质量数据集 V-FLUTE，其中包含 6,027 个 <图像、主张、标签、解释> 实例，涵盖五种多模态比喻现象：隐喻、明喻、习语、讽刺和幽默。比喻现象可以出现在图像、标题或两者中。我们进一步进行自动和人工评估，以评估当前 VLM 在理解比喻现象方面的能力。
...
摘要：聊天機器人，如 GPT-4 和 ChatGPT，現在服務數百萬用戶。儘管廣泛使用，但仍缺乏公開的數據集來展示這些工具在實務中如何被使用者使用。為了彌補這個差距，我們提供免費使用 ChatGPT 給線上使用者，以換取他們同意匿名收集他們的聊天記錄和請求標頭。由此，我們編譯了 WildChat，一個包含 100 萬個使用者與 ChatGPT 對話的語料庫，其中包含超過 250 萬個互動回合。我們將 WildChat 與其他流行的使用者與聊天機器人互動數據集進行比較，發現我們的數據集提供了最多樣化的使用者提示，包含最多的語言，並為研究人員提供了最豐富的潛在有毒使用案例。除了帶有時間戳記的聊天記錄外，我們還使用人口統計資料（包括州、國家和雜湊 IP 地址）以及請求標頭豐富了數據集。這種擴充允許對不同地理區域和時間維度中的使用者行為進行更詳細的分析。最後，由於它涵蓋了廣泛的使用案例，我們展示了該數據集在微調遵循說明的模型中的潛在效用。WildChat 在 https://wildchat.allen.ai 下根據 AI2 ImpACT 許可證發布。
...
摘要：AI 基礎模型在各種應用中獲得廣泛使用，包括放射學等醫療領域。然而，醫療基礎模型通常在有限的任務上進行測試，其概括性和偏差尚未得到探索。我們提出 RayDINO，一個大型視覺編碼器，通過對 873k 胸部 X 光片的自監督進行訓練。我們將 RayDINO 與以前最先進的模型進行比較，涵蓋從分類和密集分割到文本生成的九項放射學任務，並對我們模型的人口、年齡和性別偏差進行深入分析。我們的研究結果表明，自監督允許以患者為中心的 AI 在臨床工作流程中發揮作用，並全面解釋 X 射線。通過使用 RayDINO 和小任務特定適配器，我們達到了最先進的結果，並改善了對未見人群的概括，同時減輕了偏差，說明了基礎模型的真正前景：多功能性和魯棒性。
...
摘要：預先訓練好的對比式視覺語言模型已展現出在各種任務上的顯著效能。然而，它們在預訓練期間未適當呈現類別的精細訓練資料集上常常會遇到困難，這使得適應變得必要。最近的研究已透過利用來自網路規模資料庫的樣本來進行檢索增強適應，特別是在低資料量模式下，顯示出有前途的結果。儘管有實證上的成功，了解檢索如何影響視覺語言模型的適應仍然是一個開放的研究問題。在這項工作中，我們透過提出一個系統性研究來了解檢索增強適應中關鍵組成的角色，採用一種反思觀點。我們揭示了關於單模態和跨模態檢索的新見解，並強調了邏輯集成為有效適應的關鍵角色。我們進一步提出了直接支援我們實證觀察的理論基礎。
...
摘要：這篇論文介紹了 UQA，這是一個針對烏爾都語問答和文本理解的新穎資料集，烏爾都語是一種擁有超過 7000 萬名母語使用者的低資源語言。UQA 是透過翻譯史丹佛問答資料集 (SQuAD2.0) 所產生，SQuAD2.0 是一個大型的英文問答資料集，翻譯時使用了一種稱為 EATS（包圍到錨點、翻譯、搜尋）的技術，這個技術保留了翻譯後脈絡段落中的答案範圍。這篇論文描述了在兩個候選者中選擇和評估最佳翻譯模型的過程：Google 翻譯和 Seamless M4T。這篇論文還對 UQA 上的幾個最先進的多語言問答模型進行了基準測試，包括 mBERT、XLM-RoBERTa 和 mT5，並報告了有希望的結果。對於 XLM-RoBERTa-XL，我們的 F1 分數為 85.99，EM 為 74.56。UQA 是用於開發和測試烏爾都語多語言 NLP 系統以及增強現有模型的跨語言可傳輸性的寶貴資源。此外，這篇論文展示了 EATS 在為其他語言和領域建立高品質資料集方面的有效性。UQA 資料集和程式碼公開於 www.github.com/sameearif/UQA。
...
摘要：在本文中，我們探討了將運算創意 (CC) 與大型語言和視覺模型 (LLVM) 研究整合的方法，以解決這些模型的一個關鍵限制，即創造性問題解決。我們展示了初步實驗，說明如何將 CC 原理應用於透過擴增提示來解決此限制。透過這項工作，我們希望促進在 LLVMs 中運用 ML 演算法進行創造性問題解決的背景下，對運算創意的討論。我們的程式碼位於：https://github.com/lnairGT/creative-problem-solving-LLMs
...
摘要：本文探討使用大型語言模型 (LLM) 自動生成硬體描述碼，旨在探索其在支援和提升高效神經形態運算架構開發的潛力。基於我們先前的研究，我們使用 OpenAI 的 ChatGPT4 和自然語言提示來合成一個可程式化遞迴脈衝神經網路的 RTL Verilog 模組，同時也產生測試平台來評估系統的正確性。產生的設計已在三個案例研究中得到驗證，分別是異或、鳶尾花分類和 MNIST 手寫數字分類，準確率高達 96.6%。為了驗證其可合成性和可實作性，該設計已在現場可程式閘陣列上製作原型，並使用開源電子設計自動化流程在 SkyWater 130 nm 技術上實作。此外，我們已將其提交給 Tiny Tapeout 6 晶片製造計畫，以進一步評估系統未來在晶片上的效能。
...
摘要：大型 2D 視覺語言模型 (2D-LLM) 透過使用簡單的投影機將大型語言模型 (LLM) 與影像連結起來，因而備受矚目。受到其成功的啟發，大型 3D 點雲語言模型 (3D-LLM) 也將點雲整合到 LLM 中。但是，將點雲直接與 LLM 對齊需要昂貴的訓練成本，通常在 A100 上需要數百個 GPU 小時，這阻礙了 3D-LLM 的發展。在本文中，我們介紹了 MiniGPT-3D，這是一個高效且強大的 3D-LLM，在一個 RTX 3090 上僅訓練 27 小時就達到了多個 SOTA 結果。具體來說，我們建議使用來自 2D-LLM 的 2D 先驗來將 3D 點雲與 LLM 對齊，這可以利用 2D 和 3D 視覺資訊之間的相似性。我們介紹了一種新穎的四階段訓練策略，以串聯方式進行模態對齊，以及一個混合查詢專家模組，以高效率自適應地彙總特徵。此外，我們利用參數高效微調方法 LoRA 和 Norm 微調，導致只有 4780 萬個可學習參數，比現有方法少了 260 倍。大量的實驗表明，MiniGPT-3D 在 3D 物件分類和標題任務上達到了 SOTA，訓練成本顯著降低。值得注意的是，與 ShapeLLM-13B 相比，MiniGPT-3D 在具有挑戰性的物件標題任務上獲得了 GPT-4 評分 8.12 的提升，而後者在 8 個 A800 上總共耗費了 160 個 GPU 小時。我們是第一個探索高效 3D-LLM 的人，為社群提供了新的見解。程式碼和權重可在 https://github.com/TangYuan96/MiniGPT-3D 取得。
...
摘要：經食道心臟超音波檢查 (TEE) 在心臟科的診斷和介入性手術中扮演著關鍵角色。然而，由於影像取得和詮釋的複雜性，要有效使用它需要廣泛的訓練。為了提升新手超音波檢查員的效率並減少掃描取得的差異性，我們提出一個基於對比學習作為目標條件強化學習 (GCRL) 的新型超音波 (US) 導航輔助方法。我們使用一種新型的對比患者分組方法 (CPB) 和一個資料擴充的對比損失來擴充先前的架構，我們證明了這兩者對於確保對不同患者的解剖變異進行概化至關重要。所提出的架構能使用單一模型導航到標準診斷和複雜的介入性影像。我們的技術是使用 789 位患者的大型資料集開發的，在 140 位患者的測試資料集上取得了平均 6.56 毫米的位置誤差和 9.36 度的角度誤差，這與在個別影像上訓練的模型相比具有競爭力或優於它們。此外，我們定量驗證了我們的方法導航到介入性影像（例如 LAA 封閉中使用的左心耳 (LAA) 影像）的能力。我們的做法有望在經食道超音波檢查期間提供有價值的指導，有助於提升心臟超音波操作員的技能習得。
...
摘要：自然語言解釋已成為評估可解釋和多步驟自然語言推理 (NLI) 模型的代理。然而，評估 NLI 解釋的有效性具有挑戰性，因為它通常涉及適當數據集的群眾外包，這是一個耗時且容易發生邏輯錯誤的過程。為了解決現有局限性，本文探討了通過整合大型語言模型 (LLM) 和定理證明器 (TP) 來驗證和改進自然語言解釋。具體來說，我們提出了一個名為 Explanation-Refiner 的神經符號框架，該框架使用 LLM 擴充 TP 以生成和形式化解釋性句子，並建議 NLI 的潛在推理策略。反過來，TP 用於提供對解釋的邏輯有效性的正式保證，並為後續改進提供反饋。我們展示了如何將 Explanation-Refiner 聯合用於評估解釋性推理、自動形式化和最先進 LLM 的錯誤校正機制，以及自動提高不同領域中人類註釋解釋的質量。
...
摘要：在口語中，發音精確度降低很常見，但對於對話而言，其聲學特性和語用功能鮮少被研究。我們在此嘗試彌補這個缺口。這份技術報告包含了期刊文章（Ward 等人，2024 年，已提交）中遺漏的內容。具體來說，我們在此報告 1) 標記感知到的簡化的經驗教訓，2) 發現與朗讀不同，對話中簡化的相關因素包括高音高、音高範圍寬和強度，以及 3) 使用簡單的聲學/韻律特徵來預測對話中簡化的基準模型，與英語的人類感知相關性達到 0.24，與西班牙語相關性達到 0.17。我們還提供了英語中簡化其他可能的語用功能範例，以及各種討論、觀察和推測
...
摘要：大型機器，例如粒子加速器，通常由經驗豐富的操作員團隊運行。對於粒子加速器，這些操作員具備加速器物理和構成機器技術的背景知識。由於機器複雜，由專家負責特定子系統，操作員可以求助於這些專家。在這項工作中，推理和行動 (ReAct) 提示範例用於將開放權重大型語言模型 (LLM) 與高級機器控制系統框架和其他工具（例如電子日誌或機器設計文件）結合起來。藉此實施多專家檢索增強生成 (RAG) 系統，協助操作員進行知識檢索任務，在需要時直接與機器互動，或撰寫高級控制系統腳本。這種專家知識和機器互動的整合，可以簡化和加速新舊操作員的機器操作任務。
...
摘要：彌合大型語言模型在英文和非英文表現上的顯著差距是一項重大挑戰。雖然有些先前的研究嘗試利用翻譯的訓練資料來縮小這個差距，但最近提出的問題比對方法利用了模型的英文專長，以最少使用昂貴且容易出錯的翻譯來提升多語言表現。在本文中，我們探討了這個方法可以廣泛應用到什麼程度，方法是檢視它在可執行程式碼推理和常識推理中的效果。我們也探討如何使用代理調整，將這個方法有效應用到極大型語言模型。在多語言推理基準 mGSM、mSVAMP 和 xCSQA 上的實驗結果顯示，問題比對方法可用於提升各種推理情境、模型系列和規模的多語言表現。例如，當應用到 LLaMA2 模型時，即使使用 70B 模型，我們的模型也能讓 mGSM 的平均準確度提升 12.2%。為了了解成功機制，我們分析了表徵空間、思考鏈和翻譯資料規模，揭示了問題翻譯訓練如何強化 LLM 內部的語言比對，並形塑其工作模式。
...
摘要：本研究介紹了一個創新的 AI 驅動精準農業系統，利用 YOLOv8 進行疾病辨識，並利用檢索擴充生成 (RAG) 進行情境感知診斷。專注於解決影響卡納塔克邦咖啡產業的疾病挑戰，該系統整合了先進的物件偵測技術與語言模型，以解決與大型語言模型 (LLM) 相關的固有約束。我們的技術不僅解決了 LLM 中的幻覺問題，還引入了動態疾病識別和補救策略。即時監控、協作資料集擴充和組織參與確保了該系統在不同農業環境中的適應性。建議系統的效果不僅限於自動化，還旨在確保糧食供應、保護生計和促進生態友善的農業實務。透過精確的疾病識別，該系統有助於永續且注重環保的農業，減少對農藥的依賴。展望未來，該專案預見了 RAG 整合物件偵測系統的持續開發，強調可擴充性、可靠性和可用性。本研究致力於成為農業正面變革的燈塔，與全球朝向永續且技術提升的糧食生產的努力相一致。
...
摘要：對遞迴尖峰神經網路 (RSNN) 進行程式設計，以穩健地執行多時間尺度運算仍然是一項艱難的挑戰。為了解決這個問題，我們展示了向量符號架構 (VSA) 所提供的分散式方法，它使用高維隨機向量作為表示的最小單位，如何能被利用來將穩健的多時間尺度動態嵌入基於吸引子的 RSNN 中。我們透過疊加對稱自聯想權重矩陣和非對稱轉換項，將有限狀態機嵌入 RSNN 動態中。轉換項是由輸入的 VSA 結合和狀態之間的非對聯想外積所形成的。我們的做法已透過具有高度非理想權重的模擬、一個實驗性的閉迴路記憶電阻硬體設定，以及 Loihi 2，在其中它能無縫地擴展到大型狀態機，得到驗證。這項工作展示了 VSA 表示在將穩健運算與遞迴動態嵌入神經型態硬體中的有效性，而不需要參數微調或大量的特定平台最佳化。這促使 VSA 成為神經型態硬體中認知演算法的高階表示不變抽象語言。
...
摘要：大型語言模型 (LLM) 已成為各種自然語言任務和一系列應用領域中強大的支援工具。最近的研究專注於探索其在資料註釋方面的能力。本文提供了一份比較綜述，探討了十二項研究，這些研究調查了 LLM 在標記資料方面的潛力。雖然這些模型展示出有望節省成本和時間的優點，但仍存在相當大的限制，例如代表性、偏差、對提示變化的敏感性以及對英語的偏好。利用這些研究的見解，我們的實證分析進一步探討了人類和 GPT 生成的意見分佈在四個主觀資料集之間的一致性。與探討代表性的研究相反，我們的研究方法直接從 GPT 獲取意見分佈。因此，我們的分析支援少數在評估資料註釋任務時考慮不同觀點的研究，並強調需要朝這個方向進行進一步的研究。
...
摘要：本文探討使用混合 CTC/注意力編碼器-解碼器模型，並使用中間 CTC (InterCTC) 訓練愛爾蘭語 (蓋爾語) 低資源語音辨識 (ASR) 和方言辨識 (DID)。結果與目前訓練 ASR (TDNN-HMM) 和 DID (ECAPA-TDNN) 的最佳效能模型進行比較。最初使用 Conformer 編碼器建立最佳 InterCTC 設定。然後使用此設定訓練具有 E-branchformer 編碼器的模型，並比較兩種架構的效能。採用多任務微調方法進行語言模型 (LM) 淺層融合。實驗得出 DID 準確度相較於基準 ECAPA-TDNN 提升 10.8%，而 WER 效能接近 TDNN-HMM 模型。這種多任務方法成為愛爾蘭語低資源 ASR 和 DID 的有望策略。
...
摘要：非自迴歸 (NAR) 語言模型以其在神經機器翻譯 (NMT) 中的低延遲而聞名。然而，由於解碼空間大且難以準確捕捉目標詞彙之間的依存關係，因此 NAR 和自迴歸模型之間存在效能差距。此外，為 NAR 模型準備適當的訓練資料是一項不平凡的任務，通常會加劇曝光偏差。為了應對這些挑戰，我們將強化學習 (RL) 應用於 Levenshtein Transformer，這是一個基於編輯的 NAR 代表性模型，證明使用自我生成資料的 RL 可以增強基於編輯的 NAR 模型的效能。我們探討了兩種 RL 方法：逐步獎勵最大化和情節獎勵最大化。我們討論了這兩種方法各自的優缺點，並對它們進行實證驗證。此外，我們透過實驗探討溫度設定對效能的影響，確認適當溫度設定對於 NAR 模型訓練的重要性。
...
摘要：許多研究顯示，現有的臉部辨識系統 (FRS)，包括商業系統，經常對某些種族表現出偏見，因為資料代表性不足。在這項工作中，我們探討種族改變和膚色修改，使用合成臉部影像產生方法來增加資料集的多樣性。我們首先建構一個平衡的臉部影像資料集，代表三個種族：亞洲人、黑人和印度人，進行詳細分析。然後我們使用現有的生成對抗網路 (GAN) 影像到影像轉換和流形學習模型，將一個種族改變為另一個種族。進一步進行系統分析，使用個別類型角 (ITA) 研究逼真的膚色表現，評估此類資料集是否適合 FRS。此外，我們還使用現有的臉部影像品質評估 (FIQA) 方法分析品質特徵。然後我們使用四個不同的系統提供整體 FRS 效能分析。我們的研究結果為未來的研究工作鋪路，包括：(i) 開發特定種族和一般 (任何到任何) 種族改變模型，(ii) 擴展此類方法來建立具有不同膚色的資料庫，(iii) 建立代表各種種族的資料集，進一步有助於在解決隱私問題的同時減輕偏見。
...
摘要：本文介绍了一种专为自动驾驶设计的轨迹预测模型，重点在于在不依赖于高精度地图的情况下捕捉动态交通场景中的复杂交互。该模型称为 MFTraj，它利用历史轨迹数据结合了一种新颖的基于动态几何图的行为感知模块。在它的核心，一个自适应结构感知交互式图卷积网络捕捉了道路使用者的位置和行为特征，保留了时空的复杂性。该模型通过线性注意力机制得到增强，实现了计算效率和减少的参数开销。在 Argoverse、NGSIM、HighD 和 MoCAD 数据集上的评估突出了 MFTraj 的鲁棒性和适应性，即使在数据受限的场景中，它也优于许多基准，而不需要额外的信息，如高清地图或矢量化地图。重要的是，即使在数据缺失严重的情况下，它也能保持有竞争力的性能，与大多数现有的最先进模型相当。结果和方法表明，自动驾驶轨迹预测取得了重大进展，为更安全、更高效的自动驾驶系统铺平了道路。
...
摘要：自然語言推論 (NLI)，也稱為識別文本蘊涵 (RTE)，是自然語言理解的一個重要面向。
現在大多數研究使用機器學習和深度學習在特定資料集上執行此任務，表示其解決方案無法說明或明確。
為了滿足 RTE 可說明方法的需求，我們提出一個新的管道，其基礎是將文字翻譯成抽象意涵表示 (AMR) 圖形。為此，我們使用預先訓練的 AMR 剖析器。然後我們將 AMR 圖形翻譯成命題邏輯，並使用 SAT 求解器進行自動化推理。在文字中，通常常識會暗示前提和主張之間存在蘊涵（或矛盾）關係，但由於使用了不同的措辭，因此無法從其邏輯表示中識別出來。為了解決這個問題，我們引入了放鬆方法，允許替換或遺忘一些命題。我們的實驗結果顯示，此管道在四個 RTE 資料集上表現良好。
...
摘要：提示工程对于利用大型语言模型 (LLM) 的潜力至关重要，尤其是在使用专业术语和措辞的医学领域。然而，提示工程在医学领域的功效仍有待探索。在这项工作中，回顾了 114 项最近的研究（2022-2024 年），这些研究在医学中应用了提示工程，涵盖提示学习 (PL)、提示调整 (PT) 和提示设计 (PD)。PD 最为普遍（78 篇文章）。在 12 篇论文中，PD、PL 和 PT 术语可以互换使用。ChatGPT 是使用最广泛的 LLM，有七篇论文使用它来处理敏感的临床数据。思维链成为最常见的提示工程技术。虽然 PL 和 PT 文章通常为评估基于提示的方法提供基准，但 64% 的 PD 研究缺乏与提示无关的基准。我们提供了总结现有工作的表格和图表，并报告了指导未来研究贡献的建议。
...
摘要：我們提出 TRAMBA，一種混合Transformer和 Mamba 架構，用於聲學和骨傳導語音增強，適用於行動和穿戴式平台。由於以下幾個原因，骨傳導語音增強在行動和穿戴式平台上並不切實際：(i) 資料收集非常耗費人力，導致稀缺；(ii) 記憶體佔用數百 MB 的最先進模型與更適合資源受限系統的方法之間存在效能差距。為了讓 TRAMBA 適應基於振動的感測模式，我們使用廣泛可用的音訊語音資料集預先訓練 TRAMBA。然後，使用者使用少量骨傳導資料進行微調。TRAMBA 在 PESQ 中比最先進的 GAN 高出 7.3%，在 STOI 中高出 1.8%，記憶體佔用量小一個數量級，推理速度快 465 倍。我們將 TRAMBA 整合到實際系統中，並展示 TRAMBA (i) 透過減少資料取樣和傳輸，將穿戴式裝置的電池續航力提升 160%；(ii) 在嘈雜的環境中產生比無線語音更高的品質；(iii) 記憶體佔用量不到 20.0 MB。
...
摘要：大型語言模型 (LLM) 在各種任務中取得顯著成功，但它們仍然容易受到對抗性攻擊，特別是記錄完善的「越獄」攻擊。最近，貪婪坐標梯度 (GCG) 攻擊已證明其有效利用此漏洞，透過結合梯度啟發法和貪婪搜尋來最佳化對抗提示。然而，此攻擊的效率已成為攻擊過程中的一個瓶頸。為了減輕此限制，我們在本文中透過最佳化視角重新思考對抗提示的產生，旨在穩定最佳化流程並利用先前反覆運算中的更多啟發式見解。具體來說，我們引入了\textbf{M}omentum \textbf{A}ccelerated G\textbf{C}G (\textbf{MAC}) 攻擊，它將動量項納入梯度啟發法中。實驗結果展示了 MAP 在對齊語言模型的基於梯度的攻擊中所取得的顯著增強。我們的程式碼可在 https://github.com/weizeming/momentum-attack-llm 取得。
...
摘要：論證結構學習 (ASL) 涉及預測論證之間的關係。由於它可以建構文件以利於理解，因此已廣泛應用於許多領域（醫療、商業和科學領域）。儘管其用途廣泛，ASL 仍然是一項具有挑戰性的任務，因為它涉及檢驗潛在非結構化論述中句子之間的複雜關係。為了解決這個問題，我們開發了一種簡單但有效的方法，稱為 ASL 任務的雙塔多尺度卷積神經網路 (DMON)。具體來說，我們將論證組織成關係矩陣，該矩陣與論證嵌入一起形成關係張量，並設計一種機制來捕捉與上下文論證的關係。在三個不同領域論證挖掘資料集上的實驗結果證明，我們的框架優於最先進的模型。程式碼可在 https://github.com/VRCMF/DMON.git 取得。
...
摘要：強化學習策略通常以黑箱神經網路表示，這種網路不可解釋且不適合安全關鍵領域。為了解決這兩個問題，我們提出受約束正規化流策略，作為可解釋且建構安全策略模型。我們針對具有立即安全約束的強化學習問題達成安全性，我們可以利用領域知識，透過分析建構正規化流來確保約束滿足。正規化流對應於動作範例中一系列可解釋的轉換，每個轉換都確保相對於特定約束的對齊。我們的實驗揭示了可解釋性以外的好處，在於更容易的學習目標，並在整個學習過程中維持約束滿足。我們的做法利用獎勵工程的約束，同時提供增強的可解釋性、安全性，以及向代理提供領域知識的直接方法，而無需依賴複雜的獎勵函數。
...
摘要：邊緣運算允許人工智慧和機器學習模型部署在邊緣裝置上，在該處，它們可以從當地資料中學習並協作以形成一個全球模型。聯合學習 (FL) 是一種分散式機器學習技術，它在維護資料隱私的同時促進此程序。然而，FL 也面臨挑戰，例如關於資源受限裝置的高運算和通訊成本，以及由於邊緣用戶端間資料異質性和存在非分配資料而導致的概化效能不佳。在本文中，我們提出梯度一致性引導聯合稀疏訓練 (FedSGC)，這是一種將動態稀疏訓練和梯度一致性檢查整合到聯合學習架構中的新方法，以解決這些問題。我們的這項方法利用了這樣一個觀念：其中關聯梯度與全局模型相對於衝突方向的神經元包含不相關或較不概化的資訊，並可以在稀疏訓練程序中進行剪枝。相反地，關聯梯度具有相同方向的神經元可以優先增長。透過這種方式，FedSGC 可以大幅減少本地運算和通訊開銷，同時增強 FL 的概化能力。我們在具有挑戰性的非 i.i.d 設定上評估我們的這項方法，並展示它在各種情境下獲得與最先進的 FL 方法相競爭的準確度，同時將運算和通訊成本降至最低。
...
摘要：訓練深度神經網路 (DNN) 從有雜訊的標籤中學習，是一項重要且具挑戰性的任務。然而，現有的方法大多關注於損壞的標籤，而忽略了固有資料結構的重要性。為了彌補有雜訊的標籤和資料之間的差距，我們受到物理學中位能概念的啟發，提出了一個用於雜訊標籤學習的新穎位能混合模型 (PEMM)。我們創新了一個基於距離的分類器，在其類別中心上進行位能正則化。將我們提出的分類器嵌入現有的深度學習主幹中，我們可以擁有具有更好特徵表示的穩健網路。它們可以保留資料中的內在結構，從而產生優異的雜訊容忍度。我們進行了廣泛的實驗，以分析我們提出的模型在幾個真實世界資料集上的效率。定量結果表明，它可以實現最先進的效能。
...
摘要：自訓練是深度學習中一種強大的方法。關鍵過程是找到一個偽標籤進行建模。然而，先前的自訓練演算法會受到硬標籤所帶來的過度自信問題，即使某些與信心相關的正則化程式也無法全面捕捉不確定性。因此，我們提出一個新的自訓練架構，以結合模型和資料集的不確定性資訊。具體來說，我們建議使用期望最大化 (EM) 來平滑標籤並全面估計不確定性資訊。我們進一步設計一個基礎提取網路，以從資料集估計初始基礎。獲得的不確定性基礎可以根據不確定性資訊進行過濾。然後，它可以轉換為真正的硬標籤，以在重新訓練過程中反覆更新模型和基礎。在影像分類和語意分割方面的實驗顯示，我們的演算法在有信心的自訓練演算法中具有優勢，在不同的資料集上提高了 1-3 個百分點。
...
摘要：本文介紹了 TartuNLP 團隊在 2024 年 EvaLatin 共享任務中，針對歷史拉丁文文本的情緒極性檢測所提交的內容。我們的系統依賴於兩種不同的方法來標註監督式學習的訓練資料：1) 採用組織者提供的極性詞彙表，建立基於啟發法的標籤；2) 使用 GPT4 產生標籤。我們採用參數高效微調，使用適配器架構，並在訓練語言和任務適配器時實驗單語種和跨語言知識轉移。我們使用 LLM 產生的標籤所提交的內容，在情緒極性檢測任務中獲得總體第一名。我們的結果顯示，基於 LLM 的標註在拉丁文中顯示出有希望的結果。
...
摘要：異常偵測 (AD) 是一個常在工業環境中需要的關鍵流程。異常可以表示系統中的潛在問題，並促使進一步調查。工業流程旨在盡可能簡化操作，涵蓋最終產品的生產，使異常偵測成為達成此目標的必要手段。傳統的異常偵測方法通常將觀測值分類為正常或異常，而不會提供這些分類背後的原因。因此，鑑於工業 5.0 的出現，一種更理想的方法是提供可解釋的結果，使用戶能夠理解結果背後的原理。本文介紹了 ExIFFI 的第一個工業應用，這是一種最近開發的方法，專注於為擴展隔離森林 (EIF) 異常偵測方法產生快速且有效的解釋。ExIFFI 在兩個公開的工業資料集上進行測試，證明了其在解釋和運算效率方面優於其他最先進的可解釋 AD 模型。
...
摘要：<paragraph>在現場 X 光影像擷取中精準偵測和追蹤導管等裝置是心血管內科介入治療的基本先決條件。此資訊用於程序指導，例如引導支架置放。為了確保程序安全和有效性，需要高度穩健性，追蹤過程中不能有故障。為達成此目的，需要有效應對挑戰，例如：對比劑或其他外部裝置或導線遮蔽裝置、視野或擷取角度變更，以及心臟和呼吸運動造成的持續移動。為了克服上述挑戰，我們提出一個新穎的方法，以自監督方式從超過 1600 萬個介入性 X 光影像序列資料中學習時空特徵。我們的做法基於遮罩影像建模技術，該技術利用基於插補重建的框架來學習精細的幀間時間對應。結果模型中編碼的特徵會在後續進行微調。我們的做法達到了最先進的效能，特別是與經過高度最佳化的參考解決方案（使用多階段特徵融合、多任務和流動正規化）相比，具有穩健性。實驗顯示，我們的做法在最大追蹤誤差方面比參考解決方案降低了 66.31%（使用流動正規化時降低了 23.20%）；在 42 幀/秒的 3 倍快推論速度下達到了 97.95% 的成功率（在 GPU 上）。這些結果鼓勵在介入影像分析的各種其他任務中使用我們的做法，這些任務需要有效理解時空語義。</paragraph>
...
摘要：這項研究探討主觀歸因意義構成意識經驗的現象內容的假設。也就是說，現象內容是語義的。這種主觀意義的形式表現在質覺的內在和非表徵特徵中。經驗上，主觀意義普遍存在於意識經驗中。我們指出現象學研究提供了支持此說法的證據。此外，這個意義概念與弗雷格在形上學和語言哲學中所稱的「意義」密切相關。它也與皮爾斯的符號學中的「詮釋項」一致。我們探討弗雷格的意義如何也能延伸到意識的原始感覺。意義和指涉都在現象經驗中發揮作用。此外，在心物關係的脈絡中，我們提供了與個人心智表徵相關的主觀意義的公式化。透過辨識物理和心智領域之間的精確對應，我們主張句法和語義結構超越語言，並在這些領域中實現。形式上，意義是一種關係屬性，透過一個對應來實現，該對應在適當的語義空間中詮釋形式系統的句法結構。這個對應在心智領域中的影像與經驗相關，因此構成質覺的現象內容。我們以這個假設可能對基於經驗的意識理論的影響作為結論。
...
摘要：積極參與對話是建立共識的關鍵，
因為理解是由生產者和接收者共同調整的。
旁聽者被剝奪了執行接地動作的特權，只能猜測預期含義。儘管如此，數據生成和註解、
NLP 對話模型的建模、訓練和評估依賴於旁聽範式。有多少基礎接地過程
因此被沒收？正如我們所展示的，有證據表明不可能使用數據驅動學習
模型正確建模人類元溝通行為。在本文中，我們討論了這個問題並提供了一個初步分析
關於人類請求澄清的決策變異性。最重要的是，我們希望將這個話題帶回社區的討論桌上，
鼓勵討論僅「聽取」設計的模型的後果。
...
摘要：現有的用於建立以來源為基礎的資訊搜尋對話資料集的方法通常成本高昂且難以執行，因為它們完全依賴於人工註解員。我們建議結合大型語言模型 (LLM) 提示和人類專業知識，以進行更有效率且可靠的資料產生。我們使用 LLM 產生來模擬兩個角色，而不是勞力密集的綠野仙蹤 (WOZ) 方法，在該方法中，兩個註解員從頭開始產生對話、扮演代理和使用者的角色。然後，註解員驗證輸出並使用歸因資料對其進行擴充。我們透過建構 MISeD（會議資訊搜尋對話資料集）來展示我們的這種方法，這是第一個專注於會議記錄的資訊搜尋對話資料集。使用 MISeD 微調的模型在我們的測試集中，以及在一個新的完全手動 WOZ 測試集中和一個現有的基於查詢的摘要基準中，都展示出優異的效能，這表明我們方法的效用。
...
摘要：單眼深度估計演算法發展的主要障礙在於難以取得與收集到的 RGB 影像對應的高品質深度資料。收集此類資料耗時且成本高昂，且即使是現代感測器所收集的資料，其範圍或解析度也有限，且容易出現不一致性和雜訊。為了解決此問題，我們提出了一種使用 3D 合成環境和 CycleGAN 領域轉移的模擬資料生成方法。我們透過使用不同訓練集的真實和模擬資料，訓練基於 DenseDepth 結構的深度估計模型，將此資料生成方法與廣泛使用的 NYUDepth V2 資料集進行比較。我們在 Husky 機器人的新收集影像和 LiDAR 深度資料上評估模型的效能，以驗證此方法的泛化性，並證明 GAN 轉換資料可用作真實世界資料的有效替代方案，特別是在深度估計方面。
...
摘要：在追求為全自動駕駛精煉精確感知模型時，持續在線模型訓練變得至關重要。車輛網路中的聯合學習 (FL) 提供了一個有效的模型訓練機制，同時保持原始感測資料的完整性。然而，FL 處理非相同分佈資料（例如，數量偏差）時會遇到困難，導致模型訓練期間的收斂速度次佳。在先前的研究中，我們引入了 FedLA，一種創新的標籤感知聚合方法，用於解決 FL 中的資料異質性，適用於一般場景。
  在本文中，我們介紹了 FedProx+LA，這是一種新穎的 FL 方法，建立在最先進的 FedProx 和 FedLA 之上，用於解決資料異質性，專門針對車輛網路量身打造。我們評估了 FedProx+LA 在持續在線物體偵測模型訓練中的效力。透過與傳統方法和最先進方法進行比較分析，我們的研究結果揭示了 FedProx+LA 優異的收斂速度。值得注意的是，如果標籤分佈非常異質，與基線方法相比，我們的 FedProx+LA 方法在偵測效能方面顯示出顯著的改善，也優於我們先前的 FedLA 方法。此外，與基線方法相比，FedLA 和 FedProx+LA 都將收斂速度提高了 30%。
...
摘要：圖形Transformer由於其全局關注機制，已成為處理圖形結構數據的新工具。眾所周知，全局關注機制在完全連接的圖形中考慮了更廣泛的感受野，這讓許多人相信可以從所有節點中提取有用的信息。在本文中，我們對這種信念提出挑戰：全局化屬性是否總能使圖形Transformer受益？我們通過提供經驗證據和理論分析揭示了圖形Transformer中的過度全局化問題，即當前的關注機制過度關注那些遠距離節點，而實際上包含大部分有用信息的近距離節點則相對較弱。然後，我們提出了一種新穎的雙級全局圖形Transformer，帶有協作訓練 (CoBFormer)，包括集群間和集群內Transformer，以防止過度全局化問題，同時保持從遠距離節點提取有價值信息的的能力。此外，提出了協作訓練來提高模型的泛化能力，並提供理論保證。在各種圖形上的大量實驗很好地驗證了我們提出的 CoBFormer 的有效性。
...
摘要：<paragraph>吹哨揭弊對於確保公私部門的透明度和問責制至關重要。然而，（潛在的）吹哨者即使匿名舉報，也常常害怕或面臨報復。他們披露內容的具體內容和其獨特的寫作風格可能會重新識別他們為消息來源。法律措施（例如歐盟 WBD）的範圍和有效性有限。因此，計算機方法對於防止重新識別是鼓勵吹哨者挺身而出的重要補充工具。然而，當前的文字消毒工具採用一刀切的方法，並且對匿名性的看法過於狹隘。它們旨在通過將典型的風險詞彙（例如人名和其他 NE 標籤）及其組合替換為佔位符來降低識別風險。然而，這種方法對於吹哨場景來說是不夠的，因為它忽視了文本特徵（包括寫作風格）中進一步的重新識別潛力。因此，我們提出、實施並評估了一種新的分類和緩解策略，用於改寫文本，其中涉及吹哨者對風險和效用的評估。我們的原型工具在詞彙/術語層面半自動評估風險，並應用風險適應匿名化技術來生成語法不連貫但適當消毒的文本。然後，我們使用我們微調以進行改寫的 LLM 來使此文本連貫且風格中立。我們使用歐洲人權法院的案例和真實世界吹哨者證詞的摘錄來評估我們工具的有效性，並使用流行的 IMDb62 電影評論數據集統計衡量對作者歸因（AA）攻擊的保護和效用損失。我們的這種方法可以將 AA 準確率從 98.81% 大幅降低到 31.22%，同時保留高達 73.1% 的原始內容語義。</paragraph>
...
摘要：聯邦學習 (FL) 是一種分散式學習範例，讓參與方可以在保持其資料機密性的同時，共同訓練模型。在自動駕駛中，它具有降低資料儲存成本、減少頻寬需求和加速學習的潛力。然而，FL 容易受到中毒攻擊。在本文中，我們介紹了兩種針對自動駕駛中回歸任務量身打造的新型中毒攻擊：FLStealth 和脫軌攻擊 (OTA)。FLStealth 是一種非目標攻擊，旨在提供會損害整體模型效能的模型更新，同時看起來是良性的。另一方面，OTA 是一種目標攻擊，其目標是改變整體模型在接觸到特定觸發器時的行為。我們透過針對車輛軌跡預測任務進行全面的實驗，證明了我們攻擊的有效性。特別是，我們發現，在五種不同的非目標攻擊中，FLStealth 最成功地繞過了伺服器所採用的考慮防禦措施。對於 OTA，我們證明常見的防禦策略無法減輕攻擊，突顯了在 FL 中針對自動駕駛的目標攻擊迫切需要新的防禦機制。
...
摘要：通訊瓶頸會阻礙分散式神經網路訓練的可擴充性，特別是在分散式記憶體運算叢集上。為了大幅減少此通訊開銷，我們引入了 AB 訓練，一種新型資料平行訓練方法，它會將權重矩陣分解成低秩表示，並利用獨立的基於群組的訓練。此方法在多個擴充場景中持續減少 50% 的網路流量，增加了受通訊限制系統的訓練潛力。我們的模型在較小規模下展現了正則化效果，導致像 VGG16 之類的模型有更好的泛化能力，同時在 CIFAR-10 上進行訓練時，達到了驚人的 44.14:1 壓縮比，並維持有競爭力的準確度。儘管有希望，我們的實驗顯示，即使在低秩訓練模式中，大批次效應仍然是一個挑戰。
...
摘要：從單張 RGB 影像重建手部網格是一項具有挑戰性的任務，因為手部通常會被物件遮擋。大多數先前的工作嘗試引入更多額外資訊並採用注意力機制來改善 3D 重建結果，但這會增加運算複雜度。這個觀察促使我們提出一個新的簡潔架構，同時提高運算效率。在這項工作中，我們提出一個簡單且有效的 3D 手部網格重建網路 HandSSCA，它率先將狀態空間建模納入手部姿勢估計領域。在網路中，我們設計了一個新穎的狀態空間通道注意力模組，它擴展了有效的感測場，在空間維度中提取手部特徵，並在通道維度中增強手部區域特徵。此設計有助於重建完整且詳細的手部網格。在具有挑戰性手部物件遮擋（例如 FREIHAND、DEXYCB 和 HO3D）的知名資料集上進行的廣泛實驗證明，我們提出的 HandSSCA 在保持最小參數計數的同時，達到了最先進的效能。
...
摘要：模擬土壤反射光譜對於土壤植物輻射建模和訓練機器學習模型非常有價值，但由於土壤結構與其組成成分之間的複雜關係，這很困難。為了解決這個問題，開發了一個完全資料驅動的土壤光學生成模型 (SOGM)，用於根據土壤性質輸入模擬土壤反射光譜。該模型在一個包含來自 17 個資料集的近 180,000 個土壤光譜性質對的大型資料集上進行訓練。它從描述土壤性質及其值的基於文字的輸入中生成土壤反射光譜，而不是僅使用二進制向量格式中的數值和標籤。生成模型可以根據不完整的輸入性質集模擬輸出光譜。SOGM 基於去噪擴散機率模型 (DDPM)。還建立了兩個額外的子模型來補充 SOGM：一個光譜填充模型，可以填補小於全可見光近紅外線範圍 (VIS-NIR；400 至 2499 nm) 的光譜間隙，以及一個濕土壤光譜模型，可以估計水含量對 SOGM 預測的乾燥光譜的影響。SOGM 通過與 Helios 3D 植物建模軟體結合使用而升級，這允許生成模擬土壤和植物場景的合成航拍影像。它還可以輕鬆地與用於遙測研究的土壤植物輻射模型（如 PROSAIL）整合。SOGM 在未包含在模型訓練中的新資料集上的測試結果證明，該模型可以根據可用的性質輸入生成合理的土壤反射光譜。所提出的模型可以公開取得：
https://github.com/GEMINI-Breeding/SOGM_soil_spectra_simulation。
...
摘要：<paragraph>隨著都會私人汽車擁有率的提高，都會停車難題日益惡化，因此迫切需要進行有效的停車位可用性預測，以利於都市規劃與管理。然而，現有的預測方法都存在預測準確度低的問題，原因在於缺乏與停車量相關的時空關聯特徵，而且忽略了特定區域內相似停車場之間的流動模式和關聯性。為了應對這些挑戰，本研究提出了一個整合時空深度學習與多源數據融合的停車位可用性預測框架，涵蓋來自多個來源（例如地鐵、公車、計程車服務）的交通需求數據和停車場數據。此框架以 Transformer 為基礎，作為時空深度學習模型，並利用 K 均值聚類建立停車場群集區域，從連接到停車場的各種交通模式（例如地鐵、公車、線上叫車服務和計程車）中提取和整合交通需求特徵。使用真實世界的經驗數據驗證了所提出的方法與不同的機器學習、深度學習和傳統統計模型在預測停車位可用性方面的有效性。實驗結果表明，透過所提出的管線，所開發的 Transformer 模型在各種指標（例如均方誤差 (MSE)、平均絕對誤差 (MAE) 和平均絕對百分比誤差 (MAPE)）方面都優於其他模型。透過融合多源需求數據與時空深度學習技術，此方法具有開發停車位可用性預測系統的潛力，可為駕駛人和都市規劃者提供更準確和及時的資訊，從而促進更有效率和永續的都市行動力。</paragraph>
...
摘要：機器學習中的一般性目標是要在各種未見任務和領域中達到優異的效能。最近，自我監督學習 (SSL) 被視為達成此目標的有效方法。它可以從未標記資料中學習高品質的表徵，並在多個下游任務中達到有前景的經驗效能。現有的 SSL 方法主要從兩個方面約束一般性：(i) 大規模訓練資料，以及 (ii) 學習任務層級的共用知識。然而，這些方法缺乏在學習目標中對 SSL 一般性的明確建模，而且對 SSL 一般性的理論理解仍然有限。這可能會導致 SSL 模型在資料稀少的情況下過度擬合，並在現實世界中概化不良，使得難以達成真正的普遍性。為了解決這些問題，我們提供了 SSL 中一般性的理論定義，並定義了一個 $\sigma$-測量來幫助量化它。基於這個見解，我們將一般性明確建模到自我監督學習中，並進一步提出了一個新的 SSL 架構，稱為 GeSSL。它引入了基於 $\sigma$-測量的自我激勵目標，使模型能夠找到朝向一般性的最佳更新方向。廣泛的理論和經驗評估證明了所提出的 GeSSL 的優異效能。
...
摘要：<paragraph>深度學習的最新進展已展現出顯著的效能，在各種受監督的電腦視覺任務中，表現堪比人類能力。然而，在模型訓練之前，普遍假設擁有涵蓋所有類別的廣泛訓練資料池，這點通常與現實世界的情況不同，在現實世界中，對於新類別而言，資料的取得有限才是常態。挑戰在於將少數樣本的新類別無縫整合到訓練資料中，並要求模型靈活地容納這些新增內容，同時不影響其在基本類別上的效能。為了應對這種急迫需求，研究社群在少樣本類別增量學習 (FSCIL) 的領域中提出了多種解決方案。在這項研究中，我們提出了一個創新的 FSCIL 架構，它利用語言規範器和子空間規範器。在基礎訓練期間，語言規範器有助於納入從視覺語言模型中提取的語義資訊。子空間規範器有助於促進模型在增量訓練期間，獲取影像與文本語義之間的細微關聯，這些語義固有於基礎類別。我們提出的架構不僅能讓模型採用資料有限的新類別，還能確保在基礎類別上的效能不變。為了證實我們方法的效能，我們在三個不同的 FSCIL 基準上進行了全面的實驗，我們的架構在這些基準上達到了最先進的效能。</paragraph>
...
摘要：學習解決車輛路徑問題（VRP）已備受關注。然而，大多數神經求解器僅針對特定問題獨立建構和訓練，這讓它們較不具通用性和實用性。在本文中，我們旨在開發一個統一的神經求解器，能夠同時應對各種 VRP 變體。具體來說，我們提出一個結合專家混合（MVMoE）的多任務車輛路徑求解器，它大幅提升模型容量，而不會成比例地增加運算。我們進一步為 MVMoE 開發一個階層式閘控機制，在經驗效能和運算複雜度之間取得良好的平衡。在實驗中，我們的模型顯著提升了在 10 個未見 VRP 變體上的零次學習泛化效能，並在少次學習設定和真實世界基準實例中展示了良好的結果。我們進一步提供了關於 MoE 組態在解決 VRP 中的影響的廣泛研究。令人驚訝的是，階層式閘控可以實現更好的分佈外泛化效能。原始程式碼可在 https://github.com/RoyalSkye/Routing-MVMoE 取得。
...
摘要：儘管預先訓練語言模型在基於提示的少量學習中展現出極佳的彈性和多功能性，但它們飽受參數規模龐大和推論適用性有限之苦。最近的研究建議將 PLM 用作資料集產生器，並訓練一個微小的任務特定模型以達成有效率的推論。然而，它們在各種領域的適用性受到限制，因為它們傾向於產生特定於領域的資料集。在本文中，我們提出了一個針對通用領域泛化的創新方法，它會產生一個與目標領域無關的資料集。這允許微小任務模型泛化到任何與標籤空間共享的領域，進而增強資料集產生範例的實際適用性。我們的實驗表明，所提出的方法在使用比 PLM 小好幾個數量級的參數集時，能夠跨越各種領域達成泛化。
...
摘要：<paragraph>最近的基于变压器的 ASR 模型已经实现了低于 4% 的词错误率 (WER)，超越了人工标注员的准确性，但它们需要大量的服务器资源，导致了巨大的碳足迹。ASR 的传统基于服务器的架构还带来了隐私问题，以及由于网络依赖性而导致的可靠性和延迟问题。相比之下，设备（边缘）ASR 增强了隐私性，提高了性能，并通过有效平衡特定应用程序的能耗和准确性来促进可持续性。本研究考察了量化、内存需求和能耗对 NVIDIA Jetson Orin Nano 上各种 ASR 模型推理性能的影响。通过分析使用 FP32、FP16 和 INT8 量化在干净和嘈杂数据集上的 WER 和转录速度，我们重点介绍了准确性、速度、量化、能效和内存需求之间的关键权衡。我们发现，对于不同的模型，将精度从 fp32 更改为 fp16 会将音频转录的能耗减半，而性能下降很小。更大的模型大小和参数数量既不能保证更好的抗噪性，也不能预测给定转录负载的能耗。这些以及其他一些发现为在能源和内存受限的环境中优化 ASR 系统提供了新的见解，这对于开发高效的设备 ASR 解决方案至关重要。本文中复制结果所需的代码和输入数据是开源的，可在 [https://github.com/zzadiues3338/ASR-energy-jetson] 上获得。</paragraph>
...
摘要：伊博語面臨瀕危的風險，正如 2025 年聯合國教科文組織的研究中所指出的。這凸顯了為伊博語開發語言技術以促進溝通、學習和保存的必要性。為了為伊博語創建強大、有影響力和廣泛採用的語言技術，必須納入語言的多方言性質。實現方言感知語言技術的主要障礙是缺乏全面的方言數據集。為了解決此問題，我們提出了 IgboAPI 數據集，這是一個多方言伊博語-英語詞典數據集，旨在加強伊博語方言的表示。此外，我們通過兩項不同的研究來說明 IgboAPI 數據集的實用性：一項側重於伊博語語義詞彙，另一項側重於機器翻譯。在語義詞彙項目中，我們成功地為伊博語語義標籤器建立了初始的伊博語語義詞彙，而在機器翻譯研究中，我們證明了通過使用 IgboAPI 數據集對現有的機器翻譯系統進行微調，我們可以顯著提高它們處理句子中方言變化的能力。
...
摘要：儘管大型語言模型 (LLM) 在文字理解和生成方面取得了顯著的成功，但它們在文字群集任務中的潛力仍未得到充分的探索。我們觀察到功能強大的閉源 LLM 能提供優質的實體集群集，但由於需要大量的運算能力和相關成本，因此無法擴充。因此，我們提出了 CACTUS（透過擴增三元組損失進行的脈絡感知群集），這是一種系統化的方法，它利用開源 LLM 對實體子集進行有效率且有效的監督式群集，特別專注於基於文字的實體。現有的文字群集方法無法有效擷取實體子集提供的脈絡。此外，儘管有幾種基於語言模型的群集方法，但專門設計用於監督式群集的卻很少。本文介紹了一種透過可擴充的實體間注意機制擷取脈絡，使用 LLM 對實體子集進行群集的新方法。我們提出了一個針對監督式群集量身打造的新擴增三元組損失函數，它解決了直接將三元組損失應用於此問題的固有挑戰。此外，我們介紹了一種基於文字擴充技術的自監督群集任務，以提升我們模型的概化能力。為了進行評估，我們從閉源 LLM 收集真實的群集，並在監督式群集架構下將此知識轉移到開源 LLM，讓更快速且更便宜的開源模型執行相同的任務。在各種電子商務查詢和產品群集資料集上的實驗證明，我們提出的方法在各種外部群集評估指標下，明顯優於現有的非監督式和監督式基準。
...
摘要：神經崩潰 (NC) 是深度神經網路 (DNN) 在訓練的終端階段中一種簡單且對稱的現象，其中最後一層特徵會崩潰成它們的類別平均值，並形成一個與分類器向量對齊的單純等角緊緻框架。然而，最後一層特徵與資料和訓練期間的中間層之間的關係仍未探討。為此，我們描述了 ResNet 中間層的幾何形狀，並提出了一個新的猜想，即漸進前饋崩潰 (PFC)，聲稱崩潰的程度在 DNN 的前向傳播過程中會增加。我們根據權重衰減的 ResNet 近似於 Wasserstein 空間中的測地線在終端階段，為訓練良好的 ResNet 導出了一個透明模型。PFC 的指標確實在各種資料集上隨著深度單調遞減。我們提出了一個新的代理模型，多層非約束特徵模型 (MUFM)，通過一個最優傳輸正則化器連接中間層。MUFM 的最優解與 NC 不一致，但相對於輸入資料更集中。總的來說，這項研究將 NC 擴展到 PFC，以對中間層的崩潰現象及其對輸入資料的依賴性進行建模，這有助於了解 ResNet 在分類問題中的理論。
...
摘要：大型語言模型 (LLM) 已經實現了滿足資訊需求的新方法。儘管在將其應用於文件排名和短篇文字生成等設定上已取得長足進展，但它們在撰寫完整、準確且可驗證的长篇報告方面仍面臨挑戰。具備這些特質的報告對於滿足使用者複雜、細微或多面向的資訊需求至關重要。在這篇觀點論文中，我們彙整了來自產業、學術界和各種相關研究領域的意見，以提出我們對自動報告生成的願景，以及更重要的是，一個可以評估此類報告的彈性架構。與其他摘要任務相比，自動報告生成始於對資訊需求的詳細描述，說明報告的必要背景、需求和範圍。此外，生成的報告應完整、準確且可驗證。這些特質在許多分析報告撰寫設定中都是理想的（如果沒有必要的話），需要重新思考如何建構和評估展現這些特質的系統。為了促進建構這些系統的新努力，我們提出了一個評估架構，該架構汲取了各種評估中的構想。為了測試完整性和準確性，此架構使用資訊片段（以問答形式表示），這些片段必須是任何高品質生成報告的一部分。此外，評估引文（將報告中的主張對應到其原始文件）可確保可驗證性。
...
摘要：<paragraph>設計偏好引導（PE）方法，以便在冷啟動設定中快速確定使用者的首要項目偏好，是建置有效且個人化的對話式推薦（ConvRec）系統的一項關鍵挑戰。儘管大型語言模型（LLM）構成了一種新技術，可實現完全的自然語言（NL）PE 對話，但我們假設單體 LLM NL-PE 方法缺乏多輪、決策理論推理，而這對於有效平衡 NL 探索和利用使用者的偏好以針對任意項目集是必要的。相反地，傳統的貝氏最佳化 PE 方法定義了理論上最佳的 PE 策略，但無法使用 NL 項目描述或產生 NL 查詢，不切實際地假設使用者可以使用直接項目評分和比較來表達偏好。為了克服這兩種方法的限制，我們在貝氏最佳化（BO）架構中制定 NL-PE，該架構旨在產生 NL 查詢，主動引導自然語言回饋，以降低項目效用的不確定性，進而找出最佳推薦。我們在一個新穎的 NL-PE 演算法 PEBOL 中展示我們的架構，它使用使用者偏好語句和 NL 項目描述之間的自然語言推論（NLI）來維持偏好信念和 BO 策略，例如湯普森採樣（TS）和上置信界（UCB），以引導 LLM 查詢產生。我們在受控實驗中對我們的數值方法進行評估，發現與依賴於小得多的 400M 參數 NLI 模型進行偏好推論的單體 GPT-3.5 相比，在 10 輪冷啟動 NL-PE 對話後，PEBOL 在 MAP@10 方面取得了高達 131% 的改進。</paragraph>
...
摘要：<paragraph>本文介紹 TVB-HKSL-News，一個新的香港手語 (HKSL) 資料集，在 7 個月的期間從電視新聞節目中收集而來。該資料集的收集目的是豐富 HKSL 的資源，並支援大詞彙量連續手語識別 (SLR) 和翻譯 (SLT) 的研究。它包含了兩位手語者的 16.07 小時手語影片，詞彙量為 6,515 個手語符號 (用於 SLR) 和 2,850 個中文字或 18K 個中文詞彙 (用於 SLT)。一位手語者有 11.66 小時的影片，另一位有 4.41 小時。建立該資料集的一個目標是支援調查在給定 (相對) 大量的訓練資料時，單一一位手語者的大詞彙量連續手語識別/翻譯可以做到多好，這可能會導致新的建模方法的開發。此外，大部分的資料收集流程都是自動化的，幾乎沒有人工介入；我們相信我們的收集方法可以擴充，以便在未來為任何手語輕鬆收集更多手語資料用於 SLT，如果此類手語翻譯影片可取得的話。我們也在該資料集上執行 SOTA SLR/SLT 模型，並取得 34.08% 的基準 SLR 字元錯誤率和 23.58 的基準 SLT BLEU-4 分數，作為未來在該資料集上研究的基準。</paragraph>
...
摘要：多語言資訊檢索 (MLIR) 考量到針對以不同語言表達的查詢，對多種語言的文件進行排名的問題。最近的研究觀察到，例如合併代表單一文件語言的排名清單，或使用多語言預先訓練的語言模型等方法，會偏好某種語言多於其他語言。這會導致對不同語言的文件進行系統性的不公平處理。本研究提出一個語言公平性指標，以使用 Kruskal-Wallis 檢定進行統計等價性檢定，評估不同語言的文件是否獲得公平的排名。與群體公平性中大多數先前研究相反，我們不認為任何語言是未受保護的群體。因此，我們提出的指標，PEER（預期排名相等的機率），是第一個專門設計用於擷取 MLIR 系統語言公平性的公平性指標。我們在人工排名清單上展示 PEER 的行為。我們也針對兩個公開基準評估真實的 MLIR 系統，並顯示 PEER 分數與 MLIR 公平性的先前分析結果一致。我們的實作與 ir-measures 相容，且可在 http://github.com/hltcoe/peer_measure 取得。
...
摘要：最近在跨語言資訊檢索 (CLIR) 中的研究，其中查詢和文件以不同語言撰寫，顯示了 Translate-Distill 框架的優點，該框架使用翻譯和知識蒸餾來訓練跨語言神經雙編碼器模型。然而，Translate-Distill 僅支援單一文件語言。多語言資訊檢索 (MLIR) 對多語言文件集合進行排名，比 CLIR 更難訓練，因為模型必須為不同語言的文件指定可比較的相關性分數。這項工作擴展了 Translate-Distill，並為 MLIR 提出多語言 Translate-Distill (MTD)。我們展示了使用 MTD 訓練的 ColBERT-X 模型在 nDCG@20 中比使用多語言 Translate-Train 訓練的對應模型高出 5% 至 25%，在 MAP 中高出 15% 至 45%，而多語言 Translate-Train 是先前的最先進訓練方法。我們還展示了該模型對於在訓練批次中混合語言的方式具有穩健性。我們的實作可以在 GitHub 上取得。
...
摘要：PLAID 是一種 ColBERT 晚期互動雙編碼器的有效實作，使用預先訓練的語言模型進行排名，在單語、跨語言和多語言檢索中持續達到最先進的效能。PLAID 與 ColBERT 的不同之處在於將詞彙指派給叢集，並將這些詞彙表示為叢集質心加上壓縮殘差向量。雖然 PLAID 在批次實驗中很有效，但它的效能在串流設定中會下降，因為文件會隨著時間而出現，而且新記號的表示可能無法由用於選擇叢集質心的較早記號來建模。PLAID 串流分層索引在數 TB 的時間文字上執行（PLAID SHIRTTT）使用基於分層分片的多分段漸進式索引來解決這個問題。在 ClueWeb09 和多語言 NeuCLIR 集合上的實驗證明了這種方法的有效性，分別適用於 ColBERT 架構迄今為止索引的最大集合和多語言設定。
...
摘要：大型語言模型 (LLM) 在各種領域展現出驚人的潛力，但它們常常缺乏取得特定領域知識與工具的能力。在本文中，我們介紹了 CACTUS（化學劑連接工具使用到科學），一個基於 LLM 的代理，它整合了化學信息學工具，以在化學和分子發現中實現進階推理和問題解決。我們使用一系列多元的開源 LLM，包括 Gemma-7b、Falcon-7b、MPT-7b、Llama2-7b 和 Mistral-7b，在數千個化學問題的基準上評估 CACTUS 的效能。我們的結果證明，CACTUS 明顯優於基準 LLM，其中 Gemma-7b 和 Mistral-7b 模型獲得最高準確度，無論使用何種提示策略。此外，我們探討了特定領域提示和硬體配置對模型效能的影響，強調提示工程的重要性，以及在消費者級硬體上部署較小型模型的潛力，而準確度不會顯著下降。透過結合開源 LLM 的認知能力和特定領域工具，CACTUS 能協助研究人員執行以下任務：分子特性預測、相似性搜尋和類藥性評估。此外，CACTUS 代表了化學信息學領域的重要里程碑，提供了一個適應性工具，供從事化學和分子發現的研究人員使用。透過整合開源 LLM 的優點和特定領域工具，CACTUS 有潛力加速科學進展，並開啟探索新穎、有效和安全的治療候選物、催化劑和材料的新領域。此外，CACTUS 能與自動化實驗平台整合，並在實際時間做出資料驅動的決策，為自主發現開啟了新的可能性。
...
摘要：一對一的輔導被廣泛認為是一種有效的教學方法，條件是輔導員合格。然而，對合格輔導員的高需求仍然是一個挑戰，通常需要對新手輔導員（即受訓者）進行培訓，以確保輔導有效。研究表明，提供及時的解釋性回饋可以促進受訓者的培訓過程。然而，由於由人類專家評估受訓者表現的耗時性質，這提出了挑戰。在大型語言模型 (LLM) 最近的進展的啟發下，我們的研究採用 GPT-4 模型來建立一個解釋性回饋系統。這個系統以二進制形式（即正確/不正確）識別受訓者的回應，並自動提供基於範本的回饋，由 GPT-4 模型適當地重新表述回應。我們對來自受訓者的 410 個回應進行了研究，涵蓋三個培訓課程：給予有效的讚美、對錯誤的反應以及確定學生的知識。我們的研究結果表明：1) 使用少次嘗試的方法，GPT-4 模型有效地從三個培訓課程中識別出受訓者的正確/不正確回應，平均 F1 分數為 0.84，AUC 分數為 0.85；2) 使用少次嘗試的方法，GPT-4 模型熟練地將受訓者的不正確回應改寫為期望的回應，達到的表現與人類專家相當。
...
它在部分涵蓋的語言中產生了值得稱讚的自動語音辨識 (ASR) 結果，但該模型在數量不小的未充分代表語言中仍表現不佳，而較小模型版本中的問題更為嚴重。在這項工作中，我們檢視其限制，證明了存在與說話者相關（性別、年齡）和與模型相關（資源豐富性和模型大小）的偏差。儘管如此，我們發現只有與模型相關的偏差會因量化而放大，影響更多低資源語言和較小模型。為了尋找更好的壓縮方法，我們提出了 DistilWhisper，這是一種能夠彌合這些語言中 ASR 的效能差距，同時保留多任務和多國語言能力優勢的方法。我們的做法涉及兩個關鍵策略：使用特定語言專家對 whisper-small 進行輕量級模組化 ASR 微調，以及從 whisper-large-v2 進行知識萃取。這種雙重方法使我們能夠有效提升 ASR 效能，同時保持從多任務和多國語言預訓練繼承的穩健性。結果證明，與標準微調或 LoRA 適配器相比，我們的做法更有效，提升了目標語言在域內和域外測試集中的效能，同時在推論中僅引入了可忽略的參數開銷。
...
摘要：<paragraph>在研究中，我們介紹生成式製造系統 (GMS) 作為一種新型方法，能有效管理和協調自主製造資產，從而增強其應變能力和靈活性，以滿足廣泛的生產目標和人類偏好。偏離傳統的顯式建模，GMS 使用生成式 AI，包括擴散模型和 ChatGPT，從預想的未來進行隱式學習，標誌著從模型最優化到訓練採樣決策制定。透過整合生成式 AI，GMS 能透過與人類的互動式對話進行複雜的決策制定，讓製造資產生成多個高品質的全局決策，並能根據人類的回饋進行反覆優化。經驗發現展示了 GMS 在系統韌性和應變不確定性方面有大幅改善，決策時間從數秒縮短至毫秒。該研究強調了生成式解決方案中固有的創造力和多樣性，透過無縫且持續的人機互動，促進以人為中心的決策制定。</paragraph>
...
摘要：不安分的多臂賭徒 (RMAB) 在建模順序決策問題中扮演著核心角色，在即時啟動限制下，最多有 B 個手臂可以在任何決策時期啟動。每個不安分的手臂都擁有狀態，會根據馬可夫決策過程獨立演化，無論是否啟動。在本文中，我們考慮在未知轉換函數和對抗獎勵下學習情境 RMAB 的任務，這些獎勵可以在情境中任意改變。此外，我們考慮了一個具有挑戰性但自然的賭徒回饋設定，其中只有已啟動手臂的對抗獎勵會被揭露給決策者 (DM)。DM 的目標是在學習過程中最大化其總對抗獎勵，同時在每個決策時期都必須滿足即時啟動限制。我們開發了一種新穎的強化學習演算法，它有兩個關鍵貢獻者：一個新穎的偏差對抗獎勵估計器，用於處理賭徒回饋和未知轉換，以及一個低複雜度指標政策，用於滿足即時啟動限制。我們展示了演算法的 $\tilde{\mathcal{O}}(H\sqrt{T})$ 後悔界限，其中 $T$ 是情境數，$H$ 是情境長度。據我們所知，這是第一個演算法，可以確保在我們考慮的具有挑戰性的設定中，對抗 RMAB 的 $\tilde{\mathcal{O}}(\sqrt{T})$ 後悔。
...
摘要：本研究引入了一個系統性架構，用於比較大型語言模型 (LLM) 在各種化學資訊學任務中微調的功效。採用統一的訓練方法，我們評估了三個著名的模型，即 RoBERTa、BART 和 LLaMA，它們利用簡化的分子輸入線條輸入系統 (SMILES) 作為通用分子表示格式預測分子特性的能力。我們的比較分析涉及預訓練這些模型的 18 種配置，它們具有不同的參數大小和數據集規模，然後在 DeepChem 的六個基準任務上對它們進行微調。我們在所有模型中保持一致的訓練環境，以確保可信賴的比較。這種方法使我們能夠評估模型類型、大小和訓練數據集大小對模型性能的影響。具體來說，我們發現基於 LLaMA 的模型通常提供最低的驗證損失，表明它們在任務和規模上具有卓越的適應性。然而，我們觀察到，絕對驗證損失並非模型性能的明確指標，這與先前的研究相矛盾，至少對於微調任務而言：相反，模型大小發揮著至關重要的作用。通過嚴謹的複製和驗證，涉及多個訓練和微調週期，我們的研究不僅描述了每種類型模型的優點和局限性，還提供了一種強大的方法，用於為具體的化學資訊學應用選擇最合適的 LLM。本研究強調了在部署 AI 以進行分子特性預測時考慮模型架構和數據集特徵的重要性，為在藥物發現和相關領域更明智、更有效地利用 AI 鋪平了道路。
...
摘要：同理心需要換位思考：同理心的回應需要一個人
推測另一個人經歷了什麼，並用語言傳達這種理解。
然而，大多數自然語言處理方法對同理心並沒有明確建模這個
對齊過程。在此，我們引入一種新的方法來識別同理心言語中的對齊，
基礎是評估理論。我們引入了一個新的數據集，其中包含超過 9.2K 個
跨度層級標註，標註了對個人經驗的不同類型的評估，以及說話者和
觀察者之間超過 3K 個同理心對齊。通過計算實驗，我們證明了這些
評估和對齊可以被準確識別。在超過 920 萬個 Reddit 對話的實驗中，
我們發現評估捕捉到了有意義的行為群組，但大多數回應的對齊程度很低。
然而，我們發現心理健康專業人員參與了更多同理心對齊。
...
摘要：溝通被定義為「誰對誰說了什麼，產生了什麼效果」。傳播者的訊息會產生下游的接收者效果，也稱為行為。接收者行為作為訊息的下游效果，承載著關於訊息的豐富訊號。即使在承載了關於訊息的訊號後，行為資料在訓練大型語言模型時經常被忽略。我們表明，在接收者行為上訓練 LLM 實際上可以幫助改善它們的內容理解能力。具體來說，我們表明訓練 LLM 以預測按讚和留言的接收者行為，可以改善 LLM 在各種下游內容理解任務上的表現。我們在 23 個基準資料集上的 40 個影片和影像理解任務中顯示了這種效能提升，在零次學習和微調設定中都優於許多監督基線。此外，由於接收者行為，例如按讚和留言，會在網路上預設收集，且不需要任何人類註解即可發揮作用，因此我們在訓練此資料後獲得的效能提升基本上是免費的午餐。我們釋出了從多個平台收集的 750k 張圖片和影片的接收者行為已清除留言和按讚，以及我們的指令微調資料。
...
摘要：<paragraph>我們提出 EchoScene，一種互動且可控的生成模型，它在場景圖上產生 3D 室內場景。EchoScene 利用一個雙分支擴散模型，它動態適應場景圖。現有方法由於節點數量變化、多重邊緣組合以及操作器引發的節點邊緣操作，難以處理場景圖。EchoScene 克服了這個問題，它將每個節點與一個去噪過程關聯起來，並啟用協作資訊交換，增強可控且一致的生成，並意識到全域約束。這透過形狀和佈局分支中的資訊迴音機制達成。在每個去噪步驟中，所有程序都會與一個資訊交換單元分享它們的去噪資料，該單元使用圖形卷積結合這些更新。該機制確保去噪程序受到對場景圖整體理解的影響，從而促進生成整體一致的場景。結果場景可以在推論期間，透過編輯輸入場景圖和對擴散模型中的雜訊進行抽樣，進行操作。廣泛的實驗驗證了我們的方法，它保持了場景可控性，並在生成保真度方面超越了先前的技術。此外，生成的場景品質很高，因此可以直接與現成的紋理生成相容。程式碼和訓練模型是開源的。</paragraph>
...
摘要：背景與目的：識別缺血性中風的血栓栓塞來源對於治療和二次預防至關重要，但通常無法確定。本研究描述了數字病理學中栓子自監督深度學習方法，用於根據組織病理學圖像對缺血性中風血塊來源進行分類。方法：該數據集包括來自 STRIP AI Kaggle 挑戰賽的全幻燈片圖像 (WSI)，其中包含機械血栓切除術後缺血性中風患者的檢索血塊。使用遷移學習和自監督預訓練開發了基於 Transformer 的深度學習模型，用於對 WSI 進行分類。自定義包括注意力池化層、加權損失函數和閾值優化。測試並比較了各種模型架構，並主要使用加權對數損失評估模型性能。結果：該模型在交叉驗證中實現了 0.662 的對數損失分數，在測試集中實現了 0.659。比較了不同的模型主幹，其中 swin_large_patch4_window12_384 顯示出更高的性能。採用了血塊起源分類的閾值技術來平衡假陽性和假陰性。結論：該研究展示了基於 Transformer 的深度學習模型在根據組織病理學圖像識別缺血性中風血塊來源方面的功效程度，並強調了需要專門針對血栓 WSI 調整的精細建模技術。需要進一步研究以提高模型性能、可解釋性，驗證其有效性。未來的改進可以包括整合更大的患者群體、先進的預處理策略，以及探索集成多模式方法以提高診斷準確性。
...
摘要：視覺轉換器徹底改變了電腦視覺，但它們的運算需求為訓練和部署帶來了挑戰。本文介紹了 LOTUS（具有超稀疏性的樂透轉換器），這是一種新穎的方法，它利用數據樂透抽獎選擇和稀疏性剪枝來加速視覺轉換器訓練，同時保持準確性。我們的做法專注於識別和利用最有資訊性的數據子集，並消除冗餘模型參數，以優化訓練過程。透過廣泛的實驗，我們證明了 LOTUS 在以顯著降低的運算需求實現快速收斂和高準確性方面的有效性。這項工作突顯了結合數據選擇和稀疏性技術進行高效視覺轉換器訓練的潛力，為這個領域的進一步研究和開發開啟了大門。
...
摘要：在過去十年中，與其他應用領域類似，社群媒體內容已被證明在災害資訊學中非常有效。然而，由於資料的非結構化性質，社群媒體內容中的災害分析面臨著一些挑戰。為了充分探索社群媒體內容在災害資訊學中的潛力，取得相關內容和正確的地理位置資訊至關重要。在本文中，我們提出了一個三步驟的解決方案來應對這些挑戰。首先，所提出的解決方案旨在將社群媒體貼文分類為相關和不相關的貼文，然後通過命名實體識別 (NER) 分析從貼文的文字中自動提取位置資訊。最後，為了快速分析大量社群媒體貼文中涵蓋的主題，我們執行主題建模，產生一個頂級關鍵字清單，以突顯推文中討論的問題。對於推文相關分類 (RCTP)，我們提出了一個基於優點的融合框架，結合了四種不同模型的能力，即 BERT、RoBERTa、Distil BERT 和 ALBERT，在基準資料集上獲得了最高的 F1 分數 0.933。對於從推文文字中提取位置 (LETT)，我們在 NER 框架中評估了四個模型，即 BERT、RoBERTa、Distil BERTA 和 Electra，獲得了最高的 F1 分數 0.960。對於主題建模，我們使用 BERTopic 函式庫來發現相關推文中的隱藏主題模式。所提出的端到端解決方案所有組成的實驗結果都非常令人鼓舞，並暗示了社群媒體內容和 NLP 在災害管理中的潛力。
...
摘要：大型語言模型顯得相當有創意，在創意任務上通常與一般人表現得不相上下。然而，關於 LLM 創意的研究只專注於「產品」，很少關注創意的「過程」。人類創意的過程分析通常需要手動編碼的類別或利用反應時間，這不適用於 LLM。我們提供一個自動化的方法來描述人類和 LLM 如何在替代用途任務中探索語義空間，並與在流利口語任務中的行為形成對比。我們使用句子嵌入來識別回應類別和計算語義相似性，我們用來產生跳躍輪廓。我們的結果證實了人類早期工作的報告，即持續（在少數語義空間中深入搜尋）和靈活（在多個語義空間中廣泛搜尋）兩種途徑都可以激發創意，而且兩種途徑都會導致類似的創意分數。發現 LLM 偏向於持續或靈活的途徑，這會因任務而異。儘管 LLM 作為一個族群符合人類的輪廓，但它們與創意的關係卻不同，其中較靈活的模型在創意上得分較高。我們的資料集和腳本可在 \href{https://github.com/surabhisnath/Creative_Process}{GitHub} 上取得。
...
摘要：<paragraph>極低功耗裝置上的機器學習應用程式，一般稱為微型機器學習 (TinyML)，承諾帶來更智慧、更緊密連結的世界。然而，目前 TinyML 研究進展受限於相關資料集大小和品質不足。為了解決此挑戰，我們推出 Wake Vision，這是一個針對人物偵測量身打造的大規模、多元資料集，是 TinyML 視覺感測的典型任務。Wake Vision 包含超過 600 萬張影像，與先前的標準相比增加了百倍，並經過徹底的品質過濾。使用 Wake Vision 進行訓練，準確度比既定的基準提高了 2.41%。除了資料集之外，我們還提供五個詳細基準集，用於評估模型在測試資料特定區段上的效能，例如不同的光照條件、與相機的距離以及主體的人口特徵。這些新穎的細緻基準有助於評估模型品質，在僅關注整體準確度時經常忽略具有挑戰性的真實世界場景。透過在基準上評估 MobileNetV2 TinyML 模型，我們發現輸入解析度在偵測遠處主體時扮演比模型寬度更重要的角色，而且由於資料集品質，量化對模型穩健性的影響很小。這些發現強調了詳細評估對於找出模型開發中必要因素的重要性。資料集、基準套件、程式碼和模型根據 CC-BY 4.0 授權公開，讓它們能用於商業案例。</paragraph>
...
摘要：傳統語言模型以自回歸方式運作，亦即一次預測一個符號。模型規模的快速爆炸導致高推論時間。在這項工作中，我們提出 DynaMo，一個多符號預測語言模型套件，可減少網路推論時間。我們的模型會根據預測聯合機率分布中的信心，動態預測多個符號。我們提出一個輕量級技術來訓練這些模型，利用傳統自回歸對應項的權重。此外，我們提出增強估計聯合機率的新穎方法，以提升文字生成品質，即共現加權遮罩和自適應閾值。我們也提出系統性的定性和定量方法，以嚴格測試非自回歸生成之生成文字品質。我們套件中的其中一個模型 DynaMo-7.3B-T3，在與基線（Pythia-6.9B）相同品質的生成文字中，同時達到 2.57 倍加速，而參數和訓練時間開銷分別只有 5.87% 和 2.67%。
...
摘要：視覺語言模型 (VLM) 最近浮出檯面，並因其理解影像和文字資料雙重模式的能力而備受矚目。LLaVA、ChatGPT-4 和 Gemini 等 VLM 最近在自然影像標題、視覺問答 (VQA) 和空間推理等任務上展現出令人印象深刻的表現。此外，Meta AI 的通用分割模型 Segment Anything Model (SAM) 在從未見過的影像中分離物體方面展現出前所未有的表現。由於醫學專家、生物學家和材料科學家常規檢查顯微鏡或醫學影像，並結合標題、文獻或報告等形式的文字資訊，並得出極其重要且有價值的結論，因此無庸置疑地，在這些影像上測試 VLM 和 SAM 等基礎模型的表現至關重要。在本研究中，我們賦予 ChatGPT、LLaVA、Gemini 和 SAM 分類、分割、計數和 VQA 任務，並在各種顯微鏡影像上進行測試。我們觀察到 ChatGPT 和 Gemini 能夠令人印象深刻地理解顯微鏡影像中的視覺特徵，而 SAM 則非常擅長廣泛地分離人工製品。然而，其表現並未接近領域專家的水準 - 這些模型很容易受到影像中雜質、缺陷、人工製品重疊和多樣性的影響。
...
摘要：自動化軟體測試是軟體開發過程中不可或缺的一部分，它可以簡化工作流程並確保產品可靠性。在此背景下，視覺測試（特別是關於使用者介面 (UI) 和使用者體驗 (UX) 驗證）是整體軟體品質的重要決定因素之一。儘管如此，像逐像素比較和基於區域的視覺變化偵測等傳統方法，在捕捉情境相似性、細微變更和理解 UI 元素之間的空間關係方面仍有不足。在本文中，我們介紹了一種用於軟體測試自動化中的視覺變化偵測的新型圖形化方法。利用機器學習模型，我們的這種方法可以準確地從軟體螢幕截圖中識別 UI 控制項，並建立一個表示控制項之間情境和空間關係的圖形。然後，這些資訊用於在不同版本的軟體螢幕截圖中找到 UI 控制項之間的對應關係。產生的圖形概括了 UI 的複雜佈局和基礎情境關係，提供了一個整體的且具備情境感知能力的模型。此模型最後用於偵測和突顯 UI 中的視覺回歸。在不同資料集上的全面實驗表明，我們的變更偵測器可以在各種簡單和複雜的測試情況中準確地偵測視覺軟體變更。此外，在更複雜的測試情況中，它在像素級比較和基於區域的基準中表現出明顯的優勢。這項工作不僅有助於視覺變更偵測的進步，而且具有實際意義，為現實世界的軟體測試自動化挑戰提供了強大的解決方案，增強了可靠性，並確保了軟體介面的無縫演進。
...
摘要：選擇題 (MCQ) 是一種評量學生知識的熱門方法，因為其管理和評分效率高。
編寫高品質的數學選擇題是一個勞力密集的過程，需要教育工作者制定精確的題幹和合理的干擾項。
最近大型語言模型 (LLM) 的進展引起了自動化 MCQ 建立的興趣，但仍存在確保數學準確性和解決學生錯誤的挑戰。
本文介紹了一個原型工具，旨在促進 LLM 和教育工作者之間的協作，以簡化數學 MCQ 生成過程。
我們進行了一項涉及數學教育者的試驗研究，以調查該工具如何幫助他們簡化編寫高品質數學 MCQ 的過程。
我們發現，雖然 LLM 可以產生結構良好的問題題幹，但它們產生干擾項的能力，以捕捉常見的學生錯誤和誤解是有限的。
儘管如此，人機協作有可能提高 MCQ 生成的效率和效果。
...
摘要：流行的無根據信念，跨越偽科學、邏輯謬誤和陰謀論，對社會構成重大障礙，並有散佈錯誤訊息的風險。本研究利用既定的心理測量評估，探討大型語言模型 (LLM) 相對於一般人在偵測普遍邏輯陷阱的能力。我們進行哲學探究，將人類的理性與 LLM 的理性並置。此外，我們提出利用 LLM 來對抗誤解的方法，並借鑒認知失調理論和精緻化可能性理論等心理說服模型。透過這項努力，我們強調 LLM 作為個人化錯誤訊息破解代理的潛力。
...
摘要：在這篇論文中，我們提出 Sim-Grasp，一個穩健的 6-DOF 雙指抓取系統，它整合了先進的語言模型，以增強雜亂環境中的物體操作。我們介紹了 Sim-Grasp-Dataset，其中包含 500 個場景中的 1,550 個物體，並有 790 萬個註解標籤，並開發了 Sim-GraspNet 從點雲生成抓取姿勢。Sim-Grasp-Polices 對單一物體的抓取成功率達 97.14%，對 1-2 級和 3-4 級物體的混合雜亂場景的抓取成功率分別為 87.43% 和 83.33%。通過整合語言模型以透過文字和方框提示進行目標識別，Sim-Grasp 能夠同時進行與物體無關的和目標挑選，從而突破了智慧型機器人系統的界限。
...
摘要：<paragraph>分散式多智能體學習 (DML) 能夠在保留資料隱私的同時進行協作模型訓練。然而，智能體資源（運算、通訊和任務大小）中固有的異質性可能會導致訓練時間出現大幅度的變化。這種異質性會造成瓶頸，由於落後效應而延長整體訓練時間，並可能浪費較快智能體的備用資源。為了在異質性環境中將訓練時間降至最低，我們提出了分散式多智能體學習的通訊效率訓練工作負載平衡 (ComDML)，它透過分散式方法來平衡智能體之間的工作負載。ComDML 透過利用局部損失分割訓練，能夠進行並行更新，讓較慢的智能體將部分工作負載卸載給較快的智能體。為了將整體訓練時間降至最低，ComDML 透過同時考量智能體的通訊和運算能力來最佳化工作負載平衡，這取決於整數規劃。開發動態分散式配對排程器，以有效配對智能體並確定最佳卸載量。我們證明在 ComDML 中，較慢和較快的智能體模型都會收斂，適用於凸和非凸函數。此外，在熱門資料集（CIFAR-10、CIFAR-100 和 CINIC-10）及其非 I.I.D. 變體上進行的廣泛實驗結果，以及 ResNet-56 和 ResNet-110 等大型模型，證明與現有技術相比，ComDML 能夠大幅減少整體訓練時間，同時維持模型準確度。ComDML 在異質性環境中展現了穩健性，而且可以無縫整合隱私措施以增強資料保護。</paragraph>
...
摘要：<paragraph>我們提出 WIBA，一個新的架構和方法套件，它能讓「正在爭論的是什麼」在各種脈絡中得到全面的理解。我們的做法開發了一個全面的架構，它能偵測：(a) 存在性，(b) 主題，以及 (c) 論點的立場，並正確地考量這三項任務之間的邏輯依賴性。我們的演算法利用大型語言模型的微調和提示工程。我們評估了我們的做法，並證明它在所有這三項能力中表現良好。首先，我們開發並發布了一個論點偵測模型，它能將一段文字分類為一個論點，在三個不同的基準資料集上 F1 分數介於 79% 到 86% 之間。其次，我們發布了一個語言模型，它能辨識句子中被爭論的主題，無論是隱含的還是明確的，平均相似度分數為 71%，比目前的幼稚方法高出近 40%。最後，我們開發了一種論點立場分類的方法，並評估了我們做法的能力，證明它在三個不同的基準資料集上達到了 71% 到 78% 之間的分類 F1 分數。我們的評估證明了 WIBA 允許全面理解在各種脈絡中的大型語料庫中「正在爭論的是什麼」，這對於語言學、溝通學以及社會和電腦科學中的許多應用程式來說都是核心興趣。為了促進本工作中概述的進展的可及性，我們將 WIBA 作為一個免費的開放存取平台 (wiba.dev) 發布。</paragraph>
...
摘要：客服是公司與客戶互動的方式。它可以極大地提升整體客戶滿意度。然而，高品質的服務可能會變得昂貴，因此有誘因讓它盡可能具有成本效益，並促使大多數公司使用 AI 驅動的助理或「聊天機器人」。另一方面，客戶仍然渴望人與人之間的互動，特別是在糾紛和帳單支付等敏感話題等複雜情況中。
這提高了客服人員的門檻。他們需要準確地了解客戶的問題或疑慮，找出可接受且可行的解決方案（並符合公司的政策），同時處理多個對話。
在這項工作中，我們將「問我任何事」(AMA) 作為一個附加功能，導入客服人員介面。AMA 允許客服人員在處理客戶對話時，根據需要向大型語言模型 (LLM) 提問——LLM 會即時提供準確的回應，減少客服人員需要進行的內容切換。在我們的內部實驗中，我們發現使用 AMA 而非傳統搜尋體驗的客服人員，在包含搜尋的每次對話中平均減少約 10% 的秒數，這轉化為每年數百萬美元的節省。使用 AMA 功能的客服人員在近 80% 的時間裡提供了正面的回饋，證明了它作為 AI 輔助功能在客服方面的實用性。
...
摘要：大型文本到图像模型可以根据文本提示生成高质量且多样的图像，已经展现出显著的成功。这些模型最终旨在创建复杂的场景，解决多主体生成挑战是实现这一目标的关键一步。然而，现有的最先进的扩散模型在生成涉及多个主体的图像时面临困难。当出现包含多个主体的提示时，这些模型可能会省略一些主体或将它们合并在一起。为了应对这一挑战，我们提出了一种基于指导原则的新方法。我们允许扩散模型最初提出一个布局，然后我们重新排列布局网格。这是通过强制交叉注意图 (XAM) 遵守建议的掩码并根据我们确定的新位置从潜在映射迁移像素来实现的。我们引入了新的损失项，旨在减少 XAM 熵以更清晰地定义主体的空间，减少 XAM 之间的重叠，并确保 XAM 与其各自的掩码对齐。我们将我们的方法与几种替代方法进行对比，并表明它更忠实地捕捉了各种文本提示中的所需概念。
...
摘要：<paragraph>新興的多模式工作負載，搭載了大型語言模型等重量級模型，大幅增加了對硬體的運算和記憶體需求。為了應對這種日益增長的需求，設計可擴充的硬體架構已成為一個關鍵問題。在最近的解決方案中，基於 2.5D 矽介面層多晶片模組 (MCM) 的 AI 加速器因其在低工程成本和可組合性方面的顯著優勢，而被積極探索為有前途的可擴充解決方案。然而，先前的 MCM 加速器基於具有固定資料流程的同質架構，由於工作負載適應性有限，因此在高度異質多模式工作負載方面會遇到重大挑戰。因此，在這項工作中，我們探討了異質資料流程 MCM AI 加速器中的機會。我們確定在異質資料流程 MCM AI 加速器上排程多模式工作負載是一個重要且具有挑戰性的問題，因為它具有重要性和規模，即使在 6x6 晶片單一模型案例中，其規模也達到 O(10^18)。我們開發了一組啟發式方法來導航龐大的排程空間，並將它們編纂成一個排程器，其中包含先進的技術，例如晶片間流水線。我們對資料中心多租戶和 AR/VR 使用案例的十個多模式工作負載場景進行評估，結果顯示了我們方法的功效，與同質基準相比，對於各自的應用程式設定，平均節省了 35.3% 和 31.4% 的能量延遲乘積 (EDP)。</paragraph>
...
摘要：傳統的強化學習來自人類回饋 (RLHF) 的方法依賴於參數模型，例如布拉德利-泰瑞模型，在捕捉人類偏好的非傳遞性和非理性方面有所不足。最近的進展表明，直接使用偏好機率可以更準確地反映人類的偏好，從而實現更靈活和準確的語言模型對齊。在本文中，我們提出了一種基於自玩的方法進行語言模型對齊，將問題視為一個恆和雙人遊戲，旨在識別納什均衡策略。我們的方法稱為「自玩偏好最佳化」(SPPO)，通過反覆策略更新來逼近納什均衡，並享有理論收斂保證。我們的模型可以有效地增加所選回應的對數似然度，並減少被拒絕回應的對數似然度，這不能通過對稱成對損失（例如直接偏好最佳化 (DPO) 和身分偏好最佳化 (IPO)）輕鬆實現。在我們的實驗中，僅使用 UltraFeedback 資料集中的 60k 個提示（沒有回應），並且沒有任何提示擴充，通過利用僅有 0.4B 參數的預訓練偏好模型 PairRM，SPPO 可以從微調 Mistral-7B-Instruct-v0.2 中獲得一個模型，在 AlpacaEval 2.0 上對抗 GPT-4-Turbo 時，實現了最先進的長度控制勝率 28.53%。它還在 MT-Bench 和 Open LLM 排行榜上優於（反覆）DPO 和 IPO。值得注意的是，SPPO 的強勁效能是在沒有來自 GPT-4 或其他更強大的語言模型的額外外部監督（例如回應、偏好等）的情況下實現的。
...
摘要：本研究針對最新的大型語言模型 Llama-3，提出一個有目標的模型編輯分析。我們探討流行的模型編輯技術，包括 ROME、MEMIT 和 EMMET，這些技術都是為精確的層級介入而設計。我們透過評估來找出最有效的層級，進行有目標的編輯，評估範圍涵蓋 4096 次編輯，採用三種不同的策略：循序編輯、批次編輯，以及我們稱為循序批次編輯的混合方法。我們的研究結果顯示，與循序使用較小的編輯批次進行等量的編輯相比，增加編輯批次大小可能會更顯著地降低模型效能。因此，我們認為循序模型編輯是擴充模型編輯方法的重要組成部分，未來的研究應著重於結合批次和循序編輯的兩種方法。這個觀察結果指出目前模型編輯方法的一個潛在限制，即推動使用更大的編輯批次大小，我們希望它能為未來優化批次大小和模型編輯效能的研究鋪路。
...
摘要：语义文本相关性是语义相似性的一个更广泛的概念。它衡量两段文本传达相似含义或主题或共享相关概念或上下文的程度。这种相关性概念可应用于各种应用程序，例如文档聚类和摘要。SemRel-2024 是 SemEval-2024 中的一项共享任务，旨在通过为包括阿拉伯语在内的 14 种语言和方言提供数据集来缩小语义相关性任务中的差距。本文报告了我们参与 A 轨道（阿尔及利亚和摩洛哥方言）和 B 轨道（现代标准阿拉伯语）的情况。一个基于 BERT 的模型被增强并微调用于监督轨道 (A) 中的回归评分，而基于 BERT 的余弦相似度被用于无监督轨道 (B)。我们的系统在 SemRel-2024 中的 MSA 中排名第一，Spearman 相关系数得分为 0.49。我们在摩洛哥排名第 5，在阿尔及利亚排名第 12，得分分别为 0.83 和 0.53。
...
摘要：對於長篇文件摘要，語篇結構對於辨別文本的關鍵內容和句子之間的重要程度差異非常重要。不幸的是，將語篇結構理論 (RST) 整合到長篇文件摘要的參數高效微調策略中仍然未被探索。因此，本文介紹了 RST-LoRA，並提出了四種 RST 感知變體，以將 RST 明確納入 LoRA 模型中。我們的實證評估表明，納入語篇關係的類型和不確定性可以補充增強 LoRA 在摘要任務中的性能。此外，我們引入的性能最佳的變體優於香草 LoRA 和全參數微調模型，正如多項自動和人工評估所證實的那樣，甚至超越了以前最先進的方法。
...
摘要：為了在不確定的環境中安全地規劃，代理人必須平衡效用與安全約束。安全的規劃問題可以建模為機會約束的部分可觀察馬可夫決策過程 (CC-POMDP)，而解決方案通常使用昂貴的滾動或啟發法來估計最佳值和動作選擇策略。這項工作引入了受限零策略迭代演算法，它透過學習具有估計給定信念的失敗機率的額外網路頭的最佳值和策略的神經網路近似值，在信念空間中解決 CC-POMDP。此失敗機率會在線上蒙地卡羅樹狀搜尋 (MCTS) 期間引導安全的動作選擇。為了避免過度強調基於失敗估計的搜尋，我們引入了 $\Delta$-MCTS，它使用適應共形推論在規劃期間更新失敗閾值。此方法在安全關鍵的 POMDP 基準、飛機防撞系統和安全 CO$_2$ 儲存的可持續性問題上進行了測試。結果顯示，透過將安全約束與目標分開，我們可以在不最佳化獎勵與成本之間的平衡下，達成目標的安全等級。
...
摘要：近期研究透過訓練後量化或低位元權重表示法，針對大型語言模型 (LLM) 引進了有效的壓縮技術。
儘管量化權重提供了儲存效率，並允許更快的推論，但現有研究指出量化可能會損害效能，並加劇 LLM 中的偏差。本研究探討量化模型的信心和校準，考慮語言模型類型和規模等因素，作為量化損失的影響因素。首先，我們揭示使用 GPTQ 量化到 4 位元會導致對真實標籤的信心下降，在不同的語言模型中觀察到不同的影響。其次，我們觀察到不同規模對信心影響的波動。最後，我們根據信心水準，提出量化損失的解釋，指出量化會對原本完整模型展現低信心水準的樣本造成不成比例的影響。
...
不正確的輸出，可能會誤導依賴它們的使用者，彷彿它們是正確的。為了減少這種過度依賴，有人呼籲 LLM 向最終使用者傳達它們的不確定性。然而，很少有實證研究探討使用者如何感知和採取行動來回應 LLM 表達的不確定性。我們透過一項大規模、預先註冊的人類主體實驗 (N=404) 來探討這個問題，參與者在有或沒有來自虛構的 LLM 注入式搜尋引擎的回應的情況下回答醫療問題。使用行為和自我報告的測量，我們探討了不確定性的不同自然語言表達如何影響參與者的依賴性、信任度和整體任務表現。我們發現第一人稱表達（例如「我不確定，但...」）會降低參與者對系統的信心和同意系統答案的傾向，同時提高參與者的準確性。探索性分析表明，這種提高可以歸因於減少（但並未完全消除）對不正確答案的過度依賴。雖然我們觀察到從一般角度表達的不確定性（例如「這不確定，但...」）有類似的影響，但這些影響較弱且沒有統計意義。我們的研究結果表明，使用不確定性的自然語言表達可能是減少對 LLM 過度依賴的有效方法，但使用的精確語言很重要。這突顯了在大規模部署 LLM 之前進行使用者測試的重要性。</paragraph>
...
摘要：AI 基礎模型在各種應用中獲得關注，包括放射學等醫療領域。然而，醫療基礎模型通常在有限的任務上進行測試，讓它們的泛化性和偏差未被探索。我們提出 RayDINO，一個大型視覺編碼器，由 873k 胸部 X 光片的自我監督訓練。我們將 RayDINO 與先前的最先進模型進行比較，涵蓋九項放射學任務，從分類和密集分割到文本生成，並深入分析我們模型的人口、年齡和性別偏差。我們的研究結果表明，自我監督允許以患者為中心的 AI 在臨床工作流程中發揮作用，並全面解釋 X 光片。透過 RayDINO 和小型特定任務適配器，我們達到了最先進的結果，並改善了對未見人群的泛化，同時減輕了偏差，說明了基礎模型的真正承諾：多功能性和穩健性。
...
摘要：論證結構學習 (ASL) 涉及預測論證之間的關係。由於它可以建構文件以利於理解，因此已廣泛應用於許多領域（醫療、商業和科學領域）。儘管廣泛使用，ASL 仍然是一項具有挑戰性的任務，因為它涉及檢查潛在非結構化論述中句子之間的複雜關係。為了解決這個問題，我們開發了一種簡單但有效的方法，稱為 ASL 任務的雙塔多尺度卷積神經網路 (DMON)。具體來說，我們將論證組織成關係矩陣，該矩陣與論證嵌入一起形成關係張量，並設計一種機制來捕捉與上下文論證的關係。在三個不同領域的論證挖掘資料集上的實驗結果表明，我們的框架優於最先進的模型。程式碼可在 https://github.com/VRCMF/DMON.git 取得。
...
摘要：視覺語言模型 (VLM) 近期浮出檯面並備受矚目，因為它們具備理解影像與文字資料二元模式的能力。LLaVA、ChatGPT-4 和 Gemini 等 VLM 近期在自然影像標題、視覺問題解答 (VQA) 和空間推理等任務中展現令人印象深刻的表現。此外，Meta AI 的通用分割模型 Segment Anything Model (SAM) 在從未預見的影像中分離物體方面展現前所未有的效能。由於醫學專家、生物學家和材料科學家例行檢查顯微鏡或醫學影像，並結合標題、文獻或報告等形式的文字資訊，並得出極為重要且有價值的結論，因此無庸置疑地，在這些影像上測試 VLM 和 SAM 等基礎模型的效能至關重要。在這項研究中，我們讓 ChatGPT、LLaVA、Gemini 和 SAM 執行各種顯微鏡影像的分類、分割、計算和 VQA 任務。我們觀察到 ChatGPT 和 Gemini 能夠令人印象深刻地理解顯微鏡影像中的視覺特徵，而 SAM 則相當有能力從一般意義上分離出人工製品。然而，效能並未接近領域專家 - 這些模型很容易受到影像中雜質、缺陷、人工製品重疊和多樣性的影響。
...
摘要：<paragraph>廣泛部署的大型語言模型 (LLM) 可以產生令人信服但
不正確的輸出，可能會誤導依賴它們的使用者，好像它們是
正確的。為了減少這種過度依賴，有人呼籲 LLM 向最終使用者
傳達它們的不確定性。然而，很少有實證研究探討使用者如何
感知和根據 LLM 的不確定性表達採取行動。我們透過大規模、預先
註冊的人類主體實驗 (N=404) 來探討這個問題，其中參與者回答
醫療問題，有或沒有使用虛構的 LLM 注入式搜尋引擎的回應。
我們使用行為和自我報告的測量方法，探討不同的
自然語言不確定性表達如何影響參與者的依賴性、
信任度和整體任務表現。我們發現第一人稱表達
（例如，「我不確定，但...」）會降低參與者對系統的信心
和同意系統答案的傾向，同時提高參與者的
準確度。探索性分析表明，這種增加可以歸因於
減少（但未完全消除）對不正確答案的過度依賴。雖然
我們觀察到從一般角度表達的不確定性（例如，「這不清楚，但...」）有類似的影響，這些影響較弱，而且
沒有統計意義。我們的研究結果表明，使用自然語言
不確定性表達可能是減少對 LLM 過度依賴的有效方法，但
使用的精確語言很重要。這突顯了在廣泛部署 LLM 之前進行使用者測試的重要性。</paragraph>
...
摘要：超音波機器人愈來愈廣泛地用於醫療診斷和早期疾病篩檢。然而，現有的超音波機器人缺乏理解人類意圖和指令的智慧，阻礙了自主超音波掃描。為了解決這個問題，我們提出了一個新穎的超音波具身智慧系統，該系統為超音波機器人配備大型語言模型 (LLM) 和領域知識，從而提高超音波機器人的效率。具體來說，我們首先設計了一個超音波操作知識資料庫，將超音波掃描專業知識新增到 LLM 中，使 LLM 能夠執行精確的動作規劃。此外，我們根據「思考-觀察-執行」提示工程設計了一個動態超音波掃描策略，允許 LLM 在掃描過程中動態調整動作規劃策略。廣泛的實驗證明，我們的系統顯著提高了超音波掃描效率和品質，並透過口頭命令實現。自主醫療掃描技術的這項進展有助於非侵入性診斷和簡化的醫療工作流程。
...
摘要：大型語言模型 (LLM) 已在許多數學推理基準測試中獲得令人印象深刻的成功。然而，對於其中一些表現實際上反映了資料集污染的擔憂日益增加，其中與基準測試問題非常相似的資料會洩漏到訓練資料中，而不是真正的推理能力。為了嚴格調查此說法，我們委託了小學數學 1000 (GSM1k)。GSM1k 旨在反映已建立的 GSM8k 基準的風格和複雜性，這是衡量小學數學推理的黃金標準。我們確保這兩個基準在人類解決率、解決步驟數、答案大小等重要指標上具有可比性。在對 GSM1k 進行評估領先的開放和閉源 LLM 時，我們觀察到準確度下降了 13%，其中幾組模型（例如 Phi 和 Mistral）顯示出幾乎所有模型大小的系統性過度擬合證據。同時，許多模型，特別是處於前沿的模型（例如 Gemini/GPT/Claude）顯示出最小的過度擬合跡象。進一步的分析表明，從 GSM8k 生成範例的模型機率與其在 GSM8k 和 GSM1k 之間的效能差距之間存在正相關關係（Spearman's r^2=0.32），這表明許多模型可能已部分記憶了 GSM8k。
...
摘要：語音情感辨識 (SER) 因其在各領域廣泛的應用而備受關注，包括人機互動、虛擬助理和心理健康協助。然而，現有的 SER 方法常常忽略預訓練語音辨識任務和下游 SER 任務之間的資訊差距，導致次佳的表現。此外，目前的 SER 方法需要花費大量時間來微調每個特定語音資料集，例如 IEMOCAP，這限制了其在具有大規模雜訊資料的真實世界場景中的有效性。為了解決這些問題，我們提出了一個基於主動學習 (AL) 的 SER 微調框架，稱為 \textsc{After}，它利用任務適應預訓練 (TAPT) 和 AL 方法來提升表現和效率。具體來說，我們首先使用 TAPT 來最小化預訓練語音辨識任務和下游語音情感辨識任務之間的資訊差距。然後，採用 AL 方法來反覆選擇最具資訊性和多樣性的子集樣本進行微調，從而減少時間消耗。實驗證明，我們提出的方法 \textsc{After}，僅使用 20% 的樣本，就將準確度提升了 8.45%，並將時間消耗減少了 79%。\textsc{After} 的額外延伸和消融研究進一步證實了其在各種真實世界場景中的有效性和適用性。我們的原始程式碼可在 Github 上取得，以供重現。(https://github.com/Clearloveyuan/AFTER)
...
摘要：每年，植物寄生線蟲，植物病原體的主要群組之一，會造成全世界作物大量損失。為了減輕由線蟲造成的作物產量損失，有效率的線蟲監控方法對於植物和作物疾病管理至關重要。在其他方面，有效率的線蟲偵測有助於醫學研究和藥物發現，因為線蟲是模式生物。隨著電腦科技的快速發展，電腦視覺技術提供了量化線蟲或線蟲感染的可行解決方案。在本文中，我們調查並分類線蟲偵測的研究和可用資料集，透過深度學習模型。為了刺激相關研究的進展，這項調查呈現了潛在的最新物體偵測模型、訓練技術、最佳化技術和評估指標，供深度學習初學者使用。此外，七種最先進的物體偵測模型已在三個公共資料集和植物寄生線蟲的 AgriNema 資料集上驗證，以建立線蟲偵測的基準。
...
摘要：<paragraph>醫學與人工智慧（AI）工程代表兩個截然不同的領域，各自都有數十年的已發表歷史。隨著歷史的推移，產生了一組術語，並有特定的應用方式。然而，當兩個具有重疊術語的截然不同的領域開始合作時，可能會發生溝通不良和誤解。這篇敘述性評論旨在為這些術語提供歷史背景，強調在醫學 AI 背景下使用這些術語時清晰度的重要性，並提供解決方案以減輕來自任一領域的讀者的誤解。透過檢視歷史文件，包括文章、撰寫指南和教科書，這篇評論追溯了資料集及其影響的術語的發散演變。最初，探索了醫學和 AI 背景中「驗證」一詞的不一致詮釋。然後對用於 AI 評估的資料集進行分類，即隨機拆分、交叉驗證、時間、地理、內部和外部集合。這些資料集的準確且標準化的描述對於證明 AI 應用在醫學中的穩健性和概括性至關重要。這篇評論澄清了現有文獻，以全面了解這些分類及其在 AI 評估中的影響。然後，這篇評論找出經常被誤解的術語，並提出務實的解決方案來減輕術語混淆。這些解決方案包括使用標準化術語，例如「訓練集」、「驗證（或調整）集」和「測試集」，以及在每個醫學 AI 研究出版物中明確定義資料集拆分術語。這篇評論旨在提高醫學 AI 中溝通的精確性，從而促進這個跨學科領域更有效和透明的研究方法。</paragraph>
...
摘要：了解医疗诊断中影像所示状况的严重性至关重要，可作为临床评估、治疗以及评估纵向进展的关键指南。本文提出 Con-PrO：一种使用对比学习集成偏好优化的医学影像严重性评估新颖表征学习方法。与最大化类间距离的传统对比学习方法不同，ConPrO 将各种严重性类和正常类之间的距离偏好知识注入潜在向量。我们系统地检查了框架的关键组件，以阐明对比预测任务如何获取有价值的表征。我们展示了我们的表征学习框架在特征空间中提供了有价值的严重性排序，同时在分类任务中优于以前最先进的方法。与有监督和自监督基线相比，我们分别实现了 6% 和 20% 的相对改进。此外，我们对严重性指标和偏好比较在医学领域的相关应用进行了讨论。
...
摘要：在胸部 X 光 (CXR) 影像分析領域中，放射科醫師會仔細檢查各種區域，並在報告中記錄他們的觀察結果。CXR 診斷中錯誤的盛行，特別是在缺乏經驗的放射科醫師和醫院住院醫師中，強調了理解放射科醫師的意圖和相應的關注區域的重要性。這種理解對於糾正錯誤至關重要，因為它可以引導放射科醫師關注準確的區域，特別是在診斷胸部 X 光片異常時。為了回應這個當務之急，我們提出了一個新穎的系統，旨在識別放射科醫師在報告中表達的主要意圖和 CXR 影像中相應的關注區域。這個系統試圖闡明放射科醫師文字發現背後的視覺背景，並有可能糾正經驗較少的從業人員所犯的錯誤，並引導他們關注準確的區域。重要的是，所提出的系統可以作為向醫院中經驗不足的放射科醫師或初級住院醫師提供建設性回饋的工具，從而彌合面對面溝通的差距。這個系統代表了一個有價值的工具，可用於提高診斷準確性，並在醫療界內培養持續學習。
...
摘要：<paragraph>建構多感官 AI 系統，從文字、語音、影片、真實世界感測器、穿戴式裝置和醫療資料等多重感官輸入中學習，對許多科學領域具有重大影響力，並帶來實際效益，例如支援人類健康和福祉、協助多媒體內容處理，以及強化真實世界的自主代理人。本論文透過綜合一系列的理論架構和應用領域，旨在推進多感官 AI 的機器學習基礎。在第一部分，我們提出一個理論架構，用以正式化各個感官如何彼此互動，並為某項任務產生新的資訊。這些互動是所有多模態問題中的基本建構區塊，而對其進行量化，能使用戶瞭解其多模態資料集，設計出學習這些互動的原則性方法，並分析其模型是否已成功學習。在第二部分，我們研究實用多模態基礎模型的設計，該模型可概化至許多感官和任務，這代表著邁向將大型語言模型基礎化至真實世界的感官模式的一步。我們介紹了 MultiBench，一個統一的大規模基準，涵蓋了廣泛的感官、任務和研究領域，接著介紹了跨模態注意力和多模態轉換器架構，這些架構現已成為許多現今多模態基礎模型的基礎。在 MultiBench 上擴充這些架構，能建構出通用多感官 AI 系統，而我們也討論了我們在情感運算、心理健康、癌症預後和機器人技術中應用這些模型以產生真實世界影響力的合作成果。最後，我們透過討論未來的研究如何運用這些想法，朝向更通用、互動且安全的 AI 邁進，為本論文作結。</paragraph>
...
摘要：<paragraph>最近的人工智慧突破，將根本提升我們對醫療保健的研究和理解。整合多對多架構的開發，利用多重資料模式輸入，用於多重醫療任務的分析建模，對於統一理解現代醫學至關重要。在這項工作中，我們介紹 M3H，一個可解釋的多模式多任務機器學習醫療保健架構，它整合來自廣泛醫療任務類別和機器學習問題類別的多模式輸入的學習。該架構的模組化設計確保其資料處理、任務定義和快速模型建模的可概化性，適用於臨床和運營醫療保健設定。我們透過驗證從四種模式（表格、時間序列、語言和視覺）訓練的模型，在四個機器學習問題類別中的 41 個醫療任務上，評估 M3H 架構。我們的結果證明，M3H 持續產生多任務模型，其表現優於標準單任務模型（1.1- 37.2%），涵蓋來自 16 個醫療部門的 37 種疾病診斷、三個醫院運營預測和一個患者表型任務：跨越監督二元分類、多類別分類、回歸和聚類的 ML 問題類別。此外，該架構引入了一種新穎的注意力機制，以平衡自我開發（專注於學習來源任務）和交叉探索（鼓勵從其他任務中學習）。此外，M3H 提供可解釋性的見解，說明額外任務的聯合學習如何影響來源任務的學習，使用建議的 TIM 分數，闡明任務相互依賴性的動態。其可適應的架構促進自訂和整合，使其成為未來人工智慧驅動醫療保健系統的強大且可擴充的候選解決方案。</paragraph>
...
摘要：器官分割是醫學影像的基本任務，且有助於許多臨床自動化流程。通常，此程序包含分割整個體積，當感興趣點受限時，此程序可能不必要。在這些情況下，可以使用分類器取代分割。然而，分類器的大小和速度之間存在固有的權衡。為了解決此問題，我們提出了一種新方法，該方法採用資料選擇策略，在廣闊的視野中進行稀疏取樣，而無需影像重新取樣。此稀疏取樣策略使得在不使用加速器的情況下，可以將體素分類為多個器官。雖然我們的方法是一種獨立的分類器，但它可以透過查詢任何解析度的網格位置來產生完整的分割。我們已將我們的技術與現有的分割技術進行比較，證明其在醫學影像實際應用中具有優異執行時間的潛力。
...
摘要：量子機器學習 (QML) 是量子運算一個有前途的早期使用案例。過去五年來，從理論研究和數值模擬到概念驗證，都有所進展。在當代量子裝置上展示的使用案例包括對醫學影像和 Iris 資料集中的項目進行分類、對手寫影像進行分類和產生、毒性篩選，以及學習機率分佈。QML 的潛在優點包括更快的訓練和識別古典中找不到的特徵圖。雖然這些範例缺乏商業利用的規模，而且可能要好幾年 QML 演算法才會取代古典解法，但 QML 是個令人興奮的領域。
這篇文章是寫給那些已經對量子運算有扎實知識，現在希望對術語和一些古典機器學習的應用程式有基本概觀，準備好研究量子機器學習的人。讀者已經了解相關的線性代數，包括希爾伯特空間，一個具有內積的向量空間。
...
摘要：聯邦學習 (FL) 允許多個注重隱私的應用程式利用其資料集來建構全球模型，而無須揭露任何資訊。其中一個領域是醫療保健，在該領域中，各個孤島團體合作，以便產生具備更佳準確度和概括性的全球預測器。然而，其內在挑戰在於醫療資料的高度異質性，這使得評估和補償需要用到精密的技術。本文全面探討了 FL 環境中異質性的數學形式化和分類，重點在於醫療資料的複雜性。特別是，我們針對最受歡迎的 FL 演算法進行評估和比較，以了解其應對基於數量、特徵和標籤分佈的異質性的能力。目標是提供 FL 系統中資料異質性對醫療保健網路的影響的量化評估，以及 FL 演算法選擇的指南。我們的研究超越現有研究，透過針對醫療資料使用案例所帶來的獨特挑戰，對七種最常見的 FL 演算法進行基準測試。本文針對由不同聯合醫院孤島收集的一組表格臨床報告來預測中風復發的風險：此情境中經常遇到的資料異質性及其對 FL 效能的影響將會被討論。
...
摘要：本篇論文將 Struts 和 Hibernate 兩個架構結合起來，使用 DAO（資料存取物件）來儲存和存取資料。然後建立一套適用於深度網路的雙模態濕度醫療影像庫，並提出基於影像的雙模態醫療影像輔助診斷方法。通過各種特徵提取方法的測試，曲線產品下的最佳操作特徵（AUROC）為 0.9985，召回率為 0.9814，準確率為 0.9833。此方法可應用於臨床診斷，是一種實用的方法。任何門診醫師均可透過系統快速註冊，或登入平台上傳影像以取得更精準的影像。各門診醫師透過系統，均可快速註冊或登入平台上傳影像，進而取得更精準的影像。影像的分割，可指導臨床科別的醫師。再針對影像進行分析，判斷腫瘤的位置及性質，進而進行標靶治療。
...
摘要：<paragraph>在各種醫療應用中追求卓越對 AI 來說是一項嚴峻的挑戰，需要進階推理、取得最新的醫療知識，以及理解複雜的多模態資料。Gemini 模型在多模態和長語境推理方面具備強大的通用能力，為醫學領域提供了令人興奮的可能性。我們以 Gemini 的這些核心優勢為基礎，推出了 Med-Gemini，這是一個高度專業化於醫學領域的多模態模型系列，它能夠無縫使用網路搜尋，並且可以使用自訂編碼器有效地調整為新的模態。我們在 14 個醫療基準測試中評估了 Med-Gemini，在其中 10 個基準測試中創下了新的最先進 (SoTA) 效能，並且在每個基準測試中都超越了 GPT-4 模型系列，其中許多基準測試的差距很大。在廣受歡迎的 MedQA (USMLE) 基準測試中，我們效能最佳的 Med-Gemini 模型使用一種新的不確定性引導搜尋策略，達到了 91.1% 精確度的 SoTA 效能。在包括 NEJM 影像挑戰和 MMMU（健康與醫學）在內的 7 個多模態基準測試中，Med-Gemini 比 GPT-4V 提升了平均相對幅度 44.5%。我們透過在從長期的去識別化健康記錄和醫療影片問答中進行大海撈針的檢索任務上達到 SoTA 效能，證明了 Med-Gemini 的長語境能力的有效性，超越了僅使用語境內學習的先前客製化方法。最後，Med-Gemini 的效能表明了其實際用途，它在醫療文字摘要等任務上超越了人類專家，同時也展示了在多模態醫療對話、醫療研究和教育方面具有令人期待的潛力。綜合來看，我們的結果為 Med-Gemini 的潛力提供了令人信服的證據，儘管在這個對安全性至關重要的領域進行實際部署之前，進一步嚴謹的評估至關重要。</paragraph>
...
摘要：對稱群 $S_{n}$ 在廣泛的量子系統中展現其本身，因為量子態的某些特徵對於量子位元置換不變。$S_{n}$ 的子群會出現在許多其他脈絡中，用於描述經典圖像相對於空間轉換（例如反射或旋轉）的標籤對稱性。本研究備有幾何量子機器學習的形式主義，我們提出與 $S_{n}$ 及其子群相關的等變量子卷積神經網路 (EQCNN) 架構。我們證明仔細選擇像素到量子位元嵌入順序可以簡化 $S_{n}$ 小子群的 EQCNN 建構。我們對應於完全置換群 $S_{n}$ 的新穎 EQCNN 架構是透過以相等的機率套用所有可能的 QCNN 所建構，這也可以概念化为量子神經網路中的中斷策略。對於 $S_{n}$ 的子群，我們使用 MNIST 資料集的數值結果顯示出比非等變 QCNN 更佳的分類準確度。$S_{n}$ 等變 QCNN 架構顯示出比非等變 QCNN 明顯更佳的訓練和測試效能，用於連接和非連接圖形的分類。當使用足夠大量的資料進行訓練時，$S_{n}$ 等變 QCNN 顯示出比 $S_{n}$ 等變 QNN 更好的平均效能。這些結果有助於在置換對稱系統中建構強大的量子機器學習架構。
...
摘要：準確呈現醫療資訊對於病患安全至關重要，然而，大型語言模型 (LLM) 等人工智慧 (AI) 系統在無錯誤的臨床文字詮釋方面遭遇挑戰。本文提出了一種新穎的方法，已提交至 MEDIQA-CORR 2024 共享任務（Ben Abacha 等人，2024a），專注於自動更正臨床筆記中的單字錯誤。與依賴廣泛通用資料的 LLM 不同，我們的做法強調從可用的臨床文字資料中萃取與脈絡相關的資訊。透過運用萃取式和摘要式問答方法的集合，我們建構了一個具備領域特定特徵工程的監督式學習架構。我們的做法結合了領域專業知識，以提升錯誤更正的準確度。透過整合領域專業知識，並優先考量有意義的資訊萃取，我們的做法強調了以人為中心的策略在醫療保健中採用 AI 的重要性。
...
摘要：本文探討了群體結構多智能體系統 (MAS) 在建立醫療必要性方面的應用，這是一個涉及系統性地根據臨床指南審查特定患者的醫療結構化和非結構化資料的流程。我們透過將此複雜任務分解成較小、更易於管理的子任務來解決。每個子任務都由一個專門的 AI 代理處理。我們對各種提示策略對這些代理的影響進行系統性研究，並對不同的大型語言模型 (LLM) 進行基準測試，以確定它們完成這些任務的準確性。此外，我們探討了這些代理如何提供可解釋性，從而增強系統內的信任和透明度。
...
摘要：現有的基於 X 光的預訓練視覺模型通常在相對小規模的數據集（少於 50 萬個樣本）上進行，解析度有限（例如 224 $\times$ 224）。然而，自監督預訓練大型模型成功的關鍵在於海量的訓練數據，而 X 光影像領域中維持高解析度是有效解決各種困難疾病的保證。在本文中，我們通過提出基於我們新收集的大規模數據集的第一個高解析度（1280 $\times$ 1280）X 光預訓練基礎視覺模型來解決這些問題，該數據集包含超過 100 萬張 X 光影像。我們的模型遵循遮罩自動編碼器框架，該框架採用經過遮罩處理（以高比率）後的符號作為輸入，並由 Transformer 編碼器-解碼器網路重建遮罩影像塊。更重要的是，我們引入了一種新穎的上下文感知遮罩策略，該策略將胸部輪廓用作自適應遮罩操作的邊界。我們在兩個下游任務（包括 X 光報告生成和疾病識別）上驗證了我們模型的有效性。大量的實驗表明，我們預訓練的醫學基礎視覺模型在下游基準數據集上實現了可比甚至新的最先進性能。本文的原始碼和預訓練模型將在 https://github.com/Event-AHU/Medical_Image_Analysis 上發布。
...
摘要：放射學報告生成 (R2Gen) 展示了多模態大型語言模型 (MLLM) 如何自動化準確且連貫的放射學報告的建立。現有方法經常在基於文字的報告中產生幻覺細節，這些細節並不能準確反映影像內容。為了減輕這個問題，我們引入了一種新策略，SERPENT-VLM（使用視覺語言模型的自我精煉放射報告生成），它通過將自我精煉機制整合到 MLLM 框架中來改進 R2Gen 任務。我們採用了一種獨特的自我監督損失，它利用了匯集的影像表示與生成的放射學文字的上下文表示之間的相似性，以及標準因果語言建模目標，來精煉影像文字表示。這允許模型審查並通過給定影像與生成的文字之間的動態交互來校準生成的文字，從而減少幻覺並持續增強細緻的報告生成。SERPENT-VLM 優於現有的基準，例如 LLaVA-Med、BiomedGPT 等，在 IU X 光和情境中的放射學物件 (ROCO) 資料集上實現了 SoTA 效能，並證明對雜訊影像具有穩健性。定性案例研究強調了朝著更精密的 MLLM 框架邁出的重大進展，為進一步研究醫學影像領域中的自我監督精煉打開了道路。
...
摘要：卷積神經網路 (CNN) 已廣泛用於醫學影像分割任務，展示出令人滿意的效能。然而，卷積架構中固有的歸納偏誤限制了其建模長程依賴性和空間關聯性的能力。雖然最近基於Transformer的架構透過利用自注意力機制編碼長程依賴性和學習表達式表示來解決這些限制，但它們經常難以提取低階特徵，且高度依賴資料可用性。這促使我們開發 GLIMS，這是一種資料有效率的注意力引導混合體積分割網路。GLIMS 利用擴張特徵聚合器卷積區塊 (DACB) 來有效擷取局部和全域特徵關聯性。此外，整合的 Swin Transformer 為基礎的瓶頸橋接了局部和全域特徵，以改善模型的穩健性。此外，GLIMS 透過通道和空間感知注意力區塊 (CSAB) 採用注意力引導分割方法，以定位表達式特徵進行細粒度邊界分割。在神經膠母細胞瘤和多器官電腦斷層掃描分割任務上的量化和定性結果證明了 GLIMS 在複雜性和準確性方面的有效性。GLIMS 在 BraTS2021 和 BTCV 資料集上展現出傑出的效能，超越了 Swin UNETR 的效能。值得注意的是，GLIMS 以顯著減少的可訓練參數數量達到了如此高的效能。具體來說，GLIMS 有 47.16M 個可訓練參數和 72.30G FLOP，而 Swin UNETR 有 61.98M 個可訓練參數和 394.84G FLOP。程式碼已公開於 https://github.com/yaziciz/GLIMS。
...
摘要：多模态融合专注于整合来自多个模态的信息，目标是更准确的预测，这在包括自动驾驶和医学诊断在内的广泛场景中取得了显著进展。然而，多模态融合的可靠性在很大程度上仍未得到探索，尤其是在低质量数据设置下。本文调查了多模态融合在实际应用中的常见挑战和最新进展，并以全面的分类法呈现。从以数据为中心的角度来看，我们确定了低质量数据多模态融合面临的四个主要挑战，即（1）被异构噪声污染的噪声多模态数据，（2）某些模态缺失的不完整多模态数据，（3）不同模态的质量或属性显著不同的不平衡多模态数据，以及（4）每个模态的质量相对于不同样本动态变化的质量变化多模态数据。这一新的分类法将使研究人员能够了解该领域的现状，并确定几个潜在的方向。我们还对该领域中的开放性问题以及有趣的未来研究方向进行了讨论。
...
摘要：<paragraph>人工智能 (AI) 整合在醫療保健中，展現了提升運作效率和健康成果的轉化潛力。大型語言模型 (LLM)，例如 ChatGPT，已展現出其在支援醫療決策制定的能力。將 LLM 嵌入醫療系統，正成為醫療保健發展的一大趨勢。ChatGPT 解決急診部門分類問題的潛力已受到檢視，而鮮少有研究探討其在門診部門的應用。本研究專注於簡化工作流程和提升門診分類的效率，特別旨在評估 ChatGPT 在門診指導中所提供回應的一致性，包括版本內回應分析和版本間比較。針對版本內，結果顯示 ChatGPT-4.0 的內部回應一致性顯著高於 ChatGPT-3.5（p=0.03），且兩者在其首要建議中皆具備中等一致性（4.0 為 71.2%，3.5 為 59.6%）。然而，版本間一致性相對較低（平均一致性分數=1.43/3，中位數=1），表示兩個版本之間的建議匹配很少。此外，在比較中只有 50% 的首要建議完全匹配。有趣的是，ChatGPT-3.5 的回應比 ChatGPT-4.0 的回應更有可能完整（p=0.02），這表示兩個版本之間在資訊處理和回應產生方面可能存在差異。這些發現提供了對 AI 輔助門診作業的見解，同時也促進了探索 LLM 在醫療保健應用中的潛力與限制。未來的研究可能會專注於根據人體工學和人為因素原則，仔細最佳化 LLM 和 AI 在醫療保健系統中的整合，以精準符合有效門診分類的特定需求。</paragraph>
...
摘要：本文介紹我們團隊參與 MEDIQA-ClinicalNLP2024 共享任務 B 的成果。我們提出了一種診斷臨床皮膚科病例的新方法，方法是整合大型多模態模型，特別是利用 GPT-4V 在檢索器和重新排序架構下的能力。我們的調查顯示，GPT-4V 作為檢索代理時，使用皮膚科影像和簡短的病歷，可以在 85% 的時間內準確檢索出正確的皮膚狀況。此外，我們憑經驗證明，天真的思考鏈 (CoT) 適用於檢索，而基於醫療指南的 CoT 則適用於準確的皮膚科診斷。此外，我們引入了多代理對話 (MAC) 架構，並展示了其優於最佳 CoT 策略的卓越效能和潛力。實驗表明，使用天真的 CoT 進行檢索，並使用多代理對話進行基於批判的診斷，GPT-4V 可以對皮膚科疾病進行早期且準確的診斷。這項工作的意義在於改善診斷工作流程、支援皮膚科教育，以及透過提供可擴充、可存取且準確的診斷工具來提升病患照護。
...
摘要：從受影響組織中進行細粒度的異常細胞偵測對於臨床診斷和病理研究至關重要。單細胞定序資料為此任務提供了前所未有的機會。然而，目前的異常偵測方法難以處理多樣本和多領域單細胞定序資料中普遍存在的領域轉移，導致次佳效能。此外，這些方法無法將異常細胞區分為病理上不同的亞型。為了解決這個問題，我們提出了 ACSleuth，一個新穎的重建偏差引導生成架構，它將異常細胞的偵測、領域適應和細粒度註解整合到一個方法論上的一致工作流程中。值得注意的是，我們提出了第一個理論分析，使用生成模型輸出的重建偏差來進行異常偵測，以取代領域轉移。這個分析讓我們得以在 ACSleuth 中開發一個新穎且優異的最大平均差異異常評分器。在各種單細胞資料和其他類型的表格資料上的廣泛基準測試證明了 ACSleuth 在多樣本和多領域環境中識別和對異常進行亞型分類方面優於最先進的方法。我們的程式碼可在 https://github.com/Catchxu/ACsleuth 取得。
...
摘要：多年来，多模态移动传感已被广泛用于对健康和幸福、行为和环境进行推论。然而，阻碍此类模型在现实世界场景中广泛部署的一个重大挑战是分布偏移问题。这是训练集中数据的分布与现实世界（即部署环境）中的数据分布不同的现象。虽然在计算机视觉和自然语言处理中得到了广泛的探索，并且虽然移动传感方面的先前研究简要地解决了这个问题，但当前的工作主要集中在处理单一数据模态（如音频或加速度计读数）的模型上，因此，在处理多模态传感器数据时，很少有无监督域自适应的研究。为了解决这一差距，我们对域对抗神经网络 (DANN) 进行了广泛的实验，表明它们可以有效地处理多模态传感器数据中的分布偏移。此外，我们提出了对 DANN 的一项新改进，称为 M3BAT，即用于多模态移动传感的无监督域自适应，采用多分支对抗训练，以在域自适应期间考虑传感器数据的模态性。通过对两个多模态移动传感数据集、三个推理任务和 14 个源目标域对（包括回归和分类）进行的广泛实验，我们证明了我们的方法在看不见的域上表现有效。与将源域中训练的模型直接部署到目标域相比，该模型在分类任务上的性能提高了 12% AUC（接收器工作特征曲线下的面积），在回归任务上的性能提高了 0.13 MAE（平均绝对误差）。
...
摘要：社交焦慮症在現代社會中是一個普遍的挑戰，影響著個人和專業領域的個人。如果不加以解決，這種情況可能會產生重大的負面後果，影響社交互動和表現。進一步了解其不同的生理和情緒症狀對於全面的診斷和量身定制的治療干預至關重要。本研究分析了梅約診所的社交焦慮症狀的流行率和頻率，從利用一個專門針對此問題的大型 Reddit 數據集中探索多樣化的人類經驗。利用這些平台，該研究旨在提取見解並檢查與社交焦慮症相關的一系列生理和情緒症狀。本研究維護數據集中嚴格的用戶匿名性，以維護倫理考量。通過採用一種新方法，該研究利用基於 BART 的多標籤零次分類來識別和衡量症狀流行率和顯著性，形式為每個所考慮症狀的概率分數。結果揭示了獨特的模式：「顫抖」表現為普遍的生理症狀，而「害怕受到負面評價」等情緒症狀則表現出高頻率。這些發現提供了對社交焦慮症多方面性質的見解，有助於臨床實踐和針對其不同表現量身定制的干預措施。
...
摘要：在這項工作中，我們提出了一個不確定性量化框架的新應用，稱為深度證據學習，在放射治療劑量預測領域中。使用開放知識基礎規劃挑戰資料集的醫學影像，我們發現這個模型可以有效地利用來產生不確定性估計，這些估計繼承了在網路訓練完成後與預測誤差相關的相關性。這是在為穩定實作重新制定原始損失函數後才達成的。我們發現 (i) 認識論不確定性與預測誤差高度相關，各種關聯指標與蒙地卡羅輟學法和深度整體方法相當或更強，(ii) 中值誤差隨不確定性閾值變化，對於深度證據學習中的認識論不確定性來說更線性，相對於這其他兩個傳統框架，這表示對模型誤差的靈敏度更均勻校準，(iii) 相對於認識論不確定性，隨機不確定性在對 CT 強度加入高斯雜訊後，其分佈有更顯著的變化，這與其解釋為反映資料雜訊相符。總的來說，我們的結果表明深度證據學習是一種有前途的方法，它可以賦予放射治療劑量預測中的深度學習模型統計穩健性。為了增強其臨床相關性，我們展示了我們如何使用這樣的模型來建構預測劑量體積直方圖的信心區間。
...
摘要：人工智能 (AI) 的普遍整合在涉及 AI 啟用系統的事故中引入了責任和問責制的複雜挑戰。這些系統的互連性、AI 引發事故的道德問題，加上 AI 技術的不確定性和缺乏相應的法規，使得傳統責任歸屬具有挑戰性。為此，本研究提出了一種計算反射平衡 (CRE) 方法，為所有利害關係人建立一個連貫且在道德上可接受的責任歸屬架構。計算方法提供了一種結構化分析，克服了概念方法在處理動態和多方面的場景時的限制，展示了該框架在責任歸屬過程中可解釋性、連貫性和適應性屬性。我們探討了與平衡計算中索賠相關的初始激活級別的關鍵作用。使用 AI 輔助醫療決策支援系統作為案例研究，我們說明了不同的初始化如何導致不同的責任分配。該框架提供了對 AI 引發事故中問責制的寶貴見解，通過持續監控、修訂和反思促進了可持續和有韌性的系統的開發。
...
摘要：對於安全地部署機器學習模型在開放世界中，對非分佈 (OOD) 樣本的穩健性至關重要。最近的研究集中在設計評分函數以量化 OOD 不確定性。由於 OOD 樣本通常在前期不可用，因此為這些評分函數設定適當的 OOD 檢測閾值具有挑戰性。通常，閾值設定為達到預期的真正陽性率 (TPR)，例如 95% TPR。然而，這可能導致非常高的假陽性率 (FPR)，範圍從 60% 到 96%，如在 Open-OOD 基準中觀察到的。在安全關鍵的現實應用中，例如醫療診斷，在動態處理各種 OOD 樣本時控制 FPR 至關重要。為了應對這些挑戰，我們提出了一個數學基礎的 OOD 檢測框架，該框架利用專家回饋來動態地「安全地」更新閾值。我們提供了理論結果，表明它保證始終滿足 FPR 約束，同時最大限度地減少人類回饋的使用。我們框架的另一個關鍵特徵是它可以與任何用於 OOD 不確定性量化的評分函數一起使用。我們系統在合成和基準 OOD 資料集上的經驗評估表明，我們的模型可以在最大化 TPR 的同時將 FPR 保持在最多 5%。
...
摘要：乳房攝影檢查影像上惡性病灶的偵測對於早期乳癌診斷至關重要。在臨床實務上，影像會從兩個不同的角度取得，而放射科醫師可以充分利用來自兩個視角的資訊，同時定位同一個病灶。然而，對於自動偵測方法而言，此類資訊融合仍是一項挑戰。在本文中，我們提出一個名為 MAMM-Net 的新模型，該模型允許同時處理兩個乳房攝影視角，透過共享資訊，不僅在物件層級（如現有研究中所見），還包括在特徵層級。MAMM-Net 的關鍵元件是融合層，其基於可變形注意力，旨在提高偵測精準度，同時保持高召回率。我們的實驗顯示，與現有最先進的模型相比，在公共 DDSM 資料集上表現優異，同時引入了新的有用功能，例如像素級別的病灶註解和病灶惡性分類。
...
摘要：本報告列出 13 項以計算方式兌現的功能條件，已論證為有意識價值體驗的組成部分。
這些條件摘自現有的關於動物知覺、醫療疾病、麻醉藥、哲學、進化、神經科學和人工智慧的經驗和理論文獻。
...
摘要：大型語言模型 (LLM) 被用於協助臨床醫師，這項技術引起了極大的關注。現有的研究主要採用封閉式的問答任務，並提供選項作為評估依據。然而，在實際的臨床環境中，許多臨床決策，例如治療建議，都涉及回答沒有預設選項的開放式問題。同時，現有的研究主要使用準確度來評估模型的效能。在本文中，我們全面評定了醫療保健領域中各種 LLM，以清楚了解它們的優點和缺點。我們的評量基準包含七項任務和十三個資料集，涵蓋了醫療語言生成、理解和推理。我們在零次學習和少量學習 (即 1、3、5 次學習) 的設定下，對現有的十六個醫療保健 LLM 進行了詳細的評估。我們報告了五項指標 (即匹配、忠實度、全面性、概括性、穩健性) 的結果，這些指標對於獲得臨床使用者的信任至關重要。我們進一步邀請醫療專家進行人工評估。
...
摘要：隨著社群媒體平台在全球廣泛採用，不僅提升了使用者的連結性和溝通性，也成為傳播健康相關資訊的重要管道，因此確立社群媒體資料為公共衛生研究中極具價值的有機資料資源。電子菸或電子香菸的使用在美國和其他國家爆紅，導致電子菸和電子菸相關肺部傷害 (EVALI) 爆發，並於 2019 年造成住院和死亡案例，凸顯了了解電子菸行為和制定有效戒菸策略的迫切性。在本研究中，我們從 Reddit 上的一個電子菸次社群中萃取一個範例資料集，以分析使用者戒菸的意圖。利用大型語言模型，包括最新的 GPT-4 和傳統的 BERT-based 語言模型，進行句子層級的戒菸意圖預測任務，本研究比較了這些模型的結果與人工標註。值得注意的是，與人工評估者相比，GPT-4 模型在遵守標註指南和流程方面展現出優異的一致性，展示了偵測人類評估者可能忽略的細微使用者戒菸意圖的進階能力。這些初步發現強調了 GPT-4 在提升社群媒體資料分析的準確性和可靠性方面的潛力，特別是在辨識人類偵測可能無法察覺的使用者細微意圖方面。
...
摘要：大型語言模型 (LLM) 在處理臨床文本摘要任務方面展現出令人期待的能力。在此研究中，我們證明了一個小型開源 LLM 可以經過有效訓練，從門診病患與醫師對話中產生高品質的臨床筆記。我們透過針對 LLaMA-2 130 億個參數模型進行全面的領域和任務特定適應程序來達成此目標。此程序包含持續的預訓練、監督微調，以及來自人工智慧和人類回饋的強化學習。我們引進一種稱為 DistillDirect 的強化方法，用 Gemini Pro 作為教師模型來執行基於策略的強化學習。我們產生的模型 LLaMA-Clinic 能夠產生品質與醫師所撰寫的臨床筆記相當的臨床筆記。在一個盲審醫師閱讀研究中，大多數 (90.4%) 的個別評量將 LLaMA-Clinic 所產生的筆記評為「可接受」或更高，涵蓋所有三項準則：真實世界準備度、完整度和準確度。值得注意的是，在更具挑戰性的「評估和計畫」部分中，LLaMA-Clinic 在真實世界準備度方面獲得較高分數 (4.2/5)，高於醫師所撰寫的筆記 (4.1/5)。此外，我們在公共臨床筆記資料集（例如 ACI-BENCH）中找出一些警示。我們強調未來臨床筆記產生任務中的關鍵考量，強調預先定義最佳實務筆記格式的重要性。整體而言，我們的研究證明訓練較小型的開源 LLM 以協助臨床文件，並利用醫療保健機構取得病歷和領域專業知識的可行性與潛力。我們已公開我們新建立的合成診所對話筆記資料集和醫師回饋資料集，以促進此領域的未來研究。
...
摘要：最近，基於深度學習的語言模型已大幅提升文字轉換為 SQL 的任務，在醫療領域中可用於擷取病歷的應用令人期待。此類應用程式中的一個顯著挑戰是辨識無法回答的查詢。透過微調模型，我們展示了將病歷查詢轉換為 SQL 查詢的可行性。此外，我們引入一個基於熵的方法來識別並濾除無法回答的結果。我們進一步透過基於對數機率分布來濾除信心度較低的 SQL，以提升結果品質，同時透過在實際資料庫上執行查詢來減輕語法和架構錯誤。我們透過實驗驗證，我們的模型能夠濾除無法回答的問題，即使無法存取模型的參數，也能廣泛使用，並能有效地用於實務中。
...
摘要：大型語言模型（LLM）整合到醫療保健領域，承諾轉型醫療診斷、研究和患者照護。然而，醫療 LLM 的進展面臨障礙，例如複雜的訓練需求、嚴格的評估需求，以及限制學術探索的專有模型的主導地位。透明、全面的 LLM 資源存取對於推動該領域、促進可複製性，以及鼓勵醫療保健 AI 的創新至關重要。我們提出 Hippocrates，一個專門為醫療領域開發的開源 LLM 框架。與以往的努力截然不同，它提供對其訓練資料集、程式碼庫、檢查點和評估協定的無限制存取。這種開放式方法旨在激勵協作研究，讓社群可以在透明的生態系統中建立、改進和嚴格評估醫療 LLM。此外，我們介紹 Hippo，一個針對醫療領域量身打造的 7B 模型系列，透過持續預訓練、指令調整和人類與 AI 回饋的強化學習，從 Mistral 和 LLaMA2 微調而來。我們的模型優於現有的開放式醫療 LLM 模型，甚至超越具有 70B 參數的模型。透過 Hippocrates，我們渴望釋放 LLM 的全部潛力，不僅促進醫療知識和患者照護，也民主化醫療保健中 AI 研究的優點，讓它們在全球各地都能使用。
...
摘要：使用生成模型的弱监督医学图像分割 (MIS) 对于临床诊断至关重要。然而，分割结果的准确性通常受到监督不足和医学影像的复杂性质的限制。现有的模型也只提供单一结果，不允许测量不确定性。在本文中，我们介绍了 DiffSeg，这是一种基于扩散差别的皮肤病变分割模型，它利用扩散模型原理从具有不同语义信息的图像中提取基于噪声的特征。通过辨别这些噪声特征之间的差异，该模型识别出患病区域。此外，它的多输出能力模仿了医生的注释行为，便于可视化分割结果的一致性和歧义性。此外，它使用广义能量距离 (GED) 量化输出不确定性，帮助医生解释和决策。最后，该模型通过密集条件随机场 (DenseCRF) 算法整合输出，通过考虑像素间相关性来细化分割边界，从而提高准确性并优化分割结果。我们在 ISIC 2018 挑战数据集上展示了 DiffSeg 的有效性，其性能优于最先进的基于 U-Net 的方法。
...
摘要：自動視網膜層分割與醫學影像，例如光學同調斷層掃描 (OCT) 影像，是診斷眼科疾病的重要工具。然而，由於影像中出現對比度低和血流雜訊，因此要達成精確的分割相當具挑戰性。此外，該演算法應為輕量級，才能部署於實際臨床應用。因此，需要設計一個輕量級網路，並具備高性能的視網膜層分割。在本文中，我們提出適用於 OCT 影像的視網膜層分割 LightReSeg。具體來說，我們的做法遵循編碼器-解碼器結構，其中編碼器部分採用多尺度特徵萃取和 Transformer 區塊，以充分利用各種尺度的特徵圖的語義資訊，並讓特徵具備更好的全局推理能力，而解碼器部分，我們設計了一個多尺度非對稱注意力 (MAA) 模組，以保留每個編碼器尺度的語義資訊。實驗顯示，我們的方法與現有最先進的 TransUnet 方法相比，在我們收集的資料集和兩個其他公開資料集上，以僅 330 萬個參數就達到了更好的分割效能，而 TransUnet 有 10570 萬個參數。
...
摘要：儘管深度學習在醫學影像分析中獲得顯著的成功，
由於缺乏用於監督的高品質標籤影像，醫學影像分割仍然具有挑戰性。此外，一般自然影像和醫學影像之間的顯著領域差距，特別是超音波影像，阻礙了將訓練於自然影像的微調模型應用於手邊的任務。在這項工作中，我們探討了分割模型在低資料模式下的效能下降，並提出了一種無提示分割方法，利用分割基礎模型分割抽象形狀的能力。我們透過我們新穎的提示點生成演算法來做到這一點，該演算法使用粗略的語意分割遮罩作為輸入，並將零次提示可用的基礎模型作為最佳化目標。我們在超音波影像的分割發現任務（病理異常）上展示了我們的模型。我們的模型的優勢在小規模肌肉骨骼超音波影像資料集的低資料模式實驗中得到了不同程度的驗證，隨著訓練集大小的減少，效能提升幅度越大。
...
摘要：電子健康記錄（EHR）儘管對醫療保健從業者來說是一大福音，但每天都變得越來越複雜且冗長。在這些冗長的 EHR 中篩選很費力，並且成為醫患互動中一個繁瑣的部分。已經提出了幾種方法來幫助緩解這個普遍的問題，無論是通過摘要還是分段，然而，過去只有少數方法真正有幫助。隨著自動化方法的興起，機器學習 (ML) 在解決識別 EHR 中相關部分的任務方面顯示出前景。然而，大多數 ML 方法依賴於標記數據，這在醫療保健中很難獲得。另一方面，大型語言模型 (LLM) 在自然語言處理 (NLP) 中表現出了令人印象深刻的壯舉，而且是以零次學習的方式，即沒有任何標記數據。為此，我們建議使用 LLM 來識別相關的部分標題。我們發現 GPT-4 可以有效地解決零次學習和少次學習設置中的任務，並且比最先進的方法分段好得多。此外，我們還註釋了一個更難的現實世界數據集，並發現 GPT-4 難以執行良好，暗示需要進一步的研究和更困難的基準。
...
摘要：大型语言模型 (LLM) 中的提示泄漏构成了重大的安全
和隐私威胁，尤其是在检索增强生成 (RAG)
系统中。然而，尚未以标准化方式研究多轮 LLM 交互中的泄漏以及缓解
策略。本文
调查了 LLM 在 4 个不同
领域和 10 个闭源和开源 LLM 中针对提示泄漏的漏洞。我们独特的
多轮威胁模型利用了 LLM 的阿谀奉承效应，我们的分析剖析了任务
指令和 LLM 响应中的知识泄漏。在多轮设置中，
我们的威胁模型将平均攻击成功率 (ASR) 提高到 86.2%，
其中包括 GPT-4 和 claude-1.3 的 99% 泄漏。我们发现一些黑盒
LLM（如 Gemini）显示出对跨域泄漏的不同敏感性——它们
更有可能在新闻领域泄露上下文知识，而不是在
医学领域。我们的实验测量了 6 种黑盒防御
策略的具体效果，包括 RAG 场景中的查询重写器。我们提出的
多层防御组合仍然对黑盒 LLM 具有 5.3% 的 ASR，
表明 LLM 安全研究的增强和未来方向还有提升空间。
...
摘要：工業系統中的異常偵測對於預防設備故障、確保風險識別和維持整體系統效率至關重要。傳統監控方法通常依賴於固定閾值和經驗規則，這些規則可能不夠靈敏，無法偵測系統健康狀況的細微變化並預測即將發生的故障。為了解決這個限制，本文提出了一個基於注意力的卷積自動編碼器 (ABCD) 的新穎方法，用於風險偵測並將風險值對應到維護計畫中。ABCD 從真實世界工業冷卻系統的歷史數據中學習電導率的正常行為，並重建輸入數據，識別偏離預期模式的異常值。該架構還採用校準技術來確保其預測的可靠性。評估結果表明，與沒有注意力機制相比，ABCD 中的注意力機制使效能提高了 57.4%，並且將誤報減少了 9.37%。該方法可以有效地偵測風險，將風險優先順序對應到維護，為冷卻系統設計人員和服務人員提供有價值的見解。0.03% 的校準誤差表示該模型經過良好校準並增強了模型的信賴度，從而能夠對維護策略做出明智的決策
...
摘要：序列建模是各種領域的關鍵領域，包括自然語言處理 (NLP)、語音辨識、時間序列預測、音樂生成和生物資訊學。遞迴神經網路 (RNN) 和長短期記憶網路 (LSTM) 在歷史上一直主導著機器翻譯、命名實體識別 (NER) 等序列建模任務。然而，Transformer的進步導致了這種範例的轉變，因為它們具有優越的性能。然而，Transformer存在 $O(N^2)$ 注意力複雜度和處理歸納偏差的挑戰。已經提出了一些變體來解決這些問題，這些變體使用譜網路或卷積，並且在各種任務上表現良好。然而，它們在處理長序列時仍然存在困難。狀態空間模型 (SSM) 已成為這種情況下序列建模範例的有希望的替代方案，特別是隨著 S4 及其變體（例如 S4nd、Hippo、Hyena、Diagnol State Spaces (DSS)、Gated State Spaces (GSS)、Linear Recurrent Unit (LRU)、Liquid-S4、Mamba 等）的出現。在本次調查中，我們根據三種範例對基礎 SSM 進行分類，即閘控架構、結構架構和遞迴架構。本次調查還重點介紹了 SSM 在視覺、影片、音訊、語音、語言（尤其是長序列建模）、醫療（包括基因組學）、化學（如藥物設計）、推薦系統和時間序列分析（包括表格資料）等領域的各種應用。此外，我們整合了 SSM 在基準資料集上的效能，例如 Long Range Arena (LRA)、WikiText、Glue、Pile、ImageNet、Kinetics-400、sstv2，以及 Breakfast、COIN、LVU 等影片資料集和各種時間序列資料集。Mamba-360 工作的專案頁面可在這個網頁上找到。\url{https://github.com/badripatro/mamba360}。
...
摘要：儘管融合來自乳房攝影多視圖的資訊在提升乳癌偵測準確度中扮演重要角色，開發基於多視圖乳房攝影的電腦輔助診斷 (CAD) 計畫仍面臨挑戰，且目前尚未有此類 CAD 計畫用於臨床實務。為克服挑戰，我們探討一種基於對比式語言影像預訓練 (CLIP) 的新方法，這已在各種醫學影像任務中引起關注。透過解決 (1) 有效調整單視圖 CLIP 以進行多視圖特徵融合，以及 (2) 使用有限樣本和運算資源有效微調這個參數密集模型的挑戰，我們引入了 Mammo-CLIP，這是第一個處理多視圖乳房攝影和對應簡短文字的多模態架構。Mammo-CLIP 使用早期特徵融合策略來學習從左右乳房的 CC 和 MLO 視圖取得的四張乳房攝影中多視圖關係。為提升學習效率，將即插即用適配器加入 CLIP 影像和文字編碼器中，以進行微調參數，並將更新限制在約 1% 的參數。為了進行架構評估，我們回溯性地彙編了兩個資料集。第一個資料集包含 470 個惡性和 479 個良性病例，用於少量樣本微調和透過 5 倍交叉驗證對提出的 Mammo-CLIP 進行內部評估。第二個資料集，包括 60 個惡性和 294 個良性病例，用於測試 Mammo-CLIP 的概括性。研究結果顯示，Mammo-CLIP 在 AUC (0.841 對 0.817、0.837 對 0.807) 上優於兩個資料集中的最先進跨視圖轉換器。它也超越了先前兩個基於 CLIP 的方法 20.3% 和 14.3%。這項研究突顯了應用微調視覺語言模型開發下一代、基於影像文字的乳癌 CAD 計畫的潛力。
...
摘要：大型語言模型，例如 GPT-4 和 Med-PaLM，在臨床任務上展現出令人印象深刻的表現；然而，它們需要取得運算存取權，是封閉原始碼，且無法部署在裝置上。中型模型，例如 BioGPT-large、BioMedLM、LLaMA 2 和 Mistral 7B，避免了這些缺點，但它們在臨床任務上的能力尚未得到充分研究。為了幫助評估它們在臨床使用上的潛力，並幫助研究人員決定應使用哪個模型，我們比較它們在兩個臨床問答 (QA) 任務上的表現：MedQA 和消費者查詢回答。我們發現 Mistral 7B 是表現最好的模型，在所有基準測試中獲勝，並優於專門針對生物醫學領域訓練的模型。雖然 Mistral 7B 的 MedQA 分數為 63.0%，接近原始的 Med-PaLM，而且它通常可以對消費者健康查詢產生合理的回應，但仍有改進空間。這項研究提供了第一個對開放原始碼中型模型在臨床任務上進行面對面的評估。
...
摘要：反射式共聚焦顯微鏡 (RCM) 是一種非侵入性的影像技術，用於生物醫學研究和臨床皮膚科。它提供皮膚和表層組織的虛擬高解析度影像，減少了對實體切片的需要。RCM 使用雷射光源照射組織，擷取反射光以產生各種深度微觀結構的詳細影像。最近的研究探索了人工智慧和機器學習，特別是 CNN，用於分析 RCM 影像。我們的研究提出了一種基於紋理特徵的分割策略，以識別臨床顯著區域，讓皮膚科醫生能夠有效解讀影像並提升診斷信心。這種方法有望促進皮膚科診斷和治療。
...
摘要：遠端關鍵基礎設施的健康監控是一項複雜且昂貴的活動，因為基礎設施的可及性有限。檢查無人機是普遍存在的資產，可透過改善可及性來增強關鍵基礎設施的可靠性。然而，由於操作環境惡劣，監控其健康狀況對於確保檢查作業順利進行至關重要。電池是決定檢查無人機整體可靠性的關鍵組成部分，並透過適當的健康管理方法，有助於進行可靠且穩健的檢查。在此脈絡下，本文提出了一種新的混合機率方法，用於預測 Li-Po 電池的電池放電結束 (EOD) 電壓。混合化是在錯誤修正組態中實現的，它結合了基於物理的放電和機率錯誤修正模型，以量化隨機不確定性和認識論不確定性。混合機率方法的效能已在一個包含在不同負載條件下的 EOD 電壓的資料集上進行實證評估。該資料集是從執行不同飛行任務的實際檢查無人機中取得的，重點在於離岸風力渦輪機檢查。所提出的方法已使用不同的機率方法進行測試，並證明與最佳機率方法相比，機率準確度提高了 14.8%。此外，隨機不確定性和認識論不確定性提供了穩健的估計，以增強電池健康狀態的診斷。
...
摘要：<paragraph>我們提出一個基於機器學習的異常偵測產品，AI Detect and Respond (AIDR)，它可以即時監控 Walmart 的業務和系統健康狀況。在 3 個月的驗證期間，此產品從超過 3000 個模型中提供預測，提供給超過 25 個應用程式、平台和營運團隊，涵蓋 63% 的重大事件，並將平均偵測時間 (MTTD) 縮短超過 7 分鐘。與之前的異常偵測方法不同，我們的解決方案利用統計、ML 和深度學習模型，同時持續納入基於規則的靜態閾值，以納入特定領域的知識。單變數和多變數 ML 模型透過分散式服務進行部署和維護，以確保可擴充性和高可用性。AIDR 有個回饋迴路，會使用漂移偵測演算法和客戶回饋的組合來評估模型品質。它還提供自助導入功能和自訂功能。AIDR 已經在各種內部團隊中獲得成功，偵測時間縮短，且偽陽性比之前的做法更少。隨著我們繼續前進，我們希望擴展事件涵蓋範圍和預防措施，減少雜訊，並進一步與根本原因建議 (RCR) 整合，以實現端到端的 AIDR 體驗。</paragraph>
...
摘要：人工智能應用在疫情管理中展現了前景，並廣泛用於協助醫學影像的辨識、分類和診斷。為應對猴痘（Mpox）的全球爆發，HeHealth.ai 團隊利用現有的篩檢性傳染病工具，透過 AI 方法開發出針對有症狀 Mpox 的數位篩檢測試。在 Mpox 全球爆發之前，該團隊開發了一款智慧型手機應用程式，應用程式使用者可以使用自己的智慧型手機相機拍攝自己的陰莖照片，以篩檢有症狀的性傳染病。AI 模型最初是使用 5000 個案例開發的，並使用改良的卷積神經網路輸出視覺可診斷陰莖病理的預測分數，包括梅毒、單純疱疹病毒和人類乳突病毒。從 2022 年 6 月到 2022 年 10 月，總計約有 22,000 名使用者下載了 HeHealth 應用程式，並使用 HeHealth AI 技術分析了約 21,000 張影像。接著，我們從 2022 年 7 月開始進行形成性研究、利害關係人參與、快速彙整影像、驗證研究，以及工具實作。從 2022 年 7 月到 2022 年 10 月，總計使用 1000 張與 Mpox 相關的影像來訓練 Mpox 症狀檢查工具。我們的數位症狀檢查工具顯示，在確認 Mpox 時的準確度為 87%，在排除有症狀的 Mpox 時的準確度為 90%。已識別出的幾個障礙包括應用程式使用者的資料隱私和安全性問題、訓練 AI 工具的資料最初不足，以及輸入資料的潛在概括性。我們提供了一些建議，以協助其他人開始進行緊急情況下的類似專案，包括讓廣泛的利害關係人參與、組成跨領域團隊、優先考量務實性，以及大數據事實上是由小數據組成的概念。
...
摘要：臨床試驗匹配是找出患者可能有資格參加的試驗的任務。通常，這項任務需要大量人力，並且需要詳細驗證患者的電子健康記錄 (EHR)，並比對臨床試驗的嚴格納入和排除標準。這個過程是手動的、耗時的，而且難以擴展，導致許多患者錯失潛在的治療選擇。大型語言模型 (LLM) 的最新進展讓自動化患者試驗匹配成為可能，正如多個同時進行的研究調查所示。然而，目前的做法僅限於受限的、通常是合成式的資料集，無法充分反映實際醫療資料中遇到的複雜性。在本研究中，我們提出使用真實世界 EHR 的臨床試驗匹配的第一個端到端大規模實證評估。我們的研究展示了 LLM 準確匹配患者與適當臨床試驗的能力。我們使用專有的 LLM 進行實驗，包括 GPT-4 和 GPT-3.5，以及我們自訂微調的模型 OncoLLM，並顯示 OncoLLM 儘管規模顯著較小，不僅優於 GPT-3.5，而且還匹配合格醫生的表現。所有實驗都是在真實世界的 EHR 上進行的，其中包括臨床記錄和美國某個癌症中心的可用臨床試驗。
...
摘要：在城市環境中，綠地空間的稀少構成了一項重大的挑戰。它對市民的健康和福祉產生了多種負面影響。小規模的干預措施，例如袖珍公園，是一個可行的解決方案，但會帶來多重限制，包括在特定區域的設計和實施。在本研究中，我們利用生成式 AI 的能力進行多尺度干預規劃，重點關注基於自然的解決方案。通過利用圖像到圖像和圖像修復演算法，我們提出了一種解決城市地區綠地空間不足的方法。我們重點關注塞薩洛尼基的兩條綠化不足的小巷，展示了我們的方法在可視化 NBS 干預措施方面的效力。我們的研究結果強調了新興技術在塑造城市干預規劃過程的未來方面具有轉變的潛力。
...
摘要：在应用于临床领域的自然语言处理中，利用大型语言模型已成为临床记录错误检测和纠正的一个有前途的途径，这是一项知识密集型任务，其注释数据稀缺。本文介绍了 MedReAct'N'MedReFlex，它利用了一套由四个基于 LLM 的医疗代理组成。MedReAct 代理通过观察、分析和采取行动来启动该过程，生成轨迹以指导搜索，以针对临床记录中的潜在错误。随后，MedEval 代理使用五个评估器来评估目标错误和建议的更正。在 MedReAct 的操作被证明不足的情况下，MedReFlex 代理会进行干预，进行反思性分析并提出替代策略。最后，MedFinalParser 代理格式化最终输出，保留原始样式，同时确保错误更正过程的完整性。我们方法的一个核心组成部分是基于我们的 ClinicalCorp 语料库的 RAG 管道。在包含临床指南和信息的众多其他知名来源中，我们预处理并发布了用于临床 RAG 应用程序的开源 MedWiki 数据集。我们的结果证明了我们的 RAG 方法与通过 MedReAct'N'MedReFlex 框架利用 ClinicalCorp 的核心作用。它在 MEDIQA-CORR 2024 最终排行榜上获得了第九名。
...
摘要：反事實範例有助於探索機器學習模型的決策邊界和決定特徵歸因。我們如何應用基於反事實的方法來分析和解釋 LLM？我們找出以下關鍵挑戰。首先，產生的文字反事實應對使用者而言有意義且可讀，因此可以進行心智比較以得出結論。其次，為了讓解決方案可擴展到長篇文字，使用者應具備工具，從各種粒度層級的擾動中建立反事實批次，並互動式分析結果。在本文中，我們解決上述挑戰並貢獻 1) 一種新演算法，透過移除和替換不同粒度中的文字片段來產生批次的完整且有意義的文字反事實，以及 2) LLM Analyzer，一種互動式視覺化工具，可協助使用者透過互動式檢查和彙總有意義的反事實來了解 LLM 的行為。我們透過使用來自醫療、法律、財務、教育和新聞資料集的 1,000 個樣本，根據其產生的反事實的語法正確性來評估所提出的演算法。在我們的實驗中，97.2% 的反事實在語法上是正確的。透過使用案例、使用者研究和專家的回饋，我們證明了所提出的互動式視覺化工具的實用性和可用性。
...
摘要：本研究的目的是预测一个人是否会完成戒毒康复计划以及一个人参加的次数。该研究基于从俄克拉荷马州的药物和酒精康复中心获取的药物滥用和心理健康服务管理局 (SAMHSA) 的人口统计数据以及出院数据。人口统计数据高度分类，导致使用二进制编码并利用各种公平措施来减轻九个人口统计变量的偏差。使用支持向量机在各种参数范围内比较了线性、多项式、sigmoid 和径向基函数等核方法，以找到最优值。然后将这些方法与决策树、随机森林和神经网络等方法进行比较。平衡数据的合成少数过采样技术名义 (SMOTEN) 用于对缺失数据进行插补。然后对九个偏差变量进行交叉分析以减轻偏差，并整合双重和三重交互作用以使用概率来查看最坏情况比率公平性缓解。在二元和多类场景中都探索了不同的影响、统计奇偶校验差异、条件统计奇偶校验比率、人口统计奇偶校验、人口统计奇偶校验比率、均衡几率、均衡几率比率、同等机会和均衡机会比率。
...
摘要：<paragraph>醫學影像語言預訓練 (Med-VLP) 建立了醫學影像視覺內容與相關文字描述之間的關聯。現有的 Med-VLP 方法主要關注描繪單一身體部位的 2D 影像，尤其是胸部 X 光片。在本文中，我們通過使用 CT 影像和報告的多模態資料集，將 Med-VLP 的範圍擴展到涵蓋 3D 影像，特別是針對全身場景。與 2D 對應物相比，3D VLP 需要有效擷取 3D 影像中明顯較稀疏的表示中的基本語義。在本文中，我們介紹了 CT-GLIP（使用 CT 掃描的基礎語言影像預訓練），這是一種創新的方法，用於建構器官層級的影像文字對，以增強多模態對比學習，將基礎視覺特徵與精確的診斷文字對齊。此外，我們開發了一個異常字典，以使用不同的對比對增強對比學習。我們的訓練方法是建立在多模態 CT 資料集上，該資料集包含來自 17,702 名患者的 44,011 個器官層級的視覺文字對，涵蓋 104 個器官，證明它可以使用自然語言以零次學習的方式識別器官和異常。CT-GLIP 的效能經過一個包含 1,130 名患者的獨立測試集驗證，重點關注 7 個器官中最常見的 16 種異常。實驗結果顯示，我們的模型在零次學習和微調場景中，使用 CNN 和 ViT 架構，都優於標準 CLIP 架構。</paragraph>
...
摘要：多模態醫學影像在臨床診斷和研究中扮演著重要的角色，它結合了來自各種影像方式的資訊，以提供對潛在病理更全面的了解。最近，基於深度學習的多模態融合技術已成為改善醫學影像分類的強大工具。本篇評論提供了一個對基於深度學習的多模態融合在醫學分類任務中的發展的深入分析。我們探討了流行臨床方式之間的互補關係，並概述了多模態分類網路的三個主要融合方案：輸入融合、中間融合（包含單層融合、階層融合和基於注意力的融合）和輸出融合。透過評估這些融合技術的效能，我們深入了解不同網路架構在各種多模態融合場景和應用領域中的適用性。此外，我們深入探討與網路架構選擇、處理不完整多模態資料管理和多模態融合的潛在限制相關的挑戰。最後，我們重點介紹了基於 Transformer 的多模態融合技術的未來前景，並對這個快速發展領域的未來研究提出建議。
...
摘要：改善醫療保健的未來，首先要從更深入了解醫院中目前的實際做法開始。這激發了從患者資料中找出典型照護路徑的目標。透過分群，可以揭露同質的照護路徑群組。由帶時間戳記事件序列表示的照護路徑分群難處，在於定義語義上適當的指標和分群演算法。
  在本文中，我們將針對時間序列開發的兩種方法，改編為時間序列：drop-DTW 指標和 DBA 方法，用於建構平均時間序列。然後將這些方法應用於分群演算法，為時間序列提出原始且完善的分群演算法。
  這種方法經過實驗，並在合成和實際案例中進行評估。
...
摘要：醫學影像語言預訓練已成為一種有前景的方法，用於學習醫學影像和文字的領域通用表示。然而，目前利用醫學影像和文字之間的全局和局部對齊的演算法可能會因醫學資料中的冗餘資訊而受到影響。為了解決這個問題，我們提出了胸部 X 光的基礎知識增強醫學影像語言預訓練 (GK-MVLP) 架構。在此架構中，醫學知識透過使用基於Transformer的基礎知識增強模組，與適當的解剖區域建立基礎，以在解剖區域層級視覺特徵和醫學知識的紋理特徵之間進行細緻的對齊。GK-MVLP 的效能與現有技術競爭或超越下游胸部 X 光疾病分類、疾病定位、報告產生和醫學視覺問答任務。我們的結果顯示了結合基礎機制的優點，以消除偏差並改善胸部 X 光影像和放射科報告之間的對齊。
...
摘要：近年來，從對話資料中自動偵測憂鬱症已受到廣泛關注。由人類控制的虛擬代理人進行訪談的 DAIC-WOZ 資料集已被廣泛用於此任務。最近的研究報告指出，將訪談者的提示納入模型時，效能會提升。在此研究中，我們假設這種提升可能主要是由於這些提示中存在的偏見，而非所提出的架構和方法。透過消融實驗和定性分析，我們發現使用訪談者提示的模型會學習專注於訪談的特定區域，其中詢問有關過去心理健康問題的經驗，並將其用作區分捷徑來偵測憂鬱的參與者。相反地，使用參與者回應的模型會從整個訪談中收集證據。最後，為了強調此偏見的嚴重性，我們透過刻意利用它來達到 0.90 的 F1 分數，這是迄今為止僅使用文字資訊在此資料集上報告的最高結果。我們的研究結果強調，在將訪談者的提示納入模型時需要謹慎，因為它們可能會無意中學會利用有針對性的提示，而不是學會描述真正顯示患者心理健康狀況的語言和行為。
...
摘要：基礎模型在促進醫療領域的進步方面變得無價。
儘管有這樣的承諾，但 LLM 在複雜的醫療任務中有效應用於策略性部署仍然是一個未解決的問題。我們的新框架——醫療決策代理 (MDAgents) 旨在通過自動分配 LLM 的有效協作結構來解決這一差距。分配的單獨或群組協作結構根據手邊的醫療任務的複雜性進行調整，模擬真實世界的醫療決策制定過程。我們使用最先進的 LLM 評估我們的框架和基準方法，並採用一系列具有挑戰性的醫療基準：MedQA、MedMCQA、PubMedQA、DDXPlus、PMC-VQA、Path-VQA 和 MedVidQA，在 7 個需要理解多模態醫療推理的基準中取得了 5 個基準的最佳性能。消融研究表明，MDAgents 在調整協作代理數量以優化效率和準確性方面表現出色，展示了其在不同場景中的穩健性。我們還探索了群體共識的動態，深入了解了協作代理如何在複雜的臨床團隊動態中表現。我們的代碼可以在 https://github.com/mitmedialab/MDAgents 找到。
...
摘要：鼻腔細胞學檢查是一種診斷鼻炎和過敏的新型且有效的臨床技術，由於細胞計數耗時，因此並未廣泛使用；這就是為什麼 AI 輔助計數可能是此技術擴散的轉捩點。在本文中，我們展示了第一個鼻細胞病理學領域影像的資料集：NCD（鼻細胞病理學資料集），旨在訓練和部署物件偵測模型，以在臨床實務中支援醫師和生物學家。已經複製了填充鼻黏膜的細胞類型的實際分佈，從臨床患者的載玻片中取樣影像，並手動註解在它們上找到的每個細胞。相應的物件偵測任務呈現與嚴重的類別不平衡相關的非平凡問題，涉及最稀有的細胞類型。這項工作透過提出一個新穎的機器學習方法來協助鼻黏膜細胞的自動偵測和分類，為一些開放性挑戰做出貢獻：DETR 和 YOLO 模型在偵測細胞和正確分類它們方面表現良好，顯示出加速鼻科專家工作的巨大潛力。
...
摘要：<paragraph>合成資料生成提供了一個有前途的解決方案，藉由產生逼真的去識別資料，來提升電子健康紀錄 (EHR) 的實用性。然而，現有的文獻主要集中在合成健康資料的品質，忽略了下游預測中公平性的關鍵面向。因此，在合成 EHR 上訓練的模型因在目標任務中產生有偏差的結果而受到批評。這些偏差可能來自特徵之間的虛假關聯性，或是模型無法準確表示子群體。為了解決這些問題，我們提出了偏誤轉換生成對抗網路 (Bt-GAN)，這是一個專門為醫療保健領域設計的基於 GAN 的合成資料生成器。為了應對虛假關聯性 (i)，我們提出了一個資訊受限資料生成流程，讓生成器能夠根據明確定義的演算法公平性概念，學習一個公平的確定性轉換。為了克服捕捉精確子群體表示 (ii) 的挑戰，我們透過基於分數的加權抽樣，激勵生成器保留子群體密度。這種方法迫使生成器從資料流形的代表性不足區域學習。我們使用 MIMIC-III 資料庫進行廣泛的實驗。我們的結果表明，Bt-GAN 達到了 SOTA 精度，同時顯著提高了公平性並最大限度地減少了偏差放大。我們還進行了深入的可解釋性分析，以提供額外的證據支持我們研究的有效性。總之，我們的研究引入了一種新穎且專業的方法，來解決醫療保健領域中合成資料生成的限制。透過納入公平性考量並利用 GAN 等先進技術，我們為醫療保健應用中更可靠且無偏差的預測鋪平了道路。</paragraph>
...
摘要：这项工作是受到深度神经网络的最新发展所启发，特别是作为 ChatGPT 等应用程序基础的 Transformer 架构，以及在移动设备上执行推理的需求。专注于新兴的 Transformer（特别是那些具有计算效率高的 Swin 类架构）和大模型（例如，基于 Transformer 的 Stable Diffusion 和 LLM），我们观察到计算操作符之间的布局转换会导致这些应用程序出现明显的减速。本文提出了 SmartMem，这是一个消除大多数布局转换的综合框架，其理念是多个操作符可以通过仔细选择布局和操作实现来使用相同的张量布局。我们的方法基于将操作符分类为四组，并考虑操作符之间的生产者-消费者边界的组合。我们开发了一组用于搜索此类布局的方法。我们工作的另一个组成部分是为移动设备中常见的 2.5 维存储器开发高效的存储器布局。我们的实验结果表明，SmartMem 在移动设备上的 18 个不同的神经网络（包括 CNN、具有局部和全局注意力的 Transformer 以及 LLM）上优于 5 个最先进的 DNN 执行框架。特别是，与 DNNFusion 相比，SmartMem 平均提速 2.8 倍，并且分别以 6.9 倍和 7.9 倍的平均提速优于 TVM 和 MNN。
...
摘要：深度學習的興起標誌著在電腦視覺、自然語言處理和醫學影像等領域的顯著進展，主要是透過將預先訓練好的模型調整到特定任務。傳統的微調方法涉及調整所有參數，由於計算和記憶體需求高而面臨挑戰。這導致了參數有效微調 (PEFT) 技術的發展，它選擇性地更新參數以平衡計算效率和效能。此評論探討了 PEFT 方法，提供了各種策略的詳細比較，重點介紹了跨不同領域的應用，包括文本生成、醫學影像、蛋白質建模和語音合成。透過評估 PEFT 方法在降低計算負載、加快訓練和降低記憶體使用方面的效能，本文有助於讓深度學習更易於存取和適應，促進其更廣泛的應用並鼓勵模型最佳化的創新。最終，本文旨在為 PEFT 不斷演變的領域提供見解，指導研究人員和實務工作者克服傳統微調方法的限制。
...
摘要：從 RGB 影片中可以萃取出細微的週期性訊號，例如血液容積脈搏和呼吸，能以低成本進行非接觸式健康監測。遠端脈搏估測技術（或遠端光電容積描記法 (rPPG)）的進展，目前是由深度學習解決方案推動。然而，現代化方法是在具有接觸式 PPG 感測器真實數據的基準資料集上訓練和評估。我們提出第一個非對比式無監督學習架構，用於訊號回歸，以降低對有標籤影片資料的需求。我們的做法在週期性和有限頻寬的最小假設下，直接從未標籤的影片中找出血液容積脈搏。我們發現，鼓勵在正常生理頻寬限制和功率頻譜批次中的變異中的稀疏功率頻譜，就足以用於學習週期性訊號的視覺特徵。我們執行第一個實驗，利用未特別為 rPPG 建立的未標籤影片資料，來訓練穩健的脈搏率估測器。考量到有限的歸納偏誤，我們成功地將相同方法應用於基於相機的呼吸，方法是改變目標訊號的頻寬限制。這顯示出該方法對於從不同領域中學習頻寬限制的準週期性訊號的無監督學習而言，已經夠通用。此外，我們顯示該架構對於在單一受試者的未標籤影片上微調模型是有效的，允許個人化和適應性訊號回歸器。
...
摘要：聯邦學習 (FL) 已成為一種重要的隱私保護技術，可用於支援機密臨床機器學習等使用案例。FL 的運作方式是彙總由擁有資料的遠端裝置所訓練的模型。因此，FL 能夠使用來自大量學習者的群眾外包資料來訓練強大的全球模型，而不會損害其隱私。然而，在產生全球模型時，彙總伺服器是單一故障點。此外，當資料在所有遠端裝置上並非獨立且相同分佈 (非 IID 資料) 時，模型的效能會受到影響。這會導致彙總的模型差異很大，在某些情況下，效能可能會降低多達 50%。在本文中，我們尋求解決上述問題，同時保留 FL 的優點。我們提出多聯盟學習：一種分散式 FL 架構，旨在處理非 IID 資料。與傳統 FL 不同，多聯盟學習將並行維護多個模型 (而非單一全球模型)，以在資料為非 IID 時協助收斂。在遷移學習的幫助下，學習者可以收斂到較少的模型。為了提高適應性，學習者可以選擇要從同儕彙總哪些更新。
...
摘要：這項工作探索一種基於大型語言模型 (LLM) 的新穎資料擴充方法，用於預測美國執照醫學考試 (USMLE) 中已停用的多選題 (MCQ) 的題目難度和作答時間，此方法用於 BEA 2024 共享任務。我們的做法是使用零次學習 LLM（Falcon、Meditron、Mistral）中的答案擴充資料集，並採用基於六種替代特徵組合的基於轉換器的模型。結果表明，預測題目的難度更具挑戰性。值得注意的是，我們表現最好的方法始終包含題目文字，並受益於 LLM 答案的多變性，突顯了 LLM 在改善醫學執照考試中的自動化評估的潛力。我們公開我們的程式碼 https://github.com/ana-rogoz/BEA-2024。
...
摘要：電池健康監控和預測在電動交通時代至關重要，對安全、永續性和經濟層面都有重大影響。現有研究經常專注於預測準確性，但傾向於忽略可能會阻礙技術在實際應用中部署的實際因素。在本文中，我們探討這些實際考量，並根據貝氏神經網路開發模型來預測電池壽命終點。我們的模型使用與電池健康相關的感測器資料，並針對模型的每個參數套用分佈，而非單點。這讓模型能夠捕捉電池健康的固有隨機性和不確定性，不僅能做出準確預測，還能量化不確定性。我們進行了一項實驗研究，並展示了我們提出的模型的有效性，預測誤差率平均為 13.9%，而某些測試電池的誤差率低至 2.9%。此外，所有預測都包含可量化的確定性，從電池的初始階段到中期階段，確定性提高了 66%。這項研究對電池技術具有實際價值，並有助於加速產業中技術的採用。
...
摘要：大型語言模型 (LLM) 的進步鼓勵在醫療保健領域採用它們，那裡重要的臨床資訊通常包含在非結構化註解中。癌症分期狀態可在臨床報告中取得，但它需要自然語言處理才能從非結構化文字中萃取出狀態。隨著以臨床為導向的 LLM 進步，有望在不需廣泛訓練演算法的情況下萃取出此類狀態。預先訓練的 LLM 的提示方法會引發模型的推理程序，例如思考鏈，可能有助於改善所產生回應的可信度。使用自我一致性進一步改善模型效能，但通常會導致多個推理路徑產生不一致的結果。在本研究中，我們提出一個整體推理方法，目的是改善模型產生的結果的一致性。使用開放取用的臨床大型語言模型來確定真實世界病理報告中的病理癌症分期，我們表明整體推理方法能夠改善 LLM 在確定癌症分期的結果的一致性和效能，從而證明了在可靠性和可信度至關重要的臨床或其他領域中使用這些模型的潛力。
...
摘要：人工智慧透過預測模型支援醫療保健專業人員，大幅轉變臨床決策制定。本研究探討醫療保健中 AI 應用對於公平性和可解釋性的關鍵需求，以確保不同病患人口統計資料的公平結果。透過專注於敗血症相關死亡率的預測模型，我們提出一個方法，學習效能最佳化的預測模型，然後採用遷移學習程序產生一個更公平的模型。我們的模型也引進一個創新的基於置換特徵重要性演算法，旨在闡明每個特徵在增強預測公平性方面的貢獻。與現有的可解釋性方法專注於解釋特徵對預測效能的貢獻不同，我們提出的方法獨特地彌補了理解每個特徵如何有助於公平性的差距。這項進展至關重要，因為敗血症的死亡率很高，且在三分之一的醫院死亡中扮演角色。我們的模型不僅有助於識別和減輕預測模型中的偏見，還透過改善模型預測的透明度和公平性來促進醫療保健利害關係人之間的信任，進而促成更公平且值得信賴的醫療保健服務。
...
摘要：帕金森氏症在全球排名第二的神經退化性疾病。本研究旨在開發一個利用混合現實功能來追蹤和評估眼睛運動的系統。在本文中，我們提出了一個醫療場景，並概述了一個應用程式的開發，該應用程式旨在透過混合現實技術擷取眼球追蹤訊號，以評估神經退化性疾病。此外，我們還介紹了一個從眼球注視分析中萃取臨床相關特徵的管道，從醫療角度描述了所提出系統的能力。本研究涉及一組健康的對照個體和罹患帕金森氏症的患者，展示了所提出的技術在非侵入式監控眼球運動模式以診斷神經退化性疾病方面的可行性和潛力。
臨床相關性 - 迫切需要開發一種非侵入性的帕金森氏症生物標記，以準確檢測疾病的發作。這將允許在最早的階段及時引入神經保護治療，並能夠持續監控干預結果。檢測眼睛運動中細微變化的能力允許早期診斷，在出現更明顯的症狀之前提供干預的關鍵時機。眼球追蹤提供客觀且可量化的生物標記，確保對疾病進展和認知功能進行可靠的評估。使用混合現實眼鏡進行的眼球注視分析是無線的，方便在家庭和醫院環境中進行評估。這種方法提供了利用不需要額外專用附件的硬體的優點，能夠透過個人眼鏡進行檢查。
...
摘要：視覺任務適應已被證明可有效地使用專門可學習層或代幣將預先訓練的視覺Transformer (ViT) 適應至一般下游視覺任務。然而，對於視覺任務適應對現實且重要的醫療領域的影響，特別是跨越不同的醫療視覺模式，例如彩色影像、X 光和電腦斷層掃描，目前尚無大規模基準來充分探討。為了彌補這個差距，我們提出了 Med-VTAB，一個由 168 萬張不同器官、模式和適應方法的醫療影像組成的大規模醫療視覺任務適應基準。根據 Med-VTAB，我們探討了與可調整參數有關的醫療提示調整的縮放定律，以及使用非醫療/醫療預訓練權重進行醫療視覺適應的概括性。此外，我們研究了患者 ID 異常分佈對醫療視覺適應的影響，這是一個真實且具挑戰性的場景。此外，Med-VTAB 的結果表明，單一預訓練模型在醫療任務適應中表現不佳。因此，我們引入了 GMoE-Adapter，這是一種透過閘控混合專家適配器結合醫療和一般預訓練權重的創新方法，在醫療視覺任務適應中取得了最先進的成果。
...
摘要：憂鬱症是當今社會一個重要的議題。根據世界衛生組織（WHO）的統計，2023 年有超過 2.8 億人正與憂鬱症搏鬥。這是一個龐大的數字；若不認真看待，這些數字將會快速增加。約有 48.9 億人是社群媒體使用者。人們在 Twitter、Facebook、Reddit、Instagram 等平台上表達自己的感受和情緒。這些平台包含有價值的資訊，可用於研究目的。在各種社群媒體平台上已進行了大量的研究。然而，這些努力仍存在一定的限制。特別是，先前的研究僅專注於偵測推文中的憂鬱症和憂鬱症的嚴重程度。此外，資料集標籤中存在不準確之處。在這項研究工作中，使用 Twitter 資料庫中的推文，根據詞彙標籤預測了五種類型的憂鬱症（躁鬱症、重度憂鬱症、精神病性憂鬱症、非典型憂鬱症和產後憂鬱症）。可解釋的 AI 用於提供推理，方法是突顯代表憂鬱症類型的推文部分。雙向編碼器轉換器表示（BERT）用於特徵提取和訓練。機器學習和深度學習方法用於訓練模型。BERT 模型呈現出最有希望的結果，達到 0.96 的整體準確度。
...
摘要：深度學習正劇烈地轉變醫學影像和放射線學領域，讓病理學在醫學影像中得以辨識，包括電腦斷層掃描 (CT) 和 X 光掃描。然而，深度學習模型的表現，特別是在分割任務中，通常受到廣泛註解資料集需求的限制。為了應對這項挑戰，弱監督語意分割的能力透過可解釋 AI 和反事實解釋的產生而被探索。本研究的範圍是開發一種新穎的反事實內繪方法 (COIN)，該方法透過使用生成模型將預測的分類標籤從異常翻轉為正常。例如，如果分類器將輸入醫學影像 X 視為異常，表示存在病理，則生成模型旨在內繪異常區域，從而逆轉分類器的原始預測標籤。此方法使我們能夠產生病理的精確分割，而無需依賴於預先存在的分割遮罩。至關重要的是，影像層級標籤被利用，而這些標籤比建立詳細的分割遮罩容易取得。該方法的有效性透過從愛沙尼亞塔爾圖大學醫院取得的 CT 影像中分割合成目標和實際腎臟腫瘤來證明。研究結果表明，COIN 遠遠優於已建立的歸因方法，例如 RISE、ScoreCAM 和 LayerCAM，以及 Singla 等人提出的另一種反事實解釋方法。這些證據表明，COIN 是一種很有前景的 CT 影像中腫瘤語意分割方法，並在醫療保健中邁出了讓深度學習應用更易於使用和更有效的一步，而醫療保健中註解資料很稀少。
...
包括光聲斷層攝影 (PAT)，這是一種有前途的影像模式，
它結合了超音波和光學影像方法的優點。
使用傳統方法重建 PAT 影像會產生粗糙的瑕疵，
特別是直接應用於稀疏 PAT 資料時。近年來，
生成對抗網路 (GAN) 在影像生成和轉換方面展現出強大的效能，
使其成為應用於重建任務的明智選擇。在本研究中，
我們提出了一個端到端方法，稱為 DensePANet，以解決從
稀疏資料重建 PAT 影像的問題。所提出的模型在其
生成器中採用了 UNet 的創新修改，稱為 FD-UNet++，
這顯著改善了重建效能。我們在各種體內和模擬資料集上評估了該方法。
定量和定性結果顯示我們的模型優於其他流行的深度學習技術。
...
摘要：本研究提出了一個基於 Transformer 架構和自注意力機制的 Multitrans 多模態融合框架。此架構結合了非對比電腦斷層掃描 (NCCT) 影像和中風治療患者出院診斷報告的研究，使用基於 Transformer 架構方法的各種方法來預測中風治療的功能結果。結果表明，單模態文本分類的效能顯著優於單模態影像分類，但多模態組合的效果優於任何單一模態。雖然 Transformer 模型僅在影像資料上表現較差，但與臨床元診斷資訊結合時，兩者都能學習到更好的互補資訊，並對準確預測中風治療效果做出良好貢獻。
...
摘要：糖尿病在全球的盛行率不断上升，这凸显了糖尿病管理的需求。最近的研究重点关注糖尿病管理中的数字生物标记，包括计算框架和使用个性化葡萄糖指标的非侵入性监测技术的创新。然而，它们主要集中在胰岛素剂量和特定的葡萄糖值，或对整体血糖控制的关注有限。这在扩大数字生物标记在糖尿病管理中整体血糖控制的范围方面留下了空白。为了解决这样的研究空白，我们提出了 GluMarker——一个端到端的框架，用于使用更广泛的因素来源对数字生物标记进行建模，以预测血糖控制。通过对各种机器学习基线的评估和改进，GluMarker 在预测次日血糖控制方面在 Anderson 的数据集上取得了最先进的成果。此外，我们的研究确定了次日血糖控制预测的关键数字生物标记。这些确定的生物标记有助于阐明影响血糖管理的日常因素，为糖尿病护理提供重要的见解。
...
摘要：在大數據時代，整合多種資料模式會造成重大的挑戰，尤其是在醫療保健等複雜領域。本文介紹一個多模態資料融合的資料探勘新流程模型，將嵌入式技術和資料探勘的跨產業標準流程整合到現有的資料融合資訊群組模型中。我們的模型旨在降低運算成本、複雜度和偏差，同時提高效率和可靠性。我們也提出了「解糾纏稠密融合」，一種新穎的嵌入式融合方法，旨在最佳化互惠資訊並促進稠密模態間特徵互動，從而最小化冗餘資訊。我們透過三個使用案例來證明模型的效能：使用視網膜影像和病患元資料預測糖尿病視網膜病變、使用衛星影像、網路和人口普查資料預測家庭暴力，以及從放射線影像和臨床筆記中辨識出臨床和人口特徵。該模型在糖尿病視網膜病變預測中達到 0.92 的巨觀 F1 分數、在家庭暴力預測中達到 0.854 的 R 平方和 24.868 的 sMAPE，以及在放射學分析中分別達到 0.92 和 0.99 的巨觀 AUC，用於疾病預測和性別分類。這些結果強調了資料融合資料探勘模型在多模態資料處理方面具有顯著影響的潛力，促進其在多樣化、資源受限的環境中採用。
...
摘要：藥物推薦系統旨在提供個人化的藥物建議，與個別病患的需求緊密結合。先前的研究主要集中在開發藥物嵌入，取得顯著進展。儘管如此，這些方法通常無法準確反映個別病患的狀況，主要是因為難以區分各種病患狀況，以及無法在特定狀況與適當藥物之間建立精確的關聯性。為了回應這些問題，我們引入了 DisMed，這是一個專注於病患狀況以增強個人化的模型。DisMed 運用因果推論來辨別明確、可量化的因果關係。然後深入檢查病患狀況，辨識並適應這些狀況不斷變化的細微差異，並將其直接對應到相應的藥物。此外，DisMed 充分利用多重病患就診的資料，提出藥物的組合。在真實世界資料集上的全面測試證明，DisMed 不僅改善了病患狀況的客製化，在精準度和安全性方面也超越了領先的模型。
...
摘要：多巴胺轉運體 (DAT) 影像通常用於監控帕金森氏症 (PD)，其中計算紋狀體 DAT 攝取量以評估 PD 嚴重程度。然而，DAT 影像成本高且有輻射暴露風險，且一般診所沒有提供。最近，黑質區域的 MRI 貼片已被提議為一種更安全且更簡單的替代方案。本文提出了一個對稱回歸器，用於從黑質 MRI 貼片預測 DAT 攝取量。承認左右黑質之間的對稱性，所提出的回歸器結合了一個配對輸入輸出模型，同時預測左右紋狀體的 DAT 攝取量。此外，它採用對稱損失，對左右預測之間的差異施加約束，類似於兩側 DAT 攝取量之間的高相關性。此外，我們提出了一種對稱蒙地卡羅 (MC) 輟學方法，用於提供 DAT 攝取預測的有益不確定性估計，它利用了上述對稱性。我們在 734 個黑質貼片上評估了所提出的方法，這證明了與標準回歸器相比，對稱回歸器的性能顯著提高，同時提供了更好的可解釋性和特徵表示。對稱 MC 輟學還給出了精確的不確定性範圍，在該範圍內包含真實 DAT 攝取量的機率很高。
...
摘要：醫學影像分割通常需要大量的密集註解來進行模型訓練，這既耗時又需要技術。為了減輕這個負擔，已經引入了基於範例的醫學影像分割方法，只需一個帶註解的影像即可實現有效訓練。在本文中，我們介紹了一個用於基於範例的醫學影像分割（CMEMS）的新型跨模型相互學習框架，該框架利用兩個模型從未標記數據中挖掘隱式資訊，並具有多種粒度。CMEMS 可以消除確認偏誤，並通過在模型之間的不同粒度強制一致性來實現協作訓練，以學習互補資訊。具體來說，跨模型影像擾動基於相互學習是通過使用微弱擾動的影像來產生高置信度的偽標籤，監督跨模型對強烈擾動影像的預測而設計的。這種方法能夠在影像粒度上共同追求預測一致性。此外，跨模型多級特徵擾動基於相互學習是通過讓偽標籤監督來自不同解析度擾動多級特徵的預測而設計的，這可以擴大擾動空間並增強我們框架的穩健性。CMEMS 以端到端的方式使用範例數據、合成數據和未標籤數據進行聯合訓練。在兩個醫學影像數據集上的實驗結果表明，所提出的 CMEMS 在監督極其有限的情況下優於最先進的分割方法。
...
摘要：聯邦學習 (FL) 已成為一種很有前景的隱私保護機器學習方法，特別是在醫療保健等敏感領域。在此背景下，TRUSTroke 項目旨在利用 FL 協助臨床醫師進行缺血性中風預測。本文概述了 TRUSTroke FL 網路基礎架構。建議的架構採用具有中央參數伺服器 (PS) 的客戶端伺服器模型。我們為客戶端節點引入了基於 Docker 的設計，提供了一個在臨床環境中實施 FL 程序的靈活解決方案。分析了不同通訊協定（HTTP 或 MQTT）對 FL 網路運作的影響，並選擇了 MQTT，因為它適用於 FL 場景。還建議了一個控制平面來支援 FL 程序所需的主要運作。本文最後分析了 FL 架構的安全層面，說明潛在威脅並提出緩解策略以提高可信度。
...
摘要：機器遺忘是更新機器學習模型的過程，用於移除特定訓練資料範例的資訊，以符合資料保護法規，讓個人得以要求移除其個人資料。儘管最近開發了許多遺忘演算法，但這些演算法的可靠評估仍是一個開放的研究問題。在這項工作中，我們專注於基於成員身分推論攻擊 (MIA) 的評估，這是評估遺忘演算法最常見的方法之一，並解決現有評估指標缺乏可靠性的各種缺點。具體來說，我們提出了一個博弈論架構，將評估過程形式化為遺忘演算法和 MIA 對手的博弈，透過 MIA 對手的功能來衡量遺忘演算法的資料移除效能。透過仔細設計博弈，我們證明從博弈中引發的自然評估指標享有現有評估指標無法滿足的可證明保證。此外，我們提出一個實用且有效的演算法來估計從博弈中引發的評估指標，並透過理論分析和經驗實驗證明其有效性。這項工作提出一個新穎且可靠的方法來經驗評估遺忘演算法，為開發更有效的遺忘技術鋪路。
...
摘要：醫療報告生成自動化放射線影像描述，減輕醫師負擔並將錯誤降至最低。然而，目前的方法缺乏結構化的輸出和醫師互動，無法產生清晰、臨床上相關的報告。我們的這種方法採用提示引導的方式，使用預先訓練的大型語言模型 (LLM) 來產生結構化的胸部 X 光報告。首先，我們在胸部 X 光影像中識別解剖區域，以產生集中於關鍵視覺元素的重點句子，從而建立以解剖為基礎句子的結構化報告基礎。我們也會將偵測到的解剖轉換為文字提示，傳達解剖理解給 LLM。此外，臨床背景提示引導 LLM 強調互動性和臨床需求。透過整合解剖重點句子和解剖/臨床提示，預先訓練的 LLM 可以產生結構化的胸部 X 光報告，針對提示的解剖區域和臨床背景進行調整。我們使用語言產生和臨床有效性指標進行評估，展現出強大的效能。
...
摘要：隨著全球人口老化，慢性腎臟疾病 (CKD) 的發生率正在上升。CKD 通常在晚期才會出現症狀，這會對醫療保健系統和患者的生活品質造成顯著的負擔。本研究開發了一個可解釋的機器學習系統，用於預測有心血管風險患者的 CKD，利用病史和實驗室數據。隨機森林模型達到了 88.2% 的最高靈敏度。該研究引入了一個全面的可解釋性框架，它超越了傳統的特徵重要性方法，包括全局和局部解釋、偏差檢查、生物醫學相關性和安全性評估。在全局解釋中識別出的關鍵預測特徵是使用糖尿病和 ACEI/ARB 藥物，以及初始 eGFR 值。局部解釋通過反事實解釋提供了模型見解，這與其他系統部分一致。在進行偏差檢查後，發現初始 eGFR 值和 CKD 預測表現出一些偏差，但沒有發現顯著的性別偏差。通過範圍規則提取的模型邏輯被證實與現有的醫學文獻一致。安全性評估測試了潛在的危險案例，並確認模型表現安全。此系統增強了模型的可解釋性、可靠性和問責性，促進了它潛在地整合到醫療保健設置中並符合即將出台的法規標準，並顯示出在醫療保健機器學習中更廣泛應用的前景。
...
摘要：肺部感染區域分割對於評估肺部疾病的嚴重程度至關重要。然而，現有的影像文字多模態方法通常依賴於勞力密集型標註進行模型訓練，在時間和專業知識方面造成挑戰。為了解決這個問題，我們提出了一個新穎的屬性知識引導框架，用於無監督肺部感染區域分割（AKGNet），它僅基於影像文字資料進行分割，而無需任何遮罩標註。AKGNet 同時促進文字屬性知識學習、屬性影像交叉注意力融合和基於高信心的偽標籤探索。它可以在嵌入空間中學習統計資訊並擷取影像和文字屬性之間的空間關聯，反覆優化遮罩以增強分割。具體來說，我們通過提取屬性知識並將其納入特徵表示中，引入了文字屬性知識學習模組，使模型能夠學習統計資訊並適應不同的屬性。此外，我們設計了一個屬性影像交叉注意力模組，通過計算嵌入空間中屬性和影像之間的關聯來擷取空間依賴資訊，從而有選擇地關注相關區域，同時過濾無關區域。最後，採用自訓練遮罩改進過程，使用高信心的預測生成偽標籤，以反覆增強遮罩和分割。在基準醫學影像資料集上的實驗結果證明了我們的方法與無監督場景中最先進的分割技術相比具有優越的效能。
...
摘要：光探測與測距 (LiDAR) 和物聯網 (IoT) 技術的整合為城市安全和行人福祉中的公共衛生資訊學提供了轉型契機。本文提出了一個新的架構，利用這些技術來加強城市交通場景中的 3D 物體偵測和活動分類。透過採用高架式 LiDAR，我們取得了詳細的 3D 點雲資料，能精確監控行人的活動。為了克服都市資料的稀少性，我們透過 Blender 中的模擬交通環境建立了一個專門的資料集，促進有針對性的模型訓練。我們的做法採用了修改過的點體素區域卷積神經網路 (PV-RCNN) 來進行強健的 3D 偵測，並使用 PointNet 來分類行人的活動，透過提供對行人行為的見解和促進更安全的城市環境，大幅受益於城市交通管理和公共衛生。我們的雙模型方法不僅加強了城市交通管理，也透過提供對行人行為的見解和促進更安全的城市環境，對公共衛生做出了重大貢獻。
...
摘要：<paragraph>糖尿病患者人數不斷增加是當今社會的嚴重問題，對人們的健康和國家的財務支出造成顯著的負面影響。由於糖尿病可能會演變成潛在的嚴重併發症，因此糖尿病患者的早期葡萄糖預測對於及時的醫療治療是必要的。現有的葡萄糖預測方法通常利用患者的私人數據（例如年齡、性別、種族）和生理參數（例如血壓、心率）作為葡萄糖預測的參考特徵，這不可避免地導致隱私保護問題。此外，這些模型通常關注長期（按月）或短期（按分鐘）預測。長期預測方法通常不準確，因為外部不確定性會極大地影響葡萄糖值，而短期預測方法則無法提供及時的醫療指導。基於上述問題，我們提出了 CrossGP，這是一個新穎的機器學習框架，用於跨天葡萄糖預測，僅基於患者的外部活動，不涉及任何生理參數。同時，我們實施了三個基線模型進行比較。在 Anderson 的數據集上進行的廣泛實驗有力地證明了 CrossGP 的優越性能，並證明了其在未來實際應用中的潛力。</paragraph>
...
摘要：近期，原型学习在半监督医学影像分割中兴起，并取得了显著的性能。然而，标记数据的稀缺限制了先前方法中原型的表现力，可能阻碍了原型对类别的完整表示。为了解决这个问题，我们提出了混合原型一致性学习 (MPCL) 框架，其中包括一个平均教师和一个辅助网络。平均教师为标记和未标记数据生成原型，而辅助网络为 CutMix 处理的混合数据生成额外的原型。通过原型融合，混合原型为标记和未标记原型提供了额外的语义信息。通过融合两个增强的原型，形成了每个类别的优质全局原型，优化了一致性学习中使用的隐藏嵌入的分布。在左心房和 B 型主动脉夹层数据集上的大量实验表明，MPCL 优于以往的最新方法，证实了我们框架的有效性。代码将很快发布。
...
摘要：重組腺相關病毒 (rAAV) 載體徹底改變了基因療法，但它們廣泛的趨向性和次佳轉導效率限制了它們的臨床應用。為了克服這些限制，研究人員專注於設計和篩選衣殼庫，以識別改良的載體。然而，龐大的序列空間和有限的資源在識別可行的衣殼變體方面帶來了挑戰。在本研究中，我們提出了一種端到端的擴散模型來生成具有增強活力的衣殼序列。使用公開可用的 AAV2 數據，我們生成了 38,000 個不同的 AAV2 病毒蛋白 (VP) 序列，並評估了 8,000 個用於病毒選擇。結果證明了我們的模型優於傳統方法。此外，在沒有 AAV9 衣殼數據的情況下，除了野生型序列外，我們使用相同的模型直接生成了許多具有多達 9 個突變的可行序列。我們將剩下的 30,000 個樣本轉移到 AAV9 域。此外，我們對 AAV9 VP 超變區 VI 和 V 進行了誘變，有助於持續改進 AAV9 VP 序列。這項研究代表了 rAAV 載體設計和功能驗證的重大進展，為提高基因治療應用中的特異性和轉導效率提供了創新的解決方案。
...
摘要：自然語言處理 (NLP) 領域隨著深度學習技術的快速發展取得了顯著進展。文本情緒分析的研究方向之一是醫療文本的情緒分析，這在臨床診斷中具有巨大的應用潛力。然而，醫療領域目前缺乏足夠的文本數據集，而情緒分析的有效性受到不同的模型設計方法的影響很大，這提出了挑戰。因此，本文重點關注醫療領域，使用來自 Transformer（BERT）的雙向編碼器表示作為預訓練的基本模型，並在輸出層對卷積神經網路（CNN）、全連接網路（FCN）和圖形卷積網路（GCN）等模組進行實驗。在 METS-CoV 數據集上進行了實驗和分析，以探索整合不同深度學習網路後的訓練效能。結果表明，當與 BERT 等預訓練模型結合使用時，CNN 模型在較小的醫療文本數據集上訓練時優於其他網路。本研究強調了模型選擇在醫療領域實現有效情緒分析的重要性，並為未來開發更有效率的模型架構的研究提供了參考。
...
尤其是在醫療情境中。處理取得標籤資料相關的挑戰已導致自監督學習
與半監督學習的崛起，特別是在標註資料有限的場景中。在本文中，我們提出一個創新的方法，將
自監督學習整合到半監督模型中，以增強醫學影像辨識。我們的做法從使用 BYOL 方法在未標籤資料上進行預訓練開始。隨後，我們合併偽標籤和標籤
資料集以建構一個神經網路分類器，並透過反覆微調對其進行精煉。在三個不同資料集上的實驗結果
顯示我們的做法最佳化地利用未標籤資料，在準確度方面優於現有醫學影像辨識方法。
...
摘要：可信度是將不透明深度學習模型安全應用於高風險領域（如醫學）的主要先決條件。了解決策過程不僅有助於培養信任，還可能揭示複雜模型以前未知的決策準則，這可能會推進醫學研究的狀態。從黑盒模型中發現與決策相關的概念是一項特別具有挑戰性的任務。本研究提出了基於潛在擴散的反事實軌跡的概念發現（CDCT），這是一個用於概念發現的新型三步驟框架，利用了擴散模型的優越圖像合成能力。在第一步中，CDCT 使用潛在擴散模型 (LDM) 來生成反事實軌跡數據集。該數據集用於使用變分自動編碼器 (VAE) 推導與分類相關概念的解糾纏表示。最後，應用搜索算法來識別解糾纏潛在空間中的相關概念。將 CDCT 應用於在最大的公共皮膚病變數據集上訓練的分類器不僅揭示了幾個偏差的存在，還揭示了有意義的生物標誌物。此外，在 CDCT 中生成的對事實比以前建立的最新方法產生的對事實顯示出更好的 FID 分數，同時資源效率提高了 12 倍。無監督概念發現對於可信賴 AI 的應用以及在各種領域進一步發展人類知識具有巨大潛力。CDCT 代表了朝這個方向邁出的進一步一步。
...
摘要：異常偵測在風力渦輪機預測性維護領域中扮演著關鍵角色，但由於特定領域的公開資料集稀少，因此不同演算法的比較會構成一項困難的任務。許多不同方法的比較，不是使用由來自許多不同領域資料組成的基準、無法存取的資料，就是使用少數幾個公開可用的資料集，而這些資料集缺乏有關故障的詳細資訊。此外，許多出版品都強調了故障偵測成功的幾個案例研究。透過這篇論文，我們發布了一個高品質的資料集，其中包含來自 3 個不同風場的 36 台風力渦輪機的資料，以及我們所知任何公開風力渦輪機資料集中最詳細的故障資訊。新的資料集包含 89 年的風力渦輪機實際運作資料，分佈在 44 個標記時間框架中，這些時間框架會導致故障，以及代表正常行為的 51 個時間序列。此外，透過基於渦輪機狀態的標籤，可確保訓練資料的品質，適用於每個資料點。此外，我們提出了一種新的評分方法，稱為 CARE（涵蓋範圍、準確性、可靠性和早期性），它利用資料集中的資訊深度來識別一個良好的全方位異常偵測模型。此評分考量異常偵測效能、正確辨識正常行為的能力，以及在盡早偵測異常的同時，盡可能減少誤報的可能性。
...
摘要：近來，健康意識抬頭，新穎的方法讓個人在家監控睡眠。利用睡眠聲音優於智慧手錶等傳統方法，它不具侵入性，且能偵測各種生理活動。本研究旨在建構一個基於機器學習的睡眠評估模型，提供證據為基礎的評估，例如睡眠開始時頻繁移動導致睡眠品質不佳。萃取睡眠聲音事件，使用 VAE 衍生潛在表示，用 GMM 進行分群，並訓練 LSTM 進行主觀睡眠評估，在區分睡眠滿意度方面達到了 94.8% 的高準確度。此外，TimeSHAP 揭示了不同個人在有影響力的聲音事件類型和時間上的差異。
...
摘要：大腦網路呈現階層組織，此複雜性對現有的深度學習模型構成挑戰，此類模型通常以扁平分類器建構，導致難以詮釋及「黑盒子」問題。為了彌補此差距，我們提出一個新穎架構：一個由弱監督和新興語言 (EL) 架構所告知的符號自動編碼器。此模型透過產生階層叢集和對應影像，超越傳統的扁平分類器，進而透過符號句子表示，以改善階層組織資料（例如內在腦網路）的臨床可詮釋性，而內在腦網路可以使用靜態功能性磁振造影影像來描述。我們的創新包括一個廣義階層損失函數，旨在確保句子和影像都能準確反映功能性腦網路的階層結構。這使我們能夠從更廣泛的觀點對功能性腦網路進行建模，進而深入瞭解更精細的細節。此外，我們引入一種量化方法來評估這些符號表示的階層一致性。我們的定性分析顯示，我們的模型成功產生階層組織且臨床可詮釋的影像，此發現獲得我們的量化評估所支持。我們發現，我們效能最佳的損失函數在識別與腦網路對應的影像時，可產生超過 97% 的階層一致性。此方法不僅提升深度學習模型在神經影像分析中的可詮釋性，也代表朝建模腦網路複雜的階層性質邁出一大步。
...
摘要：<paragraph>人工智慧的最新突破已做好準備，從根本上提升我們對醫療保健的研究和理解。開發一個整合式多對多架構，利用多重資料模式輸入，進行多重醫療任務的分析建模，對於統一理解現代醫學至關重要。在這項工作中，我們介紹 M3H，一個可解釋的多模式多任務機器學習醫療保健架構，它整合了廣泛醫療任務類別和機器學習問題類別中各種多模式輸入的學習成果。此架構的模組化設計確保其資料處理、任務定義和快速模型原型製作具有通用性，適用於臨床和運營醫療保健環境。我們透過驗證從四種模式（表格、時間序列、語言和視覺）訓練出來的模型，在四種機器學習問題類別中進行 41 項醫療任務，來評估 M3H 架構。我們的結果證明，M3H 持續產生多任務模型，其表現優於標準單任務模型（1.1- 37.2%），涵蓋 16 個醫療部門的 37 種疾病診斷、三個醫院營運預測和一項患者表型任務：橫跨機器學習問題類別的監督二元分類、多類別分類、迴歸和聚類。此外，此架構引入一種新穎的注意力機制，以平衡自我開發（專注於學習來源任務）和交叉探索（鼓勵從其他任務中學習）。此外，M3H 提供可解釋性見解，說明額外任務的聯合學習如何透過提議的 TIM 分數影響來源任務的學習，進而闡明任務相互依賴的動態。其適應性架構促進自訂和整合，使其成為未來人工智慧驅動醫療保健系統的強大且可擴充的候選解決方案。</paragraph>
...
摘要：本文探討了群體結構多主體系統 (MAS) 在建立醫療必要性中的應用，這是一個涉及系統性地檢視患者特定醫療結構化和非結構化資料，並對照臨床指引的過程。我們透過將此複雜任務分解成更小、更易於管理的子任務來處理。每個子任務都由一個專門的 AI 主體處理。我們對各種提示策略對這些主體的影響進行系統性研究，並對不同的大型語言模型 (LLM) 進行基準測試，以確定它們在完成這些任務時的準確性。此外，我們探討了這些主體如何提供可解釋性，從而增強系統內的信任和透明度。
...
摘要：人工智慧 (AI) 的廣泛整合在涉及 AI 啟用系統的事件中引入了責任和問責制的複雜挑戰。這些系統的互聯性、AI 引發事件的道德考量，加上 AI 技術的不確定性和缺乏相應的法規，使得傳統的責任歸屬具有挑戰性。為此，這項工作提出了一種計算反思平衡 (CRE) 方法，為所有利害關係人建立一個連貫且在道德上可接受的責任歸屬架構。計算方法提供了一種結構化的分析，克服了概念方法在處理動態且多面向場景時的限制，展示了該架構在責任歸屬過程中的可解釋性、連貫性和適應性屬性。我們探討了與平衡計算中的索賠相關的初始啟動層級的關鍵作用。使用 AI 輔助醫療決策支援系統作為案例研究，我們說明了不同的初始化如何導致不同的責任分配。該架構提供了對 AI 引發事件中問責制的寶貴見解，透過持續監控、修訂和反思，促進建立一個永續且有韌性的系統。
...
摘要：人工智慧透過預測模型協助醫療保健專業人員，大幅轉變臨床決策制定。本研究探討在醫療保健中對 AI 應用程式公平性和可解釋性的關鍵需求，以確保不同病患人口統計資料的公平結果。透過專注於敗血症相關死亡率的預測模型，我們提出一個方法，學習效能最佳化的預測模型，然後採用轉移學習流程，產生具有較佳公平性的模型。我們的模型也導入一種新穎的基於置換特徵重要性演算法，旨在闡明每個特徵在提升預測公平性方面的貢獻。與現有的可解釋性方法專注於解釋特徵對預測效能的貢獻不同，我們提出的方法獨特地彌補了理解每個特徵如何促成公平性的差距。這項進展至關重要，因為敗血症的死亡率很高，且在三分之一的醫院死亡案例中扮演角色。我們的模型不僅有助於識別並減輕預測模型中的偏差，也透過改善模型預測的透明度和公平性，促進醫療保健利害關係人之間的信任，進而促成更公平且值得信賴的醫療保健服務。
...
摘要：憂鬱症是現今一個重要的議題。根據世界衛生組織 (WHO)，2023 年有超過 2.8 億人正在與憂鬱症搏鬥。這是一個龐大的數字；若不認真看待，這些數字將會快速增加。約有 48.9 億人是社群媒體使用者。人們在 Twitter、Facebook、Reddit、Instagram 等平台上表達他們的感覺和情緒。這些平台包含有價值的資訊，可用於研究目的。已經在各種社群媒體平台上進行了大量的研究。然而，這些努力仍存在某些限制。特別是，先前的研究僅專注於在推文中偵測憂鬱症和憂鬱症的嚴重程度。此外，在資料集標籤中存在不準確之處。在這項研究工作中，使用基於詞彙標籤的 Twitter 資料庫中的推文，預測了五種類型的憂鬱症（躁鬱症、重度憂鬱症、精神病性憂鬱症、非典型憂鬱症和產後憂鬱症）。可解釋 AI 用於透過重點標示代表憂鬱症類型的推文部分來提供理由。來自 Transformer 的雙向編碼器表示法 (BERT) 用於特徵萃取和訓練。機器學習和深度學習方法用於訓練模型。BERT 模型呈現出最有希望的結果，達到 0.96 的整體準確度。
...
摘要：深度學習大幅轉化了醫學影像和放射線學領域，能辨識醫學影像中的病理，包括電腦斷層掃描 (CT) 和 X 光掃描。然而，深度學習模型的效能，特別是在分割任務中，常常受到廣泛註解資料集需求的限制。為了應對這個挑戰，透過可解釋 AI 和反事實解釋的產生，探索了弱監督語意分割的能力。本研究的範圍是開發一種新穎的反事實內插方法 (COIN)，透過使用生成模型，將預測的分類標籤從異常翻轉為正常。例如，如果分類器將輸入醫學影像 X 視為異常，表示存在病理，那麼生成模型的目標是對異常區域進行內插，從而逆轉分類器的原始預測標籤。此方法使我們能夠產生病理的精確分割，而無需依賴於預先存在的分割遮罩。至關重要的是，利用影像層級標籤，這比建立詳細的分割遮罩容易取得許多。該方法的有效性透過分割愛沙尼亞塔爾圖大學醫院取得的 CT 影像中的合成目標和實際腎臟腫瘤來證明。研究結果表明，COIN 遠遠優於已建立的歸因方法，例如 RISE、ScoreCAM 和 LayerCAM，以及 Singla 等人提出的替代反事實解釋方法。此證據表明，COIN 是一種用於 CT 影像中腫瘤語意分割的有前途的方法，並在醫療保健中讓深度學習應用程式更容易取得和更有效邁進了一步，其中註解資料很稀少。
...
摘要：甲狀腺癌是一種日益嚴重的全球健康問題，需要進階的診斷方法。本篇評論探討了人工智慧 (AI) 與放射特徵在甲狀腺癌診斷中的應用。在符合 PRISMA 指南的規定下，針對多個資料庫進行了回顧，時間截至 2023 年 10 月。結合多個關鍵字，發現了一篇英文的學術出版品，主題為甲狀腺癌及相關主題。在移除 109 篇重複文獻後，原始搜尋回傳了 267 篇論文。在根據預先設定的標準審查了摘要和標題後，刪除了 124 篇文章，並選出相關研究。在進行綜合分析後，又排除了六篇研究。在納入的 28 篇研究中，結合超音波 (US) 影像的放射特徵分析，證明了其在診斷甲狀腺癌方面的有效性。研究結果不一，有些研究提出了新的策略，優於現狀。文獻強調了 AI 模型面臨的各種挑戰，包括可解釋性問題、資料集限制和操作員依賴性。28 篇納入研究的綜合發現提到，需要標準化工作和前瞻性多中心研究來解決這些問題。此外，還確定了克服這些障礙的方法，例如可解釋 AI 技術和個人化醫療技術的進步。本篇評論重點探討了 AI 和放射特徵如何轉變甲狀腺癌的診斷和治療。儘管面臨挑戰，未來在多學科合作、臨床適用性驗證和演算法改進方面的研究，仍有可能改善甲狀腺癌治療的患者預後和診斷精準度。
...
摘要：<paragraph>近年來，乳癌盛行率快速上升，使其成為全球主要的死亡原因之一。在所有癌症中，乳癌迄今為止是最常見的。手動診斷此疾病需要大量時間和專業知識。由於乳癌的檢測過程耗時，因此透過建立機器學習預測模型，可以協助預防乳癌進一步擴散。機器學習和可解釋 AI 在分類中至關重要，因為它們不僅提供準確的預測，還能深入了解模型如何做出決策，有助於理解和信任分類結果。在本研究中，我們評估並比較了五種不同的機器學習方法的分類準確度、精確度、召回率和 F-1 分數，並使用一個主要資料集（達卡醫學院醫院的 500 名患者）。五種不同的監督式機器學習技術，包括決策樹、隨機森林、邏輯迴歸、樸素貝氏和 XGBoost，已被用於在我們的資料集上獲得最佳結果。此外，本研究將 SHAP 分析應用於 XGBoost 模型，以解釋模型的預測並了解每個特徵對模型輸出的影響。我們比較了幾種演算法對資料進行分類的準確度，並與該領域的其他文獻進行對比。經過最終評估，本研究發現 XGBoost 達到了最佳模型準確度，為 97%。</paragraph>
...
摘要：深度學習 (DL) 用於從乳房攝影圖像診斷乳癌的模型通常以「黑盒子」運作，這使得醫療保健專業人員難以信賴和了解其決策過程。本研究提出一個整合架構，結合卷積神經網路 (CNN) 和可解釋人工智慧 (XAI)，以使用 CBIS-DDSM 資料集增強乳癌診斷。該方法包含一個精細的資料前處理管線和先進的資料擴充技術，以對抗資料集限制，並使用預先訓練的網路（例如 VGG-16、Inception-V3 和 ResNet）進行遷移學習。我們研究的重點是評估 XAI 在解釋模型預測方面的有效性，重點是利用豪斯多夫測度來評估 AI 生成的解釋與專家註釋之間的對齊程度。這種方法對於 XAI 在促進 AI 輔助診斷中的可信度和道德公平性至關重要。我們研究的結果說明了 CNN 和 XAI 在推進乳癌診斷方法方面的有效協作，從而促進了先進 AI 技術在臨床環境中的更無縫整合。透過增強 AI 驅動決策的可解釋性，這項工作為 AI 系統和醫療從業人員之間的進一步協作奠定了基礎，最終豐富了患者照護。此外，我們研究的影響遠遠超出了當前的研究方法。它鼓勵進一步研究如何結合多模式資料和改進 AI 解釋，以滿足臨床實務的需求。
...
摘要：以人为本的可解释 AI（HCXAI）主张将社会层面纳入 AI 解释中。HCXAI 话语的核心是社会透明度（ST）框架，该框架旨在让用户了解 AI 系统的社会组织背景。在这项工作中，我们建议扩展 ST 框架以解决大型语言模型（LLM）中社会错误归因的风险，尤其是在心理健康等敏感领域。事实上，LLM 能够出色地模拟角色和人物，这可能会导致设计人员的意图与用户对社会属性的认知之间出现错配，从而有风险助长情绪操纵和危险行为、认知不公正和不合理的信任。为了解决这些问题，我们建议通过第五个“W 问题”来增强 ST 框架，以阐明设计人员和用户赋予 LLM 的具体社会属性。此补充旨在弥合 LLM 能力和用户认知之间的差距，促进基于 LLM 的技术的道德负责开发和使用。
...
摘要：<paragraph>背景：氣胸是一種急性胸腔疾病，是由肺部和胸壁之間異常的空氣集合所引起。為了解決深度學習 (DL) 模型經常伴隨的不透明性，可解釋的人工智慧 (XAI) 方法已被引入，用於勾勒出與 DL 模型所做的氣胸診斷相關的區域。然而，這些解釋有時會偏離實際病灶區域，這突顯了進一步改進的必要性。方法：我們提出了一種模板引導的方法，將氣胸的臨床知識納入 XAI 方法產生的模型解釋中，從而提高這些解釋的品質。利用放射科醫師建立的一個病灶描繪，我們的做法首先產生一個模板，它代表氣胸發生的潛在區域。然後將此模板疊加在模型解釋上，以過濾掉超出模板邊界的無關解釋。為了驗證其效力，我們對三種 XAI 方法進行了比較分析，在兩個真實世界的資料集中解釋了兩個 DL 模型，分別採用和不採用我們的模板指導。結果：所提出的方法在建立在三種 XAI 方法、兩個 DL 模型和兩個資料集上的十二個基準情境中，持續改善基準 XAI 方法。在比較模型解釋和真實病灶區域時，透過效能改善相對於基準效能所計算出的平均增量百分比，在聯合交集 (IoU) 中為 97.8%，在 Dice 相似性係數 (DSC) 中為 94.1%。結論：在氣胸診斷的背景下，我們提出了一種模板引導的方法來改善 AI 解釋。我們預期我們的模板指導將透過整合臨床領域專業知識，為闡明 AI 模型建立一種新的方法。</paragraph>
...
摘要：<paragraph>在當前的機器翻譯 (MT) 領域中，Transformer 架構脫穎而出，成為黃金標準，特別適用於資源豐富的語言對。本研究探討其對資源匱乏的語言對的功效，包括英語↔愛爾蘭語和英語↔馬拉地語語言對。值得注意的是，該研究確定了最佳超參數和子詞模型類型，以顯著提高 Transformer 模型對資源匱乏的語言對的翻譯品質。
資源匱乏語言的平行語料庫的稀缺性會阻礙機器翻譯的發展。為了解決這個問題，開發了 gaHealth，這是愛爾蘭語的第一個健康數據雙語語料庫。專注於健康領域，使用這個領域內資料集開發的模型，與 LoResMT2021 共享任務中的模型相比，在 BLEU 分數方面表現出非常顯著的提升。隨後使用多維品質指標錯誤分類法進行的人工評估，展示了 Transformer 系統在降低準確性和流暢性錯誤方面，與基於 RNN 的對應系統相比，表現出優異的性能。
此外，本論文介紹了 adaptNMT 和 adaptMLLM，這兩個開源應用程式專門用於開發、微調和部署神經機器翻譯模型。這些工具大幅簡化了設定和評估流程，讓開發人員和翻譯人員都可以更輕鬆地使用機器翻譯。值得注意的是，adaptNMT 以 OpenNMT 生態系統為基礎，通過強調模型開發的環境足跡，促進生態友好的自然語言處理研究。adaptMLLM 對 MLLM 的微調，展示了對兩個資源匱乏的語言對：英語↔愛爾蘭語和英語↔馬拉地語的翻譯性能的提升，與 LoResMT2021 共享任務中的基準相比。</paragraph>
...
視網膜影像和血管反映身體的微觀和巨觀血管健康。可以用來診斷糖尿病併發症，包括糖尿病視網膜病變 (DR)、神經病變、腎病變和動脈粥樣硬化性心血管疾病，以及預測心血管事件的風險。使用數位化視網膜影像的高通量 DR 偵測人工智慧 (AI) 啟用系統已在臨床上採用。除了 DR 篩檢之外，AI 整合也具有巨大的潛力，可以解決與糖尿病患者整體照護相關的挑戰。在這項工作中，我們旨在全面檢視基於與糖尿病診斷、預後和管理相關的視網膜影像的 AI 應用研究文獻。我們將描述整體 AI 輔助糖尿病照護的發現，包括但不限於 DR 篩檢，並討論實施此類系統的障礙，包括涉及倫理、資料隱私、公平取得和可解釋性的問題。透過評估患者的健康狀態，以及未來心血管併發症的風險預後，AI 輔助視網膜影像分析有可能成為糖尿病患者現代化個人化醫療的中心工具。
...
摘要：本研究從多個利害關係人的角度調查了不同人工智慧 (AI) 應用在教育中的可接受性，包括學生、教師和家長。承認 AI 在教育中的轉型潛力，它解決了與資料隱私、AI 代理、透明度、可解釋性和 AI 的道德部署相關的問題。透過小插曲方法，向參與者展示了四種情境，其中 AI 的代理、透明度、可解釋性和隱私受到操縱。在每個情境之後，參與者完成了一項調查，其中記錄了他們對 AI 的整體效用、個別效用、公正性、信心、風險和在有機會的情況下使用每個情境的 AI 的意圖的看法。收集的資料包含 1198 名多個利害關係人參與者的最終樣本，透過合作機構和社群媒體活動進行分發，並專注於對四個 AI 使用案例的個別回應。對資料的調解分析表明，對 AI 的接受度和信任在利害關係人組之間存在顯著差異。我們發現，AI 代理、透明度和可解釋性的高低程度之間的關鍵調解因素，以及使用不同教育 AI 的意圖，包括感知到的整體效用、公正性和信心。該研究強調，接受 AI 在教育中的應用是一個微妙且多方面的問題，除了不同的利害關係人的看法之外，還需要仔細考慮具體的 AI 應用及其特點。
...
摘要：基於可穿戴單導程心電圖 (ECG) 裝置的遠端病患監測在早期偵測心臟疾病方面具有顯著的潛力，特別是與用於自動化心臟疾病偵測的人工智慧 (AI) 方法結合使用時。先前已有研究應用基於深度學習的 AI 方法進行心臟疾病偵測。然而，這些模型尚未被廣泛接受為臨床診斷的可靠輔助工具，部分原因在於目前圍繞許多 AI 演算法的黑箱感知。特別是，需要找出有助於做出準確診斷的 ECG 訊號關鍵特徵，從而增強模型的可解釋性。在本研究中，我們開發了一種視覺轉換器方法，以根據單導程 ECG 資料找出心房顫動。殘差網路 (ResNet) 方法也與視覺轉換器方法一起開發，用於比較。這些模型應用於 Chapman-Shaoxing 資料集，以分類心房顫動，以及另一種常見的心律不整，即竇性心動過緩和正常竇性心律的心跳。這些模型能夠找出決定最終分類的心跳關鍵區域，並強調 P 波和 T 波，以及心跳持續時間和訊號振幅在區分正常竇性心律、心房顫動和竇性心動過緩方面的重要性。
...
摘要：本文介紹了一種使用進階大型語言模型 (LLM) 進行憂鬱症偵測和治療的新模式：生成式預訓練轉換器 4 (GPT-4)、Llama 2 chat 和 Gemini。這些 LLM 經過微調，並使用專門提示來診斷、解釋和建議憂鬱症的治療方法。一種獨特的少次提示方法增強了模型根據 DSM-5 標準分析和解釋憂鬱症狀的能力。在互動階段，這些模型從事同理對話管理，利用 PsychDB 和認知行為療法 (CBT) 指南等資源，促進與經歷重度憂鬱症的人們的支持性互動。此外，本研究引入了 Illuminate 資料庫，其中包含各種 CBT 模組，有助於提供個人化治療建議。本研究使用 F1 分數、精確度、召回率、餘弦相似度和面向召回率的評估用於評估摘要的替身 (ROUGE) 等指標來評估 LLM 效能，並在不同的測試集中證明了它們的有效性。這種綜合方法將尖端的 AI 與既定的心理方法相結合，為心理保健提供了新的可能性，並展示了 LLM 在革新憂鬱症診斷和治療策略方面的潛力。
...
摘要：<paragraph>AI 系統的解釋很少能滿足受演算法決策 (ADM) 影響的人們資訊需求。傳達資訊與受影響利害關係人重視的資訊之間的落差會妨礙理解和遵守法規架構，例如 AI 法案。為了解決這個落差，我們提出「XAI 新手問題庫」：受影響利害關係人資訊需求的目錄，包含兩個 ADM 使用案例（就業預測和健康監控），涵蓋資料、系統脈絡、系統使用和系統規格類別。在訪談研究中收集資訊需求，參與者在詢問後收到解釋作為回應。參與者進一步報告他們的理解和決策信心，顯示雖然在收到解釋後信心往往會增加，但參與者也遇到了理解挑戰，例如無法說明為什麼他們的理解感覺不完整。解釋進一步影響參與者對系統風險和好處的看法，他們會根據使用案例確認或改變這些看法。當風險被認為很高時，參與者表示特別有興趣了解意圖的解釋，例如系統為何以及為了什麼目的而建立。透過這項工作，我們旨在透過提供在決定採用 ADM 系統時與他們相關的資訊和挑戰的概觀，來支持受影響的利害關係人參與可解釋性。我們最後總結我們的發現，列出六項關鍵影響，為未來針對受影響利害關係人受眾的解釋設計提供參考。</paragraph>
...
摘要：<paragraph>人工智慧 (AI) 的快速演變，特別是在大型語言模型 (LLM) 和生成式 AI 的領域，為各種領域的應用開啟了新途徑，但其在商業教育中的作用仍未得到充分探討。本研究首次引入了基準，以評估七個主要 LLM 的效能，包括 OpenAI 的模型 (GPT-3.5 Turbo、GPT-4 和 GPT-4 Turbo)、Google 的模型 (PaLM 2、Gemini 1.0 Pro) 和 Anthropic 的模型 (Claude 2 和 Claude 2.1)，這些模型在 GMAT 上的表現如何，而 GMAT 是研究所商業課程入學程序中的一項關鍵考試。我們的分析顯示，大多數 LLM 的表現都優於人類考生，其中 GPT-4 Turbo 不僅優於其他模型，也超越了頂尖商學院研究所學生的平均分數。透過案例研究，本研究探討了 GPT-4 Turbo 解釋答案、評估回應、找出錯誤、客製化說明和產生替代情境的的能力。與前代相比，最新的 LLM 版本 GPT-4 Turbo、Claude 2.1 和 Gemini 1.0 Pro 在推理任務方面有顯著的進步，突顯了它們在複雜問題解決方面的潛力。儘管 AI 在教育、評量和輔導方面的前景明確，但仍有挑戰存在。我們的研究不僅闡明了 LLM 的學術潛力，也強調了在教育中謹慎開發和應用 AI 的必要性。隨著 AI 技術的進步，建立 AI 互動的架構和協定、驗證 AI 生成內容的準確性、確保全球各地不同學習者的存取權，以及創造一個 AI 支援人類專業知識的教育環境至關重要。本研究為進一步探索負責任地使用 AI 以豐富教育體驗並改善考試準備和評量方法奠定了基礎。</paragraph>
...
摘要：預測加護病房 (ICU) 病患的院內死亡率是最終臨床結果的關鍵。AI 已展現出優異的準確性，但缺乏可解釋性。為了解決這個問題，本文提出可解釋的多模態死亡率預測器 (X-MMP)，以透過多模態 ICU 資料提供有效且可解釋的 AI 解決方案來預測院內死亡率。我們在架構中採用多模態學習，它可以接收臨床資料中的異質輸入並做出決策。此外，我們引進一種可解釋的方法，即層級傳播至 Transformer，作為 LRP 方法對 Transformer 的適當延伸，針對多模態輸入產生解釋，並揭示歸因於預測的顯著特徵。此外，可以視覺化每種模態對臨床結果的貢獻，協助臨床醫師了解決策背後的推理。我們根據 MIMIC-III 和 MIMIC-III Waveform Database Matched Subset 建構多模態資料集。基準資料集上的綜合實驗證明，我們提出的架構可以實現合理的解釋，並具有競爭力的預測準確性。特別是，我們的架構可以輕鬆轉移到其他臨床任務，這有助於發現醫療保健研究中的關鍵因素。
...
摘要：在過去的十年中，病理學中的人工智慧 (AI) 方法已大幅進展。然而，由於將研究結果轉化為臨床診斷產品的技術和法規障礙，以及缺乏標準化介面，整合到常規臨床實務中進展緩慢。開放且與供應商無關的 EMPAIA 計畫解決了這些挑戰。在此，我們概述了 EMPAIA 的成就和經驗教訓。EMPAIA 整合了病理學 AI 生態系統的各種利益相關者，即病理學家、電腦科學家和產業。在密切合作下，我們制定了技術互通性標準、AI 測試和產品開發建議，以及可解釋性方法。我們實作了模組化且開放原始碼的 EMPAIA 平台，並成功整合了來自 8 家不同供應商的 14 個基於 AI 的影像分析應用程式，展示了不同的應用程式如何使用單一標準化介面。我們優先考慮需求，並在歐洲和亞洲的 14 個不同的病理實驗室評估了 AI 在實際臨床環境中的使用情況。除了技術發展外，我們還為所有利益相關者建立了一個論壇，以分享數位病理學和 AI 的資訊和經驗。商業、臨床和學術利益相關者現在可以採用 EMPAIA 共同的開放原始碼介面，為大規模標準化和流程簡化提供了獨特的機會。需要進一步的努力才能在常規實驗室使用中有效且廣泛地建立 AI 協助。為此，已成立非營利協會 EMPAIA International，作為一個永續的基礎架構，以持續標準化並支援廣泛的實作和倡導 AI 輔助的數位病理學未來。
...
摘要：反事實解釋 (CE) 技術作為一種方法獲得關注，用於向與 AI 系統互動的使用者提供見解。雖然在醫學影像和自動駕駛汽車等領域廣泛研究，但圖形反事實解釋 (GCE) 方法相對而言較少探索。GCE 會產生一個與原始圖形相似的圖形，其不同的結果基於底層預測模型。在這些 GCE 技術中，儘管在其他領域（例如藝術風格和自然語言建模）中展現出令人印象深刻的成就，但植基於生成機制的技術受到的調查相對有限。對生成式解釋器的偏好源自它們在推理過程中生成反事實例項的能力，利用輸入圖形的自主獲取擾動。基於上述基本原理，我們的研究引入了 RSGG-CE，一種用於反事實解釋的強健隨機圖形生成器，能夠從學習到的潛在空間產生反事實範例，考慮部分有序的生成序列。此外，我們進行量化和定性分析，以比較 RSGG-CE 與 SoA 生成式解釋器的效能，強調其產生合理反事實候選項的能力增強。
...
摘要：可解釋 AI 的動機之一是讓人類對 AI 模型的使用和部署做出更好、更明智的決策。但需要仔細評估以評估是否已達到此預期。目前的評估主要關注解釋的演算法屬性，而涉及人類受試者的評估通常採用主觀問題來測試人類對解釋有用性的看法，而沒有以客觀指標和測量為基礎。在這項工作中，我們評估解釋是否可以在機器學習模型開發的實際場景中改善人類的決策制定。我們進行了一項涉及影像資料的混合方法使用者研究，以評估 SmoothGrad、GradCAM 和一個神諭解釋在兩個任務（模型選擇和反事實模擬）中產生的顯著性圖。令人驚訝的是，我們沒有發現任何證據表明在向使用者提供任何顯著性圖（即使是設計為易於理解且高度指示答案的合成神諭解釋）時，這些任務有顯著改善。儘管如此，解釋確實有助於使用者更準確地描述模型。這些發現表明，對於基於顯著性的解釋中的有用性和誤解的可能性，需要謹慎。
...
摘要：可解釋性和安全性會產生信任。這些需要一個模型來展現一致性和可靠性。要達成這些，必須使用和分析資料和知識，搭配與 AI 應用相關的統計和符號 AI 方法，單獨使用任何一種方法都無法達成。因此，我們主張並試圖證明神經符號 AI 方法更適合用於建立受信任的 AI 系統。我們提出 CREST 架構，展示一致性、可靠性、使用者層級的可解釋性，以及安全性是如何建立在神經符號方法上，這些方法使用資料和知識來支援關鍵應用程式的需求，例如健康和福祉。本文著重於大型語言模型 (LLM)，視為 CREST 架構中所選的 AI 系統。LLM 由於其在處理廣泛自然語言處理 (NLP) 場景方面的多功能性，而受到研究人員的廣泛關注。例如，ChatGPT 和 Google 的 MedPaLM 已成為提供一般和健康相關查詢資訊的極具前景的平台。儘管如此，這些模型仍然是黑盒子，儘管納入了人類回饋和指令引導的調整。例如，儘管制定了安全防護措施，ChatGPT 仍會產生不安全的回應。CREST 提出了一種在神經符號架構中利用程序和圖形化知識，來闡明與 LLM 相關挑戰的可行方法。
...
摘要：本研究調查了在 COVID-19 大流行期間及以後預測死亡率時已部署人工智慧 (AI) 模型的效能、可解釋性和穩健性。身為同類型研究中的首例，我們發現貝氏神經網路 (BNN) 和智慧訓練技術讓我們的模型在資料大幅變動時仍能維持效能。我們的結果強調了開發穩健的 AI 模型的重要性，此類模型即使在具挑戰性的條件下也能比擬或超越臨床醫師的預測。我們探索模型的可解釋性，發現隨機模型會產生更多元化且個人化的解釋，從而突顯了 AI 模型在現實世界臨床環境中提供詳細且個人化見解的必要性。此外，我們強調了量化 AI 模型中不確定性的重要性，這使臨床醫師能夠根據可靠的預測做出更明智的決策。我們的研究倡導在醫療保健的 AI 研究中優先實施科學，並確保 AI 解決方案在現實世界的臨床環境中實用、有益且永續。透過解決醫療保健環境中獨特的挑戰和複雜性，研究人員可以開發有效改善臨床實務和患者預後的 AI 模型。
...
摘要：肺癌是英國 21% 癌症死亡原因，五年存活率很大程度取決於發現癌症的期別。最近的研究展示了 AI 方法準確且提早從例行掃描診斷出肺癌的能力。然而，此證據尚未轉化為臨床實務，其中一個障礙是缺乏可解釋的模型。此研究探討變異自動編碼器 (VAE) 的應用，這是一種生成式 AI 模型，用於肺癌病灶。提議的模型在從 LIDC-IDRI 公共資料集的 3D 電腦斷層掃描中提取的病灶上受過訓練。透過聚類探索 VAE 產生的 2D 切片潛在向量表示，以證明其品質，並用於肺癌診斷的 MLP 分類器模型，最佳模型達成了 0.98 AUC 和 93.1% 準確度的最新指標。聚類分析顯示 VAE 潛在空間根據有意義的特徵成分，包括腫瘤大小、形狀、患者和惡性類別，將惡性和良性病灶的資料集分開。我們也包含標準高斯 VAE (GVAE) 和較新的狄利克雷 VAE (DirVAE) 的比較分析，後者以狄利克雷分佈取代先驗，以鼓勵更具可解釋性的潛在空間，並具備解開的特性表示。最後，我們展示了潛在空間穿越對應於臨床有意義特徵變化的潛力。
...
摘要：現有的用於解釋影像分類器輸出的工具可分為白盒，它依賴於存取模型內部，以及黑盒，不依賴於模型。由於 AI 在醫療領域的使用增加，因此解釋性工具的使用也隨之增加。現有關於醫學影像解釋的工作著重於白盒工具，例如 gradcam。然而，切換到黑盒工具有明顯的優點，包括能夠將其與任何分類器一起使用，以及各種可用的黑盒工具。在標準影像上，黑盒工具與白盒工具一樣精確。在本文中，我們比較了數種黑盒方法與腦癌 MRI 資料集上的 gradcam 的效能。我們證明了大多數黑盒工具不適合解釋醫學影像分類，並詳細分析了其不足的原因。我們還表明，一種黑盒工具，因果解釋性為基礎的 rex，與 \gradcam 的表現一樣好。
...
摘要：AI 發展社群日益利用 Hugging Face 等託管中介，讓使用者上傳的模型和訓練資料易於取得。這些模型市集降低了數十萬名使用者的技術部署障礙，但可能會被用於許多潛在有害和非法的方式。在本文中，我們說明了 AI 系統既可以「包含」內容，又可以成為開放式工具的方式，呈現了迄今為止最棘手的平台治理挑戰之一。我們提供 Hugging Face、GitHub 和 Civitai 等三個說明性平台上發生多起事件的案例研究，以探討模型市集如何審核模型。根據此分析，我們概述了產業為回應審核需求而發展的重要（但仍然有限）實務：授權、存取和使用限制、自動化內容審核和開放政策制定。雖然目前的政策挑戰相當大，但我們最後提出了一些想法，說明平台如何能更好地動員資源，作為謹慎、公平且適度的法規存取點。
...
摘要：<paragraph>背景與目標：透過萃取這些資訊，機器學習或深度學習 (ML/DL) 為基礎的自主資料分析工具可以協助臨床醫師和癌症研究人員從複雜的資料集中發現模式和關係。最近已發布許多基於 DL 的卵巢癌 (OC) 資料分析。這些分析在癌症的各個方面（例如，它們所探討的子領域和癌症類型）和資料分析功能上都非常多元。然而，目前對於這些分析在這些功能和 AI 保證 (AIA) 方面的全面了解仍有不足。這份系統性回顧旨在透過檢視現有文獻並找出使用 DL 進行 OC 資料分析的重要面向來填補這個缺口，並明確關注關鍵功能和 AI 保證的觀點。方法：使用 PRISMA 架構在三個期刊資料庫中進行全面的搜尋。分析僅包含 2015 年至 2023 年間發表於同行評審期刊的研究。結果：在回顧中，總共檢視了 96 項由 DL 驅動的分析。這些發現揭露了幾個關於由 DL 驅動的卵巢癌資料分析的重要見解：- 大多數研究（96 項中有 68 項，佔 71%）都聚焦於偵測和診斷，而沒有任何研究探討 OC 的預測和預防。- 這些分析主要基於來自非多元族群的樣本（72/96 項研究，佔 75%），僅限於某個地理位置或國家。- 只有少數研究（僅 32/96 項，佔 33%）執行整合式分析，其中大多數使用同質性資料（臨床或組學）。- 值得注意的是，只有 8.3%（96 項中有 8 項）的研究使用外部和多元的資料集驗證其模型，這凸顯了加強模型驗證的必要性，而且 - 在癌症資料分析中納入 AIA 仍處於非常早期的階段；只有 2.1%（96 項中有 2 項）透過可解釋性明確探討 AIA。</paragraph>
...
摘要：<paragraph>可解釋性是深度學習中長久以來的挑戰，特別是在醫療保健等高風險領域。常見的可解釋性方法會突顯驅動 AI 模型決策的影像區域。然而，人類非常依賴語言來傳達不僅僅是「在哪裡」，還有「是什麼」的解釋。此外，大多數的可解釋性方法著重於解釋個別的 AI 預測，而不是描述 AI 模型一般使用的特徵。後者對於模型和資料集稽核特別有用，甚至可能在 AI  zuneh使用於新任務時產生知識。在此，我們提出一個可解釋性策略，它使用視覺語言模型來識別視覺分類任務的基於語言的描述符。透過利用影像和文字之間預先訓練的聯合嵌入空間，我們的做法將新的分類任務估計為字詞的線性組合，結果是每個字詞的權重，表示其與基於視覺的分類器的對齊方式。我們使用兩個醫學影像分類任務評估我們的做法，我們發現，儘管缺乏特定領域的語言訓練，產生的描述符在很大程度上與臨床知識一致。然而，我們的做法也發現了所使用公眾資料集中「捷徑連線」的可能性。為了達到可解釋性的功能性衡量，我們進行了一項試驗性讀者研究，我們發現 AI 識別的字詞可以使非專家人類在非平凡的層級上執行專業的醫療任務。總的來說，我們的結果強調了使用多模態基礎模型來提供直觀、基於語言的視覺任務解釋的潛力。</paragraph>
...
摘要：<paragraph>使用醫療影像訓練的人工智慧 (AI) 模型，在臨床任務中經常展現出表現差異的偏見。由於現實世界醫療影像資料中並非所有偏見來源都容易辨識，因此要全面評估這些偏見如何編碼在模型中，以及偏見緩解方法在改善表現差異的效能如何，是一項挑戰。在本文中，我們介紹了一個新穎的分析架構，用於系統且客觀地探討醫療影像中的偏見對 AI 模型的影響。我們開發並測試了這個架構，用於執行受控的電腦內試驗，以評估醫療影像 AI 的偏見，並使用一個工具來產生已知疾病影響和偏見來源的合成磁共振影像。透過使用三個反事實偏見情境來衡量模擬偏見效應對卷積神經網路 (CNN) 分類器的影響，以及三個偏見緩解策略的效能，展示了可行性。分析顯示，當 CNN 在合成資料集上訓練時，模擬偏見會導致預期的子群表現差異。此外，重新加權被認定為此設定中最成功的偏見緩解策略，我們展示了可解釋的 AI 方法如何協助調查使用這個架構在模型中偏見的表現。開發公平的 AI 模型是一項重大的挑戰，因為醫療影像資料集中可能存在許多且通常未知的偏見來源。在這項工作中，我們提出了一種新的方法論，用於客觀地研究偏見和緩解策略對深度學習管線的影響，這可以支援開發穩健且負責任的臨床 AI。</paragraph>
...
摘要：機器學習為自動預測中風後症狀及其對復健的反應提供了極大的潛力。這項工作的重大挑戰包括神經影像資料維度極高、可用於學習的資料集規模相對較小，以及如何有效地結合神經影像和表格資料（例如人口統計資訊和臨床特徵）。本文根據兩種策略評估了多種解決方案。第一種是使用概括 MRI 掃描的 2D 影像。第二種是選擇能提升分類精確度的主要特徵。此外，我們引進了一種新的方法，即在結合從 MRI 中萃取的感興趣區域和表格資料的符號表示的影像上訓練卷積神經網路 (CNN)。我們評估了一系列 CNN 架構（2D 和 3D），這些架構經過訓練，可以根據 MRI 和表格資料的不同表示，來預測中風後口語描繪能力的綜合測量是否在失語症或非失語症範圍內。MRI 和表格資料取自 758 位參與 PLORAS 研究的英語系中風倖存者。僅針對病灶大小的基線邏輯迴歸分類準確度為 0.678，當陸續加入初始症狀嚴重程度和恢復時間後，準確度分別提升至 0.757 和 0.813。當從每個 MRI 掃描中萃取出 8 個感興趣區域，並在 2D 殘差神經網路中與病灶大小、初始嚴重程度和恢復時間結合時，觀察到最高的分類準確度 0.854。我們的研究結果展示了如何將影像和表格資料結合，以獲得很高的中風後分類準確度，即使根據機器學習的術語，資料集很小。最後，我們提出如何改善現有模型，以使用來自醫院掃描器的影像，達到更高層級的準確度。
...
摘要：可解釋人工智慧 (XAI) 已成為處理任務關鍵應用程式時的一項基本需求，確保所採用的黑盒子 AI 模型的透明度和可解釋性。XAI 的重要性涵蓋各種領域，從醫療保健到金融，其中了解深度學習演算法的決策制定過程至關重要。大多數基於 AI 的電腦視覺模型通常是黑盒子；因此，在影像處理中提供深度神經網路的可解釋性對於它們在醫學影像分析、自動駕駛和遙測應用中的廣泛採用和部署至關重要。最近，已經引入了幾種針對影像分類任務的 XAI 方法。相反地，影像分割在可解釋性的背景下受到的關注相對較少，儘管它是電腦視覺應用中的一項基本任務，特別是在遙測中。只有部分研究提出用於影像分割的基於梯度的 XAI 演算法。本文採用最近的無梯度 Sobol XAI 方法進行語意分割。為了測量 Sobol 方法在分割中的效能，我們提出了一種基於可學習雜訊模型的定量 XAI 評估方法。此模型的主要目標是在解釋圖上加入雜訊，其中較高的加入雜訊表示較低的準確度，反之亦然。進行基準分析以評估和比較三種 XAI 方法的效能，包括 Seg-Grad-CAM、Seg-Grad-CAM++ 和 Seg-Sobol，使用所提出的基於雜訊的評估技術。這構成了使用高解析度衛星影像執行和評估 XAI 方法的首次嘗試。
...
摘要：大型語言模型在短時間內已在多個領域中大量增加。然而，由於事實性、連貫性和幻覺等問題，醫療和保健領域對其採用存在猶豫。鑑於醫療保健的高風險性質，許多研究人員甚至警告在解決這些問題之前不要使用它。在醫療保健中實施和部署 LLM 的關鍵是使這些模型值得信賴、透明（儘可能）且可解釋。在本文中，我們描述了創建可靠、可信和無偏見模型的關鍵要素，作為它們在醫療保健中得到採用的必要條件。具體來說，我們專注於醫療保健背景下幻覺的量化、驗證和緩解。最後，我們討論了 LLM 在醫療保健中的未來可能是什麼樣子。
...
摘要：人工智慧（AI）已快速進展，現在已準備在各種應用中部署，例如自主系統、醫療診斷和自然語言處理。
早期採用 AI 技術於實際應用並非沒有問題，特別是神經網路可能不穩定且容易受到對抗性範例的影響。從長遠來看，需要開發適當的安全保證技術，以減少因可避免的系統故障而造成的潛在危害，並確保可信度。本文著重於認證和可解釋性，概述了已開發的技術，以確保 AI 決策的安全並討論未來的挑戰。
...
摘要：歐洲議會議事服務總局為歐洲議會議員編製了一份報告，其中列舉了人工智能 (AI) 在醫學和醫療保健方面的七項主要風險：由於 AI 錯誤造成的患者傷害、醫療 AI 工具的濫用、AI 中的偏見和現有不平等的延續、缺乏透明度、隱私和安全問題、問責差距以及實施障礙。
  在這項研究中，我們提出了十四項功能性要求，AI 系統可以實施這些要求來降低其醫療目的相關的風險：AI 護照、使用者管理、法規檢查、僅限學術用途免責聲明、資料品質評估、臨床醫師雙重檢查、持續效能評估、稽核追蹤、持續可用性測試、回顧性/模擬案例審查、偏見檢查、可解釋 AI、加密和使用經過實地測試的程式庫，以及語意互操作性。
  我們在此的目的是提供技術解決方案的具體高階規格，以確保持續良好的效能和使用 AI 系統，以符合未來歐盟法規架構，造福患者。
...
摘要：人工智能技術可協助分類病患的生理活動，並預測遠距監控病患的生命徵象。基於深度學習模型等非線性模型的回歸分析，由於其黑箱性質，可解釋性有限。這可能需要決策者根據非線性模型結果盲目做出決策，特別是在醫療保健應用中。在非侵入性監控中，來自追蹤感測器的病患資料及其疾病特徵，作為預測未來生命徵象的輸入特徵。解釋各種特徵對監控應用程式整體輸出的貢獻，對於臨床醫師的決策至關重要。在本研究中，提出了一個可解釋人工智慧定量分析 (QXAI) 框架，具備事後模型可解釋性和內在可解釋性，適用於監督式學習方法中的回歸和分類任務。這是透過利用 Shapley 值概念，並將注意力機制整合到深度學習模型中來實現的。我們採用人工神經網路 (ANN) 和基於注意力的雙向 LSTM (BiLSTM) 模型，根據感測器資料預測心率和分類生理活動。深度學習模型在預測和分類任務中都達到了最先進的結果。對輸入資料進行全局解釋和局部解釋，以了解各種病患資料的特徵貢獻。所提出的 QXAI 框架使用 PPG-DaLiA 資料來預測心率，並使用行動健康 (MHEALTH) 資料根據感測器資料來分類生理活動。蒙地卡羅近似法應用於該框架，以克服 Shapley 值計算所需的時間複雜度和高運算能力需求。
...
摘要：在可解釋人工智慧 (XAI) 研究中，主要的重點在於為專家和從業人員解釋模型。模型不可知論和局部解釋方法在許多應用中被認為是可解釋且足夠的。然而，在醫療保健等領域，最終使用者是沒有 AI 或領域專業知識的患者，因此迫切需要更易於理解且能建立對模型運作信任的模型解釋。我們假設產生具有敘事性、特定於患者和全局（模型的整體）的模型解釋將有助於更好的理解力和決策制定。我們使用決策樹模型來測試這一點，為被確定為患有冠狀動脈心臟病高風險的患者產生局部和全局解釋。這些解釋會提供給非專家使用者。我們發現對特定類型的解釋有強烈的個人偏好。大多數參與者偏好全局解釋，而較小的一組偏好局部解釋。對這些參與者的心理模型進行基於任務的評估，提供了寶貴的回饋，以增強敘事全局解釋。這反過來指導了既值得信賴又可操作的健康資訊系統的設計。
...
摘要：<paragraph>可解釋 AI 在醫學影像中至關重要。在神經科學這個充滿挑戰的領域中，視覺主題呈現出高度的複雜性，特別是在三維空間中。神經科學的應用，包括從 MRI 中識別腦溝特徵，由於專家之間不同的註釋協定以及大腦複雜的三維功能，面臨著重大的障礙。因此，傳統的可解釋性方法在有效驗證和評估這些網路時有所不足。為了解決這個問題，我們首先提出一個數學公式，描述了各種電腦視覺任務中不同類別的解釋需求，根據驗證協定的可靠性，將其分類為自解釋、半解釋、非解釋和新模式學習應用。關於這個數學公式，我們提出了一個 3D 可解釋性框架，旨在驗證深度學習網路在偵測腦回旁溝（一個重要的腦部解剖特徵）中的輸出。這個框架整合了局部 3D 解釋、透過降維的整體解釋、串聯的整體解釋和統計形狀特徵，揭示了模式學習的新見解。我們訓練並測試了兩個先進的 3D 深度學習網路，使用具有挑戰性的 TOP-OSLO 資料集，顯著提高了腦溝偵測的準確度，特別是在左半球。在使用此資料集的不同註釋協定進行評估時，我們強調了無偏註釋程序在我們提出的 3D 框架中實現精確預測和有效模式學習的關鍵作用。所提出的框架不僅註釋了可變腦溝，還揭示了隱藏的 AI 知識，有望推進我們對大腦解剖和功能的理解。</paragraph>
...
摘要：電子健康記錄 (EHR) 和例行文件記錄實務在病患的日常照護中扮演至關重要的角色，提供一個關於健康、診斷和治療的整體記錄。然而，複雜且冗長的 EHR 敘述會讓醫療保健提供者不堪負荷，並造成診斷不準確的風險。大型語言模型 (LLM) 已在各種語言任務中展現其潛力，但其在醫療領域的應用需要確保將診斷錯誤降至最低，並防止病患受到傷害。在本文中，我們概述了一種創新的方法，用於提升 LLM 在自動化診斷生成領域的熟練度，這是透過整合一個醫療知識圖譜 (KG) 和一個新穎的圖譜模型：Dr.Knows 來實現的，其靈感來自臨床診斷推理程序。我們從美國國家醫學圖書館的統一醫學語言系統 (UMLS) 中衍生出 KG，這是一個強大的生物醫學知識儲存庫。我們的方法否定了預先訓練的需要，而改為將 KG 作為輔助工具，協助解釋和總結複雜的醫療概念。使用真實世界的醫院資料集，我們的實驗結果證明，將 LLM 與 KG 結合的建議方法有潛力改善自動化診斷生成的準確性。更重要的是，我們的方法提供了一條可解釋的診斷途徑，讓我們更接近實現 AI 輔助診斷決策支援系統的目標。
...
摘要：現有的用於診斷膝骨關節炎 (OA) 的人工智慧 (AI) 模型因缺乏透明度和可解釋性而受到批評，儘管它們達到了類似醫療專家的表現。這種不透明性使得它們難以在臨床實務中獲得信任。最近，可解釋的人工智慧 (XAI) 已成為一種專門技術，它可以透過揭示預測的推導方式來提供對模型預測的信心，從而促進 AI 系統在醫療保健中的使用。本文提供了用於膝骨關節炎診斷的 XAI 技術的首次調查。XAI 技術從兩個角度進行探討：資料可解釋性和模型可解釋性。本文的目的是提供有價值的見解，了解 XAI 對更可靠的膝骨關節炎診斷方法的潛力，並鼓勵在臨床實務中採用它。
...
摘要：最近在医疗保健中应用 AI 的进步已显示出令人难以置信的潜力，在诊断和疾病预后中超越人类表现。然而，随着 AI 模型的复杂性不断提高，人们对其不透明性、潜在偏差和可解释性的需求也越来越关注。为了确保 AI 系统的信任度和可靠性，尤其是在临床风险预测模型中，可解释性变得至关重要。可解释性通常是指 AI 系统提供其决策逻辑或决策本身的稳健解释的能力，以供人类利益相关者使用。在临床风险预测中，可解释性的其他方面，如公平性、偏差、信任度和透明度，也代表了超越可解释性的重要概念。在本综述中，我们探讨了这些概念之间的关系，因为它们经常一起或互换使用。本综述还讨论了为临床风险预测开发可解释模型的最新进展，强调了在临床实践中多种常见模式中进行定量和临床评估和验证的重要性。它强调了外部验证和结合不同的可解释性方法以增强信任度和公平性的必要性。采用严格的测试，例如使用具有已知生成因素的合成数据集，可以进一步提高可解释性方法的可靠性。开放获取和代码共享资源对于透明度和可复制性至关重要，能够促进可解释研究的增长和可信度。尽管存在挑战，但一种从临床医生到开发人员纳入利益相关者的端到端可解释性方法对于临床风险预测的成功至关重要。
...
摘要：儘管在醫學和醫療保健方面的人工智慧 (AI) 有重大進展，但 AI 技術在現實世界的臨床實務中部署和採用仍然有限。近年來，人們對與醫療 AI 相關的技術、臨床、倫理和法律風險提出了疑慮。為了增加現實世界的採用率，醫療 AI 工具必須獲得患者、臨床醫生、醫療機構和主管機關的信任和接受。本研究描述了 FUTURE-AI 指南，這是第一個指導醫療保健中可信賴 AI 工具開發和部署的國際共識架構。FUTURE-AI 聯盟成立於 2021 年，目前由來自 51 個國家/地區的 118 位跨領域專家組成，代表了所有大陸，包括 AI 科學家、臨床醫生、倫理學家和社會科學家。在兩年的時間裡，該聯盟通過一個反覆運算的過程定義了可信賴 AI 的指導原則和最佳實務，包括深入的文獻回顧、修改後的德爾菲調查和線上共識會議。FUTURE-AI 架構建立在醫療保健中可信賴 AI 的 6 項指導原則之上，即公平性、普遍性、可追溯性、可用性、穩健性和可解釋性。通過共識，定義了一組 28 項最佳實務，涵蓋技術、臨床、法律和社會倫理層面。這些建議涵蓋了醫療 AI 的整個生命週期，從設計、開發和驗證到法規、部署和監控。FUTURE-AI 是一個基於風險、無假設的指南，它提供了一個結構化的方法來建構醫療 AI 工具，這些工具將在現實世界的實務中受到信任、部署和採用。鼓勵研究人員在概念驗證階段考慮這些建議，以促進未來將醫療 AI 轉化為臨床實務。
...
摘要：人工智慧在醫療領域已取得顯著進展，在醫學影像、病人照護和其他領域中出現了新興應用。雖然這些應用在回顧性研究中已證實成功，但實際應用者卻寥寥可數。醫療 AI 領域面臨各種挑戰，包括建立使用者信任、遵守法規、合乎倫理地使用資料。可解釋 AI (XAI) 的目標是讓人類了解 AI 並相信其結果。本文根據近年發表的 198 篇文章的代表性樣本，針對醫療決策支援的 XAI 解決方案的最新發展進行文獻探討。對相關文章的系統性綜合整理得出以下幾項發現：(1) 這些解決方案大多採用與模型無關的 XAI 技術，(2) 深度學習模型的使用多於其他類型的機器學習模型，(3) 可解釋性被用來促進信任，但很少有研究報告醫師參與迴圈，(4) 視覺化且互動式的使用者介面對於理解系統的解釋和建議較有幫助。需要醫療和 AI 專家合作進行更多研究，以指導設計、實作和評估醫療中 XAI 解決方案的適當架構。
...
摘要：體外受精是治療不孕症最廣泛的療法之一。其中一個主要挑戰是評估和選擇胚胎進行植入，這個過程具有很大的臨床醫生間和臨床醫生內的變異性。基於深度學習的方法正受到關注，但它們的不透明本質會影響它們在臨床環境中的接受度，而透明度在決策制定中至關重要。在本文中，我們分析了 AI 輔助胚胎分析模型的可解釋性的現有工作，並找出其局限性。我們還討論了如何將這些模型整合到臨床環境中作為決策支援系統，同時考慮臨床醫生和患者的需求。最後，我們提出了一些準則，以提高可解釋性和可信度，推進這項技術朝向既定的臨床實務。
...
摘要：在需求工程 (RE) 領域中，可解釋人工智慧 (XAI) 在將 AI 支援系統與使用者需求、社會期望和法規標準相符的重要性日益增加，已獲得認可。一般而言，可解釋性已成為影響系統品質的重要非功能性需求。然而，可解釋性和效能之間的假設取捨挑戰了可解釋性的假設正面影響。如果滿足可解釋性的需求需要降低系統效能，則必須仔細考慮這些品質面向中哪一個優先，以及如何在它們之間取得折衷。在本文中，我們批判性地檢視了假設的取捨。我們主張以細緻的方式來處理，其中納入資源可用性、領域特性和風險考量。透過提供未來研究和最佳實務的基礎，這項工作旨在推動 AI 的 RE 領域。
...
摘要：在需求工程（RE）领域，可解释人工智能（XAI）在将人工智能支持的系统与用户需求、社会期望和监管标准相结合方面的日益重要性已经得到认可。一般来说，可解释性已经成为一项重要的非功能性需求，它会影响系统质量。然而，可解释性和性能之间的所谓权衡挑战了可解释性的假定积极影响。如果满足可解释性的需求需要降低系统性能，那么必须仔细考虑这些质量方面中的哪一个优先，以及如何在它们之间进行权衡。在本文中，我们批判性地考察了所谓的权衡。我们认为，最好以一种细致入微的方式来处理它，这种方式结合了资源可用性、领域特征和风险考虑。通过为未来的研究和最佳实践提供基础，这项工作旨在推进人工智能的 RE 领域。
...
摘要：本文批判性地評估了歐盟委員會提出的 AI 法案中，對於高風險 AI 系統的風險管理和風險可接受性的方法，這些系統對基本權利和安全構成風險。該法案旨在以相稱的監管負擔促進「可信賴」的 AI。其關於風險可接受性的條款要求將高風險系統的殘餘風險「盡可能」降低或消除，同時考慮「技術現狀」。此準則，尤其是如果解釋狹隘，則無法執行，既不能促進相稱的監管負擔，也不能促進可信賴性。相比之下，議會對風險管理條款的最新修正草案引入了「合理性」、成本效益分析，並且更透明地說明了風險可接受性判斷的價值觀和背景性質。本文認為議會的方法更可行，並且更好地平衡了相稱性和可信賴性的目標。它解釋了風險可接受性判斷中的合理性將需要什麼，並借鑒了來自過失法和歐洲醫療器械法規的原則。它主張風險可接受性判斷的方法需要穩固的公民合法性基礎：包括監管機構的詳細指導或參與，以及受影響利益相關者的有意義投入。
...
摘要：可解釋人工智慧 (XAI) 是機器學習快速進展的領域，旨在解開複雜模型的預測。XAI 在敏感應用中特別需要，例如在醫療保健中，當診斷、建議和治療選擇可能依賴於人工智慧系統做出的決策時。人工智慧方法也已廣泛用於老化研究中，特別是在開發生物時鐘模型和識別老化和與年齡相關疾病的生物標誌物方面。然而，XAI 在此處的潛力有待充分發揮。我們討論了 XAI 在開發「老化時鐘」中的應用，並根據對特定生理系統的關注，對文獻進行了全面的分析。
...
摘要：部分原型模型是可解釋的設計圖像分類器，並且是黑箱 AI 的有希望的替代方案。本文探討了解釋性機器學習的適用性和潛力，特別是 PIP-Net，用於對真實世界醫學影像數據進行自動化診斷支援。PIP-Net 學習人類可以理解的原型圖像部分，我們評估其準確性和可解釋性，用於骨折檢測和皮膚癌診斷。我們發現 PIP-Net 的決策制定過程符合醫學分類標準，同時僅提供圖像級別的類別標籤。由於 PIP-Net 對原型的無監督預訓練，因此可以輕鬆識別數據質量問題，例如 X 光中的不需要文字或標籤錯誤。此外，我們首次展示人類可以通過直接禁用不需要的原型來手動糾正 PIP-Net 的推理。我們得出結論，部分原型模型由於其可解釋性和高級模型除錯的潛力，對醫學應用很有前景。
...
摘要：可解釋 AI (XAI) 是機器學習研究中日益重要的領域，旨在讓黑盒模型透明且可解釋。在本文中，我們提出了一種新的 XAI 方法，它使用由條件特徵置換生成的所謂反事實路徑。該演算法透過識別最影響模型預測變化的特徵順序置換來衡量特徵重要性。它特別適合根據包含領域知識的知識圖譜中的反事實路徑來產生解釋。反事實路徑在解釋和視覺化黑盒模型時，為當前的 XAI 方法引入了額外的圖形維度。使用合成和醫療資料進行的實驗證明了我們方法的實際適用性。
...
摘要：人工智能（AI）的可解釋性領域見證了越來越多的研究和學術興趣。然而，在解釋機器學習演算法結果時缺乏人性化和個人化的詮釋，這顯著地阻礙了臨床醫生在研究和臨床實務中接受這些方法。為了解決這個問題，我們的研究使用反事實解釋來探討「如果？」情境在醫學研究中的適用性。我們的目標是擴展我們對用於診斷小兒後顱窩腦瘤的磁共振造影（MRI）特徵的理解，超越現有的界線。在我們的案例研究中，所提出的概念提供了一個新穎的方法來檢查替代決策情境，這些情境提供個性化且特定於情境的見解，從而驗證預測並釐清在不同情況下的變異。此外，我們探討了反事實資料擴充的潛在用途，並評估它們作為我們醫學研究案例中替代方法的可行性。結果證明了使用反事實解釋來增強臨床研究中 AI 驅動方法的接受度的潛力。
...
摘要：人工智能領域的進展，導致開發出各種類型的人工智慧驅動的失智症評估，可用於識別早期失智症患者。它可以徹底改變失智症的照護環境。醫學界必須了解各種人工智能評估，並根據其在失智症患者（PwD）早期識別方面的有效性、效率、實用性、可靠性和準確性來選擇它們。另一方面，人工智能開發人員也應了解各種非人工智能評估以及最近開發的人工智能評估。因此，這篇臨床醫生和人工智能工程師都可以閱讀的論文，填補了文獻中在向臨床醫生解釋現有的失智症識別解決方案，以及向人工智能工程師解釋所使用的技術和最廣泛的失智症資料集方面的空白。它遵循對人工智能和非人工智能失智症評估論文的回顧，提供有關各種失智症評估的寶貴資訊，供人工智能和醫學界參考。討論和結論重點介紹最顯著的研究方向和現有解決方案的成熟度。
...
摘要：可解释性对人工智能 (AI) 技术构成了重大挑战。当前关于可解释 AI (XAI) 的研究缺乏提取有关学习任务的全局知识的效率，因此存在不精确的显着性、缺乏上下文感知和含义模糊等缺陷。在本文中，我们提出了类别关联嵌入 (CAE) 方法来解决这些问题。我们采用编码器-解码器架构来嵌入样本特征，并同时将它们分离为与类别相关的和与个体相关的样式向量。将给定样本的个体样式代码与另一个样本的类别样式代码重新组合，会生成一个合成样本，该样本保留了个体特征，但改变了类别分配，遵循循环对抗学习策略。类别关联嵌入将所有实例的全局类别相关特征提取到一个统一的域中，并在类别之间进行良好的分离。然后可以提取不同类别之间的转换规则，并进一步应用于单个实例。然后，我们提出了一个主动 XAI 框架，该框架沿引导路径操纵特定样本的类别样式向量朝向反类别，从而产生一系列具有相同个体特征的反例合成样本。将这些反事实样本与原始样本进行比较，可以直观地全局说明分类任务的性质。我们在医学图像分类任务中采用了该框架，结果表明，与现有方法相比，可以实现具有强大的上下文感知表示的更精确的显着性图。此外，可以通过遍历类别样式空间中的路径直接可视化疾病病理。
...
摘要：提供基於機器學習的 AI 預測的高品質解釋是一項具有挑戰性且複雜的任務。要順利進行，需要具備以下條件：選擇適當的解釋普遍性/特殊性層次；考慮受益者對所考慮的 AI 任務的熟悉程度的假設；參照促成決策的特定元素；利用額外的知識（例如專家證據），這些知識可能不屬於預測流程的一部分；以及提供支持否定假設的證據。最後，系統需要以清晰可解釋且可能令人信服的方式制定解釋。考量到這些因素，ANTIDOTE 促成了可解釋 AI 的整合願景，其中深度學習流程的低層級特徵與人類論證能力的高層級架構相結合。ANTIDOTE 將利用深度學習和論證中的跨學科能力，支持對可解釋 AI 進行更廣泛且創新的觀點，其中對臨床案例審議的高品質解釋需求至關重要。作為該專案的第一個成果，我們發布了 Antidote CasiMedicos 資料集，以促進對一般可解釋 AI 以及特別是醫療領域論證的研究。
...
摘要：近年来，图神经网络取得了迅速发展，在药物发现、医疗诊断和推荐系统方面取得了许多新进展。虽然这一进展意义重大，但许多网络都是“黑匣子”，对网络究竟学习了“什么”知之甚少。许多高风险应用，如药物发现，需要模型提供人类可理解的解释，以便用户能够识别错误并发现新知识。因此，可解释人工智能算法的开发对于我们充分利用人工智能的好处至关重要。
我们提出了一种用于 GNN 的可解释性算法，称为 eXplainable Insight (XInsight)，它使用 GFlowNets 生成模型解释的分布。由于 GFlowNets 以与奖励成正比的概率生成对象，因此与仅学习最大奖励样本的先前方法相比，XInsight 可以生成多种解释。我们通过生成针对在两个图分类任务上训练的 GNN 的解释来演示 XInsight：使用 MUTAG 数据集对诱变化合物进行分类，并使用我们已开源的合成数据集对无环图进行分类。我们通过使用 QSAR 建模分析生成的化合物来展示 XInsight 解释的效用，我们发现 XInsight 生成的化合物按亲脂性（诱变性的已知相关性）聚类。我们的结果表明，XInsight 生成了一个解释分布，揭示了模型所证明的潜在关系。它们还强调了生成多种解释的重要性，因为它使我们能够发现模型中的隐藏关系，并为进一步分析提供有价值的指导。
...
摘要：我們提出並實作一個可解釋機器學習分類模型，用於基於表達布林公式的可解釋 AI (XAI)。潛在應用包括信用評分和醫療狀況診斷。布林公式定義一個具有可調整複雜度（或可解釋性）的規則，根據該規則對輸入資料進行分類。此類公式可以包含任何可套用於一個或多個布林變數的運算子，因此與更嚴格的基於規則和基於樹的方法相比，可提供更高的表達能力。分類器使用原生局部最佳化技術進行訓練，有效地搜尋可行公式的空間。淺層規則可由快速的整數線性規劃 (ILP) 或二次無約束二元最佳化 (QUBO) 求解器確定，這些求解器可能由特殊用途硬體或量子裝置提供支援。我們結合了原生局部最佳化器的表達能力和效率，以及這些裝置的快速運作，方法是執行非局部移動，對完整布林公式的子樹進行最佳化。我們提供廣泛的數值基準測試結果，在眾所周知的公開資料集上採用多個基線。根據結果，我們發現原生局部規則分類器通常與其他分類器具有競爭力。加入非局部移動以較少的反覆運算達成類似的結果，因此使用專用或量子硬體可以透過快速提出非局部移動來加速。
...
摘要：針對全球心理健康問題的挑戰，我們提出一個基於邏輯神經網路 (LNN) 的神經符號人工智慧方法，用於診斷心理疾病。由於缺乏針對心理疾病的有效治療覆蓋範圍，因此需要一種可以協助治療師進行診斷的人工智慧解決方案。然而，目前的神經網路模型缺乏可解釋性，治療師可能不會信任它們。LNN 是一種遞迴神經網路架構，結合了神經網路的學習能力和基於經典邏輯的人工智慧的推理能力。所提出的系統使用來自臨床訪談的輸入謂詞來輸出心理疾病類別，並使用不同的謂詞剪枝技術來實現可擴充性和更高的分數。此外，我們提供了一個見解提取方法，以協助治療師進行診斷。所提出的系統解決了當前神經網路模型缺乏可解釋性的問題，並為心理疾病診斷提供了一個更值得信賴的解決方案。
...
摘要：隨著機器學習模型在醫療診斷中日益普及，可解釋性和透明度的需求變得至關重要。XAI 復興標誌著該領域的重大轉變，旨在重新定義醫療診斷模型的可解釋性。本文探討了可解釋 AI (XAI) 領域內的創新方法和方法論，這些方法和方法論正在革新醫療診斷模型的可解釋性。通過闡明基礎決策制定過程，XAI 技術使醫療保健專業人員能夠理解、信任並有效利用這些模型進行準確且可靠的醫療診斷。這篇評論重點介紹了 XAI 在醫療診斷方面的關鍵進展及其轉變醫療保健領域的潛力，最終改善患者的治療結果並促進對 AI 驅動的診斷系統的信任。
...
摘要：<paragraph>在以高度連接性和行動力為特徵的環境中，加上心血管疾病的激增，透過遠端監控心血管健康來減少醫療保健費用的必要性變得更加明顯。準確檢測和分類心律不整對於診斷患有心臟不規則的人至關重要。這項研究強調了在家庭環境中使用心電圖 (ECG) 測量值進行即時心律不整檢測的可行性。本論文提出心律不整檢測的新應用，利用尖端的 You-Only-Look-Once (YOLO)v8 演算法對單導程 ECG 訊號進行分類。我們引入了一個新的損失修改 YOLOv8 模型，在 MIT-BIH 心律不整資料集上進行微調，實現即時連續監控。獲得的結果證實了我們方法的有效性，該模型在 NVIDIA Tesla V100 上達到了 99.5% 的平均準確度和 0.992 mAP@50，以及 0.002 秒的快速檢測時間。我們的研究說明了即時心律不整檢測的潛力，使用戶能夠在舒適的家中視覺化地解釋模型輸出。此外，這項研究為擴展到能夠在醫療保健部門部署的即時可解釋 AI (XAI) 模型奠定了基礎，從而顯著推進了醫療保健解決方案的領域。</paragraph>
...
摘要：乳癌 (BC) 仍然是一個重大的健康威脅，目前沒有長期治癒的方法。早期發現至關重要，但乳房攝影的判讀受到高假陽性和假陰性的阻礙。隨著 BC 的發生率預計將超過肺癌，改善早期檢測方法至關重要。熱像儀使用高解析度紅外線相機，提供希望，特別是與人工智慧 (AI) 結合使用時。這項工作提出了一個基於注意力的卷積神經網路用於分割，在 BC 檢測和分類中提供了更高的速度和精度。該系統增強影像並執行可解釋的 AI 癌症分割。我們提出了一個基於Transformer注意力的卷積架構 (UNet) 用於故障識別，並使用梯度加權類激活映射 (Grad-CAM) 來分析 UNet 架構中偏見和弱點的區域，使用 IRT 影像。與現有的深度學習框架相比，我們提出的框架的優越性得到了證實。
...
摘要：憂鬱症是最普遍且嚴重的精神疾病，會造成嚴重的財務和社會後果。憂鬱症偵測是早期介入以減輕這些後果的關鍵。如此重大的決定本質上需要可解釋性。儘管一些憂鬱症偵測研究嘗試根據重要性評分或注意力權重來解釋決定，但這些解釋與基於憂鬱症狀的臨床憂鬱症診斷標準不一致。為了填補這個缺口，我們遵循計算設計科學範例來開發一種新穎的多尺度時間原型網路 (MSTPNet)。MSTPNet 創新地偵測和解釋憂鬱症狀以及它們持續多久。使用大型資料集進行的廣泛實證分析顯示，MSTPNet 以 0.851 的 F1 分數優於最先進的憂鬱症偵測方法。此結果還揭示了調查方法中未注意到的新症狀，例如分享對不同生活的欽佩。我們進一步進行使用者研究，以證明其在可解釋性方面優於基準。本研究為 IS 文獻貢獻了一個新穎的可解釋深度學習模型，用於社交媒體中的憂鬱症偵測。在實務中，我們提出的方法可以實作在社交媒體平台中，為偵測出的憂鬱症患者提供個人化的線上資源。
...
摘要：電子健康紀錄（EHR）是醫療保健中預想的人工智慧（AI）驅動轉型的基本數據來源。
然而，反映在 EHR 備註中的臨床偏見可能導致 AI 模型繼承並擴大這些偏見，從而造成健康的差異。
本研究調查了 EHR 備註中汙名化語言（SL）對使用 Transformer 為基礎的深度學習模型和可解釋 AI（XAI）技術進行死亡率預測的影響。
我們的研究結果表明，由臨床醫生撰寫的 SL 會對 AI 的表現產生負面影響，特別是對於黑人患者，強調 SL 是 AI 模型開發中種族差異的來源。
為了探索一種運作上有效的方法來減輕 SL 的影響，我們通過臨床醫生的協作網路研究了 SL 生成中的模式，發現核心臨床醫生對 AI 模型中的種族差異有更大的影響。
我們發現，刪除由核心臨床醫生撰寫的 SL 比消除所有 SL 更有效地減少偏見。
...
摘要：當代透過 AI 進行的自動化，包含了大量的幕後人力勞動，這通常既不可見，且薪資低落。由於不可見的勞動，包括標記和維護工作，是當代 AI 系統中不可或缺的一部分，因此讓使用者了解其角色仍然很重要。我們建議這可透過可解釋 AI (XAI) 設計來完成，特別是女性主義交叉性 XAI。我們提出源自女性主義交叉性研究的製圖方法，以描繪 AI 的系統觀點，並納入與不可見勞動相關的 AI 向度。
...
摘要：虛擬心理健康助理 (VMHA) 持續進步，以支援不堪重負的全球醫療保健系統，該系統每年有 6000 萬次初級保健就診和 600 萬次急診室 (ER) 就診。這些系統由臨床心理學家、精神科醫生和人工智慧 (AI) 研究人員為認知行為療法 (CBT) 建立。目前，VMHA 的作用是透過資訊提供情緒支持，較少著重於與患者進行反思性對話。需要更全面、安全且可解釋的方法來建立負責任的 VMHA，以提出後續問題或提供見多識廣的回應。這項調查提供了對心理健康中現有對話代理的系統性批判性回顧，然後深入探討了 VMHA 在脈絡知識、資料集及其在臨床決策支援中新興角色的改進。我們還提供了新的方向，以透過可解釋性、安全性以及整體可信度來豐富 VMHA 的使用者體驗。最後，我們提供了評估指標和實務考量，讓 VMHA 超越目前的文獻，在活躍的溝通中建立 VMHA 與患者之間的信任。
...
摘要：XAI 指的是建立 AI 應用程式的技術和方法，協助最終使用者詮釋 AI 模型的輸出和預測。在高風險決策情境中，黑箱 AI 應用程式，例如醫療領域，提高了透明度和可解釋性的需求，因為錯誤的預測可能會造成嚴重的後果。模型的可解釋性和可詮釋性對於 AI 模型在醫療實務中的成功部署至關重要。AI 應用程式的基礎推理需要對臨床醫生透明，才能獲得他們的信任。本文提供了醫療保健領域中 XAI 層面和挑戰的系統性回顧。本研究的主要目標是回顧各種 XAI 方法、其挑戰，以及相關的醫療保健機器學習模型。這些方法分為六類進行討論：以特徵為導向的方法、全域方法、概念模型、代理模型、局部像素化方法和以人為中心的方法。最重要的是，本文探討了 XAI 在醫療保健問題中的角色，以釐清其在安全關鍵應用程式中的必要性。本文旨在透過回顧相關實驗結果，建立對醫療保健領域中 XAI 相關應用程式的全面理解。為了促進未來研究填補研究差距，本文研究了 XAI 模型從不同觀點出發的重要性及其限制。
...
摘要：最先進的機器學習模型通常會學習到訓練資料中嵌入的虛假關聯性。這在將這些模型部署到高風險決策制定時會構成風險，例如皮膚癌偵測等醫療應用。為了解決這個問題，我們提出了 Reveal to Revise (R2R)，一個包含了整個可解釋人工智慧 (XAI) 生命週期的架構，讓從業人員能夠反覆地找出、減輕並（重新）評估虛假的模型行為，同時將人為互動降到最低。在第一步 (1) 中，R2R 透過找出歸因中的異常值或檢查模型所學習到的潛在概念來揭示模型的弱點。其次 (2)，偵測負責的偽像並在輸入資料中進行空間定位，然後利用它來 (3) 修正模型行為。具體來說，我們應用 RRR、CDEP 和 ClArC 的方法來進行模型修正，並 (4)（重新）評估模型的效能和對偽像的剩餘敏感性。我們使用兩個醫療基準資料集進行黑色素瘤偵測和骨齡估算，將我們的 R2R 架構應用到 VGG、ResNet 和 EfficientNet 架構，從而揭示並修正真實的資料集內在偽像，以及受控設定中的合成變異。為了完成 XAI 生命週期，我們展示了多重 R2R 迭代以減輕不同的偏差。程式碼可在 https://github.com/maxdreyer/Reveal2Revise 取得。
...
摘要：可解釋人工智慧 (XAI) 領域近年來有長足進展，但進展主要在於電腦視覺和自然語言處理。對於時間序列，其輸入通常無法解釋，僅有有限的研究探討 XAI。在這項工作中，我們提出一個虛擬檢驗層，將時間序列轉換為可解釋的表示，並允許透過局部 XAI 方法（例如層級相關性傳播 (LRP)）將相關性歸因傳播到此表示。藉此，我們將 XAI 方法家族的適用性擴展到輸入僅在轉換後才可解釋的領域（例如語音）。在此，我們專注於傅立葉轉換，其廣泛應用於時間序列和 LRP 的解釋，並將我們的方法稱為 DFT-LRP。我們展示了 DFT-LRP 在各種時間序列分類設定（例如音訊和電子健康記錄）中的實用性。我們展示 DFT-LRP 如何揭示在不同領域（例如時間 vs. 頻率域）中訓練的模型在分類策略上的差異，或有助於發現模型如何針對資料中的虛假關聯採取行動。
...
摘要：解釋深度學習模型預測的能力給予最終使用者，是利用人工智慧 (AI) 力量進行醫療決策過程的一項重要功能，通常被認為是不透明且難以理解的。在本文中，我們應用最先進的可解釋人工智慧 (XAI) 方法來解釋黑盒子 AI 模型在甲狀腺結節診斷應用中的預測。我們提出新的基於統計的 XAI 方法，即核密度估計和密度圖，來解釋未檢測到結節的情況。XAI 方法的效能被視為一種定性和定量的比較，作為改進資料品質和模型效能的回饋。最後，我們調查評估醫師和患者對 XAI 解釋模型在甲狀腺結節影像上決策的信任度。
...
摘要：醫療器材和人工智慧系統快速轉型醫療保健條款。同時，由於其性質，醫療器材中或作為醫療器材的人工智慧可能會遭受網路攻擊，導致病患安全和安全風險。本章節分為三部分。第一部分從設定場景開始，我們在其中說明網路安全在醫療保健中的角色。接著，我們簡要定義當我們談論被視為醫療器材本身或支援醫療器材的人工智慧時所指的是什麼。為了說明此類醫療器材造成的風險，我們提供三個範例：資料集中毒、社會工程以及資料或原始碼萃取。在第二部分，本文概述了歐盟相關法規架構，以確保人工智慧作為或在醫療器材中的網路安全（MDR、NIS 指令、網路安全法案、GDPR、AI 法案提案和 NIS 2 指令提案）。最後，本文的第三部分探討源自歐盟法規架構的潛在挑戰。特別是，我們展望源自兩個立法提案的挑戰及其與現行有關人工智慧醫療器材網路安全的立法互動。它們被架構為對下列問題的解答：(1) 人工智慧法案將如何與 MDR 互動，關於網路安全和安全需求？(2) 我們應該如何詮釋 NIS 2 指令提案和 MDR 的事件通知需求？(3) 重大基礎設施演進條款的後果是什麼？[這是一個草稿章節。最終版本將收錄在 Barry Solaiman 和 I. Glenn Cohen 編輯的《健康、人工智慧和法律研究手冊》中，預計於 2023 年由 Edward Elgar Publishing Ltd 出版]
...
摘要：阿茲海默症 (AD) 是一種進行性神經退化疾病，也是失智症的主要原因。早期診斷對於患者受益於潛在的干預和治療至關重要。由於視網膜與大腦的解剖連接，因此假設視網膜是 AD 檢測的診斷部位。為此目的開發的 AI 模型尚未對決策提供合理的解釋，也沒有推斷出疾病進展的階段。順著這個方向，我們提出了一個新的與模型無關的可解釋 AI 框架，稱為顆粒神經元級解釋器 (LAVA)，這是一個解釋原型，它探測捲積神經網路 (CNN) 模型的中間層，以直接從視網膜影像評估 AD 連續體，而無需縱向或臨床評估。此方法用於驗證視網膜血管作為阿茲海默症 (AD) 評估的生物標記和診斷方式。英國生物銀行認知測試和血管形態特徵表明，LAVA 在識別整個進程連續體中的 AD 階段方面顯示出強大的前景和有效性。
...
摘要：許多視覺化已經為可解釋 AI (XAI) 而開發，但它們通常需要使用者進一步推理才能解釋。我們主張 XAI 應該支援圖解和演繹推理，以便 AI 執行假設產生和評估，以縮小可解釋性差距。我們提出圖表化以 i) 執行皮爾斯的演繹推理，ii) 遵循領域慣例，以及 iii) 以視覺或口頭方式用圖表說明。我們實作 DiagramNet 以進行臨床應用，從心臟聽診中預測心臟診斷，並以基於形狀的雜音圖表進行說明。在建模研究中，我們發現 DiagramNet 不僅提供忠實的雜音形狀說明，而且比基線模型有更好的預測效能。我們進一步在與醫學生的定性使用者研究中展示圖解說明的可解釋性和可信度，顯示臨床相關的圖解說明比技術顯著性圖說明更受青睞。這項工作有助於提供以使用者為中心的 XAI 的領域慣例演繹說明。
...
摘要：皮膚癌是人類最普遍的癌症類型之一。它主要是透過視覺辨識，從臨床篩檢開始，再進行皮膚鏡檢查、組織病理學評估和標本收集。深度卷積神經網路 (CNN) 會針對分類的細緻物件執行高度區隔且潛在通用的任務。本研究提出一個新穎的多類別預測架構，它根據 ViT 和 ViTGAN 對皮膚病灶進行分類。基於視覺轉換器的 GAN（生成對抗網路）用於解決類別不平衡問題。該架構包含四個主要階段：ViTGAN、影像處理和可解釋 AI。階段 1 包含產生合成影像以平衡資料集中所有類別。階段 2 包含套用不同的資料擴充技術和形態運算，以增加資料大小。階段 3 和 4 包含開發一個適用於邊緣運算系統的 ViT 模型，該模型可以識別模式並分類影像中使用者皮膚可見的皮膚病灶。在階段 3 中，在使用 ViT 將病灶分類到所需的類別後，我們將使用可解釋 AI (XAI)，它會產生更多可解釋的結果（使用啟動圖等），同時確保高預測準確度。皮膚疾病的即時影像可以用相機或行動應用程式由醫生或患者拍攝，以執行早期檢查並確定皮膚病灶的原因。整個架構與現有的皮膚病灶偵測架構進行比較。
...
摘要：<paragraph>對於人工智能 (AI) 在高風險環境（例如醫療保健）中的部署，提供可解釋性/可說明性或允許精細錯誤分析的方法至關重要。許多最近的可解釋性/可說明性和精細錯誤分析方法使用概念，這些概念是對人類具有語義意義的元標籤。然而，只有少數數據集包含概念級別的元標籤，而且這些元標籤大多與不需要領域專業知識的自然圖像相關。專注於與單一疾病（例如黑色素瘤）相關的元標籤的醫學密集註釋數據集。在皮膚科中，皮膚疾病使用既定的臨床詞彙來描述，允許臨床醫生彼此描述身體檢查結果。為了提供由領域專家密集註釋的醫療數據集，其中註釋可應用於多種疾病過程中，我們開發了 SkinCon：皮膚科醫生密集註釋的皮膚疾病數據集。SkinCon 包含來自 Fitzpatrick 17k 數據集的 3230 張圖像，這些圖像使用 48 個臨床概念進行了密集註釋，其中 22 個概念至少有 50 張圖像代表該概念。所使用的概念是由兩位皮膚科醫生考慮用於描述皮膚病變的臨床描述術語而選擇的。範例包括「斑塊」、「鱗屑」和「糜爛」。相同的概念也用於標記來自 Diverse Dermatology Images 數據集的 656 張皮膚疾病圖像，提供了一個具有多樣化膚色表示的額外外部數據集。我們回顧了 SkinCon 數據集的潛在應用，例如探測模型、基於概念的解釋和概念瓶頸。此外，我們使用 SkinCon 來展示其中兩個用例：使用概念調試現有皮膚科 AI 模型的錯誤，以及使用事後概念瓶頸模型開發可解釋模型。</paragraph>
...
摘要：不安靜的多臂強盜（RMAB）是一個流行的決策理論框架，已被用於模擬公共衛生、野生動物保育、通訊系統等領域的真實世界序列決策問題。已部署的 RMAB 系統通常分兩個階段運行：第一個階段預測定義 RMAB 實例的未知參數，第二個階段採用最佳化演算法來解決建構的 RMAB 實例。在這項工作中，我們提供了在公共衛生領域首次部署 RMAB 系統的結果，並對其進行分析，目的是改善孕產婦和兒童健康。我們的分析重點在於了解預測準確度與已部署 RMAB 系統的整體效能之間的關係。這對於確定投資於改善預測準確度以提升最終系統效能的價值至關重要，並且有助於診斷、監控已部署的 RMAB 系統。使用我們已部署 RMAB 系統的真實世界資料，我們證明整體預測準確度的提升甚至可能伴隨著 RMAB 系統效能的下降——投入大量資源以提升整體預測準確度可能無法獲得預期的結果。在此之後，我們開發了以決策為重點的評估指標來評估預測組成，並證明它更能（無論是經驗上還是理論上）解釋已部署 RMAB 系統的整體效能。
...
摘要：可解釋 AI 是在開發一種機制，讓決策系統和人類之間可以互動，方法是讓後者的決策可以理解。這在敏感的環境中特別重要，例如在醫療領域。我們提出一個使用案例研究，用於皮膚病變診斷，說明如何為從範例中訓練出的最先進深度神經網路分類器提供病變決策的說明。我們的架構包含一個訓練過的分類器，說明模組對其執行操作。後者能夠為分類診斷提供實例和反例，因此允許醫師與自動診斷系統互動。實例是透過對抗式自動編碼器產生的。我們說明系統在代表性範例中的行為。
...
摘要：人工智慧（AI）應用中的演算法公平性對於建立更美好的社會至關重要。公平性作為社會機制的基礎公理，包含多個面向。儘管機器學習（ML）社群著重於統計平權的交叉性，特別是在歧視問題上，但新興的文獻探討了另一個面向——單調性。根據領域專業知識，單調性在許多與公平性相關的領域中扮演著至關重要的角色，違反單調性可能會誤導人類決策並導致災難性的後果。在本文中，我們首先系統性地評估應用單調神經加法模型（MNAM）的重要性，該模型使用公平感知機器學習演算法來強制執行個人和成對單調性原則，以確保 AI 倫理和社會的公平性。我們透過理論推理、模擬和廣泛的實證分析的混合方法發現，在所有公平性領域（包括犯罪學、教育、醫療保健和金融）中，考量單調性公理至關重要。我們的研究有助於 AI 倫理、可解釋 AI（XAI）和人機互動（HCI）介面中的跨領域研究。透過證明不符合單調性的災難性後果，我們探討了 AI 應用中單調性需求的重要性。此外，我們證明 MNAM 是一種有效的公平感知機器學習方法，透過施加單調性限制來整合人類智慧。
...
摘要：<paragraph>在安全關鍵應用程式中，例如法律或醫療領域，解釋 AI 模型的預測至關重要。一種預測形式的解釋是萃取式依據，即一個實例功能的子集，引導模型對該實例做出預測。例如，子詞組「他偷了手機」可以作為「盜竊」預測的萃取式依據。先前關於產生萃取式依據的研究通常採用兩階段模型：一個選擇器，選擇最重要的功能（即依據），然後是一個預測器，僅根據所選功能進行預測。這些研究的一個缺點是，學習選擇功能的主要信號來自將預測器給出的答案與基本事實答案進行比較。在這項研究中，我們提議透過資訊校正方法從預測器中提取更多資訊。更精確地說，我們聯合訓練兩個模型：一個是典型的解決手邊任務的神經模型，以準確但黑盒的方式，另一個是選擇器預測器模型，它另外產生其預測的依據。第一個模型用作第二個模型的指南。我們使用對抗技術來校正兩個模型提取的資訊，使得它們之間的差異成為遺漏或過度選擇功能的指標。此外，對於自然語言任務，我們提出了一個基於語言模型的正則化器，以鼓勵提取流利的依據。情緒分析任務、仇恨言論識別任務以及法律領域三個任務的實驗結果顯示了我們在依據提取方面的方法的有效性。</paragraph>
...
摘要：近期醫療保健領域經過認證的人工智慧 (AI) 工具激增，使得採用這項技術的爭論再度浮上檯面。其中一項爭論的主題是可解釋 AI (XAI)，以及它承諾讓 AI 裝置更透明且更值得信賴。在醫療 AI 領域中，一些活躍的聲音表達了對可解釋 AI 技術的可靠性，特別是特徵歸因方法的疑慮，質疑它們在準則和標準中的使用和納入。儘管有這些合理的疑慮，我們主張，對事後局部可解釋性方法的可行性提出現有批評，等於是把洗澡水連同嬰兒一起倒掉，因為它將一個特定於影像資料的問題加以概化。我們從將問題描述為解釋與人類理解之間缺乏語義匹配開始。為了了解何時能可靠地使用特徵重要性，我們引入了低階特徵和高階特徵特徵重要性的區別。我們主張，對於低階特徵具備明確語義的資料類型，例如電子健康紀錄 (EHR) 等表格資料，可以獲得語義匹配，因此特徵歸因方法仍能以有意義且有用的方式使用。最後，我們概述一個程序來測試是否已達成語義匹配。
...
摘要：雖然人工智慧 (AI) 的機器學習 (ML) 模型在醫學領域中表現優異，但它們並非沒有錯誤。讓臨床醫生能夠找出錯誤的模型建議，對於建立對醫療 AI 的信任至關重要。可解釋 AI (XAI) 旨在透過釐清 AI 推理來滿足此需求，以支援最終使用者。最近，幾項關於生物醫學影像的研究獲得了有希望的結果。然而，使用表格資料的模型解決方案還不足以滿足臨床醫生的需求。本文提出了一種方法，以協助臨床醫生找出使用表格資料訓練的 ML 模型的失敗之處。我們的方法建立在三大支柱上：透過利用臨床背景潛在空間分解特徵集、評估整體解釋的臨床關聯性，以及基於潛在空間相似性 (LSS) 的局部解釋。我們在 ML 基於感染所致早產兒疾病的識別上展示了我們的做法。由於模型失敗而導致的死亡風險、終身殘疾和抗生素抗藥性是此領域中一個公開的研究問題。透過我們的做法，我們成功找出兩個模型的錯誤分類案例。我們的解決方案透過將局部解釋脈絡化，為臨床醫生提供可行的見解，以支持其自主做出明智的最終決定。
...
摘要：<paragraph>我們正見證著人工智能經濟和社會的興起，其中人工智能技術正日益影響著醫療保健、商業、交通運輸和日常生活中的許多方面。已經報導了許多人工智能系統甚至超越了人類專家準確性的成功案例。然而，人工智能系統可能會產生錯誤、可能表現出偏見、可能對數據中的雜訊敏感，而且通常缺乏技術和司法透明度，導致信任度降低和採用它們時面臨挑戰。這些最近的缺點和擔憂已在科學和一般媒體中得到記錄，例如自動駕駛汽車事故、醫療保健、招聘和有色人種面部識別系統中的偏見、看似正確的醫療決策後來發現是出於錯誤的原因等。這導致許多政府和監管倡議的出現，要求可信賴且合乎道德的人工智能提供準確性和穩健性、某種形式的可解釋性、人類控制和監督、消除偏見、司法透明度和安全性。提供可信賴的人工智能系統的挑戰激勵了對可解釋人工智能系統 (XAI) 的深入研究。XAI 的目的是提供人類可以理解的信息，說明人工智能系統如何做出決策。在本文中，我們首先簡要總結當前 XAI 工作，然後挑戰最近關於準確性與可解釋性相互排斥且僅專注於深度學習的論點。然後，我們提出我們的建議，以在高風險可信賴人工智能系統交付的整個生命週期中使用 XAI，例如開發、驗證和認證，以及可信賴的生產和維護。</paragraph>
...
摘要：深度神经網路 (DNN) 可偵測資料中的模式，並在許多電腦視覺應用中展現出多功能性和強勁效能。然而，單獨的 DNN 容易犯下違反簡單常識概念的明顯錯誤，且在使用明確知識來引導搜尋和決策制定方面具有限制。雖然整體 DNN 效能指標可能良好，但這些明顯的錯誤加上缺乏可解釋性，已阻礙其廣泛用於醫療影像分析等關鍵任務。本文的目的是介紹 SimpleMind，一個專注於醫療影像理解的認知 AI 開源軟體架構。它允許建立知識庫，以直觀的人類可讀形式描述影像物件的預期特徵和關係。SimpleMind 架構透過以下方式為 DNN 帶來思考能力：(1) 提供方法來推論影像內容的知識庫，例如空間推論和條件推理來檢查 DNN 輸出；(2) 以通用軟體代理的形式套用流程知識，將其串連在一起以完成影像前處理、DNN 預測和結果後處理，以及 (3) 執行所有知識庫參數的自動共同最佳化，以使代理適應特定問題。SimpleMind 能夠對多個偵測到的物件進行推理以確保一致性，並提供 DNN 輸出之間的交叉檢查。這種機器推理透過可解釋的模型和可解釋的決策來提升 DNN 的可靠性和可信度。提供的範例應用程式說明了 SimpleMind 如何透過將其嵌入認知 AI 架構中來支援和改善深度神經網路。
...
摘要：在過去十年，基於深度學習方法的視覺感知任務進步神速，在從自動駕駛到臨床決策支援系統等眾多應用領域中得到廣泛採用。儘管這些基於深度學習的模型效能令人印象深刻，但它們的決策過程仍然相當不透明，這使得它們在人類關鍵任務中的部署成為一項冒險的嘗試。這反過來又使得理解這些模型做出的決策對於它們的可靠部署至關重要。可解釋 AI (XAI) 方法試圖通過為這種黑盒深度學習方法提供解釋來解決這個問題。在本文中，我們對計算機視覺中的基於歸因的 XAI 方法進行了全面的調查，並回顧了現有的基於梯度的、基於擾動的和基於對比的 XAI 方法文獻，並對開發和評估穩健的 XAI 方法的關鍵挑戰提供了見解。
...
摘要：自 10 年代中期以來，深度學習 (DL) 的時代持續至今，
每年都帶來新的最高級和創新。儘管如此，
這些創新轉化為實際應用程式的速度落後於
這個快節奏。特別是安全關鍵應用程式，
依賴嚴格的法規和道德要求，需要小心處理，
仍然是爭論的活躍領域。可解釋 AI (XAI) 和
隱私保護機器學習 (PPML) 都是至關重要的研究領域，
旨在減輕 DL 中盛行的資料密集型黑盒模型的一些缺點。儘管
在各自的領域中進行了熱烈的研究活動，但
尚未關注它們的互動。這項工作是第一個調查
私人學習技術對基於 DL 的模型生成的解釋的影響。在
涵蓋多個領域的各種影像和時間序列
資料集以及不同的隱私技術、XAI
方法和模型架構的廣泛實驗分析中，研究了私人訓練對
生成的解釋的影響。研究結果表明，通過引入隱私，
解釋發生了不可忽視的變化。除了報告
PPML 對 XAI 的個別影響外，本文還對
實際應用中技術的選擇給出了明確的建議。通過揭示
這些關鍵技術的相互依賴性，這項工作是克服
安全關鍵領域中實際可應用 AI 的剩餘障礙的第一步。
...
摘要：肺結核（TB）是一種具有傳染性的細菌疾病，是造成死亡的重要原因，特別是在低收入國家，2020 年全球報告約有一千萬新病例。雖然肺結核可以治療，但未遵守藥物治療方案是造成發病率和死亡率的重要原因。因此，主動找出有中斷藥物治療風險的患者，可以採取矯正措施來減輕不良後果。我們使用極端未遵守治療的替代衡量指標，以及來自印度四個邦近 70 萬名患者的資料集，根據自訂的基於排名指標，制定並解決機器學習 (ML) 問題，以早期預測未遵守治療。我們訓練機器學習模型並根據基準進行評估，在考量全國大規模未來部署的情況下，相較於基於規則的基準提升約 100%，相較於隨機分類器提升約 214%。我們在過程中處理各種問題，包括資料品質、高基數類別資料、低目標流行率、分佈轉移、各群組間的差異、演算法公平性，以及對穩健性和可解釋性的需求。我們的研究結果表明，未遵守治療患者的風險分層是一種可行的、可大規模部署的機器學習解決方案。作為印度中央肺結核部門的官方人工智慧合作夥伴，我們正在與多個城市和州級試點計畫合作，目標是全印度部署。
...
摘要：可解釋人工智慧 (XAI) 藉由提升終端用戶對機器信任度，轉變了人工智慧 (AI) 領域。隨著連網裝置數量持續增加，物聯網 (IoT) 市場需要獲得終端用戶的信任。然而，現有文獻仍缺乏關於 XAI 在物聯網中使用的系統化且全面的調查工作。為了彌補這個不足，我們在這篇論文中探討 XAI 架構，重點在於它們的特性和對物聯網的支援。我們說明了廣泛使用的 XAI 服務，以用於物聯網應用程式，例如安全性增強、醫療物聯網 (IoMT)、工業物聯網 (IIoT) 和城市物聯網 (IoCT)。我們也建議在這些應用程式中，透過適當的範例在物聯網系統上實作 XAI 模型，並總結出對未來工作的關鍵推論。此外，我們提出了邊緣 XAI 結構的尖端發展，以及對物聯網應用程式的第六代 (6G) 通訊服務的支援，連同關鍵推論。簡而言之，本文構成了針對未來物聯網使用案例需求量身打造的 XAI 基礎架構發展的第一個整體彙編。
...
摘要：美國每年約有 125 萬人因燒傷接受治療。精確的燒傷分類是醫療 AI 領域的重要面向。在這項工作中，我們提出一個可解釋的人在迴路架構，以改善燒傷超音波分類模型。我們的架構利用基於 LIME 分類說明器的說明系統來驗證和整合燒傷專家的知識，提出新功能並確保模型的有效性。使用這個架構，我們發現 B 模式超音波分類器可以透過提供紋理特徵來增強。更具體地說，我們確認基於超音波影像的灰階共生矩陣 (GLCM) 的紋理特徵可以提高深度學習燒傷深度分類器的準確性。我們在豬隻受試者的真實資料上測試我們的假設。我們展示了燒傷深度分類準確性的改善，從約 88% 提高到約 94%，一旦按照我們的架構進行修改。
...
摘要：在醫療保健中，AI 系統支援臨床醫生和患者進行診斷、治療和監控，但許多系統的解釋性差，對於實際應用來說仍然具有挑戰性。克服這個障礙是可解釋 AI (XAI) 的目標。然而，解釋可以被不同地理解，因此無法為所有人解決黑箱問題。以人為中心的 AI 領域通過將 AI 適應使用者來解決這個問題。我們提出一個以使用者為中心的假想人物概念來評估 XAI，並使用它來調查最終使用者在行動健康壓力監控應用程式中對各種解釋風格和內容的偏好。我們的線上調查結果顯示，使用者的年齡層和個性，以及解釋類型，都會影響解釋偏好，這表示這些是 XAI 設計的基本特徵。我們將結果歸納到三個典型使用者假想人物：權力型、休閒型和注重隱私的使用者。我們的見解讓互動式、以人為中心的 XAI 更接近實際應用。
...
摘要：青光眼是造成部分或完全失明的第二大原因，在所有視覺障礙中，主要是因為焦慮或憂鬱造成的過度眼壓，損害視神經並造成視力問題。傳統的青光眼篩檢是一個耗時的過程，需要醫療專業人員持續的關注，即使如此，由於時間限制和壓力，他們仍時常無法正確分類，導致錯誤的治療。已經有許多努力嘗試自動化整個青光眼分類程序，然而，這些現有的模型通常具有黑盒特性，使用戶無法了解預測背後的主要原因，因此醫療從業人員通常無法依賴這些系統。在本文中，我們比較了各種預訓練模型後，提出了一個轉移學習模型，能夠以 94.71% 的準確率對青光眼進行分類。此外，我們利用了局部可解釋模型不可知解釋（LIME），在我們的系統中引入了可解釋性。這項改進使醫療專業人員能夠獲得重要且全面的資訊，幫助他們做出判斷。它也降低了傳統深度學習模型的不透明性和脆弱性。
...
摘要：深度學習在偵測物件和分類影像方面展現了極佳的效能，確保了分析醫學影像的極大前景。
將深度學習的成功轉化為醫學影像，其中醫生需要了解底層的過程，需要有解釋和說明神經網路預測的能力。深度神經網路的可解釋性通常依賴於估計輸入特徵（例如，像素）對於結果（例如，類別機率）的重要性。然而，已經開發出許多重要性估計器（也稱為顯著性圖），並且不清楚哪些對於醫學影像應用更相關。在目前的工作中，我們使用三個不同的評估指標，探討了幾個重要性估計器在解釋卷積深度網路對電腦斷層掃描 (CT) 影像分類的效能。首先，以模型為中心的保真度衡量在某些輸入受到擾動時模型準確度的下降。其次，使用接收器操作特性 (ROC) 曲線在像素層級衡量重要性分數和專家定義的分割遮罩之間的一致性。第三，我們使用 Dice 相似性係數 (DSC) 衡量基於 XRAI 的地圖和分割遮罩之間的區域重疊。整體而言，兩個版本的 SmoothGrad 在保真度和 ROC 排名中名列前茅，而積分梯度和 SmoothGrad 在 DSC 評估中表現出色。有趣的是，以模型為中心的（保真度）和以人為中心的（ROC 和 DSC）評估之間存在關鍵差異。分割地圖中嵌入的專家期望和直覺不一定與模型如何得出其預測相符。了解可解釋性的這種差異將有助於利用深度學習在醫學上的力量。
...
摘要：深度學習方法，特別是卷積神經網路，已成為醫學影像運算任務中強大的工具。儘管這些複雜的模型提供了極佳的效能，但它們的黑盒性質可能會阻礙在高風險決策制定中實際採用。在本文中，我們提出一個互動系統，利用最先進的可解釋性技術來協助放射科醫師進行乳癌篩檢。我們的系統將深度學習模型整合到放射科醫師的工作流程中，並提供新穎的互動方式，以促進對模型決策過程的理解。此外，我們證明我們的系統可以逐漸利用使用者互動，以提供更細緻的可解釋性報告，而標記負擔很小。由於採用的可解釋性技術的通用性質，我們的系統與領域無關，可用於許多不同的醫學影像運算任務，對我們如何利用視覺分析將原本靜態的可解釋性技術轉變為擴增人類決策制定和促進採用醫學 AI，提出了一個新觀點。
...
摘要：如今，人工智能 (AI) 已成為醫療保健應用程式中的一項基本組成部分，無論是臨床還是遠程，但效能最佳的 AI 系統通常過於複雜，無法自行解釋。可解釋 AI (XAI) 技術被定義為揭示系統預測和決策背後的推理，在處理敏感和個人健康資料時，它們變得更加關鍵。值得注意的是，XAI 在不同的研究領域和資料類型中並未獲得相同的關注，尤其是在醫療保健方面。特別是，許多臨床和遠程健康應用程式分別基於表格和時間序列資料，而 XAI 通常不會在這些資料類型上進行分析，而電腦視覺和自然語言處理 (NLP) 則為參考應用程式。為了概述最適合醫療保健領域中表格和時間序列資料的 XAI 方法，本文回顧了過去 5 年的文獻，說明了所產生的解釋類型以及為評估其相關性和品質所做的努力。具體來說，我們將臨床驗證、一致性評估、客觀和標準化品質評估以及以人為中心的品質評估確定為確保最終使用者獲得有效解釋的關鍵特徵。最後，我們重點說明了該領域的主要研究挑戰以及現有 XAI 方法的限制。
...
摘要：<paragraph>目標：本研究評估了一種市售可解釋 AI 演算法，用於增強臨床醫生在胸部 X 光片 (CXR) 上辨識肺癌的能力。
設計：這項回溯性研究評估了 11 位臨床醫生在有和沒有市售 AI 演算法（紅點，Behold.ai）的協助下，從胸部 X 光片中偵測肺癌的表現，該演算法會預測 CXR 中疑似肺癌。臨床醫生的表現根據臨床確診結果進行評估。
背景：該研究分析了來自 NHS 醫院的匿名患者資料；該資料集包含 400 張成人患者（18 歲及以上）的胸部 X 光片，這些患者於 2020 年進行了 CXR，並附有對應的臨床文字報告。
參與者：一個由 11 位臨床醫生（顧問放射科醫師、放射科醫師受訓人員和報告放射技師）組成的讀者小組參與了這項研究。
主要結果指標：臨床醫生在有和沒有 AI 輸入的情況下，在 CXR 上偵測肺癌的整體準確度、敏感度、特異度和精確度。臨床醫生之間的一致率和效能標準差也在有和沒有 AI 輸入的情況下進行評估。
結果：臨床醫生使用 AI 演算法導致肺腫瘤偵測的整體表現提升，整體增加了 17.4% 的肺癌在 CXR 上被辨識出來，而這些肺癌原本可能會被遺漏，整體上增加了較小腫瘤的偵測率，分別增加了 24% 和 13% 的第一期和第二期肺癌偵測率，並標準化了臨床醫生的表現。
結論：本研究顯示 AI 演算法在改善早期肺癌診斷和透過整體提升讀者表現來促進健康公平方面，具有極大的臨床應用前景，同時不會影響下游影像資源。</paragraph>
...
摘要：缺乏足够大的开放式医疗数据库是 AI 驱动的医疗保健中最大的挑战之一。使用生成对抗网络 (GAN) 创建的合成数据似乎是缓解隐私政策问题的一个好解决方案。另一种治疗方法是在多个医疗机构之间进行分散式协议，而无需交换本地数据样本。在本文中，我们探索了集中式和分散式设置中的无条件和条件 GAN。集中式设置模仿了对大但高度不平衡的皮肤病变数据集的研究，而分散式设置模拟了三个机构之间更现实的医院场景。我们根据保真度、多样性、训练速度以及在生成合成数据上训练的分类器的预测能力评估了模型的性能。此外，我们通过探索潜在空间和嵌入投影提供了可解释性，重点关注全局和局部解释。计算出的真实图像与其在潜在空间中的投影之间的距离证明了训练过的 GAN 的真实性和泛化性，这是此类应用中的主要关注点之一。进行的研究的开源代码可在以下位置公开获得：\url{https://github.com/aidotse/stylegan2-ada-pytorch}。
...
摘要：在醫院世界中，有許多複雜的組合問題，解決這些問題對於增加患者滿意度和提供照護品質非常重要。醫療保健中的問題很複雜，因為解決它們需要考慮到許多限制和不同類型的資源。此外，必須在短時間內評估解決方案，以確保在實際情況中可用。我們計畫提出解決這類問題的方案，既擴充已測試的解決方案，也透過建模解決新問題，並在可行時考慮文獻和使用真實資料。解決這類問題很重要，但由於歐盟委員會制定了《一般資料保護條例》，規定每個人都有權要求解釋 AI 做出的決定，因此在未開發可解釋性方法之前，基於 AI 的求解器（例如基於 Answer Set 程式設計的求解器）的使用將受到限制。因此，研究的另一部分將致力於研究並提出新的方法來解釋獲得的解決方案。
...
摘要：在現代動態持續發展的社會中，越來越多的人們飽受慢性疾病和嚴重疾病的折磨，醫生和患者需要特殊且精密的醫療和健康支援。因此，傑出的醫療利害關係人已經意識到發展此類服務的重要性，以讓患者的生活更輕鬆。此類支援需要收集大量複雜的患者資料，例如臨床、環境、營養、日常活動、來自智慧型穿戴式裝置的各種資料、來自配備感測器的服裝的資料等。整體患者資料必須適當地彙總、處理、分析，並提供給醫生和照護者，以建議適當的治療和行動，以改善患者的健康相關參數和整體健康。進階的人工智慧技術提供了分析此類大資料、使用它們，並衍生新知識以支援個人化醫療決策的機會。新的方法，例如那些基於進階機器學習、聯邦學習、遷移學習、可解釋人工智慧的方法，為未來更優質地使用健康和醫療資料開啟了新的道路。在本文中，我們將介紹一些關鍵面向和特徵範例，說明各種人工智慧方法在個人化醫療決策中的應用領域。
...
摘要：<paragraph>持續監控訓練過的 ML 模型以確定何時應該和不應該信任其預測，對於其安全部署至關重要。這種架構應具有高性能、可解釋性、事後性和可操作性。我們提出 TRUST-LAPSE，一個用於持續模型監控的「不信任」評分架構。我們使用一系列潛在空間嵌入來評估每個輸入樣本的模型預測的可信度。具體來說，(a) 我們的潛在空間不信任分數使用潛在空間中的距離度量（馬氏距離）和相似度量（餘弦相似度）來估計不信任，(b) 我們的順序不信任分數確定過去輸入表示序列中相關性的偏差，用於可操作的持續監控的非參數、滑動窗口演算法。我們通過兩個下游任務評估 TRUST-LAPSE：(1) 分布式偏移輸入檢測，以及 (2) 資料漂移檢測。我們使用公共資料集跨不同領域（音訊和視覺）進行評估，並進一步在具有挑戰性的真實世界腦電圖 (EEG) 資料集上對我們的做法進行基準測試，以進行癲癇檢測。我們的潛在空間不信任分數取得了最先進的結果，AUROC 為 84.1（視覺）、73.9（音訊）和 77.1（臨床 EEG），比基準高出 10 分以上。我們揭露了流行基準中的嚴重失敗，這些基準對輸入語義內容不敏感，使其不適合用於真實世界的模型監控。我們表明，我們的順序不信任分數達到了很高的漂移檢測率；對於所有領域，超過 90% 的串流顯示 < 20% 的錯誤。通過廣泛的定性和定量評估，我們表明我們的信任分數更強大，並提供可解釋性，以便輕鬆採用實務。</paragraph>
...
摘要：機器學習的普及增加了不公平模型在高風險應用程式中部署的風險，例如司法系統、藥物/疫苗設計和醫療診斷。儘管有從頭開始訓練公平模型的有效方法，但如何自動揭露和解釋訓練模型的不公平性仍然是一項具有挑戰性的任務。以可解釋的方式揭露機器學習模型的不公平性是邁向公平且值得信賴的人工智慧的關鍵一步。在本文中，我們系統性地解決了透過挖掘可解釋證據 (RUMIE) 來揭露不公平模型的新任務。其關鍵思想是找到一組由模型最常區分的資料實例，以作為證據。為了使證據可解釋，我們還找到了一組人類可以理解的關鍵屬性和決策規則，這些屬性和規則描述了被區分的資料實例，並將其與其他未被區分的資料區分開來。正如在許多真實世界資料集上進行的大量實驗所證明的那樣，我們的模型找到了高度可解釋且有力的證據來有效揭露訓練模型的不公平性。此外，它比所有基線模型都更具可擴充性。
...
摘要：可解釋人工智慧 (XAI) 在安全關鍵系統中是必要的，例如在臨床診斷中，因為致命決策的風險很高。然而，目前 XAI 就像是一堆鬆散的方法集合，而不是定義良好的流程。在這項工作中，我們闡述了 XAI 最大子群體可解釋機器學習 (IML) 與古典統計之間的概念相似性。基於這些相似性，我們提出了 IML 的形式化，類似於統計程序。採用這種統計觀點使我們能夠將機器學習模型和 IML 方法解釋為精密的統計工具。基於這種解釋，我們推論出三個關鍵問題，我們將其視為在安全關鍵設定中成功採用 IML 的關鍵。透過制定這些問題，我們進一步旨在引發關於 IML 與古典統計之間的區別，以及我們的觀點對該領域的未來有何影響的討論。
...
摘要：人工智慧已在各學科和領域中普及，而生物醫學影像和訊號處理也不例外。對此主題日益增長且廣泛的興趣引發了廣泛的研究活動，這反映在指數級的研究工作中。透過研究大量且多樣化的生物醫學資料，機器和深度學習模型徹底改變了各種任務，例如建模、分割、配準、分類和合成，其表現優於傳統技術。然而，難以將結果轉換為生物/臨床可解釋的資訊，這阻礙了它們在該領域的充分利用。可解釋人工智慧 (XAI) 試圖透過提供讓模型可解釋的方法和提供說明來填補這種轉譯差距。到目前為止，已提出不同的解決方案，並且越來越受到社群的關注。本文旨在提供生物醫學資料處理中 XAI 的概述，並指出 IEEE 訊號處理雜誌中即將推出的生物醫學影像和訊號處理深度學習特刊，該特刊將於 2022 年 3 月出刊。
...
摘要：在使用醫學影像進行診斷時，無論是臨床醫生或人工智慧 (AI) 系統，影像品質都很重要。當影像品質不佳時，產生該影像的醫療檢查通常需要重新進行。遠距醫療中常見的問題是，品質問題只會在病人離開診所後才被標示出來，這表示他們必須回去重新檢查。這對居住在偏遠地區的人來說特別困難，而這些人佔了 Portal Telemedicina 的病患相當大的比例，Portal Telemedicina 是巴西一家數位醫療保健組織。在這篇論文中，我們報告了關於 (i) 開發一個 AI 系統來標示和解釋低品質醫學影像的正在進行中的工作，(ii) 一項訪談研究，以了解在 OurCompany 使用 AI 系統的利害關係人對解釋的需求，以及 (iii) 一個縱向使用者研究設計，用來檢視在我們的診所中，加入解釋對技術人員工作流程的影響。據我們所知，這將是第一個評估 XAI 方法對最終使用者（使用 AI 系統但沒有 AI 專業知識的利害關係人）影響的縱向研究。我們歡迎對我們的實驗設置提供回饋和建議。
