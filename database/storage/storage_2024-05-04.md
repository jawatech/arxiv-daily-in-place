# arxiv-daily
 Automated deployment @ 2024-05-04 12:59:41 Asia/Taipei
...
摘要：專有的 LM，例如 GPT-4，通常用於評估各種 LM 的回應品質。然而，包含透明度、可控性和負擔能力在內的疑慮強烈促成了專精於評估的開源 LM 的發展。另一方面，現有的開放評估器 LM 呈現出嚴重的缺點：1) 它們發出的分數與人類分配的分數有顯著差異，以及 2) 它們缺乏執行直接評估和成對排名這兩種最普遍的評估形式的彈性。此外，它們不具備根據自訂評估標準進行評估的能力，而是專注於一般屬性，例如有益性和無害性。為了解決這些問題，我們引入了 Prometheus 2，一個比其前身更強大的評估器 LM，它緊密反映人類和 GPT-4 的判斷。此外，它有能力處理直接評估和成對排名格式，並與使用者定義的評估標準分組。在四個直接評估基準和四個成對排名基準上，Prometheus 2 在所有經過測試的開放評估器 LM 中，與人類和專有 LM 評審的分數相關性和一致性最高。我們的模型、程式碼和資料都公開於 https://github.com/prometheus-eval/prometheus-eval。
...
摘要：<paragraph>大型語言模型 (LLM) 已被證明能夠執行長期機器人任務的高階規劃，但現有方法需要存取預先定義的技能庫（例如，拾取、放置、拉動、推動、導航）。然而，LLM 規劃並未說明如何設計或學習這些行為，這仍然具有挑戰性，特別是在長期設定中。此外，對於許多感興趣的任務，機器人需要能夠以細微的方式調整其行為，這要求代理能夠修改低階控制動作。我們能否改用 LLM 的網際網路規模知識來制定高階政策，引導強化學習 (RL) 政策有效率地解決機器人控制任務，而無需預先設定的技能組？在本文中，我們提出 Plan-Seq-Learn (PSL)：一種模組化方法，它使用動作規劃來彌合抽象語言和學習的低階控制之間的差距，以從頭開始解決長期機器人任務。我們展示了 PSL 在超過 25 項具有挑戰性的機器人任務（最多有 10 個階段）上取得了最先進的結果。PSL 以超過 85% 的成功率解決了來自原始視覺輸入的長期任務，跨越四個基準，優於基於語言、經典和端到端的方法。影片結果和程式碼請見 https://mihdalal.github.io/planseqlearn/</paragraph>
...
摘要：概念瓶頸模型 (CBM) 將影像分類建立在人類可理解的概念上，以允許可解釋的模型決策。
至關重要的是，CBM 設計本質上允許人類介入，其中專家使用者有能力修改潛在錯誤對齊的概念選擇，以可解釋的方式影響模型的決策行為。
然而，現有方法通常需要每張影像進行大量的人工介入才能達到強大的效能，這在取得人工回饋很昂貴的情況下會造成實際挑戰。
在本文中，我們發現這顯著地受到介入期間概念的獨立處理所驅動，其中一個概念的改變並不會影響模型最終決策中其他概念的使用。
為了解決這個問題，我們引入了一個可訓練的概念介入重新對齊模組，它利用概念關係在介入後重新對齊概念指派。
在標準的真實世界基準中，我們發現概念重新對齊可以顯著改善介入效能；顯著減少達到目標分類效能或概念預測準確度所需的介入次數。
此外，它可以輕鬆整合到現有的基於概念的架構中，而不需要改變模型本身。
這種降低的人力模型協作成本對於提高 CBM 在資源受限環境中的可行性至關重要。
...
摘要：校準是一種標準程序，可微調預先訓練的大語言模型 (LLM)，以遵循自然語言指令並作為有用的 AI 助理。然而，我們觀察到，傳統的校準過程無法增強 LLM 的事實準確性，而且經常導致產生更多虛假事實（即幻覺）。在本文中，我們研究如何讓 LLM 校準過程更具事實依據，首先找出導致校準步驟中產生幻覺的因素：監督微調 (SFT) 和強化學習 (RL)。特別是，我們發現訓練 LLM 學習新知識或不熟悉的文本會助長幻覺。這使得 SFT 較不具事實依據，因為它訓練的是對 LLM 來說可能是新穎的人類標籤資料。此外，標準 RL 中使用的獎勵函數也可能助長幻覺，因為它引導 LLM 對各種指令提供更多有用的回應，通常偏好更長且更詳細的回應。根據這些觀察，我們提出具備事實意識的校準，它包含具備事實意識的 SFT 和透過直接偏好最佳化具備事實意識的 RL。實驗顯示，我們提出的具備事實意識的校準引導 LLM 輸出更多事實回應，同時維持遵循指令的能力。

##### **A separability-based approach to quantifying generalization: which layer is best?**
2405.01524v1 by Luciano Dyballa,Evan Gerritz,Steven W. Zucker
...
摘要：對於深度學習分類和基礎模型，對未見數據的概括仍然知之甚少。如何評估網路適應輸入空間的新版本或擴充版本的能力，以符合小樣本學習、分布外概括和領域適應的精神？網路的哪一層最有可能產生最佳概括？我們提供了一種新方法，用於評估網路表示取樣領域的能力，而不管網路是否已針對該領域中的所有類別進行訓練。我們的做法如下：在特定領域對視覺分類的最新預訓練模型進行微調後，我們評估它們在該領域中相關但不同的變異數據上的效能。概括能力被量化為來自中間層未見數據的潛在嵌入函數，適用於無監督和監督設定。在網路的所有階段中，我們發現 (i) 高分類準確度並不表示高概括性；(ii) 模型中的較深層並不總是產生最佳概括，這對修剪有影響。由於在所有資料集觀察到的趨勢在很大程度上是一致的，我們得出結論，我們的做法揭示了模型不同層的內在概括能力（函數）。
...
摘要：提出各種對齊語言模型的方法，包括監督微調、RLHF 和 DPO 等直接最佳化方法。
儘管 DPO 因其直接的訓練過程和競爭結果而迅速獲得歡迎，但仍有一個開放問題，即是否仍有使用判別器（例如獎勵模型）來評估回應的實際優勢。我們提出 D2PO，判別器引導的 DPO，一種在整個學習過程中收集偏好的線上設定方法。
當我們收集金牌偏好時，我們不僅用這些來訓練我們的政策，還用來訓練一個區分性回應評估模型，以對更多合成資料進行銀標籤，用於政策訓練。我們在各種任務中探索這種方法，包括逼真的聊天設定，我們發現與具有相同資料預算的 DPO 相比，我們的做法會產生更高品質的輸出，而且在偏好資料需求方面更有效率。此外，我們展示了銀標籤最有助益的條件：與 DPO 一起訓練政策時最有效，表現優於傳統 PPO，並受益於維持一個與政策模型分開的判別器。

##### **Analyzing the Role of Semantic Representations in the Era of Large Language Models**
2405.01502v1 by Zhijing Jin,Yuen Chen,Fernando Gonzalez,Jiarui Liu,Jiayi Zhang,Julian Michael,Bernhard Schölkopf,Mona Diab
...
摘要：傳統上，自然語言處理 (NLP) 模型通常使用由語言學專業知識建立的豐富特徵集，例如語義表示。然而，在大語言模型 (LLM) 的時代，越來越多的任務轉變為通用的端到端序列產生問題。在本文中，我們探討了這個問題：在 LLM 時代，語義表示扮演什麼角色？具體來說，我們探討了抽象意義表示 (AMR) 在五項不同的 NLP 任務中的效果。我們提出了一個 AMR 驅動的思考鏈提示方法，我們稱之為 AMRCoT，並發現它通常會損害效能，而不是幫助它。為了探討 AMR 在這些任務中可能提供的內容，我們進行了一系列的分析實驗。我們發現很難預測 AMR 可能在哪些輸入範例中提供幫助或造成傷害，但錯誤往往出現在多字詞表達式、命名實體以及 LLM 必須將其對 AMR 的推理與其預測聯繫起來的最終推論步驟中。我們建議在語義表示的未來工作中專注於這些領域，以供 LLM 使用。我們的程式碼：https://github.com/causalNLP/amr_llm。
...
摘要：儘管大多數關於可控文字生成的的研究都專注於引導基礎語言模型，但新興的指令調整和提示範例提供了可控性的另一種方法。我們編制並發布了 ConGenBench，一個由 17 項不同的可控生成任務組成的測試平台，並使用其中一部分來評量 9 個不同的基準和方法在指令調整語言模型上的效能。令我們驚訝的是，我們發現基於提示的方法在大多數的資料集和任務上都優於可控文字生成方法，這突顯了對特定指令調整語言模型中可控文字生成的研究需求。基於提示的方法在大多數風格任務上與人類表現相符，但在結構任務上則落後，這凸顯了研究更多元限制和更具挑戰性風格任務的需求。為了促進此類研究，我們提供了一種演算法，它僅使用任務資料集和具備情境能力的大語言模型來自動生成限制資料集。此方法消除了欄位對預先整理限制資料集的依賴性，因此大幅擴展了未來可以研究的限制範圍。
...
摘要：近年來，出現了許多大型多模態模型 (LMM)，可有效解決單一影像視覺語言任務。然而，它們解決多影像視覺語言任務的能力還有待改進。現有的多影像 LMM（例如 OpenFlamingo、Emu、Idefics 等）大多透過預先訓練數億筆來自網路的雜訊交錯影像文字資料來獲得多影像能力，這既不有效率也不有效。在本文中，我們旨在透過使用學術級資源進行指令微調來建立強大的多影像 LMM。因此，我們精心建構了 Mantis-Instruct，其中包含來自 14 個多影像資料集的 721K 個實例。我們設計 Mantis-Instruct 來涵蓋不同的多影像技能，例如共同引用、推理、比較、時間理解。我們將 Mantis-Instruct 與多個單一影像視覺語言資料集結合，以訓練我們的模型 Mantis 來處理任何交錯的影像文字輸入。我們在五個多影像基準和八個單一影像基準上評估訓練後的 Mantis。儘管只需要學術級資源（即 16xA100-40G 上的 36 小時），Mantis-8B 仍可在所有多影像基準上達到最先進的效能，並以平均 9 個絕對點數擊敗現有的最佳多影像 LMM Idefics2-8B。我們觀察到 Mantis 在保留和排除評估基準上的表現同樣出色。我們進一步在單一影像基準上評估 Mantis，並證明 Mantis 可以維持與 CogVLM 和 Emu2 相當的強勁單一影像效能。我們的結果特別令人鼓舞，因為它表明低成本指令微調在建構多影像 LMM 方面確實比密集預訓練有效得多。
...
摘要：將大型語言模型 (LLM) 與人類價值觀和偏好相結合對於讓它們變得有幫助且安全至關重要。然而，建立有效率的工具來執行校準可能會很有挑戰性，特別是對於通常包含數十億或數百億個參數的最大且最具能力的 LLM。我們建立了 NeMo-Aligner，這是一個模型校準工具包，可以有效地擴展到使用數百個 GPU 進行訓練。NeMo-Aligner 隨附針對模型校準的主要範例進行高度最佳化且可擴展的實作，例如：人類回饋強化學習 (RLHF)、直接偏好最佳化 (DPO)、SteerLM 和自玩微調 (SPIN)。此外，我們的工具包支援在參數有效微調 (PEFT) 設定中執行大部分校準技術。NeMo-Aligner 被設計為可擴充的，允許以最小的工作量支援其他校準技術。它採用 Apache 2.0  Lizenz 開源，我們歡迎社群在 https://github.com/NVIDIA/NeMo-Aligner 進行貢獻
...
摘要：大型視覺語言模型 (VLM) 已在需要精細理解文字影像和文字的任務中展現強大的推理能力，例如視覺問答或視覺蘊涵。然而，當這些模型面對含有隱喻或幽默等比喻現象的影像和標題時，其能力的探討卻很少，而這些現象的意義通常是隱含的。為了填補這個鴻溝，我們提出一個新任務和一個高品質的資料集：具有文字說明的視覺比喻語言理解 (V-FLUTE)。我們將視覺比喻語言理解問題構建為一個可解釋的視覺蘊涵任務，其中模型必須預測影像（前提）是否蘊涵一個主張（假設），並以文字說明為預測標籤提供依據。使用人類與人工智慧協作架構，我們建立了一個高品質的資料集 V-FLUTE，其中包含 6,027 個 <影像、主張、標籤、說明> 實例，涵蓋五種不同的多模態比喻現象：隱喻、明喻、慣用語、諷刺和幽默。比喻現象可以出現在影像、標題或兩者中。我們進一步進行自動和人工評估，以評估當前 VLM 在理解比喻現象方面的能力。
...
摘要：聊天機器人，例如 GPT-4 和 ChatGPT，現在服務數百萬用戶。儘管它們被廣泛使用，但仍缺乏公開的資料集，展示這些工具是如何被大量使用者實際使用的。為了彌補這個差距，我們免費提供 ChatGPT 給線上使用者，以換取他們肯定、同意自願匿名收集他們的聊天記錄和請求標頭。從中，我們編制了 WildChat，一個包含 100 萬個使用者與 ChatGPT 對話的語料庫，其中包含超過 250 萬個互動回合。我們將 WildChat 與其他流行的使用者聊天機器人互動資料集進行比較，發現我們的資料集提供了最多樣化的使用者提示，包含最多的語言，並為研究人員提供了最豐富的潛在有毒使用案例。除了帶有時間戳記的聊天記錄外，我們還使用人口統計資料豐富了資料集，包括州、國家和雜湊 IP 地址，以及請求標頭。這種擴充允許對不同地理區域和時間維度中的使用者行為進行更詳細的分析。最後，由於它涵蓋了廣泛的用例，我們展示了該資料集在微調遵循指令的模型中的潛在效用。WildChat 在 https://wildchat.allen.ai 下根據 AI2 ImpACT 許可證發布。
...
摘要：AI 基礎模型在各種應用領域中獲得關注，包括放射學等醫療領域。然而，醫療基礎模型通常在有限的任務上進行測試，其泛化性和偏差尚未得到探討。我們展示了 RayDINO，一個由 873k 胸部 X 光片上的自我監督訓練的大型視覺編碼器。我們將 RayDINO 與之前的最先進模型在九項放射學任務中進行比較，從分類和密集分割到文本生成，並深入分析我們模型的人口、年齡和性別偏差。我們的研究結果表明，自我監督允許以患者為中心的 AI 在臨床工作流程中發揮作用，並全面解釋 X 光片。通過 RayDINO 和小的特定任務適配器，我們達到了最先進的結果，並改進了對未見人群的泛化，同時減輕了偏差，說明了基礎模型的真正承諾：通用性和穩健性。
...
摘要：預先訓練好的對比式視覺語言模型已證明在各種任務中具有顯著的效能。然而，它們通常會在微調資料集上遇到困難，因為這些資料集的類別在預訓練期間並未得到充分的表示，這使得適應變得必要。最近的研究顯示，利用來自網路規模資料庫的樣本進行檢索增強適應，特別是在低資料量模式下，已展現出有前景的成果。儘管取得了經驗上的成功，但了解檢索如何影響視覺語言模型的適應仍然是一個開放的研究問題。在這項工作中，我們採用反思觀點，提出了一項系統性研究，以了解檢索增強適應中關鍵組成的作用。我們揭示了關於單模態和跨模態檢索的新見解，並強調了邏輯集合對有效適應的關鍵作用。我們進一步提出了理論基礎，直接支援我們的經驗觀察。
...
摘要：本文介紹 UQA，這是一個針對烏爾都語的全新問答和文本理解資料集，烏爾都語是一種擁有超過 7 千萬名母語使用者的低資源語言。UQA 是透過翻譯史丹佛問答資料集 (SQuAD2.0) 而產生，SQuAD2.0 是個大型的英文問答資料集，使用一種稱為 EATS（包圍錨定、翻譯、尋找）的技術，在翻譯後的背景段落中保留答案範圍。本文描述了在 Google 翻譯和 Seamless M4T 這兩個候選者中選擇和評估最佳翻譯模型的流程。本文也在 UQA 上對幾個最先進的多語言問答模型進行基準測試，包括 mBERT、XLM-RoBERTa 和 mT5，並報告了有希望的結果。對於 XLM-RoBERTa-XL，我們的 F1 分數為 85.99，EM 為 74.56。UQA 是用於開發和測試烏爾都語多語言 NLP 系統以及增強現有模型的跨語言可傳輸性的寶貴資源。此外，本文證明了 EATS 在為其他語言和領域建立高品質資料集方面的有效性。UQA 資料集和程式碼可在 www.github.com/sameearif/UQA 公開取得。
...
摘要：在本文中，我們討論了將計算創造力 (CC) 與大型語言和視覺模型 (LLVM) 研究整合的方法，以解決這些模型的一個關鍵限制，即創造性問題解決。我們展示了初步實驗，說明如何將 CC 原則應用於透過增強提示來解決此限制。透過這項工作，我們希望促進在 LLVMs 的創造性問題解決中，針對 ML 演算法的計算創造力進行討論。我們的程式碼位於：
https://github.com/lnairGT/creative-problem-solving-LLMs
...
摘要：這篇論文探討使用大型語言模型 (LLM) 來自動化硬體描述碼的生成，旨在探索其在支援和增強高效能神經形態運算架構開發中的潛力。在我們先前工作的基礎上，我們採用 OpenAI 的 ChatGPT4 和自然語言提示來合成可程式化遞迴尖峰神經網路的 RTL Verilog 模組，同時也產生測試平台來評估系統的正確性。結果設計在三個案例研究中得到驗證，分別是獨佔或、IRIS 花朵分類和 MNIST 手寫數字分類，準確率高達 96.6%。為了驗證其可合成性和可實作性，該設計在現場可程式閘陣列上製作原型，並使用開源電子設計自動化流程在 SkyWater 130 奈米技術上實作。此外，我們已將其提交給 Tiny Tapeout 6 晶片製造計畫，以進一步評估系統未來在晶片上的效能。
...
摘要：大型 2D 視覺語言模型 (2D-LLM) 透過使用簡單的投影機將大型語言模型 (LLM) 與圖像橋接起來，因而獲得了廣泛的關注。受其成功啟發，大型 3D 點雲語言模型 (3D-LLM) 也將點雲整合到 LLM 中。然而，直接將點雲與 LLM 對齊需要昂貴的訓練成本，通常需要在 A100 上進行數百個 GPU 小時，這阻礙了 3D-LLM 的發展。在本文中，我們介紹了 MiniGPT-3D，這是一個高效且強大的 3D-LLM，僅在一個 RTX 3090 上訓練 27 小時，就達到了多項 SOTA 結果。具體來說，我們提議使用來自 2D-LLM 的 2D 先驗將 3D 點雲與 LLM 對齊，這可以利用 2D 和 3D 視覺資訊之間的相似性。我們引入了一種新穎的四階段訓練策略，以串聯的方式進行模態對齊，並混合查詢專家模組以高效率自適應地聚合特徵。此外，我們利用了參數高效的微調方法 LoRA 和 Norm 微調，從而僅產生了 47.8M 個可學習參數，比現有方法減少了 260 倍。廣泛的實驗表明，MiniGPT-3D 在 3D 物件分類和標題任務上達到了 SOTA，訓練成本顯著降低。值得注意的是，與 ShapeLLM-13B 相比，MiniGPT-3D 在具有挑戰性的物件標題任務上獲得了 GPT-4 評分 8.12 的提升，而後者在 8 個 A800 上共耗費了 160 個 GPU 小時。我們率先探索了高效的 3D-LLM，為社群提供了新的見解。程式碼和權重可在 https://github.com/TangYuan96/MiniGPT-3D 獲得。
...
摘要：經食道超音波心臟圖 (TEE) 在心臟科診斷和介入性操作中扮演著關鍵角色。然而，由於影像擷取和解讀的複雜性質，有效使用它需要大量的訓練。為了提升新手超音波檢查員的效率並減少掃描擷取的可變性，我們提出了一種新的超音波 (US) 導航輔助方法，該方法基於對比學習作為目標條件強化學習 (GCRL)。我們使用一種新的對比患者批次法 (CPB) 和一種資料擴充對比損失來擴充先前的架構，我們證明兩者對於確保對患者解剖變異的概化至關重要。所提出的架構能夠使用單一模型導航至標準診斷和複雜介入性視圖。我們的這種方法是使用 789 名患者的大型資料集開發的，在 140 名患者的測試資料集上獲得了 6.56 毫米的平均位置誤差和 9.36 度的角度誤差，這與針對個別視圖訓練的模型具有競爭力或優於它們。此外，我們定量驗證了我們的方法導航到介入性視圖（例如 LAA 封閉中使用的左心耳 (LAA) 視圖）的能力。我們的這種方法有望在經食道超音波檢查期間提供有價值的指導，有助於提升心臟超音波從業人員的技能習得。

##### **Verification and Refinement of Natural Language Explanations through LLM-Symbolic Theorem Proving**
2405.01379v1 by Xin Quan,Marco Valentino,Louise A. Dennis,André Freitas
...
摘要：自然語言解釋已成為評估可解釋且多步驟的自然語言推理 (NLI) 模型的代理。然而，評估 NLI 解釋的有效性具有挑戰性，因為它通常涉及適當資料集的群眾外包，這是一個耗時且容易發生邏輯錯誤的過程。為了解決現有的限制，本文探討了通過整合大型語言模型 (LLM) 和定理證明器 (TP) 來驗證和改進自然語言解釋。具體來說，我們提出了一個名為 Explanation-Refiner 的神經符號框架，該框架使用 LLM 擴充 TP 以生成和形式化解釋性句子，並為 NLI 提出潛在的推理策略。反過來，TP 被用於提供對解釋的邏輯有效性的正式保證，並為後續改進提供反饋。我們展示了如何將 Explanation-Refiner 結合使用來評估解釋性推理、自動形式化和最先進 LLM 的錯誤校正機制，以及自動提高不同領域中變量複雜性的人工註釋解釋的質量。
...
摘要：在言语中，降低發音精確度很常見，但對於對話而言，其聲學特性和語用功能鮮少被研究。我們在此嘗試彌補這個差距。這份技術報告包含從期刊文章（Ward 等人，2024 年，已提交）中遺漏的內容。具體來說，我們在此報告 1) 關於感知到的簡約化的標註經驗，2) 發現與朗讀不同，對話中簡約化的相關因素包括高音高、音高範圍廣泛和強度，以及 3) 使用簡單的聲學/韻律特徵來預測對話中簡約化的基準模型，其與人類對英語的感知相關性達到 0.24，對西班牙語的感知相關性達到 0.17。我們還提供了英語中簡約化的其他可能的語用功能範例，以及各種討論、觀察和推測
...
摘要：大型機器，例如粒子加速器，通常由一群經驗豐富的操作員運行。對於粒子加速器，這些操作員具備有關加速器物理和構成該機器技術的適當背景知識。由於機器複雜，該機器的特定子系統由專家負責，操作員可以求助於這些專家。在這項工作中，推理和行動 (ReAct) 提示範例用於將開放權重大型語言模型 (LLM) 與高級機器控制系統框架和其他工具（例如電子日誌或機器設計文件）結合起來。通過這樣做，實現了一個多專家檢索增強生成 (RAG) 系統，它協助操作員執行知識檢索任務，在需要時直接與機器互動，或撰寫高級控制系統腳本。這種專家知識和機器互動的整合可以簡化和加快新舊人類操作員的機器操作任務。
...
摘要：彌合大型語言模型的英語和非英語表現之間的顯著差距是一項巨大的挑戰。雖然一些先前的研究嘗試使用翻譯的訓練資料來縮小這個差距，最近提出的問題比對方法利用了模型的英語專業知識，以最少使用昂貴且容易出錯的翻譯來提升多語言表現。在本文中，我們探討了這種方法可以廣泛應用在哪些方面，方法是檢視它在使用可執行程式碼進行推理和使用常識進行推理時的效應。我們也探討如何使用代理調整，將這種方法有效地應用於極大型語言模型。在多語言推理基準 mGSM、mSVAMP 和 xCSQA 上的實驗結果證明，問題比對方法可用於提升各種推理情境、模型系列和規模的多語言表現。例如，當應用於 LLaMA2 模型時，即使使用 70B 模型，我們的模型也能讓 mGSM 的平均準確度提升 12.2%。為了了解其成功機制，我們分析了表徵空間、思考鏈和翻譯資料規模，這揭示了問題翻譯訓練如何強化大型語言模型內的語言比對，並形塑它們的工作模式。
...
摘要：本研究提出了一個創新的 AI 驅動精準農業系統，利用 YOLOv8 進行疾病辨識，並使用檢索擴充生成 (RAG) 進行情境感知診斷。專注於解決影響卡納塔克邦咖啡生產部門的疾病挑戰，該系統整合了先進的物件偵測技術與語言模型，以解決與大型語言模型 (LLM) 相關的固有約束。我們的技術不僅解決了 LLM 中的幻覺問題，還引入了動態疾病辨識和補救策略。即時監控、協作式資料集擴充和組織參與確保了該系統在多樣化農業環境中的適應性。建議系統的效果超越了自動化，旨在確保糧食供應、保護生計，並推廣環保農業實務。透過促進精確的疾病辨識，該系統有助於永續且注重環境的農業，減少對農藥的依賴。展望未來，該專案預計持續開發整合 RAG 的物件偵測系統，強調可擴充性、可靠性和可用性。本研究致力於成為農業中正向變革的燈塔，與全球在永續且技術提升的糧食生產上的努力保持一致。
...
摘要：對遞迴尖峰神經網路（RSNN）進行編程，以穩健地執行多時間尺度的運算，仍然是一項艱難的挑戰。為了解決這個問題，我們展示了向量符號架構（VSA）所提供的分散式方法是如何利用高維隨機向量作為表示的最小單位，將穩健的多時間尺度動態嵌入基於吸引子的 RSNN 中。我們通過疊加對稱自聯想權重矩陣和非對稱轉換項，將有限狀態機嵌入 RSNN 動態中。轉換項是由輸入的 VSA 結合和狀態之間的異聯想外積形成的。我們的做法已通過使用高度非理想權重的模擬、實驗性的閉迴路憶阻硬體設定，以及在 Loihi 2 上進行驗證，在 Loihi 2 上，它可以無縫地擴展到大型狀態機。這項工作證明了 VSA 表示對於將具有遞迴動態的穩健運算嵌入神經形態硬體中的有效性，而不需要參數微調或大量的特定於平台的最佳化。這促使 VSA 成為神經形態硬體中認知演算法的高階表示不變抽象語言。
...
摘要：大型語言模型 (LLM) 已成為各種自然語言任務和一系列應用領域中的強大支援工具。最近的研究專注於探索它們在資料標註方面的能力。本文提供了對十二項研究的比較概述，這些研究調查了 LLM 在標記資料方面的潛力。儘管這些模型展示了有希望的成本和省時效益，但仍存在相當大的限制，例如代表性、偏見、對提示變化的敏感性和偏好英語語言。利用這些研究的見解，我們的實證分析進一步檢驗了人類和 GPT 生成的意見分佈在四個主觀資料集之間的一致性。與檢視代表性的研究相反，我們的研究方法直接從 GPT 獲取意見分佈。因此，我們的分析支持少數在評估資料標註任務時考慮不同觀點的研究，並強調需要朝這個方向進行進一步的研究。

##### **Low-resource speech recognition and dialect identification of Irish in a multi-task framework**
2405.01293v1 by Liam Lonergan,Mengjie Qian,Neasa Ní Chiaráin,Christer Gobl,Ailbhe Ní Chasaide
...
摘要：本文探討了混合 CTC/注意力編碼器-解碼器模型在愛爾蘭語（蓋爾語）低資源語音辨識 (ASR) 和方言辨識 (DID) 中使用中間 CTC (InterCTC) 進行訓練的用途。結果與目前為 ASR (TDNN-HMM) 和 DID (ECAPA-TDNN) 訓練的最佳效能模型進行比較。最初使用 Conformer 編碼器建立最佳的 InterCTC 設定。然後使用此設定訓練一個具有 E-branchformer 編碼器的模型，並比較這兩種架構的效能。採用多任務微調方法進行語言模型 (LM) 淺層融合。實驗結果顯示，相較於基準 ECAPA-TDNN，DID 準確率提升了 10.8%，而 WER 效能接近 TDNN-HMM 模型。這種多任務方法成為愛爾蘭語低資源 ASR 和 DID 的有前景策略。
...
摘要：非自迴歸 (NAR) 語言模型以其在神經機器翻譯 (NMT) 中的低延遲而聞名。然而，由於解碼空間大以及難以準確捕捉目標詞之間的依存關係，因此 NAR 和自迴歸模型之間存在效能差距。更複雜的是，為 NAR 模型準備適當的訓練資料是一項非平凡的任務，通常會加劇暴露偏差。為了應對這些挑戰，我們將強化學習 (RL) 應用於 Levenshtein Transformer，一個基於編輯的 NAR 代表模型，證明使用自我生成資料的 RL 可以增強基於編輯的 NAR 模型的效能。我們探討了兩種 RL 方法：逐步獎勵最大化和情節獎勵最大化。我們討論這兩種方法各自的優缺點，並對其進行實證驗證。此外，我們通過實驗探討溫度設定對效能的影響，確認適當的溫度設定對於 NAR 模型的訓練至關重要。
...
摘要：許多研究顯示，現有的臉部辨識系統（FRS），包括商業系統，通常會對特定種族表現出偏見，這是因為資料代表性不足所致。在這項工作中，我們探索使用合成臉部影像生成方法來進行種族變更和膚色修改，以增加資料集的多樣性。我們首先建立一個平衡的臉部影像資料集，代表三個種族：亞洲人、黑人和印度人，然後進行詳細分析。接著，我們利用現有的基於生成對抗網路（GAN）的影像到影像轉換和流形學習模型，將種族從一個變更到另一個。進一步進行系統分析，以研究使用個人類型角（ITA）的逼真膚色表示，評估此類資料集是否適合 FRS。此外，我們還使用現有的臉部影像品質評估（FIQA）方法分析品質特性。然後，我們使用四個不同的系統提供整體 FRS 效能分析。我們的發現為未來的研究工作鋪路，包括：(i) 開發特定種族和一般（任何到任何）種族變更模型，(ii) 擴展此類方法以建立具有不同膚色的資料庫，(iii) 建立代表各種種族的資料集，進一步有助於在解決隱私問題的同時減輕偏見。
...
摘要：本文介紹了一種專為自動駕駛量身打造的軌跡預測模型，專注於在動態交通場景中捕捉複雜的交互作用，而無需依賴高解析度地圖。該模型稱為 MFTraj，它利用歷史軌跡數據結合創新的動態幾何圖形行為感知模組。其核心是一個適應性結構感知互動式圖形卷積網路，它捕捉道路使用者的位置和行為特徵，保留時空複雜性。透過線性注意力機制的加強，該模型實現了計算效率和降低的參數開銷。在 Argoverse、NGSIM、HighD 和 MoCAD 資料集上的評估突顯了 MFTraj 的穩健性和適應性，即使在資料不足的場景中，也優於許多基準，而無需 HD 地圖或向量化地圖等額外資訊。重要的是，即使在資料大量遺失的場景中，它也能維持競爭力的效能，與現有最先進的模型不相上下。結果和方法論表明，自動駕駛軌跡預測有了顯著的進展，為更安全、更有效率的自動系統鋪平了道路。
...
摘要：自然語言推論 (NLI)，也稱為辨識文本蘊涵 (RTE)，是自然語言理解的重要面向。
現在大部分研究使用機器學習和深度學習在特定資料集上執行這項任務，表示他們的解決方案既不可解釋，也不明確。
為了滿足對 RTE 可解釋方法的需求，我們提出一個創新的管道，其基礎是將文字翻譯成抽象意義表示 (AMR) 圖形。為此，我們使用預先訓練的 AMR 剖析器。接著我們將 AMR 圖形翻譯成命題邏輯，並使用 SAT 求解器進行自動推理。在文字中，常識通常會暗示前提和主張之間存在蘊涵（或矛盾）關係，但由於使用了不同的文字，因此無法從其邏輯表示中辨識出來。為了解決這個問題，我們引進放寬方法，允許替換或遺忘一些命題。我們的實驗結果顯示，這個管道在四個 RTE 資料集上表現良好。

##### **Prompt engineering paradigms for medical applications: scoping review and recommendations for better practices**
2405.01249v1 by Jamil Zaghir,Marco Naguib,Mina Bjelogrlic,Aurélie Névéol,Xavier Tannier,Christian Lovis
...
摘要：提示工程對於發揮大型語言模型 (LLM) 的潛力至關重要，特別是在使用專業術語和措辭的醫療領域。然而，提示工程在醫療領域的效力仍有待探討。在這項工作中，回顧了 114 篇近期研究 (2022-2024)，這些研究在醫學中應用提示工程，涵蓋提示學習 (PL)、提示調整 (PT) 和提示設計 (PD)。PD 最為普遍（78 篇文章）。在 12 篇論文中，PD、PL 和 PT 術語可以互換使用。ChatGPT 是最常用的 LLM，有七篇論文使用它來處理敏感的臨床數據。思想鏈成為最常見的提示工程技術。雖然 PL 和 PT 文章通常提供評估基於提示的方法的基準，但 64% 的 PD 研究缺乏與提示無關的基準。我們提供表格和圖形，總結現有工作，並報告建議以指導未來的研究貢獻。
...
摘要：我們提出 TRAMBA，一種混合Transformer和 Mamba 架構，用於聲音和骨傳導語音增強，適用於行動和穿戴式平台。由於以下幾個原因，骨傳導語音增強在行動和穿戴式平台上採用並不實際：(i) 資料收集非常耗費人力，導致資料稀少；(ii) 記憶體佔用量數百 MB 的最新模型與更適合資源受限系統的方法之間存在效能差距。為了讓 TRAMBA 適應基於振動的感測方式，我們使用廣泛可用的音訊語音資料集預先訓練 TRAMBA。然後，使用者使用少量骨傳導資料進行微調。TRAMBA 在 PESQ 中比最新 GAN 高出 7.3%，在 STOI 中高出 1.8%，記憶體佔用量小一個數量級，推論速度快達 465 倍。我們將 TRAMBA 整合到真實系統中，並展示 TRAMBA (i) 透過減少資料取樣和傳輸，將穿戴式裝置的電池續航力提升至 160%；(ii) 在嘈雜環境中產生比無線語音品質更高的聲音；(iii) 所需記憶體佔用量小於 20.0 MB。
...
摘要：大型語言模型 (LLM) 已在各種任務中取得顯著的成功，但它們仍然容易受到對抗性攻擊，特別是記錄完善的「越獄」攻擊。最近，貪婪坐標梯度 (GCG) 攻擊已證明其有效利用此漏洞，可透過梯度啟發法和貪婪搜尋的組合來最佳化對抗性提示。然而，此攻擊的效率已成為攻擊過程中的一個瓶頸。為了減輕此限制，我們在本文中透過最佳化鏡頭重新思考對抗性提示的產生，旨在穩定最佳化過程並利用先前反覆運算中的更多啟發式見解。具體來說，我們引進了\textbf{M}omentum \textbf{A}ccelerated G\textbf{C}G (\textbf{MAC}) 攻擊，其中將動量項納入梯度啟發法中。實驗結果展示了 MAP 在對齊語言模型的基於梯度的攻擊中所取得的顯著增強。我們的程式碼可在 https://github.com/weizeming/momentum-attack-llm 取得。
...
摘要：論證結構學習 (ASL) 涉及預測論證之間的關係。由於它可以對文件進行結構化以促進理解，因此已廣泛應用於許多領域（醫療、商業和科學領域）。儘管其用途廣泛，ASL 仍然是一項具有挑戰性的任務，因為它涉及檢查潛在非結構化話語中句子之間的複雜關係。為了解決這個問題，我們開發了一種簡單但有效的方法，稱為 ASL 任務的雙塔多尺度卷積神經網路 (DMON)。具體來說，我們將論證組織成一個關係矩陣，該矩陣與論證嵌入一起形成一個關係張量，並設計一個機制來捕捉與上下文論證的關係。在三個不同領域的論證挖掘資料集上的實驗結果表明，我們的框架優於最先進的模型。程式碼可在 https://github.com/VRCMF/DMON.git 取得。
...
摘要：強化學習政策通常由黑箱神經網路表示，這些網路不可解釋且不適合安全關鍵領域。為了解決這兩個問題，我們提出受約束的正規化流動政策，作為可解釋且安全建構的政策模型。我們使用即時安全約束來實現強化學習問題的安全性，我們可以利用領域知識通過分析構造正規化流動來確保約束滿足。正規化流動對應於對動作樣本進行一系列可解釋的轉換，每個轉換確保相對於特定約束的一致性。我們的實驗揭示了可解釋性之外的優點，在於更簡單的學習目標和在整個學習過程中維持約束滿足。我們的做法利用了獎勵工程方面的約束，同時提供增強的可解釋性、安全性，以及向代理提供領域知識的直接手段，而無需依賴複雜的獎勵函數。
...
摘要：邊緣運算允許人工智慧和機器學習模型部署在邊緣裝置上，在邊緣裝置上，這些模型可以從本地資料中學習，並協作形成一個全域模型。聯合學習 (FL) 是一種分散式機器學習技術，在維護資料隱私的同時促進此流程。然而，FL 也面臨挑戰，例如關於受資源限制裝置的高運算和通訊成本，以及由於邊緣用戶端資料的異質性和存在分布外資料而導致的概化效能不佳。在本文中，我們提出梯度一致性引導聯合稀疏訓練 (FedSGC)，這是一種新方法，將動態稀疏訓練和梯度一致性檢查整合到聯合學習架構中，以解決這些問題。我們的模型利用了這樣一個想法：相關梯度與全域模型相對於衝突方向的神經元包含與其他用戶端無關或概化程度較低的信息，並且可以在稀疏訓練過程中被剪枝。相反，相關梯度與一致方向的神經元可以優先增長。通過這種方式，FedSGC 可以大幅減少本地運算和通訊開銷，同時增強 FL 的概化能力。我們在具有挑戰性的非 i.i.d 設置上評估我們的模型，並表明它在各種場景中以最先進的 FL 模型實現了競爭優勢，同時最大程度地減少運算和通訊成本。
...
摘要：訓練深度神經網路（DNN）以從雜訊標籤中學習是一項重要且具有挑戰性的任務。然而，現有的大多數方法都專注於損壞標籤，而忽略了固有資料結構的重要性。為了縮小雜訊標籤和資料之間的差距，我們受到物理學中勢能概念的啟發，為雜訊標籤學習提出了一種新的基於勢能的混合模型（PEMM）。我們創新了一種基於距離的分類器，並在其類別中心進行勢能正則化。將我們提出的分類器與現有的深度學習主幹網路結合，我們可以獲得具有更好特徵表示的強健網路。它們可以保留資料中的內在結構，從而產生卓越的雜訊容忍度。我們進行了廣泛的實驗，以分析我們提出的模型在多個真實世界資料集上的效率。量化結果表明，它可以實現最先進的效能。
...
摘要：自訓練是一種強大的深度學習方法。其關鍵過程是找到一個偽標籤進行建模。然而，先前的自訓練演算法會受到硬標籤所帶來的過度自信問題，即使某些與信心相關的正則化器也無法全面捕捉不確定性。因此，我們提出一個新的自訓練架構，以結合模型和資料集的不確定性資訊。具體來說，我們建議使用期望最大化 (EM) 來平滑標籤並全面估計不確定性資訊。我們進一步設計了一個基礎提取網路，以從資料集估計初始基礎。獲得的不確定性基礎可以根據不確定性資訊進行過濾。然後，它可以轉換為真正的硬標籤，以便在重新訓練過程中反覆更新模型和基礎。影像分類和語意分割的實驗顯示，我們的演算法在具備信心感知的自訓練演算法中具有優勢，在不同的資料集上提升了 1-3 個百分點。
...
摘要：本文介紹 TartuNLP 團隊在 EvaLatin 2024 共享任務中提交的歷史拉丁語文本情緒極性偵測。我們的系統依賴兩種不同的方法來為監督式學習標註訓練資料：1) 採用由組織者提供的極性詞彙表，建立基於啟發法的標籤；2) 使用 GPT4 產生標籤。我們使用適配器架構進行參數有效的微調，並針對訓練語言和任務適配器實驗了單語和跨語言知識轉移。我們提交的 LLM 生成的標籤在情緒極性偵測任務中獲得整體第一名。我們的結果顯示，基於 LLM 的標註在拉丁語文本中展現出有希望的結果。
...
摘要：異常偵測 (AD) 是工業環境中經常需要的重要程序。異常可能會傳達系統內部的潛在問題，促使進一步調查。工業程序旨在盡可能簡化作業，包含最終產品的生產，讓異常偵測成為達成此目標的必要手段。傳統的異常偵測方法通常將觀察結果分類為正常或異常，而未提供這些分類背後的原因見解。因此，根據工業 5.0 的出現，一種更理想的方法涉及提供可解釋的結果，讓使用者了解結果背後的原理。本文介紹 ExIFFI 的第一個工業應用，這是一種最近開發的方法，專注於產生快速且有效的說明，以擴充孤立森林 (EIF) 異常偵測方法。ExIFFI 在兩個公開的工業資料集上進行測試，證明其在說明和運算效率方面優於其他最先進的可解釋異常偵測模型。
...
摘要：準確偵測和追蹤裝置（如導引導管）在即時 X 光影像擷取中，是血管內心臟介入治療的基本先決條件。此資訊可提升程序指導，例如指導支架置放。為確保程序安全性和有效性，需要高度穩健性，在追蹤過程中不會出現任何失敗。為了達成此目的，需要有效地解決挑戰，例如：對比劑或其他外部裝置或導線遮蔽裝置、視野或擷取角度的變化，以及因心臟和呼吸運動而產生的持續移動。為了克服上述挑戰，我們提出一個創新的方法，從超過 1600 萬個介入式 X 光影像序列資料中，使用影像序列資料的自監督學習時空特徵。我們的做法是基於遮罩影像建模技術，該技術利用基於插補的重建來學習精細的影格間時序對應。編碼在結果模型中的特徵會在後續進行微調。與經過高度最佳化的參考解決方案（使用多階段特徵融合、多任務和流動正規化）相比，我們的做法達到了最先進的效能，特別是穩健性。實驗顯示，我們的做法在最大追蹤誤差方面，與參考解決方案相比，減少了 66.31%（使用流動正規化時為 23.20%）；在 42 幀/秒（在 GPU 上）的 3 倍更快的推論速度下，達到了 97.95% 的成功率。這些結果鼓勵在介入式影像分析中其他各種任務中使用我們的做法，這些任務需要有效理解時空語義。
...
摘要：本研究探討主觀賦予意義的假設，構成意識經驗的現象內容。也就是說，現象內容是語義的。這種主觀意義的形式，表現為心質的內在且非表徵的特性。從經驗上來說，主觀意義在意識經驗中無處不在。我們指出現象學研究，以提供證據支持此說法。此外，這種意義的概念與弗雷格在形上學和語言哲學中所指的「意涵」密切相關。它也與皮爾斯的「詮釋項」在符號學中相符。我們探討弗雷格的意涵如何也能延伸到意識的原始感受。意涵和指涉都在現象經驗中扮演角色。此外，在心物關係的脈絡中，我們提供與個人心智表徵相關的主觀意義的形式化。透過找出物理和心智領域之間的精確對應，我們主張語法和語義結構超越語言，並在這些領域的每一個中實現。形式上，意義是一種關係屬性，透過一個對應實現，這個對應會在適當的語義空間中詮釋形式系統的語法結構。這個對應在心智領域中的影像，就是與經驗相關的，因此構成心質的現象內容。我們以這個概念可能對基於經驗的意識理論造成的影響做為結論。
...
摘要：積極參與對話是建立共同基礎的關鍵，
因為理解是由生產者和接收者共同量身打造的。
旁聽者被剝奪了執行接地行為的特權，只能猜測預期的含義。儘管如此，資料生成和註解、
NLP 對話模型的建模、訓練和評估依賴於旁聽範例。有多少基礎過程因此被沒收？正如我們所展示的，有證據表明無法使用資料驅動學習模型適當地建模人類元溝通行為。在本文中，我們討論了這個問題，並對人類要求澄清的決策變異性進行了初步分析。最重要的是，我們希望將這個話題帶回社群的討論桌上，鼓勵討論只有「傾聽」的模型的後果。
...
摘要：現有建立基於來源資訊尋求對話資料集的方法通常成本高且難以實作，因為它們僅依賴人工註解員。我們提議結合大型語言模型 (LLM) 提示和人類專業知識，以進行更有效率且可靠的資料產生。我們使用 LLM 產生來模擬兩個角色，而不是勞力密集的綠野仙蹤 (WOZ) 方法，其中兩個註解員從頭開始產生對話，扮演代理和使用者的角色。然後，註解員驗證輸出並使用歸因資料擴充輸出。我們透過建構 MISeD（會議資訊尋求對話資料集）來展示我們的做法，這是第一個專注於會議記錄的資訊尋求對話資料集。使用 MISeD 微調的模型在我們的測試集，以及一個新的全手動 WOZ 測試集和現有的基於查詢的摘要基準上，都展現出優異的效能，這表示我們的方法很有用。
...
摘要：有效的單目深度估計演算法開發主要障礙在於難以取得與收集的 RGB 影像對應的高品質深度資料。收集此資料耗時且昂貴，而且即使現代感測器收集的資料也有範圍或解析度限制，且容易出現不一致和雜訊。為了解決此問題，我們提出一個使用 3D 合成環境和 CycleGAN 領域轉移的資料產生模擬方法。我們將此資料產生方法與熱門的 NYUDepth V2 資料集進行比較，方法是使用不同的真實和模擬資料訓練組訓練基於 DenseDepth 結構的深度估計模型。我們在 Husky 機器人新收集的影像和 LiDAR 深度資料上評估模型效能，以驗證此方法的概化能力，並顯示 GAN 轉換資料可以作為真實世界資料的有效替代品，特別是在深度估計方面。
...
摘要：在追求精進全自動駕駛的精準感知模型時，持續的在線模型訓練變得至關重要。車輛網路內的聯合學習 (FL) 提供了一種用於模型訓練的有效機制，同時保留原始感測資料的完整性。然而，FL 卻受限於非同質分布的資料（例如數量偏差），導致模型訓練期間的收斂速度次佳。在先前的研究中，我們介紹了 FedLA，一種創新的標籤感知聚合方法，用於解決 FL 中的資料異質性，適用於一般情境。
  在本文中，我們介紹了 FedProx+LA，一種新穎的 FL 方法，建立在最先進的 FedProx 和 FedLA 之上，用於處理資料異質性，專門針對車輛網路量身打造。我們評估了 FedProx+LA 在持續在線物件偵測模型訓練中的效能。透過與傳統方法和最先進方法進行比較分析，我們的研究結果揭示了 FedProx+LA 優異的收斂速度。值得注意的是，如果標籤分佈非常異質，與基準方法相比，我們的 FedProx+LA 方法在偵測效能方面顯示出顯著的改善，也優於我們先前的 FedLA 方法。此外，與基準方法相比，FedLA 和 FedProx+LA 都將收斂速度提高了 30%。
...
摘要：圖形Transformer，由於其全局注意力機制，已成為處理圖形結構數據的新工具。眾所周知，全局注意力機制在完全連接的圖形中考慮更廣泛的感受野，導致許多人相信可以從所有節點中提取有用的信息。在本文中，我們挑戰了這一信念：全局化屬性是否總能使圖形Transformer受益？我們通過提供經驗證據和理論分析揭示了圖形Transformer中的過度全局化問題，即當前的注意力機制過度關注那些遠程節點，而實際上包含大部分有用信息的近節點則相對弱化。然後，我們提出了一個新穎的具有協作訓練的雙層全局圖形Transformer (CoBFormer)，包括集群間和集群內Transformer，以防止過度全局化問題，同時保持從遠程節點提取有價值信息的能力。此外，提出了協作訓練，以在理論保證下提高模型的泛化能力。在各種圖形上的大量實驗很好地驗證了我們提出的 CoBFormer 的有效性。
...
摘要：<paragraph>吹哨揭弊對於確保公私部門的透明度和問責制至關重要。然而，（潛在的）吹哨者經常害怕或面臨報復，即使是在匿名舉報的情況下。他們所揭露的具體內容和其獨特的寫作風格可能會重新確認他們是消息來源。法律措施，例如歐盟 WBD，在範圍和有效性方面受到限制。因此，計算機方法來防止重新識別是鼓勵吹哨者挺身而出的重要補充工具。然而，目前的文本清理工具採用一刀切的方法，對匿名性採取過於狹隘的看法。它們旨在通過將典型的風險詞（例如人名和其他 NE 標籤）及其組合替換為佔位符來降低識別風險。然而，這種方法對於舉報場景來說是不夠的，因為它忽視了文本特徵（包括寫作風格）中進一步的重新識別潛力。因此，我們提出、實施並評估了一種新的分類和緩解策略，用於重寫文本，其中涉及舉報人在風險和效用評估中的參與。我們的原型工具在詞/術語級別半自動地評估風險，並應用風險適應的匿名化技術來生成語法脫節但適當清理的文本。然後，我們使用經過微調的 LLM 來進行改寫，以使文本連貫且風格中立。我們使用歐洲人權法院的案例和現實世界中舉報人證詞的摘錄來評估我們工具的有效性，並使用流行的 IMDb62 電影評論數據集在統計上衡量對作者歸因 (AA) 攻擊的保護和效用損失。我們的這種方法可以將 AA  accuracy 從 98.81% 大幅降低到 31.22%，同時保留了高達 73.1% 的原始內容語義。</paragraph>

##### **Poisoning Attacks on Federated Learning for Autonomous Driving**
2405.01073v1 by Sonakshi Garg,Hugo Jönsson,Gustav Kalander,Axel Nilsson,Bhhaanu Pirange,Viktor Valadi,Johan Östman
...
摘要：聯邦學習 (FL) 是一種分散式學習範例，使各方能夠在保持其數據機密性的同時協作訓練模型。在自動駕駛中，它帶來降低數據儲存成本、減少頻寬需求和加速學習的潛力。然而，FL 容易受到中毒攻擊。在本文中，我們介紹了針對自動駕駛中回歸任務量身打造的兩種新型中毒攻擊：FLStealth 和 Off-Track Attack (OTA)。FLStealth 是一種非目標攻擊，旨在提供會損害全局模型效能的模型更新，同時看起來是良性的。另一方面，OTA 是一種目標攻擊，其目的是在暴露於特定觸發器時改變全局模型的行為。我們透過對車輛軌跡預測任務進行全面實驗，證明了我們攻擊的有效性。特別是，我們展示了在五種不同的非目標攻擊中，FLStealth 在繞過伺服器採用的考慮防禦方面最為成功。對於 OTA，我們展示了常見防禦策略無法減輕攻擊的能力，強調了在 FL 中針對自動駕駛中的目標攻擊迫切需要新的防禦機制。

##### **AB-Training: A Communication-Efficient Approach for Distributed Low-Rank Learning**
2405.01067v1 by Daniel Coquelin,Katherina Flügel,Marie Weiel,Nicholas Kiefer,Muhammed Öz,Charlotte Debus,Achim Streit,Markus Götz
...
摘要：通訊瓶頸會阻礙分散式神經網路訓練的可擴充性，特別是在分散式記憶體運算叢集上。為了大幅減少這種通訊負擔，我們引進了 AB 訓練，一種新穎的資料平行訓練方法，它會將權重矩陣分解成低階表示並利用獨立的基於群組的訓練。這種方法在多種擴充範例中持續減少 50% 的網路流量，增加了通訊受限系統上的訓練可能性。我們的這種方法在較小的規模上展現出規範化效果，導致像 VGG16 一樣的模型的概化能力獲得改善，同時在 CIFAR-10 上訓練期間達到了 44.14 : 1 的驚人壓縮比，並維持了競爭力的準確度。儘管前景看好，我們的實驗顯示，即使在低階訓練模式中，大批次效應仍然是一個挑戰。
...
摘要：從單一 RGB 影像重建手部網格是一項具有挑戰性的任務，因為手部經常會被物體遮擋。大多數先前的工作嘗試引入更多額外的資訊，並採用注意力機制來改善 3D 重建結果，但這會增加運算複雜度。這個觀察促使我們提出一個新的簡潔架構，同時改善運算效率。在這項工作中，我們提出一個簡單且有效的 3D 手部網格重建網路 HandSSCA，這是第一個將狀態空間建模納入手部姿勢估計領域的網路。在網路中，我們設計了一個新穎的狀態空間通道注意力模組，它擴展了有效的感測場，在空間維度中提取手部特徵，並在通道維度中增強手部區域特徵。此設計有助於重建完整且詳細的手部網格。在具有挑戰性手部物體遮擋（例如 FREIHAND、DEXYCB 和 HO3D）的知名資料集上進行的廣泛實驗證明，我們提出的 HandSSCA 在維持最小參數計數的同時，達到了最先進的效能。
...
摘要：模擬土壤反射光譜對於土壤-植物輻射建模和訓練機器學習模型非常有價值，但由於土壤結構及其組成成分之間的複雜關係，這很困難。為了解決這個問題，開發了一個基於土壤屬性輸入的土壤反射光譜模擬全數據驅動土壤光學生成模型 (SOGM)。該模型在一個廣泛的數據集上進行訓練，該數據集包含來自 17 個數據集的近 180,000 個土壤光譜-屬性對。它從描述土壤屬性及其值的基於文本的輸入中生成土壤反射光譜，而不是僅以二進制向量格式的數值和標籤。生成模型可以根據一組不完整的輸入屬性模擬輸出光譜。SOGM 基於去噪擴散概率模型 (DDPM)。還構建了兩個額外的子模型來補充 SOGM：一個光譜填充模型，它可以填補小於全可見光近紅外範圍 (VIS-NIR；400 至 2499 nm) 的光譜的空白，以及一個濕土壤光譜模型，它可以估計水含量對土壤反射光譜的影響，給定 SOGM 預測的乾燥光譜。SOGM 通過與 Helios 3D 植物建模軟體結合來升級，這允許生成模擬土壤和植物場景的合成航拍影像。它還可以輕鬆地與用於遙感研究的土壤-植物輻射模型（如 PROSAIL）集成。SOGM 在未包含在模型訓練中的新數據集上的測試結果證明，該模型可以根據可用的屬性輸入生成合理的土壤反射光譜。所提出的模型可以在以下位置公開訪問：
https://github.com/GEMINI-Breeding/SOGM_soil_spectra_simulation。
...
摘要：<paragraph>隨著都市私人小客車擁有率的攀升，都市停車難題日益惡化，迫切需要有效的停車位可用性預測，以利都市規劃與管理。然而，現有的預測方法存在預測準確度低、缺乏與停車量相關的時空關聯特徵，以及忽略特定區域內相似停車場之間的流動模式和關聯性等問題。為了解決這些挑戰，本研究提出一個整合時空深度學習與多來源資料融合的停車位可用性預測架構，涵蓋來自多個來源（例如地鐵、公車、計程車服務）的交通需求資料和停車場資料。該架構基於 Transformer 作為時空深度學習模型，並利用 K-means 聚類建立停車場群集區域，從連接到停車場的各種運輸模式（即地鐵、公車、線上叫車和計程車）中提取和整合交通需求特徵。使用真實世界中的經驗資料驗證所提出的方法與不同的機器學習、深度學習和傳統統計模型在預測停車位可用性方面的有效性。實驗結果顯示，透過所提出的流程，所開發的 Transformer 模型在各種指標（例如均方誤差 (MSE)、平均絕對誤差 (MAE) 和平均絕對百分比誤差 (MAPE)）方面優於其他模型。透過融合多來源需求資料與時空深度學習技術，此方法有潛力開發出停車位可用性預測系統，為駕駛人和都市規劃者提供更準確及時的資訊，從而促進更有效率和永續的都市流動性。</paragraph>
...
摘要：機器學習中泛化的目標是在各種未見任務和領域中達到優良的表現。最近，自我監督學習（SSL）被視為達成此目標的有效方法。它可以從未標記的資料中學習高品質的表徵，並在多個下游任務中達到有希望的經驗表現。現有的 SSL 方法主要從兩個方面約束泛化：(i) 大規模訓練資料，以及 (ii) 學習任務層級共享知識。然而，這些方法缺乏在學習目標中對 SSL 泛化的明確建模，而對 SSL 泛化的理論理解仍然有限。這可能會導致 SSL 模型在資料稀少的情況下過度擬合，並在真實世界中泛化不良，使得難以達到真正的泛化。為了解決這些問題，我們提供了 SSL 中泛化的理論定義，並定義了一個 $\sigma$-量測來幫助量化它。基於這個見解，我們將泛化明確地建模到自我監督學習中，並進一步提出一個新的 SSL 架構，稱為 GeSSL。它引入了一個基於 $\sigma$-量測的自激勵目標，使模型能夠找到朝向泛化的最佳更新方向。廣泛的理論和經驗評估證明了所提出的 GeSSL 的優異性能。
...
摘要：<paragraph>深度學習最近的進展已經展示出顯著的表現，在各種監督式電腦視覺任務中與人類能力相當。然而，在模型訓練之前擁有包含所有類別的廣泛訓練資料集的流行假設通常與現實世界的場景不同，在現實世界中，新類別的資料可用性有限是常態。挑戰在於將少數樣本的新類別無縫整合到訓練資料中，要求模型靈活地容納這些新增內容，同時不影響其在基礎類別上的表現。為了應對這種緊急情況，研究社群在少樣本類別增量學習 (FSCIL) 的領域中引入了多種解決方案。在這個研究中，我們介紹了一個創新的 FSCIL 框架，它利用語言正規化器和子空間正規化器。在基礎訓練期間，語言正規化器有助於納入從視覺語言模型中提取的語義資訊。子空間正規化器有助於促進模型在增量訓練期間獲取內含於基礎類別的影像和文字語義之間的細微連結。我們提出的框架不僅賦予模型擁抱具有有限資料的新類別的能力，而且還確保在基礎類別上的表現不變。為了證實我們方法的功效，我們在三個不同的 FSCIL 基準上進行了全面的實驗，我們的框架達到了最先進的表現。</paragraph>
...
摘要：學習解決車輛路線問題 (VRP) 已引起許多關注。然而，大多數神經求解器僅針對特定問題進行結構化和獨立訓練，這使得它們較不通用且不切實際。在本文中，我們旨在開發一個統一的神經求解器，可以同時應對一系列 VRP 變體。具體來說，我們提出了一個具有專家混合 (MVMoE) 的多任務車輛路線求解器，它在不按比例增加計算的情況下大大增強了模型容量。我們進一步為 MVMoE 開發了一個分層閘控機制，在經驗效能和計算複雜度之間取得了良好的平衡。在實驗中，我們的模型顯著提升了 10 個未見 VRP 變體的零次學習泛化效能，並在小次學習設定和真實世界基準實例中展示了良好的結果。我們進一步提供了對 MoE 配置在解決 VRP 中的影響的廣泛研究。令人驚訝的是，分層閘控可以實現更好的分布外泛化效能。原始碼可在 https://github.com/RoyalSkye/Routing-MVMoE 取得。

##### **UniGen: Universal Domain Generalization for Sentiment Classification via Zero-shot Dataset Generation**
2405.01022v1 by Juhwan Choi,Yeonghwa Kim,Seunguk Yu,JungMin Yun,YoungBin Kim
...
摘要：儘管預先訓練的語言模型在基於提示的少量學習中展現出極大的靈活性與多功能性，但它們卻有參數規模龐大、推論適用性受限的問題。最近的研究建議將 PLM 用作資料集產生器，並訓練一個微小的特定任務模型以達成有效率的推論。然而，它們在各種領域的適用性受到限制，因為它們傾向於產生特定領域的資料集。在這項工作中，我們提出了一種通用領域泛化的創新方法，可以產生與目標領域無關的資料集。這允許將微小任務模型泛化到任何共享標籤空間的領域，從而增強資料集產生範例在現實世界中的適用性。我們的實驗表明，所提出的方法在使用比 PLM 小好幾個數量級的參數集時，就能在各種領域中達成泛化能力。
...
摘要：<paragraph>最近的基于变压器的 ASR 模型已实现低于 4% 的单词错误率 (WER)，超越了人工注释员的准确性，但它们需要大量的服务器资源，从而导致了显著的碳足迹。ASR 的传统基于服务器的架构也带来了隐私问题，以及由于网络依赖性而产生的可靠性和延迟问题。相比之下，设备 (边缘) ASR 增强了隐私性，提升了性能，并通过有效平衡特定应用程序的能耗和准确性来促进可持续性。本研究考察了量化、内存需求和能耗对 NVIDIA Jetson Orin Nano 上各种 ASR 模型推理性能的影响。通过分析使用 FP32、FP16 和 INT8 量化在干净和噪声数据集上跨模型的 WER 和转录速度，我们重点介绍了准确性、速度、量化、能效和内存需求之间的关键权衡。我们发现，对于不同的模型，将精度从 fp32 改变为 fp16 可将音频转录的能耗减半，同时性能下降很小。更大的模型大小和参数数量既不能保证更好的抗噪性，也不能预测给定转录负载的能耗。这些以及其他一些发现为在能源和内存受限的环境中优化 ASR 系统提供了新的见解，这对于开发高效的设备 ASR 解决方案至关重要。本文中复制结果所需的代码和输入数据是开源的，可在 [https://github.com/zzadiues3338/ASR-energy-jetson] 获得。</paragraph>
...
摘要：伊格博語正面臨瀕臨滅絕的風險，根據 2025 年聯合國教科文組織的研究指出。這突顯出開發伊格博語語言技術以促進溝通、學習和保存的必要性。為了建立強大、有影響力且廣泛採用的伊格博語語言技術，納入語言的多方言性質至關重要。實現方言感知語言技術的主要障礙是缺乏全面的方言數據集。有鑑於此，我們提出了 IgboAPI 數據集，這是一個多方言的伊格博語-英語詞典數據集，旨在增強伊格博語方言的代表性。此外，我們透過兩項不同的研究來說明 IgboAPI 數據集的實用性：一項專注於伊格博語語義詞彙，另一項則專注於機器翻譯。在語義詞彙專案中，我們成功為伊格博語語義標記器建立了初始的伊格博語語義詞彙，而在機器翻譯研究中，我們證明透過使用 IgboAPI 數據集微調現有的機器翻譯系統，我們可以顯著提高它們處理句子中方言變化的能力。
...
摘要：儘管大型語言模型 (LLM) 在文字理解和生成方面取得了顯著的成功，但它們在文字分群任務中的潛力仍未得到充分探討。我們觀察到功能強大的閉源 LLM 提供了良好的實體集分群，但由於需要大量的運算能力和相關成本，因此無法擴展。因此，我們提出了 CACTUS（使用擴增三重損失的上下文感知分群），這是一種系統化方法，利用開源 LLM 來有效率且有效地監督實體子集的分群，特別是專注於基於文字的實體。現有的文字分群方法無法有效擷取實體子集提供的上下文。此外，儘管有幾種基於語言建模的分群方法，但很少有方法是設計用於監督分群任務。本文介紹了一種新穎的方法，透過可擴展的實體間注意力機制擷取上下文，使用 LLM 分群實體子集。我們提出了一個新穎的擴增三重損失函數，專門用於監督分群，它解決了將三重損失直接應用於此問題的固有挑戰。此外，我們介紹了一項基於文字擴充技術的自我監督分群任務，以改善我們模型的泛化性。為了評估，我們從閉源 LLM 收集地面實況分群，並在監督分群架構下將此知識轉移到開源 LLM，讓速度較快且成本較低的開源模型執行相同的任務。在各種電子商務查詢和產品分群資料集上的實驗證明，我們提出的方法在各種外部分群評估指標下，顯著優於現有的無監督和監督基準。
...
摘要：神經崩潰 (NC) 是深度神經網路 (DNN) 在訓練終端階段的一種簡單且對稱的現象，其中最後一層特徵會崩潰成類別平均值，並形成與分類器向量對齊的單純等角緊緻框架。然而，最後一層特徵與資料和訓練期間的中間層的關係仍未探討。為此，我們描述了 ResNet 中間層的幾何形狀，並提出了一個新的猜想，即漸進前饋崩潰 (PFC)，聲稱崩潰程度會在 DNN 的前向傳播過程中增加。我們根據權重衰減的 ResNet 近似於 Wasserstein 空間中的測地線曲線，為訓練良好的 ResNet 推導了一個透明模型。PFC 的指標確實在各種資料集上隨著深度單調遞減。我們提出了新的替代模型，即多層無約束特徵模型 (MUFM)，通過最佳傳輸正則化器連接中間層。MUFM 的最佳解與 NC 不一致，但相對於輸入資料更集中。總的來說，這項研究將 NC 擴展到 PFC，以建模中間層的崩潰現象及其對輸入資料的依賴性，從而闡明了 ResNet 在分類問題中的理論理解。
...
摘要：大型語言模型 (LLM) 啟用滿足資訊需求的新方法。儘管已在應用於文件排名和簡短文字產生等設定方面取得重大進展，但它們仍難以撰寫完整、準確且可驗證的長篇報告。具有這些品質的報告對於滿足使用者複雜、細微差別或多面向的資訊需求是必要的。在本文中，我們彙整來自產業和學術界，以及各種相關研究領域的意見，以呈現我們對自動報告產生的願景，以及更重要的是，一個可藉此評估此類報告的彈性架構。與其他摘要任務不同，自動報告產生從資訊需求的詳細描述開始，說明報告的必要背景、需求和範圍。此外，產生的報告應完整、準確且可驗證。這些品質在許多分析報告撰寫設定中是理想的（如果不需要的話），需要重新思考如何建構和評估展現這些品質的系統。為了促進建構這些系統的新努力，我們提出一個評估架構，該架構利用各種評估中發現的想法。為了測試完整性和準確性，該架構使用資訊片段（表示為問題和答案），這些資訊片段必須是任何高品質產出報告的一部分。此外，評估引述（將報告中提出的主張對應到其原始文件）可確保可驗證性。
...
摘要：<paragraph>設計偏好引導（PE）方法，可以在冷啟動設定中快速確定使用者的首要項目偏好，是建構有效且個人化的對話式推薦（ConvRec）系統的一項關鍵挑戰。儘管大型語言模型（LLM）構成了一種新穎的技術，可實現完全自然語言（NL）PE 對話，但我們假設單體 LLM NL-PE 方法缺乏多輪、決策理論推理，而這對於有效平衡 NL 探索和利用使用者偏好以針對任意項目集是必要的。相比之下，傳統貝氏最佳化 PE 方法定義了理論上最佳的 PE 策略，但無法使用 NL 項目描述或產生 NL 查詢，不切實際地假設使用者可以用直接項目評分和比較來表達偏好。為了克服這兩種方法的限制，我們在貝氏最佳化（BO）架構中制定 NL-PE，該架構旨在產生 NL 查詢，主動引導自然語言回饋以降低項目效用的不確定性，以找出最佳推薦。我們在一個新穎的 NL-PE 演算法 PEBOL 中展示了我們的架構，該演算法使用使用者偏好語句和 NL 項目描述之間的自然語言推論（NLI）來維護偏好信念和 BO 策略，例如湯普森取樣（TS）和上置信界（UCB），以引導 LLM 查詢產生。我們在受控實驗中以數值方式評估我們的模型，發現與依賴於一個小得多的 400M 參數 NLI 模型進行偏好推論的單體 GPT-3.5 相比，PEBOL 在 10 輪冷啟動 NL-PE 對話後在 MAP@10 中實現了高達 131% 的改進。</paragraph>
...
摘要：<paragraph>本文介紹 TVB-HKSL-News，一種新的香港手語 (HKSL) 資料集，是在 7 個月內從電視新聞節目中收集而來。此資料集的收集目的在於豐富 HKSL 的資源，並支援大詞彙量連續手語辨識 (SLR) 和翻譯 (SLT) 的研究。資料集包含兩位手語者的 16.07 小時手語影片，詞彙量為 6,515 個手勢 (用於 SLR) 和 2,850 個中文字或 18K 個中文詞彙 (用於 SLT)。一位手語者有 11.66 小時的手語影片，另一位有 4.41 小時。建立資料集的其中一個目標是支援調查，針對特定手語者，在有 (相對) 大量訓練資料的情況下，大詞彙量連續手語辨識/翻譯的表現如何，這有可能導致新的建模方法的發展。此外，資料收集流程的大部分都已自動化，只需少量人工介入；我們相信我們的收集方法可以擴大規模，以便在未來輕鬆地為任何手語收集更多手語資料以進行 SLT，只要有此類手語翻譯影片。我們也針對資料集執行 SOTA SLR/SLT 模型，並取得 34.08% 的基線 SLR 字元錯誤率和 23.58 的基線 SLT BLEU-4 分數，作為未來針對資料集研究的基準。</paragraph>

##### **Language Fairness in Multilingual Information Retrieval**
2405.00978v1 by Eugene Yang,Thomas Jänich,James Mayfield,Dawn Lawrie
...
摘要：多語言資訊檢索 (MLIR) 考慮了在多種語言中對文件進行排序的問題，而查詢是以可能與這些語言不同的語言表達的。最近的工作觀察到，例如將表示單一文件語言的排序清單組合起來或使用多語言預訓練語言模型的方法表現出偏好一種語言勝過其他語言。這導致了對不同語言中的文件進行系統性不公平處理。這項工作提出了一種語言公平性指標，以評估不同語言的文件是否通過使用 Kruskal-Wallis 檢定進行統計等價性檢定而得到公平排序。與群體公平性中的大多數先前工作相反，我們不認為任何語言都是不受保護的群體。因此，我們提出的測量標準 PEER（EqualExpected Rank 的機率）是第一個公平性指標，專門設計用於捕捉 MLIR 系統的語言公平性。我們在人工排序清單上展示了 PEER 的行為。我們還對兩個公開基準上的實際 MLIR 系統進行評估，並顯示 PEER 分數與 MLIR 公平性的先前分析結果一致。我們的實作與 ir-measures 相容，可在 http://github.com/hltcoe/peer_measure 取得。
...
摘要：在跨语言信息检索 (CLIR) 的最新工作中，查询和文档使用不同的语言，已展示了 Translate-Distill 框架的优势，该框架使用翻译和蒸馏训练跨语言神经双编码器模型。然而，Translate-Distill 仅支持单一文档语言。多语言信息检索 (MLIR) 对多语言文档集合进行排名，其训练难度高于 CLIR，因为该模型必须为不同语言的文档分配可比较的相关性分数。这项工作扩展了 Translate-Distill，并为 MLIR 提出多语言 Translate-Distill (MTD)。我们展示了使用 MTD 训练的 ColBERT-X 模型在 nDCG@20 中比使用多语言 Translate-Train（这是以前最先进的训练方法）训练的对应模型高出 5% 至 25%，在 MAP 中高出 15% 至 45%。我们还展示了该模型对训练批次中语言混合方式的鲁棒性。我们的实现可在 GitHub 上获得。
...
摘要：PLAID 是一種高效的 ColBERT 晚期互動雙編碼器實作，它使用預訓練的語言模型進行排名，在單語、跨語言和多語言檢索方面持續達到最先進的效能。PLAID 與 ColBERT 的不同之處在於將詞彙指定給群集，並將這些詞彙表示為群集質心加上壓縮的殘差向量。儘管 PLAID 在批次實驗中很有效，但其效能會在串流設定中下降，這是因為文件會隨著時間而陸續抵達，而新詞彙的表示方式可能會受到用於選擇群集質心的較早詞彙不佳的建模影響。PLAID 串流階層式索引可處理 TB 等級的時間文字 (PLAID SHIRTTT) 透過基於階層分片的多分段漸進式索引來解決這個問題。在 ClueWeb09 和多語言 NeuCLIR 彙編上的實驗分別證明了這種方法的有效性，分別適用於 ColBERT 架構迄今為止索引的最大彙編和多語言設定。
...
摘要：大型語言模型（LLM）在各種領域展現出驚人的潛力，但它們通常缺乏存取和推理特定領域知識和工具的能力。在本文中，我們介紹了 CACTUS（化學代理連接工具使用到科學），一種基於 LLM 的代理，它整合了化學資訊學工具，以在化學和分子發現中實現先進的推理和問題解決。我們使用一組多樣化的開源 LLM，包括 Gemma-7b、Falcon-7b、MPT-7b、Llama2-7b 和 Mistral-7b，在數千個化學問題的基準上評估 CACTUS 的效能。我們的結果表明，CACTUS 明顯優於基準 LLM，而 Gemma-7b 和 Mistral-7b 模型在不考慮提示策略的情況下實現了最高的準確度。此外，我們探討了特定領域提示和硬體配置對模型效能的影響，強調了提示工程的重要性以及在消費者級硬體上部署較小模型的潛力，而準確度不會顯著下降。通過將開源 LLM 的認知能力與特定領域的工具相結合，CACTUS 可以協助研究人員完成分子性質預測、相似性搜尋和藥物相似性評估等任務。此外，CACTUS 代表了化學資訊學領域的一個重要里程碑，為從事化學和分子發現的研究人員提供了一個適應性工具。通過整合開源 LLM 的優勢和特定領域的工具，CACTUS 有可能加速科學進步，並在探索新的、有效和安全的治療候選物、催化劑和材料方面開闢新的領域。此外，CACTUS 與自動化實驗平台整合並實時做出資料驅動決策的能力，為自主發現開闢了新的可能性。
...
摘要：一对一辅导被广泛认为是一种有效的教学方法，条件是辅导员合格。然而，对合格辅导员的高需求仍然是一个挑战，通常需要对新手辅导员（即受训者）进行培训，以确保有效的辅导。研究表明，提供及时的解释性反馈可以促进受训者的培训过程。然而，由于人类专家评估受训者表现的耗时性质，它带来了挑战。受大型语言模型 (LLM) 近期进步的启发，我们的研究采用了 GPT-4 模型来构建一个解释性反馈系统。该系统以二进制形式（即正确/不正确）识别受训者的反应，并根据 GPT-4 模型适当重新表述的反应自动提供基于模板的反馈。我们对来自受训者的 410 份答复进行了研究，涵盖三个培训课程：给予有效的表扬、对错误做出反应以及确定学生知道什么。我们的研究结果表明：1）使用少样本学习方法，GPT-4 模型有效地识别了来自三个培训课程的正确/不正确的受训者反应，平均 F1 分数为 0.84，AUC 分数为 0.85；2）使用少样本学习方法，GPT-4 模型巧妙地将不正确的受训者反应重新表述为所需的反应，实现的性能与人类专家的性能相当。
...
摘要：Whisper 是一款多任務且支援多國語言的語音模型，涵蓋 99 種語言。
它在所涵蓋語言的子集中產生了令人稱讚的自動語音辨識 (ASR) 結果，但該模型在數量不小的代表性不足語言中仍表現不佳，這個問題在較小的模型版本中會更加嚴重。在這項工作中，我們檢視了它的限制，證明了說話者相關 (性別、年齡) 和模型相關 (資源豐富度和模型大小) 偏差的存在。儘管如此，我們發現只有模型相關偏差會因量化而放大，影響更多資源不足的語言和較小的模型。為了尋找更好的壓縮方法，我們提出了 DistilWhisper，一種能夠彌合這些語言的 ASR 效能差距，同時保留多任務和多語言功能優勢的方法。我們的做法包含兩個關鍵策略：使用特定語言專家對 whisper-small 進行輕量級模組化 ASR 微調，以及從 whisper-large-v2 進行知識蒸餾。這種雙管齊下的方法讓我們能夠有效提升 ASR 效能，同時保持從多任務和多語言預訓練繼承而來的穩健性。結果證明，我們的方法比標準微調或 LoRA 適配器更有效，提升了目標語言在域內和域外測試集中的效能，同時在推論中僅引入了可忽略的參數開銷。
...
摘要：<paragraph>在本研究中，我們引入了生成式製造系統 (GMS)，作為有效管理和協調自主製造資產的新方法，從而增強其響應能力和靈活性，以滿足廣泛的生產目標和人類偏好。偏離傳統的明確建模，GMS 採用生成式 AI，包括擴散模型和 ChatGPT，從設想的未來進行隱式學習，標誌著從模型最優決策轉向訓練抽樣決策。通過整合生成式 AI，GMS 能夠通過與人類的互動對話進行複雜的決策，允許製造資產生成多個高品質的全局決策，這些決策可以基於人類的回饋進行反覆改進。實證研究表明，GMS 大幅改善了系統的復原力和對不確定性的響應能力，決策時間從幾秒縮短到幾毫秒。該研究強調了生成解決方案中固有的創造力和多樣性，通過無縫且持續的人機互動促進以人為中心的決策。</paragraph>
...
摘要：不安的的多臂老虎機（RMAB）在建模順序決策問題中扮演著核心角色，在即時啟動約束下，最多 B 個手臂可以在任何決策時期被啟動。每個不安的手臂被賦予一個狀態，這個狀態會根據馬可夫決策過程獨立演化，而不管是否被啟動。在本文中，我們考慮在具有未知轉換函數和對抗獎勵的 RMAB 中學習的任務，這些獎勵可以在各個時期中任意改變。此外，我們考慮一個具有挑戰性但自然的賭徒回饋設定，其中只有已啟動手臂的對抗獎勵會被揭露給決策者（DM）。DM 的目標是在學習過程中最大化其總對抗獎勵，同時在每個決策時期必須滿足即時啟動約束。我們開發了一種新穎的強化學習演算法，它有兩個關鍵貢獻者：一個新穎的偏差對抗獎勵估計器，用於處理賭徒回饋和未知轉換，以及一個低複雜度索引策略，用於滿足即時啟動約束。我們展示了演算法的 $\tilde{\mathcal{O}}(H\sqrt{T})$ 後悔界限，其中 $T$ 是時期數，$H$ 是時期長度。據我們所知，這是第一個演算法，可以確保在我們考慮的具有挑戰性的設定中對抗 RMAB 的 $\tilde{\mathcal{O}}(\sqrt{T})$ 後悔。
...
摘要：本研究提出一個系統性架構，用於比較大型語言模型 (LLM) 在各種化學資訊學任務中進行微調的效能。採用統一的訓練方法，我們評估了三個著名的模型 RoBERTa、BART 和 LLaMA，以其使用簡化的分子輸入線條輸入系統 (SMILES) 作為通用分子表示格式來預測分子屬性的能力。我們的比較分析涉及預訓練這些模型的 18 種配置，其中參數大小和資料集規模各不相同，然後在 DeepChem 的六個基準任務上對它們進行微調。我們在所有模型中維護一致的訓練環境，以確保可靠的比較。這種方法使我們能夠評估模型類型、大小和訓練資料集大小對模型效能的影響。具體來說，我們發現基於 LLaMA 的模型通常提供最低的驗證損失，表明它們在任務和規模上的適應性更強。然而，我們觀察到絕對驗證損失並非模型效能的決定性指標——與先前的研究相矛盾——至少對於微調任務而言：相反，模型大小發揮著至關重要的作用。通過嚴格的複製和驗證，包括多個訓練和微調週期，我們的研究不僅描述了每種類型模型的優點和缺點，還提供了一種用於選擇最適合特定化學資訊學應用程式的 LLM 的強健方法。本研究強調了在部署 AI 進行分子屬性預測時考慮模型架構和資料集特徵的重要性，為在藥物發現和相關領域更明智和有效地利用 AI 鋪平了道路。
...
摘要：同理心需要換位思考：同理心的回應需要一個人推論另一個人經歷過什麼，並用語言傳達這種理解。然而，大多數對同理心的自然語言處理方法並未明確建模此對齊程序。在此，我們引入一種新的方法來識別同理心言語中的對齊，其基礎是評估理論。我們引入了一個新的數據集，其中包含對某人經歷的不同類型評估的 9.2K 以上跨度級別註釋，以及說話者和觀察者言語之間的 3K 以上同理心對齊。通過計算實驗，我們表明這些評估和對齊可以被準確識別。在超過 920 萬 Reddit 對話的實驗中，我們發現評估捕捉到了有意義的行為分組，但大多數回應的對齊程度很低。然而，我們發現心理健康專業人員參與了更多同理心對齊。
...
摘要：沟通被定义为「谁对谁说什么，产生什么效果」。来自沟通者的讯息会产生下游的接收者效果，也称为行为。接收者行为作为讯息的下游效果，会携带丰富的讯息。即使在携带讯息的讯号后，行为资料在训练大型语言模型时往往会被忽略。我们显示出在接收者行为上训练 LLM 实际上有助于提升其内容理解能力。具体来说，我们显示出训练 LLM 来预测按赞和评论的接收者行为，可以提升 LLM 在各种下游内容理解任务上的表现。我们在超过 23 个基准数据集上的 40 个影片和图像理解任务上显示出这种表现提升，在零次学习和微调设定中都优于许多监督基准。此外，由于接收者行为（例如按赞和评论）在网路世界中是默认收集的，而且不需要任何人工标注即可发挥作用，因此我们在训练此资料后获得的表现提升基本上是免费的午餐。我们发布了从多个平台收集的 75 万张图像和影片的接收者行为已清理评论和按赞，以及我们的指令微调资料。

##### **EchoScene: Indoor Scene Generation via Information Echo over Scene Graph Diffusion**
2405.00915v1 by Guangyao Zhai,Evin Pınar Örnek,Dave Zhenyu Chen,Ruotong Liao,Yan Di,Nassir Navab,Federico Tombari,Benjamin Busam
...
摘要：我們提出 EchoScene，一個互動且可控的生成模型，可以在場景圖上生成 3D 室內場景。EchoScene 利用雙分支擴散模型，動態適應場景圖。現有方法由於節點數量不同、多重邊緣組合和操作員誘發的節點邊緣操作，難以處理場景圖。EchoScene 通過將每個節點與去噪程序關聯起來克服了這個問題，並啟用了協作資訊交換，增強了可控和一致的生成，並意識到全局約束。這通過形狀和佈局分支中的資訊迴聲方案實現。在每個去噪步驟中，所有程序都會與資訊交換單元共享其去噪資料，該單元使用圖形卷積結合這些更新。該方案確保去噪程序受到對場景圖的整體理解的影響，從而促進生成全局相干場景。產生的場景可以在推論期間通過編輯輸入場景圖和採樣擴散模型中的噪聲來操作。大量的實驗驗證了我們的做法，它保持了場景的可控性，並在生成保真度方面超越了以前的方法。此外，生成的場景品質很高，因此直接與現成的紋理生成相容。程式碼和訓練模型是開源的。
...
摘要：背景與目的：在缺血性中風中，確定血栓栓塞來源對於治療和二次預防至關重要，但通常無法確定。本研究描述了數字病理學中栓塞的自監督深度學習方法，用於根據組織病理學圖像對缺血性中風血塊來源進行分類。方法：該數據集包括來自 STRIP AI Kaggle 挑戰賽的全幻燈片圖像 (WSI)，其中包含機械血栓切除術後缺血性中風患者的檢回血塊。使用遷移學習和自監督預訓練開發了基於變換器的深度學習模型，用於對 WSI 進行分類。自定義包括注意力池化層、加權損失函數和閾值優化。測試並比較了各種模型架構，並主要使用加權對數損失評估模型性能。結果：該模型在交叉驗證中實現了 0.662 的對數損失分數，在測試集中實現了 0.659。比較了不同的模型主幹，其中 swin_large_patch4_window12_384 表現出更高的性能。採用了血塊來源分類的閾值技術來平衡假陽性和假陰性。結論：該研究證明了基於變換器的深度學習模型在根據組織病理學圖像識別缺血性中風血塊來源方面的有效性，並強調了專門針對血栓 WSI 調整建模技術的必要性。需要進一步的研究來提高模型性能、可解釋性，驗證其有效性。未來的改進可以包括整合更大的患者群組、先進的預處理策略，以及探索集成多模式方法以提高診斷準確性。
...
摘要：視覺轉換器徹底改變了電腦視覺，但其運算需求對訓練和部署提出了挑戰。本文介紹 LOTUS（具有超稀疏性的樂透轉換器），這是一種新穎的方法，它利用資料樂透選票選擇和稀疏剪枝來加速視覺轉換器訓練，同時維持準確度。我們的做法專注於識別和利用最多資訊的資料子集，並消除多餘的模型參數，以最佳化訓練流程。透過廣泛的實驗，我們證明了 LOTUS 在顯著降低運算需求的同時，能快速收斂並獲得高準確度的效能。這項工作突顯了結合資料選擇和稀疏技術以進行有效率的視覺轉換器訓練的潛力，為這個領域進一步的研究和發展開啟了大門。
...
摘要：在過去的十年中，與其他應用領域類似，社群媒體內容已被證明在災害資訊學中非常有效。然而，由於資料結構化程度低，社群媒體內容的災害分析面臨著許多挑戰。為了充分探索社群媒體內容在災害資訊學中的潛力，存取相關內容和正確的地理位置資訊非常重要。在本文中，我們提出了一個三步驟解決方案來應對這些挑戰。首先，提出的解決方案旨在將社群媒體貼文分類為相關和不相關的貼文，然後通過命名實體識別 (NER) 分析從貼文的文字中自動擷取位置資訊。最後，為了快速分析大量社群媒體貼文中涵蓋的主題，我們執行主題建模，產生一個熱門關鍵字清單，重點說明推文中討論的問題。對於 Twitter 貼文相關分類 (RCTP)，我們提出了一個基於優點的融合架構，結合四個不同模型（BERT、RoBERTa、Distil BERT 和 ALBERT）的能力，在基準資料集上獲得最高的 0.933 F1 分數。對於從 Twitter 文字中提取位置 (LETT)，我們在 NER 框架中評估了四個模型（BERT、RoBERTa、Distil BERTA 和 Electra），獲得最高的 0.960 F1 分數。對於主題建模，我們使用 BERTopic 函式庫來發現相關推文中的隱藏主題模式。所提出的端到端解決方案所有組成的實驗結果非常令人鼓舞，並暗示了社群媒體內容和 NLP 在災害管理中的潛力。
...
摘要：大型語言模型顯得相當有創造力，在創意任務上往往與一般人表現得不相上下。然而，對 LLM 創造力的研究僅專注於「產品」，鮮少關注創造「過程」。對人類創造力的過程分析通常需要手動編碼類別或運用反應時間，這並不適用於 LLM。我們提供了一種自動化方法來描述人類和 LLM 在替代用途任務中如何探索語義空間，並與在言語流利度任務中的行為形成對比。我們使用句子嵌入來識別回應類別和計算語義相似性，我們用它們來產生跳躍輪廓。我們的結果證實了人類先前的研究，報告了持續（在少數語義空間中深入搜尋）和靈活（在多個語義空間中廣泛搜尋）兩種創造力途徑，這兩種途徑都通向類似的創造力評分。發現 LLM 偏向於持續或靈活的途徑，這會因任務而異。儘管 LLM 作為一個群體符合人類的輪廓，但它們與創造力的關係不同，其中較靈活的模型在創造力上得分較高。我們的數據集和腳本可在 \href{https://github.com/surabhisnath/Creative_Process}{GitHub} 上取得。
...
摘要：<paragraph>在極低功耗裝置上進行機器學習應用，通常稱為微型機器學習 (TinyML)，承諾提供更智慧、更緊密連結的世界。然而，當前微型機器學習研究的進展受到相關資料集大小和品質的限制。為了應對此項挑戰，我們推出 Wake Vision，一個針對人員偵測量身打造的大規模、多樣化資料集——微型機器學習視覺感測的典型任務。Wake Vision 包含超過 600 萬張影像，與之前的標準相比增加了數百倍，並且經過徹底的品質過濾。使用 Wake Vision 進行訓練，與既定的基準相比，準確度提高了 2.41%。除了資料集之外，我們還提供了一組五個詳細的基準集，用來評估模型在測試資料特定區段的效能，例如不同的光照條件、與相機的距離，以及主體的人口特徵。這些新穎的細緻基準有助於評估模型品質在具有挑戰性的真實世界場景中，這些場景在僅專注於整體準確度時常常被忽略。透過在基準上評估 MobileNetV2 TinyML 模型，我們發現輸入解析度在偵測遠處主體時扮演比模型寬度更重要的角色，並且由於資料集品質，量化對模型穩健性的影響很小。這些發現強調了詳細評估對於找出模型開發基本要素的重要性。資料集、基準套件、程式碼和模型在 CC-BY 4.0 授權下公開提供，讓它們能夠用於商業案例。</paragraph>
...
摘要：傳統語言模型以自回歸的方式運作，亦即一次預測一個符號。模型規模的快速爆炸導致推論時間長。在這項工作中，我們提出 DynaMo，這是一組多符號預測語言模型，可減少網路推論時間。我們的模型根據對預測聯合機率分佈的信心，動態預測多個符號。我們提出了一種輕量級技術來訓練這些模型，利用傳統自回歸對應項的權重。此外，我們提出了增強估計聯合機率的新方法，以提高文字生成品質，即共現加權遮罩和自適應閾值。我們還提出了系統性的定性和定量方法，以嚴格測試非自回歸生成的生成文字品質。我們套件中的其中一個模型 DynaMo-7.3B-T3，在與基準（Pythia-6.9B）相同品質的生成文字的同時，分別以僅 5.87% 和 2.67% 的參數和訓練時間開銷，實現了 2.57 倍的加速。
...
摘要：視覺語言模型 (VLM) 近期浮現並備受矚目，原因在於它們能夠理解影像和文字資料的雙重樣態。LLaVA、ChatGPT-4 和 Gemini 等 VLM 最近在自然影像標題、視覺問題解答 (VQA) 和空間推理等任務上展現了令人印象深刻的表現。此外，Meta AI 推出的通用分割模型 Segment Anything Model (SAM) 在從未見過的影像中分離物體方面展現了前所未有的表現。由於醫學專家、生物學家和材料科學家例行檢視顯微鏡或醫學影像，並結合標題、文獻或報告等形式的文字資訊，並得出極為重要且有價值的結論，因此無庸置疑地，在這些影像上測試 VLM 和 SAM 等基礎模型的表現至關重要。在這項研究中，我們賦予 ChatGPT、LLaVA、Gemini 和 SAM 在各種顯微鏡影像上進行分類、分割、計數和 VQA 任務。我們觀察到 ChatGPT 和 Gemini 能夠令人印象深刻地理解顯微鏡影像中的視覺特徵，而 SAM 則相當擅長於廣義上分離人工製品。然而，表現並未接近領域專家 - 模型很容易受到影像中雜質、缺陷、人工製品重疊和多樣性的影響。
...
摘要：自動化軟體測試是軟體開發過程中不可或缺的一部分，它能簡化工作流程並確保產品的可靠性。在此背景下的視覺測試，特別是關於使用者介面 (UI) 和使用者體驗 (UX) 的驗證，是決定整體軟體品質的關鍵因素之一。儘管如此，像逐像素比較和基於區域的視覺變化偵測等傳統方法，在捕捉情境相似性、細微變更以及理解 UI 元素之間的空間關係方面仍有不足。在本文中，我們介紹了一種用於軟體測試自動化中的視覺變化偵測的新型圖形化方法。我們的這種方法利用機器學習模型，能從軟體螢幕截圖中準確識別 UI 控制項，並建立一個表示控制項之間的情境和空間關係的圖形。然後使用這些資訊來找出不同版本軟體螢幕截圖中 UI 控制項之間的對應關係。產生的圖形包含了 UI 的複雜佈局和底層情境關係，提供了一個整體且具備情境感知能力的模型。此模型最後用於偵測和突顯 UI 中的視覺回歸。針對不同資料集進行的綜合實驗顯示，我們的變更偵測器能準確偵測各種簡單和複雜測試場景中的視覺軟體變更。此外，在更複雜的測試場景中，它比逐像素比較和基於區域的基準表現好很多。這項工作不僅有助於視覺變化偵測的進步，也具有實際意義，為現實世界的軟體測試自動化挑戰提供了強大的解決方案，增強了可靠性，並確保了軟體介面的無縫演進。
...
摘要：多項選擇題 (MCQ) 是一種評量學生知識的熱門方法，因為它們在管理和評分方面效率高。
編寫高品質的數學 MCQ 是一個勞力密集的過程，需要教育工作者制定精確的題幹和合理的干擾選項。
大型語言模型 (LLM) 的最新進展引起了自動化 MCQ 建立的興趣，但在確保數學準確性和解決學生錯誤方面仍然存在挑戰。
本文介紹了一個原型工具，旨在促進 LLM 和教育工作者之間的協作，以簡化數學 MCQ 生成過程。
我們進行了一項涉及數學教育者的試點研究，以調查該工具如何幫助他們簡化編寫高品質數學 MCQ 的過程。
我們發現，雖然 LLM 可以生成結構良好的題幹，但它們生成能捕捉常見學生錯誤和誤解的干擾選項的能力有限。
儘管如此，人類與 AI 的協作有可能提高 MCQ 生成的效率和效能。
...
摘要：普遍存在的不合理信念，包括偽科學、邏輯謬誤和陰謀論，在社會上造成重大的阻礙，並有散播錯誤訊息的風險。本研究利用既定的心理測量評估，探討大型語言模型 (LLM) 相對於一般人，在偵測普遍存在的邏輯陷阱方面的能力。我們進行哲學探討，將人類的理性與 LLM 的理性並置。此外，我們提出利用 LLM 來對抗錯誤觀念的方法，並利用認知失調理論和訊息接收可能性理論等心理說服模型。透過這項努力，我們強調 LLM 作為個人化錯誤訊息揭穿代理的潛力。
...
摘要：在本文中，我們提出 Sim-Grasp，一個強健的 6-DOF 雙指抓取系統，它整合了先進的語言模型，以增強在混亂環境中的物體操作。我們介紹了 Sim-Grasp-Dataset，其中包含 500 個場景中的 1,550 個物體，帶有 790 萬個註解標籤，並開發了 Sim-GraspNet 以從點雲生成抓取姿勢。Sim-Grasp-Polices 對於單個物體實現了 97.14% 的抓取成功率，對於 1-2 級和 3-4 級物體的混合混亂場景分別實現了 87.43% 和 83.33% 的抓取成功率。通過將語言模型整合到目標識別中（通過文本和框提示），Sim-Grasp 能夠進行與物體無關的和目標拾取，從而突破了智慧機器人系統的界限。
...
摘要：<paragraph>分散式多智能體學習 (DML) 能在保留資料隱私的同時進行協作模型訓練。然而，智能體資源（運算、通訊和任務大小）中內在的異質性可能導致訓練時間出現顯著差異。這種異質性會造成瓶頸，由於落後效應而延長整體訓練時間，並可能浪費較快智能體的備用資源。為了在異質環境中將訓練時間降至最低，我們提出分散式多智能體學習的通訊高效訓練工作負載平衡 (ComDML)，它透過分散式方法在智能體之間平衡工作負載。ComDML 採用局部損失分割訓練，能進行並行更新，其中較慢的智能體會將部分工作負載卸載給較快的智能體。為了將整體訓練時間降至最低，ComDML 會透過同時考量智能體的通訊和運算能力來最佳化工作負載平衡，這取決於整數規劃。開發了一種動態分散式配對排程器，以有效配對智能體並確定最佳卸載量。我們證明在 ComDML 中，對於凸函數和非凸函數，較慢和較快的智能體模型都會收斂。此外，在熱門資料集（CIFAR-10、CIFAR-100 和 CINIC-10）及其非 I.I.D. 變體上進行的廣泛實驗結果，以及 ResNet-56 和 ResNet-110 等大型模型，證明 ComDML 能顯著減少整體訓練時間，同時維持模型準確度，與最先進的方法相比。ComDML 在異質環境中展現了穩健性，而且可以無縫整合隱私措施以增強資料保護。</paragraph>
...
摘要：<paragraph>我們提出 WIBA，這是一個新穎的架構和方法套件，可讓您全面了解不同脈絡中的「論點」。我們的做法發展了一個全面的架構，用於偵測：(a) 存在性，(b) 主題，以及 (c) 論點的立場，並正確考量這三個任務之間的邏輯依賴性。我們的演算法利用大型語言模型的微調和提示工程。我們評估我們的做法，並證明它在所有這三項能力中表現良好。首先，我們開發並發布了一個論點偵測模型，該模型可以將一段文字分類為論點，在三個不同的基準資料集上的 F1 分數介於 79% 到 86% 之間。其次，我們發布了一個語言模型，它可以識別句子中被爭論的主題，無論是隱含的還是明確的，平均相似度分數為 71%，比目前的樸素方法高出近 40%。最後，我們開發了一種論點立場分類方法，並評估了我們方法的能力，證明它在三個不同的基準資料集上達到了 71% 到 78% 之間的分類 F1 分數。我們的評估表明，WIBA 允許全面了解在不同脈絡中的大型語料庫中「被爭論的內容」，這對於語言學、溝通以及社會和電腦科學中的許多應用來說都是核心興趣。為了促進取得本工作中概述的進展，我們將 WIBA 作為一個免費的開放存取平台 (wiba.dev) 發布。</paragraph>
...
摘要：客戶服務是公司與其客戶互動的方式。它可以大幅提升整體客戶滿意度。然而，高品質的服務可能會變得昂貴，造成讓服務盡可能成本效益的誘因，並促使大多數公司利用 AI 驅動的助理或「聊天機器人」。另一方面，客戶仍然渴望人與人的互動，特別是在遇到複雜情況時，例如爭議和帳單付款等敏感主題。
這提高了客戶服務人員的門檻。他們需要準確理解客戶的問題或疑慮，找出可接受且可行的解決方案（且符合公司政策），同時處理多個對話。
在這項工作中，我們將「問我任何事」(AMA) 作為一個附加功能，新增到面向代理商的客戶服務介面。AMA 讓代理商在處理客戶對話時，可以依需求向大型語言模型 (LLM) 提出問題，而 LLM 會即時提供準確的回應，減少代理商需要切換脈絡的次數。在我們的內部實驗中，我們發現使用 AMA 而非傳統搜尋體驗的代理商，在包含搜尋的對話中，每段對話花費的時間減少約 10%，換算下來每年可節省數百萬美元。使用 AMA 功能的代理商有將近 80% 的時間提供正面的回饋，證明其作為客戶服務的 AI 輔助功能的實用性。
...
摘要：大型文字轉圖像模型可根據文字提示產生高品質且多樣化的圖像，已展現卓越的成功。這些模型最終旨在建立複雜場景，而處理多主體生成的挑戰是邁向此目標的關鍵步驟。然而，現有的最先進擴散模型在產生包含多個主體的圖像時會遇到困難。當出現包含多個主體的提示時，這些模型可能會省略某些主體或將它們合併在一起。為了解決此挑戰，我們提出了一個基於指導原則的新方法。我們允許擴散模型最初提出一個版面，然後我們重新排列版面網格。這是透過強制跨注意力圖 (XAM) 遵守建議的遮罩，並將像素從潛在圖遷移到我們確定的新位置來實現的。我們引入了新的損失項，旨在降低 XAM 熵以更清楚地定義主體的空間，減少 XAM 之間的重疊，並確保 XAM 與它們各自的遮罩對齊。我們將我們的方法與幾種替代方法進行對比，並表明它更忠實地捕捉了各種文字提示中的所需概念。
...
摘要：<paragraph>新興的多模式工作負載加上大型語言模型等大型模型，顯著增加了對硬體的運算和記憶體需求。為了解決這些日益增長的需求，設計可擴充的硬體架構已成為一個關鍵問題。在最近的解決方案中，基於 2.5D 矽介面層多晶片模組 (MCM) 的 AI 加速器由於其在低工程成本和可組合性方面的顯著優勢，已被積極探索為一種有前途的可擴充解決方案。然而，以前的 MCM 加速器基於具有固定資料流的同質架構，由於其工作負載適應性有限，因此在高度異質的多模式工作負載中遇到了重大挑戰。因此，在這項工作中，我們探索了異質資料流 MCM AI 加速器中的機會。我們發現，在異質資料流 MCM AI 加速器上安排多模式工作負載是一個重要且具有挑戰性的問題，因為其重要性和規模，即使對於 6x6 晶片上的單一模型案例，也達到 O(10^18) 的規模。我們開發了一組啟發法來導航龐大的排程空間，並將它們編入一個排程器中，其中包含先進的技術，例如晶片間流水線。我們對資料中心多租戶和 AR/VR 使用案例的十個多模式工作負載場景的評估顯示了我們方法的有效性，與同質基線相比，對於各自的應用程式設定，平均節省了 35.3% 和 31.4% 的能量延遲積 (EDP)。</paragraph>
...
摘要：傳統的人類回饋強化學習 (RLHF) 方法依賴於 Bradley-Terry 模型等參數模型，在捕捉人類偏好中的非傳遞性和非理性方面力有未逮。最近的進展表明，直接使用偏好機率可以更準確地反映人類偏好，從而實現更靈活、更準確的語言模型對齊。在本文中，我們提出了一種基於自博弈的語言模型對齊方法，將問題視為一個恆和雙人遊戲，旨在找出納什均衡策略。我們的做法稱為「自博弈偏好優化」(SPPO)，通過迭代策略更新來近似納什均衡，並享有理論收斂保證。我們的做法可以有效增加所選回應的對數似然性，並減少被拒絕回應的對數似然性，這無法通過對稱成對損失（例如直接偏好優化 (DPO) 和身分偏好優化 (IPO)）輕易實現。在我們的實驗中，僅使用 UltraFeedback 資料集中的 60k 個提示（沒有回應），且沒有任何提示擴充，通過利用僅有 0.4B 個參數的預訓練偏好模型 PairRM，SPPO 可以從微調 Mistral-7B-Instruct-v0.2 中獲得一個模型，在 AlpacaEval 2.0 上對抗 GPT-4-Turbo 時，達到 28.53% 的長度控制勝率，這是目前最先進的技術。它還在 MT-Bench 和 Open LLM 排行榜上優於（迭代）DPO 和 IPO。值得注意的是，SPPO 的強勁表現是在沒有來自 GPT-4 或其他更強大的語言模型的額外外部監督（例如回應、偏好等）的情況下實現的。
...
摘要：本研究針對最新的大型語言模型 Llama-3 提出了一種目標模型編輯分析。我們探討了熱門模型編輯技術的功效，包括 ROME、MEMIT 和 EMMET，這些技術專為精準的層級干預而設計。我們透過評估來找出最有效的目標編輯層級，評估涵蓋了三種不同策略共 4096 次編輯：順序編輯、批次編輯，以及我們稱為順序-批次編輯的混合方法。我們的發現顯示，增加編輯批次大小可能會比使用較小的編輯批次來進行等量編輯更顯著地降低模型效能。因此，我們認為順序模型編輯是擴展模型編輯方法的重要組成部分，未來的研究應著重於結合批次編輯和順序編輯的兩種方法。此觀察結果顯示了當前模型編輯方法中潛在的限制，這些方法推動了更大的編輯批次大小，我們希望它能為未來優化批次大小和模型編輯效能的研究鋪路。
...
摘要：语义文本相关性是语义相似性的一个更广泛的概念。它衡量两段文本传达相似含义或主题的程度，或共享相关概念或上下文。这种相关性的概念可应用于各种应用程序，例如文档聚类和摘要。SemRel-2024 是 SemEval-2024 中的一项共享任务，旨在通过为包括阿拉伯语在内的 14 种语言和方言提供数据集来缩小语义相关性任务中的差距。本文报告了我们参与 A 组（阿尔及利亚和摩洛哥方言）和 B 组（现代标准阿拉伯语）的情况。BERT 模型被增强并针对监督轨道（A）中的回归评分进行了微调，而 BERT 余弦相似度则用于无监督轨道（B）。我们的系统在 SemRel-2024 中的 MSA 排名第 1，Spearman 相关系数为 0.49。我们在摩洛哥语中排名第 5，在阿尔及利亚语中排名第 12，得分分别为 0.83 和 0.53。
...
摘要：對於長篇文件摘要，語篇結構對於辨別文本的關鍵內容和句子之間的重要性差異非常重要。不幸的是，將語篇結構理論 (RST) 整合到長篇文件摘要的參數有效微調策略中仍未被探討。因此，本文介紹了 RST-LoRA，並提出了四個 RST 感知變體，以將 RST 明確納入 LoRA 模型中。我們的經驗評估表明，納入語篇關係的類型和不確定性可以互補地增強 LoRA 在摘要任務中的表現。此外，我們引入的效能最佳變體優於原始 LoRA 和全參數微調模型，這已通過多項自動和人工評估得到證實，甚至超越了先前的最先進方法。
...
摘要：在不確定的環境中安全地規劃，代理人必須平衡效用與安全限制。安全的規劃問題可以建模為機會約束的部分可觀察馬可夫決策過程 (CC-POMDP)，而解決方案通常使用昂貴的展開或啟發式方法來估計最佳值和動作選擇策略。這項工作引入了約束零策略迭代演算法，它透過學習最佳值和策略的神經網路近似值來解決信念空間中的 CC-POMDP，並使用額外的網路頭來估計給定信念下的失敗機率。這個失敗機率會在線上蒙地卡羅樹狀搜尋 (MCTS) 期間引導安全的動作選擇。為了避免過度強調基於失敗估計的搜尋，我們引入了 $\Delta$-MCTS，它使用適應共形推論在規劃期間更新失敗閾值。這種方法在安全關鍵 POMDP 基準、飛機防撞系統和安全的 CO$_2$ 儲存永續性問題上進行了測試。結果表明，透過將安全約束與目標分開，我們可以在不最佳化獎勵和成本之間的平衡下實現目標安全等級。
...
摘要：最近的研究透過訓練後的量化或低位元權重表示，介紹了大型語言模型 (LLM) 的有效壓縮技術。
儘管量化的權重提供了儲存效率並允許更快速的推論，但現有研究表明量化可能會損害效能並加劇 LLM 中的偏差。這項研究調查了量化模型的信心和校準，考慮了語言模型類型和規模等因素作為量化損失的貢獻者。首先，我們揭示了使用 GPTQ 將權重量化到 4 位元會導致對真實標籤的信心下降，在不同的語言模型中觀察到的影響也有所不同。其次，我們觀察到不同規模對信心影響的波動。最後，我們根據信心水準提出了量化損失的解釋，表明量化對原本完整模型信心水準低的樣本影響不成比例。
...
傳達它們的不確定性。然而，幾乎沒有實證研究探討使用者如何
感知和依循 LLM 對不確定性的表達。我們透過一項大規模、預先
註冊的人類受試者實驗 (N=404) 來探討這個問題，其中參與者回答
醫療問題，有或沒有存取虛構的 LLM 注入式搜尋引擎的回應。
我們使用行為和自我報告的測量方法，探討不確定性的不同自然語言
表達如何影響參與者的依賴性、信任度和整體任務表現。我們發現
第一人稱表達（例如「我不確定，但...」）會降低參與者對系統的
信心，以及同意系統答案的傾向，同時提高參與者的準確性。探索性
分析顯示，這種提升可以歸因於對不正確答案的過度依賴減少（但
並未完全消除）。雖然我們觀察到對不確定性表達的一般觀點（例
如「不清楚，但...」）有類似的影響，但這些影響較弱且沒有
統計意義。我們的研究結果表明，使用不確定性的自然語言表達可
能是減少對 LLM 過度依賴的有效方法，但具體使用的語言很重要。
這突顯了在大規模部署 LLM 之前進行使用者測試的重要性。</paragraph>
...
摘要：AI 基礎模型在各種應用中獲得關注，包括放射學等醫療領域。然而，醫療基礎模型通常在有限的任務上進行測試，而其泛化性和偏差尚未得到探討。我們提出 RayDINO，這是一個通過 873k 胸部 X 光片上的自我監督訓練的大型視覺編碼器。我們將 RayDINO 與先前的最新模型進行比較，涵蓋九項放射學任務，從分類和密集分割到文本生成，並深入分析我們模型的人口、年齡和性別偏差。我們的研究結果表明，自我監督允許以患者為中心的 AI 在臨床工作流程中發揮作用並全面解釋 X 光片。通過 RayDINO 和小型特定任務適配器，我們達到了最新技術，並改善了對未知人群的泛化，同時減輕了偏差，說明了基礎模型的真正前景：通用性和魯棒性。
...
摘要：論證結構學習 (ASL) 涉及預測論證之間的關係。由於它可以建構文件以利於理解，因此已廣泛應用於許多領域（醫療、商業和科學領域）。儘管其廣泛的應用，ASL 仍然是一項具有挑戰性的任務，因為它涉及檢查潛在非結構化論述中句子之間的複雜關係。為了解決這個問題，我們開發了一個簡單但有效的方法，稱為 ASL 任務的雙塔多尺度卷積神經網路 (DMON)。具體來說，我們將論證組織成關係矩陣，該矩陣與論證嵌入一起形成關係張量，並設計一種機制來捕捉與上下文論證的關係。在三個不同領域的論證挖掘資料集上的實驗結果表明，我們的架構優於最先進的模型。程式碼可在 https://github.com/VRCMF/DMON.git 取得。
...
摘要：視覺語言模型 (VLM) 近期浮現並備受矚目，因為它們能理解影像和文字資料的雙重模式。LLaVA、ChatGPT-4 和 Gemini 等 VLM 近期在自然影像標題、視覺問答 (VQA) 和空間推理等任務上展現了令人印象深刻的表現。此外，由 Meta AI 推出的通用分割模型 Segment Anything Model (SAM) 在從未見過的影像中分離物體時展現了前所未見的表現。由於醫學專家、生物學家和材料科學家會例行檢查顯微鏡或醫學影像，並結合標題、文獻或報告形式的文字資訊，並得出極為重要且有價值的結論，因此無庸置疑地，在這些影像上測試 VLM 和 SAM 等基礎模型的表現至關重要。在本研究中，我們賦予 ChatGPT、LLaVA、Gemini 和 SAM 在各種顯微鏡影像上進行分類、分割、計數和 VQA 任務。我們觀察到 ChatGPT 和 Gemini 能夠令人印象深刻地理解顯微鏡影像中的視覺特徵，而 SAM 則相當有能力在一般意義上分離人工製品。然而，表現並未接近領域專家的水準 - 這些模型很容易受到影像中雜質、缺陷、人工製品重疊和多樣性的影響。
...
摘要：<paragraph>廣泛部署的大型語言模型 (LLM) 能產生令人信服但
不正確的輸出，可能會誤導依賴它們的使用者，彷彿它們是正確的。為了減少這種過度依賴，有人呼籲 LLM 向最終使用者傳達它們的不確定性。然而，很少有實證研究探討使用者如何感知和根據 LLM 表達的不確定性採取行動。我們透過一項大規模、預先註冊的人類受試者實驗 (N=404) 來探討這個問題，在實驗中，參與者回答醫療問題，有或沒有使用虛構的 LLM 注入式搜尋引擎的回應。我們使用行為和自我報告測量，探討不確定性的不同自然語言表達如何影響參與者的依賴性、信任度和整體任務表現。我們發現第一人稱表達（例如「我不確定，但...」）會降低參與者對系統的信心，並減少他們同意系統答案的傾向，同時提高參與者的準確性。探索性分析表明，這種提升可以歸因於減少（但並未完全消除）對不正確答案的過度依賴。雖然我們觀察到從一般觀點表達的不確定性（例如「這不清楚，但...」）有類似的效果，但這些效果較弱且在統計上不顯著。我們的研究結果表明，使用不確定性的自然語言表達可能是減少對 LLM 過度依賴的有效方法，但使用的精確語言很重要。這凸顯了在大規模部署 LLM 之前進行使用者測試的重要性。</paragraph>
...
摘要：超音波機器人日益用於醫療診斷和早期疾病篩檢。然而，目前的超音波機器人缺乏理解人類意圖和指示的智慧，阻礙了自主超音波掃描。為了解決這個問題，我們提出了一種創新的超音波具身智慧系統，它為超音波機器人配備了大型語言模型 (LLM) 和領域知識，從而提高了超音波機器人的效率。具體來說，我們首先設計了一個超音波操作知識資料庫，將超音波掃描的專業知識新增到 LLM 中，使 LLM 能夠執行精確的動作規劃。此外，我們根據「思考-觀察-執行」提示工程設計了一個動態超音波掃描策略，允許 LLM 在掃描過程中動態調整動作規劃策略。廣泛的實驗表明，我們的系統顯著提高了超音波掃描效率和品質，從而實現了口頭命令。這種自主醫療掃描技術的進步有助於非侵入性診斷和簡化的醫療工作流程。

##### **A Careful Examination of Large Language Model Performance on Grade School Arithmetic**
2405.00332v2 by Hugh Zhang,Jeff Da,Dean Lee,Vaughn Robinson,Catherine Wu,Will Song,Tiffany Zhao,Pranav Raja,Dylan Slack,Qin Lyu,Sean Hendryx,Russell Kaplan,Michele Lunati,Summer Yue
...
摘要：大型語言模型（LLM）在許多數學推理基準測試中取得令人印象深刻的成功。然而，愈來愈多人擔心，其中一些表現實際上反映了資料集汙染，其中與基準考題非常相似的資料會滲入訓練資料，而不是真正的推理能力。為了嚴謹地調查此說法，我們委託製作小學數學 1000（GSM1k）。GSM1k 旨在反映已建立的 GSM8k 基準的風格和複雜性，GSM8k 基準是衡量小學數學推理的黃金標準。我們確保這兩個基準在人類解決率、解答步驟數、答案大小等重要指標上具有可比性。在 GSM1k 上評估領先的開放和閉源 LLM 時，我們觀察到準確度下降多達 13%，其中幾種類型的模型（例如 Phi 和 Mistral）顯示出幾乎所有模型大小的系統性過度擬合的證據。同時，許多模型，特別是那些處於前沿的模型（例如 Gemini/GPT/Claude）顯示出過度擬合的跡象最少。進一步的分析表明，模型從 GSM8k 生成範例的機率與其在 GSM8k 和 GSM1k 之間的效能差距之間存在正相關關係（Spearman's r^2=0.32），這表示許多模型可能已部分記憶 GSM8k。
...
摘要：<paragraph>語音情緒辨識 (SER) 由於在各種領域中廣泛的應用而備受關注，包括人機互動、虛擬助理和心理健康協助。然而，現有的 SER 方法經常忽略預訓練語音辨識任務和下游 SER 任務之間的資訊差距，導致次優的表現。此外，目前的 SER 方法需要花費大量時間針對每個特定語音資料集進行微調，例如 IEMOCAP，這限制了它們在處理大量雜訊資料的實際場景中的有效性。為了解決這些問題，我們提出了一個基於主動學習 (AL) 的 SER 微調框架，稱為 \textsc{After}，它利用任務適應預訓練 (TAPT) 和 AL 方法來提升效能和效率。具體來說，我們首先使用 TAPT 來最小化預訓練語音辨識任務和下游語音情緒辨識任務之間的資訊差距。然後，採用 AL 方法來反覆選擇最具資訊性和多樣性的樣本子集進行微調，從而減少時間消耗。實驗表明，我們提出的方法 \textsc{After}，僅使用 20% 的樣本，就能將準確度提升 8.45%，並將時間消耗減少 79%。\textsc{After} 的額外延伸和消融研究進一步證實了其在各種實際場景中的有效性和適用性。我們的原始程式碼可在 Github 上取得，以供重現。(https://github.com/Clearloveyuan/AFTER)。</paragraph>
...
摘要：每年，植物寄生線蟲，植物病原體的主要群組之一，會造成全球作物重大損失。為了減輕線蟲造成的作物產量損失，有效率的線蟲監測方法對於植物和作物疾病管理至關重要。在其他方面，有效率的線蟲檢測有助於醫學研究和藥物發現，因為線蟲是模式生物。隨著電腦技術的快速發展，電腦視覺技術提供了量化線蟲或線蟲感染的可行解決方案。在本文中，我們調查並分類了透過深度學習模型進行線蟲檢測的研究和可用資料集。為了刺激相關研究的進展，本調查提出了潛在的最新目標檢測模型、訓練技術、最佳化技術和深度學習初學者的評估指標。此外，七個最先進的目標檢測模型在三個公共資料集和 AgriNema 資料集上進行驗證，以建立植物寄生線蟲檢測的基準。
...
摘要：<paragraph>醫學與人工智慧（AI）工程是兩個截然不同的領域，各自擁有數十年的已發表歷史。有了這段歷史，就有一套術語，其應用方式很具體。然而，當兩個具有重疊術語的不同領域開始合作時，就會發生溝通不良和誤解。本敘述性回顧旨在為這些術語提供歷史背景，強調在醫療 AI 背景中使用這些術語時明確性的重要性，並提供解決方案以減少來自任一領域的讀者的誤解。透過檢視歷史文件，包括文章、撰寫指南和教科書，本回顧追溯了資料集及其影響術語的不同演變。最初，探討了醫學和 AI 背景中「驗證」一詞不一致的詮釋。然後對用於 AI 評估的資料集進行分類，即隨機拆分、交叉驗證、時間、地理、內部和外部集合。這些資料集的準確且標準化的描述對於證明 AI 應用在醫學中的穩健性和普遍性至關重要。本回顧釐清現有文獻，以全面了解這些分類及其在 AI 評估中的含義。然後，本回顧找出經常被誤解的術語，並提出實用的解決方案來減輕術語混淆。這些解決方案包括使用標準化術語，例如「訓練集」、「驗證（或調整）集」和「測試集」，以及在每篇醫學 AI 研究出版物中明確定義資料集拆分術語。本回顧旨在提高醫學 AI 中溝通的精確度，從而促進這個跨學科領域更有效和透明的研究方法。</paragraph>
...
摘要：了解醫療診斷影像中病情的嚴重程度至關重要，這有助於臨床評估、治療，以及評估縱向進展。本文提出 Con-PrO：一種用於評估醫療影像嚴重程度的新穎表徵學習方法，該方法利用對比學習整合偏好最佳化。與最大化類別間距離的傳統對比學習方法不同，ConPrO 將各種嚴重程度類別與正常類別之間的距離偏好知識注入潛在向量中。我們系統性地檢驗我們架構的關鍵組成，以闡明對比預測任務如何獲取有價值的表徵。我們展示我們的表徵學習架構在特徵空間中提供了有價值的嚴重程度排序，同時在分類任務中優於先前的最先進方法。與監督式和自監督式基準相比，我們分別取得了 6% 和 20% 的相對改進。此外，我們對醫療領域的嚴重程度指標和偏好比較相關應用進行了討論。
...
摘要：<paragraph>在胸部 X 光 (CXR) 影像分析領域中，放射科醫師會仔細檢查各個區域，並在報告中記錄他們的觀察結果。CXR 診斷中的錯誤盛行，特別是在缺乏經驗的放射科醫師和住院醫師中，這強調了瞭解放射科醫師的意圖和對應的感興趣區域的重要性。這種理解對於糾正錯誤至關重要，它可以引導放射科醫師關注正確的感興趣區域，特別是在診斷胸部 X 光異常時。為了回應這個需求，我們提出了一個新系統，旨在識別放射科醫師在報告中表達的主要意圖以及 CXR 影像中對應的感興趣區域。此系統試圖闡明放射科醫師文字發現背後的視覺背景，並有可能糾正經驗較少的從業人員所犯的錯誤，並將他們引導至精確的感興趣區域。重要的是，所提出的系統可用於向缺乏經驗的放射科醫師或醫院的初級住院醫師提供建設性的回饋，彌補面對面溝通的差距。此系統是一個有價值的工具，可用於提高診斷準確性，並促進醫學界持續學習。</paragraph>
...
摘要：<paragraph>建立多重感官AI系統，從多重感官輸入中學習，例如文字、語音、影片、真實世界感測器、穿戴式裝置和醫療資料，在許多科學領域中承諾產生重大影響，並具有實際效益，例如支持人類健康和福祉、支援多媒體內容處理，以及提升真實世界自主代理。透過綜合一系列理論架構和應用領域，本論文旨在推進多重感官AI的機器學習基礎。在第一部分，我們提出一個理論架構，正式說明模態如何彼此互動，以產生任務的新資訊。這些互動是所有多模態問題中的基本建構模組，它們的量化使使用者能夠了解他們的多模態資料集，設計有原則的方法來學習這些互動，並分析他們的模型是否成功學習。在第二部分，我們研究實用多模態基礎模型的設計，該模型概括許多模態和任務，這表示朝向將大型語言模型基礎化到真實世界感官模態邁進一步。我們介紹MultiBench，這是一個統一的大規模基準，涵蓋各種模態、任務和研究領域，接著是跨模態注意力和多模態轉換器架構，這些架構現在支撐許多現今的多模態基礎模型。在MultiBench上擴充這些架構，能夠建立通用多重感官AI系統，我們討論我們在應用這些模型來對情感運算、心理健康、癌症預後和機器人技術產生真實世界影響力的合作工作。最後，我們透過討論未來的研究如何利用這些想法朝向更通用、互動且安全的AI，來總結本論文。</paragraph>
...
摘要：<paragraph>最近在 AI 領域的突破，準備從根本上提升我們對醫療保健的研究和理解。一個整合性的多對多架構的發展，它利用多個資料模式輸入，用於多個醫療任務的分析建模，對於統一理解現代醫學至關重要。在這項工作中，我們介紹了 M3H，一個可解釋的多模式多任務機器學習醫療保健架構，它整合了來自廣泛醫療任務類別和機器學習問題類別的多模式輸入的學習。這個架構的模組化設計確保了它可概括的資料處理、任務定義，以及快速的模型原型製作，適用於臨床和運作的醫療保健設定。我們透過驗證從四種模式（表格、時間序列、語言和視覺）訓練的模型，在 4 個機器學習問題類別中的 41 個醫療任務上，來評估 M3H 架構。我們的結果證明了 M3H 持續產生多任務模型，它們在 16 個醫療部門的 37 種疾病診斷、三個醫院運作預測和一個病人表型任務中，表現優於規範的單一任務模型（1.1-37.2%）。涵蓋了機器學習問題類別的監督二元分類、多類分類、回歸和聚類。此外，這個架構引進了一個新穎的注意力機制，來平衡自我開發（專注於學習來源任務），和交叉探索（鼓勵從其他任務中學習）。此外，M3H 提供了可解釋的見解，說明附加任務的聯合學習，如何透過一個提議的 TIM 分數影響來源任務的學習，進而闡明任務相互依賴的動態。它可適應的架構促進了自訂和整合，將它建立為一個強健且可擴充的候選解決方案，用於未來的 AI 驅動醫療保健系統。</paragraph>
...
摘要：器官分割是醫學影像中的一項基本任務，且對許多臨床自動化流程很有用。通常，此程序會涉及分割整個體積，當感興趣的點有限時，這可能是沒必要的。在這些情況下，可以使用分類器來取代分割。然而，分類器的內容大小和速度之間存在著固有的權衡。為了解決此問題，我們提出了一種新的方法，該方法採用數據選擇策略，在廣闊的視野中進行稀疏取樣，而無需影像重新取樣。這種稀疏取樣策略使得在不使用加速器的情況下，可以將體素分類為多個器官，並即時執行。儘管我們的這種方法是一個獨立的分類器，但它可以透過查詢任何解析度的網格位置來產生完整的分割。我們已將我們的這種方法與現有的分割技術進行比較，證明了它在醫學影像實際應用中具有優異執行時間的潛力。
...
摘要：量子機器學習（QML）是量子運算有望率先應用的案例。在過去五年中，從理論研究和數值模擬到概念驗證，都取得了進展。在當代量子裝置上展示的用例包括對醫學影像和 Iris 資料集中的項目進行分類、對手寫影像進行分類和生成、毒性篩選以及學習機率分佈。QML 的潛在優點包括更快速的訓練和識別傳統上找不到的特徵映射。儘管如此，這些範例缺乏商業化應用的規模，而且可能還要好幾年 QML 演算法才能取代傳統解法，但 QML 仍是一個令人興奮的領域。
本文撰寫給已經對量子運算有紮實知識，並且現在希望對術語和一些傳統機器學習應用程式有基本概觀，以便準備好研究量子機器學習的讀者。讀者已經會了解相關的線性代數，包括希爾伯特空間，一個具有內積的向量空間。
...
摘要：聯邦學習 (FL) 允許多個注重隱私的應用程式利用其資料集，而不揭露資訊，就能建構全球模型。其中一個領域是醫療保健，其中孤島群組合作，以產生具備改善準確度和概括能力的全球預測器。然而，內在挑戰在於醫療資料的高度異質性，這需要複雜的技術進行評估和補償。本文全面探討了 FL 環境中異質性的數學形式化和分類，重點在於醫療資料的複雜性。特別是，我們針對最熱門的 FL 演算法，評估並比較它們應對基於數量、特徵和標籤分佈的異質性的能力。目標是提供 FL 系統中資料異質性對醫療保健網路影響的量化評估，以及 FL 演算法選擇的指南。我們的研究延伸到現有研究，透過比較七種最常見的 FL 演算法，針對醫療資料使用案例所帶來的獨特挑戰進行基準測試。本文目標是透過不同聯邦醫院孤島收集的一組表格臨床報告，預測中風復發的風險：此情境中經常遇到的資料異質性及其對 FL 性能的影響。
...
摘要：本文将 Struts 和 Hibernate 两种架构结合在一起，使用 DAO（数据访问对象）来存储和访问数据。然后建立了一套适用于深度网络的双模态湿度医学图像库，并提出了一种基于图像的双模态医学图像辅助诊断方法。通过各种特征提取方法的测试，曲线下面积产品（AUROC）最优操作特征为 0.9985，召回率为 0.9814，准确率为 0.9833。该方法可应用于临床诊断，是一种实用方法。任何门诊医生均可通过系统快速注册，或登录平台上传图像以获取更准确的图像。通过系统，各门诊医师可快速注册或登录平台进行图像上传，从而获取更准确的图像。图像分割可指导临床科室医生。然后对图像进行分析，确定肿瘤的位置和性质，以便进行针对性治疗。
...
摘要：<paragraph>在廣泛的醫療應用中追求卓越對 AI 而言構成相當大的挑戰，需要進階的推理能力、取得最新的醫療知識，以及理解複雜的多模態資料。Gemini 模型在多模態和長語境推理方面具備強大的通用能力，在醫學領域提供令人興奮的可能性。在 Gemini 的這些核心優勢的基礎上，我們引進了 Med-Gemini，這是一個高能力的多模態模型系列，專精於醫學，能夠無縫使用網路搜尋，而且可以使用自訂編碼器有效地調整為新的模態。我們在 14 個醫療基準上評估 Med-Gemini，在其中 10 個基準上建立了新的最先進 (SoTA) 效能，並在每個可進行直接比較的基準上都超越 GPT-4 模型系列，而且差距通常很大。在熱門的 MedQA (USMLE) 基準上，我們效能最佳的 Med-Gemini 模型使用新穎的不確定性引導搜尋策略，達到了 91.1% 精確度的 SoTA 效能。在包括 NEJM 影像挑戰和 MMMU (健康與醫學) 在內的 7 個多模態基準上，Med-Gemini 比 GPT-4V 提升了平均相對幅度 44.5%。我們透過在從長去識別化健康記錄和醫療影片問答中進行大海撈針檢索任務上展現 SoTA 效能，證明了 Med-Gemini 長語境能力的有效性，超越了僅使用語境內學習的先前客製化方法。最後，Med-Gemini 的效能顯示出實際用途，在醫療文字摘要等任務中超越人類專家，同時也展示了多模態醫療對話、醫學研究和教育的潛力。綜合來說，我們的結果為 Med-Gemini 的潛力提供了令人信服的證據，儘管在這個對安全性至關重要的領域進行實際部署之前，進一步的嚴謹評估至關重要。</paragraph>
...
摘要：對稱群 $S_{n}$ 在大量的量子系統中表現為量子態的某些特徵對交換量子位元不變。$S_{n}$ 的子群會在許多其他脈絡中出現，用以描述經典影像在空間轉換（例如反射或旋轉）下的標籤對稱性。本研究備有幾何量子機器學習的形式主義，提出遵循 $S_{n}$ 及其子群的等變量子卷積神經網路 (EQCNN) 架構。我們示範，仔細選擇像素到量子位元的嵌入順序，可以簡化 $S_{n}$ 的小子群的 EQCNN 建構。我們對應於全排列群 $S_{n}$ 的新穎 EQCNN 架構，是透過以相等的機率套用所有可能的 QCNN 所建構，這也可以視為量子神經網路中的中斷策略。對於 $S_{n}$ 的子群，我們使用 MNIST 資料集的數值結果顯示，分類精確度優於非等變的 QCNN。$S_{n}$-等變的 QCNN 架構，在連接和非連接圖形的分類上，顯示出比非等變的 QCNN 大幅提升的訓練和測試效能。當使用足夠大量的資料進行訓練時，與 $S_{n}$-等變的 QNN 相比，$S_{n}$-等變的 QCNN 顯示出較佳的平均效能。這些結果有助於在排列對稱系統中建構強大的量子機器學習架構。
...
摘要：醫療資訊的準確呈現對於病患安全至關重要，
但人工智慧（AI）系統，例如大型語言模型（LLM），
在無錯誤的臨床文字詮釋中遭遇挑戰。本文
提出一個新的方法，提交到 MEDIQA-CORR 2024 共享任務（Ben
Abacha 等人，2024a），專注於自動修正臨床筆記中的單字錯誤。不同於依賴廣泛通用資料的 LLM，我們的
方法強調從可用的臨床文字資料中萃取與脈絡相關的資訊。利用萃取式和摘要式問答方法的集合，我們建構一個監督式學習架構
具備領域特定的特徵工程。我們的做法結合領域專業知識，以增強錯誤修正的準確性。透過整合領域專業知識
並優先進行有意義的資訊萃取，我們的做法強調以人為中心的策略對於 AI 適應醫療保健的重要性。
...
摘要：本文探討了應用群體結構的多重代理系統 (MAS) 來建立醫療必要性，這是一個涉及系統性審查病患特定醫療結構化和非結構化資料，並與臨床指引相互比對的流程。我們透過將這個複雜任務分解成更小、更易於管理的子任務來處理它。每個子任務都由一個專門的 AI 代理處理。我們對各種提示策略對這些代理的影響進行系統性的研究，並對不同的大型語言模型 (LLM) 進行基準測試，以確定它們在完成這些任務時的準確度。此外，我們研究了這些代理如何提供可解釋性，從而增強系統內的信任和透明度。
...
摘要：現有的基於 X 光的預訓練視覺模型通常在具有有限解析度（例如 224 × 224）的相對小規模資料集（少於 50 萬個樣本）上進行。然而，自監督預訓練大型模型成功的關鍵在於海量的訓練資料，而 X 光影像領域中維持高解析度則是有效解決各種困難疾病的保證。在本文中，我們通過提出第一個基於高解析度（1280 × 1280）X 光的預訓練基礎視覺模型來解決這些問題，該模型基於我們新收集的大規模資料集，其中包含超過 100 萬張 X 光影像。我們的模型採用遮罩自動編碼器架構，該架構採用遮罩處理（速率高）後的代幣作為輸入，並通過 Transformer 編碼器-解碼器網路重建遮罩影像貼片。更重要的是，我們引入了一種新穎的上下文感知遮罩策略，該策略利用胸部輪廓作為自適應遮罩操作的邊界。我們在兩個下游任務上驗證了我們模型的有效性，包括 X 光報告生成和疾病識別。大量的實驗表明，我們預訓練的醫學基礎視覺模型在下游基準資料集上實現了可比甚至新的最先進性能。本文的原始碼和預訓練模型將在 https://github.com/Event-AHU/Medical_Image_Analysis 上發布。
...
摘要：放射學報告生成 (R2Gen) 說明多模態大型語言模型 (MLLM) 如何自動建立精準且一致的放射學報告。現有方法經常在基於文字的報告中產生幻覺，而這些報告並未準確反映影像內容。為了減輕這個問題，我們引進一種新策略，SERPENT-VLM (使用視覺語言模型的自精煉放射報告生成)，它透過將自精煉機制整合到 MLLM 架構中來改善 R2Gen 任務。我們採用一種獨特的自監督損失，它利用匯集影像表示和生成放射學文字的脈絡表示之間的相似性，以及標準因果語言建模目標，來精煉影像文字表示。這允許模型透過給定影像和生成文字之間的動態互動來審查和對齊生成的文字，因此減少幻覺並持續增強細緻的報告生成。SERPENT-VLM 優於現有的基準，例如 LLaVA-Med、BiomedGPT 等，在 IU X 射線和脈絡中的放射學物件 (ROCO) 資料集上達成 SoTA 效能，並證明對雜訊影像具有穩健性。定性個案研究強調朝向更精密的 R2Gen MLLM 架構的重大進展，為進一步研究醫學影像領域中的自監督精煉開闢道路。
...
摘要：卷積神經網路 (CNN) 已廣泛用於醫學影像分割任務，展現出令人滿意的效能。然而，卷積架構中固有的歸納偏見限制了其建模長程依賴性和空間關聯性的能力。雖然最近基於 Transformer 的架構透過利用自我注意機制來編碼長程依賴性和學習表達式表示來解決這些限制，但它們通常難以提取低階特徵，且高度依賴資料可用性。這促使我們開發 GLIMS，一種資料有效率的注意力引導混合體積分割網路。GLIMS 利用擴張特徵聚合器卷積區塊 (DACB) 來有效擷取局部和全域特徵關聯性。此外，結合的 Swin Transformer 為基礎的瓶頸橋接了局部和全域特徵，以提升模型的穩健性。此外，GLIMS 透過通道和空間感知注意力區塊 (CSAB) 採用注意力引導分割方法，以定位表達式特徵，進行細粒度的邊界分割。在神經膠母細胞瘤和多器官電腦斷層掃描分割任務上的量化和定性結果證明了 GLIMS 在複雜性和準確性方面的有效性。GLIMS 在 BraTS2021 和 BTCV 資料集上展現出傑出的效能，超越了 Swin UNETR 的效能。值得注意的是，GLIMS 以顯著減少的可訓練參數量達到了這項高效能。具體來說，GLIMS 擁有 47.16M 個可訓練參數和 72.30G FLOP，而 Swin UNETR 擁有 61.98M 個可訓練參數和 394.84G FLOP。程式碼已公開在 https://github.com/yaziciz/GLIMS。
...
摘要：多模态融合专注于整合来自多种模态的信息，目标是更准确地预测，这在包括自动驾驶和医疗诊断在内的广泛场景中取得了显著进展。然而，多模态融合的可靠性在很大程度上仍未得到探索，尤其是在低质量数据设置下。本文调查了野外多模态融合的常见挑战和最新进展，并以全面的分类学方式呈现。从以数据为中心的角度来看，我们确定了低质量数据上的多模态融合面临的四个主要挑战，即（1）被异构噪声污染的噪声多模态数据，（2）某些模态缺失的不完整多模态数据，（3）不同模态的质量或属性显著不同的不平衡多模态数据，以及（4）每种模态的质量相对于不同样本动态变化的质量变化多模态数据。这种新的分类法将使研究人员能够了解该领域的现状，并确定几个潜在的方向。我们还针对该领域的开放性问题提供了讨论，以及有趣的未来研究方向。
...
摘要：<paragraph>人工智慧 (AI) 整合在醫療保健中展現出轉型潛力，可提升營運效率和健康成果。大型語言模型 (LLM)，例如 ChatGPT，已展現其在支援醫療決策方面的能力。將 LLM 嵌入醫療系統已成為醫療保健發展中一個有前途的趨勢。ChatGPT 解決急診部門分流問題的潛力已經過檢視，但鮮少有研究探討其在門診部門的應用。本研究專注於簡化工作流程和提升門診分流的效率，特別旨在評估 ChatGPT 在門診指導中提供的回應一致性，包括版本內回應分析和版本間比較。對於版本內，結果顯示 ChatGPT-4.0 的內部回應一致性顯著高於 ChatGPT-3.5 (p=0.03)，且兩者在最高建議中都具有一致性 (4.0 為 71.2%，3.5 為 59.6%)。然而，版本間一致性相對較低 (平均一致性分數=1.43/3，中位數=1)，表示兩個版本之間幾乎沒有建議相符。此外，在比較中只有 50% 的最高建議完全相符。有趣的是，ChatGPT-3.5 的回應比 ChatGPT-4.0 的回應更可能是完整的 (p=0.02)，這表示兩個版本之間在資訊處理和回應產生上可能有差異。這些發現提供了對 AI 輔助門診作業的見解，同時也促進探索 LLM 在醫療保健利用中的潛力與限制。未來的研究可能會專注於根據人體工學和人為因素原則仔細最佳化 LLM 和 AI 在醫療保健系統中的整合，以精確符合有效門診分流的特定需求。</paragraph>
...
摘要：本論文介紹我們團隊參與 MEDIQA-ClinicalNLP2024 共享任務 B 的成果。我們提出了一種診斷臨床皮膚科案例的新方法，方法是整合大型多模態模型，特別是利用 GPT-4V 在檢索器和重新排序框架下的能力。我們的研究表明，當 GPT-4V 用作檢索代理時，它可以使用皮膚病學影像和簡短的病歷在 85% 的時間內準確地檢索正確的皮膚狀況。此外，我們經驗性地證明了樸素的思考鏈 (CoT) 適用於檢索，而基於醫療指南的 CoT 則適用於準確的皮膚病診斷。此外，我們引入了一個多代理對話 (MAC) 框架，並展示了其優於最佳 CoT 策略的卓越性能和潛力。實驗表明，通過使用樸素的 CoT 進行檢索和使用多代理對話進行基於批判的診斷，GPT-4V 可以對皮膚科疾病進行早期準確的診斷。這項工作的影響擴展到改善診斷工作流程、支持皮膚病學教育，以及通過提供可擴展、可訪問和準確的診斷工具來增強患者照護。
...
摘要：從受影響組織中找出細微異常細胞對於臨床診斷和病理研究至關重要。單細胞定序資料提供了前所未有的機會來執行這項任務。然而，目前的異常偵測方法難以處理多樣本和多領域單細胞定序資料中普遍存在的領域轉移，導致次佳效能。此外，這些方法無法將異常細胞區分為病理學上不同的亞型。為了解決這個問題，我們提出了 ACSleuth，一個新穎的重建偏差引導生成框架，它將異常細胞的偵測、領域適應和細微註解整合到一個方法論上的一致工作流程中。特別是，我們提出了第一個理論分析，使用生成模型輸出的重建偏差來代替領域轉移進行異常偵測。這個分析讓我們得以在 ACSleuth 中開發一個新穎且優異的基於最大平均差異的異常評分器。在各種單細胞資料和其他類型的表格資料上的廣泛基準測試證明了 ACSleuth 在多樣本和多領域背景中辨識和分類異常的優越性，優於最先進的方法。我們的程式碼可在 https://github.com/Catchxu/ACsleuth 取得。
...
摘要：多年来，多模态移动传感已被广泛用于推断健康和幸福、行为和背景。然而，阻碍此类模型在现实世界场景中广泛部署的一个重大挑战是分布偏移问题。这是训练集中数据的分布与现实世界（部署环境）中数据的分布不同的现象。虽然在计算机视觉和自然语言处理中进行了广泛的探索，并且虽然先前的移动传感研究简要地解决了这个问题，但目前的工作主要集中于处理单一数据模态（例如音频或加速计读数）的模型，因此，在处理多模态传感器数据时几乎没有无监督域自适应的研究。为了解决这一差距，我们对域对抗神经网络 (DANN) 进行了广泛的实验，表明它们可以有效地处理多模态传感器数据中的分布偏移。此外，我们提出了一种对 DANN 的新改进，称为 M3BAT，用于多模态移动传感的无监督域自适应，并采用多分支对抗训练，以在域自适应期间考虑传感器数据的多分模态性。通过在两个多模态移动传感数据集、三个推理任务和 14 个源目标域对（包括回归和分类）上进行的广泛实验，我们证明了我们的方法在看不见的域上表现有效。与将源域中训练的模型直接部署到目标域相比，该模型在分类任务上的性能提升高达 12% AUC（接收器操作特征曲线下的面积），在回归任务上的 MAE（平均绝对误差）提升高达 0.13。
...
摘要：社交焦慮症在現代社會中是一個普遍的挑戰，影響著個人和職業領域的個人。如果不加以解決，這種情況可能會產生重大的負面後果，影響社交互動和表現。進一步了解其多樣化的生理和情緒症狀對於全面的診斷和量身定制的治療干預至關重要。本研究分析了梅約診所的社交焦慮症狀的患病率和頻率，從一個專門針對此問題的大型 Reddit 資料集中探討了不同的人類經驗。利用這些平台，該研究旨在提取見解並檢查與社交焦慮症相關的一系列生理和情緒症狀。本研究堅持道德考量，在資料集中維持嚴格的使用者匿名性。通過採用一種新穎的方法，該研究利用基於 BART 的多標籤零次分類來識別和衡量症狀患病率和重要性，以每個考慮症狀的機率分數的形式。結果揭示了獨特的模式：「顫抖」表現為普遍的生理症狀，而「害怕受到負面評價」等情緒症狀表現出高頻率。這些發現提供了對社交焦慮症多方面性質的見解，有助於臨床實踐和針對其不同表現形式量身定制的干預措施。
...
摘要：在這項工作中，我們提出了一個名為深度證據學習的不確定性量化框架在放射治療劑量預測領域的新應用。使用開放知識基礎規劃挑戰資料集的醫學影像，我們發現這個模型可以有效地用於產生不確定性估計，這些估計繼承了在網路訓練完成後與預測誤差相關的關聯性。這是在重新制定原始損失函數以實現穩定實現後才實現的。我們發現 (i) 認識論不確定性與預測誤差高度相關，各種關聯指標與蒙特卡羅輟學和深度整體方法相當或更強，(ii) 中位數誤差隨著不確定性閾值變化，對於深度證據學習中的認識論不確定性而言，比這兩個其他傳統框架更加線性，這表明對模型誤差的靈敏度更加統一校準，(iii) 相對於認識論不確定性，隨機不確定性在響應於添加到 CT 強度的 Gaussian 雜訊時展示了其分佈的更顯著轉變，這與其解釋為反映資料雜訊相符。總的來說，我們的結果表明，深度證據學習是一種很有前景的方法，它可以賦予放射治療劑量預測中的深度學習模型統計穩健性。為了增強其臨床相關性，我們展示了我們如何使用這樣的模型來構造預測劑量體積直方圖的置信區間。
...
摘要：人工智能 (AI) 的普遍整合在涉及 AI 啟用系統的事故中引入了責任和問責的複雜挑戰。這些系統的互連性、AI 引發事件的道德考量，加上 AI 技術的不確定性和對應法規的缺乏，使得傳統的責任歸屬具有挑戰性。為此，本研究提出了一種計算反省均衡 (CRE) 方法，為所有利害關係人建立一個連貫且在道德上可接受的責任歸屬架構。計算方法提供了一種結構化的分析，克服了概念方法在處理動態和多方面的場景時的限制，展示了該架構在責任歸屬過程中可解釋性、連貫性和適應性特性。我們探討了與均衡計算中的索賠相關的初始啟動層級的關鍵作用。使用 AI 輔助醫療決策支援系統作為案例研究，我們說明了不同的初始化如何導致不同的責任分配。該架構提供了對 AI 引發事件中問責制的寶貴見解，透過持續監控、修訂和反省，促進了永續且有韌性的系統開發。
...
摘要：對於安全地部署機器學習模型在開放世界中，對分布外 (OOD) 樣本的穩健性至關重要。最近的研究專注於設計評分函數來量化 OOD 不確定性。為這些評分函數設定適當的 OOD 偵測閾值具有挑戰性，因為 OOD 樣本通常無法預先取得。通常，閾值設定為達成所需的真陽性率 (TPR)，例如 $95\%$ TPR。然而，這可能會導致非常高的假陽性率 (FPR)，範圍從 60% 到 96%，如在 Open-OOD 基準中所觀察到的。在安全關鍵的實際應用中，例如醫療診斷，在動態處理各種 OOD 樣本時，控制 FPR 至關重要。為了應對這些挑戰，我們提出一個數學基礎的 OOD 偵測架構，利用專家的回饋來動態地、安全地更新閾值。我們提供了理論結果，顯示它保證隨時滿足 FPR 約束，同時最小化人類回饋的使用。我們架構的另一個關鍵特徵是，它可以與任何 OOD 不確定性量化的評分函數一起使用。在合成和基準 OOD 資料集上對我們系統進行的經驗評估顯示，我們的模型可以在最大化 TPR 的同時，將 FPR 維持在最多 $5\%$。
...
摘要：乳房攝影檢查影像上惡性病灶的偵測對於早期乳癌診斷極為重要。在臨床實務中，影像會從兩個不同的角度取得，而放射科醫師可以充分利用來自兩個視角的資訊，同時定位同一個病灶。然而，對於自動偵測方法而言，這種資訊融合仍然是一個挑戰。在本文中，我們提出一個名為 MAMM-Net 的新模型，它允許同時處理兩個乳房攝影檢視，不僅在物件層級分享資訊，如同現有研究中所見，也同時在特徵層級分享資訊。MAMM-Net 的關鍵組成部分是融合層，它基於可變形注意力，旨在提高偵測精度，同時保持高召回率。我們的實驗顯示，與先前最先進的模型相比，在公共 DDSM 資料集上表現出優異的效能，同時引入了新的有幫助功能，例如像素級病灶註解和病灶惡性分類。
...
摘要：本報告列出 13 項以運算術方式兌現的功能條件，這些條件被認為是意識價值經驗的組成部分。
這些條件從現有關於動物感知、醫療疾病、麻醉藥、哲學、演化、神經科學和人工智慧等主題的經驗和理論文獻中萃取出來。
...
摘要：大型語言模型 (LLM) 用於協助臨床醫師，這項技術的採用備受矚目。現有作品主要採用封閉式問答任務，並提供選項供評估。然而，在實際的臨床環境中，許多臨床決策（例如治療建議）涉及回答沒有預設選項的開放式問題。同時，現有研究主要使用準確度來評估模型效能。在本文中，我們全面評量醫療保健領域中各種 LLM，以清楚了解它們的優缺點。我們的評量基準包含七項任務和十三個資料集，涵蓋醫療語言生成、理解和推理。我們在零次學習和少次學習（即 1、3、5 次學習）設定下，對醫療保健領域現有的十六種 LLM 進行詳細評估。我們報告了五項指標的結果（即匹配、忠實度、全面性、概括性、穩健性），這些指標對於獲得臨床使用者的信任至關重要。我們進一步邀請醫學專家進行人工評估。
...
摘要：社群媒體平台在全球廣泛採用，不僅增強了使用者的連結性和溝通，也成為傳播健康相關資訊的重要管道，進而將社群媒體資料建立為公共衛生研究中無價的有機資料資源。電子煙或電子香菸的使用在美國和其他國家大幅增加，導致電子煙與電子煙相關肺部傷害 (EVALI) 爆發，造成 2019 年的住院和死亡，這突顯了理解電子煙行為和發展戒除電子煙有效策略的急迫性。在本研究中，我們從 Reddit 上的一個電子煙社群中萃取了一個樣本資料集，以分析使用者戒除電子煙的意圖。利用包含最新 GPT-4 和傳統 BERT 為基礎的語言模型等大型語言模型，進行句子層級的戒除電子煙意圖預測任務，本研究將這些模型的結果與人工標註進行比較。值得注意的是，與人工評估人員相比，GPT-4 模型在遵守標註指南和流程方面表現出優異的一致性，展現了偵測人工評估人員可能忽略的細微使用者戒除電子煙意圖的先進能力。這些初步發現強調了 GPT-4 在增強社群媒體資料分析的準確性和可靠性方面的潛力，特別是在找出可能逃過人工偵測的使用者細微意圖方面。
...
摘要：大型語言模型 (LLM) 在處理臨床文字摘要任務方面展現出令人滿意的能力。在此研究中，我們證明了一個小型開放原始碼 LLM 能夠有效訓練，從門診病人與醫師的對話中產生高品質的臨床筆記。我們透過 LLaMA-2 130 億個參數模型進行全面的領域和任務特定適應程序來達成此目標。此程序包含持續的預訓練、監督微調，以及來自 AI 和人類回饋的強化學習。我們引進一種增強的方法，稱為 DistillDirect，用於執行政策強化學習，並以 Gemini Pro 作為教師模型。我們產生的模型 LLaMA-Clinic，能夠產生品質與醫師撰寫的臨床筆記相當的臨床筆記。在盲測醫師閱讀研究中，大多數 (90.4%) 的個別評估將 LLaMA-Clinic 產生的筆記評為「可接受」或更高，涵蓋所有三個標準：真實世界準備度、完整性和準確性。值得注意的是，在更具挑戰性的「評估和計畫」部分，LLaMA-Clinic 在真實世界準備度方面獲得較高分數 (4.2/5)，高於醫師撰寫的筆記 (4.1/5)。此外，我們在公共臨床筆記資料集中（例如 ACI-BENCH）發現了注意事項。我們強調未來臨床筆記產生任務中的關鍵考量，強調預先定義最佳實務筆記格式的重要性。整體而言，我們的研究證明了訓練較小、開放原始碼 LLM 以協助臨床文件編寫的潛力和可行性，並利用醫療保健機構存取病歷和領域專業知識。我們已將我們新建立的綜合診所對話筆記資料集和醫師回饋資料集公開，以促進此領域的未來研究。
...
摘要：最近，基于深度学习的语言模型大幅增强了文本到 SQL 任务，在医疗领域检索患者记录方面具有广阔的应用前景。此类应用中一个值得注意的挑战在于辨别不可回答的查询。通过微调模型，我们展示了将医疗记录查询转换为 SQL 查询的可行性。此外，我们引入了一种基于熵的方法来识别和过滤掉不可回答的结果。我们通过基于对数概率分布过滤低置信度 SQL 进一步提高结果质量，同时通过在实际数据库上执行查询来减轻语法和模式错误。我们通过实验验证了我们的方法可以过滤掉不可回答的问题，即使无法访问模型的参数，也可以广泛使用，并且可以在实践中有效使用。
...
摘要：大型語言模型 (LLM) 整合至醫療保健領域，承諾將轉化醫療診斷、研究和患者照護。然而，醫療 LLM 的進展面臨障礙，例如複雜的訓練需求、嚴格的評估需求，以及限制學術探索的專有模型的支配地位。透明且全面的 LLM 資源存取對於推進此領域、促進可複製性，以及鼓勵醫療保健 AI 的創新至關重要。我們提出 Hippocrates，一個專門為醫療領域開發的開源 LLM 架構。與先前的努力形成鮮明對比的是，它提供對其訓練資料集、程式碼庫、檢查點和評估協定的無限制存取。這種開放式方法旨在刺激協作研究，讓社群能夠在透明的生態系統中建立、改進和嚴格評估醫療 LLM。此外，我們介紹 Hippo，一個針對醫療領域量身打造的 7B 模型系列，透過持續預訓練、指令調整和人類與 AI 回饋的強化學習，從 Mistral 和 LLaMA2 微調而來。我們的模型優於現有的開放式醫療 LLM 模型，差距很大，甚至超越了具有 70B 參數的模型。透過 Hippocrates，我們渴望釋放 LLM 的全部潛力，不僅能促進醫療知識和患者照護，還能使醫療保健中 AI 研究的益處民主化，讓它們在全球各地都能取得。
...
摘要：<paragraph>利用生成模型的弱监督医学影像分割 (MIS) 对于临床诊断至关重要。然而，分割结果的准确性通常受到监督不足和医学影像的复杂性的限制。现有的模型也只提供单一结果，不允许测量不确定性。在本文中，我们介绍了 DiffSeg，这是一种基于扩散差的皮肤病变分割模型，它利用扩散模型原理从具有不同语义信息的图像中提取基于噪声的特征。通过辨别这些噪声特征之间的差异，该模型识别出患病区域。此外，其多输出能力模仿了医生的注释行为，便于可视化分割结果的一致性和歧义性。此外，它使用广义能量距离 (GED) 量化输出不确定性，帮助医生解释和决策。最后，该模型通过密集条件随机场 (DenseCRF) 算法整合输出，通过考虑像素间相关性来细化分割边界，从而提高准确性并优化分割结果。我们在 ISIC 2018 挑战数据集上展示了 DiffSeg 的有效性，其性能优于最先进的基于 U-Net 的方法。</paragraph>
...
摘要：利用醫學影像（例如光學相干斷層掃描 (OCT) 影像）進行自動視網膜層分割，是診斷眼科疾病的重要工具。然而，由於影像中對比度低且有血流雜訊，要達到精準的分割是一項挑戰。此外，演算法應輕量化，才能部署於實際的臨床應用中。因此，需要設計一個輕量化且效能高的網路，用於視網膜層分割。在本文中，我們提出 LightReSeg 用於視網膜層分割，可應用於 OCT 影像。具體而言，我們的做法遵循編碼器-解碼器結構，其中編碼器部分採用多尺度特徵萃取和 Transformer 區塊，以充分利用所有尺度特徵圖的語義資訊，並讓特徵具有更好的全局推理能力，而解碼器部分，我們設計了一個多尺度非對稱注意力 (MAA) 模組，用於保留每個編碼器尺度的語義資訊。實驗顯示，與目前最先進、參數達 105.7M 的 TransUnet 方法相比，我們的做法在我們收集的資料集和兩個其他公開資料集上，都達到了更好的分割效能，而參數僅有 3.3M。
...
摘要：儘管深度學習在醫學影像分析方面取得顯著成功，但由於缺乏高品質標記影像以供監督，醫學影像分割仍然具有挑戰性。此外，一般而言自然影像和醫學影像之間的顯著領域差距，特別是超音波影像，阻礙了將針對自然影像訓練的微調模型套用於手邊的任務。在這項工作中，我們解決了低資料量模式中分割模型的效能下降問題，並提出了一種無提示分割方法，利用分割基礎模型分割抽象形狀的能力。我們透過我們新穎的提示點生成演算法來做到這一點，該演算法使用粗略的語意分割遮罩作為輸入，並將零次提示模型作為最佳化目標。我們在超音波影像的分割發現任務（病理異常）上展示了我們的模型。我們的方法優勢在小規模肌肉骨骼超音波影像資料集的低資料量模式實驗中得到了不同程度的驗證，隨著訓練集大小的減少，效能提升幅度越大。
...
摘要：電子病歷 (EHR) 儘管對醫療保健從業者來說是一大福音，但每天都變得越來越複雜且冗長。在這些冗長的電子病歷中篩選既費力又成為醫病互動中一個繁瑣的部分。已經提出了幾種方法來幫助減輕這個普遍問題，無論是通過摘要還是分段，然而，過去只有少數方法真正有幫助。隨著自動化方法的興起，機器學習 (ML) 在解決識別電子病歷中相關部分的任務方面顯示出前景。然而，大多數機器學習方法依賴於標記資料，而這在醫療保健中很難獲得。另一方面，大型語言模型 (LLM) 在自然語言處理 (NLP) 中表現出令人印象深刻的壯舉，而且是以零次學習的方式，即沒有任何標記資料。為此，我們建議使用大型語言模型來識別相關的部分標題。我們發現 GPT-4 可以有效地解決零次學習和少次學習設置中的任務，並且比最先進的方法分段好得多。此外，我們還註釋了一個更困難的真實世界資料集，並發現 GPT-4 難以執行良好，暗示需要進一步的研究和更困難的基準。
...
摘要：大型語言模型 (LLM) 中的提示洩漏會造成重大的安全和隱私威脅，尤其是在檢索增強生成 (RAG) 系統中。然而，多輪 LLM 互動中的洩漏以及緩解策略尚未以標準化方式進行研究。本文探討了 LLM 在 4 個不同領域和 10 個封閉和開放原始碼 LLM 中針對提示洩漏的漏洞。我們獨特的多輪威脅模型利用了 LLM 的阿諛效應，我們的分析剖析了 LLM 回應中的任務指令和知識洩漏。在多輪設置中，我們的威脅模型將平均攻擊成功率 (ASR) 提高到 86.2%，包括 GPT-4 和 claude-1.3 的 99% 洩漏。我們發現，像 Gemini 這樣的某些黑盒 LLM 在不同領域對洩漏的敏感性不同 - 與醫療領域相比，它們更有可能在新聞領域洩漏上下文知識。我們的實驗測量了 6 種黑盒防禦策略的具體效果，包括 RAG 場景中的查詢重寫器。我們提出的多層防禦組合對黑盒 LLM 仍有 5.3% 的 ASR，這表示 LLM 安全研究的改進和未來方向仍有提升空間。
...
摘要：工業系統中的異常偵測對於預防設備故障、確保風險識別以及維持整體系統效率至關重要。傳統的監控方法通常依賴於固定閾值和經驗法則，這些法則可能不足以偵測系統健康狀況的細微變化，並預測即將發生的故障。為了解決此限制，本文提出一個基於注意力的卷積自動編碼器（ABCD），用於風險偵測，並將風險值映射到維護計畫中。ABCD 從真實世界工業冷卻系統的歷史數據中學習導電度的正常行為，並重建輸入數據，識別出與預期模式不同的異常值。該框架還採用校準技術來確保其預測的可靠性。評估結果表明，與不使用注意力相比，使用 ABCD 中的注意力機制，效能提升了 57.4%，誤報減少了 9.37%。這種方法可以有效地偵測風險，將風險優先順序映射到維護，為冷卻系統設計人員和維修人員提供有價值的見解。0.03% 的校準誤差表示模型經過良好校準，並增強了模型的可信度，能夠對維護策略做出明智的決策
...
摘要：序列建模是各種領域的關鍵領域，包括自然語言處理 (NLP)、語音識別、時間序列預測、音樂生成和生物資訊學。遞迴神經網路 (RNN) 和長短期記憶網路 (LSTM) 歷來主導機器翻譯、命名實體識別 (NER) 等序列建模任務。然而，Transformer的進步導致了這種範式的轉變，因為它們具有更優越的性能。然而，Transformer存在 $O(N^2)$ 注意力複雜度和處理歸納偏見的挑戰。已經提出了多種變體來解決這些問題，它們使用譜網路或卷積，並且在各種任務上表現良好。然而，它們在處理長序列時仍然存在困難。狀態空間模型 (SSM) 已成為這種情況下序列建模範式的有希望的替代方案，特別是隨著 S4 及其變體（例如 S4nd、Hippo、Hyena、Diagnol State Spaces (DSS)、Gated State Spaces (GSS)、Linear Recurrent Unit (LRU)、Liquid-S4、Mamba 等）的出現。在本次調查中，我們根據三種範式對基礎 SSM 進行分類，即閘控架構、結構架構和遞迴架構。本次調查還重點介紹了 SSM 在視覺、視訊、音訊、語音、語言（尤其是長序列建模）、醫學（包括基因組學）、化學（如藥物設計）、推薦系統和時間序列分析（包括表格資料）等領域的各種應用。此外，我們整合了 SSM 在基準資料集上的效能，例如 Long Range Arena (LRA)、WikiText、Glue、Pile、ImageNet、Kinetics-400、sstv2，以及 Breakfast、COIN、LVU 等視訊資料集，以及各種時間序列資料集。Mamba-360 工作的專案頁面可以在此網頁上找到。\url{https://github.com/badripatro/mamba360}。
...
摘要：儘管融合多種乳房攝影術視圖的資訊在提高乳癌偵測準確度中扮演重要角色，開發基於多視圖乳房攝影術的電腦輔助診斷 (CAD) 計畫仍面臨挑戰，且目前沒有此類 CAD 計畫已用於臨床實務。為了克服這些挑戰，我們探討一種基於對比語言影像預訓練 (CLIP) 的新方法，這在各種醫學影像任務中引起了興趣。透過解決 (1) 有效調整單視圖 CLIP 以進行多視圖特徵融合，以及 (2) 有效微調這個具有受限樣本和運算資源的參數密集模型的挑戰，我們引進 Mammo-CLIP，這是第一個處理多視圖乳房攝影術和對應簡短文字的多模式架構。Mammo-CLIP 使用早期特徵融合策略，以學習從左右乳房的 CC 和 MLO 視圖取得的四個乳房攝影術中的多視圖關係。為了提高學習效率，將即插即用轉接器新增至 CLIP 影像和文字編碼器，以進行微調參數，並將更新限制在約 1% 的參數。為了進行架構評估，我們回溯性地組裝兩個資料集。第一個資料集包含 470 個惡性和 479 個良性病例，用於少樣本微調和透過 5 倍交叉驗證對提出的 Mammo-CLIP 進行內部評估。第二個資料集包含 60 個惡性和 294 個良性病例，用於測試 Mammo-CLIP 的泛化能力。研究結果顯示，Mammo-CLIP 在兩個資料集的 AUC 中優於最先進的跨視圖轉換器 (0.841 對 0.817，0.837 對 0.807)。它也超越了先前兩種基於 CLIP 的方法 20.3% 和 14.3%。這項研究強調了應用微調的視覺語言模型，以開發下一代、基於影像文字的乳癌 CAD 計畫的潛力。
...
摘要：大型語言模型，例如 GPT-4 和 Med-PaLM，在臨床任務上表現出色；然而，它們需要取得運算存取權，是封閉原始碼的，且無法部署在裝置上。中型模型例如 BioGPT-large、BioMedLM、LLaMA 2 和 Mistral 7B 避免了這些缺點，但它們在臨床任務上的容量尚未得到充分研究。為了幫助評估它們在臨床使用上的潛力，並幫助研究人員決定應使用哪個模型，我們比較了它們在兩個臨床問答 (QA) 任務上的表現：MedQA 和消費者查詢回答。我們發現 Mistral 7B 是表現最佳的模型，在所有基準測試中獲勝，且表現優於專門針對生物醫學領域訓練的模型。雖然 Mistral 7B 的 MedQA 分數為 63.0%，接近原始的 Med-PaLM，且它通常可以對消費者健康查詢產生合理的回應，但仍有改進空間。本研究首次對開放原始碼中型模型在臨床任務上的表現進行了正面評估。
...
摘要：反射共軛顯微鏡 (RCM) 是一種非侵入性影像技術，用於生物醫學研究和臨床皮膚科。它提供皮膚和表層組織的虛擬高解析度影像，減少了對物理切片的需要。RCM 使用雷射光源照射組織，捕捉反射光以產生不同深度微觀結構的詳細影像。最近的研究探索了 AI 和機器學習，特別是 CNN，用於分析 RCM 影像。我們的研究提出了一種基於紋理特徵的分割策略，以識別臨床上的重要區域，讓皮膚科醫師能夠有效地解釋影像並提升診斷信心。這種方法有望推進皮膚科的診斷和治療。
...
摘要：由於基礎設施可及性有限，遠端關鍵基礎設施的健康監測是一項複雜且昂貴的活動。檢查無人機是無處不在的資產，可透過改善可及性來提升關鍵基礎設施的可靠性。然而，由於嚴苛的操作環境，監測其健康狀況以確保檢查作業順利進行至關重要。電池是決定檢查無人機整體可靠性的關鍵元件，並透過適當的健康管理方法，有助於進行可靠且穩健的檢查。在此背景下，本文提出了一種針對鋰聚合物電池放電結束 (EOD) 電壓預測的新型混合機率方法。這種混合化是在錯誤修正組態中實現的，它結合了基於物理的放電和機率錯誤修正模型，以量化隨機不確定性和認識論不確定性。混合機率方法的效能已在包含在各種負載條件下 EOD 電壓的資料集上進行經驗評估。該資料集來自在不同飛行中操作的實際檢查無人機，重點在於離岸風力渦輪機檢查。已使用不同的機率方法測試建議的方法，並證明與最佳機率方法相比，機率準確度提高了 14.8%。此外，隨機不確定性和認識論不確定性提供了穩健的估計，以增強電池健康狀態的診斷。
...
摘要：我們推出了一款基於機器學習的異常偵測產品，AI Detect and Respond (AIDR)，它可以即時監控 Walmart 的業務和系統健康狀況。在超過 3 個月的驗證期間，此產品從超過 3000 個模型中提供預測給超過 25 個應用程式、平台和營運團隊，涵蓋了 63% 的重大事件，並將平均偵測時間 (MTTD) 縮短了超過 7 分鐘。與先前的異常偵測方法不同，我們的解決方案利用了統計、機器學習和深度學習模型，同時繼續納入基於規則的靜態閾值，以納入特定領域的知識。單變數和多變數機器學習模型都是透過分散式服務來部署和維護的，以確保可擴充性和高可用性。AIDR 有一個回饋迴路，它利用漂移偵測演算法和客戶回饋的組合來評估模型品質。它還提供自助入門功能和可自訂性。AIDR 已在各種內部團隊中獲得成功，偵測時間縮短，且誤報比先前的做法更少。隨著我們向前邁進，我們的目標是擴展事件涵蓋範圍和預防措施，減少雜訊，並進一步與根本原因建議 (RCR) 整合，以實現端到端的 AIDR 體驗。
...
摘要：人工智慧應用程式在管理流行病方面已展現前景，且已被廣泛用於協助識別、分類和診斷醫學影像。為因應猴痘 (Mpox) 全球爆發，HeHealth.ai 團隊利用現有工具篩檢性傳染疾病，透過人工智慧方法開發出針對症狀性 Mpox 的數位篩檢測試。在 Mpox 全球爆發之前，該團隊開發了一款智慧型手機應用程式，應用程式使用者可以使用自己的智慧型手機相機拍攝自己陰莖的照片，以篩檢症狀性性傳染病。人工智慧模型最初使用 5000 個案例開發，並使用修改後的卷積神經網路來輸出視覺上可診斷的陰莖病理預測分數，包括梅毒、單純疱疹病毒和人類乳突病毒。從 2022 年 6 月到 2022 年 10 月，總計約有 22,000 名使用者下載 HeHealth 應用程式，並使用 HeHealth 人工智慧技術分析了約 21,000 張影像。接著，我們從 2022 年 7 月開始進行形成性研究、利害關係人參與、快速整合影像、驗證研究和實作此工具。從 2022 年 7 月到 2022 年 10 月，總計已使用 1000 張與 Mpox 相關的影像來訓練 Mpox 症狀檢查工具。我們的數位症狀檢查工具顯示出 87% 的準確度可判定 Mpox，以及 90% 的準確度可排除症狀性 Mpox。所識別出的幾個障礙包括應用程式使用者的資料隱私和安全性問題、訓練人工智慧工具的初始資料不足，以及輸入資料的潛在普遍性。我們提供了一些建議，以協助其他人著手進行類似緊急狀況專案，包括讓廣泛的利害關係人參與、組成跨領域團隊、優先考量實用主義，以及大數據實際上是由小數據組成這個概念。
...
摘要：臨床試驗配對是找出患者可能符合資格參加的試驗的任務。通常，這項任務需要大量人力，並且需要詳細驗證病患的電子健康記錄 (EHR) 是否符合臨床試驗的嚴格納入和排除標準。這個過程是手動的、耗時的，且難以擴大規模，導致許多患者錯失潛在的治療選項。大型語言模型 (LLM) 的最新進展讓自動化患者試驗配對成為可能，正如多項並行的研究報告中所示。然而，目前的做法僅限於受限的、通常是合成式的資料集，無法充分反映現實世界醫療資料中遇到的複雜性。在這項研究中，我們提出第一個端到端的、大規模的臨床試驗配對經驗評估，使用現實世界的 EHR。我們的研究展示了 LLM 準確配對患者與適當臨床試驗的能力。我們使用專有的 LLM 進行實驗，包括 GPT-4 和 GPT-3.5，以及我們稱為 OncoLLM 的自訂微調模型，並顯示 OncoLLM 儘管規模小很多，不僅優於 GPT-3.5，而且與合格的醫療醫師表現相當。所有實驗都是在現實世界的 EHR 上進行，其中包括臨床筆記和美國某個癌症中心的可用臨床試驗。
...
摘要：在城市環境中，綠地的稀缺構成了一項嚴峻的挑戰。這會產生多重的不良影響，影響市民的健康和福祉。小規模的干預措施，例如袖珍公園，是一種可行的解決方案，但會帶來多重限制，涉及特定區域的設計和實施。在本研究中，我們利用生成式 AI 的功能進行多尺度干預規劃，重點關注基於自然的解決方案。通過利用圖像到圖像和圖像修復演算法，我們提出了一種解決城市地區綠地不足的方法。我們專注於塞薩洛尼基的兩條缺乏綠化的巷道，展示了我們的方法在可視化 NBS 干預措施方面的功效。我們的研究結果強調了新興技術在塑造城市干預規劃流程的未來方面具有變革潛力。
...
摘要：在应用於臨床領域的自然語言處理中，利用大型語言模型已成為臨床筆記中錯誤偵測和校正的一條有前景的途徑，這是一個知識密集型任務，其註解資料稀少。本文提出了 MedReAct'N'MedReFlex，它利用了一套四個基於 LLM 的醫療代理。MedReAct 代理透過觀察、分析和採取行動來啟動流程，產生軌跡以引導搜尋，以針對臨床筆記中的潛在錯誤。隨後，MedEval 代理使用五個評估器來評估目標錯誤和建議的更正。在 MedReAct 的行動被證明不足的情況下，MedReFlex 代理會介入，進行反思分析並提出替代策略。最後，MedFinalParser 代理會格式化最終輸出，保留原始樣式，同時確保錯誤校正過程的完整性。我們方法的一個核心組成部分是基於我們的 ClinicalCorp 語料庫的 RAG 管線。在包含臨床指南和資訊的其他眾所周知來源中，我們預處理並發布開源 MedWiki 資料集以供臨床 RAG 應用。我們的結果證明了我們的 RAG 方法與透過 MedReAct'N'MedReFlex 框架運用的 ClinicalCorp 的核心作用。它在 MEDIQA-CORR 2024 最終排行榜上獲得第九名。
...
摘要：反事實範例有助於探索機器學習模型的決策邊界，並確定特徵歸因。我們如何應用基於反事實的方法來分析和解釋 LLM？我們找出以下關鍵挑戰。首先，產生的文字反事實應對使用者有意義且可讀，因此可以在心智上進行比較以得出結論。其次，為了使解決方案可擴展到長篇文字，使用者應配備工具，以便從不同粒度層級的擾動中建立反事實批次，並互動分析結果。在本文中，我們解決上述挑戰並貢獻 1) 一種新演算法，用於透過移除和替換不同粒度層級的文字區段來產生批次的完整且有意義的文字反事實，以及 2) LLM Analyzer，一種互動式視覺化工具，可幫助使用者透過互動式檢查和彙總有意義的反事實來了解 LLM 的行為。我們透過使用來自醫療、法律、金融、教育和新聞資料集的 1,000 個樣本，依其所產生的反事實的語法正確性來評估所提出的演算法。在我們的實驗中，97.2% 的反事實在語法上是正確的。透過使用案例、使用者研究和專家的回饋，我們證明了所提出的互動式視覺化工具的實用性和可用性。
...
摘要：本研究的目的是预测一个人是否会完成药物和酒精康复计划以及一个人参加的次数。该研究基于从药物滥用和心理健康服务管理局 (SAMHSA) 获得的人口统计数据，包括俄克拉荷马州药物和酒精康复中心入院和出院数据。人口统计数据高度分类，导致使用二进制编码，并利用各种公平措施来减轻九个人口统计变量的偏差。使用支持向量机在各种参数范围内比较了线性、多项式、sigmoid 和径向基函数等核方法，以找到最优值。然后将这些方法与决策树、随机森林和神经网络等方法进行比较。用于分类数据的合成少数过采样技术名义 (SMOTEN) 用于平衡数据，并对缺失数据进行插补。然后对九个偏差变量进行交叉分析以减轻偏差，并整合双重和三重交互作用以使用概率来查看最坏情况比率公平性缓解。在二元和多类场景中都探索了不同的影响、统计奇偶校验差异、条件统计奇偶校验比率、人口统计奇偶校验、人口统计奇偶校验比率、均等几率、均等几率比率、均等机会和均等机会比率。
...
摘要：<paragraph>醫療視覺語言預訓練 (Med-VLP) 在醫療影像的視覺內容和相關文字描述之間建立關聯。現有的 Med-VLP 方法主要著重於描繪單一部位（特別是胸部 X 光）的 2D 影像。在本文中，我們透過使用 CT 影像和報告的多模態資料集，將 Med-VLP 的範圍擴展到 3D 影像，特別針對全身情境。與 2D 對應關係相比，3D VLP 必須有效擷取 3D 影像中顯著稀疏表示的基本語義。在本文中，我們介紹 CT-GLIP（使用 CT 掃描的基礎語言影像預訓練），這是一種創新的方法，用於建構器官層級的影像文字配對，以增強多模態對比學習，將基礎視覺特徵與精確的診斷文字對齊。此外，我們還開發了一個異常字典，以使用多樣化的對比配對來增強對比學習。我們的訓練方法是建立在多模態 CT 資料集上，該資料集包含來自 104 個器官的 17,702 位患者的 44,011 個器官層級的視覺文字配對，結果證明它可以使用自然語言以零次學習的方式識別器官和異常。CT-GLIP 的效能通過一個包含 1,130 位患者的獨立測試集進行驗證，重點關注 7 個器官中最常見的 16 種異常。實驗結果顯示，在零次學習和微調情境中，我們的模型使用 CNN 和 ViT 架構，表現優於標準 CLIP 架構。</paragraph>
...
摘要：多模態醫學影像在臨床診斷和研究中扮演著關鍵角色，因為它結合了來自各種影像方式的資訊，以提供對基礎病理學更全面的了解。最近，基於深度學習的多模態融合技術已成為改善醫學影像分類的強大工具。本篇評論提供了對基於深度學習的多模態融合在醫學分類任務中發展的深入分析。我們探討了流行臨床方式之間的互補關係，並概述了多模態分類網路的三種主要融合架構：輸入融合、中間融合（包含單層級融合、階層式融合和基於注意力的融合）以及輸出融合。透過評估這些融合技術的效能，我們深入了解了不同網路架構對各種多模態融合場景和應用領域的適用性。此外，我們深入探討了與網路架構選擇、處理不完整多模態資料管理以及多模態融合的潛在限制相關的挑戰。最後，我們重點介紹了基於 Transformer 的多模態融合技術的未來前景，並針對這個快速演進的領域提出未來研究建議。
...
摘要：改善醫療保健的未來始於更好地了解醫院當前的實際做法。這激勵了從患者數據中發現典型照護路徑的目標。透過分群可以揭示同質的照護路徑群組。由時間戳記事件序列表示的照護路徑分群的難度在於定義語義上適當的指標和分群演算法。
  在本文中，我們將兩種為時間序列開發的方法調整為時間序列：drop-DTW 指標和 DBA 方法，用於建構平均時間序列。然後將這些方法應用於分群演算法，以提出針對定時序列的原始且完善的分群演算法。
...
摘要：醫學視覺語言預訓練已成為一種有前途的方法，用於學習醫學影像和文字的領域通用表示。然而，當前利用醫學影像和文字之間的全局和局部對齊的演算法可能會因醫學資料中的冗餘資訊而受到影響。為了解決這個問題，我們提出了一個針對胸部 X 光的基礎知識增強型醫學視覺語言預訓練 (GK-MVLP) 架構。在此架構中，醫學知識透過使用基於轉換器的基礎知識增強模組，將適當的解剖區域作為基礎，以在解剖區域層級的視覺特徵和醫學知識的紋理特徵之間進行細粒度對齊。GK-MVLP 的效能與下游胸部 X 光疾病分類、疾病定位、報告產生和醫學視覺問答任務的最新技術相當或超越最新技術。我們的結果顯示了將基礎機制納入以消除偏差並改善胸部 X 光影像和放射科報告之間對齊的優點。
...
摘要：近年來，從對話資料中自動偵測憂鬱症引起了相當大的興趣。由人類控制的虛擬代理人進行的訪談所組成的 DAIC-WOZ 資料集已被廣泛用於此任務。最近的研究報告指出，將訪談者的提示納入模型中時，表現會有所提升。在這項研究中，我們假設這種提升可能主要是由於這些提示中存在的偏誤，而不是所提出的架構與方法。透過消融實驗和定性分析，我們發現使用訪談者提示的模型學會專注於訪談中的特定區域，其中詢問了有關過去心理健康問題的經驗，並將其用作區分捷徑來偵測憂鬱症患者。相反地，使用參與者回應的模型會從整個訪談中收集證據。最後，為了強調此偏誤的嚴重性，我們透過故意利用它來達到 0.90 的 F1 分數，這是迄今為止使用僅有文字資訊在這個資料集上報告的最高結果。我們的研究結果強調了在將訪談者的提示納入模型時需要謹慎，因為它們可能會無意間學會利用有針對性的提示，而不是學會描述真正能顯示患者心理健康狀況的語言和行為。
...
摘要：基礎模型在推進醫療領域方面已變得極其寶貴。
儘管有這些前景，但仍有待解決的問題是，如何策略性地部署 LLM，以在複雜的醫療任務中發揮有效效用。我們的新架構，即醫療決策代理 (MDAgents)，旨在透過自動分配 LLM 的有效協作結構來解決此差距。分配的單獨或群組協作結構會根據手邊的醫療任務複雜性進行調整，模擬真實世界的醫療決策制定流程。我們使用最先進的 LLM 對我們的架構和基準方法進行評估，並採用一系列具有挑戰性的醫療基準測試：MedQA、MedMCQA、PubMedQA、DDXPlus、PMC-VQA、Path-VQA 和 MedVidQA，在 7 個基準測試中有 5 個需要了解多模式醫療推理，並在其中獲得最佳效能。消融研究顯示，MDAgents 在調整協作代理數量以最佳化效率和準確度方面表現出色，展示了其在各種場景中的穩健性。我們還探討了群體共識的動態，提供了協作代理如何在複雜的臨床團隊動態中表現的見解。我們的程式碼可以在 https://github.com/mitmedialab/MDAgents 找到。
...
摘要：鼻部細胞學是一種診斷鼻炎和過敏的新穎且有效率的臨床技術，但由於細胞計數耗時，因此並未廣泛使用；這就是為什麼 AI 輔助計數可以成為此技術擴散的轉捩點。在本文中，我們介紹了第一個鼻細胞學視野影像的資料集：NCD（鼻細胞學資料集），旨在訓練和部署物件偵測模型，以在臨床實務中協助醫生和生物學家。已經複製了填充鼻黏膜的細胞類型的實際分佈，從臨床病患的載玻片中取樣影像，並手動標記在上面找到的每個細胞。相應的物件偵測任務呈現出與嚴重的類別不平衡相關的非平凡問題，涉及最稀有的細胞類型。這項工作透過提出一個新穎的基於機器學習的方法來協助鼻黏膜細胞的自動化偵測和分類，解決了一些公開的挑戰：DETR 和 YOLO 模型在偵測細胞和正確分類方面表現良好，顯示出加速鼻科專家工作的巨大潛力。
...
摘要：<paragraph>合成資料生成提供了一個有前途的解決方案，可透過產生逼真的去識別資料來增強電子健康記錄 (EHR) 的效用。然而，現有的文獻主要關注合成健康資料的品質，忽略了下游預測中公平性的關鍵面向。因此，在合成 EHR 上訓練的模型因在目標任務中產生有偏差的結果而受到批評。這些偏差可能來自特徵之間的虛假關聯或模型無法準確表示子群組。為了解決這些問題，我們提出了偏見轉換生成對抗網路 (Bt-GAN)，一種專為醫療保健領域設計的基於 GAN 的合成資料產生器。為了應對虛假關聯 (i)，我們提出了一個資訊約束資料產生程序，使產生器能夠基於一個定義良好的演算法公平性概念來學習公平的確定性轉換。為了克服捕捉精確子群組表示 (ii) 的挑戰，我們透過基於評分的加權抽樣來激勵產生器保留子群組密度。這種方法迫使產生器從資料流形的代表性不足區域學習。我們使用 MIMIC-III 資料庫進行了廣泛的實驗。我們的結果表明，Bt-GAN 達到了 SOTA 精度，同時顯著提高了公平性並最大程度地減少了偏差放大。我們還進行了深入的可解釋性分析，以提供額外的證據來支持我們研究的有效性。總之，我們的研究引入了一種新穎且專業的方法來解決醫療保健領域合成資料生成的限制。透過納入公平性考量並利用 GAN 等先進技術，我們為醫療保健應用中更可靠且無偏見的預測鋪平了道路。</paragraph>
...
摘要：这项工作源于深度神经网络的最新发展，特别是作为 ChatGPT 等应用程序底层的 Transformer 架构，以及在移动设备上执行推理的需要。专注于新兴的 Transformer（特别是具有计算高效的 Swin 类架构的 Transformer）和基于 Transformer 的大型模型（例如 Stable Diffusion 和 LLM），我们观察到计算算子之间的布局转换导致这些应用程序出现显著的减速。本文提出了 SmartMem，这是一个用于消除大多数布局转换的综合框架，其思想是多个算子可以通过仔细选择布局和操作实现来使用相同的张量布局。我们的方法基于将算子分为四组，并考虑算子之间的生产者-消费者边界的组合。我们开发了一组用于搜索此类布局的方法。我们工作的另一个组成部分是为移动设备中常见的 2.5 维内存开发高效的内存布局。我们的实验结果表明，SmartMem 在 18 个不同的神经网络（包括 CNN、具有局部和全局注意力的 Transformer 以及 LLM）上优于 5 个最先进的 DNN 执行框架。特别是，与 DNNFusion 相比，SmartMem 的平均加速比为 2.8 倍，并且分别以 6.9 倍和 7.9 倍的平均加速比优于 TVM 和 MNN。
...
摘要：深度學習的興起標誌著電腦視覺、自然語言處理和醫學影像等領域的顯著進展，主要是透過調整預先訓練的模型以執行特定任務。傳統的微調方法涉及調整所有參數，由於高運算和記憶體需求而面臨挑戰。這導致了參數高效微調 (PEFT) 技術的發展，它選擇性地更新參數以平衡運算效率與效能。本篇評論探討了 PEFT 方法，提供各種策略的詳細比較，重點說明了跨不同領域的應用，包括文字生成、醫學影像、蛋白質建模和語音合成。透過評估 PEFT 方法在降低運算負載、加快訓練速度和降低記憶體使用率方面的有效性，本文有助於讓深度學習更易於使用和適應，促進其更廣泛的應用並鼓勵模型最佳化的創新。最終，本文旨在深入探討 PEFT 的演進藍圖，引導研究人員和從業者克服傳統微調方法的限制。
...
摘要：從 RGB 影片中可以萃取出微妙的週期性訊號，例如血容積脈搏和呼吸，能以低成本進行非接觸式健康監測。遠端脈搏估計技術的進展，也就是遠端光電容積描記法 (rPPG)，目前是由深度學習解決方案推動。然而，現代化方法是在基於接觸式光電容積描記法感測器的基準資料集上進行訓練和評估。我們提出第一個非對比無監督學習架構，用於訊號回歸，以減輕對標記影片資料的需求。在週期性和有限頻寬的最小假設下，我們的做法直接從未標記的影片中找出血容積脈搏。我們發現，在正常的生理頻帶限制內鼓勵稀疏功率譜和功率譜批次的變異，就足以學習週期性訊號的視覺特徵。我們執行第一個實驗，利用未標記的影片資料（並非專門為 rPPG 所建立）來訓練穩健的脈搏率估計器。在有限的歸納偏誤下，我們成功地將相同的方法應用於基於相機的呼吸，方法是改變目標訊號的頻帶限制。這顯示出此方法足夠通用，能從不同領域的頻帶限制準週期訊號進行無監督學習。此外，我們顯示出此架構有效於針對單一受試者的未標記影片進行微調模型，能讓訊號回歸器個人化和適應性。
...
摘要：Federated Learning（FL）已成為一種重要的隱私保護技術，可用於實現機密臨床機器學習等用例。FL 通過聚合由擁有資料的遠端裝置所訓練的模型來運作。因此，FL 能夠使用來自大量學習者的群眾外包資料來訓練強大的全球模型，而不會損害其隱私。然而，在產生全球模型時，聚合伺服器是單點故障。此外，當資料在所有遠端裝置上不是獨立且同分布（非 IID 資料）時，模型的效能會受到影響。這導致聚合的模型截然不同，在某些情況下可能會將效能降低多達 50%。在本文中，我們尋求解決前述問題，同時保留 FL 的優點。我們提出 MultiConfederated Learning：一個分散式 FL 架構，旨在處理非 IID 資料。與傳統 FL 不同，MultiConfederated Learning 將並行維護多個模型（而不是單一全球模型），以在資料為非 IID 時協助收斂。在轉移學習的幫助下，學習者可以收斂到較少的模型。為了提高適應性，學習者可以選擇從其同儕聚合哪些更新。
...
摘要：這項研究探討了一種新的資料擴充方法，該方法基於大型語言模型 (LLM)，用於預測美國執照醫學考試 (USMLE) 2024 年共享任務中已停用的多選題 (MCQ) 的項目難度和反應時間。我們的做法是使用零次學習 LLM（Falcon、Meditron、Mistral）的答案來擴充資料集，並採用基於六種替代特徵組合的基於轉換器的模型。結果表明，預測問題的難度更具挑戰性。值得注意的是，我們表現最好的方法始終包含問題文字，並受益於 LLM 答案的可變性，突出了 LLM 在改善醫學執照考試中自動評估的潛力。我們公開了我們的程式碼 https://github.com/ana-rogoz/BEA-2024。
...
摘要：電池健康監控和預測在電氣移動時代至關重要，對安全、永續和經濟層面有重大影響。現有研究通常專注於預測準確性，但往往忽略實際因素，這可能會阻礙技術在實際應用中的部署。在本文中，我們探討這些實際考量，並基於貝氏神經網路開發模型，以預測電池壽命結束。我們的模型使用與電池健康相關的感測器資料，並針對模型的每個參數套用分佈，而非單點。這讓模型能夠捕捉電池健康的內在隨機性和不確定性，不僅能做出準確預測，還能量化不確定性。我們進行了一項實驗研究，並展示了所提出模型的有效性，預測誤差率平均為 13.9%，而某些測試電池的誤差率低至 2.9%。此外，所有預測都包含可量化的確定性，從電池的初始階段到中壽命階段，確定性提升了 66%。這項研究對電池技術具有實用價值，有助於加速產業採用技術。
...
摘要：大型語言模型 (LLM) 的進步鼓勵在醫療保健領域採用它們，在該領域中，重要的臨床資訊通常包含在非結構化筆記中。癌症分期狀態可在臨床報告中取得，但需要自然語言處理才能從非結構化文字中提取狀態。隨著臨床導向 LLM 的進步，有望在無需大量訓練演算法的情況下提取此類狀態。引發模型推理過程的預訓練 LLM 的提示方法（例如思考鏈）可能有助於提高產生回應的可信度。使用自我一致性進一步改善模型效能，但通常會導致多個推理路徑產生不一致的結果。在本研究中，我們提出了一個整體推理方法，目的是提高模型生成的結果一致性。使用開放取用的臨床大型語言模型來確定真實世界病理報告中的病理癌症分期，我們表明整體推理方法能夠改善 LLM 在確定癌症分期方面的結果一致性和效能，從而證明了在可靠性和可信度至關重要的臨床或其他領域使用這些模型的潛力。
...
摘要：人工智能透過預測建模協助醫療保健專業人員，大幅轉變臨床決策制定。本研究探討醫療保健中 AI 應用對公平性和可解釋性的關鍵需求，以確保不同患者人口統計資料的公平結果。透過專注於敗血症相關死亡率的預測建模，我們提出了一種方法，該方法學習效能最佳化的預測模型，然後採用轉移學習流程，以產生具有更好公平性的模型。我們的模型還引入了一種新穎的基於置換的特徵重要性演算法，旨在闡明每個特徵在增強預測公平性方面的貢獻。與現有的可解釋性方法專注於解釋特徵對預測效能的貢獻不同，我們提出的方法獨特地彌補了理解每個特徵如何影響公平性的差距。鑑於敗血症顯著的死亡率及其在醫院死亡中佔三分之一的角色，這項進展至關重要。我們的模型不僅有助於識別和減輕預測模型中的偏見，還透過改善模型預測的透明度和公平性，促進醫療保健利害關係人之間的信任，進而促成更公平且值得信賴的醫療保健服務。
...
摘要：帕金森氏症是全球第二常見的神經退化性疾病。本研究旨在開發一個系統，利用混合現實功能來追蹤和評估眼睛運動。在本文中，我們提出了一個醫療場景，並概述了一個應用程式的開發，該應用程式旨在透過混合現實技術擷取眼球追蹤訊號，以評估神經退化性疾病。此外，我們還介紹了一個從眼球注視分析中萃取臨床相關特徵的管道，從醫療角度描述了所提出系統的能力。這項研究涉及一組健康對照個體和罹患帕金森氏症的患者，展示了所提出的技術在非侵入式監控眼球運動模式以診斷神經退化性疾病方面的可行性和潛力。
臨床相關性 - 迫切需要開發一種非侵入性的帕金森氏症生物標記，以準確檢測疾病的發作。這將允許在最早的階段及時引入神經保護治療，並能夠持續監控介入結果。檢測眼睛運動中細微變化的能力有助於早期診斷，在出現更明顯的症狀之前提供關鍵的介入時機。眼球追蹤提供客觀且可量化的生物標記，確保對疾病進展和認知功能進行可靠的評估。使用混合現實眼鏡進行的眼球注視分析是無線的，方便在家庭和醫院環境中進行評估。這種方法的優點是利用不需要額外專用附件的硬體，能夠透過個人眼鏡進行檢查。
...
摘要：視覺任務適應已被證明可用於採用可學習的專用層或標記來適應預先訓練的視覺轉換器 (ViT)，以適應一般下游視覺任務。然而，目前仍缺乏大規模基準來充分探討視覺任務適應對真實且重要的醫療領域的影響，特別是跨越不同的醫療視覺方式，例如彩色影像、X 光和電腦斷層掃描。為了填補此差距，我們提出 Med-VTAB，一個大規模的醫療視覺任務適應基準，包含 168 萬張針對不同器官、方式和適應方法的醫療影像。根據 Med-VTAB，我們探討了與可調整參數有關的醫療提示調整的規模定律，以及使用非醫療/醫療預訓練權重的醫療視覺適應的概括性。此外，我們研究了病患 ID 異常分佈對醫療視覺適應的影響，這是一個真實且具有挑戰性的情況。此外，Med-VTAB 的結果指出，單一的預先訓練模型在醫療任務適應中有所不足。因此，我們引入了 GMoE-Adapter，這是一種透過閘控混合專家適配器結合醫療和一般預訓練權重的創新方法，在醫療視覺任務適應中取得了最先進的成果。
...
摘要：<paragraph>憂鬱症是現今社會中一個重要的議題。根據世界衛生組織（WHO）的統計，在 2023 年，超過 2.8 億人正在與憂鬱症搏鬥。這是一個龐大的數字；如果沒有認真看待，這些數字將會快速增加。大約有 48.9 億人是社群媒體的使用者。人們在 Twitter、Facebook、Reddit、Instagram 等平台上表達自己的感受與情緒。這些平台包含了有價值的資訊，可以用於研究目的。已經在各種社群媒體平台上進行了大量的研究。然而，這些努力仍然存在某些限制。特別是，先前的研究僅專注於偵測推文中的憂鬱症和憂鬱症的嚴重程度。此外，在資料集標籤中也存在不準確的情況。在這項研究工作中，使用基於詞彙標籤的 Twitter 資料庫中的推文來預測五種類型的憂鬱症（雙極性、重度、精神病性、非典型和產後）。可解釋的 AI 用於透過強調代表憂鬱症類型的推文部分來提供推理。雙向編碼器轉換器表示（BERT）用於特徵萃取和訓練。機器學習和深度學習方法用於訓練模型。BERT 模型呈現出最有希望的結果，達到了 0.96 的整體準確度。</paragraph>
...
摘要：深度學習顯著地改變了醫學影像和放射學領域，能識別醫學影像中的病理，包括電腦斷層掃描 (CT) 和 X 光掃描。然而，深度學習模型的效能，特別是在分割任務中，通常受到廣泛標註資料集需求的限制。為了應對這項挑戰，透過可解釋 AI 和反事實解釋的生成，探討了弱監督語意分割的能力。本研究的範圍是開發一種新穎的反事實內插方法 (COIN)，它透過生成模型將預測的分類標籤從異常翻轉為正常。例如，如果分類器將輸入醫學影像 X 視為異常，表示存在病理，則生成模型旨在內插異常區域，從而逆轉分類器的原始預測標籤。該方法使我們能夠為病理產生精確的分割，而無需依賴於現有的分割遮罩。至關重要的是，利用影像層級標籤，這比建立詳細的分割遮罩容易得多。該方法的有效性透過分割愛沙尼亞塔爾圖大學醫院取得的 CT 影像中的合成目標和實際腎臟腫瘤來證明。研究結果表明，COIN 遠遠優於已建立的歸因方法，例如 RISE、ScoreCAM 和 LayerCAM，以及 Singla 等人提出的替代反事實解釋方法。這些證據表明，COIN 是一種很有前景的 CT 影像中腫瘤語意分割方法，並代表了在醫療保健中使深度學習應用更易於使用和更有效邁出了一步，而醫療保健中標註資料很稀缺。
...
摘要：影像重建是所有醫學影像方法中不可或缺的一步，
包含光聲斷層攝影 (PAT)，這是一種有前途的影像方式，
結合了超音波和光學影像方法的優點。使用傳統方法重建 PAT 影像會產生
粗糙的人工產物，特別是直接應用於稀疏 PAT 資料時。近年來，生成對抗網路 (GAN) 在影像產生和轉換方面展現強大的效能，讓它們成為應用於重建任務的明智選擇。在本研究中，我們提出了一個端對端的
方法，稱為 DensePANet，用於解決從稀疏資料重建 PAT 影像的問題。所提出的模型在其產生器中採用 UNet 的創新修改，稱為 FD-UNet++，大幅提升了重建效能。我們在各種體內和模擬資料集上評估此方法。
定量和定性結果顯示，我們的模型比其他流行深度學習技術有更好的效能。
...
摘要：本研究提出了一個基於 Transformer 架構和自注意力機制的 Multitrans 多模態融合框架。此架構結合了非對比電腦斷層掃描 (NCCT) 影像與接受中風治療病患的出院診斷報告，使用多種基於 Transformer 架構的方法來預測中風治療的功能性結果。結果顯示，單模態文本分類的表現顯著優於單模態影像分類，但多模態組合的效果優於任何單一模態。儘管 Transformer 模型僅在影像資料上表現較差，但與臨床元診斷資訊結合時，兩者都能學習到更好的互補資訊，並對準確預測中風治療效果做出良好的貢獻。
...
摘要：糖尿病在全球盛行率不断上升，这凸显了糖尿病管理的需求。最近的研究重点关注糖尿病管理中的数字生物标记，在计算框架和使用个性化葡萄糖指标的非侵入性监测技术方面进行创新。然而，它们主要集中在胰岛素剂量和特定的葡萄糖值，或者对整体血糖控制的关注有限。这在糖尿病管理中为扩大数字生物标记的范围以实现整体血糖控制留下了空白。为了解决这样的研究空白，我们提出了 GluMarker——一个端到端框架，用于使用更广泛的因素来源对数字生物标记进行建模以预测血糖控制。通过对各种机器学习基线的评估和改进，GluMarker 在预测次日血糖控制方面实现了 Anderson 数据集的最新技术。此外，我们的研究确定了次日血糖控制预测的关键数字生物标记。这些确定的生物标记有助于阐明影响血糖管理的日常因素，为糖尿病护理提供重要见解。
...
摘要：在大數據時代，整合多樣化的數據模式會帶來重大的挑戰，特別是在醫療保健等複雜領域。本文介紹了一個新的多模態數據融合數據挖掘流程模型，將嵌入式技術和跨產業標準數據挖掘流程與現有的數據融合資訊群組模型整合在一起。我們的模型旨在降低運算成本、複雜性和偏差，同時提高效率和可靠性。我們還提出了「解糾的密集融合」，這是一種新穎的嵌入式融合方法，旨在最佳化互惠資訊並促進密集的模間特徵交互作用，從而最大限度地減少冗餘資訊。我們透過三個使用案例來展示模型的效能：使用視網膜影像和患者元數據預測糖尿病視網膜病變、使用衛星影像、網路和人口普查數據預測家庭暴力，以及從放射線影像和臨床筆記中識別臨床和人口特徵。該模型在糖尿病視網膜病變預測中達到了 0.92 的巨觀 F1 分數、在家庭暴力預測中達到了 0.854 的 R 平方和 24.868 的 sMAPE，在放射學分析中，疾病預測和性別分類的巨觀 AUC 分別為 0.92 和 0.99。這些結果強調了數據融合數據挖掘模型在多模態數據處理方面具有顯著影響的潛力，促進了它在多樣化、資源受限的環境中的應用。
...
摘要：藥物推薦系統旨在提供個人化的藥物建議，這些建議與個別患者的需求緊密結合。先前的研究主要集中在開發藥物嵌入，取得顯著進展。儘管如此，這些方法通常無法準確反映個別患者的概況，這主要是因為難以區分各種患者狀況，以及無法在特定狀況和適當藥物之間建立精確的關聯性。為了回應這些問題，我們引入了 DisMed，這是一個專注於患者狀況以增強個人化的模型。DisMed 使用因果推論來辨別明確且可量化的因果關係。然後，它深入檢查患者狀況，識別並適應這些狀況不斷變化的細微差別，並將其直接對應到相應的藥物。此外，DisMed 利用來自多個患者就診的數據來提出藥物組合建議。在真實世界數據集上的全面測試表明，DisMed 不僅改善了患者概況的客製化，而且在精準度和安全性方面都超越了領先的模型。
...
摘要：多巴胺轉運體 (DAT) 影像常被用於監測帕金森氏症 (PD)，其中紋狀體 DAT 攝取量用於評估 PD 嚴重程度。然而，DAT 影像的成本高昂，且有輻射曝露的風險，且一般診所並不提供。近期，黑質區域的 MRI 影像被提議為一種更安全且更簡便的替代方案。本文提出一個對稱回歸器，用於從黑質 MRI 影像預測 DAT 攝取量。考量到左右黑質之間的對稱性，所提出的回歸器結合了一個配對輸入輸出模型，同時預測左右紋狀體的 DAT 攝取量。此外，它採用一種對稱損失函數，對左右預測之間的差異施加約束，類似於兩側 DAT 攝取量之間的高度相關性。此外，我們提出一個對稱蒙地卡羅 (MC) 中輟法，用於提供 DAT 攝取預測的可靠不確定性估計，它利用了上述對稱性。我們在 734 個黑質影像上評估所提出的方法，結果顯示對稱回歸器的效能顯著優於標準回歸器，同時提供更好的可解釋性和特徵表示。對稱 MC 中輟法也給出精確的不確定性範圍，且有很高的機率將真實的 DAT 攝取量包含在範圍內。
...
摘要：醫學影像分割通常需要廣泛的密集註解來進行模型訓練，這既耗時又需要專業技能。為了減輕這個負擔，已經引入了基於範例的醫學影像分割方法，只需一個帶註解的影像即可實現有效訓練。在本文中，我們介紹了一個新穎的跨模型互學習框架，用於基於範例的醫學影像分割 (CMEMS)，它利用兩個模型從未標記數據中挖掘隱含訊息，並具有多重粒度。CMEMS 可以消除確認偏誤，並透過強制模型之間在不同粒度上的一致性，來實現協作訓練以學習互補訊息。具體來說，透過使用微弱擾動的影像來生成高置信度偽標籤，並監督跨模型對強烈擾動影像的預測，設計了基於跨模型影像擾動的互學習。這種方法能夠共同追求影像粒度上的預測一致性。此外，透過讓偽標籤監督來自具有不同解析度的擾動多層級特徵的預測，設計了基於跨模型多層級特徵擾動的互學習，這可以擴大擾動空間並增強我們框架的穩健性。CMEMS 以端到端的方式，使用範例數據、合成數據和未標記數據進行聯合訓練。在兩個醫學影像數據集上的實驗結果表明，所提出的 CMEMS 在極其有限的監督下，優於最先進的分割方法。
...
摘要：聯邦學習 (FL) 已成為一種很有前景的隱私保護機器學習方法，特別是在醫療保健等敏感領域。在此背景下，TRUSTroke 專案旨在利用 FL 來協助臨床醫師預測缺血性中風。本文概述了 TRUSToke FL 網路基礎架構。所提出的架構採用客戶端伺服器模型，並配備中央參數伺服器 (PS)。我們為客戶端節點引進了基於 Docker 的設計，提供了一種靈活的解決方案，可在臨床環境中實施 FL 程序。分析了不同通訊協定 (HTTP 或 MQTT) 對 FL 網路運作的影響，並選擇 MQTT，因為它適用於 FL 場景。還提出了控制平面來支援 FL 程序所需的主要運作。本文最後分析了 FL 架構的安全層面，探討潛在威脅並提出緩解策略，以提高可信度。
...
摘要：機器遺忘是更新機器學習模型的過程，用於移除特定訓練資料範例的資訊，以符合資料保護法規，允許個人要求移除其個人資料。儘管最近開發了許多遺忘演算法，但這些演算法的可靠評估仍然是一個開放的研究問題。在這項工作中，我們專注於基於成員身分推論攻擊 (MIA) 的評估，這是評估遺忘演算法最常見的方法之一，並解決現有評估指標缺乏可靠性的各種缺點。具體來說，我們提出一個博弈論框架，將評估過程形式化為遺忘演算法和 MIA 對手的博弈，透過 MIA 對手的移除資料能力來衡量遺忘演算法的資料移除效能。透過仔細設計博弈，我們證明從博弈中推論出的自然評估指標享有現有評估指標無法滿足的可證明保證。此外，我們提出一個實用且有效的演算法來估計從博弈中推論出的評估指標，並透過理論分析和實證實驗證明其有效性。這項工作提出一個新穎且可靠的方法來實證評估遺忘演算法，為開發更有效的遺忘技術鋪路。
...
摘要：醫療報告生成自動化放射影像描述，
減輕醫師負擔並將錯誤降至最低。然而，目前的技術
缺乏結構化的輸出和醫師互動性，無法產生清晰、臨床上
相關的報告。我們的技術引入了提示引導方法，使用預先訓練的大型語言模型 (LLM) 來產生結構化的胸部 X 光報告。
首先，我們識別胸部 X 光中的解剖區域，以產生專注於關鍵視覺元素的句子，從而建立一個結構化的
報告基礎，包含基於解剖的句子。我們也會將偵測到的解剖結構轉換為文字提示，傳達解剖理解給 LLM。
此外，臨床背景提示引導 LLM 強調互動性和臨床需求。透過整合專注於解剖的
句子和解剖/臨床提示，預先訓練的 LLM 可以產生結構化的胸部 X 光報告，針對提示的解剖區域和
臨床背景進行調整。我們使用語言生成和臨床有效性指標進行評估，證明了強大的效能。
...
摘要：隨著全球人口老化，慢性腎臟病 (CKD) 的發生率正在上升。CKD 通常在晚期才會出現症狀，這會對醫療保健系統和患者的生活品質造成重大負擔。本研究開發了一個可解釋的機器學習系統，用於預測有心血管風險的患者的 CKD，利用病史和實驗室數據。隨機森林模型達到了 88.2% 的最高靈敏度。該研究引入了一個全面的可解釋性框架，超越了傳統的特徵重要性方法，納入了全局和局部解釋、偏差檢查、生物醫學相關性和安全性評估。在全局解釋中識別出的關鍵預測特徵是使用糖尿病和 ACEI/ARB 藥物，以及初始 eGFR 值。局部解釋通過反事實解釋提供了模型見解，這與其他系統部分一致。在進行偏差檢查後，發現初始 eGFR 值和 CKD 預測表現出一些偏差，但沒有發現顯著的性別偏差。通過範圍規則提取的模型邏輯被證實與現有的醫學文獻一致。安全評估測試了潛在的危險案例，並確認模型表現安全。此系統增強了模型的可解釋性、可靠性和問責制，促進了其潛在整合到醫療保健環境中並符合即將出台的法規標準，並顯示出在醫療保健機器學習中更廣泛應用的前景。
...
摘要：肺部感染区域分割对于评估肺部疾病的严重程度至关重要。然而，现有的图像文本多模态方法通常依赖于劳动密集型注释进行模型训练，在时间和专业知识方面提出了挑战。为了解决这个问题，我们提出了一种用于无监督肺部感染区域分割的新型属性知识指导框架 (AKGNet)，该框架仅基于图像文本数据实现分割，而无需任何掩码注释。AKGNet 同时促进了文本属性知识学习、属性图像交叉注意融合和基于高置信度的伪标签探索。它可以在嵌入空间中学习统计信息并捕获图像和文本属性之间的空间相关性，迭代细化掩码以增强分割。具体来说，我们通过提取属性知识并将其合并到特征表示中来引入文本属性知识学习模块，使模型能够学习统计信息并适应不同的属性。此外，我们通过计算嵌入空间中属性和图像之间的相关性来设计一个属性图像交叉注意模块，以捕获空间依赖性信息，从而有选择地关注相关区域，同时过滤掉不相关区域。最后，通过使用高置信度预测生成伪标签来实现自训练掩码改进过程，以迭代增强掩码和分割。在基准医学图像数据集上的实验结果表明，与无监督场景中的最先进分割技术相比，我们的方法具有卓越的性能。
...
摘要：光達 (LiDAR) 與物聯網 (IoT) 技術的整合，為都市安全與行人福祉中的公共衛生資訊學提供了變革性的機會。本文提出了一個新的架構，利用這些技術來增強 3D 物件偵測和都市交通情境中的活動分類。透過使用架高的光達，我們取得了詳細的 3D 點雲資料，能精確監控行人的活動。為了克服都市資料的稀少性，我們透過 Blender 中的模擬交通環境建立了一個專門的資料集，以利進行目標模型訓練。我們的做法採用改良的基於點體素區域的卷積神經網路 (PV-RCNN) 進行穩健的 3D 偵測，並使用 PointNet 分類行人活動，透過提供行人行為的見解並促進更安全的都市環境，顯著地受益於都市交通管理和公共衛生。我們的雙模型方法不僅增強了都市交通管理，也透過提供行人行為的見解並促進更安全的都市環境，為公共衛生做出了顯著的貢獻。
...
摘要：糖尿病患者人数不断增加是当今社会的一个严重问题，这对人们的健康和国家的财政支出产生了重大的负面影响。由于糖尿病可能发展成潜在的严重并发症，因此糖尿病患者的早期血糖预测对于及时进行医疗治疗是必要的。现有的血糖预测方法通常利用患者的私人数据（例如年龄、性别、种族）和生理参数（例如血压、心率）作为血糖预测的参考特征，这不可避免地导致了隐私保护问题。此外，这些模型通常专注于长期（基于月度）或短期（基于分钟）预测。长期预测方法通常不准确，因为外部不确定性会极大地影响血糖值，而短期预测方法则无法提供及时的医疗指导。基于上述问题，我们提出了 CrossGP，这是一种新颖的机器学习框架，用于跨天血糖预测，仅基于患者的外部活动，不涉及任何生理参数。同时，我们实现了三个基线模型进行比较。在 Anderson 的数据集上进行的广泛实验有力地证明了 CrossGP 的优越性能，并证明了其在未来实际应用中的潜力。
...
摘要：最近，原型學習已出現在半監督醫療影像分割中，並取得顯著的表現。然而，標籤資料的稀少性限制了先前方法中原型的表現力，可能會阻礙類別嵌入原型的完整表示。為了解決這個問題，我們提出了混合原型一致性學習 (MPCL) 框架，其中包括一個平均教師和一個輔助網路。平均教師會為標籤和未標籤資料產生原型，而輔助網路會為 CutMix 處理的混合資料產生額外的原型。透過原型融合，混合原型為標籤和未標籤原型提供額外的語義資訊。每個類別的高品質全局原型是透過融合兩個增強的原型形成，優化一致性學習中使用的隱藏嵌入分佈。在左心房和 B 型主動脈剝離資料集上的廣泛實驗證明了 MPCL 優於以往的最新方法，確認了我們框架的有效性。程式碼將很快發布。
...
摘要：重組腺相關病毒 (rAAV) 載體徹底改變了基因療法，但它們廣泛的趨向性和次優的轉導效率限制了它們的臨床應用。為了克服這些限制，研究人員專注於設計和篩選衣殼庫，以識別改良的載體。然而，龐大的序列空間和有限的資源在識別可行的衣殼變體方面提出了挑戰。在本研究中，我們提出了一個端到端的擴散模型，以產生具有增強活力的衣殼序列。使用公開的 AAV2 資料，我們生成了 38,000 個不同的 AAV2 病毒蛋白 (VP) 序列，並評估了 8,000 個用於病毒選擇。結果證明了我們的模型優於傳統方法。此外，在沒有 AAV9 衣殼資料的情況下，除了使用一個野生型序列外，我們還使用相同的模型直接產生了許多具有多達 9 個突變的可行序列。我們將剩下的 30,000 個樣本轉移到 AAV9 結構域。此外，我們對 AAV9 VP 高變異區 VI 和 V 進行了誘變，有助於持續改進 AAV9 VP 序列。這項研究代表了 rAAV 載體設計和功能驗證的重大進展，為提高基因療法應用中的特異性和轉導效率提供了創新的解決方案。
...
摘要：自然語言處理 (NLP) 領域隨著深度學習技術的快速發展取得了顯著進展。文本情緒分析的研究方向之一是醫療文本的情緒分析，這在臨床診斷應用中具有巨大的潛力。然而，醫療領域目前缺乏足夠的文本資料集，而情緒分析的有效性受到不同模型設計方法的很大影響，這提出了挑戰。因此，本文專注於醫療領域，使用來自 Transformer 的雙向編碼器表示 (BERT) 作為預訓練的基本模型，並在輸出層對卷積神經網路 (CNN)、全連接網路 (FCN) 和圖形卷積網路 (GCN) 等模組進行實驗。在 METS-CoV 資料集上進行了實驗和分析，以探討整合不同深度學習網路後的訓練效能。結果表明，在與 BERT 等預訓練模型結合使用時，CNN 模型在較小的醫療文本資料集上訓練時優於其他網路。這項研究強調了模型選擇在醫療領域實現有效情緒分析中的重要性，並為未來研究開發更有效的模型架構提供了參考。
...
摘要：影像辨識技術高度仰賴豐富的標籤資料，特別是在醫療情境中。因應取得標籤資料的挑戰，促使自我監督學習與半監督學習的重要性提升，特別是在標註資料有限的場景中。在本文中，我們提出一個創新的方法，將自我監督學習整合到半監督模型中，以增強醫學影像辨識。我們的做法從使用 BYOL 方法在未標籤資料上進行預訓練開始。隨後，我們合併偽標籤和標籤資料集，建構一個神經網路分類器，並透過反覆微調來精進它。在三個不同資料集上的實驗結果顯示，我們的做法最佳化地利用未標籤資料，在醫學影像辨識的準確度方面優於現有方法。
...
摘要：可信度是將不透明深度學習模型安全應用於高風險領域（如醫學）的主要先決條件。了解決策過程不僅有助於培養信任，而且還可能揭示複雜模型以前未知的決策標準，這可能會促進醫學研究的進展。從黑盒模型中發現與決策相關的概念是一項特別具有挑戰性的任務。本研究提出了通過潛在擴散對抗事實軌跡（CDCT）進行概念發現，這是一個用於概念發現的新型三步驟框架，利用了擴散模型卓越的圖像合成能力。在第一步中，CDCT 使用潛在擴散模型 (LDM) 來生成對抗事實軌跡數據集。該數據集用於使用變分自動編碼器 (VAE) 推導與分類相關概念的解糾纏表示。最後，應用搜索算法來識別解糾纏潛在空間中的相關概念。將 CDCT 應用於在最大的公共皮膚病變數據集上訓練的分類器，不僅揭示了多種偏差的存在，還揭示了有意義的生物標記。此外，在 CDCT 中生成的對抗事實比以前建立的最新方法產生的對抗事實顯示出更好的 FID 分數，同時資源效率提高了 12 倍。無監督概念發現對可信賴 AI 的應用以及各個領域人類知識的進一步發展具有巨大的潛力。CDCT 代表著朝這個方向邁出的進一步一步。
...
摘要：異常偵測在風力渦輪機的預測性維護領域中扮演關鍵的角色，然而，由於特定領域的公開資料集相當稀少，因此不同演算法的比較構成了困難的任務。許多不同方法的比較不是使用由來自許多不同領域的資料組成的基準，就是使用難以取得的資料，或使用少數公開可用的資料集之一，而這些資料集缺乏關於故障的詳細資訊。此外，許多出版物強調了故障偵測成功的幾個案例研究。透過這篇論文，我們公開了一個高品質的資料集，其中包含來自 36 部風力渦輪機（橫跨 3 個不同的風力發電場）的資料，以及迄今為止我們所知最詳細的任何公開風力渦輪機資料集的故障資訊。新的資料集包含 89 年的風力渦輪機實際運作資料，分佈在 44 個標記時間框架中，這些時間框架導致故障，以及代表正常行為的 51 個時間序列。此外，透過基於渦輪機狀態的標籤，可以確保訓練資料的品質。此外，我們提出了一種新的評分方法，稱為 CARE（涵蓋範圍、準確性、可靠性和早期性），它利用資料集中存在的資訊深度來識別一個良好的全方位異常偵測模型。此評分考量異常偵測效能、適當辨識正常行為的能力，以及在及早偵測異常的同時，盡可能減少誤報的可能性。
...
摘要：最近，隨著健康意識的增長，新的方法讓個人能夠在家中監控睡眠。利用睡眠聲音相較於智慧手錶等傳統方法，具有非侵入性且能夠偵測各種生理活動的優勢。本研究旨在建構一個基於機器學習的睡眠評估模型，提供基於證據的評估，例如由於睡眠開始時頻繁移動而導致的睡眠不良。提取睡眠聲音事件、使用 VAE 推導潛在表徵、使用 GMM 進行聚類以及訓練 LSTM 以進行主觀睡眠評估，在區分睡眠滿意度方面達到了 94.8% 的高準確度。此外，TimeSHAP 揭示了不同個人在影響力大的聲音事件類型和時間上的差異。
...
摘要：大腦網路顯示出階層化組織，這種複雜性對現有的深度學習模型構成挑戰，後者通常被結構化為扁平分類器，導致難以解釋和「黑盒子」問題。為了彌合這個差距，我們提出了一種新穎的架構：一個由弱監督和新興語言 (EL) 架構告知的符號自動編碼器。這個模型透過產生階層群集和對應的影像，超越傳統的扁平分類器，隨後透過符號句子表示，以改善臨床可解釋性，例如內在腦網路等階層化組織的資料，可以用靜態功能性磁振造影影像來表徵。我們的創新包括一個廣義的階層化損失函數，旨在確保句子和影像都準確反映功能性腦網路的階層結構。這使我們能夠從更廣泛的角度對功能性腦網路建模，直到更精細的細節。此外，我們引入了一種量化方法來評估這些符號表示的階層一致性。我們的定性分析顯示，我們的模型成功產生了階層化組織、臨床上可解釋的影像，我們的定量評估支持了這一發現。我們發現，我們效能最好的損失函數在識別與腦網路對應的影像時，會導致超過 97% 的階層一致性。這種方法不僅提升了深度學習模型在神經影像分析中的可解釋性，也代表了朝向建模腦網路複雜的階層性質邁出的重要一步。
...
摘要：<paragraph>人工智慧的最新突破有望從根本上提升我們對醫療保健的研究和理解。開發一個整合多對多的架構，利用多重資料模式輸入進行多重醫療任務的分析建模，對現代醫學的統一理解至關重要。在這項工作中，我們引入了 M3H，一個可解釋的多模態多任務機器學習醫療保健架構，它整合了來自廣泛醫療任務類別和機器學習問題類別的多樣化多模態輸入的學習。這個架構的模組化設計確保了其資料處理、任務定義和快速模型建構的可概化性，適用於臨床和運作醫療保健環境。我們透過驗證從四種模態（表格、時間序列、語言和視覺）訓練的模型，在四個機器學習問題類別中執行 41 項醫療任務，來評估 M3H 架構。我們的結果表明，M3H 持續產生多任務模型，其表現優於標準單任務模型（1.1-37.2%），涵蓋來自 16 個醫療部門的 37 種疾病診斷、三個醫院運作預測和一個患者表型分析任務：跨越了機器學習問題類別的監督二元分類、多類分類、迴歸和聚類。此外，這個架構還引入了一種新穎的注意力機制，以平衡自我開發（專注於學習來源任務）和交叉探索（鼓勵從其他任務中學習）。此外，M3H 提供了可解釋性見解，說明額外任務的聯合學習如何透過提議的 TIM 分數影響來源任務的學習，從而闡明任務相互依賴的動態。其適應性架構促進了自訂和整合，使其成為未來人工智能驅動醫療保健系統的強大且可擴充的候選解決方案。</paragraph>
...
摘要：本文探討了應用 Swarm-Structured Multi-Agent Systems (MAS) 來建立醫療必要性，這是一個涉及系統性檢視特定患者醫療結構化和非結構化數據，並與臨床指南進行比對的程序。我們透過將這個複雜的任務分解成較小、更易管理的子任務來處理這個問題。每個子任務都由一個專門的 AI 代理處理。我們對各種提示策略對這些代理的影響進行系統性研究，並對不同的大型語言模型 (LLM) 進行基準測試，以確定它們完成這些任務的準確性。此外，我們探討這些代理如何提供可解釋性，進而增強系統內的信任和透明度。
...
摘要：人工智能 (AI) 的普遍整合在涉及人工智能系統的事件中引入了責任和問責方面的複雜挑戰。這些系統的互聯性、AI 引發事件的倫理問題，加上 AI 技術的不確定性和相應法規的缺失，使得傳統責任歸屬具有挑戰性。為此，本文提出了一種計算反射平衡 (CRE) 方法，以建立一個對所有利益相關者而言既連貫又符合道德的責任歸屬架構。計算方法提供了一種結構化的分析，克服了概念方法在處理動態和多方面場景時的局限性，展示了該框架在責任歸屬過程中可解釋性、連貫性和適應性屬性。我們探討了與平衡計算中索賠相關的初始激活級別的關鍵作用。使用 AI 輔助醫療決策支援系統作為案例研究，我們說明了不同的初始化如何導致不同的責任分配。該框架為 AI 引發事件中的問責制提供了寶貴的見解，通過持續監控、修訂和反思促進了可持續和彈性系統的開發。
...
摘要：人工智慧透過預測建模協助醫療保健專業人員，大幅轉化臨床決策制定。本研究探討醫療保健中 AI 應用中公平性和可解釋性的關鍵需求，以確保在多元的患者人口統計資料中獲得公平的結果。透過專注於敗血症相關死亡率的預測建模，我們提出了一種方法，該方法學習效能最佳化的預測模型，然後採用遷移學習流程來產生具有更佳公平性的模型。我們的模型也引進了一種新穎的基於置換的特徵重要性演算法，旨在闡明每個特徵在增強預測公平性中的貢獻。與現有的可解釋性方法（專注於解釋特徵對預測效能的貢獻）不同，我們提出的方法獨特地彌合了理解每個特徵如何促成公平性的鴻溝。這項進展至關重要，因為敗血症的死亡率很高，且在三分之一的醫院死亡病例中扮演角色。我們的模型不僅有助於識別和減輕預測模型中的偏差，還能透過改善模型預測的透明度和公平性來促進醫療保健利害關係人之間的信任，進而促成更公平且值得信賴的醫療保健服務。
...
摘要：現今憂鬱症為一項重大的議題。根據世界衛生組織（WHO），2023 年，超過 2 億 8 千萬人正與憂鬱症搏鬥。這是一個龐大的數字；若不認真看待，這些數字將會快速增加。約有 48.9 億人為社群媒體使用者。人們在 Twitter、Facebook、Reddit、Instagram 等平台上表達自己的感受與情緒。這些平台包含有價值的資訊，可用於研究目的。已在各種社群媒體平台上進行大量的研究。然而，這些努力仍存在某些限制。特別是，先前的研究僅專注於偵測推文中的憂鬱症和憂鬱症的嚴重程度。此外，資料集標記中存在不準確之處。在這項研究工作中，根據詞彙標記，使用來自 Twitter 資料庫的推文預測了五種類型的憂鬱症（雙極性、重度、精神病性、非典型和產後）。可解釋的人工智慧用於提供推理，藉由突顯代表憂鬱症類型的推文部分。來自 Transformer 的雙向編碼器表示（BERT）用於特徵萃取和訓練。機器學習和深度學習方法用於訓練模型。BERT 模型呈現最令人滿意的結果，達到 0.96 的整體準確度。
...
摘要：深度學習正劇烈地轉變醫療影像和放射學領域，能識別醫療影像中的病理，包括電腦斷層掃描 (CT) 和 X 光掃描。然而，深度學習模型的效能，特別是在分割任務中，通常受到廣泛註解資料集需求的限制。為了應對此挑戰，透過可解釋 AI 和反事實解釋的產生，探討了弱監督語意分割的功能。本研究的範圍是開發一種新穎的反事實內插方法 (COIN)，透過使用生成模型，將預測的分類標籤從異常翻轉為正常。例如，如果分類器將輸入的醫療影像 X 視為異常，表示存在病理，則生成模型旨在內插異常區域，從而逆轉分類器的原始預測標籤。此方法使我們能夠為病理產生精確的分割，而無需依賴於預先存在的分割遮罩。至關重要的是，利用影像級別標籤，其比建立詳細的分割遮罩容易取得得多。該方法的有效性透過分割合成目標和愛沙尼亞塔爾圖大學醫院取得的 CT 影像中的實際腎臟腫瘤得到證明。研究結果表明，COIN 遠遠優於已建立的歸因方法，例如 RISE、ScoreCAM 和 LayerCAM，以及 Singla 等人提出的替代反事實解釋方法。此證據表明，COIN 是 CT 影像中腫瘤語意分割的一種有前途的方法，並代表了讓深度學習應用在醫療保健中更易於取得且更有效的一步，而醫療保健中註解資料很稀少。
...
摘要：甲狀腺癌是一個日益嚴重的全球健康問題，需要進階的診斷方法。本篇評論探討人工智慧和放射特徵在甲狀腺癌診斷中的應用。根據 PRISMA 指南，我們在 2023 年 10 月前檢閱了多個資料庫。結合多個關鍵字，我們發現了一篇關於甲狀腺癌和相關主題的英文學術出版品。在移除 109 篇重複文獻後，原始搜尋結果回傳了 267 篇論文。根據預先設定的標準，在根據摘要和標題審查了 124 篇文章後，我們選出了相關研究。在進行全面分析後，我們排除了另外六項研究。在納入的 28 項研究中，結合超音波 (US) 影像的放射特徵分析證明了其在診斷甲狀腺癌方面的有效性。研究結果不一，有些研究提出了優於現狀的新策略。文獻強調了人工智慧模型面臨的各種挑戰，包括可解釋性問題、資料集限制和操作員依賴性。這 28 項納入研究的綜合發現提到，需要標準化工作和前瞻性多中心研究來解決這些問題。此外，我們也找出克服這些障礙的方法，例如可解釋人工智慧技術和個人化醫療技術的進步。本篇評論重點探討人工智慧和放射特徵如何轉變甲狀腺癌的診斷和治療。儘管面臨挑戰，未來針對多學科合作、臨床適用性驗證和演算法改進的研究仍有潛力改善甲狀腺癌治療的患者預後和診斷精準度。
...
摘要：乳癌近年來盛行率快速上升，使其成為全球主要的死亡原因之一。在所有癌症中，它迄今為止是最常見的。手動診斷此疾病需要大量時間和專業知識。由於乳癌的檢測是一個耗時的過程，因此透過建立基於機器學習的預測，可以幫助防止其進一步擴散。機器學習和可解釋 AI 在分類中至關重要，因為它們不僅提供準確的預測，還提供模型如何做出決策的見解，有助於理解和信任分類結果。在本研究中，我們評估並比較了五種不同的機器學習方法使用主要數據集（達卡醫學院醫院的 500 名患者）的分類準確度、精確度、召回率和 F1 分數。五種不同的監督式機器學習技術，包括決策樹、隨機森林、邏輯迴歸、樸素貝葉斯和 XGBoost，已被用於在我們的數據集上實現最佳結果。此外，本研究將 SHAP 分析應用於 XGBoost 模型，以解釋模型的預測並了解每個特徵對模型輸出的影響。我們比較了幾種演算法對數據進行分類的準確度，並與該領域的其他文獻進行了對比。經過最終評估，本研究發現 XGBoost 達到了最佳模型準確度，即 97%。
...
摘要：深度學習 (DL) 用於從乳房攝影檢查影像診斷乳癌的模型通常運作為「黑盒子」，這讓醫療保健專業人員難以信任和理解其決策過程。本研究提出了一個整合架構，結合卷積神經網路 (CNN) 和可解釋人工智慧 (XAI)，以使用 CBIS-DDSM 資料集增強乳癌診斷。該方法包含一個精細的資料前處理管道和先進的資料擴充技術，以對抗資料集限制，並採用使用預先訓練網路（例如 VGG-16、Inception-V3 和 ResNet）的遷移學習。我們研究的重點是評估 XAI 在解釋模型預測方面的有效性，重點在於利用豪斯多夫測度來評估 AI 生成的解釋和專家註釋之間的對齊程度。這種方法對於 XAI 在促進 AI 輔助診斷中的可信度和道德公平性至關重要。我們研究的結果說明了 CNN 和 XAI 在推進乳癌診斷方法中的有效合作，從而促進了先進 AI 技術在臨床環境中的更無縫整合。透過增強 AI 驅動決策的可解釋性，這項工作為 AI 系統和醫療從業人員之間的更好合作奠定了基礎，最終豐富了患者照護。此外，我們研究的影響遠遠超出了當前的研究方法。它鼓勵進一步研究如何結合多模式資料和改善 AI 解釋，以滿足臨床實務的需求。
...
摘要：以人为本的可解释 AI（HCXAI）倡导将社会层面整合到 AI 解释中。HCXAI 话语的核心是社会透明度（ST）框架，其旨在让 AI 系统的社会组织背景对用户而言易于理解。在这项工作中，我们建议扩展 ST 框架以解决大型语言模型（LLM）中社会错误归因的风险，尤其是在心理健康等敏感领域。事实上，LLM 具有惊人的角色和人物模拟能力，可能导致设计者的意图与用户对社会属性的认知之间产生错位，从而存在助长情绪操纵和危险行为、认知不公正和不合理信任的风险。为了解决这些问题，我们建议用第五个“W 问题”来增强 ST 框架，以澄清设计者和用户赋予 LLM 的具体社会属性。这一补充旨在弥合 LLM 能力与用户认知之间的差距，促进基于 LLM 的技术的道德负责开发和使用。
...
摘要：<paragraph>背景：氣胸是一種急性胸腔疾病，是由肺部和胸壁之間異常的空氣聚集所造成。為了解決深度學習（DL）模型經常伴隨的不透明性，可解釋人工智慧（XAI）方法已被引入，用於勾勒出與 DL 模型所做的氣胸診斷相關的區域。然而，這些解釋有時會與實際病灶區域有所出入，突顯出進一步改進的必要性。方法：我們提出一個模板導向的方法，將氣胸的臨床知識納入 XAI 方法所產生的模型解釋中，從而提升這些解釋的品質。利用放射科醫師所建立的一個病灶描繪，我們的做法首先產生一個模板，表示氣胸可能發生的區域。然後將此模板疊加在模型解釋上，以篩選出落在模板邊界之外的多餘解釋。為了驗證其效能，我們對三個 XAI 方法進行了比較分析，在解釋兩個真實世界資料集中的兩個 DL 模型時，使用了和未使用的模板指導。結果：所提出的方法在建立於三個 XAI 方法、兩個 DL 模型和兩個資料集的十二個基準情境中，始終如一地改進了基準 XAI 方法。在比較模型解釋和真實病灶區域時，透過效能改進計算出的平均增量百分比，在聯合交集（IoU）中為 97.8%，在骰子相似係數（DSC）中為 94.1%。結論：在氣胸診斷的背景下，我們提出了一個模板導向的方法來改進 AI 解釋。我們預期我們的模板指導將透過整合臨床領域專業知識，為闡明 AI 模型建立一個新的方法。</paragraph>
...
摘要：<paragraph>在當前的機器翻譯 (MT) 領域中，Transformer 架構脫穎而出，成為黃金標準，特別是對於高資源語言對而言。本研究深入探討其對低資源語言對的功效，包括英語↔愛爾蘭語和英語↔馬拉地語語言對。值得注意的是，該研究確定了最佳超參數和子詞模型類型，以顯著提高低資源語言對的 Transformer 模型的翻譯品質。
  低資源語言的平行語料庫的稀缺性會阻礙 MT 的發展。為了解決這個問題，開發了 gaHealth，這是第一個愛爾蘭語的健康數據雙語語料庫。專注於健康領域，使用此領域內數據集開發的模型與 LoResMT2021 共享任務的模型相比，在 BLEU 分數方面表現出非常顯著的進步。隨後使用多維品質指標錯誤分類法進行的人工評估顯示，與基於 RNN 的對應模型相比，Transformer 系統在減少準確性和流暢性錯誤方面表現出卓越的性能。
  此外，本論文介紹了 adaptNMT 和 adaptMLLM，這兩個開源應用程式專門用於神經機器翻譯模型的開發、微調和部署。這些工具大大簡化了設定和評估過程，使開發人員和翻譯人員都能更輕鬆地使用 MT。值得注意的是，植基於 OpenNMT 生態系統的 adaptNMT，透過強調模型開發的環境足跡，促進了生態友善的自然語言處理研究。與 LoResMT2021 共享任務的基準相比，adaptMLLM 對 MLLM 的微調證明了英語↔愛爾蘭語和英語↔馬拉地語這兩個低資源語言對的翻譯性能有了進步。</paragraph>
...
摘要：糖尿病 (DM) 會讓患者容易出現血管併發症。
視網膜影像和血管反映了身體的微血管和巨血管健康狀況。可利用這些影像和血管診斷糖尿病併發症，包括糖尿病視網膜病變 (DR)、神經病變、腎病變和動脈粥狀硬化性心血管疾病，還能預測心血管事件風險。已經在臨床上採用了利用數位化視網膜影像進行高通量 DR 檢測而開發的人工智慧 (AI) 系統。除了 DR 篩檢外，AI 整合也極具潛力，能解決與糖尿病患者整體照護相關的挑戰。在這項工作中，我們旨在全面檢視與糖尿病診斷、預後和管理相關的視網膜影像 AI 應用研究文獻。我們將描述整體 AI 協助糖尿病照護的發現，包括但不限於 DR 篩檢，並討論實施此類系統的障礙，包括有關倫理、資料隱私、公平取得和解釋性的問題。AI 協助視網膜影像分析具備評估患者健康狀態與糖尿病併發症，以及預後未來心血管併發症風險的能力，因此有潛力成為糖尿病患者現代化個人化醫療的中心工具。
...
摘要：本研究從多個利益相關者的角度調查了不同人工智能 (AI) 應用在教育中的可接受性，包括學生、教師和家長。承認 AI 在教育中的轉型潛力，它解決了與數據隱私、AI 代理、透明度、可解釋性和 AI 的道德部署相關的問題。通過小插曲方法，向參與者展示了四種場景，其中 AI 的代理、透明度、可解釋性和隱私被操縱。在每個場景之後，參與者完成了一項調查，其中記錄了他們對 AI 的整體效用、個別效用、公正性、信心、風險和使用每個場景的 AI（如果可用）的意圖的看法。收集的數據包括 1198 名多利益相關者參與者的最終樣本，通過合作機構和社交媒體活動進行分發，並重點關注對四個 AI 使用案例的個別回應。對數據的調解分析表明，對 AI 的接受度和信任在利益相關者群體中存在顯著差異。我們發現，AI 的代理、透明度和可解釋性的高低水平之間的關鍵調解因素，以及使用不同教育 AI 的意圖，包括感知到的整體效用、公正性和信心。該研究強調，在教育中接受 AI 是個微妙且多方面的問題，除了不同的利益相關者的看法外，還需要仔細考慮具體的 AI 應用及其特徵。
...
摘要：遠端病人監控基於可穿戴式單導程心電圖 (ECG) 裝置，對於早期偵測心臟疾病具有顯著的潛力，特別是結合人工智慧 (AI) 方法進行自動化的心臟疾病偵測。過去有研究應用基於深度學習的 AI 方法進行心臟疾病偵測。然而，這些模型尚未被廣泛接受為臨床診斷的可靠輔助工具，部分原因是圍繞許多 AI 演算法的當前黑盒子認知。特別是，有必要找出有助於做出準確診斷的 ECG 訊號關鍵特徵，從而增強模型的可解釋性。在本研究中，我們開發了一種視覺轉換器方法，以根據單導程 ECG 資料辨識心房顫動。殘差網路 (ResNet) 方法也與視覺轉換器方法進行比較而開發出來。這些模型應用於 Chapman-Shaoxing 資料集，以分類心房顫動，以及另一種常見心律不整，竇性心動過緩，和正常竇性心律的心跳。這些模型能夠辨識決定最終分類的心跳關鍵區域，並強調 P 波和 T 波的重要性，以及心跳持續時間和訊號振幅，以區分正常竇性心律、心房顫動和竇性心動過緩。
...
摘要：這篇論文介紹了一個新的範例，用於憂鬱症的偵測和治療，使用先進的大語言模型 (LLM)：生成式預訓練Transformer 4 (GPT-4)、Llama 2 聊天機器人，以及 Gemini。這些 LLM 使用專門的提示進行微調，以診斷、解釋和建議憂鬱症的治療介入措施。一種獨特的少次提示方法增強了模型根據 DSM-5 標準分析和解釋憂鬱症狀的能力。在互動階段，這些模型從事同理心的對話管理，利用 PsychDB 和認知行為療法 (CBT) 指南等資源，促進與經歷重度憂鬱症的人們的支持性互動。此外，這項研究還介紹了 Illuminate 資料庫，其中包含各種 CBT 模組，有助於個人化治療建議。這項研究使用 F1 分數、準確度、召回率、餘弦相似性和面向召回率的評估替身 (ROUGE) 等指標評估不同測試集中的 LLM 效能，證明了它們的有效性。這種綜合方法將尖端的 AI 與既定的心理方法相結合，為心理健康照護提供了新的可能性，並展示了 LLM 在革新憂鬱症診斷和治療策略方面的潛力。
...
摘要：<paragraph>人工智慧系統的說明很少會針對受演算法決策 (ADM) 影響的人們的資訊需求。傳達的資訊與受影響利害關係人重視的資訊之間的差距可能會阻礙人們了解和遵守法規架構，例如 AI 法案。為了解決這個差距，我們提出了「XAI 新手問題庫」：受影響利害關係人資訊需求的目錄，涵蓋兩個 ADM 使用案例（就業預測和健康監測），涵蓋資料、系統脈絡、系統使用和系統規格等類別。資訊需求是透過訪談研究收集的，參與者會針對他們的詢問收到說明。參與者進一步報告了他們的理解和決策信心，顯示雖然在收到說明後，信心傾向於增加，但參與者也遇到了理解上的挑戰，例如無法說明為什麼他們的理解感覺不完整。說明進一步影響了參與者對系統風險和好處的看法，他們會根據使用案例確認或更改這些看法。當風險被認為很高時，參與者表示特別有興趣了解意圖的說明，例如系統為何以及為了什麼目的而建立。透過這項工作，我們旨在透過提供資訊和挑戰的概觀，在受影響利害關係人決定採用 ADM 系統時，協助他們了解相關資訊，進而支持他們參與可解釋性。最後，我們將我們的發現總結為六項關鍵影響的清單，這些影響會告知未來針對受影響利害關係人受眾的說明的設計。</paragraph>
...
摘要：人工智能 (AI) 的快速演進，特別是在大型語言模型 (LLM) 和生成式 AI 的領域，為各種領域的應用開啟了新的途徑，但其在商業教育中的作用仍未得到充分探討。本研究首次引入了基準，用於評估七個主要 LLM 的效能，包括 OpenAI 的模型 (GPT-3.5 Turbo、GPT-4 和 GPT-4 Turbo)、Google 的模型 (PaLM 2、Gemini 1.0 Pro) 和 Anthropic 的模型 (Claude 2 和 Claude 2.1)，這些模型用於 GMAT，這是研究生商業課程入學考試中的關鍵考試。我們的分析顯示，大多數 LLM 的表現都優於人類考生，其中 GPT-4 Turbo 不僅優於其他模型，還超越了頂尖商學院的研究生平均分數。透過案例研究，本研究探討了 GPT-4 Turbo 解釋答案、評估回應、辨識錯誤、調整說明和產生替代情境的的能力。與前代版本相比，最新的 LLM 版本 GPT-4 Turbo、Claude 2.1 和 Gemini 1.0 Pro 在推理任務方面有顯著的進步，凸顯了它們在複雜問題解決方面的潛力。儘管 AI 在教育、評量和輔導方面的應用前景明確，但仍存在挑戰。我們的研究不僅闡明了 LLM 的學術潛力，也強調了在教育中謹慎開發和應用 AI 的必要性。隨著 AI 技術的進步，建立 AI 互動的架構和協定、驗證 AI 生成的內容的準確性、確保全球各地不同的學習者都能使用，並創造一個 AI 支援人類專業知識的教育環境至關重要。本研究為進一步探討負責任地使用 AI 以豐富教育體驗並改善考試準備和評量方法奠定了基礎。
...
摘要：預測加護病房 (ICU) 病患的院內死亡率，是最終臨床結果的關鍵。AI 已展現出優異的準確度，但缺乏可解釋性。為了解決這個問題，本文提出可解釋的多模式死亡率預測器 (X-MMP)，提供一種有效、可解釋的 AI 解決方案，透過多模式 ICU 資料預測院內死亡率。我們在架構中採用多模式學習，可以接收來自臨床資料的異質輸入並做出決策。此外，我們引入一種可解釋的方法，即層級傳播到 Transformer，作為 LRP 方法到 Transformer 的適當延伸，對多模式輸入產生解釋，並揭示歸因於預測的顯著特徵。此外，每種模式對臨床結果的貢獻可以視覺化，協助臨床醫生了解決策背後的推理。我們根據 MIMIC-III 和 MIMIC-III 波形資料庫匹配子集建構一個多模式資料集。基於基準資料集的綜合實驗證明，我們提出的架構可以實現合理的解釋，並具有競爭力的預測準確度。特別是，我們的架構可以輕鬆轉移到其他臨床任務，這有助於發現醫療保健研究中的關鍵因素。
...
摘要：在過去十年中，病理學中的人工智慧 (AI) 方法已大幅進步。然而，由於在將研究結果轉化為臨床診斷產品以及缺乏標準化介面上存在技術和法規障礙等眾多挑戰，導致整合到常規臨床實務中進展緩慢。開放且與供應商無關的 EMPAIA 計畫應對了這些挑戰。在此，我們概述了 EMPAIA 的成就和經驗教訓。EMPAIA 整合了病理學 AI 生態系統的各種利害關係人，即病理學家、電腦科學家和產業。在密切合作下，我們制定了技術互操作性標準、AI 測試和產品開發建議，以及可解釋性方法。我們實作了模組化且開源的 EMPAIA 平台，並成功整合了來自 8 家不同供應商的 14 個基於 AI 的影像分析應用程式，展示了不同的應用程式如何使用單一標準化介面。我們優先考慮需求，並評估了在歐洲和亞洲 14 個不同的病理實驗室中使用 AI 的實際臨床設定。除了技術開發外，我們還為所有利害關係人建立了一個論壇，以分享數位病理學和 AI 的資訊和經驗。商業、臨床和學術利害關係人現在可以採用 EMPAIA 共用的開源介面，為大規模標準化和簡化流程提供了獨特的機會。需要進一步的努力才能有效且廣泛地在常規實驗室使用中建立 AI 協助。為此，已建立永續基礎架構，即非營利協會 EMPAIA International，以繼續標準化並支援廣泛實作和倡導 AI 輔助數位病理學的未來。
...
摘要：反事實解釋 (CE) 技術作為向使用 AI 系統的使用者提供見解的一種方法而備受關注。雖然在醫學影像和自動駕駛等領域廣泛研究，但圖形反事實解釋 (GCE) 方法相對而言尚未得到充分探索。GCE 會產生一個與原始圖形相似的新的圖形，其不同的結果基於底層預測模型。在這些 GCE 技術中，儘管在其他領域（例如藝術風格和自然語言建模）中展示了令人印象深刻的成就，但植根於生成機制的技術卻受到相對有限的研究。對生成解釋器的偏好源於它們在推理過程中生成反事實實例的能力，利用輸入圖形的自主採集的擾動。基於上述基本原理，我們的研究引入了 RSGG-CE，這是一種新的穩健隨機圖形生成器，用於反事實解釋，能夠從學習到的潛在空間中產生反事實範例，考慮部分有序的生成序列。此外，我們進行定量和定性分析，以比較 RSGG-CE 的效能與 SoA 生成解釋器，突顯其產生合理解釋候選項的能力有所提升。
...
摘要：可解釋 AI 的動機之一是讓人類能夠對 AI 模型的使用和部署做出更好、更明智的決策。但需要仔細評估才能評估是否已達到此預期。目前的評估主要集中在解釋的演算法特性，而涉及人類受試者的評估通常使用主觀問題來測試人類對解釋有用性的看法，而沒有依據客觀的指標和測量。在這項工作中，我們評估了解釋是否能改善人類在機器學習模型開發的實際場景中做決策的能力。我們進行一項包含影像資料的混合方法使用者研究，以評估 SmoothGrad、GradCAM 和一個神諭解釋在兩個任務中產生的顯著性圖：模型選擇和反事實模擬。令我們驚訝的是，我們沒有發現任何證據顯示當使用者獲得任何顯著性圖時，這些任務有顯著改善，即使是設計成易於理解且高度指示答案的合成神諭解釋也是如此。儘管如此，解釋確實有助於使用者更準確地描述模型。這些發現顯示出對於基於顯著性的解釋的實用性和潛在誤解，需要謹慎。
...
摘要：可解釋性和安全性會產生信任。這些需要一個模型來展現一致性和可靠性。要達成這些，必須使用和分析資料和知識，使用與 AI 應用相關的統計和符號 AI 方法，單獨使用其中一種方法是不行的。因此，我們主張並試圖證明神經符號 AI 方法更適合用來讓 AI 變成值得信賴的 AI 系統。我們提出 CREST 架構，展示一致性、可靠性、使用者層級的可解釋性，以及安全性如何建立在神經符號方法上，這些方法使用資料和知識來支援關鍵應用（例如健康和福祉）的要求。本文著重於大型語言模型 (LLM)，視其為 CREST 架構中所選擇的 AI 系統。LLM 由於在處理廣泛的自然語言處理 (NLP) 場景時具有多功能性，因此獲得研究人員的大量關注。例如，ChatGPT 和 Google 的 MedPaLM 已成為提供一般資訊和健康相關查詢的高度有前途的平台。儘管如此，這些模型仍然是黑盒子，儘管納入了人類回饋和指導式微調。例如，儘管建立了安全防護措施，ChatGPT 仍會產生不安全的回應。CREST 提出了一個可行的途徑，在神經符號架構中利用程序和基於圖形的知識，以闡明與 LLM 相關的挑戰。
...
摘要：本研究調查了在 COVID-19 大流行期間及之後，已部署的人工智慧 (AI) 模型在預測死亡率方面的表現、可解釋性和穩健性。作為同類研究中的第一個，我們發現貝氏神經網路 (BNN) 和智慧訓練技術讓我們的模型在資料大幅變動的情況下仍能維持效能。我們的結果強調了開發穩健的 AI 模型的重要性，即使在具挑戰性的情況下，這些模型仍能達到或超越臨床醫師的預測。我們對模型可解釋性的探討顯示，隨機模型會產生更多元且個人化的解釋，因此突顯了在現實世界的臨床環境中提供詳細且個別見解的 AI 模型之必要性。此外，我們強調了量化 AI 模型中不確定性的重要性，這使臨床醫師能夠根據可靠的預測做出更明智的決策。我們的研究主張在醫療保健的 AI 研究中優先考慮實施科學，並確保 AI 解決方案在現實世界的臨床環境中實用、有益且永續。透過解決醫療保健環境中獨特的挑戰和複雜性，研究人員可以開發出有效改善臨床實務和患者預後的 AI 模型。
...
摘要：肺癌占英國癌症死亡人數的 21%，五年存活率受癌症確診期的影響很大。最近的研究已證明 AI 方法具有從例行掃描中準確及早診斷肺癌的能力。然而，此證據並未轉化為臨床實務，其中一項障礙是缺乏可解釋的模型。本研究探討了應用變異自編碼器 (VAE)，一種生成式 AI 模型，於肺癌病灶。提出的模型在從 LIDC-IDRI 公共資料集中的 3D CT 掃描中提取的病灶上進行訓練。透過分群探討 VAE 所產生的 2D 切片的潛在向量表示，以證明其品質，並用於肺癌診斷的 MLP 分類器模型，最佳模型達到了 AUC 0.98 和 93.1% 準確度的最先進指標。叢集分析顯示 VAE 潛在空間可根據有意義的特性成分（包括腫瘤大小、形狀、患者和惡性類別）將惡性和良性病灶的資料集分開。我們還納入了標準高斯 VAE (GVAE) 和較新的狄利克雷 VAE (DirVAE) 的比較分析，後者以狄利克雷分佈取代先驗，以鼓勵具有解開特徵表示的更具可解釋性的潛在空間。最後，我們展示了對應於具有臨床意義的特徵變化的潛在空間橫越的潛力。
...
摘要：現有的圖像分類器輸出解釋工具可分為白盒，依賴於模型內部的存取，以及黑盒，與模型無關。隨著人工智慧在醫療領域的使用增加，可解釋性工具的使用也隨之增加。現有的醫學影像解釋工作重點在於白盒工具，例如 gradcam。然而，切換到黑盒工具有明顯的優點，包括能夠與任何分類器一起使用，以及廣泛的黑盒工具可供選擇。在標準影像上，黑盒工具與白盒工具一樣精確。在本文中，我們比較了幾個黑盒方法在腦癌 MRI 資料集上與 gradcam 的效能。我們證明了大多數黑盒工具不適合解釋醫學影像分類，並詳細分析了它們缺點的原因。我們還展示了一個黑盒工具，一個基於因果可解釋性的 rex，其效能與 \gradcam 一樣好。
...
摘要：AI 開發社群日益利用 Hugging Face 等託管中介機構，以便讓使用者輕鬆存取使用者上傳的模型和訓練資料。這些模型市集降低了數十萬使用者的技術部署障礙，但可能會被用於許多潛在有害且非法的用途。在本文中，我們說明 AI 系統既可以「包含」內容，又可以成為開放式工具，這提出迄今為止最棘手的平台治理挑戰之一。我們提供三個說明性平台（Hugging Face、GitHub 和 Civitai）中幾個事件的個案研究，以探討模型市集如何審核模型。根據此分析，我們概述了產業為回應審核需求而開發的重要（但仍然有限）實務：授權、存取和使用限制、自動化內容審核，以及開放政策制定。儘管目前的政策挑戰相當可觀，我們最後提出一些想法，說明平台如何能更好地動員資源，作為謹慎、公平且適度的法規存取點。
...
摘要：背景與目標：通過提取這些資訊，基於機器學習或深度學習 (ML/DL) 的自動化資料分析工具，能協助臨床醫生和癌症研究人員從複雜的資料集中發現模式和關係。許多基於 DL 的卵巢癌 (OC) 資料分析最近已發表。這些分析在癌症的各個方面（例如，它們所探討的子領域和癌症類型）和資料分析功能上都非常多樣化。然而，目前在這些功能和 AI 保證 (AIA) 方面，對於這些分析的全面理解仍有所欠缺。本系統性回顧旨在透過檢視現有文獻並找出使用 DL 進行 OC 資料分析的重要面向，明確地關注關鍵功能和 AI 保證觀點，以填補此一缺口。方法：使用 PRISMA 架構在三個期刊資料庫中進行全面的搜尋。分析中僅納入 2015 年至 2023 年間發表於同行評審期刊的研究。結果：在回顧中，總共檢視了 96 項由 DL 驅動的分析。研究結果揭露了幾個關於由 DL 驅動的卵巢癌資料分析的重要見解：- 大多數研究 71%（96 項中有 68 項）專注於偵測和診斷，而沒有研究探討 OC 的預測和預防。- 這些分析主要基於來自非多元族群的樣本（75%（96 項研究中有 72 項）），僅限於某個地理位置或國家。- 只有少部分研究（僅 33%（96 項中有 32 項））執行整合分析，其中大多數使用同質資料（臨床或組學）。- 值得注意的是，僅有 8.3%（96 項中有 8 項）的研究使用外部和多元的資料集驗證其模型，突顯了增強模型驗證的必要性，以及 - 將 AIA 納入癌症資料分析仍處於非常早期的階段；僅有 2.1%（96 項中有 2 項）透過可解釋性明確地探討 AIA。
...
摘要：<paragraph>可解釋性是深度學習中長久以來的挑戰，特別是在醫療保健等高風險領域。常見的可解釋性方法會強調驅動 AI 模型決策的影像區域。然而，人類高度依賴語言來傳達不僅是「在哪裡」，還有「是什麼」的解釋。此外，大多數可解釋性方法都專注於解釋個別 AI 預測，而不是描述 AI 模型一般使用的特徵。後者對於模型和資料集稽核特別有用，而且隨著 AI 愈來愈多用於新穎任務時，甚至可能產生知識。在此，我們提出一個可解釋性策略，使用視覺語言模型來識別視覺分類任務的基於語言的描述符。透過利用影像和文字之間預先訓練的聯合嵌入空間，我們的做法將新的分類任務估計為一個線性詞彙組合，產生每個字詞的權重，表示其與基於視覺的分類器的對齊方式。我們使用兩個醫學影像分類任務來評估我們的方法，發現產生的描述符在很大程度上與臨床知識一致，儘管缺乏特定領域的語言訓練。然而，我們的做法也發現了所用公開資料集中「捷徑連線」的可能性。為了達到可解釋性的功能性衡量，我們進行了一項試驗性的讀者研究，發現 AI 識別的字詞可以讓非專家人類在非平凡的層級上執行專業的醫療任務。總而言之，我們的結果強調了使用多模態基礎模型來提供視覺任務的直觀、基於語言的解釋的潛力。</paragraph>
...
摘要：<paragraph>用於臨床任務的醫療影像訓練的人工智慧 (AI) 模型，經常會在效能上展現出不同子群之間的差異，形成偏誤。由於並非所有真實世界醫療影像資料中的偏誤來源都容易辨識，因此要全面評估這些偏誤是如何編碼到模型中，以及偏誤緩解方法在改善效能差異上的能力，是一項挑戰。在本文中，我們介紹了一個新穎的分析架構，用於系統且客觀地調查醫療影像中的偏誤對 AI 模型的影響。我們開發並測試了這個架構，用於執行受控的電腦模擬試驗，以評估醫療影像 AI 中的偏誤，方法是使用一個工具來產生具有已知疾病影響和偏誤來源的合成磁共振影像。我們使用三個反事實偏誤情境來測量模擬偏誤效應對卷積神經網路 (CNN) 分類器的影響，以及三個偏誤緩解策略的功效，以展示其可行性。分析顯示，當 CNN 在合成資料集上受訓時，模擬偏誤會導致預期的子群效能差異。此外，重新加權被認為是針對此設定最成功的偏誤緩解策略，我們展示了可解釋的 AI 方法如何協助使用此架構調查模型中偏誤的表現。由於醫療影像資料集中可能存在許多且經常未知的偏誤來源，因此開發公平的 AI 模型是一項重大的挑戰。在這項工作中，我們提出了一種新穎的方法，用於客觀地研究偏誤和緩解策略對深度學習管線的影響，這可以支援穩健且負責任的臨床 AI 的開發。</paragraph>
...
摘要：機器學習提供了一個巨大的潛力，可以自動預測中風後的症狀及其對康復的反應。這項工作的重大挑戰包括神經影像數據的維度非常高、可用於學習的數據集規模相對較小，以及如何有效地結合神經影像和表格數據（例如人口統計信息和臨床特徵）。本文基於兩種策略評估了多種解決方案。第一個是使用總結 MRI 掃描的 2D 圖像。第二個是選擇可以提高分類精度的關鍵特徵。此外，我們引入了在結合從 MRI 中提取的感興趣區域和表格數據的符號表示的圖像上訓練卷積神經網絡 (CNN) 的新方法。我們評估了一系列 CNN 架構（2D 和 3D），它們在 MRI 和表格數據的不同表示上進行訓練，以預測中風後口語圖片描述能力的綜合測量是否在失語症或非失語症範圍內。MRI 和表格數據來自 758 名參與 PLORAS 研究的英語中風倖存者。僅針對病灶大小的基線邏輯迴歸的分類準確率為 0.678，當依次添加初始症狀嚴重程度和恢復時間時，準確率上升至 0.757 和 0.813。在從每個 MRI 掃描中提取 8 個感興趣區域並在 2D 殘差神經網絡中與病灶大小、初始嚴重程度和恢復時間相結合時，觀察到最高的分類準確率 0.854。我們的研究結果展示了如何將影像和表格數據結合起來以獲得較高的中風後分類準確率，即使在機器學習術語中數據集很小的情況下也是如此。我們最後提出如何改進當前模型以使用來自醫院掃描儀的圖像來實現更高水平的準確率。
...
摘要：<paragraph>可解釋人工智慧 (XAI) 已成為處理任務關鍵應用程式時的一項必要需求，確保所採用黑盒 AI 模型的透明度和可解釋性。XAI 的重要性橫跨各種領域，從醫療保健到金融，其中了解深度學習演算法的決策制定過程至關重要。大多數基於 AI 的電腦視覺模型通常是黑盒子；因此，在影像處理中提供深度神經網路的可解釋性對於它們在醫學影像分析、自動駕駛和遙測應用中的廣泛採用和部署至關重要。最近，已經引入了幾種針對影像分類任務的 XAI 方法。相反地，影像分割在可解釋性的背景下受到的關注相對較少，儘管它是電腦視覺應用中的基本任務，尤其是在遙測中。只有部分研究提出基於梯度的 XAI 演算法進行影像分割。本文採用了最近的無梯度 Sobol XAI 方法進行語意分割。為了衡量 Sobol 方法在分割方面的效能，我們提出了一種基於可學習雜訊模型的量化 XAI 評估方法。此模型的主要目的是在解釋圖上誘發雜訊，其中較高的誘發雜訊表示較低的準確度，反之亦然。進行基準分析以評估和比較三種 XAI 方法的效能，包括 Seg-Grad-CAM、Seg-Grad-CAM++ 和 Seg-Sobol，使用所提出的基於雜訊的評估技術。這構成了使用高解析度衛星影像執行和評估 XAI 方法的首次嘗試。</paragraph>
...
摘要：大型語言模型在短時間內已在多個領域中大量增加。然而，由於事實性、連貫性和幻覺等問題，醫療和保健領域對其採用猶豫不決。鑑於醫療保健的高風險性質，許多研究人員甚至警告在這些問題解決之前不要使用它。在醫療保健中實施和部署 LLM 的關鍵是使這些模型值得信賴、透明（儘可能透明）且可解釋。在本文中，我們描述了建立可靠、值得信賴和無偏見模型的關鍵要素，作為它們在醫療保健中採用的必要條件。具體來說，我們專注於量化、驗證和減輕醫療保健背景下的幻覺。最後，我們討論了 LLM 在醫療保健中的未來可能是什麼樣子。
...
摘要：人工智慧 (AI) 以快速的速度進展，現已準備部署於各種應用程式中，例如自駕系統、醫療診斷和自然語言處理。早期採用 AI 技術於實際應用並非沒有問題，特別是對神經網路而言，它可能不穩定且容易受到對抗性範例的影響。從長遠來看，需要開發適當的安全保證技術，以減少由於可避免的系統故障而造成的潛在危害，並確保其可信度。本文重點關注認證和可解釋性，概述了為確保 AI 決策安全而開發的技術，並討論未來的挑戰。
...
摘要：歐洲議會議會研究服務總處已準備一份報告給歐洲議會議員，列舉出人工智慧 (AI) 在醫療保健中的七項主要風險：因 AI 錯誤造成的患者傷害、醫療 AI 工具的誤用、AI 中的偏見和現有不公平現象的延續、缺乏透明度、隱私和安全問題、問責差距以及實施障礙。
  
在這項研究中，我們提出十四項 AI 系統可能會實施的功能需求，以降低與其醫療目的相關的風險：AI 護照、使用者管理、法規檢查、僅供學術用途免責聲明、資料品質評估、臨床醫師雙重檢查、持續效能評估、稽核追蹤、持續可用性測試、回顧回溯/模擬案例、偏見檢查、可解釋 AI、加密和使用經過實地測試的程式庫，以及語意互操作性。
  
我們在此的目的是提供技術解決方案的具體高階規格，以確保持續良好的效能，並使用 AI 系統讓患者受益，符合未來的歐盟法規架構。
...
摘要：<paragraph>人工智慧技術可運用於分類病患的身體活動，並預測遠距監控病患的生命徵象。基於非線性模型（如深度學習模型）的回歸分析，由於其黑盒子的本質，解釋性有限。這可能需要決策者根據非線性模型的結果，做出盲目的信任飛躍，特別是在醫療保健應用中。在非侵入性監控中，來自追蹤感測器的病患資料及其易感臨床屬性，作為預測未來生命徵象的輸入特徵。解釋各種特徵對監控應用整體輸出的貢獻，對於臨床醫師的決策至關重要。在本研究中，提出了一個可解釋人工智慧的定量分析 (QXAI) 架構，具有事後模型可解釋性和內在可解釋性，適用於監督式學習方法中的回歸和分類任務。這是透過利用 Shapley 值概念，並將注意力機制納入深度學習模型中來實現的。我們採用人工神經網路 (ANN) 和基於注意力的雙向 LSTM (BiLSTM) 模型，根據感測器資料預測心率和分類身體活動。深度學習模型在預測和分類任務中都取得了最先進的成果。對輸入資料進行全局解釋和局部解釋，以了解各種病患資料的特徵貢獻。所提出的 QXAI 架構使用 PPG-DaLiA 資料評估，以預測心率，並使用行動健康 (MHEALTH) 資料根據感測器資料分類身體活動。蒙地卡羅近似法應用於架構，以克服 Shapley 值計算所需的時間複雜度和高運算能力需求。</paragraph>
...
摘要：<paragraph>在可解釋的人工智慧（XAI）的研究中，主要的重點在於為專家和從業人員解釋模型。模型不可知論和局部解釋方法在許多應用中被認為是可以解釋且足夠的。然而，在醫療保健等領域，最終使用者是沒有 AI 或領域專業知識的患者，因此迫切需要更易於理解且能建立對模型運作的信任的模型解釋。我們假設產生具有敘事性、患者特定性和全局性（模型的整體性）的模型解釋將有助於更好的理解和決策制定。我們使用決策樹模型來測試這一點，為被認定患有冠狀動脈心臟病高風險的患者產生局部和全局解釋。這些解釋會提供給非專家使用者。我們發現對特定類型的解釋有強烈的個人偏好。大多數參與者偏好全局解釋，而較少數的參與者偏好局部解釋。基於這些參與者的心理模型的任務評估提供了有價值的回饋，以增強敘事的全局解釋。這反過來指導了既可信賴又可操作的健康資訊系統的設計。</paragraph>
...
摘要：<paragraph>可解釋 AI 在醫學影像中至關重要。在神經科學的挑戰性領域中，視覺主題呈現出高度的複雜性，特別是在三維空間中。神經科學的應用，包括從 MRI 中識別腦溝特徵，由於專家之間不同的註解協定和腦部複雜的三維功能，面臨著重大的障礙。因此，傳統的可解釋性方法無法有效驗證和評估這些網路。為了解決這個問題，我們首先提出一個數學公式，說明各種不同電腦視覺任務的解釋需求類別，根據驗證協定的可靠性，分類為自解釋、半解釋、不可解釋和新模式學習應用。關於這個數學公式，我們提出一個 3D 可解釋性框架，旨在驗證深度學習網路在偵測副扣帶迴溝（一個重要的腦部解剖特徵）的輸出。該框架整合了局部 3D 解釋、通過降維的全局解釋、串聯的全局解釋和統計形狀特徵，揭示了模式學習的新見解。我們在具有挑戰性的 TOP-OSLO 資料集上訓練並測試了兩個先進的 3D 深度學習網路，顯著提高了腦溝偵測的準確度，特別是在左半球。在使用此資料集的不同註解協定進行評估期間，我們強調了無偏註解程序在我們提出的 3D 框架中實現精確預測和有效模式學習的關鍵作用。所提出的框架不僅註解了可變腦溝，還揭示了隱藏的 AI 知識，有望推進我們對腦部解剖和功能的理解。</paragraph>
...
摘要：電子健康紀錄 (EHR) 和例行文件編寫實務在病患的日常照護中扮演至關重要的角色，提供了一份關於健康、診斷和治療的全面紀錄。然而，複雜且冗長的 EHR 敘述會讓醫療保健提供者不堪負荷，進而造成診斷不準確的風險。儘管大型語言模型 (LLM) 已展現其在各種語言任務中的潛力，但其在醫療領域的應用需要確保將診斷錯誤降到最低，並防止病患受到傷害。在本文中，我們概述了一種創新的方法，用於提升 LLM 在自動化診斷生成領域的能力，這方法是透過結合一個醫療知識圖 (KG) 和一個新穎的圖模型：Dr.Knows，靈感來自臨床診斷推理過程。我們從美國國家醫學圖書館的統一醫學語言系統 (UMLS) 中衍生出 KG，而 UMLS 是生物醫學知識的強大知識庫。我們的方法否定了預先訓練的必要性，而改以將 KG 作為輔助工具，協助解釋和總結複雜的醫學概念。使用真實世界的醫院資料集，我們的實驗結果證明了將 LLM 與 KG 結合的建議方法有潛力改善自動化診斷生成的準確性。更重要的是，我們的方法提供了一條可解釋的診斷途徑，促使我們更接近實現 AI 增強的診斷決策支援系統。
...
摘要：現有的人工智慧 (AI) 診斷膝關節骨關節炎 (OA) 模型，儘管達到了醫療專家等級的表現，但仍因缺乏透明度和可解釋性而受到批評。這種不透明性使得它們在臨床實務中難以獲得信任。最近，可解釋人工智慧 (XAI) 已成為一種專門技術，它可以透過揭示預測的推導方式，提供對模型預測的信心，從而促進在醫療保健中使用 AI 系統。本文提出了一項針對用於膝關節骨關節炎診斷的 XAI 技術之首次調查。XAI 技術從兩個角度進行討論：資料可解釋性和模型可解釋性。本文的目的是提供有價值的見解，了解 XAI 對更可靠的膝關節骨關節炎診斷方法的潛力，並鼓勵在臨床實務中採用它。
...
摘要：<paragraph>最近人工智能應用於醫療保健的進展令人難以置信，在診斷和疾病預後方面超越人類表現。然而，隨著人工智能模型的複雜性日益增加，人們開始關注其模糊性、潛在偏見以及對可解釋性的需求。為了確保人工智能系統的信任度和可靠性，尤其是在臨床風險預測模型中，可解釋性變得至關重要。可解釋性通常是指人工智能系統提供其決策邏輯或決策本身對人類利益相關者進行可靠解釋的能力。在臨床風險預測中，可解釋性的其他方面，如公平性、偏見、信任和透明度，也代表了超越可解釋性的重要概念。在這篇評論中，我們探討了這些概念之間的關係，因為它們經常一起或互換使用。本評論還討論了開發可解釋的臨床風險預測模型的最新進展，強調了在臨床實踐中跨多種常見模式進行定量和臨床評估和驗證的重要性。它強調了外部驗證和結合多種可解釋性方法以增強信任和公平性的必要性。採用嚴格的測試，例如使用具有已知生成因子的合成數據集，可以進一步提高可解釋性方法的可靠性。開放訪問和代碼共享資源對於透明度和可複製性至關重要，有助於可解釋性研究的發展和可信度。儘管存在挑戰，但從臨床醫生到開發人員，將利益相關者納入臨床風險預測的可解釋性中，對於成功至關重要。</paragraph>
...
摘要：儘管人工智慧 (AI) 在醫療保健領域有重大進展，但 AI 技術在現實世界臨床實務中的部署和採用仍有限。近年來，人們對醫療 AI 相關的技術、臨床、倫理和法律風險提出疑慮。為了增加現實世界的採用率，醫療 AI 工具必須獲得患者、臨床醫師、醫療機構和主管機關的信任和接受。本研究描述了 FUTURE-AI 指南，這是第一個指導醫療保健領域中可信賴 AI 工具開發和部署的國際共識架構。FUTURE-AI 聯盟於 2021 年成立，目前由來自 51 個國家/地區的 118 位跨領域專家組成，代表所有洲，包括 AI 科學家、臨床醫師、倫理學家和社會科學家。在兩年的時間裡，聯盟透過一項反覆運算的程序定義了可信賴 AI 的指導原則和最佳實務，包括深入的文獻回顧、修改後的德爾菲調查和線上共識會議。FUTURE-AI 架構建立在醫療保健領域中可信賴 AI 的 6 項指導原則之上，即公平性、普遍性、可追溯性、可用性、穩健性和可解釋性。透過共識，定義了一組 28 項最佳實務，涵蓋技術、臨床、法律和社會倫理層面。建議涵蓋醫療 AI 的整個生命週期，從設計、開發和驗證到法規、部署和監控。FUTURE-AI 是一個以風險為基礎、不預設假設的指南，提供了一個架構化的方法來建構醫療 AI 工具，這些工具將在現實世界實務中獲得信任、部署和採用。鼓勵研究人員在概念驗證階段考慮建議，以促進醫療 AI 未來向臨床實務的轉譯。
...
摘要：人工智慧在醫療領域中已取得重大進展，並在醫學影像、病人照護和其他領域中出現了新興應用。儘管這些應用已在回顧性研究中被證明是成功的，但實際上應用它們的卻很少。醫療人工智慧領域面臨各種挑戰，包括建立使用者信任、遵守法規、合乎道德地使用資料。可解釋人工智慧 (XAI) 旨在讓人們能夠理解人工智慧並相信其結果。本文根據近年來發表的 198 篇文章的代表性樣本，對用於醫療決策支援的 XAI 解決方案的最新發展進行文獻回顧。對相關文章的系統性綜合產生了幾個發現。(1) 這些解決方案大多採用與模型無關的 XAI 技術，(2) 深度學習模型的使用多於其他類型的機器學習模型，(3) 可解釋性被用於促進信任，但很少有研究報告醫師參與其中，(4) 視覺化且互動式的使用者介面對於理解系統的解釋和建議更有用。需要更多醫療和人工智慧專家合作進行研究，這可以指導設計、實作和評估醫療領域中 XAI 解決方案的合適架構的發展。
...
摘要：體外受精是治療不孕症最廣泛的方法之一。其主要挑戰之一是評估和選擇胚胎進行植入，這是一個臨床醫生之間和臨床醫生內部變異性很大的過程。基於深度學習的方法正引起關注，但其不透明的性質損害了它們在臨床背景中的接受度，而在臨床背景中，決策制定中的透明度至關重要。在本文中，我們分析了 AI 輔助胚胎分析模型的可解釋性方面的當前工作，並找出其局限性。我們還討論了如何將這些模型作為決策支持系統整合到臨床背景中，同時考慮臨床醫生和患者的需求。最後，我們提出了提高可解釋性和可信度的指南，推進這項技術朝著既定的臨床實踐發展。
...
摘要：在需求工程 (RE) 領域中，可解釋人工智慧 (XAI) 在將 AI 支援系統與使用者需求、社會期望和法規標準保持一致方面的日益重要性已獲得認可。一般而言，可解釋性已成為影響系統品質的重要非功能性需求。然而，可解釋性與效能之間的假定權衡挑戰了可解釋性的假定正面影響。如果滿足可解釋性的需求會導致系統效能下降，則必須仔細考慮這些品質面向哪個優先，以及如何在它們之間取得折衷。在本文中，我們批判性地檢驗了假定的權衡。我們認為，最好以一種微妙的方式來處理它，這種方式納入了資源可用性、領域特性和風險考量。透過提供未來研究和最佳實務的基礎，這項工作旨在推動 RE for AI 領域的進步。
...
摘要：在需求工程（RE）領域中，可解釋人工智慧（XAI）在將 AI 支援系統與使用者需求、社會期望和法規標準相符中的重要性日益受到重視。一般來說，可解釋性已成為影響系統品質的重要非功能性需求。然而，可解釋性與效能之間的假定取捨挑戰了可解釋性的假定正面影響。如果滿足可解釋性需求會導致系統效能下降，那麼必須仔細考慮這些品質面向中哪一項優先，以及如何在它們之間折衷。在本文中，我們批判性地檢視了假定的取捨。我們主張最好以一種微妙的方式來處理，這種方式結合了資源可用性、領域特性和風險考量。透過提供未來研究和最佳實務的基礎，這項工作旨在推進 AI 的 RE 領域。
...
摘要：本論文批判性地評估了歐盟委員會提出的 AI 法案，該法案針對高風險 AI 系統的風險管理和風險可接受性方法，這些系統對基本權利和安全構成風險。該法案旨在以相稱的監管負擔促進「值得信賴」的 AI。其關於風險可接受性的條款要求將高風險系統的殘餘風險盡可能「降低或消除」，並考慮「最新技術」。此標準，特別是如果狹義解釋，是不可行的，既不能促進相稱的監管負擔，也不能促進值得信賴。相比之下，議會最近對風險管理條款的修正草案引入了「合理性」、成本效益分析，並且更透明地說明了風險可接受性判斷的價值觀和情境性質。本文認為，議會的方法更可行，並且能更好地平衡相稱性和值得信賴的目標。本文說明了風險可接受性判斷中的合理性將會是什麼，並借鑒過失法和歐洲醫療器材法規中的原則。本文主張，風險可接受性判斷的方法需要堅實的公民合法性基礎：包括監管機構的詳細指導或參與，以及受影響利益相關者的有意義投入。
...
摘要：可解釋人工智慧 (XAI) 是機器學習中快速進展的領域，旨在解開複雜模型的預測。XAI 在敏感應用中特別需要，例如在醫療保健中，當診斷、建議和治療選擇可能依賴於人工智慧系統做出的決策時。AI 方法已廣泛用於老化研究中，特別是在開發生物時鐘模型和識別老化和與年齡相關疾病的生物標誌物方面。然而，XAI 在此處的潛力尚待充分認識。我們討論了 XAI 在開發「老化時鐘」中的應用，並對按特定生理系統的焦點分類的文獻進行了全面的分析。
...
摘要：部分原型模型是可解釋的影像分類器，也是黑箱 AI 的一個有前途的替代方案。本文探討了解釋性機器學習的適用性和潛力，特別是 PIP-Net，用於對真實世界醫學影像數據進行自動診斷支援。PIP-Net 學習人類可理解的原型影像部分，我們評估其準確性和可解釋性，以進行骨折檢測和皮膚癌診斷。我們發現 PIP-Net 的決策制定過程符合醫學分類標準，同時僅提供影像層級的類別標籤。由於 PIP-Net 的原型未經監督的預訓練，因此可以輕鬆識別資料品質問題，例如 X 光中的不需要文字或標籤錯誤。此外，我們率先表明，人類可以透過直接停用不需要的原型來手動修正 PIP-Net 的推理。我們得出結論，部分原型模型由於其可解釋性和進階模型除錯的潛力，對於醫學應用很有前景。
...
摘要：可解釋人工智慧 (XAI) 是機器學習研究中日益重要的領域，旨在使黑盒子模型透明且可解釋。在本文中，我們提出了一種新的 XAI 方法，該方法使用條件特徵置換產生的所謂反事實路徑。該演算法透過識別特徵的順序置換來衡量特徵重要性，這些置換最能影響模型預測的變化。它特別適合根據知識圖譜中的反事實路徑產生解釋，並結合領域知識。反事實路徑為當前的 XAI 方法引入了額外的圖形維度，用於解釋和視覺化黑盒子模型。使用合成和醫療資料進行的實驗證明了我們方法的實用適用性。
...
摘要：人工智能 (AI) 中的可解释性领域见证了越来越多的研究和越来越多的学术兴趣。然而，在解释机器学习算法结果时缺乏人性化和个性化的解释，极大地阻碍了临床医生在研究和临床实践中接受这些方法。为了解决这个问题，我们的研究使用反事实解释来探索“如果？”在医学研究中的方案适用性。我们的目标是扩展我们对用于诊断小儿后颅窝脑肿瘤的磁共振成像 (MRI) 特征的理解，超越现有的界限。在我们的案例研究中，提出的概念提供了一种新颖的方法来检查提供个性化和特定于上下文的见解的替代决策制定方案，从而能够验证预测并澄清在不同情况下出现的差异。此外，我们探索了反事实数据扩充的潜在用途，并评估了它们作为我们医学研究案例中替代方法的可行性。结果证明了使用反事实解释来增强临床研究中 AI 驱动方法的可接受性的巨大潜力。
...
摘要：人工智慧領域的進展已導致開發出各種由人工智慧驅動的失智症評估，可用於識別失智症的早期患者。它可以徹底改變失智症的照護設定。醫學界必須了解各種人工智慧評估，並根據其有效性、效率、實用性、可靠性和準確性來選擇它們，以早期識別失智症患者 (PwD)。另一方面，人工智慧開發人員應了解各種非人工智慧評估以及最近開發的人工智慧評估。因此，這篇臨床醫師和人工智慧工程師都可以閱讀的論文填補了文獻中的空白，向臨床醫師說明了現有的失智症識別解決方案，以及向人工智慧工程師說明了所使用的技術和最廣泛的失智症資料集。它遵循對人工智慧和非人工智慧失智症評估論文的回顧，以提供有關各種失智症評估的寶貴資訊，供人工智慧和醫學界使用。討論和結論重點說明了最顯著的研究方向和現有解決方案的成熟度。
...
摘要：<paragraph>可解釋性對人工智慧 (AI) 技術構成一項重大挑戰。當前關於可解釋 AI (XAI) 的研究缺乏提取學習任務整體知識的效率，因而存在不精確顯著性、情境感知缺失和含義模糊等缺陷。在本文中，我們提出類別關聯嵌入 (CAE) 方法來解決這些問題。我們採用編碼器解碼器架構來嵌入樣本特徵，並將它們同時分為與類別相關和與個體相關的樣式向量。將給定樣本的個體樣式代碼與另一個類別的類別樣式代碼重新組合，會導致一個合成樣本，該樣本保留了個體特徵，但改變了類別分配，遵循循環對抗學習策略。類別關聯嵌入將所有實例的整體類別相關特徵提煉到一個統一的域中，在類別之間有良好的分離。然後可以提取不同類別之間的轉換規則，並進一步應用於個別實例。然後，我們提出一個主動 XAI 框架，該框架沿著引導路徑操縱特定樣本的類別樣式向量朝向對立類別，從而產生一系列具有相同個體特徵的對立範例合成樣本。將這些反事實樣本與原始樣本進行比較，可以對分類任務的性質提供一個整體的、直觀的說明。我們在醫學影像分類任務中採用該框架，結果表明，與現有方法相比，可以實現更精確的顯著性圖，並具有強大的情境感知表示。此外，疾病病理學可以直接通過在類別樣式空間中遍歷路徑來視覺化。</paragraph>
...
摘要：<paragraph>提供基於機器學習的 AI 預測的高品質解釋是一項充滿挑戰且複雜的任務。要順利運作，它需要以下因素（除其他因素外）：選擇適當的解釋概括性/特殊性層級；考量解釋受益人對所考慮的 AI 任務的熟悉度假設；參考促成決策的具體元素；利用額外的知識（例如專家證據），這些知識可能不屬於預測程序的一部分；以及提供支持否定假設的證據。最後，系統需要以清晰可解釋且可能令人信服的方式制定解釋。考量這些因素後，ANTIDOTE 培養了可解釋 AI 的整合願景，其中深度學習流程的低階特徵與人類論證能力的高階架構相結合。ANTIDOTE 將利用深度學習和論證的跨領域能力來支持更廣泛且創新的可解釋 AI 觀點，其中對於臨床案例審議的高品質解釋需求至關重要。作為專案的第一個成果，我們發布了 Antidote CasiMedicos 資料集，以促進對一般可解釋 AI 以及特別是醫療領域論證的研究。</paragraph>
...
摘要：近年來圖神經網路的進展迅速，在藥物發現、醫療診斷和推薦系統方面都有許多新的發展。雖然進展顯著，但許多網路都是「黑盒子」，對於網路究竟在學習「什麼」了解甚少。許多高風險應用，例如藥物發現，需要模型提供人類可理解的解釋，以便使用者能辨識錯誤並發現新知識。因此，可解釋 AI 演算法的開發對於我們收穫 AI 的好處至關重要。
我們提出了一種名為 eXplainable Insight (XInsight) 的 GNN 可解釋性演算法，它使用 GFlowNets 產生模型解釋的分配。由於 GFlowNets 根據獎勵生成機率成正比的物件，與僅學習最大獎勵樣本的先前方法相比，XInsight 可以產生多樣化的解釋。我們透過針對兩個圖形分類任務訓練的 GNN 產生解釋來展示 XInsight：使用 MUTAG 資料集對致突變化合物進行分類，以及使用我們已開源的合成資料集對無環圖形進行分類。我們透過使用 QSAR 建模分析產生的化合物來展示 XInsight 解釋的效用，並且我們發現 XInsight 產生的化合物會根據親脂性進行分類，而親脂性是致突變性的已知相關因素。我們的結果顯示 XInsight 產生解釋的分配，揭示了模型所展示的底層關係。它們也強調產生多樣化解釋的重要性，因為它使我們能夠發現模型中的隱藏關係，並為進一步的分析提供有價值的指導。
...
摘要：<paragraph>我們提出並實作一個可解釋的機器學習分類模型，用於基於表達布林公式的可解釋 AI (XAI)。潛在應用包括信用評分和醫療狀況診斷。布林公式定義了一個具有可調整複雜度（或可解釋性）的規則，根據該規則對輸入數據進行分類。此類公式可以包含任何可應用於一個或多個布林變數的運算子，因此與更嚴格的基於規則和基於樹的方法相比，提供了更高的表達能力。使用原生局部最佳化技術訓練分類器，有效地搜尋可行公式的空間。淺層規則可以由快速的整數線性規劃 (ILP) 或二次無約束二元最佳化 (QUBO) 求解器確定，這些求解器可能由特殊用途硬體或量子裝置提供支援。我們將原生局部最佳化器的表達能力和效率與這些裝置的快速運作相結合，方法是執行非局部移動，對完整布林公式的子樹進行最佳化。我們提供了廣泛的數值基準測試結果，在眾所周知的公開資料集上採用了幾個基線。根據結果，我們發現原生局部規則分類器通常與其他分類器具有競爭力。加入非局部移動以更少的反覆運算獲得類似的結果，因此使用專用或量子硬體可以透過快速提出非局部移動來加速。</paragraph>
...
摘要：為了解決心理健康問題的全球挑戰，我們提出了一種基於邏輯神經網路 (LNN) 的神經符號 AI 方法來診斷心理疾病。由於缺乏對心理疾病的有效治療，因此需要一種 AI 解決方案來協助治療師進行診斷。然而，目前的類神經網路模型缺乏可解釋性，治療師可能無法信任。LNN 是一種遞迴神經網路架構，它結合了神經網路的學習能力和基於經典邏輯的 AI 的推理能力。所提出的系統使用臨床訪談中的輸入謂詞來輸出心理疾病類別，並使用不同的謂詞剪枝技術來實現可擴充性和更高的分數。此外，我們提供一種見解提取方法來幫助治療師進行診斷。所提出的系統解決了當前神經網路模型缺乏可解釋性的問題，並為心理疾病診斷提供了更值得信賴的解決方案。
...
摘要：隨著機器學習模型在醫療診斷中變得越來越普遍，對可解釋性和透明度的需求變得至關重要。XAI 文藝復興標誌著該領域的重大轉變，旨在重新定義醫療診斷模型的可解釋性。本文探討了解釋性人工智慧 (XAI) 領域內的創新方法和方法論，這些方法和方法論正在革新醫療診斷模型的可解釋性。通過闡明基礎決策制定過程，XAI 技術讓醫療保健專業人員能夠理解、信任並有效利用這些模型來進行準確且可靠的醫療診斷。本綜述重點介紹了 XAI 在醫療診斷方面的關鍵進展及其轉變醫療保健領域的潛力，最終改善患者的治療結果並促進對 AI 驅動的診斷系統的信任。
...
摘要：在以高度連接性和行動性為特徵的環境中，再加上心血管疾病激增，透過遠程監測心血管健康來減少醫療保健支出的必要性變得更加明顯。準確檢測和分類心律不整對於診斷患有心臟不規則的人至關重要。本研究強調了在家中環境中使用心電圖 (ECG) 測量進行實時心律不整檢測的可行性。本文提出心律不整檢測的新應用，利用尖端的 You-Only-Look-Once (YOLO)v8 演算法對單導程 ECG 訊號進行分類。我們引入了一個新穎的損失修改 YOLOv8 模型，並針對 MIT-BIH 心律不整資料集進行微調，以實現實時連續監測。獲得的結果證實了我們方法的有效性，該模型在 NVIDIA Tesla V100 上達到了 99.5% 的平均準確度和 0.992 mAP@50，以及 0.002 秒的快速檢測時間。我們的研究例證了實時心律不整檢測的潛力，使用戶能夠在家中舒適地視覺化解讀模型輸出。此外，本研究為擴展到能夠部署在醫療保健領域的實時可解釋 AI (XAI) 模型奠定了基礎，從而顯著推動了醫療保健解決方案的領域。
...
摘要：乳癌 (BC) 仍然是一個重大的健康威脅，目前尚無可用的長期療法。早期發現至關重要，但乳房攝影的判讀卻受到高假陽性和假陰性的阻礙。由於乳癌的發生率預計將超過肺癌，因此改善早期檢測方法至關重要。熱像攝影使用高解析度紅外線相機，特別是與人工智慧 (AI) 結合使用時，提供了希望。這項工作提出了一個基於注意力的卷積神經網路用於分割，在乳癌檢測和分類中提供了更高的速度和準確度。該系統增強影像並使用可解釋的 AI 執行癌症分割。我們提出了一個基於Transformer注意力的卷積架構 (UNet) 用於故障識別，並採用梯度加權類激活對應 (Grad-CAM) 來分析 UNet 架構中 IRT 影像的偏差和弱點區域。與現有的深度學習架構相比，我們提出的框架的優越性得到了證實。
...
摘要：憂鬱症是最普遍且嚴重的精神疾病，會造成嚴重的財務和社會影響。憂鬱症的偵測對於早期介入以減輕這些後果至關重要。如此重大的決定本質上需要可解釋性。儘管一些憂鬱症偵測研究嘗試根據重要性分數或注意力權重來解釋決策，但這些解釋與基於憂鬱症狀的臨床憂鬱症診斷標準不符。為了填補這個差距，我們遵循計算設計科學範例，開發出一種新穎的多尺度時間原型網路 (MSTPNet)。MSTPNet 創新地偵測和解釋憂鬱症狀以及它們持續多久。使用大規模資料集進行的廣泛實證分析顯示，MSTPNet 以 0.851 的 F1 分數優於最先進的憂鬱症偵測方法。此結果也揭示了在調查方法中沒有註明的新的症狀，例如分享對不同生活的仰慕。我們進一步進行使用者研究，以證明其在可解釋性方面優於基準。本研究透過一種新穎的可解釋深度學習模型，為憂鬱症在社群媒體中的偵測，做出貢獻。在實務上，我們提出的方法可以實作在社群媒體平台中，以提供個人化的線上資源，給偵測出的憂鬱症患者。
...
摘要：電子健康記錄 (EHR) 是預想中由人工智慧 (AI) 推動的醫療保健轉型中一個重要的資料來源。
然而，反映在 EHR 記錄中的臨床偏見可能會導致 AI 模型繼承並放大這些偏見，使健康差距持續存在。
本研究探討了 EHR 記錄中污名化語言 (SL) 對使用基於 Transformer 的深度學習模型和可解釋 AI (XAI) 技術進行死亡率預測的影響。
我們的研究結果表明，由臨床醫生撰寫的 SL 對 AI 效能產生負面影響，尤其是對黑人患者而言，突顯 SL 是 AI 模型開發中種族差異的根源。
為了探索一種在操作上有效的方法來減輕 SL 的影響，我們透過臨床醫生的協作網路探討 SL 產生的模式，並發現核心臨床醫生對 AI 模型中的種族差異有更大的影響。
我們發現刪除由核心臨床醫生撰寫的 SL 是一種比消除資料集中所有 SL 更有效的偏見減少策略。
本研究為負責任的 AI 開發提供了可行的見解，並有助於了解臨床醫生的行為和在醫療保健中撰寫 EHR 記錄。
...
摘要：當代透過 AI 的自動化，需要大量的幕後人力，這些人力常被隱形化且薪資過低。由於包含標籤和維護工作在內的隱形勞動，是當代 AI 系統的整合部分，因此讓使用者了解其角色仍然很重要。我們建議可透過可解釋 AI（XAI）設計來達成，特別是女性主義交叉的 XAI。我們提出源自女性主義交叉研究的製圖方法，以提出 AI 的系統性觀點，並納入與隱形勞動有關的 AI 向度。
...
摘要：虛擬心理健康助理 (VMHA) 持續進步，以支持負擔過重的全球醫療保健系統，該系統每年有 6000 萬人次初級保健就診和 600 萬人次急診室 (ER) 就診。這些系統是由臨床心理學家、精神科醫生和人工智慧 (AI) 研究人員為認知行為療法 (CBT) 所建置。目前，VMHA 的角色是透過資訊提供情緒支持，較少著重於與患者建立反思對話。需要更全面、安全且可解釋的方法來建置負責任的 VMHA，以提出後續問題或提供明智的回應。此調查提供了對現有心理健康對話代理的系統性批判性回顧，接著深入探討透過脈絡知識、資料集和其在臨床決策支援中新興角色來改善 VMHA。我們也提供新的方向，以透過可解釋性、安全性，以及整體可信度來豐富 VMHA 的使用者體驗。最後，我們提供評估指標和實務考量，以超越現有文獻，在 VMHA 和患者之間的積極溝通中建立信任。
...
摘要：XAI 指用於建置 AI 應用程式的技術和方法，可協助最終使用者詮釋 AI 模型的輸出和預測。在高風險決策情境中，例如醫療領域，黑箱 AI 應用程式增加了透明度和可解釋性的需求，因為錯誤的預測可能會造成嚴重後果。模型可解釋性和可詮釋性是 AI 模型在醫療實務中成功部署的關鍵。AI 應用程式的基礎推理需要對臨床醫師透明，才能獲得他們的信任。本文提出醫療領域中 XAI 面向和挑戰的系統性回顧。本研究的主要目標是回顧各種 XAI 方法、其挑戰，以及醫療保健中相關的機器學習模型。這些方法分為六類討論：面向特徵的方法、全局方法、概念模型、替代模型、局部基於像素的方法和以人為中心的方法。最重要的是，本文探討 XAI 在醫療保健問題中的角色，以釐清其在安全關鍵應用中的必要性。本文旨在透過回顧相關實驗結果，建立對醫療保健領域中 XAI 相關應用的全面理解。為了促進未來研究填補研究差距，本文探討了 XAI 模型從不同觀點來看的重要性及其限制。
...
摘要：最先进的机器学习模型通常会学习训练数据中内嵌的虚假关联。当将这些模型用于高风险决策时，例如皮肤癌检测等医学应用，就会带来风险。为了解决这个问题，我们提出了 Reveal to Revise (R2R)，一个涵盖了整个可解释人工智能 (XAI) 生命周期的框架，使从业者能够通过最少的人工交互迭代识别、缓解和（重新）评估虚假的模型行为。在第一步 (1) 中，R2R 通过找出归因中的异常值或通过检查模型学习的潜在概念来揭示模型的弱点。其次 (2)，检测出负责的工件并在输入数据中进行空间定位，然后利用它来 (3) 修改模型行为。具体来说，我们应用 RRR、CDEP 和 ClArC 的方法进行模型修正，并 (4) (重新) 评估模型的性能和对工件的剩余敏感性。使用两个用于黑色素瘤检测和骨龄估计的医学基准数据集，我们将我们的 R2R 框架应用于 VGG、ResNet 和 EfficientNet 架构，从而揭示和纠正了真实的数据集内在工件以及受控设置中的合成变体。完成 XAI 生命周期，我们演示了多个 R2R 迭代以减轻不同的偏差。代码可在 https://github.com/maxdreyer/Reveal2Revise 上找到。
...
摘要：可解釋人工智慧 (XAI) 領域在近年來有長足的進展，但進展主要在於電腦視覺和自然語言處理。對於時間序列，輸入通常是不可解釋的，只有有限的研究可用於 XAI。在這項工作中，我們提出了虛擬檢查層，將時間序列轉換為可解釋的表示，並允許通過局部 XAI 方法（如逐層相關性傳播 (LRP)）將相關性歸因傳播到此表示。通過這種方式，我們將 XAI 方法家族的適用性擴展到輸入僅在轉換後才能解釋的領域（例如語音）。在這裡，我們專注於傅立葉轉換，它在時間序列和 LRP 的解釋中得到顯著應用，並將我們的稱為 DFT-LRP。我們展示了 DFT-LRP 在各種時間序列分類設定（如音訊和電子健康記錄）中的用途。我們展示了 DFT-LRP 如何揭示在不同領域（例如時間域與頻率域）訓練的模型的分類策略的差異，或有助於發現模型如何作用於資料中的虛假相關性。
...
摘要：解釋深度學習模型預測結果給最終使用者，是利用人工智慧 (AI) 進行醫療決策過程中的一項重要功能，通常被認為是不透明且難以理解的。在本文中，我們運用最先進的可解釋人工智慧 (XAI) 方法，來解釋黑盒子 AI 模型在甲狀腺結節診斷應用中的預測結果。我們提出新的基於統計的 XAI 方法，即核密度估計和密度圖，來解釋未檢測到結節的情況。XAI 方法的表現被視為質化和量化比較下的回饋，以改善資料品質和模型表現。最後，我們進行調查，以評估醫生和患者對 XAI 解釋模型對甲狀腺結節影像決策的信任度。
...
摘要：醫療設備和人工智慧系統快速轉變醫療保健服務。同時，由於其本質，醫療設備中或作為醫療設備的人工智慧可能會遭受網路攻擊，導致患者安全和安全風險。本書章節分為三個部分。第一部分從設定場景開始，我們在其中說明網路安全在醫療保健中的作用。然後，我們簡要定義我們在談論被認為本身就是醫療設備或支援醫療設備的人工智慧時所指涉的內容。為了說明此類醫療設備帶來的風險，我們提供了三個範例：資料集中毒、社會工程以及資料或原始碼萃取。在第二部分中，本文概述了歐盟的監管架構，與確保醫療設備中或作為醫療設備的人工智慧的網路安全相關（MDR、NIS 指令、網路安全法、GDPR、AI 法案提案和 NIS 2 指令提案）。最後，本文的第三部分探討了源自歐盟監管架構的潛在挑戰。特別是，我們展望源自兩個立法提案的挑戰，以及它們與現行有關人工智慧醫療設備網路安全的立法之間的互動。它們被結構化為對以下問題的回答：(1) 人工智慧法案將如何與 MDR 互動，就網路安全和安全要求而言？(2) 我們應如何解釋 NIS 2 指令提案和 MDR 中的事件通知要求？(3) 關鍵基礎設施演變條款的後果是什麼？[這是草稿章節。最終版本將收錄在 Barry Solaiman 和 I. Glenn Cohen 編輯的《健康、人工智慧與法律研究手冊》中，愛德華·埃爾加出版社，即將於 2023 年出版]
...
摘要：阿茲海默症 (AD) 是一種進行性神經退化性疾病，也是失智症的主要原因。早期診斷對於患者從潛在干預和治療中獲益至關重要。由於視網膜與大腦具有解剖學上的連接，因此假設視網膜是 AD 檢測的診斷部位。為此目的開發的 AI 模型尚未對決策提供合理的解釋，也沒有推論出疾病進展的階段。沿著這個方向，我們提出了一個新的與模型無關的可解釋 AI 框架，稱為顆粒神經元級解釋器 (LAVA)，這是一個解釋原型，可以探查卷積神經網路 (CNN) 模型的中間層，以直接從視網膜影像評估 AD 連續體，而無需縱向或臨床評估。此方法用於驗證視網膜血管作為阿茲海默症 (AD) 評估的生物標記和診斷方式。英國生物銀行的認知測試和血管形態特徵表明，LAVA 在識別整個進程連續體中的 AD 階段方面顯示出強大的前景和有效性。
...
摘要：許多視覺化已針對可解釋 AI (XAI) 進行開發，但它們通常需要使用者進行進一步的推理才能解讀。我們主張 XAI 應支援圖解和演繹推理，讓 AI 執行假設產生和評估，以縮小可解釋性差距。我們提出圖表化，以 i) 執行皮爾士演繹-演繹推理，ii) 遵循領域慣例，以及 iii) 以視覺或口語方式使用圖表進行說明。我們實作了 DiagramNet 以進行臨床應用，以根據心臟聽診預測心臟診斷，並使用基於形狀的雜音圖表進行說明。在建模研究中，我們發現 DiagramNet 不僅提供了忠實的雜音形狀說明，而且比基線模型具有更好的預測效能。我們進一步在與醫學生的定性使用者研究中展示了圖解說明的可解釋性和可信度，顯示出臨床相關的圖解說明比技術顯著性圖說明更受青睞。這項工作有助於提供以使用者為中心的 XAI 的領域慣例演繹說明。
...
摘要：皮膚癌是人類最普遍的癌症形式之一。它的辨識主要透過視覺，從臨床篩檢開始，接著進行皮膚鏡檢查、組織學評估和標本收集。深度卷積神經網路 (CNN) 對分類的細緻物體執行高度區隔且潛在通用的任務。本研究提出一個新穎的多類別預測架構，它根據 ViT 和 ViTGAN 對皮膚病灶進行分類。基於視覺轉換器的 GAN（生成對抗網路）用於解決類別不平衡問題。該架構包含四個主要階段：ViTGAN、影像處理和可解釋 AI。階段 1 包含生成合成影像，以平衡資料集中的所有類別。階段 2 包含應用不同的資料擴充技術和形態運算，以增加資料大小。階段 3 和 4 包含為邊緣運算系統開發 ViT 模型，該模型可以識別圖案，並對影像中可見使用者皮膚上的皮膚病灶進行分類。在階段 3 中，在使用 ViT 將病灶分類到所需類別後，我們將使用可解釋 AI (XAI)，它會產生更多可解釋的結果（使用激勵映射等），同時確保高預測準確度。皮膚疾病的即時影像可以用相機拍攝，由醫生或患者使用行動應用程式進行早期檢查，並確定皮膚病灶的原因。整個架構與現有的皮膚病灶偵測架構進行比較。
...
摘要：在医疗保健等高风险环境中部署人工智能 (AI) 时，提供可解释性/可说明性或允许细粒度错误分析的方法至关重要。许多用于可解释性/可说明性和细粒度错误分析的最新方法使用概念，这些概念是语义上对人类有意义的元标签。然而，只有少数数据集包含概念级别的元标签，并且大多数元标签与不需要领域专业知识的自然图像相关。医学中密集注释的数据集专注于与单一疾病（如黑色素瘤）相关的元标签。在皮肤病学中，皮肤病使用既定的临床词汇进行描述，允许临床医生彼此描述体格检查结果。为了提供一个由领域专家密集注释的医学数据集，其中注释对多种疾病过程有用，我们开发了 SkinCon：一个由皮肤科医生密集注释的皮肤病数据集。SkinCon 包含来自 Fitzpatrick 17k 数据集的 3230 张图像，其中密集注释了 48 个临床概念，其中 22 个概念至少有 50 张代表该概念的图像。所使用的概念是由两位皮肤科医生考虑用于描述皮肤病变的临床描述术语而选择的。示例包括“斑块”、“鳞屑”和“糜烂”。同样的概念也用于标记来自 Diverse Dermatology Images 数据集的 656 张皮肤病图像，提供了一个具有不同肤色表示的附加外部数据集。我们回顾了 SkinCon 数据集的潜在应用，例如探测模型、基于概念的解释和概念瓶颈。此外，我们使用 SkinCon 来演示其中两个用例：使用概念调试现有皮肤病 AI 模型的错误，并使用事后概念瓶颈模型开发可解释模型。
...
摘要：多臂賭博機（RMAB）是一種流行的決策理論框架，已用於模擬公共衛生、野生動物保育、通訊系統等領域的真實世界順序決策問題。
已部署的 RMAB 系統通常分兩個階段運行：第一階段預測定義 RMAB 範例的未知參數，第二階段採用最佳化演算法來解決已建構的 RMAB 範例。
在這項工作中，我們提供並分析了在公共衛生領域首次部署 RMAB 系統的結果，目的是改善孕產婦和兒童健康。我們的分析著重於了解預測準確度和已部署 RMAB 系統的整體效能之間的關係。這對於確定投資於提高預測準確度以改善最終系統效能的價值至關重要，並且有助於診斷、監控已部署的 RMAB 系統。
使用我們已部署 RMAB 系統的真實世界資料，我們證明整體預測準確度的提升甚至可能伴隨著 RMAB 系統效能的下降——廣泛投入資源以提高整體預測準確度可能無法產生預期的結果。在此之後，我們制定了以決策為中心的評估指標來評估預測組成部分，並表明它在解釋已部署 RMAB 系統的整體效能方面做得更好（無論是經驗上還是理論上）。
...
摘要：可解釋 AI 包含開發允許決策系統與人類互動的機制，讓前者的決策變得可以理解。這在敏感的脈絡中特別重要，例如在醫療領域。我們提出一個使用案例研究，用於皮膚病灶診斷，說明如何為從範例中訓練來表徵皮膚病灶的最新深度神經網路分類器提供決策說明。我們的架構包含一個訓練好的分類器，說明模組在其上運作。後者能夠為分類診斷提供實例和反例，因此允許醫師與自動診斷系統互動。實例是透過對抗式自動編碼器產生的。我們說明系統在代表性範例中的行為。
...
摘要：人工智慧（AI）應用中的演算法公平性對於一個更美好的社會至關重要。公平性作為社會機制的基礎公理，包含多種面向。儘管機器學習（ML）社群已將交叉性視為統計同質性的問題，尤其是在歧視問題上，但新興的文獻探討了另一個面向——單調性。根據領域專家，單調性在許多與公平性相關的領域中扮演著至關重要的角色，其中違反單調性可能會誤導人類決策並導致災難性的後果。在本文中，我們首先系統性地評估應用單調神經加法模型（MNAM）的重要性，該模型使用公平意識 ML 演算法來強制執行個人和成對單調性原則，以確保 AI 倫理和社會的公平性。我們透過理論推理、模擬和廣泛的實證分析的混合方法發現，在所有公平性領域（包括犯罪學、教育、醫療保健和金融）中，考量單調性公理至關重要。我們的研究有助於 AI 倫理、可解釋 AI（XAI）和人機互動（HCI）介面中的跨領域研究。透過證明不符合單調性的災難性後果，我們探討了單調性需求在 AI 應用中的重要性。此外，我們證明 MNAM 是一種有效的公平意識 ML 方法，透過施加單調性限制來整合人類智慧。
...
摘要：<paragraph>在安全關鍵應用程式（例如法律或醫療領域）中，解釋 AI 模型的預測至關重要。一種預測的解釋形式是萃取推理，即一個實例特徵的子集，引導模型對該實例做出預測。例如，子詞組「他偷了手機」可以作為「盜竊」預測的萃取推理。先前關於產生萃取推理的研究通常採用兩階段模型：選擇器選擇最重要的特徵（即推理），然後預測器僅根據所選特徵進行預測。這些研究的一個缺點是，學習選擇特徵的主要訊號來自將預測器給出的答案與真實答案進行比較。在這項研究中，我們建議透過資訊校正方法從預測器中提取更多資訊。更精確地說，我們聯合訓練兩個模型：一個是典型的解決手邊任務的神經模型，但以準確但黑盒的方式進行，另一個是選擇器預測器模型，它另外為其預測產生推理。第一個模型用作第二個模型的指南。我們使用對抗技術來校正兩個模型提取的資訊，使得它們之間的差異成為遺漏或過度選擇的特徵的指標。此外，對於自然語言任務，我們提出了一個基於語言模型的正則化器，以鼓勵提取流暢的推理。在情緒分析任務、仇恨言論識別任務以及法律領域的三個任務上的實驗結果顯示了我們在推理提取方法中的有效性。</paragraph>
...
摘要：近期通過認證的人工智慧（AI）醫療保健工具數量激增，這讓採用此技術的辯論再起。其中一個辯論的主題是可解釋 AI（XAI），以及它讓 AI 裝置更透明且更值得信賴的承諾。在醫療 AI 領域活躍的少數聲音表達了對可解釋 AI 技術，尤其是特徵歸因方法的可靠性疑慮，並質疑它們在準則和標準中的使用和納入。儘管有合理的疑慮，我們認為現有的批評，認為事後局部可解釋性方法不可行，是因將特定於影像資料的問題概括化，而將嬰兒連同洗澡水一起倒掉。我們從將問題描述為解釋與人類理解之間缺乏語意匹配開始。為了理解特徵重要性何時可用於可靠用途，我們引入了低階和高階特徵特徵重要性的區別。我們認為，對於低階特徵附帶明確語意的資料類型，例如電子健康記錄（EHR）等表格資料，可以取得語意匹配，因此特徵歸因方法仍然可用於有意義且有用的方式中。最後，我們勾勒出一項程序，以測試是否已達成語意匹配。
...
摘要：儘管人工智慧的機器學習 (ML) 模型在醫療領域中獲得高成效，但它們並非沒有錯誤。賦能臨床醫師辨識不正確的模型建議對於建立對醫療人工智慧的信任至關重要。可解釋人工智慧 (XAI) 旨在透過釐清人工智慧的推理來滿足此項需求，以支援最終使用者。最近在生物醫學影像方面進行的幾項研究獲得了有希望的結果。儘管如此，針對使用表格資料的模型的解決方案還不足以滿足臨床醫師的需求。本文提出了一種方法論，以協助臨床醫師找出使用表格資料訓練的 ML 模型的失敗之處。我們在三個主要支柱上建構了我們的方法論：透過利用臨床背景潛在空間分解特徵集、評估全局解釋的臨床關聯性，以及基於潛在空間相似性 (LSS) 的局部解釋。我們在 ML 基於感染所造成的早產兒發病率識別上展示了我們的做法。由於模型失敗而導致的死亡、終身殘疾和抗生素抗藥性的風險是這個領域中一個公開的研究問題。我們透過我們的做法識別出兩個模型的錯誤分類案例。透過將局部解釋脈絡化，我們的解決方案為臨床醫師提供了可行的見解，以支援他們自主做出明智的最終決定。
...
摘要：<paragraph>我們見證著 AI 經濟和社會的興起，其中 AI 技術日益影響著醫療保健、商業、運輸和日常生活中的許多方面。已經報導了許多成功案例，其中 AI 系統甚至超越了人類專家的準確性。然而，AI 系統可能會產生錯誤，可能會表現出偏見，可能對數據中的雜訊敏感，並且經常缺乏技術和司法透明度，導致信任度下降和採用方面的挑戰。這些最近的缺點和擔憂已在科學和一般媒體中得到記錄，例如自動駕駛汽車事故、醫療保健中的偏見、招聘和有色人種的臉部辨識系統、看似正確的醫療決定後來發現是由於錯誤的原因等。這導致許多政府和監管倡議的出現，要求可信賴且合乎道德的 AI 提供準確性和穩健性、某種形式的可解釋性、人類控制和監督、消除偏見、司法透明度和安全性。提供可信賴 AI 系統的挑戰促使對可解釋 AI 系統 (XAI) 進行深入研究。XAI 的目的是提供人類可以理解的資訊，說明 AI 系統如何做出決策。在本文中，我們首先簡要總結當前的 XAI 工作，然後挑戰最近關於準確性與可解釋性相互排斥且僅關注深度學習的論點。然後，我們提出我們對在高風險可信賴 AI 系統交付的整個生命週期中使用 XAI 的建議，例如開發、驗證和認證，以及可信賴的生產和維護。</paragraph>
...
摘要：深度神经網路 (DNN) 可偵測資料中的模式，並已在許多電腦視覺應用中展現多功能性和強大效能。
然而，單獨使用 DNN 容易犯下違反簡單常識概念的明顯錯誤，且在使用明確知識引導其搜尋和決策制定能力方面受到限制。雖然整體 DNN 效能指標可能不錯，但這些明顯的錯誤加上缺乏可解釋性，已阻礙其廣泛應用於諸如醫學影像分析等關鍵任務。本文的目的是介紹 SimpleMind，一個專注於醫學影像理解的認知 AI 開源軟體架構。它允許建立知識庫，以直觀的人類可讀形式描述影像物件預期的特性和關係。SimpleMind 框架透過下列方式為 DNN 帶來思考能力：(1) 提供方法以知識庫推論影像內容，例如空間推論和條件推論，以檢查 DNN 輸出；(2) 應用程序知識，以通用軟體代理程式形式，串連在一起以完成影像前處理、DNN 預測和結果後處理；(3) 對所有知識庫參數執行自動共同最佳化，以適應代理程式到特定問題。SimpleMind 能對多個偵測到的物件進行推理以確保一致性，提供 DNN 輸出之間的交叉檢查。此機器推理透過可解釋模型和可解釋決策，提升 DNN 的可靠性和可信度。範例應用程式說明 SimpleMind 如何透過將深度神經網路嵌入認知 AI 框架中，提供支援並加以改善。
...
摘要：在過去十年中，基於深度學習的方法在視覺感知任務方面取得了驚人的進展，並在從自動駕駛到臨床決策支援系統等眾多應用領域中得到廣泛採用。儘管這些基於深度學習的模型效能令人印象深刻，但它們在決策過程中仍然相當不透明，這使得它們在人類關鍵任務中的部署成為一項有風險的事業。這反過來又使得理解這些模型做出的決策對於它們的可靠部署至關重要。可解釋 AI (XAI) 方法試圖通過為這種黑盒深度學習方法提供解釋來解決這個問題。在本文中，我們對電腦視覺中的基於歸因的 XAI 方法進行了全面的調查，並回顧了現有的基於梯度的、基於擾動的和基於對比的 XAI 方法的文獻，並對開發和評估穩健的 XAI 方法的關鍵挑戰提供了見解。
...
摘要：深度學習（DL）自 2010 年代中期以來持續至今，
每年都帶來新的最高級和創新。儘管如此，
這些創新轉化為實際應用程式的速度落後於
這個快速的步伐。特別是安全關鍵應用程式，
需要嚴格的監管和道德要求，這些要求需要
得到照顧，並且仍然是爭論的活躍領域。可解釋 AI (XAI) 和
隱私保護機器學習 (PPML) 都是至關重要的研究領域，
旨在減輕 DL 中盛行的資料密集型黑盒模型的一些缺點。儘管
在各自領域進行了熱烈的研究活動，但尚未關注
它們的互動。這項工作是第一個研究
私人學習技術對基於 DL 的模型產生的解釋的影響。在
涵蓋來自多個領域的各種影像和時間序列
資料集以及不同的隱私技術、XAI
方法和模型架構的廣泛實驗分析中，研究了私人訓練對產生的
解釋的影響。研究結果表明，隱私的引入會對
解釋產生不可忽視的變化。除了報告
PPML 對 XAI 的個別影響外，本文還對
實際應用中技術的選擇提出了明確的建議。透過揭示
這些關鍵技術的相互依賴性，這項工作是克服
安全關鍵領域中實際可應用的 AI 所剩障礙的第一步。
...
摘要：<paragraph>肺結核（TB）是一種傳染性細菌疾病，是導致死亡的重要原因，特別是在低收入國家，據估計在 2020 年全球報告了 1000 萬例新病例。雖然肺結核可以治療，但未遵守藥物治療方案是導致發病率和死亡率的重要原因。因此，主動識別有中斷藥物治療風險的患者，可以採取糾正措施來減輕不良後果。使用極端不遵守治療的代理測量和來自印度四個邦的近 70 萬患者的數據集，我們根據自訂的基於排名指標制定並解決了非遵守治療的早期預測機器學習 (ML) 問題。我們訓練 ML 模型並根據基準進行評估，在基於規則的基準上提升了約 100%，在隨機分類器上提升了約 214%，考慮了全國範圍內的大規模未來部署。我們在過程中處理了各種問題，包括數據質量、高基數分類數據、低目標患病率、分佈轉移、不同群體之間的差異、演算法公平性，以及對穩健性和可解釋性的需求。我們的研究結果表明，非遵守治療患者的風險分層是一種可行的、可大規模部署的 ML 解決方案。作為印度中央肺結核部門的官方 AI 合作夥伴，我們正在與多個城市和州級試點項目合作，目標是全印度部署。</paragraph>
...
摘要：可解釋人工智慧 (XAI) 透過提升機器在最終用戶間的信任度，轉化人工智慧 (AI) 領域。由於連線裝置數量持續增加，物聯網 (IoT) 市場需要取得最終用戶的信任。然而，現有的文獻仍缺乏關於物聯網 XAI 使用的系統性且全面的調查工作。為了彌補這個不足，我們在本文中探討 XAI 架構，重點關注它們的特徵和對物聯網的支援。我們說明了廣泛使用的物聯網應用程式 XAI 服務，例如安全性增強、醫療物聯網 (IoMT)、工業物聯網 (IIoT) 和城市物聯網 (IoCT)。我們也建議在這些應用程式中，透過適當的範例，選擇在物聯網系統上實作 XAI 模型，並歸納出對未來工作的關鍵推論。此外，我們提出邊緣 XAI 架構的尖端發展，以及對物聯網應用程式的第六代 (6G) 通訊服務的支援，連同關鍵推論。簡而言之，本文構成第一個針對未來物聯網使用案例需求量身打造的 XAI 基礎架構發展的整體彙編。
...
摘要：美國每年約有 125 萬人因燒傷而接受治療。精確的燒傷分類是醫學 AI 領域的重要面向。在這項工作中，我們提出一個可解釋的人員參與迴圈架構，用於改善燒傷超音波分類模型。我們的架構利用基於 LIME 分類說明器的說明系統，來證實和整合燒傷專家的知識，建議新功能並確保模型的有效性。使用此架構，我們發現 B 模式超音波分類器可以透過提供紋理特徵來增強。更具體地說，我們確認基於超音波影像的灰階共生矩陣 (GLCM) 的紋理特徵可以提高轉移學習的燒傷深度分類器的準確度。我們在豬隻受試者的真實資料上測試我們的假設。我們顯示在根據我們的架構修改後，燒傷深度分類的準確度從 ~88% 提升至 ~94%。
...
摘要：在醫療保健領域，AI系統支援臨床醫生和患者進行診斷、治療和監控，但許多系統的解釋性不足，這對實際應用來說仍然是一個挑戰。克服這個障礙是可解釋性AI（XAI）的目標。然而，解釋可以被不同地感知，因此無法為所有人解決黑盒問題。以人為中心的AI領域通過將AI適應使用者來解決這個問題。我們提出了一個以使用者為中心的個性概念來評估XAI，並使用它來調查最終使用者在行動健康壓力監控應用程式中對各種解釋風格和內容的偏好。我們的線上調查結果顯示，使用者的背景資料和個性，以及解釋類型，會影響解釋偏好，這表明這些是XAI設計的基本特徵。我們將結果歸納為三個原型使用者角色：權力導向型、休閒型和隱私導向型使用者。我們的見解將互動式、以人為中心的XAI更接近實際應用。
...
摘要：青光眼是造成部分或完全失明的原因中排名第二的，在所有視力缺陷中，青光眼主要發生於眼睛因焦慮或憂鬱症而壓力過大，進而損害視神經並造成視力問題。傳統的青光眼篩檢是一個耗時的過程，需要醫療專業人員持續關注，即使如此，由於時間限制和壓力，他們還是無法正確分類，導致治療錯誤。已經有許多努力嘗試自動化整個青光眼分類程序，然而，這些現有的模型通常具有黑盒特性，使用戶無法了解預測背後的主要原因，因此醫護人員通常無法依賴這些系統。在本文中，我們在比較過各種預先訓練的模型後，提出一個轉移學習模型，該模型能夠以 94.71% 的準確度分類青光眼。此外，我們利用了局部可解釋模型不可知解釋（LIME），在我們的系統中引入了可解釋性。這項改進使醫療專業人員能夠獲得重要且全面的資訊，協助他們做出判斷。它也降低了傳統深度學習模型的不透明性和脆弱性。
...
摘要：深度学习在检测对象和分类图像方面表现出色，为分析医学影像提供了巨大的希望。
将深度学习的成功转化为医学影像，其中医生需要了解潜在过程，需要解释和说明神经网络的预测。深度神经网络的可解释性通常依赖于估计输入特征（例如，像素）相对于结果（例如，类概率）的重要性。然而，已经开发了许多重要性估计器（也称为显着性图），但尚不清楚哪些对于医学影像应用更相关。在目前的工作中，我们调查了几个重要性估计器在解释卷积深度网络对计算机断层扫描 (CT) 图像进行分类时的性能，使用了三个不同的评估指标。首先，模型中心保真度测量当某些输入受到干扰时模型精度的下降。其次，重要性评分和专家定义的分割掩模之间的协调性通过接收器操作特性 (ROC) 曲线在像素级别进行测量。第三，我们通过骰子相似系数 (DSC) 测量基于 XRAI 的地图和分割掩模之间的区域重叠。总体而言，SmoothGrad 的两个版本在保真度和 ROC 排名中名列前茅，而集成梯度和 SmoothGrad 在 DSC 评估中表现出色。有趣的是，模型中心（保真度）和以人为中心（ROC 和 DSC）评估之间存在严重差异。嵌入分割图中的专家期望和直觉不一定与模型如何得出其预测相一致。了解可解释性中的这种差异将有助于利用深度学习在医学中的力量。
...
摘要：深度學習方法，尤其是卷積神經網路，已成為醫學影像運算任務中強大的工具。儘管這些複雜模型提供了極佳的效能，但其黑箱本質可能會阻礙在高風險決策中實際採用。在本文中，我們提出一個互動系統，利用最先進的可解釋性技術，協助放射科醫師進行乳癌篩檢。我們的系統將深度學習模型整合到放射科醫師的工作流程中，並提供新穎的互動方式，以促進對模型決策過程的理解。此外，我們證明我們的系統可以逐步利用使用者互動，以提供更精細的解釋報告，而標記工作量很小。由於所採用的可解釋性技術的通用性，我們的系統與領域無關，可用於許多不同的醫學影像運算任務，對我們如何利用視覺分析將原本靜態的可解釋性技術轉化為擴增人類決策制定並促進醫學人工智慧採用的新觀點。
...
摘要：如今，人工智能 (AI) 已成為醫療保健應用程式（臨床和遠程）的基本組成部分，但效能最佳的 AI 系統通常過於複雜，無法自我解釋。可解釋 AI (XAI) 技術被定義為揭示系統預測和決策背後的推理，當處理敏感和個人健康資料時，它們變得更加關鍵。值得注意的是，XAI 並未在不同的研究領域和資料類型中獲得相同的關注，特別是在醫療保健方面。特別是，許多臨床和遠程醫療保健應用程式分別基於表格和時間序列資料，而 XAI 並未針對這些資料類型進行分析，而電腦視覺和自然語言處理 (NLP) 是參考應用程式。為了概述最適合醫療保健領域中表格和時間序列資料的 XAI 方法，本文回顧了過去 5 年的文獻，說明產生的解釋類型以及評估其相關性和品質所做的努力。具體來說，我們將臨床驗證、一致性評估、客觀且標準化的品質評估，以及以人為中心的品質評估，確定為確保對最終使用者進行有效解釋的主要特徵。最後，我們強調了該領域的主要研究挑戰，以及現有 XAI 方法的限制。
...
摘要：<paragraph>目的：本研究評估了市售可解釋 AI 演算法在提升臨床醫師透過胸部 X 光 (CXR) 辨識肺癌的能力方面的影響。
設計：這項回顧性研究評估了 11 位臨床醫師在有和沒有市售 AI 演算法（紅點，Behold.ai）協助的情況下，從胸部 X 光片中偵測肺癌的表現，該演算法會預測 CXR 中疑似肺癌。臨床醫師的表現會與臨床確診結果進行比對。
背景：這項研究分析了英國國家醫療服務體系 (NHS) 醫院的匿名患者資料；資料集包含 400 張 2020 年接受 CXR 檢查的成年患者（18 歲以上）的胸部 X 光片，以及對應的臨床文字報告。
參與者：一個由 11 位臨床醫師（顧問放射科醫師、放射科醫師受訓人員和報告放射技師）組成的讀者小組參與了這項研究。
主要結果指標：臨床醫師在有和沒有 AI 輸入的情況下，偵測 CXR 中肺癌的整體準確度、敏感度、特異度和精準度。臨床醫師之間的一致性評分和表現標準差也經過評估，並納入和排除 AI 輸入。
結果：臨床醫師使用 AI 演算法，提升了肺部腫瘤偵測的整體表現，讓原本會遺漏的 CXR 中肺癌偵測率整體提升 17.4%，整體偵測較小腫瘤的比率提升，第一期和第二期肺癌的偵測率分別提升 24% 和 13%，並標準化了臨床醫師的表現。
結論：這項研究顯示 AI 演算法在臨床應用上極具前景，能透過提升讀者整體表現，改善早期肺癌診斷並促進健康公平，同時不影響後續影像資源。</paragraph>
...
摘要：由於缺乏足夠大的開放式醫療資料庫，是 AI 驅動醫療保健中最大的挑戰之一。使用生成對抗網路 (GAN) 所建立的合成資料，似乎是減輕隱私政策問題的良好解決方案。另一種治療方法是在多個醫療機構中使用分散式協定，而無需交換本地資料樣本。在本文中，我們探討了在集中式和分散式設定中無條件和有條件的 GAN。集中式設定模仿了大型但高度不平衡的皮膚病變資料集的研究，而分散式設定則模擬了具有三個機構的更實際的醫院場景。我們根據保真度、多樣性、訓練速度和對使用所產生的合成資料訓練的分類器的預測能力，評估模型的效能。此外，我們透過探索潛在空間和嵌入式投影，提供了可解釋性，重點放在整體和局部解釋。計算實體影像與其在潛在空間中的投影之間的距離，證明了訓練過 GAN 的真實性和概括性，而這正是這類應用程式的主要考量之一。已公開執行研究的開放原始碼，網址為 \url{https://github.com/aidotse/stylegan2-ada-pytorch}。
...
摘要：在醫院的世界裡，存在著許多複雜的組合問題，解決這些問題對於提升病患滿意度和照護品質非常重要。醫療保健中的問題很複雜，因為要解決它們，必須考慮到許多限制和不同類型的資源。此外，必須在短時間內評估解決方案，以確保在實際情況中可用。我們計畫提出解決這些類型的問題的方法，包括擴展已經測試過的解決方案，以及透過建模新問題的解決方案，同時考慮文獻，並在可行的情況下使用真實資料。解決這些類型的問題很重要，但由於歐盟執委會制定了《一般資料保護條例》，規定每個人都有權要求解釋 AI 做出的決定，因此在沒有發展出可解釋性方法的情況下，基於 AI 的求解器（例如基於答案設定程式設計的求解器）的使用將會受到限制。因此，研究的另一部分將致力於研究並提出新的方法來解釋所獲得的解決方案。
...
摘要：在現代動態且不斷發展的社會中，越來越多人飽受慢性疾病和嚴重疾病所苦，而醫生和病患需要特殊且精密的醫療與健康支援。因此，傑出的醫療保健利益相關者已認知到此類服務發展的重要性，以讓病患的生活更輕鬆。此類支援需要收集大量病患複雜的資料，例如臨床、環境、營養、日常活動、各種智慧穿戴式裝置資料、配備感測器的服裝資料等。整體病患資料必須適當地彙總、處理、分析，並提供給醫生和照護人員，以建議適當的治療和行動，以改善病患的健康相關參數和整體健康狀況。進階的人工智慧技術提供機會來分析此類大數據、使用它們，並衍生新知識，以支援個人化的醫療決策。新的方法，例如基於進階機器學習、聯盟學習、轉移學習、可解釋的人工智慧，為未來更優質地使用健康和醫療資料開啟新途徑。在本文中，我們將提出一些關鍵面向和特徵範例，說明在個人化醫療決策中應用一系列人工智慧方法的領域。
...
摘要：<paragraph>持續監控已訓練的 ML 模型，以判斷何時應該信任或不信任其預測，對於安全部署它們至關重要。這樣的架構應該具有高性能、可解釋性、事後性和可操作性。我們提出 TRUST-LAPSE，一個用於持續模型監控的「不信任」評分架構。我們使用一系列潛在空間嵌入來評估每個輸入範例的模型預測的可信度。具體來說，(a) 我們的潛在空間不信任評分使用潛在空間中的距離度量（馬氏距離）和相似度度量（餘弦相似度）來估計不信任，以及 (b) 我們的順序不信任評分確定過去輸入表示順序中的相關性偏差，採用非參數、基於滑動視窗的演算法進行可操作的持續監控。我們透過兩個下游任務評估 TRUST-LAPSE：(1) 分布式偏移輸入偵測，以及 (2) 資料漂移偵測。我們使用公共資料集評估各種領域 - 音訊和視覺，並進一步在具有挑戰性的真實世界腦電圖 (EEG) 資料集上評估我們的做法，以進行癲癇偵測。我們的潛在空間不信任評分獲得了最先進的結果，AUROC 為 84.1（視覺）、73.9（音訊）和 77.1（臨床 EEG），優於基準線 10 個百分點以上。我們揭露了流行基準線中的嚴重失敗，這些失敗對輸入語義內容不敏感，使其不適合於真實世界的模型監控。我們表明，我們的順序不信任評分達到了很高的漂移偵測率；所有領域中超過 90% 的串流顯示錯誤小於 20%。透過廣泛的定性和定量評估，我們表明我們的信任評分更強大，並提供可解釋性，以便於實務採用。</paragraph>
...
摘要：機器學習的普及性增加了不公平模型在高風險應用程式中部署的風險，例如司法系統、藥物/疫苗設計和醫療診斷。雖然有從頭訓練公平模型的有效方法，但如何自動揭露和解釋訓練模型的不公平性仍然是一項具有挑戰性的任務。以可解釋的方式揭露機器學習模型的不公平性是邁向公平且值得信賴的 AI 的關鍵一步。在本文中，我們系統性地解決了通過挖掘可解釋證據 (RUMIE) 來揭露不公平模型的新任務。其關鍵思想是以一群資料實例的形式找到具體證據，這些實例受到模型最大的歧視。為了使證據可解釋，我們還找到了一組人類可理解的關鍵屬性和決策規則，這些屬性和決策規則表徵了受歧視的資料實例，並將它們與其他未受歧視的資料區分開來。正如在許多真實世界資料集上進行的廣泛實驗所證明的那樣，我們的模型找到了高度可解釋且具體的證據，可以有效地揭露訓練模型的不公平性。此外，它比所有基線模型都更具可擴充性。
...
摘要：可解釋 AI (XAI) 在安全關鍵系統中是必需的，例如在臨床診斷中，因為有做出致命決策的高風險。然而，目前 XAI 就像是一個鬆散的方法集合，而不是一個定義良好的流程。在這項工作中，我們闡述了 XAI 最大子群體可解釋機器學習 (IML) 與古典統計之間的概念相似性。根據這些相似性，我們提出 IML 的形式化，類似於統計過程。採用這種統計觀點使我們能夠將機器學習模型和 IML 方法解釋為複雜的統計工具。根據這種解釋，我們推論出三個關鍵問題，我們將其視為在安全關鍵環境中 IML 成功和採用的關鍵。透過制定這些問題，我們進一步旨在激發關於 IML 與古典統計之間的區別以及我們的觀點對該領域的未來有何影響的討論。
...
摘要：人工智慧已廣泛應用於各學科和領域，而生物醫學影像和訊號處理也不例外。對此主題日益增長且廣泛的興趣已觸發了大量的研究活動，這反映在指數級的研究成果中。透過研究大量的多元生物醫學資料，機器和深度學習模型已徹底改變了建模、分割、配準、分類和合成等各種任務，其效能優於傳統技術。然而，將結果轉譯為生物學上/臨床上可解讀的資訊的難度阻礙了其在該領域的充分利用。可解釋人工智慧 (XAI) 嘗試透過提供方法來使模型可解釋並提供說明，以填補這種轉譯差距。迄今已提出不同的解決方案，並獲得社群日益增長的興趣。本文旨在提供生物醫學資料處理中 XAI 的概觀，並指出 IEEE 訊號處理雜誌中即將出版的生物醫學影像和訊號處理深度學習特刊，該特刊將於 2022 年 3 月出刊。
...
摘要：在使用醫療影像進行診斷時，無論是臨床醫生或人工智慧 (AI) 系統，影像品質都很重要。當影像品質低落時，產生影像的醫療檢查通常需要重新進行。在遠距醫療中，常見的問題是，只有在患者離開診所後才會標示品質問題，這表示他們必須回來重新進行檢查。這對居住在偏遠地區的人來說尤其困難，而這些人佔巴西一家數位醫療保健組織 Portal Telemedicina 的病患很大一部分。在本文中，我們報告關於以下事項的進行中工作：(i) 開發一種 AI 系統，用於標示並解釋低品質醫療影像，並提供即時說明，(ii) 一項訪談研究，用於了解在 OurCompany 使用 AI 系統的利害關係人的說明需求，以及 (iii) 一項縱向使用者研究設計，用於檢查在我們的診所中，將說明納入技術人員的工作流程中所產生的影響。據我們所知，這將是第一個針對 XAI 方法對最終使用者（使用 AI 系統但沒有特定 AI 專業知識的利害關係人）的影響進行評估的縱向研究。我們歡迎針對我們的實驗設定提供意見回饋和建議。
