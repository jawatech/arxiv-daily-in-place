...
摘要：<paragraph>最近人工智能的突破有望從根本上提升我們對醫療保健的研究和理解。開發一個整合多對多的架構，利用多種資料模式輸入進行多種醫療任務的分析建模，對於統一理解現代醫學至關重要。在這項工作中，我們介紹了 M3H，一個可解釋的多模式多任務機器學習醫療保健框架，它整合了來自廣泛醫療任務類別和機器學習問題類別的多種多模式輸入的學習。框架的模組化設計確保了其資料處理、任務定義和快速模型原型製作的可概化性，適用於臨床和運營醫療保健環境。我們通過驗證從四種模式（表格、時間序列、語言和視覺）訓練的模型，在四個機器學習問題類別中的 41 個醫療任務上評估 M3H 框架。我們的結果表明，M3H 持續產生多任務模型，其表現優於來自 16 個醫療部門的 37 種疾病診斷、三個醫院運營預測和一個患者表型任務的典型單任務模型（1.1-37.2%）。涵蓋了監督二元分類、多類分類、迴歸和聚類的 ML 問題類別。此外，該框架引入了一種新穎的注意力機制，以平衡自我開發（專注於學習源任務）和交叉探索（鼓勵從其他任務中學習）。此外，M3H 提供了可解釋性見解，說明額外任務的聯合學習如何影響源任務的學習，使用提出的 TIM 分數，揭示任務相互依賴的動態。其適應性架構促進了自訂和整合，使其成為未來 AI 驅動醫療保健系統的強大且可擴充的候選解決方案。</paragraph>
...
摘要：本文探討了群體結構多智能體系統 (MAS) 在建立醫療必要性中的應用，這是一個涉及系統性審查特定患者醫療結構化和非結構化資料，並對照臨床指南的過程。我們透過將此複雜任務分解成更小、更易於管理的子任務來解決。每個子任務都由一個專業的 AI 代理處理。我們進行了一項系統性研究，探討了各種提示策略對這些代理的影響，並對不同的大型語言模型 (LLM) 進行基準測試，以確定它們在完成這些任務時的準確性。此外，我們探討了這些代理如何提供可解釋性，從而增強系統內的信任和透明度。
...
摘要：人工智慧 (AI) 的普遍整合在涉及 AI 啟用系統的事故中引入了責任和問責的複雜挑戰。這些系統的互連性、AI 誘發事件的道德考量，加上 AI 技術的不確定性與缺乏相應法規，使得傳統責任歸屬具有挑戰性。為此，本研究提出計算反思均衡 (CRE) 方法，為所有利害關係人建立一個連貫且在道德上可接受的責任歸屬架構。計算方法提供結構化分析，克服概念方法在處理動態且多面向場景時的限制，展示架構在責任歸屬過程中的可解釋性、連貫性和適應性。我們探討與均衡計算中的主張相關的初始啟動層級的關鍵角色。使用 AI 輔助醫療決策支援系統作為案例研究，我們說明不同的初始化如何導致不同的責任分配。該架構提供對 AI 誘發事件中問責制的寶貴見解，透過持續監控、修正和反思，促進永續且有韌性的系統開發。
...
摘要：人工智慧藉由預測模型協助醫療專業人員，大幅轉變臨床決策制定。本研究探討在醫療保健中 AI 應用程式中公平性和可解釋性的關鍵需求，以確保不同患者人口統計資料的公平結果。藉由專注於敗血症相關死亡率的預測模型，我們提出一個學習效能最佳化預測模型的方法，然後採用轉移學習程序來產生具有更好公平性的模型。我們的模型也引進一個新穎的基於置換特徵重要性演算法，旨在闡明每個特徵在增強預測公平性上的貢獻。與現有的可解釋性方法專注於解釋特徵對預測效能的貢獻不同，我們提出的方法獨特地彌補了理解每個特徵如何有助於公平性的差距。鑑於敗血症的顯著死亡率及其在醫院死亡中所扮演的三分之一角色，這項進展至關重要。我們的模型不僅有助於找出並減輕預測模型中的偏差，也藉由改善模型預測的透明度和公平性來促進醫療保健利害關係人之間的信任，進而促成更公平且值得信賴的醫療保健服務。
...
摘要：<paragraph>憂鬱症是現今社會中一個重要的議題。根據世界衛生組織（WHO）的數據，在 2023 年，超過 2.8 億人正與憂鬱症搏鬥。這是一個龐大的數字；如果沒有認真看待，這些數字將會快速增加。大約有 48.9 億人是社群媒體的使用者。人們在 Twitter、Facebook、Reddit、Instagram 等平台上表達自己的感受和情緒。這些平台含有有價值的資訊，可以用於研究目的。已經在各種社群媒體平台上進行了大量的研究。然而，這些努力中仍然存在某些限制。特別是，先前的研究僅專注於偵測推文中的憂鬱症和憂鬱症的嚴重程度。此外，資料集標籤中存在不準確之處。在這項研究工作中，使用基於詞彙標籤的 Twitter 資料庫中的推文預測了五種類型的憂鬱症（躁鬱症、重度憂鬱症、精神病性憂鬱症、非典型憂鬱症和產後憂鬱症）。可解釋 AI 用於透過強調代表憂鬱症類型的推文部分來提供推理。雙向編碼器表徵來自 Transformers（BERT）用於特徵萃取和訓練。機器學習和深度學習方法用於訓練模型。BERT 模型呈現出最有希望的結果，達到了 0.96 的整體準確度。</paragraph>
...
摘要：深度學習顯著地轉變了醫學影像和放射學的領域，能辨識醫學影像中的病理，包括電腦斷層掃描 (CT) 和 X 光掃描。然而，深度學習模型的效能，特別是在分割任務中，常受到廣泛註解資料集需求的限制。為了解決此挑戰，透過可解釋 AI 和反事實解釋的產生，探索了弱監督語意分割的能力。本研究的範圍是開發一種新穎的反事實內插方法 (COIN)，透過使用生成模型，將預測的分類標籤從異常翻轉為正常。例如，如果分類器將輸入醫學影像 X 視為異常，表示存在病理，則生成模型旨在內插異常區域，從而逆轉分類器的原始預測標籤。此方法使我們能夠產生病理的精確分割，而無需依賴於預先存在的分割遮罩。至關重要的是，利用影像層級標籤，其比建立詳細的分割遮罩容易取得。此方法的有效性透過從愛沙尼亞塔爾圖大學醫院取得的 CT 影像中分割合成目標和實際腎臟腫瘤來證明。研究結果表明，COIN 遠遠優於已建立的歸因方法，例如 RISE、ScoreCAM 和 LayerCAM，以及 Singla 等人提出的替代反事實解釋方法。此證據表明，COIN 是一種用於 CT 影像中腫瘤語意分割的有前景方法，並在醫療保健中邁出了一步，使深度學習應用程式更容易取得且更有效，特別是註解資料稀少的情況。
...
摘要：甲狀腺癌是一個日益嚴重的全球健康問題，需要進階的診斷方法。本文探討了人工智慧和放射組學在甲狀腺癌診斷中的應用。根據 PRISMA 指南，我們檢視了多個資料庫，截止時間為 2023 年 10 月。結合多個關鍵字，我們發現了關於甲狀腺癌和相關主題的英文學術出版品。在移除 109 個重複資料後，原始搜尋回傳了 267 篇論文。在根據預先設定的標準審查了 124 篇論文的摘要和標題後，我們選出了相關的研究。在進行全面分析後，我們又排除了六篇研究。在納入的 28 篇研究中，結合超音波（US）影像的放射組學分析證明了其在診斷甲狀腺癌方面的有效性。我們觀察到各種結果，有些研究提出了優於現狀的新策略。文獻強調了人工智慧模型面臨的各種挑戰，包括可解釋性問題、資料集限制和操作員依賴性。納入的 28 篇研究的綜合發現提到，需要標準化工作和前瞻性多中心研究來解決這些問題。此外，我們也找出了克服這些障礙的方法，例如可解釋人工智慧技術和個人化醫療技術的進展。本文探討了人工智慧和放射組學如何轉變甲狀腺癌的診斷和治療。儘管面臨挑戰，未來在多學科合作、臨床適用性驗證和演算法改進方面的研究仍有潛力改善甲狀腺癌治療的患者預後和診斷精準度。
...
摘要：<paragraph>乳癌近年來盛行率快速攀升，成為全球主要的死亡原因之一，在所有癌症中，乳癌更是高居榜首。手動診斷此疾病需要耗費大量的時間和專業知識。由於乳癌的檢測是一個耗時的過程，因此透過建立機器學習預測模型，可以協助預防乳癌進一步擴散。機器學習和可解釋人工智慧在分類中至關重要，它們不僅提供準確的預測，還能提供模型如何做出決策的見解，有助於理解和信賴分類結果。在本研究中，我們評估並比較了五種不同的機器學習方法在分類準確度、精確度、召回率和 F-1 分數，並使用了一個主要的資料集（來自達卡醫學院醫院的 500 名患者）。五種不同的監督式機器學習技術，包括決策樹、隨機森林、邏輯迴歸、樸素貝氏和 XGBoost，已被用於在我們的資料集上實現最佳結果。此外，本研究將 SHAP 分析應用於 XGBoost 模型，以解釋模型的預測並了解每個特徵對模型輸出的影響。我們比較了幾種演算法對資料進行分類的準確度，並與該領域的其他文獻進行了對比。經過最終評估，本研究發現 XGBoost 達到了最佳的模型準確度，為 97%。</paragraph>
...
摘要：深度學習 (DL) 模型用於透過乳房攝影影像診斷乳癌，通常運作方式為「黑盒子」，讓醫療專業人員難以信賴和理解其決策過程。本研究提出一個整合架構，結合卷積神經網路 (CNN) 和可解釋人工智慧 (XAI)，以使用 CBIS-DDSM 資料集來增強乳癌診斷。此方法包含一個詳盡的資料前處理管線和進階資料擴充技術，以因應資料集限制，並採用預先訓練的網路（例如 VGG-16、Inception-V3 和 ResNet）進行遷移學習。我們研究的重點在於評估 XAI 在詮釋模型預測的效能，其重點在於利用豪斯多夫測度來評估 AI 生成的說明和專家註解之間的對齊程度。此方法對於 XAI 在促進 AI 輔助診斷中的可信度和倫理公平性至關重要。我們研究的發現說明了 CNN 和 XAI 在推進乳癌診斷方法上的有效協作，進而促進先進 AI 技術在臨床環境中的無縫整合。透過增強 AI 驅動決策的可解釋性，這項工作為 AI 系統和醫療從業人員之間的進階協作奠定基礎，最終豐富了病患照護。此外，我們研究的影響遠遠超出了目前的技術。它鼓勵進一步研究如何結合多模式資料並改善 AI 說明，以滿足臨床實務的需求。
...
摘要：以人为本的可解释人工智能（HCXAI）提倡将社会方面整合到人工智能解释中。HCXAI 话语的核心是社会透明度 (ST) 框架，其目标是让用户能够了解人工智能系统的社会组织背景。在这项工作中，我们建议扩展 ST 框架以解决大型语言模型 (LLM) 中社会错误归因的风险，尤其是在心理健康等敏感领域。事实上，LLM 具有模拟角色和人物的惊人能力，可能导致设计者的意图与用户对社会属性的感知之间出现错配，从而有引发情感操纵和危险行为、认知不公正和不合理信任的风险。为了解决这些问题，我们建议使用第五个“W 问题”来增强 ST 框架，以阐明设计者和用户赋予 LLM 的具体社会属性。此补充旨在弥合 LLM 能力和用户感知之间的差距，促进基于 LLM 的技术的道德责任开发和使用。
...
摘要：<paragraph>背景：氣胸是一種急性胸腔疾病，是由肺部和胸壁之間異常的空氣集合所引起的。為了解決深度學習 (DL) 模型經常出現的不透明性，可解釋人工智慧 (XAI) 方法已被引入，用於概述與 DL 模型所做的氣胸診斷相關的區域。然而，這些解釋有時會與實際病灶區域有所出入，這凸顯了進一步改進的必要性。方法：我們提出一個模板引導方法，將氣胸的臨床知識納入 XAI 方法產生的模型解釋中，從而提高這些解釋的品質。利用放射科醫師建立的一個病灶描繪，我們的做法首先產生一個模板，用於表示氣胸發生的潛在區域。然後將這個模板疊加在模型解釋上，以過濾掉落在模板邊界之外的無關解釋。為了驗證其功效，我們對三種 XAI 方法進行了比較分析，在兩個真實世界資料集中解釋兩個 DL 模型時，分別採用和不採用我們的模板指導。結果：所提出的方法在建立在三種 XAI 方法、兩個 DL 模型和兩個資料集上的十二個基準情境中，一致地改進了基準 XAI 方法。在比較模型解釋和真實病灶區域時，通過基準效能的效能改進計算出的平均增量百分比，在聯合交集 (IoU) 中為 97.8%，在 Dice 相似性系數 (DSC) 中為 94.1%。結論：在氣胸診斷的背景下，我們提出了一個模板引導方法，用於改進 AI 解釋。我們預期我們的模板指導將通過整合臨床領域專業知識，為闡明 AI 模型建立一個新的方法。</paragraph>
...
摘要：<paragraph>在當前的機器翻譯 (MT) 領域中，Transformer 架構脫穎而出，成為黃金標準，特別適用於高資源語言對。本研究探討其對低資源語言對的效能，包括英語↔愛爾蘭語和英語↔馬拉提語語言對。值得注意的是，本研究找出最佳超參數和子詞模型類型，以顯著提升 Transformer 模型對低資源語言對的翻譯品質。
  低資源語言平行資料集的稀少性可能會阻礙機器翻譯的發展。為了解決這個問題，開發了 gaHealth，這是愛爾蘭語的第一個雙語健康資料語料庫。專注於健康領域，使用此領域內資料集開發的模型在 BLEU 分數上展現非常顯著的進步，與 LoResMT2021 共享任務的模型相比更是如此。後續使用多維品質指標錯誤分類法進行的人工評估顯示，與基於 RNN 的對應模型相比，Transformer 系統在減少準確性和流暢性錯誤方面表現出優異的效能。
  此外，本論文介紹 adaptNMT 和 adaptMLLM，這兩個開源應用程式簡化了神經機器翻譯模型的開發、微調和部署。這些工具大幅簡化了設定和評估流程，讓開發人員和翻譯人員都能更輕鬆地使用機器翻譯。值得注意的是，基於 OpenNMT 生態系統的 adaptNMT 透過強調模型開發的環境足跡，促進生態友善的自然語言處理研究。與 LoResMT2021 共享任務的基準相比，adaptMLLM 對 MLLM 進行微調，證明了英語↔愛爾蘭語和英語↔馬拉提語這兩個低資源語言對的翻譯效能有所進步。</paragraph>
...
視網膜影像和血管反映了身體的微血管和巨血管健康。它們可用於診斷糖尿病併發症，包括糖尿病視網膜病變（DR）、神經病變、腎病和動脈粥樣硬化性心血管疾病，以及預測心血管事件的風險。使用數位化視網膜影像進行高通量 DR 檢測的人工智慧（AI）啟用系統已在臨床上採用。除了 DR 篩檢之外，AI 整合也具有巨大的潛力，可以解決與糖尿病患者整體照護相關的挑戰。在這項工作中，我們的目標是全面回顧基於視網膜影像的 AI 應用研究文獻，這些研究與糖尿病的診斷、預後和管理有關。我們將描述整體 AI 輔助糖尿病照護的發現，包括但不限於 DR 篩檢，並討論實施此類系統的障礙，包括涉及倫理、資料隱私、公平取得和可解釋性的問題。透過評估患者的健康狀態，同時考量糖尿病併發症以及未來心血管併發症的風險預後，AI 輔助視網膜影像分析有可能成為糖尿病患者現代化個人化醫療的中心工具。
...
摘要：本研究從多個利害關係人的角度探討了不同的人工智慧 (AI) 應用在教育中的可接受性，包括學生、教師和家長。承認人工智慧在教育中的轉型潛力，它解決了與資料隱私、人工智慧代理、透明度、可解釋性和人工智慧的道德部署相關的問題。透過小插曲方法，參與者被呈現四種場景，其中操縱了人工智慧的代理、透明度、可解釋性和隱私。在每個場景之後，參與者完成了一項調查，調查捕捉了他們對人工智慧的整體效用、個人效用、正義、信心、風險以及如果可用，使用每個場景的人工智慧的意圖的看法。收集的資料包括 1198 位多個利害關係人參與者的最終樣本，透過合作機構和社群媒體活動進行分發，並專注於對四個人工智慧使用案例的個人回應。對資料的調解分析表明，對人工智慧的接受度和信任在利害關係人組別中存在顯著差異。我們發現，人工智慧的代理、透明度和可解釋性的高低層級之間的關鍵調解因素，以及使用不同教育人工智慧的意圖，包括感知的整體效用、正義和信心。該研究強調，在教育中接受人工智慧是一個微妙且多方面的問題，除了不同的利害關係人的看法之外，還需要仔細考慮具體的人工智慧應用及其特徵。
...
摘要：遠端病人監測基於可穿戴式單導程心電圖 (ECG) 裝置，對於早期偵測心臟疾病有顯著的潛力，特別是結合人工智慧 (AI) 方法進行自動化的心臟疾病偵測。先前已有研究應用基於深度學習的 AI 方法進行心臟疾病偵測。然而，這些模型尚未被廣泛接受為臨床診斷的可靠輔助工具，部分原因是目前許多 AI 演算法存在黑盒子的認知。特別是，有必要找出心電圖訊號中有助於做出準確診斷的主要特徵，從而增強模型的可解釋性。在本研究中，我們開發了一種視覺轉換器方法，根據單導程心電圖資料辨識心房顫動。殘差網路 (ResNet) 方法也與視覺轉換器方法進行比較。這些模型應用於 Chapman-Shaoxing 資料集，以分類心房顫動，以及另一種常見的心律不整，竇性心動過緩，和正常竇性心律的心跳。這些模型能夠辨識心跳中決定最終分類的關鍵區域，並強調 P 波和 T 波的重要性，以及心跳持續時間和訊號振幅，在區分正常竇性心律、心房顫動和竇性心動過緩方面。
...
摘要：本文介紹了一種用於憂鬱症偵測和治療的新典範，利用先進的大語言模型 (LLM)：生成式預訓練Transformer 4 (GPT-4)、Llama 2 聊天機器人和 Gemini。這些 LLM 經過微調，並採用專業提示來診斷、解釋和建議憂鬱症的治療介入措施。一種獨特的少次提示方法增強了模型根據 DSM-5 標準分析和解釋憂鬱症狀的能力。在互動階段，這些模型從事同理對話管理，從 PsychDB 等資源和認知行為療法 (CBT) 指南中汲取，促進與經歷重度憂鬱症的個人進行支持性互動。此外，本研究還介紹了 Illuminate 資料庫，其中包含各種 CBT 模組，有助於個性化治療建議。本研究使用 F1 分數、準確度、召回率、餘弦相似性和面向召回率的評估替身 (ROUGE) 等指標，在不同的測試集中評估 LLM 的效能，證明了它們的有效性。這種綜合方法將尖端的 AI 與既定的心理方法相結合，為心理保健提供了新的可能性，並展示了 LLM 在革新憂鬱症診斷和治療策略方面的潛力。
...
摘要：<paragraph>AI 系統的說明很少能滿足受演算法決策 (ADM) 影響之人士的資訊需求。傳達資訊與受影響利害關係人重視的資訊之間的差距，可能會阻礙理解和遵守 AI 法規等法規框架。為了解決此差距，我們提出了「XAI 新手問題庫」：受影響利害關係人資訊需求的目錄，涵蓋兩個 ADM 使用案例（就業預測和健康監控），涵蓋資料、系統背景、系統使用和系統規格類別。在受訪者根據詢問獲得說明的訪談研究中收集資訊需求。參與者進一步報告他們的理解和決策信心，顯示雖然在收到說明後信心往往會增加，但參與者也遇到了理解挑戰，例如無法說明為什麼他們的理解感覺不完整。說明進一步影響了參與者對系統風險和好處的看法，他們會根據使用案例確認或更改這些看法。當風險被認為很高時，參與者表示特別有興趣了解意圖的說明，例如系統為何以及為了什麼目的而建立。透過這項工作，我們旨在透過提供資訊和挑戰的概觀，在受影響利害關係人決定採用 ADM 系統時，支持將受影響利害關係人納入可解釋性。我們最後總結我們的發現，列出六項關鍵影響，為未來針對受影響利害關係人受眾的說明設計提供資訊。</paragraph>
...
摘要：<paragraph>人工智慧（AI）的快速演進，特別是在大型語言模型（LLM）和生成式 AI 的領域，為各個領域的應用開啟了新途徑，但其在商業教育中的角色仍未被充分探討。本研究首次引進基準來評估七個主要 LLM 的效能，包括 OpenAI 的模型（GPT-3.5 Turbo、GPT-4 和 GPT-4 Turbo）、Google 的模型（PaLM 2、Gemini 1.0 Pro）和 Anthropic 的模型（Claude 2 和 Claude 2.1），這些模型在 GMAT 上的表現，GMAT 是研究所商學課程錄取過程中的一項重要考試。我們的分析顯示，大多數 LLM 的表現都優於人類考生，其中 GPT-4 Turbo 不僅優於其他模型，還超越了頂尖商學院研究所學生的平均分數。透過個案研究，本研究探討了 GPT-4 Turbo 解釋答案、評估回應、找出錯誤、調整說明和產生替代方案的能力。最新的 LLM 版本 GPT-4 Turbo、Claude 2.1 和 Gemini 1.0 Pro 與其前身相比，在推理任務方面有顯著的進步，突顯了它們在解決複雜問題上的潛力。儘管 AI 在教育、評量和輔導方面的承諾很明確，但挑戰仍然存在。我們的研究不僅闡明了 LLM 的學術潛力，也強調了在教育中謹慎開發和應用 AI 的必要性。隨著 AI 技術的進步，建立 AI 互動的架構和協定、驗證 AI 生成內容的準確性、確保全球各地不同學習者都能使用，並創造一個 AI 支援人類專業知識的教育環境至關重要。本研究為進一步探索負責任地使用 AI 來豐富教育體驗和改善考試準備與評量方法奠定了基礎。</paragraph>
...
摘要：在重症监护室 (ICU) 中预测住院死亡率是最终临床结果的关键。AI 已显示出优势准确性，但缺乏可解释性。为了解决这个问题，本文提出了一种可解释的多模态死亡率预测器 (X-MMP)，为预测通过多模态 ICU 数据住院死亡率提供了一种高效、可解释的 AI 解决方案。我们在框架中采用多模态学习，它可以接收来自临床数据的异构输入并做出决策。此外，我们引入了一种可解释的方法，即层级传播到 Transformer，作为 LRP 方法到 Transformer 的适当扩展，对多模态输入进行解释并揭示归因于预测的显着特征。此外，可以可视化每个模态对临床结果的贡献，帮助临床医生理解决策背后的推理。我们基于 MIMIC-III 和 MIMIC-III 波形数据库匹配子集构建了一个多模态数据集。基准数据集上的综合实验表明，我们提出的框架可以在具有竞争力的预测准确性的情况下实现合理的解释。特别是，我们的框架可以轻松地转移到其他临床任务，这有助于发现医疗保健研究中的关键因素。
...
摘要：<paragraph>在過去的十年中，病理學中的人工智慧 (AI) 方法已大幅進展。然而，由於將研究結果轉化為臨床診斷產品的技術和法規障礙，以及缺乏標準化介面，整合到常規臨床實務的進度緩慢。開放且與供應商無關的 EMPAIA 計畫應對了這些挑戰。在此，我們提供了 EMPAIA 成就和經驗教訓的概述。EMPAIA 整合了病理 AI 生態系統的各種利益相關者，即病理學家、電腦科學家和產業。在密切合作下，我們制定了技術互通性標準、AI 測試和產品開發建議，以及可解釋性方法。我們實作了模組化和開放原始碼的 EMPAIA 平台，並成功整合了來自 8 家不同供應商的 14 個基於 AI 的影像分析應用程式，展示了不同的應用程式如何使用單一標準化介面。我們優先考慮需求，並評估了 AI 在歐洲和亞洲 14 個不同病理實驗室的實際臨床環境中的使用情況。除了技術開發之外，我們還為所有利益相關者建立了一個論壇，以分享數位病理學和 AI 的資訊和經驗。商業、臨床和學術利益相關者現在可以採用 EMPAIA 的通用開放原始碼介面，為大規模標準化和流程簡化提供了獨特的機會。需要進一步的努力才能有效且廣泛地在常規實驗室使用中建立 AI 協助。為此，已成立非營利協會 EMPAIA International，作為一個永續基礎架構，以持續標準化和支援 AI 輔助數位病理學未來的廣泛實作和倡導。</paragraph>
...
摘要：反事實解釋 (CE) 技術已引起關注，可用於為與 AI 系統互動的使用者提供見解。儘管已廣泛研究醫療影像和自動駕駛汽車等領域，但圖形反事實解釋 (GCE) 方法相對較少被探索。GCE 會產生一個與原始圖形相似的圖形，其不同的結果基於底層預測模型。在這些 GCE 技術中，儘管在其他領域（例如藝術風格和自然語言建模）中展現出令人印象深刻的成就，但植根於生成機制的技術卻受到相對有限的研究。對生成式解釋器的偏好源於它們在推理過程中生成反事實實例的能力，利用輸入圖形自主獲得的擾動。基於上述理由，我們的研究引入了 RSGG-CE，這是一個用於反事實解釋的新型穩健隨機圖形生成器，能夠從已學習的潛在空間中產生反事實範例，考慮部分有序的生成序列。此外，我們進行量化和質性分析，以比較 RSGG-CE 與 SoA 生成式解釋器的效能，強調其產生合理反事實候選項的能力有所提升。
...
摘要：可解釋 AI 的動機之一是讓人類能夠對 AI 模型的使用和部署做出更好、更明智的決策。但需要仔細評估才能評估是否已達到此預期。目前的評估主要關注解釋的演算法特性，而涉及人類受試者的評估通常使用主觀問題來測試人類對解釋有用性的看法，而沒有依據客觀的指標和測量。在這項工作中，我們評估解釋是否能在機器學習模型開發的實際場景中改善人類的決策制定。我們進行了一項涉及影像資料的混合方法使用者研究，以評估 SmoothGrad、GradCAM 和一個神諭解釋在兩個任務中產生的顯著性圖：模型選擇和反事實模擬。令人驚訝的是，我們沒有發現當使用者獲得任何顯著性圖時，即使是設計為易於理解且高度指示答案的合成神諭解釋，這些任務有顯著改善的證據。儘管如此，解釋確實有助於使用者更準確地描述模型。這些發現表明對於基於顯著性的解釋中，有用性和潛在的誤解應持謹慎態度。
...
摘要：可解釋性和安全性培養信任。這些需要一個模型來展現一致性和可靠性。要達成這些，必須使用和分析資料和知識，以及與 AI 應用相關的統計和符號 AI 方法 - 單獨使用其中一種方法無法達成。因此，我們主張並試圖證明神經符號 AI 方法更適合用來打造令人信賴的 AI 系統。我們提出 CREST 架構，說明一致性、可靠性、使用者層級的可解釋性，以及安全性是如何建立在神經符號方法上，該方法使用資料和知識來支援關鍵應用程式的需求，例如健康和福祉。本文著重於大型語言模型 (LLM)，視其為 CREST 架構內所選定的 AI 系統。LLM 由於能夠處理廣泛的自然語言處理 (NLP) 情境，因此獲得研究人員的極大關注。例如，ChatGPT 和 Google 的 MedPaLM 已成為極具前景的平台，分別用於提供一般資訊和健康相關的查詢。儘管如此，這些模型仍然是黑盒子，儘管納入了人類回饋和指導式調整。例如，ChatGPT 能夠產生不安全的回應，儘管已建立安全防護措施。CREST 提出一個可行的途徑，利用神經符號架構中的程序和圖形知識，來闡明與 LLM 相關的挑戰。
...
摘要：本研究調查了已部署人工智慧 (AI) 模型在預測 COVID-19 大流行期間及之後的死亡率方面的效能、可解釋性和穩健性。作為這類型的首項研究，我們發現貝氏神經網路 (BNN) 和智慧型訓練技術讓我們的模型在資料大幅變動的情況下仍能維持效能。我們的結果強調開發穩健 AI 模型的重要性，這些模型即使在具挑戰性的條件下，也能與臨床醫師的預測相符或超越。我們對模型可解釋性的探討顯示，隨機模型會產生更多元化且個人化的解釋，因此突顯出 AI 模型在現實世界的臨床環境中提供詳細且個人化見解的必要性。此外，我們強調量化 AI 模型中不確定性的重要性，這能讓臨床醫師根據可靠的預測做出更明智的決策。我們的研究主張在 AI 醫療保健研究中優先實施科學，並確保 AI 解決方案在現實世界的臨床環境中實用、有益且永續。透過解決醫療保健環境中獨特的挑戰和複雜性，研究人員可以開發出有效改善臨床實務和患者預後的 AI 模型。
...
摘要：肺癌占英國癌症死亡人數的 21%，五年存活率很大程度取決於發現癌症時所處的階段。最近的研究證明了 AI 方法對肺癌常規掃描進行準確早期診斷的能力。然而，這一證據並未轉化為臨床實踐，其中一個障礙是缺乏可解釋的模型。本研究探討了變分自編碼器 (VAE) 的應用，這是一種生成式 AI 模型，用於肺癌病灶。所提出的模型在從 LIDC-IDRI 公共數據集中提取的 3D CT 掃描病灶上進行訓練。通過聚類探索了 VAE 產生的 2D 切片的潛在向量表示，以證明其質量，並用於肺癌診斷的 MLP 分類器模型，最好的模型達到了 AUC 0.98 和 93.1% 準確率的最新指標。聚類分析表明，VAE 潛在空間根據有意義的特徵成分（包括腫瘤大小、形狀、患者和惡性類別）將惡性和良性病灶的數據集分開。我們還對標準高斯 VAE (GVAE) 和更新的狄利克雷 VAE (DirVAE) 進行了比較分析，後者用狄利克雷分佈替換了先驗，以促進具有解糾特徵表示的更具可解釋性的潛在空間。最後，我們展示了與臨床有意義的特徵變化相應的潛在空間遍歷的潛力。
...
摘要：現有的圖像分類器輸出解釋工具可分為白盒和黑盒，白盒依賴於模型內部的存取權，而黑盒則與模型無關。隨著 AI 在醫療領域的使用增加，可解釋性工具的使用也隨之增加。現有關於醫學影像解釋的研究重點在於白盒工具，例如 gradcam。然而，切換到黑盒工具有明顯的優點，包括能夠將其與任何分類器一起使用，以及可用的黑盒工具種類繁多。在標準影像上，黑盒工具與白盒一樣精確。在本文中，我們比較了多種黑盒方法在腦癌 MRI 資料集上與 gradcam 的效能。我們證明了大多數黑盒工具不適合解釋醫學影像分類，並詳細分析了其缺點的原因。我們還展示了一個黑盒工具，一個基於因果可解釋性的 rex，其效能與 \gradcam 一樣好。
...
摘要：人工智慧發展社群日益利用 Hugging Face 等託管中介，提供使用者上傳的模型和訓練資料，便於存取。這些模型市集降低了數十萬使用者的技術部署門檻，但可能會被用於許多潛在有害和非法的方式。在本文中，我們說明人工智慧系統既可以「包含」內容，又可以成為開放式工具，因此提出了迄今為止最棘手的平台治理挑戰之一。我們提供 Hugging Face、GitHub 和 Civitai 等三個說明性平台上數起事件的案例研究，以探討模型市集如何調整模型。根據此分析，我們概述了產業為回應調整需求而發展的重要（但仍然有限）實務：授權、存取和使用限制、自動內容調整和開放式政策發展。儘管目前面臨的政策挑戰相當大，我們仍提出一些想法，說明平台如何能更好地動員資源，作為謹慎、公平且相稱的法規存取點。
...
摘要：<paragraph>背景與目標：透過萃取這些資訊，機器或深度學習 (ML/DL) 為基礎的自主資料分析工具可以協助臨床醫師和癌症研究人員從複雜的資料集中發現模式和關係。最近已發表許多基於 DL 的卵巢癌 (OC) 資料分析。這些分析在癌症的各個方面（例如，它們探討的子領域和癌症類型）和資料分析功能方面差異很大。然而，目前對於這些分析在這些功能和 AI 保證 (AIA) 方面的全面理解仍然不足。這篇系統性回顧旨在透過檢視現有文獻並找出使用 DL 進行 OC 資料分析的重要面向來填補此一缺口，特別著重於關鍵功能和 AI 保證觀點。方法：使用 PRISMA 架構在三個期刊資料庫中進行全面搜尋。分析僅納入 2015 年至 2023 年間發表於同行評審期刊的研究。結果：在回顧中，總共檢視了 96 項由 DL 驅動的分析。研究結果揭露了幾個關於由 DL 驅動的卵巢癌資料分析的重要見解：- 大多數研究 71%（96 項中的 68 項）專注於偵測和診斷，而沒有研究探討 OC 的預測和預防。- 這些分析主要基於來自非多元族群的樣本（75%（96 項研究中的 72 項）），僅限於某個地理位置或國家。- 只有少部分研究（僅 33%（96 項中的 32 項））執行整合分析，其中大多數使用均質資料（臨床或組學）。- 值得注意的是，僅有 8.3%（96 項中的 8 項）的研究使用外部和多元資料集驗證其模型，突顯出加強模型驗證的必要性，- 將 AIA 納入癌症資料分析仍處於非常早期的階段；只有 2.1%（96 項中的 2 項）透過可解釋性明確探討 AIA。</paragraph>
...
摘要：<paragraph>可解釋性是深度學習中長久以來的挑戰，特別是在醫療保健等高風險領域。常見的可解釋性方法會強調驅動 AI 模型決策的影像區域。然而，人類在傳達解釋時，高度依賴語言，不僅是「在哪裡」，還有「是什麼」。此外，大多數的可解釋性方法專注於解釋個別的 AI 預測，而非描述 AI 模型在一般情況下使用的特徵。後者對於模型和資料集稽核特別有用，甚至在 AI 愈來愈多用於新穎任務時，還能產生知識。在此，我們提出一個可解釋性策略，使用視覺語言模型來識別視覺分類任務的語言描述符。透過利用影像和文字之間預先訓練的聯合嵌入空間，我們的做法將新的分類任務估計為線性文字組合，結果會為每個文字產生一個權重，以表示其與基於視覺的分類器對齊。我們使用兩個醫學影像分類任務來評估我們的做法，結果發現，儘管缺乏特定領域的語言訓練，但產生的描述符在很大程度上與臨床知識一致。然而，我們的做法也發現了在所使用的公開資料集中存在「捷徑連線」的可能性。為了達到可解釋性的功能性衡量，我們執行了一項試驗性讀者研究，結果發現，AI 識別的文字能讓非專家人類在非平凡的層級上執行專業的醫療任務。總的來說，我們的結果強調了使用多模態基礎模型來提供直觀的、基於語言的視覺任務解釋的潛力。</paragraph>
...
摘要：<paragraph>使用醫療影像訓練的用於臨床任務的人工智慧 (AI) 模型，常會以效能上的差異形式表現出偏見，在各個子群體間。由於並非所有實際醫療影像資料中的偏見來源都容易辨識，因此要全面評估這些偏見如何編碼在模型中，以及偏見緩解方法在改善效能差異上的能力，是一項挑戰。在本文中，我們介紹一個新穎的分析架構，用於系統且客觀地調查醫療影像中的偏見對 AI 模型的影響。我們開發並測試了這個架構，用於進行受控的電腦模擬試驗，以評估醫療影像 AI 中的偏見，使用一個工具來產生具有已知疾病影響和偏見來源的合成磁共振影像。透過使用三個反事實偏見情境來衡量模擬偏見效應對卷積神經網路 (CNN) 分類器的影響，以及三個偏見緩解策略的效能，展示了可行性。分析顯示，當 CNN 在合成資料集上訓練時，模擬偏見會導致預期的子群體效能差異。此外，重新加權被認為是此設定中最成功的偏見緩解策略，我們展示了解釋性 AI 方法如何協助調查使用此架構的模型中偏見的表現。開發公平的 AI 模型是一項重大的挑戰，因為在醫療影像資料集中可能存在許多且經常未知的偏見來源。在這項工作中，我們提出一個新穎的方法，用於客觀地研究偏見和緩解策略對深度學習管線的影響，這可以支援開發強健且負責任的臨床 AI。</paragraph>
...
摘要：機器學習為自動預測中風後症狀及其對復健的反應提供了巨大的潛力。這項工作的重大挑戰包括神經影像資料的維度非常高、可供學習的資料集規模相對較小，以及如何有效結合神經影像和表格資料（例如人口統計資訊和臨床特徵）。本文根據兩種策略評估了多種解決方案。第一個是使用總結 MRI 掃描的 2D 影像。第二個是選擇可提升分類精確度的主要特徵。此外，我們引入了在結合從 MRI 中萃取的感興趣區域和表格資料的符號表徵的影像上訓練卷積神經網路 (CNN) 的新方法。我們評估了一系列 CNN 架構（2D 和 3D 皆有），這些架構經過訓練，可辨識 MRI 和表格資料的不同表徵，以預測中風後口語圖片描述能力的綜合測量值是否在失語或非失語的範圍內。MRI 和表格資料是從 758 名參與 PLORAS 研究的英語系中風倖存者取得的。僅針對病灶大小的基線邏輯迴歸的分類精確度為 0.678，在連續加入初始症狀嚴重程度和恢復時間後，上升至 0.757 和 0.813。在從每個 MRI 掃描中萃取 8 個感興趣區域，並在 2D 殘差神經網路中與病灶大小、初始嚴重程度和恢復時間結合時，觀察到最高的分類精確度 0.854。我們的研究結果證明了即使在機器學習術語中資料集很小的情況下，如何結合影像和表格資料以獲得高精確度的中風後分類。我們最後提出如何改善現有模型，以使用來自醫院掃描儀的影像達到更高的精確度。
...
摘要：可解釋人工智慧 (XAI) 已成為處理任務關鍵應用程式時的一項基本需求，確保所採用黑箱 AI 模型的透明度和可解釋性。XAI 的重要性跨越各種領域，從醫療保健到金融，其中了解深度學習演算法的決策過程至關重要。大多數基於 AI 的電腦視覺模型通常是黑箱；因此，提供影像處理中深度神經網路的可解釋性對於它們在醫學影像分析、自動駕駛和遙測應用中的廣泛採用和部署至關重要。最近，已推出幾種針對影像分類任務的 XAI 方法。相反地，影像分割在可解釋性的背景下受到的關注相對較少，儘管它是電腦視覺應用中的一項基本任務，特別是在遙測中。只有少數研究提出基於梯度的 XAI 演算法用於影像分割。本文採用最近的無梯度 Sobol XAI 方法進行語意分割。為了衡量 Sobol 方法在分割中的效能，我們提出了一種基於可學習雜訊模型的量化 XAI 評估方法。此模型的主要目的是在解釋圖上產生雜訊，其中較高的誘導雜訊表示較低的準確度，反之亦然。進行基準分析以評估和比較三種 XAI 方法的效能，包括 Seg-Grad-CAM、Seg-Grad-CAM++ 和 Seg-Sobol，使用提出的基於雜訊的評估技術。這是使用高解析度衛星影像執行和評估 XAI 方法的首次嘗試。
...
摘要：大型語言模型在多個領域中迅速發展。然而，由於事實性、連貫性和幻覺等問題，醫療和保健領域對其採用猶豫不決。由於醫療保健性質的風險很高，許多研究人員甚至警告不要使用它，直到這些問題得到解決。在醫療保健中實施和部署 LLM 的關鍵是使這些模型值得信賴、透明（儘可能）且可解釋。在本文中，我們描述了建立可靠、值得信賴且無偏見模型的關鍵要素，作為它們在醫療保健中採用的必要條件。具體來說，我們專注於量化、驗證和減輕醫療保健背景下的幻覺。最後，我們討論了 LLM 在醫療保健中的未來可能是什麼樣子。
...
摘要：人工智能（AI）發展迅速，目前已準備部署於各種應用程式中，例如自主系統、醫療診斷和自然語言處理。早期採用 AI 技術於實際應用程式並非沒有問題，特別是對神經網路而言，其可能不穩定且容易受到對抗範例影響。長期而言，需要開發適當的安全保證技術，以減少因可避免的系統故障而造成的潛在危害，並確保可信度。本文專注於認證和可解釋性，概述了已開發出來以確保 AI 決策安全性的技術，並討論未來的挑戰。
...
摘要：歐洲議會議會研究服務總局已為歐洲議會議員準備了一份報告，其中列舉了人工智能 (AI) 在醫學和醫療保健中的七項主要風險：由於 AI 錯誤而造成的患者傷害、醫療 AI 工具的誤用、AI 中的偏見和現有不平等的延續、缺乏透明度、隱私和安全問題、問責制中的差距以及實施中的障礙。
  在本研究中，我們提出了 14 項功能性要求，AI 系統可以實施這些要求來降低與其醫療目的相關的風險：AI 護照、使用者管理、法規檢查、僅限學術用途免責聲明、資料品質評估、臨床醫生雙重檢查、持續效能評估、稽核追蹤、持續可用性測試、回顧性/模擬案例審查、偏見檢查、可解釋的 AI、加密和使用經過現場測試的程式庫以及語義互操作性。
  我們在此的目的是提供技術解決方案的具體高階規格，以確保持續良好的效能並使用 AI 系統，以符合未來歐盟法規架構，造福患者。
...
摘要：人工智能技術可用於分類病患的生理活動，並預測遠距病患監測的重要生命徵象。基於非線性模型（如深度學習模型）的回歸分析由於其黑箱特性而導致可解釋性有限。這可能需要決策者根據非線性模型結果盲目做出信仰上的飛躍，特別是在醫療保健應用中。在非侵入式監測中，來自追蹤感測器及其誘發性臨床屬性的病患資料作為預測未來生命徵象的輸入特徵。解釋各種特徵對監控應用程式整體輸出的貢獻對於臨床醫生的決策至關重要。在本研究中，提出了一個用於量化分析的可解釋 AI（QXAI）架構，其中包含事後模型可解釋性和內在可解釋性，用於監督式學習方法中的回歸和分類任務。這是透過利用 Shapley 值概念並將注意力機制納入深度學習模型中來實現的。我們採用人工神經網路 (ANN) 和基於注意力的雙向 LSTM (BiLSTM) 模型，根據感測器資料預測心率和對生理活動進行分類。深度學習模型在預測和分類任務中都取得了最先進的成果。對輸入資料進行了整體解釋和局部解釋，以了解各種病患資料的特徵貢獻。使用 PPG-DaLiA 資料評估所提出的 QXAI 架構，以預測心率，並使用行動健康 (MHEALTH) 資料根據感測器資料對生理活動進行分類。蒙地卡羅近似法應用於該架構，以克服 Shapley 值計算所需的時間複雜度和高運算能力需求。
...
摘要：在可解釋的人工智慧 (XAI) 研究中，主要的重點
一直放在解釋專家和從業人員的模型上。模型不可知論
和局部解釋方法被認為在
許多應用中是可解釋且足夠的。然而，在醫療保健等領域，最終用戶是
沒有 AI 或領域專業知識的患者，迫切需要模型
解釋更易於理解並建立對模型的信任
操作。我們假設生成模型解釋是
敘述性的、特定於患者的和全局的（模型的整體）將使
更好地理解並能夠做出決策。我們使用
決策樹模型來測試這一點，為被識別為患有高風險冠狀動脈疾病的患者生成局部和全局解釋。這些解釋
呈現給非專家用戶。我們發現對特定類型解釋有強烈的個人偏好。大多數參與者更喜歡全局
解釋，而較小的一組則更喜歡局部解釋。基於任務
對這些參與者的心理模型的評估提供了有價值的回饋，以
增強敘述性的全局解釋。這反過來又指導了
既值得信賴又可操作的健康信息系統的設計。
...
摘要：<paragraph>可解釋人工智慧在醫療影像中至關重要。在神經科學的挑戰領域中，視覺主題呈現高度的複雜性，特別是在三維空間中。神經科學的應用涉及從核磁共振成像中識別腦溝特徵，由於專家之間不同的註解協定和腦部複雜的三維功能，因此面臨重大障礙。因此，傳統的可解釋性方法無法有效驗證和評估這些網路。為了解決這個問題，我們首先提出一個數學公式，描繪出各種電腦視覺任務中不同類別的解釋需求，根據驗證協定的可靠性，將其分類為自解釋、半解釋、非解釋和新模式學習應用。關於這個數學公式，我們提出一個三維可解釋性架構，旨在驗證深度學習網路在偵測腦回溝中的輸出，腦回溝是重要的腦部解剖特徵。這個架構整合了局部三維解釋、透過降維的全局解釋、串接的全局解釋和統計形狀特徵，揭示了模式學習的新見解。我們在具有挑戰性的 TOP-OSLO 資料集上訓練和測試了兩個先進的三維深度學習網路，顯著提高了腦溝偵測的準確性，特別是在左半球。在使用此資料集的不同註解協定進行評估時，我們強調了無偏註解程序在我們提出的三維架構中實現精確預測和有效模式學習的關鍵作用。所提出的架構不僅註解了可變腦溝，還揭示了隱藏的人工智慧知識，有望增進我們對腦部解剖和功能的理解。</paragraph>
...
摘要：電子健康紀錄 (EHR) 和例行文件記錄實務在病患的日常照護中扮演至關重要的角色，提供全面的健康、診斷和治療紀錄。然而，複雜且冗長的 EHR 敘述會讓醫療保健提供者不堪負荷，進而導致診斷不準確。儘管大型語言模型 (LLM) 在各種語言任務中展現其潛力，其在醫療保健領域的應用需要確保將診斷錯誤降至最低，並防止病患受到傷害。在本文中，我們概述一種創新的方法，用於增強 LLM 在自動化診斷生成領域的熟練度，透過整合醫學知識圖譜 (KG) 和新穎的圖譜模型：Dr.Knows，靈感來自臨床診斷推理程序。我們從美國國家醫學圖書館的統一醫學語言系統 (UMLS) 中衍生出 KG，這是一個強大的生物醫學知識儲存庫。我們的方法否定了預先訓練的必要性，而是將 KG 作為輔助工具，協助詮釋和總結複雜的醫療概念。使用真實世界的醫院資料集，我們的實驗結果證明，將 LLM 與 KG 結合的提議方法有潛力改善自動化診斷生成的準確性。更重要的是，我們的方法提供可解釋的診斷途徑，讓我們更接近實現 AI 增強診斷決策支援系統。
...
摘要：現有的用於診斷膝骨關節炎 (OA) 的人工智慧 (AI) 模型因缺乏透明度和可解釋性而受到批評，儘管它們達到了醫療專家級的表現。這種不透明性讓它們在臨床實務中難以被信任。最近，可解釋人工智慧 (XAI) 已成為一種專業技術，它能透過揭露預測的推導方式來提供對模型預測的信心，從而促進 AI 系統在醫療保健中的使用。本文提供了用於膝骨關節炎診斷的 XAI 技術的首次調查。XAI 技術從兩個角度進行討論：資料可解釋性和模型可解釋性。本文的目的是提供有價值的見解，了解 XAI 在更可靠的膝骨關節炎診斷方法中的潛力，並鼓勵在臨床實務中採用它。
...
摘要：近來在醫療保健領域中人工智能應用上的進展，已在診斷和疾病預後上展現出超越人類表現的驚人潛力。然而，隨著人工智能模型的日益複雜，人們開始關注其不透明性、潛在的偏見以及對可解釋性的需求。為了確保人工智能系統的信賴度和可靠性，特別是在臨床風險預測模型中，可解釋性變得至關重要。可解釋性通常是指人工智能系統提供其決策邏輯或決策本身的穩健解釋，以供人類利益相關者理解的能力。在臨床風險預測中，可解釋性的其他方面，例如公平性、偏見、信任和透明度，也代表了超越可解釋性的重要概念。在本篇評論中，我們探討了這些概念之間的關係，因為它們經常被一起使用或交替使用。本篇評論還討論了開發用於臨床風險預測的可解釋模型的最新進展，強調了在臨床實務中跨多種常見模式進行量化和臨床評估和驗證的重要性。它強調了外部驗證和結合多樣的可解釋性方法以增強信任和公平性的必要性。採用嚴謹的測試，例如使用具有已知生成因子的合成數據集，可以進一步提高可解釋性方法的可靠性。開放獲取和代碼共享資源對於透明度和可複製性至關重要，有助於可解釋性研究的成長和可信度。儘管存在挑戰，但從臨床醫生到開發人員，採用端到端的方法來實現臨床風險預測的可解釋性，對於成功至關重要。
...
摘要：儘管醫療和保健方面的人工智慧 (AI) 有長足進展，但 AI 技術的部署和採用在實際臨床實務中仍然有限。近年來，人們對與醫療 AI 相關的技術、臨床、倫理和法律風險提出了疑慮。為了增加實際採用率，醫療 AI 工具必須獲得患者、臨床醫生、醫療機構和主管機關的信賴和接受。本研究描述了 FUTURE-AI 指南，這是第一個國際共識架構，用於指導在醫療保健中開發和部署可信賴的 AI 工具。FUTURE-AI 聯盟成立於 2021 年，目前由來自 51 個國家的 118 位跨領域專家組成，代表所有洲，包括 AI 科學家、臨床醫生、倫理學家和社會科學家。在兩年的時間裡，該聯盟通過一個反覆運算的過程定義了可信賴 AI 的指導原則和最佳實務，包括深入的文獻回顧、修改後的德爾菲調查和線上共識會議。FUTURE-AI 架構基於醫療保健中可信賴 AI 的 6 項指導原則建立，即公平性、普遍性、可追溯性、可用性、穩健性和可解釋性。通過共識，定義了一組 28 項最佳實務，涵蓋技術、臨床、法律和社會倫理層面。這些建議涵蓋了醫療 AI 的整個生命週期，從設計、開發和驗證到法規、部署和監控。FUTURE-AI 是一個以風險為依據、不預設假設的指南，它提供了一個結構化的方法來建構醫療 AI 工具，這些工具將在實際實務中獲得信賴、部署和採用。鼓勵研究人員在概念驗證階段考慮這些建議，以促進未來醫療 AI 向臨床實務的轉化。
...
摘要：醫療中的人工智慧在醫療影像、病人照護和其他領域的新興應用中取得顯著進展。雖然這些應用已在回顧性研究中證明成功，但實際應用的卻很少。醫療 AI 領域面臨各種挑戰，包括建立使用者信任、遵守法規、合乎道德地使用資料。可解釋 AI (XAI) 旨在讓人們了解 AI 並信任其結果。本文針對醫療決策支援的 XAI 解決方案的最新發展進行文獻回顧，根據近年發表的 198 篇文章的代表性樣本。相關文章的系統性綜合得出以下幾項發現：(1) 這些解決方案大多採用與模型無關的 XAI 技術，(2) 深度學習模型的使用多於其他類型的機器學習模型，(3) 可解釋性用於促進信任，但很少有研究報告醫師參與迴路，(4) 視覺和互動式使用者介面在理解系統的解釋和建議方面更有用。需要醫療和 AI 專家之間進一步合作研究，以指導設計、實作和評估醫學中 XAI 解決方案的合適架構。
...
摘要：體外受精是治療不孕最廣泛的方法之一。其中一個主要挑戰是評估和選擇胚胎進行植入，這是一個臨床醫生之間和臨床醫生內部差異很大的過程。基於深度學習的方法正受到關注，但其不透明的性質會影響它們在臨床環境中的接受度，而透明度在決策制定中至關重要。在本文中，我們分析了 AI 輔助胚胎分析模型的可解釋性方面的現有工作，並找出其限制。我們還討論了如何將這些模型作為決策支持系統整合到臨床環境中，同時考慮臨床醫生和患者的需求。最後，我們提出了提高可解釋性和可信度的指南，推動這項技術走向既定的臨床實務。
...
摘要：在需求工程 (RE) 领域，可解释人工智能 (XAI) 在将人工智能支持的系统与用户需求、社会期望和监管标准相一致方面的重要性日益凸显，并获得了认可。一般来说，可解释性已成为影响系统质量的重要非功能性需求。但是，可解释性和性能之间的权衡挑战了可解释性的假定积极影响。如果满足可解释性的要求需要降低系统性能，那么必须仔细考虑这些质量方面中哪一个优先，以及如何在它们之间进行权衡。在本文中，我们批判性地研究了所谓的权衡。我们认为，最好以一种微妙的方式来处理它，这种方式结合了资源可用性、领域特征和风险考虑。通过为未来的研究和最佳实践提供基础，这项工作旨在推进人工智能的 RE 领域。
...
摘要：在需求工程（RE）領域中，可解釋人工智慧（XAI）在將 AI 支援系統與使用者需求、社會期望和法規標準相符時的重要性日益提升，並已獲得認可。一般來說，可解釋性已成為影響系統品質的重要非功能性需求。然而，可解釋性和效能之間的假設取捨挑戰了可解釋性的假設正面影響。如果滿足可解釋性的需求會導致系統效能降低，則必須仔細考量這些品質面向哪一個優先，以及如何在它們之間取得折衷。在本文中，我們批判性地檢視了假設的取捨。我們主張，以一種微妙的方式來探討它會是最好的，這種方式結合了資源可用性、領域特性和風險考量。透過提供未來研究和最佳實務的基礎，這項工作旨在推動 AI 的 RE 領域。
...
摘要：本文嚴格評估了歐盟委員會提出的 AI 法案，其風險管理和風險可接受性的方法，針對對基本權利和安全構成風險的高風險 AI 系統。該法案旨在以適度的監管負擔促進「值得信賴」的 AI。其關於風險可接受性的條款要求盡可能減少或消除高風險系統的殘餘風險，並考慮到「最新技術」。這個標準，特別是如果狹義解釋，是不可行的，既不能促進適度的監管負擔，也不能促進可信賴性。相反，議會最近對風險管理條款的修正案草案引入了「合理性」、成本效益分析，並更透明地說明了風險可接受性判斷的價值觀和情境性質。本文主張議會的方法更可行，且能更好地平衡相稱性和可信賴性的目標。本文說明風險可接受性判斷中的合理性會是什麼，並根據過失法和歐洲醫療器材法規的原則進行說明。它主張風險可接受性判斷的方法需要穩固的公民合法性基礎：包括監管機構的詳細指導或參與，以及受影響利益相關者的有意義投入。
...
摘要：可解釋人工智慧 (XAI) 是機器學習中快速發展的領域，旨在解開複雜模型的預測。XAI 特別需要在敏感應用中，例如在醫療保健中，當診斷、建議和治療選擇可能依賴於人工智慧系統做出的決策時。AI 方法也已廣泛用於老化研究中，特別是在開發生物時鐘模型和識別老化和與年齡相關疾病的生物標記方面。然而，XAI 在此處的潛力尚待充分認識。我們討論了 XAI 在開發「老化時鐘」中的應用，並對按特定生理系統重點分類的文獻進行了全面分析。
...
摘要：部分原型模型是可解釋設計的影像分類器，也是黑箱 AI 的有希望替代方案。本文探討可解釋機器學習，特別是 PIP-Net，在實際醫療影像資料上自動診斷支援的適用性和潛力。PIP-Net 學習人類可理解的原型影像部分，我們評估其準確性和可解釋性，用於骨折偵測和皮膚癌診斷。我們發現 PIP-Net 的決策制定過程符合醫療分類標準，同時僅提供影像級別的類別標籤。由於 PIP-Net 的原型無監督預訓練，因此資料品質問題（例如 X 光中的不需要文字或標籤錯誤）可以輕易辨識。此外，我們率先證明人類可以透過直接停用不需要的原型，手動修正 PIP-Net 的推理。我們得出結論，部分原型模型由於其可解釋性和進階模型除錯的潛力，對於醫療應用很有希望。
...
摘要：可解釋 AI (XAI) 是機器學習研究中日益重要的領域，其目標是使黑盒模型透明且可解釋。在本文中，我們提出了一種使用條件特徵置換所產生的反事實路徑的新穎 XAI 方法。該演算法透過識別影響模型預測變化的特徵順序置換來衡量特徵重要性。它特別適合根據包含領域知識的知識圖譜中的反事實路徑產生解釋。反事實路徑為目前的 XAI 方法在解釋和視覺化黑盒模型時引入了額外的圖形維度。使用合成和醫療資料的實驗證明了我們方法的實際應用性。
...
摘要：人工智能 (AI) 中的可解釋性領域見證了越來越多的研究和越來越高的學術興趣。然而，在解釋機器學習算法的結果時，缺乏對人類友好的個人化解釋，顯著阻礙了臨床醫生在研究和臨床實務中接受這些方法。為了解決這個問題，我們的研究使用反事實解釋來探討「如果？」情境在醫學研究中的適用性。我們的目標是擴展我們對用於診斷小兒後顱窩腦腫瘤的磁共振成像 (MRI) 特徵的理解，超越現有的界限。在我們的案例研究中，所提出的概念提供了一種新的方式來檢驗提供個人化和特定於情境的見解的替代決策制定情境，從而驗證預測並釐清在不同情況下的變化。此外，我們探討了反事實用於資料擴充的潛在用途，並評估它們作為我們醫學研究案例中替代方法的可行性。結果證明了使用反事實解釋來提高臨床研究中 AI 驅動方法的接受度的潛力。
...
摘要：目前人工智能領域的進展已導致開發出各種由 AI 驅動的失智症評估，可用於識別失智症的早期患者。它可以徹底改變失智症照護設定。醫學界必須了解各種 AI 評估，並根據其對失智症患者 (PwD) 的早期識別的有效性、效率、實用性、可靠性和準確性來選擇它們。另一方面，AI 開發人員也應了解各種非 AI 評估以及最近開發的 AI 評估。因此，這篇臨床醫師和 AI 工程師都可以閱讀的論文填補了文獻中在向臨床醫師解釋現有失智症識別解決方案以及向 AI 工程師解釋所使用的技術和最廣泛的失智症資料集方面的空白。它遵循對 AI 和非 AI 失智症評估論文的回顧，以提供有關各種失智症評估的寶貴資訊，供 AI 和醫學界使用。討論和結論重點介紹了最突出的研究方向和現有解決方案的成熟度。
...
摘要：<paragraph>可解釋性對人工智慧 (AI) 技術構成一項重大挑戰。目前關於可解釋 AI (XAI) 的研究缺乏萃取學習任務整體知識的效率，因此存在著諸如不精確顯著性、與脈絡無關的缺失和含糊意義等缺陷。在本文中，我們提出類別關聯嵌入 (CAE) 方法來解決這些問題。我們採用編碼器-解碼器架構來嵌入樣本特徵，並將其同時分為與類別相關和與個體相關的樣式向量。將給定樣本的個體樣式代碼與另一個樣本的類別樣式代碼重新組合，會產生一個合成樣本，該樣本保留了個體特徵，但改變了類別指派，遵循循環對抗學習策略。類別關聯嵌入將所有實例的整體類別相關特徵提煉到一個統一的領域中，並在類別之間有良好的區分。然後可以萃取出不同類別之間的轉換規則，並進一步應用於個別實例。然後，我們提出一個主動 XAI 框架，該框架沿著導引路徑操縱特定樣本的類別樣式向量朝向對立類別，從而產生一系列具有相同個體特徵的對立範例合成樣本。將這些反事實樣本與原始樣本進行比較，可以對分類任務的性質提供一個整體、直觀的說明。我們採用該框架進行醫學影像分類任務，結果表明，與現有方法相比，可以獲得更精確的顯著性圖，並具有強大的與脈絡相關的表示。此外，疾病病理學可以直接通過在類別樣式空間中遍歷路徑來視覺化。</paragraph>
...
摘要：提供基於機器學習的 AI 預測的高品質解釋是一項具有挑戰性且複雜的任務。要發揮良好的作用，除了其他因素外，還需要：選擇適當的解釋概括性/特異性層級；考慮解釋受益人對所考慮的 AI 任務的熟悉程度的假設；參照有助於做出決定的具體元素；利用額外的知識（例如專家證據），這些知識可能不是預測過程的一部分；並提供支持否定假設的證據。最後，系統需要以清晰可解釋且可能令人信服的方式制定解釋。基於這些考量，ANTIDOTE 促進了可解釋 AI 的整合願景，其中深度學習過程的低階特徵與人類論證能力的高階方案相結合。ANTIDOTE 將利用深度學習和論證的跨領域能力，來支持對可解釋 AI 更廣泛且創新的觀點，其中對於臨床案例審議的高品質解釋需求至關重要。作為該專案的第一個成果，我們發布了 Antidote CasiMedicos 資料集，以促進對一般可解釋 AI 以及特別是醫療領域中的論證的研究。
...
摘要：近年来，图神经网络的进展迅速，在药物发现、医疗诊断和推荐系统中出现了许多新发展。虽然这一进展意义重大，但许多网络都是“黑匣子”，对网络究竟在学习什么了解甚少。许多高风险应用（如药物发现）需要模型提供人类可理解的解释，以便用户能够识别错误并发现新知识。因此，可解释人工智能算法的开发对于我们充分利用人工智能的好处至关重要。
我们提出了一种用于 GNN 的可解释性算法，称为 eXplainable Insight (XInsight)，它使用 GFlowNets 生成模型解释的分布。由于 GFlowNets 根据奖励生成概率对象，因此与仅学习最大奖励样本的先前方法相比，XInsight 可以生成多种解释。我们通过生成针对两个图分类任务训练的 GNN 的解释来演示 XInsight：使用 MUTAG 数据集对诱变化合物进行分类，并使用我们开源的合成数据集对无环图进行分类。我们通过使用 QSAR 建模分析生成的化合物来展示 XInsight 解释的效用，我们发现 XInsight 生成的化合物按亲脂性聚类，亲脂性是诱变性的已知相关因素。我们的结果表明，XInsight 生成了一个解释分布，揭示了模型所展示的潜在关系。它们还强调了生成多种解释的重要性，因为它使我们能够发现模型中的隐藏关系并为进一步分析提供有价值的指导。
...
摘要：我們提出並實作一個可解釋機器學習分類模型，用於基於表達式布林公式的可解釋 AI (XAI)。潛在應用包括信用評分和醫療狀況診斷。布林公式定義了一個具有可調整複雜性（或可解釋性）的規則，根據此規則對輸入資料進行分類。此類公式可以包含任何可套用於一個或多個布林變數的運算子，因此與更嚴格的基於規則和基於樹狀的方法相比，提供了更高的表達力。分類器是使用原生局部最佳化技術訓練的，有效率地搜尋可行公式的空間。淺層規則可以由快速整數線性規劃 (ILP) 或二次無約束二元最佳化 (QUBO) 求解器確定，這些求解器可能由特殊用途硬體或量子裝置提供支援。我們透過執行非局部移動來結合原生局部最佳化器的表達力和效率，這些移動會最佳化完整布林公式的子樹。我們提供廣泛的數值基準測試結果，這些結果包含在眾所周知的公開資料集上的幾個基線。根據結果，我們發現原生局部規則分類器通常與其他分類器具有競爭力。加入非局部移動會以較少的反覆運算達成類似的結果，因此使用專用或量子硬體可能會透過快速提出非局部移動來加速。
...
摘要：為了解決全球心理健康問題的挑戰，我們提出一個基於邏輯神經網路 (LNN) 的神經符號 AI 方法來診斷心理疾病。由於缺乏有效的心理疾病治療覆蓋範圍，因此需要一種 AI 解決方案來協助治療師進行診斷。然而，目前的類神經網路模型缺乏可解釋性，治療師可能無法信賴。LNN 是一種遞迴神經網路架構，結合了神經網路的學習能力和基於經典邏輯的 AI 的推理能力。所提出的系統使用臨床訪談中的輸入謂詞來輸出心理疾病類別，並使用不同的謂詞剪枝技術來實現可擴充性和更高的分數。此外，我們提供了一個見解提取方法來協助治療師進行診斷。所提出的系統解決了當前類神經網路模型缺乏可解釋性的問題，並為心理疾病診斷提供了一個更值得信賴的解決方案。
...
摘要：隨著機器學習模型在醫療診斷中日益普及，對可解釋性和透明度的需求變得至關重要。XAI 復興標誌著該領域的重大轉變，旨在重新定義醫療診斷模型的可解釋性。本文探討了可解釋 AI (XAI) 領域內的創新方法和方法論，這些方法和方法論正在革新醫療診斷模型的可解釋性。通過闡明基礎決策制定過程，XAI 技術使醫療保健專業人員能夠理解、信任和有效地利用這些模型進行準確可靠的醫療診斷。本綜述重點介紹了 XAI 在醫療診斷方面的關鍵進展及其轉變醫療保健領域的潛力，最終改善患者的治療效果並增強對 AI 驅動的診斷系統的信任。
...
摘要：在以高度连通性和流动性为特征的环境中，再加上心血管疾病的激增，通过远程监测心血管健康来减少医疗保健支出的必要性变得更加明显。准确检测和分类心律失常对于诊断患有心脏不规则的人至关重要。本研究强调了在家中使用心电图 (ECG) 测量进行实时心律失常检测的可行性。本文提出了一种新的心律失常检测应用，利用最先进的 You-Only-Look-Once (YOLO)v8 算法对单导联 ECG 信号进行分类。我们引入了一个新颖的损失修改 YOLOv8 模型，在 MIT-BIH 心律失常数据集上进行了微调，从而实现了实时连续监测。获得的结果证实了我们方法的有效性，该模型在 NVIDIA Tesla V100 上实现了 99.5% 的平均准确率和 0.992 mAP@50，以及 0.002 秒的快速检测时间。我们的调查例证了实时心律失常检测的潜力，使用户能够在家中舒适地直观地解释模型输出。此外，本研究为扩展到能够在医疗保健领域部署的实时可解释 AI (XAI) 模型奠定了基础，从而极大地推进了医疗保健解决方案的领域。
...
摘要：乳癌（BC）仍然是一個重大的健康威脅，目前沒有長期的治癒方法。早期發現至關重要，但乳房攝影的判讀受到高假陽性和假陰性的阻礙。隨著 BC 的發生率預計將超過肺癌，改善早期檢測方法至關重要。熱像攝影使用高解析度紅外線相機，很有前景，特別是在與人工智慧（AI）結合時。這項工作提出了一個基於注意力的卷積神經網路進行分割，在 BC 檢測和分類中提供了更高的速度和精確度。該系統增強影像並執行可解釋的 AI 癌症分割。我們提出了一個基於Transformer注意力的卷積架構（UNet）進行故障識別，並採用梯度加權類激活映射（Grad-CAM）來分析 UNet 架構中 IRT 影像的偏差和弱點區域。與現有的深度學習框架相比，我們提出的框架的優越性得到了證實。
...
摘要：憂鬱症是最普遍且嚴重的精神疾病，會導致嚴重的財務和社會後果。憂鬱症偵測對於早期介入以減輕這些後果至關重要。如此高風險的決策本質上需要可解釋性。儘管一些憂鬱症偵測研究嘗試根據重要性評分或注意力權重來解釋決策，但這些解釋與基於憂鬱症狀的臨床憂鬱症診斷標準不一致。為了填補這個空白，我們遵循計算設計科學範例來開發一個新穎的多尺度時間原型網路 (MSTPNet)。MSTPNet 創新地偵測和解釋憂鬱症狀以及它們持續多久。使用大規模資料集進行的廣泛實證分析顯示，MSTPNet 以 0.851 的 F1 分數優於最先進的憂鬱症偵測方法。此結果也揭示了調查方法中未注意到的新症狀，例如分享對不同生活的欽佩。我們進一步進行使用者研究，以證明其在可解釋性方面優於基準。本研究以一種新穎的可解釋深度學習模型為社會媒體中的憂鬱症偵測，為 IS 文獻做出貢獻。在實務上，我們提出的方法可以在社交媒體平台中實施，以提供個人化的線上資源給偵測出的憂鬱症患者。
...
摘要：電子健康紀錄 (EHR) 充當醫療保健中構思的人工智慧 (AI) 驅動轉型的基本資料來源。
然而，EHR 備忘中反映的臨床偏見可能導致 AI 模型繼承並放大這些偏見，進而導致健康差距持續存在。本研究利用基於 Transformer 的深度學習模型和可解釋 AI (XAI) 技術，調查 EHR 備忘中汙名化語言 (SL) 對死亡率預測的影響。我們的研究結果表明，臨床醫師所寫的 SL 對 AI 效能產生負面影響，特別是對黑人患者而言，凸顯 SL 是 AI 模型開發中種族差異的來源。為了探索一種在運作上有效率的方法來減輕 SL 的影響，我們透過臨床醫師的協作網路調查 SL 產生的模式，並找出核心臨床醫師對 AI 模型中的種族差異影響較大。我們發現，移除核心臨床醫師所寫的 SL，比消除資料全集中所有的 SL，是一種更有效率的偏見降低策略。本研究為負責任的 AI 開發提供了可行的見解，並有助於了解臨床醫師行為和醫療保健中的 EHR 備忘撰寫。
...
摘要：當代通過 AI 的自動化包含大量的幕後人工作業，這些工作通常既不可見，又報酬不足。由於不可見的工作，包括標籤和維護工作，是當代 AI 系統的組成部分，因此讓使用者了解其角色仍然很重要。我們建議這可以通过可解釋的 AI (XAI) 設計來完成，特別是女權主義交叉 XAI。我們提出源自女權主義交叉研究的製圖法，以找出 AI 的系統觀點，並納入與不可見工作相關的 AI 向度。
...
摘要：虛擬心理健康助理 (VMHA) 持續進步，以支援每年有 6000 萬人次初級保健就診和 600 萬人次急診室 (ER) 就診的全球醫療保健系統。這些系統由臨床心理學家、精神科醫師和人工智慧 (AI) 研究人員為認知行為療法 (CBT) 所建置。目前，VMHA 的角色是透過資訊提供情緒支持，較少著重於與患者建立反思性對話。需要更全面、安全且可解釋的方法來建置負責任的 VMHA，以提出後續問題或提供見多識廣的回應。這項調查提供了對現有心理健康對話代理的系統性批判性回顧，接著深入探討透過脈絡知識、資料集和 VMHA 在臨床決策支援中新興角色的改進。我們也提供新的方向，以可解釋性、安全性以及整體可信度豐富 VMHA 的使用者體驗。最後，我們提供評量指標和 VMHA 的實務考量，超越現有文獻，以在 VMHA 和患者之間建立信任，進行主動溝通。
...
摘要：XAI 指的是建立 AI 應用程式的技術和方法，這些應用程式能協助最終使用者解讀 AI 模型的輸出和預測。在高風險決策情境中，例如醫療領域，黑箱 AI 應用程式增加了對透明度和可解釋性的需求，因為錯誤的預測可能會造成嚴重的後果。模型可解釋性和可詮釋性對於 AI 模型在醫療實務中的成功部署至關重要。AI 應用程式的底層推理需要對臨床醫生透明，才能獲得他們的信任。本文提出 XAI 面向和挑戰在醫療領域的系統性回顧。本研究的主要目標是回顧各種 XAI 方法、它們的挑戰，以及醫療保健中相關的機器學習模型。這些方法分為六類來討論：面向特徵的方法、全局方法、概念模型、代理模型、局部基於像素的方法，以及以人為中心的方法。最重要的是，本文探討 XAI 在醫療保健問題中的角色，以釐清其在安全關鍵應用中的必要性。本文旨在透過回顧相關的實驗結果，建立對醫療保健領域中與 XAI 相關應用程式的全面理解。為了促進未來的研究以填補研究差距，本文研究了 XAI 模型從不同觀點的重要性及其限制。
...
摘要：最先进的機器學習模型通常會學習訓練資料中內嵌的虛假關聯性。這在將這些模型部署於高風險決策制定時會構成風險，例如在皮膚癌偵測等醫療應用中。為了解決這個問題，我們提出了 Reveal to Revise (R2R)，一個涵蓋整個可解釋人工智慧 (XAI) 生命週期的框架，使實務工作者能夠透過最少的人為互動，反覆找出、減輕和（重新）評估虛假的模型行為。在第一步 (1) 中，R2R 會透過找出歸因值中的異常值，或透過檢查模型學習到的潛在概念，來揭露模型的弱點。其次 (2)，找出負責的偽影並在輸入資料中進行空間定位，然後利用它來 (3) 修正模型行為。具體來說，我們應用 RRR、CDEP 和 ClArC 的方法來進行模型修正，並 (4)（重新）評估模型的效能和對偽影的剩餘敏感度。使用兩個醫療基準資料集進行黑色素瘤偵測和骨齡估計，我們將我們的 R2R 框架應用於 VGG、ResNet 和 EfficientNet 架構，並藉此揭露和修正真正的資料內在偽影，以及受控設定中的合成變異。為了完成 XAI 生命週期，我們示範多個 R2R 迭代以減輕不同的偏差。程式碼可於 https://github.com/maxdreyer/Reveal2Revise 取得。
...
摘要：可解釋人工智慧 (XAI) 的領域在近年來有了大幅進展，但進展主要在於電腦視覺和自然語言處理。對於時間序列，其輸入通常無法解釋，僅有有限的研究可用於 XAI。在此研究中，我們提出了虛擬檢查層，它將時間序列轉換為可解釋的表示形式，並允許透過局部 XAI 方法（例如逐層相關性傳播 (LRP)）將相關性歸因傳播到此表示形式。藉此，我們將 XAI 方法系列的適用性擴展到輸入僅在轉換後才可解釋的領域（例如語音）。在此，我們專注於傅立葉轉換，它主要應用於時間序列和 LRP 的解釋，並將我們的方法稱為 DFT-LRP。我們在各種時間序列分類設定（例如音訊和電子健康記錄）中展示了 DFT-LRP 的實用性。我們展示了 DFT-LRP 如何揭示在不同領域（例如時間相對於頻率域）中訓練的模型的分類策略差異，或有助於發現模型如何對資料中的虛假相關性採取行動。
...
摘要：解釋深度學習模型的預測結果給最終使用者，是一項重要的功能，能利用人工智慧 (AI) 的力量，進行醫療決策，這通常被認為是不透明且難以理解的。在本文中，我們運用最先進的可解釋人工智慧 (XAI) 方法，來解釋黑盒子 AI 模型在甲狀腺結節診斷應用中的預測結果。我們提出新的基於統計的 XAI 方法，即核密度估計和密度圖，來解釋未檢測到結節的情況。XAI 方法的效能會在定性和定量的比較下，被視為改進資料品質和模型效能的回饋。最後，我們進行調查，以評估醫師和患者對 XAI 解釋模型在甲狀腺結節影像決策上的信任度。
...
摘要：醫療設備和人工智慧系統快速轉變醫療保健的提供方式。同時，由於其本質，醫療設備中或作為醫療設備的人工智慧可能會遭受網路攻擊，導致患者安全和安全風險。本章節分為三部分。第一部分從設定場景開始，我們在其中說明網路安全在醫療保健中的角色。然後，我們簡要定義當我們談論被視為醫療設備本身或支援醫療設備的人工智慧時所指涉的內容。為了說明此類醫療設備帶來的風險，我們提供了三個範例：資料集中毒、社會工程和資料或原始碼萃取。在第二部分，本文概述了歐盟監管架構，與確保醫療設備中或作為醫療設備的人工智慧的網路安全相關（醫療器材法規、網路與資訊安全指令、網路安全法、一般資料保護規範、人工智慧法規提案和網路與資訊安全 2 指令提案）。最後，本文的第三部分探討源自歐盟監管架構的可能挑戰。特別是，我們展望源自兩項立法提案的挑戰，以及它們與現有關於人工智慧醫療設備網路安全的立法互動。它們被建構為對以下問題的解答：(1) 人工智慧法規將如何與醫療器材法規互動，關於網路安全和安全要求？(2) 我們應如何詮釋網路與資訊安全 2 指令提案和醫療器材法規中的事件通知要求？(3) 關鍵基礎設施演進術語的後果是什麼？[這是草稿章節。最終版本將收錄在 Barry Solaiman 和 I. Glenn Cohen 編輯的《健康、人工智慧與法律研究手冊》中，預計於 2023 年由 Edward Elgar Publishing Ltd 出版]
...
摘要：阿茲海默症（AD）是一種進行性神經退化疾病，也是失智症的主要原因。早期診斷對於患者從潛在的干預和治療中受益至關重要。由於視網膜與大腦的解剖連接，因此假設視網膜是 AD 檢測的診斷部位。为此目的開發的 AI 模型尚未對決策提供合理的解釋，也沒有推斷疾病進程的階段。順著這個方向，我們提出了一個新的與模型無關的可解釋 AI 框架，稱為顆粒神經元級解釋器 (LAVA)，這是一個解釋原型，可以探查卷積神經網路 (CNN) 模型的中間層，以直接從視網膜影像評估 AD 連續體，而無需縱向或臨床評估。此方法用於驗證視網膜血管作為阿茲海默症 (AD) 評估的生物標記和診斷方式。英國生物樣本庫的認知測試和血管形態特徵表明，LAVA 在識別進程連續體中的 AD 階段方面顯示出強大的前景和有效性。
...
摘要：<paragraph>許多視覺化技術已針對可解釋 AI (XAI) 進行開發，但它們通常需要使用者進一步推理才能進行詮釋。我們主張 XAI 應支援圖解式和演繹推理，以便 AI 執行假設產生和評估，以縮小可解釋性差距。我們提出圖表化以 i) 執行皮爾士演繹推理，ii) 遵循領域慣例，以及 iii) 以視覺或口頭方式透過圖表進行說明。我們實作了 DiagramNet，進行臨床應用以根據心臟聽診預測心臟診斷，並以基於形狀的雜音圖表進行說明。在建模研究中，我們發現 DiagramNet 不僅提供忠實的雜音形狀說明，而且比基準模型具有更好的預測效能。我們進一步在與醫學生的定性使用者研究中展示了圖解說明的可解釋性和可信度，顯示出臨床上相關的圖解說明優於技術顯著性圖說明。這項工作有助於提供以使用者為中心的 XAI 的領域慣例演繹說明。</paragraph>
...
摘要：皮膚癌是人類最普遍的癌症形式之一。它的主要辨識方式是視覺，從臨床篩檢開始，並持續到皮膚鏡檢查、組織學評估和標本收集。深度卷積神經網路 (CNN) 對分類的細粒度物件執行高度區隔且潛在通用的任務。本研究提出一個新穎的多類別預測架構，根據 ViT 和 ViTGAN 對皮膚病灶進行分類。基於視覺轉換器的 GAN（生成對抗網路）用於解決類別不平衡的問題。該架構包含四個主要階段：ViTGAN、影像處理和可解釋 AI。階段 1 包含生成合成影像以平衡資料集中所有類別。階段 2 包含套用不同的資料擴充技術和形態運算以增加資料大小。階段 3 和 4 包含為邊緣運算系統開發一個 ViT 模型，該模型可以識別模式並從使用者的皮膚影像中分類皮膚病灶。在階段 3 中，在使用 ViT 將病灶分類到所需的類別後，我們將使用可解釋 AI (XAI)，這將導致更可解釋的結果（使用啟用映射等），同時確保高預測準確度。皮膚疾病的即時影像可以由醫生或患者使用行動應用程式的相機拍攝，以執行早期檢查並確定皮膚病灶的原因。整個架構與現有的皮膚病灶偵測架構進行比較。
...
摘要：<paragraph>在高風險環境中部署人工智慧 (AI)，例如醫療保健，提供可解釋性/可說明性或允許精細錯誤分析的方法至關重要。許多最近的可解釋性/可說明性和精細錯誤分析方法使用概念，這些概念是對人類具有語義意義的元標籤。然而，只有少數幾個包含概念級元標籤的資料集，而這些元標籤大多與不需要領域專業知識的自然影像相關。針對單一疾病（例如黑色素瘤）的元標籤，在醫學上密集註解的資料集。在皮膚科中，皮膚疾病使用既定的臨床詞彙進行描述，讓臨床醫生可以互相描述身體檢查結果。為了提供一個由領域專家密集註解的醫療資料集，其中註解可用於多種疾病過程，我們開發了 SkinCon：一個由皮膚科醫生密集註解的皮膚疾病資料集。SkinCon 包含來自 Fitzpatrick 17k 資料集的 3230 張影像，並密集註解了 48 個臨床概念，其中 22 個至少有 50 張影像代表該概念。所使用的概念是由兩位皮膚科醫生選定的，他們考慮了用於描述皮膚病灶的臨床描述詞。範例包括「斑塊」、「鱗屑」和「糜爛」。相同的概念也用於標記來自 Diverse Dermatology Images 資料集的 656 張皮膚疾病影像，提供了具有多樣膚色的額外外部資料集。我們回顧了 SkinCon 資料集的潛在應用，例如探測模型、基於概念的說明和概念瓶頸。此外，我們使用 SkinCon 來展示這兩個使用案例：使用概念偵錯現有皮膚科 AI 模型的錯誤，以及使用事後概念瓶頸模型開發可解釋模型。</paragraph>
...
摘要：不安分的多臂賭博機 (RMAB) 是一種流行的決策理論架構，
已用於模擬公共衛生、野生動物保育、通訊系統等領域的真實世界順序決策問題。
已部署的 RMAB 系統通常分兩個階段運作：第一個階段預測定義 RMAB 執行個體的未知參數，第二個階段使用最佳化演算法來解決已建構的 RMAB 執行個體。
在這項工作中，我們提供並分析了在公共衛生領域部署 RMAB 系統的第一個同類型系統的結果，目的是改善孕產婦和兒童健康。我們的分析重點在於了解預測準確度與已部署 RMAB 系統的整體效能之間的關係。這對於決定投資於改善預測準確度以改善最終系統效能的價值至關重要，並且對於診斷和監控已部署的 RMAB 系統很有用。
使用我們已部署的 RMAB 系統中的真實世界資料，我們證明整體預測準確度的提升甚至可能伴隨著 RMAB 系統效能的下降——廣泛投資資源以改善整體預測準確度可能無法產生預期的結果。在此之後，我們開發了以決策為重點的評估指標來評估預測組成，並顯示它更能（無論是經驗上還是理論上）解釋已部署 RMAB 系統的整體效能。
...
摘要：可解釋 AI 包含在決策系統與人類之間建立互動機制的發展，使前者的決策變得易於理解。這在敏感情境中特別重要，例如在醫療領域。我們提出一個使用案例研究，針對皮膚病灶診斷，說明如何為從範例中訓練出的最新深度神經網路分類器決策提供執業醫師解釋。我們的架構包含一個訓練好的分類器，說明模組在該分類器上執行。後者能夠為分類診斷提供執業醫師範例和反例，因此允許醫師與自動診斷系統互動。範例是透過對抗式自動編碼器產生的。我們說明系統在代表性範例中的行為。
...
摘要：人工智慧（AI）應用中的演算法公平性對於一個更好的社會至關重要。公平性作為社會機制的基礎公理，包含多個面向。儘管機器學習（ML）社群已將交叉性視為統計平權的問題，特別是在歧視議題上，但新興的文獻探討了另一個面向——單調性。基於領域專業知識，單調性在許多與公平性相關的領域中扮演至關重要的角色，違反單調性可能會誤導人類決策並導致災難性的後果。在本文中，我們首先系統性地評估應用單調神經加法模型（MNAM）的重要性，它使用公平感知的 ML 演算法來強制執行個人和成對單調性原則，以確保 AI 倫理和社會的公平性。我們透過理論推理、模擬和廣泛的實證分析的混合方法發現，在所有公平性領域（包括犯罪學、教育、醫療保健和金融）中，考慮單調性公理至關重要。我們的研究有助於 AI 倫理、可解釋 AI（XAI）和人機互動（HCI）介面中的跨領域研究。透過證明不符合單調性的災難性後果，我們探討了單調性要求在 AI 應用中的重要性。此外，我們證明 MNAM 是一種有效的公平感知 ML 方法，透過施加單調性限制整合人類智慧。
...
摘要：<paragraph>在安全關鍵應用程式中，例如法律或醫療領域，解釋 AI 模型的預測至關重要。對於預測的一種解釋形式是萃取式依據，亦即引導模型對該實例做出預測的實例特徵子集。例如，子詞組「他偷了手機」可以是「竊盜」預測的萃取式依據。先前關於產生萃取式依據的研究通常採用兩階段模型：一個選擇器用於選擇最重要的特徵（即依據），然後是一個僅根據所選特徵進行預測的預測器。這些研究的一個缺點是，學習選擇特徵的主要訊號來自於將預測器給出的答案與基本事實答案進行比較。在這項研究中，我們建議透過資訊校正方法從預測器中擷取更多資訊。更精確地說，我們聯合訓練兩個模型：一個是典型的解決手邊任務的神經模型，但以精確但黑盒子的方式進行，另一個是選擇器預測器模型，此外還為其預測產生依據。第一個模型用作第二個模型的指南。我們使用對抗技術來校正兩個模型提取的資訊，使得它們之間的差異成為遺漏或過度選擇特徵的指標。此外，對於自然語言任務，我們提出了一個基於語言模型的正則化器，以鼓勵提取流暢的依據。在情緒分析任務、仇恨言論辨識任務以及法律領域的三個任務上的實驗結果顯示了我們在依據提取方面的做法的有效性。</paragraph>
...
摘要：最近通過認證的人工智慧 (AI) 工具在醫療保健領域的激增，重新引發了關於採用此技術的辯論。其中一個爭論的主題涉及可解釋 AI (XAI) 及其承諾讓 AI 裝置更透明且更值得信賴。醫療 AI 領域中的一些活躍人士表達了對可解釋 AI 技術的可靠性，尤其是特徵歸因方法的疑慮，質疑其用途以及在準則和標準中的納入。儘管有這些合理的疑慮，我們認為現有的批評，也就是關於事後局部可解釋性方法的可行性，將問題過度概括到影像資料，因而把好壞混為一談。我們從將問題描述為解釋和人類理解之間缺乏語意匹配開始。為了了解特徵重要性何時能可靠地使用，我們引入了低階特徵和高階特徵特徵重要性之間的區別。我們認為，對於低階特徵具有明確語意的資料類型，例如電子健康紀錄 (EHR) 等表格資料，可以獲得語意匹配，因此特徵歸因方法仍能以有意義且有用的方式使用。最後，我們勾勒了一個程序，以測試是否已達成語意匹配。
...
摘要：儘管 AI 的機器學習 (ML) 模型在醫療領域達成高水準的表現，它們並非毫無錯誤。賦予臨床醫師辨識錯誤模型建議的能力，對於建立對醫療 AI 的信任至關重要。可解釋 AI (XAI) 旨在透過釐清 AI 推論來滿足此項需求，以協助最終使用者。最近有幾項關於生物醫學影像的研究獲得了有希望的結果。然而，對於使用表格資料的模型，其解決方案仍不足以滿足臨床醫師的需求。本文提出了一種方法，以協助臨床醫師辨識使用表格資料訓練的 ML 模型的失敗。我們在三個主要支柱上建構了我們的做法：透過利用臨床背景潛在空間來分解特徵集、評估整體解釋的臨床關聯性，以及基於潛在空間相似性 (LSS) 的局部解釋。我們在 ML 基於感染所造成的早產兒發病率識別上展示了我們的做法。由於模型失靈所造成的死亡風險、終身殘疾和抗生素抗藥性，一直是此領域中一個公開的研究問題。我們透過我們的做法，成功辨識出兩個模型的錯誤分類案例。我們的解決方案透過將局部解釋脈絡化，為臨床醫師提供可行的見解，以協助他們自主做出明智的最終決策。
...
摘要：<paragraph>我們正在見證 AI 經濟和社會的興起，其中 AI 技術正日益影響醫療保健、商業、交通和日常生活中的許多方面。已經報導了許多成功案例，其中 AI 系統甚至超過了人類專家的準確性。然而，AI 系統可能會產生錯誤，可能表現出偏差，可能對數據中的雜訊敏感，而且通常缺乏技術和司法透明度，導致信任度降低和採用難題。這些最近的缺點和疑慮已在科學和一般媒體中記錄下來，例如自動駕駛汽車事故、醫療保健中的偏見、有色人種的招聘和面部識別系統、看似正確的醫療決策後來發現是出於錯誤的原因等。這導致許多政府和監管倡議要求值得信賴且符合道德的 AI 來提供準確性和穩健性，某種形式的可解釋性、人類控制和監督、消除偏見、司法透明度和安全性。提供可信賴的 AI 系統所面臨的挑戰促使對可解釋 AI 系統 (XAI) 進行深入研究。XAI 的目的是提供人類可以理解的資訊，說明 AI 系統如何做出決策。在本文中，我們首先簡要總結當前的 XAI 工作，然後質疑準確性與可解釋性互斥且僅關注深度學習的最新論點。然後，我們提出我們對在高風險可信賴 AI 系統交付的完整生命週期中使用 XAI 的建議，例如開發、驗證和認證，以及可信賴的生產和維護。</paragraph>
...
摘要：<paragraph>深度神经網路 (DNN) 可偵測資料中的模式，並在許多電腦視覺應用程式中展現多功能性和強大效能。然而，DNN 本身容易犯下違反簡單常識概念的明顯錯誤，且其運用明確知識來引導搜尋和決策的能力有限。雖然整體 DNN 效能指標可能良好，但這些明顯的錯誤，加上缺乏可解釋性，已阻礙其廣泛用於醫療影像分析等重要任務。本論文的目的是介紹 SimpleMind，一個專注於醫療影像理解的認知 AI 開源軟體架構。它允許建立一個知識庫，以直觀且人類可讀的形式描述影像物件預期的特徵和關係。SimpleMind 框架透過以下方式為 DNN 帶來思考能力：(1) 提供方法，利用知識庫對影像內容進行推理，例如空間推理和條件推理，以檢查 DNN 輸出；(2) 以通用軟體代理程式形式應用程序知識，將其串連在一起以完成影像前處理、DNN 預測和結果後處理；以及 (3) 執行所有知識庫參數的自動共同最佳化，以適應代理程式以解決特定問題。SimpleMind 能夠對多個偵測到的物件進行推理以確保一致性，並提供 DNN 輸出之間的交叉檢查。這種機器推理透過可解釋的模型和可解釋的決策，提升了 DNN 的可靠性和可信度。本文提供了範例應用程式，說明 SimpleMind 如何透過將其嵌入認知 AI 框架中來支援和改善深度神經網路。</paragraph>
...
摘要：在過去十年中，基於深度學習的方法在視覺感知任務方面取得了驚人的進展，在從自動駕駛到臨床決策支援系統的眾多應用領域中得到廣泛採用。儘管這些基於深度學習的模型效能令人印象深刻，但它們在決策過程中仍然相當不透明，這使得它們在人類關鍵任務中的部署成為一項有風險的事業。這反過來又使得理解這些模型做出的決策對於它們的可靠部署至關重要。可解釋 AI (XAI) 方法試圖通過為這種黑箱深度學習方法提供解釋來解決這個問題。在本文中，我們對電腦視覺中的基於歸因的 XAI 方法進行了全面的調查，並回顧了現有的基於梯度、基於擾動和對比的 XAI 方法文獻，並提供了關於開發和評估健壯 XAI 方法的關鍵挑戰的見解。
...
摘要：自 10 年代中期以來，深度學習 (DL) 時代持續至今，每年都帶來新的最高級和創新。然而，這些創新轉化為實際應用程式的速度落後於這個快速步伐。特別是安全關鍵應用程式，其基礎是嚴格的法規和道德要求，需要小心處理，並且仍然是爭論的活躍領域。可解釋 AI (XAI) 和隱私保護機器學習 (PPML) 都是至關重要的研究領域，旨在減輕 DL 中盛行的數據密集型黑盒模型的一些缺點。儘管在各自的領域中進行了熱烈的研究活動，但尚未關注它們之間的互動。這項工作是第一個探討私人學習技術對基於 DL 的模型產生的解釋的影響。在涵蓋多個領域的各種影像和時間序列資料集以及不同的隱私技術、XAI 方法和模型架構的廣泛實驗分析中，研究了私人訓練對產生的解釋的影響。研究結果表明，透過引入隱私，解釋產生了不可忽略的變化。除了報告 PPML 對 XAI 的個別影響外，本文還對實際應用中技術的選擇提出了明確的建議。透過揭示這些關鍵技術的相互依賴性，這項工作是克服在安全關鍵領域中實用 AI 剩餘障礙的第一步。
...
摘要：<paragraph>肺結核 (TB) 是一種傳染性細菌疾病，是造成死亡的重要原因，尤其是在低收入國家，2020 年全球報告約有一千萬例新病例。雖然肺結核可以治療，但未遵守用藥計畫是造成發病率和死亡率的重要原因。因此，主動找出有中斷用藥計畫風險的患者，可以採取矯正措施來減輕不良後果。我們使用極端未遵守用藥計畫的代理測量指標，以及來自印度四個邦約 700,000 名患者的資料集，根據自訂的基於排名度量來制定並解決機器學習 (ML) 問題，以早期預測未遵守用藥計畫。我們訓練機器學習模型並針對基準進行評估，在考量全國規模的未來部署後，相較於基於規則的基準提高了約 100%，相較於隨機分類器則提高了約 214%。我們在過程中處理了各種問題，包括資料品質、高基數類別資料、低目標盛行率、分佈轉移、不同群體之間的差異、演算法公平性，以及對穩健性和可解釋性的需求。我們的研究結果表明，非遵守用藥計畫患者的風險分層是一種可行的、可大規模部署的機器學習解決方案。作為印度中央肺結核部門的官方人工智慧合作夥伴，我們正在與多個城市和州級試點計畫合作，目標是全印度部署。</paragraph>
...
摘要：可解釋人工智慧 (XAI) 透過提升終端使用者對機器信任度，轉變了人工智慧 (AI) 的領域。
隨著連網裝置數量持續成長，物聯網 (IoT) 市場需要讓終端使用者信賴。
然而，現有文獻仍缺乏系統性且全面的調查工作，探討 XAI 在 IoT 的應用。
為了彌補這個不足，我們在本文中探討 XAI 架構，重點在於它們的特性和對 IoT 的支援。
我們說明了廣泛使用的 XAI 服務，用於 IoT 應用，例如安全強化、醫療物聯網 (IoMT)、工業物聯網 (IIoT) 和城市物聯網 (IoCT)。
我們也建議在這些應用中，透過適當的範例，在 IoT 系統中實作 XAI 模型的選擇，並總結未來工作的關鍵推論。
此外，我們介紹了邊緣 XAI 架構的尖端發展，以及第六代 (6G) 通訊服務對 IoT 應用的支援，以及關鍵推論。
簡而言之，本文構成第一個針對 XAI 為基礎的架構開發的整體彙編，專為未來 IoT 使用案例的需求量身打造。
...
摘要：美國每年約有 125 萬人因燒傷接受治療。精確的燒傷分類是醫療 AI 領域中的一個重要面向。在這項工作中，我們提出一個可解釋的人在迴圈架構，用於改善燒傷超音波分類模型。我們的架構利用基於 LIME 分類說明器的說明系統，來佐證並整合燒傷專家的知識，建議新的功能並確保模型的有效性。使用這個架構，我們發現 B 模式超音波分類器可以透過提供紋理特徵來增強。更具體地說，我們確認基於超音波影像的灰階共生矩陣 (GLCM) 的紋理特徵可以提高轉移學習的燒傷深度分類器的準確性。我們在豬隻受試者的真實資料上測試我們的假設。我們展示了燒傷深度分類的準確性改進，從約 88% 提升至約 94%，只要根據我們的架構進行修改。
...
摘要：在醫療保健領域，人工智慧系統支援臨床醫師和病患進行診斷、治療和監測，但許多系統的解釋性不佳，對於實際應用仍是一大挑戰。克服此障礙是可解釋人工智慧 (XAI) 的目標。然而，解釋可以有不同的理解，因此無法為所有人解決黑箱問題。以人為中心的 AI 領域透過調整 AI 以適應使用者來處理此問題。我們提出以使用者為中心的假想人物概念來評估 XAI，並使用此概念來調查最終使用者在行動健康壓力監測應用程式中對各種解釋風格和內容的偏好。我們的線上調查結果顯示使用者的年齡層和個性，以及解釋類型，會影響解釋偏好，這表示這些是 XAI 設計的重要特徵。我們將結果歸納成三種典型的使用者假想人物：重視權力、休閒和隱私的使用者。我們的見解讓互動式、以人為中心的 XAI 更接近實際應用。
...
摘要：青光眼是所有視力缺陷中導致部分或完全失明的第二大原因，其主要發生原因是焦慮或憂鬱導致眼睛壓力過大，進而損害視神經並導致視力併發症。傳統青光眼篩檢是一個耗時的過程，需要醫療專業人員持續關注，即使如此，有時由於時間限制和壓力，他們無法正確分類，導致治療錯誤。已經做出許多努力來自動化整個青光眼分類程序，然而，這些現有模型通常具有黑盒特性，使用戶無法了解預測背後的主要原因，因此醫療從業人員通常無法依賴這些系統。在本文中，在與各種預訓練模型比較後，我們提出了一個轉移學習模型，能夠以 94.71% 的準確率對青光眼進行分類。此外，我們利用了局部可解釋模型不可知解釋（LIME），在我們的系統中引入了可解釋性。這項改進使醫療專業人員能夠獲得重要且全面的資訊，幫助他們做出判斷。它也降低了傳統深度學習模型的不透明性和脆弱性。
...
摘要：深度學習在偵測物件和分類影像方面展現出卓越的效能，確保了分析醫學影像的巨大前景。
將深度學習的成功轉化為醫學影像，其中醫生需要了解潛在的過程，需要有能力詮釋和說明神經網路的預測。深度神經網路的可解釋性通常依賴於估計輸入特徵（例如，像素）相對於結果（例如，類別機率）的重要性。然而，已經開發出許多重要性估計器（也稱為顯著性圖），但尚不清楚哪些對於醫學影像應用更為相關。在目前的工作中，我們使用三個不同的評估指標研究了幾個重要性估計器在解釋卷積深度網路對電腦斷層掃描 (CT) 影像分類中的效能。首先，模型中心保真度衡量當某些輸入受到擾動時模型精確度的下降。其次，在像素層級上，透過接收器操作特性 (ROC) 曲線測量重要性分數與專家定義的分割遮罩之間的一致性。第三，我們透過 Dice 相似性係數 (DSC) 測量基於 XRAI 的地圖與分割遮罩之間的區域重疊。總體而言，兩個版本的 SmoothGrad 在保真度和 ROC 排名中名列前茅，而積分梯度和 SmoothGrad 在 DSC 評估中表現出色。有趣的是，模型中心（保真度）和以人為中心（ROC 和 DSC）評估之間存在關鍵差異。分割地圖中嵌入的專家期望和直覺不一定與模型如何得出其預測相符。了解可解釋性中的這種差異將有助於利用深度學習在醫學中的力量。
...
摘要：深度學習方法，特別是卷積神經網路，已成為醫學影像運算任務中強大的工具。雖然這些複雜的模型提供優異的效能，但其黑箱性質可能會阻礙在高風險決策制定中採用於實際應用。在本文中，我們提出一個互動式系統，利用最先進的可解釋性技術，協助放射科醫師進行乳癌篩檢。我們的系統將深度學習模型整合到放射科醫師的工作流程中，並提供新穎的互動，以促進對模型決策過程的理解。此外，我們證明我們的系統可以逐漸利用使用者互動，以提供更細緻的可解釋性報告，而標記工作量很小。由於所採用的可解釋性技術具有一般性，因此我們的系統與領域無關，可用於許多不同的醫學影像運算任務，提供視覺分析如何能轉化原本靜態的可解釋性技術，以擴增人類決策制定並促進採用醫療 AI 的新觀點。
...
摘要：現今，人工智慧（AI）已成為醫療保健應用程式中的一項基本組成部分，無論是臨床或遠距醫療，但效能最佳的 AI 系統通常過於複雜，無法自行解釋。可解釋 AI（XAI）技術被定義為揭示系統預測和決策背後的推理，在處理敏感且個人的健康資料時，它們變得更加重要。值得注意的是，XAI 並未在不同的研究領域和資料類型中獲得相同的關注，特別是在醫療保健領域。特別是，許多臨床和遠距醫療應用程式分別基於表格和時間序列資料，而 XAI 並未針對這些資料類型進行分析，而電腦視覺和自然語言處理（NLP）則是參考應用程式。為了概述最適合醫療領域中表格和時間序列資料的 XAI 方法，本文回顧了過去 5 年的文獻，說明了產生的解釋類型以及評估其相關性和品質所做的努力。具體來說，我們將臨床驗證、一致性評估、客觀且標準化的品質評估以及以人為中心的品質評估確定為確保最終使用者獲得有效解釋的主要特徵。最後，我們強調了該領域的主要研究挑戰以及現有 XAI 方法的限制。
...
摘要：<paragraph>目的：本研究評估了一種市售可解釋 AI 演算法在增強臨床醫生識別胸部 X 光片（CXR）上肺癌的能力方面的影響。
設計：這項回顧性研究評估了 11 位臨床醫生在有和沒有市售 AI 演算法（紅點，Behold.ai）協助的情況下從胸部 X 光片中偵測肺癌的表現，該演算法可預測 CXR 中疑似肺癌。臨床醫生的表現根據臨床確診進行評估。
背景：該研究分析了來自 NHS 醫院的匿名患者資料；該資料集包含 400 張來自成年患者（18 歲及以上）的胸部 X 光片，這些患者在 2020 年接受了 CXR 檢查，並附有對應的臨床文字報告。
參與者：由 11 位臨床醫生（顧問放射科醫生、放射科醫生受訓者和報告放射科醫生）組成的讀者小組參與了這項研究。
主要結果指標：臨床醫生在有和沒有 AI 輸入的情況下，在 CXR 上檢測肺癌的整體準確度、敏感度、特異性和精確度。還評估了臨床醫生和績效標準差之間的一致性比率，有和沒有 AI 輸入的情況。
結果：臨床醫生使用 AI 演算法導致肺部腫瘤檢測的整體表現得到改善，總共增加了 17.4% 的肺癌在 CXR 上被識別出來，否則這些肺癌將會被遺漏，整體上增加了對較小腫瘤的檢測，分別增加了 24% 和 13% 的 1 期和 2 期肺癌檢測，並標準化了臨床醫生的表現。
結論：這項研究顯示了 AI 演算法在改善早期肺癌診斷和通過整體改善讀者表現促進健康公平方面的臨床效用，而不會影響下游影像資源。</paragraph>
...
摘要：<paragraph>缺乏足够大的开放医疗数据库是人工智能驱动的医疗保健领域中最大的挑战之一。利用生成对抗网络 (GAN) 创建的合成数据似乎是缓解隐私政策问题的良好解决方案。另一种治疗方法是在多个医疗机构之间分散协议，而不交换本地数据样本。在本文中，我们在集中和分散的环境中探索了无条件和条件 GAN。集中式设置模仿了对庞大但高度不平衡的皮肤病变数据集的研究，而分散式设置模拟了三个机构参与的更真实的医院场景。我们评估了模型在保真度、多样性、训练速度和对生成合成数据训练的分类器的预测能力方面的性能。此外，我们还通过探索潜在空间和嵌入投影提供了可解释性，重点关注全局和局部解释。计算出的真实图像与其在潜在空间中的投影之间的距离证明了训练过的 GAN 的真实性和泛化性，这是此类应用程序中的主要关注点之一。进行研究的开源代码可在
\url{https://github.com/aidotse/stylegan2-ada-pytorch} 公开获得。</paragraph>
...
摘要：在醫院的世界中，有許多複雜的組合問題，而解決這些問題對於增加患者的滿意度和提供的照護品質非常重要。醫療保健中的問題很複雜，因為要解決這些問題，必須考慮到許多限制和不同類型的資源。此外，必須在短時間內評估解決方案，以確保在實際情況中可用。我們計畫提出解決此類問題的方案，同時擴充已測試的方案，並根據文獻為新問題建模解決方案，並在有資料時使用實際資料。解決此類問題很重要，但由於歐盟委員會制定了《一般資料保護法規》，規定每個人都有權要求說明 AI 做出的決定，因此在未開發出可解釋性方法之前，基於 AI 的求解器（例如基於答案集程式設計的求解器）的使用將受到限制。因此，研究的另一部分將致力於研究和提出新的方法，以說明獲得的解決方案。
...
摘要：在現代動態不斷發展的社會中，越來越多的人罹患慢性病和重症，醫生和患者需要特殊且精密的醫療保健支持。因此，傑出的醫療保健利益相關者已意識到開發此類服務的重要性，以讓患者的生活更輕鬆。此類支持需要收集大量患者複雜的資料，例如臨床、環境、營養、日常活動、各種來自智慧型穿戴式裝置的資料、配備感測器的服飾資料等。整體患者資料必須適當地彙整、處理、分析並提供給醫生和照護者，以建議適當的治療和行動，以改善患者的健康相關參數和整體健康。進階人工智慧技術提供了分析此類大數據、使用它們並衍生新知識以支援個人化醫療決策的機會。基於進階機器學習、聯邦學習、遷移學習、可解釋人工智慧等新方法，為未來更優質地使用健康和醫療資料開啟了新途徑。在本文中，我們將介紹在個人化醫療決策中應用一系列人工智慧方法的領域中的一些關鍵面向和特徵範例。
...
摘要：<paragraph>持續監控已訓練的 ML 模型以確定何時應信賴和不應信賴其預測，對於它們的安全部署至關重要。這樣的架構應該是高性能、可解釋、事後和可操作的。我們提出 TRUST-LAPSE，一個用於持續模型監控的「不信任」評分架構。我們使用一系列潛在空間嵌入來評估每個輸入樣本的模型預測的可信度。具體來說，(a) 我們的潛在空間不信任分數使用潛在空間中的距離度量（馬氏距離）和相似度量（餘弦相似度）來估計不信任，(b) 我們的順序不信任分數確定過去輸入表示序列中相關性的偏差，採用非參數、基於滑動視窗的演算法進行可操作的持續監控。我們透過兩個下游任務評估 TRUST-LAPSE：(1) 分佈轉移輸入偵測，以及 (2) 資料漂移偵測。我們使用公共資料集評估各種領域 - 音訊和視覺，並進一步在具有挑戰性的真實世界腦電圖 (EEG) 資料集上對我們的做法進行基準測試，以進行癲癇偵測。我們的潛在空間不信任分數獲得了最先進的結果，AUROC 分別為 84.1（視覺）、73.9（音訊）和 77.1（臨床 EEG），比基準高出 10 分以上。我們揭露了流行基準中對輸入語義內容不敏感的嚴重失敗，使其不適合於真實世界的模型監控。我們表明，我們的順序不信任分數可實現高漂移偵測率；所有領域中超過 90% 的串流顯示 < 20% 的誤差。透過廣泛的定性和定量評估，我們表明我們的信任分數更強大，並提供可解釋性，以便於實務採用。</paragraph>
...
摘要：機器學習的普及提高了不公平模型在高風險應用中部署的風險，例如司法系統、藥物/疫苗設計和醫療診斷。儘管有從頭訓練公平模型的有效方法，但如何自動揭示和解釋訓練模型的不公平性仍然是一項具有挑戰性的任務。以可解釋的方式揭示機器學習模型的不公平性是邁向公平且值得信賴的 AI 的關鍵一步。在本文中，我們系統性地應對了通過挖掘可解釋證據（RUMIE）揭示不公平模型的新任務。關鍵思想是以一群模型最歧視的數據實例的形式找到確鑿證據。為了使證據具有可解釋性，我們還找到了一組人類可以理解的關鍵屬性和決策規則，這些屬性和決策規則表徵了被歧視的數據實例，並將其與其他未被歧視的數據區分開來。正如在許多真實世界數據集上進行的廣泛實驗所證明的那樣，我們的模型找到了高度可解釋且確鑿的證據，可以有效地揭示訓練模型的不公平性。此外，它比所有基準方法的可擴展性都要好得多。
...
摘要：可解釋人工智慧 (XAI) 在安全關鍵系統中是必要的，例如在臨床診斷中，因為有高風險做出致命的決定。然而，目前 XAI 就像是一堆方法的集合，而不是一個定義良好的流程。在這項工作中，我們闡述了 XAI 中最大的子群體，可解釋機器學習 (IML) 與經典統計之間的概念相似性。根據這些相似性，我們提出 IML 的形式化，類似統計流程。採用這種統計觀點讓我們能夠將機器學習模型和 IML 方法解釋為精密的統計工具。根據這種解釋，我們推論出三個關鍵問題，我們確定這些問題對於 IML 在安全關鍵設定中的成功和採用至關重要。透過制定這些問題，我們進一步旨在引發一場討論，討論 IML 與經典統計之間的區別，以及我們的觀點對該領域的未來有何影響。
...
摘要：人工智慧已普遍存在於各個領域和專業，
生物醫學影像和訊號處理也不例外。對此主題日益增長且廣泛的興趣引發了大量的研究活動，並反映在指數級的研究工作中。透過研究大量且多樣化的生物醫學資料，機器和深度學習模型已徹底改變了建模、分割、配準、分類和合成等各種任務，表現優於傳統技術。然而，難以將結果轉化為生物學/臨床可解釋的資訊，阻礙了它們在該領域的充分利用。可解釋人工智慧 (XAI) 嘗試透過提供使模型可解釋並提供解釋的方法來填補這種轉譯差距。迄今為止已提出不同的解決方案，並越來越受到社群的關注。本文旨在概述生物醫學資料處理中的 XAI，並指出將於 2022 年 3 月出現在 IEEE 訊號處理雜誌的生物醫學影像和訊號處理深度學習特刊。
...
摘要：在使用醫療影像進行診斷時，無論是臨床醫師或人工智慧 (AI) 系統，影像品質都很重要。當影像品質不佳時，產生該影像的醫療檢查通常需要重新執行。遠距醫療中常見的問題是，品質問題往往在患者離開診所後才會被標示出來，這表示他們必須回來重新檢查。這對居住在偏遠地區的人來說特別困難，而這些人占巴西數位醫療保健組織 Portal Telemedicina 病患的很大一部分。在本文中，我們報告了關於 (i) 開發一種 AI 系統以標示和解釋低品質醫療影像的即時工作、(ii) 訪談研究以了解在 OurCompany 使用 AI 系統的利害關係人對解釋的需求，以及 (iii) 縱向使用者研究設計以檢驗在我們的診所中，將解釋納入技術人員工作流程的效果。據我們所知，這將是第一個針對 XAI 方法對最終使用者（使用 AI 系統但沒有 AI 專業知識的利害關係人）的影響進行評估的縱向研究。我們歡迎針對我們的實驗設定提供意見回饋和建議。
