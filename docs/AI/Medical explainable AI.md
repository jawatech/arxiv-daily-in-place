...
摘要：<paragraph>最近人工智能的突破有望从根本上提升我们对医疗保健的研究和理解。开发一个集成的多对多框架，利用多种数据模态输入进行多项医疗任务的分析建模，对于统一理解现代医学至关重要。在这项工作中，我们介绍了 M3H，一个可解释的多模态多任务机器学习医疗框架，它整合了跨越广泛医疗任务类别和机器学习问题类别的不同多模态输入的学习。该框架的模块化设计确保了其可推广的数据处理、任务定义和快速模型原型制作，适用于临床和运营医疗保健环境。我们通过验证从四种模态（表格、时间序列、语言和视觉）训练的模型来评估 M3H 框架，涵盖 4 个机器学习问题类别中的 41 项医疗任务。我们的结果表明，M3H 持续生成多任务模型，其性能优于规范的单任务模型（1.1-37.2%），涵盖 16 个医疗科室的 37 种疾病诊断、三个医院运营预测和一项患者表型任务：跨越监督二元分类、多类分类、回归和聚类的 ML 问题类别。此外，该框架引入了一种新颖的注意力机制来平衡自我利用（专注于学习源任务）和交叉探索（鼓励从其他任务中学习）。此外，M3H 提供了可解释性见解，说明附加任务的联合学习如何使用提出的 TIM 分数影响源任务的学习，从而阐明任务相互依赖的动态。其可适应的架构促进了定制和集成，使其成为未来人工智能驱动的医疗保健系统的稳健且可扩展的候选解决方案。</paragraph>
...
摘要：本文探討了結構化多主體系統 (MAS) 的應用，以建立醫療必要性，此過程涉及系統性地檢視病患特定醫療結構化和非結構化資料，並對照臨床指南。我們透過將此複雜任務分解成較小、較易管理的子任務來解決。每個子任務都由一個專門的 AI 主體處理。我們進行了一項系統性研究，探討了各種提示策略對這些主體的影響，並對不同的大型語言模型 (LLM) 進行基準測試，以確定它們在完成這些任務時的準確性。此外，我們研究了這些主體如何提供可解釋性，進而增強系統中的信任和透明度。
...
摘要：人工智能 (AI) 的普遍整合在涉及人工智能系統的事件中引入了複雜的責任和問責挑戰。這些系統的互連性、人工智能引發事件的倫理問題，加上人工智能技術的不確定性和缺乏相應的法規，使得傳統責任歸屬具有挑戰性。為此，本研究提出了一種計算反射均衡 (CRE) 方法，為所有利益相關者建立一個連貫且倫理上可接受的責任歸屬架構。計算方法提供了一個結構化的分析，克服了概念方法在處理動態和多方面的場景時的局限性，展示了該架構在責任歸屬過程中的可解釋性、連貫性和適應性屬性。我們探討了與均衡計算中的索賠相關的初始激活層級的關鍵作用。使用人工智能輔助醫療決策支援系統作為案例研究，我們說明了不同的初始化如何導致不同的責任分配。該架構提供了對人工智能引發事件中問責制的寶貴見解，通過持續監控、修訂和反思，促進建立一個可持續且具有韌性的系統。
...
摘要：人工智慧透過預測模型協助醫療專業人員，大幅轉變臨床決策。本研究探討在醫療保健中使用人工智慧應用程式時，公平性和可解釋性的重要需求，以確保不同病患人口統計資料的公平結果。我們專注於敗血症相關死亡率的預測模型，提出一個方法，學習一個效能最佳化的預測模型，然後採用轉移學習流程，產生一個具有較佳公平性的模型。我們的模型也引進一個新的基於置換特徵重要性演算法，旨在闡明每個特徵在提升預測公平性上的貢獻。與現有專注於解釋特徵對預測效能的貢獻的可解釋性方法不同，我們提出的方法獨特地彌補了了解每個特徵如何影響公平性的差距。這項進展至關重要，因為敗血症的死亡率很高，且在三分之一的醫院死亡中扮演角色。我們的模型不僅有助於識別和減輕預測模型中的偏差，也透過改善模型預測的透明度和公平性，促進醫療保健利害關係人之間的信任，進而促成更公平且值得信賴的醫療保健服務。
...
摘要：<paragraph>憂鬱症是現今社會中的重要議題。根據世界衛生組織 (WHO) 的資料，2023 年有超過 2.8 億人與憂鬱症搏鬥。這是一個龐大的數字，若不認真看待，這些數字將會快速增加。大約有 48.9 億人是社群媒體的使用者。人們在 Twitter、Facebook、Reddit、Instagram 等平台上表達自己的感受與情緒。這些平台包含有價值的資訊，可用於研究目的。許多研究已在各種社群媒體平台上進行。然而，這些努力中仍存在某些限制。特別是，先前的研究僅專注於偵測推文中的憂鬱症和憂鬱症的嚴重程度。此外，資料集標籤也存在不準確的情況。在這項研究工作中，使用基於詞彙標籤的 Twitter 資料庫中的推文預測了五種類型的憂鬱症（躁鬱症、重度憂鬱症、精神病性憂鬱症、非典型憂鬱症和產後憂鬱症）。可解釋的 AI 用於透過突顯代表憂鬱症類型的推文部分來提供推理。雙向編碼器表徵（BERT）用於特徵提取和訓練。機器學習和深度學習方法用於訓練模型。BERT 模型呈現出最有希望的結果，達到 0.96 的整體準確度。</paragraph>
...
摘要：深度學習徹底改變了醫學影像和放射學領域，使我們能夠辨識醫學影像中的病理，包括電腦斷層掃描 (CT) 和 X 光掃描。然而，深度學習模型的效能，特別是在分割任務中，往往受到廣泛註解資料集需求的限制。為了應對這項挑戰，透過可解釋 AI 和反事實解釋的產生，我們探索了弱監督語意分割的能力。本研究的範圍是開發一種新穎的反事實內繪方法 (COIN)，它透過使用生成模型將預測的分類標籤從異常翻轉為正常。例如，如果分類器將輸入醫學影像 X 視為異常，表示存在病理，生成模型的目標是內繪異常區域，從而逆轉分類器的原始預測標籤。這種方法使我們能夠為病理產生精確的分割，而不需要依賴於預先存在的分割遮罩。至關重要的是，我們利用了影像層級標籤，與建立詳細的分割遮罩相比，這些標籤顯著更容易取得。該方法的有效性透過分割愛沙尼亞塔爾圖大學醫院取得的 CT 影像中的合成目標和實際腎臟腫瘤來證明。研究結果表明，COIN 遠遠優於已建立的歸因方法，例如 RISE、ScoreCAM 和 LayerCAM，以及 Singla 等人提出的另一種反事實解釋方法。這些證據表明，COIN 是一種類似於 CT 影像中腫瘤語意分割的有前景方法，並朝著讓深度學習應用在標註資料稀缺的醫療保健領域更易於使用和更有效邁進了一步。
...
摘要：甲狀腺癌是一種日益嚴重的全球健康問題，需要進階的診斷方法。本篇評論探討了人工智慧和放射組學在甲狀腺癌診斷中的應用。在符合 PRISMA 指南的情況下，我們回顧了多個資料庫，時間截至 2023 年 10 月。我們結合多個關鍵字，發現了與甲狀腺癌和相關主題相關的英文學術出版品。在移除 109 篇重複文獻後，原始搜尋結果共回傳了 267 篇論文。在根據預先確定的標準審查了論文摘要和標題後，我們淘汰了 124 篇文章，並選出了相關的研究。在進行綜合分析後，我們又排除了六篇研究。在納入的 28 篇研究中，結合超音波 (US) 影像的放射組學分析證明了其在診斷甲狀腺癌方面的有效性。我們記錄了各種結果，其中一些研究提出了優於現狀的新策略。文獻強調了人工智慧模型面臨的各種挑戰，包括可解釋性問題、資料集限制和操作員依賴性。28 篇納入研究的綜合發現提到，需要透過標準化工作和前瞻性多中心研究來解決這些問題。此外，我們也找出克服這些障礙的方法，例如可解釋人工智慧技術和個人化醫療技術的進展。本篇評論重點探討了人工智慧和放射組學如何轉化甲狀腺癌的診斷和治療。儘管存在挑戰，未來針對多學科合作、臨床適用性驗證和演算法改進的研究仍有潛力改善甲狀腺癌治療的患者預後和診斷精準度。
...
摘要：近年來，乳癌的盛行率迅速增加，使其成為全球主要的死亡原因之一。在所有癌症中，這迄今為止是最常見的。手動診斷此疾病需要大量的時間和專業知識。由於乳癌的檢測是一個耗時的過程，因此透過建立基於機器學習的預測，可以協助防止其進一步擴散。機器學習和可解釋 AI 在分類中至關重要，因為它們不僅提供準確的預測，還提供有關模型如何做出決策的見解，有助於理解和信任分類結果。在本研究中，我們評估並比較了五種不同的機器學習方法的分類準確度、精確度、召回率和 F-1 分數，這些方法使用了一個主要資料集（來自達卡醫學院醫院的 500 名患者）。五種不同的監督式機器學習技術，包括決策樹、隨機森林、邏輯迴歸、樸素貝氏和 XGBoost，已被用於在我們的資料集上取得最佳結果。此外，本研究將 SHAP 分析應用於 XGBoost 模型，以解釋模型的預測並了解每個特徵對模型輸出的影響。我們比較了幾種演算法對資料進行分類的準確度，並與該領域的其他文獻進行了對比。經過最後評估，本研究發現 XGBoost 達到了最佳模型準確度，為 97%。
...
摘要：深度學習 (DL) 用於診斷乳房攝影中的乳癌模型通常以「黑盒子」運作，這讓醫療保健專業人員難以信任和理解他們的決策過程。這項研究提出了一個整合架構，結合卷積神經網路 (CNN) 和可解釋人工智慧 (XAI) 來增強使用 CBIS-DDSM 資料集診斷乳癌。此方法包含一個精密的資料前處理管線和進階資料擴充技術，以因應資料集限制，並使用預先訓練的網路（例如 VGG-16、Inception-V3 和 ResNet）進行遷移學習。我們研究的重點在於評估 XAI 在解釋模型預測中的效能，重點在於利用豪斯多夫測量來評估 AI 生成的解釋與專家註解之間的對齊程度。這種方法對於 XAI 在促進 AI 輔助診斷中的可信度和道德公平性至關重要。我們研究的結果說明了 CNN 和 XAI 在推進乳癌診斷方法中的有效協作，從而促進了先進 AI 技術在臨床環境中的更順暢整合。透過增強 AI 驅動決策的可解釋性，這項工作為 AI 系統和醫療從業人員之間的改進協作奠定了基礎，最終豐富了患者照護。此外，我們研究的影響遠遠超出了當前的研究方法。它鼓勵進一步研究如何結合多模式資料和改進 AI 解釋，以滿足臨床實務的需求。
...
摘要：以人为本的可解释 AI (HCXAI) 主张将社会层面整合到 AI 解释中。HCXAI 话语的核心是社会透明度 (ST) 框架，其目标是让 AI 系统的社会组织环境对用户来说触手可及。在这项工作中，我们建议扩展 ST 框架以解决大型语言模型 (LLM) 中社会错误归因的风险，尤其是在心理健康等敏感领域。事实上，LLM 能够出色地模拟角色和人物，这可能会导致设计者的意图与用户对社会属性的看法之间出现错位，从而有可能助长情绪操纵和危险行为、认知不公正和不合理的信任。为了解决这些问题，我们建议用第五个“W 问题”来增强 ST 框架，以澄清设计者和用户赋予 LLM 的具体社会属性。这一补充旨在弥合 LLM 能力和用户看法之间的差距，促进基于 LLM 的技术的道德负责开发和使用。
...
摘要：<paragraph>背景：氣胸是一種由肺部和胸壁之間異常的空氣聚集所引起的急性胸腔疾病。為了解決深度學習 (DL) 模型經常出現的不透明性，可解釋的人工智慧 (XAI) 方法已被引入，用於概述與 DL 模型做出的氣胸診斷相關的區域。然而，這些解釋有時會與實際病灶區域不同，這凸顯了進一步改進的必要性。方法：我們提出了一種模板引導方法，將氣胸的臨床知識整合到 XAI 方法產生的模型解釋中，從而提高這些解釋的品質。利用放射科醫師所建立的一個病灶描繪，我們的做法首先產生一個模板，代表氣胸發生的潛在區域。然後將此模板疊加在模型解釋上，以過濾掉超出模板邊界的無關解釋。為了驗證其效能，我們對三種 XAI 方法進行了比較分析，在兩個真實世界資料集中解釋了兩個 DL 模型，並使用了我們的模板指導和未使用的狀況。結果：所提出的方法在建立在三種 XAI 方法、兩個 DL 模型和兩個資料集上的十二個基準情境中，始終改善了基準 XAI 方法。在比較模型解釋和真實病灶區域時，透過效能改善相對於基準效能計算出的平均增量百分比，在交集並集 (IoU) 中為 97.8%，在 Dice 相似性係數 (DSC) 中為 94.1%。結論：在氣胸診斷的背景下，我們提出了一種模板引導的方法來改善 AI 解釋。我們預期我們的模板指導將透過整合臨床領域專業知識，為闡明 AI 模型建立一種新的方法。</paragraph>
...
摘要：<paragraph>在當前機器翻譯 (MT) 領域中，Transformer 架構脫穎而出，成為黃金標準，特別適用於資源豐富的語言對。本研究深入探討其對資源匱乏的語言對的效能，包括英語 ↔ 愛爾蘭語和英語 ↔ 馬拉提語語言對。值得注意的是，本研究識別出最佳超參數和子詞模型類型，以顯著提升 Transformer 模型對資源匱乏語言對的翻譯品質。
  平行資料集的稀缺性會阻礙資源匱乏語言的 MT 發展。為了解決此問題，開發了 gaHealth，這是愛爾蘭語的第一個健康資料雙語語料庫。專注於健康領域，使用此領域內資料集開發的模型，與 LoResMT2021 共享任務的模型相比，在 BLEU 分數方面展現出非常顯著的進步。隨後使用多維品質指標錯誤分類法進行的人工評估，展示了 Transformer 系統在減少準確度和流暢度錯誤方面，優於基於 RNN 的對應系統的卓越效能。
  此外，本論文介紹了 adaptNMT 和 adaptMLLM，這兩個開源應用程式專為神經機器翻譯模型的開發、微調和部署而簡化。這些工具大幅簡化了設定和評估流程，讓 MT 更容易讓開發人員和翻譯人員使用。值得注意的是，adaptNMT 以 OpenNMT 生態系統為基礎，透過強調模型開發的環境足跡，推廣生態友善的自然語言處理研究。與 LoResMT2021 共享任務的基準相比，adaptMLLM 微調 MLLM，證明了英語 ↔ 愛爾蘭語和英語 ↔ 馬拉提語這兩個資源匱乏語言對的翻譯效能有了進步。</paragraph>
...
視網膜影像和血管反映身體的微血管和巨血管健康狀況。它們可用於診斷糖尿病併發症，包括糖尿病視網膜病變 (DR)、神經病變、腎病變和動脈粥樣硬化性心血管疾病，以及預測心血管事件的風險。使用數位化視網膜影像進行高通量 DR 檢測的人工智慧 (AI) 啟用系統已在臨床上採用。除了 DR 篩檢之外，AI 整合也具有巨大的潛力來解決與糖尿病患者整體照護相關的挑戰。在這項工作中，我們旨在全面回顧有關基於視網膜影像的 AI 應用研究的文獻，這些研究與糖尿病診斷、預後和管理相關。我們將描述整體 AI 輔助糖尿病照護的發現，包括但不限於 DR 篩檢，並討論實施此類系統的障礙，包括與倫理、資料隱私、公平存取和可解釋性相關的問題。透過評估患者的健康狀態，同時考量糖尿病併發症以及未來心血管併發症的風險預測，AI 輔助視網膜影像分析有潛力成為糖尿病患者現代化個人化醫療的中心工具。
...
摘要：本研究从多方利益相关者的角度调查了不同人工智能 (AI) 应用在教育中的可接受性，包括学生、教师和家长。承认 AI 在教育中的变革潜力，它解决了与数据隐私、AI 代理、透明度、可解释性和 AI 的道德部署相关的问题。通过小插曲方法，向参与者展示了四种场景，其中 AI 的代理、透明度、可解释性和隐私被操纵。在每个场景之后，参与者完成了一项调查，调查了他们对 AI 的全局效用、个人效用、公正性、信心、风险以及在可用情况下使用每个场景的 AI 的意图的看法。收集的数据包括 1198 名多方利益相关者参与者的最终样本，通过合作伙伴机构和社交媒体活动分发，重点关注对四个 AI 使用案例的个人回应。对数据的调解分析表明，对 AI 的接受度和信任在不同利益相关者群体中差异很大。我们发现，AI 代理、透明度和可解释性的高低水平之间的关键调解因素，以及使用不同教育 AI 的意图，包括感知的全局效用、公正性和信心。该研究强调，接受 AI 教育是一个细致入微且多方面的议题，除了需要仔细考虑特定的 AI 应用及其特征外，还需要考虑不同利益相关者的看法。
...
摘要：遠端病人監控基於可穿戴單導程心電圖 (ECG) 裝置，對於早期偵測心臟疾病有顯著的潛力，特別是結合人工智慧 (AI) 方法進行自動化心臟疾病偵測。過去已有研究應用基於深度學習的 AI 方法進行心臟疾病偵測。然而，這些模型尚未被廣泛接受為臨床診斷的可靠輔助工具，部分原因是圍繞許多 AI 演算法的當前黑盒認知。特別是，需要找出 ECG 訊號中有助於做出準確診斷的主要特徵，從而增強模型的可解釋性。在本研究中，我們開發了一個視覺轉換器方法，以根據單導程 ECG 資料辨識心房顫動。還開發了一個殘差網路 (ResNet) 方法，以與視覺轉換器方法進行比較。這些模型應用於 Chapman-Shaoxing 資料集，以分類心房顫動，以及另一種常見心律不整，竇性心動過緩和正常竇性心律心跳。這些模型能夠辨識決定最終分類的心跳關鍵區域，並強調 P 波和 T 波，以及心跳持續時間和訊號振幅在區分正常竇性心律與心房顫動和竇性心動過緩中的重要性。
...
摘要：本論文介紹了一個使用先進大型語言模型 (LLM) 的憂鬱症偵測和治療新範例：生成式預先訓練Transformer 4 (GPT-4)、Llama 2 聊天機器人和 Gemini。這些 LLM 經過微調，具備診斷、解釋和建議治療憂鬱症的專業提示。一種獨特的 few-shot 提示方法增強了模型根據 DSM-5 標準分析和解釋憂鬱症狀的能力。在互動階段，這些模型從事同理心對話管理，汲取 PsychDB 和認知行為療法 (CBT) 指南等資源，促進與經歷重度憂鬱症的個體之間的支持性互動。此外，本研究引入了 Illuminate 資料庫，其中包含各種 CBT 模組，有助於提供個人化療法建議。本研究使用 F1 分數、精確度、召回率、餘弦相似度和面向召回率的摘要評估 (ROUGE) 等指標評估 LLM 效能，跨不同測試集驗證其有效性。這種綜合方法將尖端的 AI 與既定的心理方法相結合，在心理保健方面提供了新的可能性，並展示了 LLM 在革新憂鬱症診斷和治療策略方面的潛力。
...
摘要：<paragraph>人工智慧系統的說明很少會處理受演算法決策（ADM）影響的人們的資訊需求。傳達資訊與受影響利害關係人重視的資訊之間的差距，可能會阻礙理解和遵守法規架構，例如 AI 法。為了解決這個差距，我們提出了「XAI 新手問題庫」：受影響利害關係人資訊需求的目錄，涵蓋兩個 ADM 使用案例（就業預測和健康監控），涵蓋資料、系統脈絡、系統使用和系統規格類別。在訪談研究中收集資訊需求，參與者根據他們的詢問收到說明作為回應。參與者進一步報告他們的理解和決策信心，顯示雖然在收到說明後信心往往會增加，但參與者也遇到了理解上的挑戰，例如無法說明為什麼他們的理解感覺不完整。說明進一步影響了參與者對系統風險和好處的看法，他們會根據使用案例確認或改變這些看法。當風險被認為很高時，參與者表示特別有興趣了解意圖的說明，例如系統為何以及為了什麼目的而建立。透過這項工作，我們旨在透過在決定採用 ADM 系統時提供相關資訊和挑戰的概觀，來支援受影響的利害關係人納入可解釋性。最後，我們透過總結我們的發現，列出六項關鍵影響，為受影響利害關係人受眾的未來說明設計提供資訊。</paragraph>
...
摘要：人工智能 (AI) 的快速發展，特別是在大型語言模型 (LLM) 和生成式 AI 領域，為各個領域的應用開啟了新途徑，但其在商業教育中的作用仍未得到充分探討。本研究首次引入了基準，用於評估七個主要 LLM 的效能，包括 OpenAI 的模型 (GPT-3.5 Turbo、GPT-4 和 GPT-4 Turbo)、Google 的模型 (PaLM 2、Gemini 1.0 Pro) 和 Anthropic 的模型 (Claude 2 和 Claude 2.1)，這些模型將用於 GMAT 考試，這是研究生商業課程錄取過程中的一項重要考試。我們的分析顯示，大多數 LLM 的表現都優於人類考生，其中 GPT-4 Turbo 不僅優於其他模型，還超過了頂尖商學院的研究生平均分數。透過案例研究，本研究探討了 GPT-4 Turbo 解釋答案、評估回應、找出錯誤、調整說明和產生替代方案的能力。最新的 LLM 版本，GPT-4 Turbo、Claude 2.1 和 Gemini 1.0 Pro，與其前身相比，在推理任務方面表現出顯著的進步，突顯了它們在複雜問題解決方面的潛力。儘管 AI 在教育、評量和輔導方面的承諾很明確，但仍存在挑戰。我們的研究不僅闡明了 LLM 的學術潛力，也強調了在教育中謹慎開發和應用 AI 的必要性。隨著 AI 技術的進步，建立 AI 互動的架構和協定、驗證 AI 生成的內容的準確性、確保全球各地多元化學習者的存取權，並建立一個 AI 支援人類專業知識的教育環境至關重要。本研究為進一步探討負責任地使用 AI 以豐富教育體驗並改善考試準備和評量方法奠定了基礎。
...
摘要：<paragraph>預測加護病房 (ICU) 病患的院內死亡率是最終臨床結果的關鍵。AI 已展現出優異的準確度，但缺乏可解釋性。為了解決這個問題，本文提出了一個可解釋的多模態死亡率預測器 (X-MMP)，採用一種有效且可解釋的 AI 解決方案，透過多模態 ICU 資料預測院內死亡率。我們在架構中採用多模態學習，可以接收來自臨床資料的異質輸入並做出決策。此外，我們引進一種可解釋的方法，即層級傳播到 Transformer，作為 LRP 方法到 Transformer 的適當延伸，對多模態輸入產生解釋，並揭示歸因於預測的顯著特徵。此外，每個模態對臨床結果的貢獻都可以視覺化，協助臨床醫生了解決策背後的推理。我們根據 MIMIC-III 和 MIMIC-III 波形資料庫匹配子集建構一個多模態資料集。基準資料集上的綜合實驗證明，我們提出的架構可以實現合理的解釋，並具有競爭力的預測準確度。特別是，我們的架構可以輕鬆轉移到其他臨床任務，這有助於發現醫療保健研究中的關鍵因素。</paragraph>
...
摘要：在過去十年中，病理學中的人工智慧（AI）方法已大幅進展。然而，由於將研究結果轉化為臨床診斷產品的技術和法規障礙以及缺乏標準化介面等眾多挑戰，整合到常規臨床實務中的速度很慢。開放且與供應商無關的 EMPAIA 計畫解決了這些挑戰。在此，我們概述了 EMPAIA 的成就和經驗教訓。EMPAIA 整合了病理學 AI 生態系統的各個利害關係人，即病理學家、電腦科學家和產業。在密切合作下，我們制定了技術互操作性標準、AI 測試和產品開發建議，以及可解釋性方法。我們實施了模組化和開放原始碼的 EMPAIA 平臺，並成功整合了來自 8 家不同供應商的 14 個基於 AI 的影像分析應用程式，展示了不同的應用程式如何使用單一標準化介面。我們優先考慮需求，並在歐洲和亞洲的 14 個不同的病理實驗室中評估了 AI 在實際臨床環境中的使用。除了技術開發外，我們還為所有利害關係人建立了一個論壇，以分享數位病理學和 AI 的資訊和經驗。商業、臨床和學術利害關係人現在可以採用 EMPAIA 的通用開放原始碼介面，為大規模標準化和簡化流程提供了獨特的機會。需要進一步的努力才能有效且廣泛地建立 AI 協助在常規實驗室使用。為此，已成立非營利協會 EMPAIA International，作為一個永續基礎架構，以繼續標準化並支援廣泛的實施和倡導 AI 協助的數位病理學未來。
...
摘要：反事實解釋 (CE) 技術作為向與 AI 系統互動的使用者提供見解的一種方式而備受關注。雖然在醫學影像和自動駕駛等領域廣泛研究，但圖形反事實解釋 (GCE) 方法相對而言尚未得到充分探索。GCE 會產生一個與原始圖形相似的全新圖形，且具有基於底層預測模型的不同結果。在這些 GCE 技術中，儘管在其他領域（例如藝術風格和自然語言建模）中展示了令人印象深刻的成就，但植基於生成機制的技術卻受到相對有限的研究。對生成解釋器的偏好源於它們在推理期間產生反事實實例的能力，利用輸入圖形的自主獲取擾動。基於上述理由，我們的研究引入了 RSGG-CE，一種用於反事實解釋的新型穩健隨機圖形生成器，它能夠從學習到的潛在空間中產生反事實範例，考慮部分有序的生成序列。此外，我們進行了定量和定性分析，以比較 RSGG-CE 與 SoA 生成解釋器的效能，強調了它產生合理解釋候選的能力有所提升。
...
摘要：可解釋人工智慧的動機之一，是讓人類能對人工智慧模型的使用和部署做出更好、更明智的決策。但需要仔細評估，才能評估這個期望是否已達成。目前的評估主要著重於解釋演算法的特性，而涉及人類受試者的評估，通常會使用主觀問題來測試人類對解釋有用性的認知，而沒有以客觀的指標和測量為基礎。在這項工作中，我們評估了解釋是否能改善人類在機器學習模型開發的實際情境中做決策的品質。我們進行了一項混合方法使用者研究，其中包含影像資料，以評估 SmoothGrad、GradCAM 和一個神諭解釋所產生的顯著性圖，並針對兩個任務進行評估：模型選擇和反事實模擬。令我們驚訝的是，我們沒有發現任何證據顯示，當使用者獲得任何顯著性圖時，這些任務有顯著改善，即使是設計為易於理解且高度指示答案的合成神諭解釋也是如此。儘管如此，解釋確實有助於使用者更準確地描述模型。這些發現表明，對於基於顯著性的解釋，在實用性和潛在誤解方面應保持謹慎。
...
摘要：可解釋性和安全性會產生信任。這些需要一個模型來展現一致性和可靠性。為了達成這些，有必要使用並分析資料和知識，並搭配與 AI 應用相關的統計和符號 AI 方法 - 單獨使用任何一個方法都不會成功。因此，我們主張並試圖證明神經符號 AI 方法更適合用於建立一個值得信賴的 AI 系統。我們提出 CREST 架構，展示了一致性、可靠性、使用者等級的可解釋性和安全性如何建立在神經符號方法上，而該方法使用資料和知識來支援關鍵應用程式的需求，例如健康和福祉。本文專注於大型語言模型 (LLM)，作為 CREST 架構中選擇的 AI 系統。LLM 由於其在處理廣泛自然語言處理 (NLP) 場景中的多功能性，而獲得研究人員的極大關注。例如，ChatGPT 和 Google 的 MedPaLM 已成為提供一般和健康相關查詢資訊的高度有前途的平台。儘管如此，這些模型仍然是黑盒子，儘管已納入人類回饋和指令引導的調整。例如，儘管制定了安全防護措施，ChatGPT 仍會產生不安全的回應。CREST 提出了一種可行的途徑，在神經符號架構中利用程序和基於圖表的知識，以闡明與 LLM 相關的挑戰。
...
摘要：本研究调查了部署的人工智能 (AI) 模型在预测 COVID-19 大流行期间及之后的死亡率方面的性能、可解释性和稳健性。作为同类研究中的第一项研究，我们发现贝叶斯神经网络 (BNN) 和智能训练技术使我们的模型能够在数据发生重大变化的情况下保持性能。我们的结果强调了开发健壮的 AI 模型的重要性，即使在具有挑战性的条件下，这些模型也能够匹配或超越临床医生预测。我们对模型可解释性的探索表明，随机模型会生成更多样化和个性化的解释，从而突出了在现实世界的临床环境中提供详细且个性化见解的 AI 模型的必要性。此外，我们强调了量化 AI 模型中不确定性的重要性，这使临床医生能够根据可靠的预测做出更明智的决策。我们的研究提倡在医疗保健中优先考虑实施科学在 AI 研究中，并确保 AI 解决方案在现实世界的临床环境中实用、有益且可持续。通过解决医疗保健环境中的独特挑战和复杂性，研究人员可以开发出有效改善临床实践和患者预后的 AI 模型。
...
摘要：肺癌占英國癌症死亡人數的 21%，五年存活率很大程度上取決於發現癌症的階段。最近的研究已證明 AI 方法具有從常規掃描中準確及早診斷肺癌的能力。然而，由於缺乏可解釋的模型，這些證據尚未轉化為臨床實務。本研究探討了將變異自動編碼器 (VAE)，一種生成式 AI 模型，應用於肺癌病灶。所提出的模型是根據從 LIDC-IDRI 公共資料集中的 3D CT 掃描中提取的病灶進行訓練的。通過聚類探索了 VAE 產生的 2D 切片的潛在向量表示，以證明其品質，並用於肺癌診斷的 MLP 分類器模型，最佳模型達到了 AUC 0.98 和 93.1% 精度的最先進指標。聚類分析顯示，VAE 潛在空間根據有意義的特徵成分（包括腫瘤大小、形狀、患者和惡性類別）將惡性和良性病灶的資料集分開。我們還包括了標準高斯 VAE (GVAE) 和更新的狄利克雷 VAE (DirVAE) 的比較分析，後者用狄利克雷分佈取代先驗，以促進具有解開特徵表示的更具可解釋性的潛在空間。最後，我們展示了與臨床有意義的特徵變化相應的潛在空間穿越的潛力。
...
摘要：現有的圖像分類器輸出解釋工具可分為白盒，依賴於模型內部的存取，以及黑盒，與模型無關。隨著 AI 在醫療領域的使用增加，可解釋性工具的使用也隨之增加。現有醫學影像解釋工作重點在於白盒工具，例如 gradcam。然而，切換到黑盒工具有明顯的優點，包括能夠與任何分類器一起使用以及廣泛的黑盒工具選擇。在標準影像上，黑盒工具與白盒一樣精確。在本文中，我們比較了多種黑盒方法在腦癌 MRI 資料集上與 gradcam 的效能。我們證明大多數黑盒工具不適合解釋醫學影像分類，並詳細分析其缺點的原因。我們還表明，一種黑盒工具，基於因果可解釋性的 rex，與 \gradcam 的效能一樣好。
...
摘要：AI 開發社群正日益利用 Hugging Face 等託管中介機構，提供使用者上傳模型和訓練資料的簡易存取方式。這些模型市集降低了數十萬名使用者的技術部署障礙，但可能會被用於許多潛在有害且非法的用途。在本文中，我們說明 AI 系統（既可以「包含」內容，又可以作為開放式工具）提出迄今為止最棘手的平台治理挑戰之一的方式。我們提供了 Hugging Face、GitHub 和 Civitai 等三個說明性平台上數起事件的案例研究，以探討模型市集如何調整模型。根據此分析，我們概述了產業為因應調整需求而發展的重要（但仍然有限的）實務：授權、存取和使用限制、自動內容調整以及開放政策發展。儘管目前的政策挑戰相當嚴峻，我們最後提出一些想法，說明平台如何能更好地動員資源，作為謹慎、公平且適度的法規存取點。
...
摘要：<paragraph>背景和目標：通過提取這些資訊，機器或深度學習 (ML/DL) 基礎的自主數據分析工具可以協助臨床醫師和癌症研究人員從複雜的數據集中發現模式和關係。最近已發表許多基於 DL 的卵巢癌 (OC) 數據分析。這些分析在癌症的各個方面（例如，它們所探討的子領域和癌症類型）和數據分析特徵方面高度多元。然而，目前對於這些分析在這些特徵和 AI 保證 (AIA) 方面的全面理解仍然不足。這篇系統性回顧旨在透過檢視現有文獻並明確關注關鍵特徵和 AI 保證觀點，來填補這個缺口。方法：使用 PRISMA 架構在三個期刊資料庫中進行全面搜尋。分析僅納入 2015 年至 2023 年間發表於同行評審期刊的研究。結果：在回顧中，總共檢視了 96 項由 DL 驅動的分析。研究結果揭露了幾個關於由 DL 驅動的卵巢癌數據分析的重要見解：- 大多數研究 71%（96 項中的 68 項）專注於檢測和診斷，而沒有研究探討 OC 的預測和預防。- 這些分析主要基於來自非多元族群的樣本（75%（96 項研究中的 72 項）），僅限於特定地理位置或國家。- 只有少部分研究（僅 33%（96 項研究中的 32 項））執行整合分析，其中大多數使用同質數據（臨床或組學）。- 值得注意的是，只有 8.3%（96 項研究中的 8 項）使用外部和多元數據集驗證其模型，突顯出增強模型驗證的必要性，而且 - 在癌症數據分析中納入 AIA 仍處於非常早期的階段；只有 2.1%（96 項研究中的 2 項）透過可解釋性明確探討 AIA。</paragraph>
...
摘要：<paragraph>可解釋性是深度學習中長久以來的挑戰，尤其是在醫療保健等高風險領域中。常見的可解釋性方法會強調驅動 AI 模型決策的影像區域。然而，人類高度依賴語言來傳達解釋，不只說明「在哪裡」，還包括「是什麼」。此外，大多數的可解釋性方法專注於解釋個別的 AI 預測，而不是描述 AI 模型一般使用的特徵。後者對於模型和資料集稽核特別有用，甚至可能在 AI  zuneh使用於新穎任務時產生知識。在此，我們提出一個可解釋性策略，它使用視覺語言模型來識別視覺分類任務的基於語言的描述符。透過利用影像和文字之間預先訓練的聯合嵌入空間，我們的做法將新的分類任務估計為一個線性詞語組合，結果是每個詞語的權重，表示其與基於視覺的分類器的對齊。我們使用兩個醫學影像分類任務來評估我們的做法，我們發現，儘管缺乏特定領域的語言訓練，但產生的描述符在很大程度上與臨床知識相符。然而，我們的做法也識別出在所使用的公開資料集中的「捷徑連接」的可能性。為了達成可解釋性的功能性衡量，我們進行了一項試驗性讀者研究，我們發現，AI 識別的詞語可以使非專家人類在非平凡的層級上執行專業的醫療任務。總之，我們的結果強調了使用多模態基礎模型來提供直觀的、基於語言的視覺任務解釋的潛力。</paragraph>
...
摘要：<paragraph>以醫療影像訓練的人工智慧 (AI) 模型，常會在臨床任務中表現出偏見，例如不同次群組之間的效能差異。由於並非所有真實世界醫療影像資料的偏見來源都容易辨識，因此要全面評估這些偏見是如何編碼到模型中，以及偏見緩解方法在改善效能差異方面的能力，是一項挑戰。在本文中，我們提出一個新穎的分析架構，用於系統且客觀地探討醫療影像中的偏見對 AI 模型的影響。我們開發並測試了這個架構，用於進行受控的電腦模擬試驗，以評估醫療影像 AI 中的偏見，使用一個工具來產生具有已知疾病影響和偏見來源的合成磁共振影像。透過使用三種反事實偏見情境來衡量模擬偏見效應對卷積神經網路 (CNN) 分類器的影響，以及三種偏見緩解策略的效能，展示了可行性。分析顯示，當 CNN 在合成資料集上訓練時，模擬偏見會導致預期的次群組效能差異。此外，重新加權被認為是此設定中最成功的偏見緩解策略，我們展示了解釋性 AI 方法如何協助使用這個架構來探討模型中偏見的表現。開發公平的 AI 模型是一項相當大的挑戰，因為醫療影像資料集中可能存在許多且通常未知的偏見來源。在這項工作中，我們提出一個新穎的方法來客觀地研究偏見和緩解策略對深度學習管線的影響，這有助於開發健全且負責任的臨床 AI。</paragraph>
...
摘要：機器學習在自動預測中風後的症狀及其對復健的反應方面具有巨大的潛力。此項工作的重大挑戰包括神經影像數據的維度非常高、可用於學習的數據集規模相對較小，以及如何有效地結合神經影像和表格數據（例如人口統計信息和臨床特徵）。本文基於兩種策略評估了多種解決方案。第一個是使用摘要 MRI 掃描的 2D 圖像。第二個是選擇提高分類準確度的關鍵特徵。此外，我們引入了在圖像上訓練卷積神經網路 (CNN) 的新方法，該圖像結合了從 MRI 中提取的感興趣區域，以及表格數據的符號表示。我們評估了一系列 CNN 架構（2D 和 3D），這些架構在 MRI 和表格數據的不同表示上進行訓練，以預測中風後口述圖片描述能力的綜合測量是否在失語或非失語範圍內。MRI 和表格數據來自 758 名參加 PLORAS 研究的英語中風倖存者。僅病灶大小的基線邏輯迴歸分類準確度為 0.678，當依次添加初始症狀嚴重程度和恢復時間時，準確度上升至 0.757 和 0.813。在從每個 MRI 掃描中提取 8 個感興趣區域並在 2D 殘差神經網路中與病灶大小、初始嚴重程度和恢復時間相結合時，觀察到最高的分類準確度 0.854。我們的研究結果展示了即使在機器學習術語中數據集很小的情況下，如何結合影像和表格數據以獲得高於中風後的分類準確度。最後，我們提出如何改進當前模型，以使用來自醫院掃描儀的圖像實現更高層級的準確度。
...
摘要：可解釋人工智慧 (XAI) 已成為處理任務關鍵應用程式時的一項基本需求，確保所採用的黑盒 AI 模型具有透明性和可解釋性。XAI 的重要性涵蓋從醫療保健到金融的各種領域，在這些領域中，了解深度學習演算法的決策過程至關重要。大多數基於 AI 的電腦視覺模型通常是黑盒子；因此，在影像處理中提供深度神經網路的可解釋性對於它們在醫學影像分析、自動駕駛和遙測應用中的廣泛採用和部署至關重要。最近，已經引入了幾種用於影像分類任務的 XAI 方法。相反地，影像分割在可解釋性的背景下受到的關注相對較少，儘管它在電腦視覺應用中是一項基本任務，特別是在遙測中。只有少數研究提出用於影像分割的基於梯度的 XAI 演算法。本文採用了最近的無梯度 Sobol XAI 方法進行語意分割。為了衡量 Sobol 方法在分割中的效能，我們提出了一種基於可學習雜訊模型的定量 XAI 評估方法。此模型的主要目的是在解釋圖上產生雜訊，其中較高的誘導雜訊表示較低的準確度，反之亦然。進行基準分析以評估和比較三種 XAI 方法的效能，包括 Seg-Grad-CAM、Seg-Grad-CAM++ 和 Seg-Sobol，使用所提出的基於雜訊的評估技術。這構成了使用高解析度衛星影像執行和評估 XAI 方法的首次嘗試。
...
摘要：大型語言模型在多個領域中迅速擴散，時間很短。然而，由於事實、連貫性和幻覺等問題，在醫療和保健領域對其採用猶豫不決。鑑於醫療保健的高風險性質，許多研究人員甚至警告不要使用它，直到這些問題得到解決。在醫療保健中實施和部署 LLM 的關鍵是讓這些模型值得信賴、透明（儘可能透明）且可解釋。在本文中，我們描述了在醫療保健中採用可靠、值得信賴和無偏見模型的關鍵要素，作為其必要條件。具體來說，我們專注於醫療保健背景下幻覺的量化、驗證和減輕。最後，我們討論了 LLM 在醫療保健中的未來可能是什麼樣子。
...
摘要：人工智慧（AI）正以快速的速度進步，現在準備部署在各種應用程式中，例如自動系統、醫療診斷和自然語言處理。早期採用 AI 技術於實際應用並非沒有問題，特別是神經網路，它可能不穩定且容易受到對抗性範例的影響。從長遠來看，需要開發適當的安全保證技術，以減少由於可避免的系統故障而造成的潛在危害，並確保可信度。本文專注於認證和可解釋性，概述了已開發出來以確保 AI 決策安全性的技術，並討論未來的挑戰。
...
摘要：歐洲議會議會研究服務總局已為歐洲議會議員準備了一份報告，其中列舉了人工智能 (AI) 在醫療保健中的七項主要風險：因 AI 錯誤造成的患者傷害、醫療 AI 工具的濫用、AI 中的偏見和現有不平等的延續、缺乏透明度、隱私和安全問題、問責差距以及實施障礙。
  在這項研究中，我們提出了十四項功能性要求，AI 系統可以實施這些要求來降低與其醫療目的相關的風險：AI 護照、使用者管理、法規檢查、僅限學術用途免責聲明、資料品質評估、臨床醫生雙重檢查、持續效能評估、稽核追蹤、持續可用性測試、回顧回溯/模擬案例、偏見檢查、可解釋的 AI、加密和使用經過現場測試的程式庫，以及語意互操作性。
  我們此處的目的是提供技術解決方案的具體高階規格，以確保持續良好的效能，並使用 AI 系統使患者受益，符合未來的歐盟法規架構。
...
摘要：人工智慧技術可被用於分類病患的生理活動，並預測遠距監控病患的重要生命徵象。基於非線性模型的回歸分析，例如深度學習模型，由於其黑箱性質，可解釋性有限。這可能會要求決策者根據非線性模型結果盲目地做出信仰上的飛躍，特別是在醫療保健應用中。在非侵入性監控中，來自追蹤感測器的病患資料及其誘發的臨床屬性作為輸入特徵，用於預測未來的生命徵象。解釋各種特徵對監控應用程式整體輸出的貢獻，對於臨床醫生的決策至關重要。在本研究中，提出了一個可解釋人工智慧的定量分析 (QXAI) 架構，其中包含事後模型可解釋性和內在可解釋性，用於監督式學習方法中的回歸和分類任務。這透過利用 Shapley 值概念並將注意力機制納入深度學習模型中來實現。我們採用人工神經網路 (ANN) 和基於注意力的雙向 LSTM (BiLSTM) 模型，根據感測器資料預測心率並分類生理活動。深度學習模型在預測和分類任務中都達到了最先進的結果。對輸入資料進行全局解釋和局部解釋，以了解各種病患資料的特徵貢獻。所提出的 QXAI 架構使用 PPG-DaLiA 資料評估，以預測心率，並使用行動健康 (MHEALTH) 資料根據感測器資料分類生理活動。蒙地卡羅近似應用於架構，以克服 Shapley 值計算所需的時間複雜度和高運算能力需求。
...
摘要：在可解釋的人工智慧 (XAI) 研究中，主要的重點在於為專家和從業人員解釋模型。模型不可知論和局部解釋方法在許多應用中被認為是可以解釋的且足夠的。然而，在醫療保健等領域，最終使用者是沒有 AI 或領域專業知識的患者，因此迫切需要更易於理解且能建立對模型運作的信任的模型解釋。我們假設生成具有敘述性、特定於患者和全局性的（模型的整體性）模型解釋將能夠提高可理解性並使決策制定成為可能。我們使用決策樹模型來測試這一點，為被確定為患有冠狀動脈疾病高風險的患者生成局部和全局解釋。這些解釋會提供給非專家使用者。我們發現對於特定類型的解釋有強烈的個人偏好。大多數參與者偏好全局解釋，而較小的一組則偏好局部解釋。基於任務對這些參與者的心理模型的評估提供了有價值的回饋，以增強敘述性全局解釋。反過來，這指導了既值得信賴又可操作的健康資訊系統的設計。
...
摘要：<paragraph>可解釋的人工智慧在醫學影像中至關重要。在具有挑戰性的神經科學領域中，視覺主題呈現出高度的複雜性，特別是在三維空間中。神經科學的應用涉及從 MRI 識別腦溝特徵，由於專家之間不同的註解協定和腦部複雜的三維功能，因此面臨重大的障礙。因此，傳統的可解釋性方法在有效驗證和評估這些網路時有所不足。為了解決此問題，我們首先提出一個數學公式，說明各種電腦視覺任務中不同類別的解釋需求，根據驗證協定的可靠性，將其分類為自解釋、半解釋、非解釋和新模式學習應用。關於這個數學公式，我們提出一個 3D 可解釋性框架，旨在驗證深度學習網路在偵測腦回旁溝（paracingulate sulcus）的輸出，這是一個重要的腦部解剖特徵。該框架整合了局部 3D 解釋、透過降維的全局解釋、串接的全局解釋和統計形狀特徵，揭示了模式學習的新見解。我們在具有挑戰性的 TOP-OSLO 資料集上訓練和測試了兩個先進的 3D 深度學習網路，顯著提高了腦溝偵測的準確度，特別是在左半球。在使用此資料集的不同註解協定進行評估期間，我們強調了無偏註解程序在我們提出的 3D 框架中實現精確預測和有效模式學習的關鍵作用。所提出的框架不僅標註了可變腦溝，還揭示了隱藏的人工智慧知識，有望促進我們對腦部解剖和功能的理解。</paragraph>
...
摘要：電子健康紀錄 (EHR) 和例行文件編寫實務在病患的日常照護中扮演至關重要的角色，提供全面的健康、診斷和治療紀錄。然而，複雜且冗長的 EHR 敘述會讓醫療保健提供者超載，導致診斷不準確。儘管大型語言模型 (LLM) 已在各種語言任務中展現其潛力，但在醫療保健領域的應用需要確保將診斷錯誤降到最低，並防止病患受到傷害。在本文中，我們概述了一種創新的方法，用於提升 LLM 在自動化診斷生成領域的熟練度，這是透過整合醫學知識圖譜 (KG) 和一種新穎的圖譜模型：Dr.Knows，靈感來自於臨床診斷推理流程。我們從美國國家醫學圖書館的統一醫學語言系統 (UMLS) 中衍生出 KG，這是一個強大的生物醫學知識儲存庫。我們的做法否定了預訓練的需要，而是將 KG 作為輔助工具，協助解釋和總結複雜的醫學概念。使用真實世界的醫院資料集，我們的實驗結果證明，將 LLM 與 KG 結合的提議方法有潛力提升自動化診斷生成的準確度。更重要的是，我們的方法提供了一條可解釋的診斷途徑，讓我們更接近實現 AI 輔增的診斷決策支援系統。
...
摘要：現有的用於診斷膝骨關節炎 (OA) 的人工智慧 (AI) 模型因缺乏透明度和可解釋性而受到批評，儘管它們達到了接近醫療專家的表現。這種不透明性讓它們難以在臨床實務中獲得信任。最近，可解釋人工智慧 (XAI) 已成為一種專門技術，它能透過揭露預測的推導方式來提供對模型預測的信心，從而促進 AI 系統在醫療保健中的使用。本文提供了第一份針對用於膝骨關節炎診斷的 XAI 技術的調查。XAI 技術從兩個角度進行討論：資料可解釋性和模型可解釋性。本文的目的是提供有價值的見解，了解 XAI 在更可靠的膝骨關節炎診斷方法中的潛力，並鼓勵在臨床實務中採用它。
...
摘要：近來，人工智慧應用於醫療保健的進展已展現出驚人的前景，有望在診斷和疾病預後方面超越人類表現。然而，隨著人工智慧模型的複雜性日益提高，人們開始關注其不透明性、潛在偏見，以及對可解釋性的需求。為了確保人工智慧系統的信任度和可靠性，特別是在臨床風險預測模型中，可解釋性變得至關重要。可解釋性通常是指人工智慧系統提供其決策邏輯或決策本身的穩健解釋的能力，以供人類利益相關者理解。在臨床風險預測中，可解釋性的其他方面，如公平性、偏見、信任度和透明度，也代表了超越可解釋性的重要概念。在本篇評論中，我們探討了這些概念之間的關係，因為它們通常一起或交互使用。本篇評論還討論了用於臨床風險預測的可解釋模型的最新進展，強調了在臨床實踐中多種常見模式中進行定量和臨床評估和驗證的重要性。它強調了外部驗證和結合多種可解釋性方法以增強信任度和公平性的必要性。採用嚴格的測試，例如使用具有已知生成因素的合成數據集，可以進一步提高可解釋性方法的可靠性。開放獲取和代碼共享資源對於透明度和可複製性至關重要，可以促進可解釋性研究的發展和可信度。儘管存在挑戰，但從臨床醫生到開發人員，採用端到端的可解釋性方法對於臨床風險預測的成功至關重要。
...
摘要：儘管人工智慧 (AI) 在醫療和保健領域有重大進展，但 AI 技術在實際臨床實務中的部署和採用仍有限。近年來，人們對醫療 AI 相關的技術、臨床、倫理和法律風險提出疑慮。為了增加實際採用率，醫療 AI 工具必須獲得病患、臨床醫師、醫療機構和主管機關的信任和接受。本研究描述 FUTURE-AI 指南，這是第一個國際共識架構，用於指導醫療保健領域中值得信賴的 AI 工具的開發和部署。FUTURE-AI 聯盟成立於 2021 年，目前由來自 51 個國家/地區的 118 位跨領域專家組成，代表所有洲，包括 AI 科學家、臨床醫師、倫理學家和社會科學家。在兩年的時間裡，該聯盟透過一項反覆運算的程序，包括深入的文獻回顧、修正的德爾菲調查和線上共識會議，定義了值得信賴的 AI 指導原則和最佳實務。FUTURE-AI 架構建立在醫療保健中值得信賴的 AI 的 6 項指導原則之上，即公平性、普遍性、可追溯性、可用性、穩健性和可解釋性。透過共識，定義了一組 28 項最佳實務，涵蓋技術、臨床、法律和社會倫理層面。這些建議涵蓋了醫療 AI 的整個生命週期，從設計、開發和驗證到法規、部署和監控。FUTURE-AI 是一個基於風險、無假設的指南，提供了一個結構化的方法來建構值得信賴、部署和採用於實際實務中的醫療 AI 工具。鼓勵研究人員在概念驗證階段考慮這些建議，以促進未來醫療 AI 的臨床實務轉譯。
...
摘要：醫療領域的人工智慧在醫療影像、病患照護和其他領域的新興應用中取得顯著進展。儘管這些應用在回顧性研究中已證明成功，但實際應用的案例卻很少。醫療人工智慧領域面臨各種挑戰，包括建立使用者信任、遵守法規、以合乎道德的方式使用資料。可解釋人工智慧 (XAI) 旨在讓人們理解人工智慧並相信其結果。本文針對醫療決策支援的 XAI 解決方案的最新發展，根據近年來發表的 198 篇文章的代表性樣本，提出文獻回顧。對相關文章的系統性綜合得出了以下幾個發現。(1) 這些解決方案大多採用與模型無關的 XAI 技術，(2) 深度學習模型的使用多於其他類型的機器學習模型，(3) 可解釋性用於促進信任，但很少有研究報告醫師參與迴圈，(4) 視覺化且互動式的使用者介面對於理解系統的解釋和建議更有用。需要更多醫療和人工智慧專家合作進行研究，這有助於引導設計、實作和評估醫療領域 XAI 解決方案的合適架構。
...
摘要：體外受精是治療不孕症最廣泛的方法之一。其中一項主要挑戰是評估和選擇胚胎進行植入，這個過程在臨床醫師之間和臨床醫師內部有很大的變異性。基於深度學習的方法正受到關注，但它們的不透明性質會影響它們在臨床環境中的接受度，因為在臨床環境中，決策的透明度至關重要。在本文中，我們分析了 AI 輔助胚胎分析模型的可解釋性方面的現有工作，並找出其限制。我們還會討論如何將這些模型整合到臨床環境中作為決策支援系統，同時考慮臨床醫師和患者的需求。最後，我們提出了一些準則，以提高可解釋性和可信度，推動這項技術朝向既定的臨床實務邁進。
...
摘要：在需求工程 (RE) 領域中，可解釋人工智慧 (XAI) 在將 AI 支援的系統與使用者需求、社會期望和法規標準相符方面的重要性日益增加，已獲得認可。一般來說，可解釋性已成為影響系統品質的重要非功能性需求。然而，可解釋性與效能之間的假定權衡挑戰了可解釋性的假定正面影響。如果滿足可解釋性的需求需要降低系統效能，那麼必須仔細考慮這些品質面向中的哪一個優先，以及如何在它們之間妥協。在本文中，我們批判性地審視了假定的權衡。我們主張最好以細緻入微的方式來處理，其中包含資源可用性、網域特性和風險考量。透過為未來的研究和最佳實務提供基礎，這項工作旨在推進 AI 的 RE 領域。
...
摘要：在需求工程 (RE) 領域內，可解釋人工智慧 (XAI) 在將 AI 支援系統與使用者需求、社會期望和法規標準相符中日益重要的意義已獲得認可。一般來說，可解釋性已成為影響系統品質的重要非功能性需求。然而，可解釋性和效能之間的假設權衡挑戰了可解釋性的假設正面影響。如果滿足可解釋性的需求需要降低系統效能，那麼必須仔細考慮這些品質面向中哪一個優先，以及如何在它們之間取得折衷。在本文中，我們批判性地審視了假設的權衡。我們主張最好以一種細緻的方式來處理它，其中包含資源可用性、領域特性和風險考量。透過提供未來研究和最佳實務的基礎，這項工作旨在推進 AI 的 RE 領域。
...
摘要：本文批判性地評估了歐盟委員會提出的 AI 法案，該法案針對高風險 AI 系統對基本權利和安全構成風險，提出了風險管理和風險可接受性的方法。該法案旨在以相應的監管負擔來促進「值得信賴」的 AI。其關於風險可接受性的條款要求盡可能降低或消除高風險系統的殘餘風險，同時考慮「技術水準」。此標準，特別是如果狹義解釋，是不可行的，既不能促進相應的監管負擔，也不能促進可信賴性。相比之下，議會最近針對風險管理條款提出的修正草案引入了「合理性」、成本效益分析，並且更透明地說明了風險可接受性判斷的價值觀和情境性質。本文認為，議會的方法更可行，並且能更好地平衡相稱性和可信賴性的目標。本文說明風險可接受性判斷中的合理性將會是什麼，並從過失法和歐洲醫療器械法規中汲取原則。本文主張，風險可接受性判斷的方法需要穩固的公民合法性基礎：包括監管機構的詳細指導或參與，以及受影響的利益相關者的有意義意見。
...
摘要：可解釋人工智慧 (XAI) 是機器學習中快速進展的領域，旨在解開複雜模型的預測。XAI 在敏感應用中特別需要，例如在醫療保健中，當診斷、建議和治療選擇可能依賴於人工智慧系統做出的決策時。AI 方法也已廣泛用於老化研究，特別是在開發生物時鐘模型和識別老化和年齡相關疾病的生物標誌物方面。然而，XAI 在此處的潛力尚未得到充分重視。我們討論了 XAI 在開發「老化時鐘」中的應用，並對按特定生理系統的重點分類的文獻進行了全面分析。
...
摘要：部分原型模型是可解釋設計的影像分類器，也是黑箱 AI 的有前途替代方案。本文探討可解釋機器學習的適用性和潛力，特別是 PIP-Net，在真實世界醫學影像資料的自動診斷支援上。PIP-Net 學習人類可理解的原型影像部分，我們評估其在骨折偵測和皮膚癌診斷上的準確性和可解釋性。我們發現 PIP-Net 的決策制定過程符合醫療分類標準，同時僅提供影像層級類別標籤。由於 PIP-Net 的原型無監督預訓練，因此可以輕鬆識別資料品質問題，例如 X 光中的不需要文字或標籤錯誤。此外，我們首次證明人類可以透過直接停用不需要的原型來手動修正 PIP-Net 的推理。我們得出結論，部分原型模型由於其可解釋性和進階模型除錯的潛力，對於醫療應用很有前景。
...
摘要：可解釋 AI (XAI) 是機器學習研究中日益重要的領域，旨在讓黑箱模型透明且可解釋。在本文中，我們提出了一種使用由特徵條件置換生成的所謂反事實路徑的新 XAI 方法。該演算法透過找出對模型預測變動影響最大的特徵順序置換來衡量特徵重要性。它特別適合根據納入領域知識的知識圖譜中的反事實路徑產生解釋。反事實路徑為目前的 XAI 方法加入了一個額外的圖形維度，用於解釋和視覺化黑箱模型。使用合成和醫療資料進行的實驗證明了我們方法的實用適用性。
...
摘要：人工智慧（AI）的可解釋性領域見證了越來越多的研究和學術興趣。然而，在解釋機器學習演算法結果時，缺乏對人類友善和個別的解釋，顯著地阻礙了臨床醫生在研究和臨床實務中接受這些方法。為了解決這個問題，我們的研究使用反事實解釋來探討「如果？」情境在醫學研究中的適用性。我們的目標是擴展我們對磁共振造影（MRI）特徵的理解，這些特徵用於診斷小兒後顱窩腦瘤，超越現有的界限。在我們的案例研究中，提出的概念提供了一種新方法來檢視替代決策情境，這些情境提供個人化和特定於情境的見解，使我們能夠驗證預測並釐清在不同情況下的變化。此外，我們探討了反事實用於資料擴充的潛在用途，並評估它們作為我們醫學研究案例中替代方法的可行性。結果證明了使用反事實解釋來增強臨床研究中 AI 驅動方法的可接受性的潛力。
...
摘要：人工智能領域的進展已導致開發出各種由 AI 驅動的失智症評估，可用於識別失智症早期患者。它可以徹底改變失智症照護環境。醫學界必須了解各種 AI 評估，並根據其有效性、效率、實用性、可靠性和準確性程度，選擇用於早期識別失智症患者 (PwD) 的評估。另一方面，AI 開發人員也應了解各種非 AI 評估以及最近開發的 AI 評估。因此，這篇臨床醫生和 AI 工程師都可以閱讀的論文填補了文獻中對臨床醫生解釋現有失智症識別解決方案、以及對 AI 工程師解釋所用技術和最廣泛的失智症數據集的空白。它遵循對 AI 和非 AI 失智症評估論文的回顧，為 AI 和醫學界提供有關各種失智症評估的寶貴資訊。討論和結論重點介紹最顯著的研究方向和現有解決方案的成熟度。
...
摘要：<paragraph>可解释性对人工智能 (AI) 技术构成了重大挑战。当前对可解释 AI (XAI) 的研究缺乏提取有关学习任务的全局知识的效率，因此存在不精确的显着性、缺乏上下文感知和含义模糊等缺陷。在本文中，我们提出了类别关联嵌入 (CAE) 方法来解决这些问题。我们采用编码器-解码器架构来嵌入样本特征，并将它们同时分成与类别相关的和与个体相关的样式向量。将给定样本的个体样式代码与另一个样本的类别样式代码重新组合，就会产生一个合成的样本，该样本保留了个人特征，但改变了类别分配，遵循循环对抗学习策略。类别关联嵌入将所有实例的全局类别相关特征提炼到一个统一的域中，该域在类别之间具有良好的分离性。然后可以提取不同类别之间的转换规则，并进一步应用于单个实例。然后，我们提出了一个主动 XAI 框架，该框架沿着引导路径操纵特定样本的类别样式向量朝向反类别，从而产生一系列具有相同个体特征的反例合成样本。将这些反事实样本与原始样本进行比较，可以直观地全局说明分类任务的性质。我们采用该框架进行医学图像分类任务，结果表明，与现有方法相比，可以获得更精确的显着性图，并具有强大的上下文感知表示。此外，可以通过遍历类别样式空间中的路径直接可视化疾病病理学。</paragraph>
...
摘要：提供基於機器學習的 AI 預測的高品質解釋是一項具有挑戰性和複雜性的任務。要順利進行，需要具備以下因素：選擇適當的解釋概括性/特殊性層級；考慮解釋受益人對所考慮的 AI 任務的熟悉程度的假設；參考有助於決策的特定元素；利用額外的知識（例如專家證據），這些知識可能不屬於預測過程的一部分；並提供支持否定假設的證據。最後，系統需要以清楚可解釋且可能令人信服的方式制定解釋。基於這些考量，ANTIDOTE 促成了可解釋 AI 的整合願景，其中深度學習過程的低階特徵與人類論證能力的高階架構相結合。ANTIDOTE 將利用深度學習和論證的跨領域能力，來支持對可解釋 AI 更廣泛且創新的觀點，其中對臨床案例審議進行高品質解釋的需求至關重要。作為該專案的第一個成果，我們發布了 Antidote CasiMedicos 資料集，以利於對一般可解釋 AI 以及特別是醫療領域中的論證進行研究。
...
摘要：近年來，圖神經網路的進展迅速，在藥物發現、醫療診斷和推薦系統方面有許多新發展。儘管進展顯著，許多網路都是「黑盒子」，對網路學習的「內容」了解甚少。許多高風險應用，例如藥物發現，需要模型提供人類可理解的解釋，以便使用者能辨識錯誤並發現新知識。因此，可解釋 AI 演算法的發展對我們獲取 AI 的好處至關重要。
我們提出了一種稱為 eXplainable Insight (XInsight) 的 GNN 可解釋性演算法，它使用 GFlowNets 產生模型解釋的分布。由於 GFlowNets 會產生與獎勵成正比的機率物件，因此與僅學習最大獎勵範例的先前方法相比，XInsight 能夠產生多樣化的解釋。我們透過產生在兩個圖形分類任務上訓練的 GNN 解釋來展示 XInsight：使用 MUTAG 資料集分類致突變化合物，以及使用我們已開源的合成資料集分類非環圖形。我們透過使用 QSAR 建模分析產生的化合物來展示 XInsight 解釋的效用，我們發現 XInsight 產生的化合物會根據親脂性進行分群，這是致突變性的已知相關性。我們的結果顯示，XInsight 會產生一個解釋的分布，揭示了模型所展示的底層關係。它們也突顯了產生多樣化解釋集的重要性，因為它能讓我們發現模型中的隱藏關係，並為進一步分析提供有價值的指導。
...
摘要：我們提出並實作一個可解釋的機器學習分類模型，以表達布林公式為基礎，用於可解釋 AI (XAI)。潛在應用包括信用評分和醫療狀況診斷。布林公式定義了一個具有可調整複雜性（或可解釋性）的規則，根據此規則對輸入資料進行分類。此類公式可以包含任何可應用於一個或多個布林變數的運算子，因此與更嚴格的基於規則和基於樹狀的方法相比，提供了更高的表達能力。分類器使用原生局部最佳化技術進行訓練，有效地搜尋可行公式的空間。淺層規則可以用快速整數線性規劃 (ILP) 或二次非約束二元最佳化 (QUBO) 求解器來確定，這些求解器可能由特殊用途硬體或量子裝置提供支援。我們將原生局部最佳化器的表達能力和效率與這些裝置的快速運作相結合，透過執行非局部移動來最佳化完整布林公式的子樹。我們提供了廣泛的數值基準測試結果，其中包含多個基線，這些基線建置在眾所周知的公開資料集上。根據結果，我們發現原生局部規則分類器通常與其他分類器具有競爭力。新增非局部移動以較少的反覆運算次數達成類似的結果，因此使用專用或量子硬體可以透過快速提出非局部移動來加速運算。
...
摘要：為了解決心理健康問題的全球挑戰，我們提出了一種基於邏輯神經網路 (LNN) 的神經符號人工智慧方法，用於診斷心理疾病。由於缺乏有效的心理疾病治療方法，因此需要一種可以協助治療師進行診斷的人工智慧解決方案。然而，目前的神經網路模型缺乏可解釋性，治療師可能無法信賴它們。LNN 是一種遞迴神經網路架構，結合了神經網路的學習能力和基於經典邏輯的人工智慧的推理能力。所提出的系統使用臨床訪談中的輸入謂詞來輸出心理疾病類別，並使用不同的謂詞剪枝技術來實現可擴充性和更高的分數。此外，我們提供了一個見解提取方法，以幫助治療師進行診斷。所提出的系統解決了當前神經網路模型缺乏可解釋性的問題，並為心理疾病診斷提供了一個更值得信賴的解決方案。
...
摘要：隨著機器學習模型在醫療診斷中越來越普遍，對可解釋性和透明度的需求變得至關重要。XAI 的復興標誌著該領域的重大轉變，旨在重新定義醫療診斷模型的可解釋性。本文探討了可解釋 AI (XAI) 領域內的創新方法和方法論，這些方法和方法論正在革新醫療診斷模型的可解釋性。通過闡明決策過程的基礎，XAI 技術使醫療保健專業人員能夠理解、信任並有效地利用這些模型進行準確可靠的醫療診斷。本綜述重點介紹了醫療診斷中 XAI 的關鍵進展及其轉變醫療保健領域的潛力，最終改善患者的治療結果並培養對 AI 驅動的診斷系統的信任。
...
摘要：<paragraph>在以高度連接性和流動性為特徵的環境中，加上心血管疾病激增，通過遠程監測心血管健康來減少醫療保健支出的必要性變得更加明顯。準確檢測和分類心律不整對於診斷心臟不規則的個人至關重要。本研究強調了在家中環境中使用心電圖 (ECG) 測量進行實時心律不整檢測的可行性。本文提出心律不整檢測的新應用，利用最先進的 You-Only-Look-Once (YOLO)v8 演算法對單導程 ECG 信號進行分類。我們引入了一個新的損失修改 YOLOv8 模型，並針對 MIT-BIH 心律不整資料集進行微調，實現實時連續監測。獲得的結果證實了我們方法的有效性，該模型在 NVIDIA Tesla V100 上達到了 99.5% 的平均準確度和 0.992 mAP@50，以及 0.002 秒的快速檢測時間。我們的調查例證了實時心律不整檢測的潛力，使用戶能夠在家中舒適地視覺化解讀模型輸出。此外，本研究為擴展到實時可解釋 AI (XAI) 模型奠定了基礎，該模型能夠部署在醫療保健領域，從而顯著推進醫療保健解決方案的領域。</paragraph>
...
摘要：乳癌 (BC) 仍然是一項重大的健康威脅，目前沒有長期可用的療法。早期檢測至關重要，但乳房攝影的判讀卻受到高假陽性和假陰性的影響。由於乳癌發生率預計將超越肺癌，因此改善早期檢測方法至關重要。熱像儀使用高解析度紅外線相機，特別是結合人工智慧 (AI) 時，提供了希望。這項研究提出了一個基於注意力的卷積神經網路用於分割，在乳癌檢測和分類中提供了更高的速度和精確度。該系統增強影像並執行可解釋 AI 的癌症分割。我們提出了一個基於Transformer注意力的卷積架構 (UNet) 用於故障識別，並使用梯度加權類別激活映射 (Grad-CAM) 來分析 UNet 架構中偏差和弱點的區域與 IRT 影像。與現有的深度學習架構相比，我們提出的架構的優越性得到了證實。
...
摘要：憂鬱症是最普遍且嚴重的精神疾病，會造成嚴重的財務和社會影響。憂鬱症偵測是早期介入以減輕這些後果的關鍵。如此高風險的決策本質上需要可解釋性。儘管一些憂鬱症偵測研究嘗試根據重要性分數或注意力權重來解釋決策，但這些解釋與基於憂鬱症狀的臨床憂鬱症診斷標準不一致。為了填補這個缺口，我們遵循計算設計科學範例來開發一個新穎的多尺度時間原型網路 (MSTPNet)。MSTPNet 創新地偵測和解釋憂鬱症狀以及它們持續多久。使用大規模資料集進行的廣泛實證分析顯示，MSTPNet 以 0.851 的 F1 分數優於最先進的憂鬱症偵測方法。此結果也揭示了問卷調查方法中未注意到的新症狀，例如分享對不同生活的欽佩。我們進一步進行使用者研究，以證明其在可解釋性方面優於基準。本研究以一個新穎的可解釋深度學習模型為社會媒體中的憂鬱症偵測貢獻 IS 文獻。在實務中，我們提出的方法可以實作在社交媒體平台中，以提供個人化的線上資源給偵測出的憂鬱症患者。
...
摘要：電子健康紀錄 (EHR) 是醫療保健中預期的由人工智慧 (AI) 推動的轉型的基本資料來源。然而，反映在 EHR 備註中的臨床偏見可能導致 AI 模型繼承並擴大這些偏見，使健康差距永續存在。本研究調查了 EHR 備註中汙名化語言 (SL) 對使用基於 Transformer 的深度學習模型和可解釋 AI (XAI) 技術進行死亡率預測的影響。我們的研究結果表明，臨床醫生所寫的 SL 會對 AI 效能產生不利影響，特別是對黑人患者而言，突顯 SL 是 AI 模型開發中種族差異的來源。為了探索一種在作業上有效率的方式來減輕 SL 的影響，我們透過臨床醫生的協作網路調查 SL 產生的模式，並找出中心臨床醫生對 AI 模型中的種族差異有較大的影響。我們發現，移除由中心臨床醫生撰寫的 SL 是一種比消除資料集中所有 SL 更有效率的偏見減少策略。本研究為負責任的 AI 開發提供了可行的見解，並有助於了解臨床醫生的行為和醫療保健中的 EHR 備註撰寫。
...
摘要：當代透過人工智慧的自動化包含了大量的幕後人力勞動，這通常既被隱形化又低於應得的報酬。由於隱形勞動，包括標籤和維護工作，是當代人工智慧系統不可或缺的一部分，因此讓使用者了解其角色仍然很重要。我們建議這可以透過可解釋人工智慧（XAI）設計來完成，特別是女性主義交叉學科 XAI。我們提出源自女性主義交叉學科研究的製圖方法，以描繪出人工智慧的系統觀點，並納入與隱形勞動相關的人工智慧面向。
...
摘要：虛擬心理健康助理 (VMHA) 持續進步，以支援每年有 6,000 萬初級保健就診和 600 萬急診室 (ER) 就診的過度負擔的全球醫療保健系統。這些系統是由臨床心理學家、精神科醫生和人工智慧 (AI) 研究人員為認知行為療法 (CBT) 所建置。目前，VMHA 的角色是透過資訊提供情緒支持，較少著重於與患者進行反思性對話。需要更全面、安全且可解釋的方法，才能建置負責任的 VMHA 來詢問後續問題或提供見多識廣的回應。這項調查提供了對現有心理健康對話代理的系統性批判性回顧，接著深入探討具備脈絡知識、資料集和在臨床決策支援中新興角色的 VMHA 改進。我們也提供了新的方向，以可解釋性、安全性與整體可信度來豐富 VMHA 的使用者體驗。最後，我們提供了評量指標和實務考量，以協助 VMHA 超越現有文獻，在主動溝通中建立 VMHA 和患者之間的信任。
...
摘要：XAI 指的是建立 AI 應用程式的技術和方法，協助最終使用者詮釋 AI 模型的輸出和預測。在高風險決策情境中，例如醫療領域的黑箱 AI 應用程式，增加了透明度和可解釋性的需求，因為錯誤的預測可能會造成嚴重的後果。模型可解釋性和可詮釋性對於 AI 模型在醫療實務中的成功部署至關重要。AI 應用程式的底層推論需要對臨床醫生透明，才能獲得他們的信任。本文針對醫療領域 XAI 面向和挑戰進行系統性回顧。本研究的主要目標是回顧各種 XAI 方法、它們的挑戰，以及相關的醫療保健機器學習模型。這些方法分為六類討論：面向特徵的方法、全域方法、概念模型、替代模型、局部基於像素的方法和以人為中心的方法。最重要的是，本文探討 XAI 在醫療保健問題中的角色，以釐清其在安全關鍵應用中的必要性。本文旨在透過回顧相關實驗結果，建立對醫療保健領域 XAI 相關應用程式的全面理解。為了促進未來研究填補研究差距，我們調查了 XAI 模型從不同觀點來看的重要性及其限制。
...
摘要：最先进的机器学习模型经常学习训练数据中嵌入的虚假相关性。当将这些模型用于高风险决策时，这会带来风险，例如在皮肤癌检测等医学应用中。为了解决这个问题，我们提出了 Reveal to Revise (R2R)，一个包含整个可解释人工智能 (XAI) 生命周期的框架，使从业者能够通过最少的人工交互迭代识别、缓解和（重新）评估虚假的模型行为。在第一步（1）中，R2R 通过找出归因中的异常值或通过检查模型学习的潜在概念来揭示模型的弱点。其次（2），检测负责的伪像并在输入数据中进行空间定位，然后利用它（3）修改模型行为。具体来说，我们应用 RRR、CDEP 和 ClArC 的方法进行模型校正，并（4）重新评估模型的性能和对伪像的剩余敏感性。使用两个用于黑素瘤检测和骨龄估计的医学基准数据集，我们将 R2R 框架应用于 VGG、ResNet 和 EfficientNet 架构，从而揭示和校正真实的、数据集固有的伪像以及受控设置中的合成变体。完成 XAI 生命周期，我们演示了多个 R2R 迭代以减轻不同的偏差。代码可在 https://github.com/maxdreyer/Reveal2Revise 上获得。
...
摘要：可解釋人工智慧 (XAI) 領域在近年已大幅進展，但進展主要發生在電腦視覺和自然語言處理。對於時間序列，其輸入通常無法解釋，僅有有限的研究可供 XAI 使用。在此研究中，我們提出一個虛擬檢查層，將時間序列轉換為可解釋的表示，並允許透過局部 XAI 方法（如逐層關聯性傳播 (LRP)）將關聯性歸因傳播到此表示。藉此，我們將 XAI 方法系列的適用性擴展到輸入僅在轉換後才能解釋的領域（例如語音）。在此，我們專注於傅立葉轉換，其顯著應用於時間序列和 LRP 的解釋，並將我們的方法稱為 DFT-LRP。我們展示 DFT-LRP 在各種時間序列分類設定中的效用，例如音訊和電子病歷。我們展示 DFT-LRP 如何揭露在不同領域（例如時間與頻率領域）訓練的模型的分類策略差異，或有助於發現模型如何針對資料中的虛假關聯作用。
...
摘要：解釋深度學習模型預測的能力對最終使用者而言，是一項重要的功能，可利用人工智慧 (AI) 的力量進行醫療決策程序，而醫療決策程序通常被認為不透明且難以理解。在本文中，我們應用最先進的可解釋人工智慧 (XAI) 方法來解釋黑箱 AI 模型在甲狀腺結節診斷應用中的預測。我們提出新的基於統計的 XAI 方法，即核密度估計和密度圖，以解釋未檢測到結節的情況。XAI 方法的效能會在定性和定量比較下被視為回饋，以改善資料品質和模型效能。最後，我們調查以評估醫師和患者對 XAI 解釋模型對甲狀腺結節影像決策的信任度。
...
摘要：醫療設備和人工智慧系統快速轉變醫療保健的提供方式。同時，由於其本質，醫療設備中或作為醫療設備的人工智慧可能會遭受網路攻擊，導致患者安全和安全風險。本章分為三部分。第一部分從設定場景開始，我們在其中說明網路安全在醫療保健中的作用。然後，當我們談論被視為醫療設備本身或支援醫療設備的人工智慧時，我們簡要定義我們所指的內容。為了說明此類醫療設備帶來的風險，我們提供了三個範例：資料集中毒、社會工程和資料或原始碼提取。在第二部分，本文概述了歐盟的監管架構，該架構與確保人工智慧作為或在醫療設備中的網路安全相關（MDR、NIS 指令、網路安全法案、GDPR、人工智慧法案提案和 NIS 2 指令提案）。最後，本文的第三部分探討了歐盟監管架構可能產生的挑戰。特別是，我們展望了源自兩個立法提案的挑戰，以及它們與現有關於人工智慧醫療設備網路安全的立法的互動。它們被結構化為對以下問題的回答：(1) 人工智慧法案將如何與 MDR 互動，就網路安全和安全要求而言？(2) 我們應如何解釋 NIS 2 指令提案和 MDR 的事件通知要求？(3) 關鍵基礎設施術語的演變會帶來什麼後果？[這是草稿章節。最終版本將出現在 Barry Solaiman 和 I. Glenn Cohen 編輯的《健康、人工智慧與法律研究手冊》中，2023 年出版，Edward Elgar Publishing Ltd]
...
摘要：許多視覺化已為可解釋 AI (XAI) 而開發，但它們通常需要使用者進一步推理才能詮釋。我們主張 XAI 應支援圖解和演繹推理，以便 AI 執行假設產生和評估，以縮小可解釋性差距。我們提出圖解化以 i) 執行皮爾士演繹推理，ii) 遵循領域慣例，以及 iii) 以視覺或口語方式使用圖表進行說明。我們實作 DiagramNet 進行臨床應用，以根據心臟聽診預測心臟診斷，並以基於形狀的雜音圖表進行說明。在建模研究中，我們發現 DiagramNet 不僅提供忠實的雜音形狀說明，而且比基線模型具有更好的預測效能。我們進一步在與醫學生的定性使用者研究中展示了圖解說明的可解釋性和可信度，說明臨床相關的圖解說明優於技術顯著性圖說明。這項工作有助於提供以使用者為中心的 XAI 的領域慣例演繹說明。
...
摘要：皮膚癌是人類癌症中最普遍的形式之一。它主要透過視覺識別，從臨床篩檢開始，並持續進行皮膚鏡檢查、組織學評估和標本收集。深度卷積神經網路 (CNN) 對分類的細粒度物件執行高度分離且潛在通用的任務。本研究提出了一個新的多類別預測架構，它根據 ViT 和 ViTGAN 對皮膚病灶進行分類。基於視覺轉換器的 GAN（生成對抗網路）用於處理類別不平衡。該架構包含四個主要階段：ViTGAN、影像處理和可解釋的人工智慧。第 1 階段包括產生合成影像，以平衡資料集中所有類別。第 2 階段包括應用不同的資料擴充技術和形態運算，以增加資料量。第 3 和第 4 階段涉及開發一個適用於邊緣運算系統的 ViT 模型，該模型可以識別模式，並對影像中可見使用者皮膚上的皮膚病灶進行分類。在第 3 階段，在使用 ViT 將病灶分類到所需的類別後，我們將使用可解釋的人工智慧 (XAI)，這將導致更具可解釋性的結果（使用啟動圖等），同時確保高度的預測準確度。皮膚病的即時影像可以用行動應用程式的相機由醫生或患者拍攝，以進行早期檢查並確定皮膚病灶的原因。整個架構與現有的皮膚病灶檢測架構進行比較。
...
摘要：<paragraph>對於人工智慧 (AI) 在高風險環境中（例如醫療保健）的部署，提供可解釋性/可說明性或允許精細錯誤分析的方法至關重要。許多最近的可解釋性/可說明性和精細錯誤分析方法使用概念，這些概念是對人類具有語義意義的元標籤。然而，只有少數數據集包含概念級別的元標籤，而且這些元標籤大多與不需要領域專業知識的自然圖像相關。專注於與單一疾病（例如黑色素瘤）相關的元標籤的醫學密集註釋數據集。在皮膚科中，皮膚疾病是使用既定的臨床詞彙描述的，該詞彙允許臨床醫生彼此描述身體檢查結果。為了提供一個由領域專家密集註釋的醫療數據集，其中註釋對多種疾病過程有用，我們開發了 SkinCon：一個由皮膚科醫生密集註釋的皮膚疾病數據集。SkinCon 包含來自 Fitzpatrick 17k 數據集的 3230 張圖像，其中密集註釋了 48 個臨床概念，其中 22 個概念至少有 50 張代表該概念的圖像。所使用的概念是由兩位皮膚科醫生考慮用於描述皮膚病變的臨床描述詞而選擇的。範例包括「斑塊」、「鱗屑」和「糜爛」。相同的概念也用於標記來自 Diverse Dermatology Images 數據集的 656 張皮膚疾病圖像，提供了一個具有不同膚色表現的額外外部數據集。我們回顧了 SkinCon 數據集的潛在應用，例如探測模型、基於概念的解釋和概念瓶頸。此外，我們使用 SkinCon 來展示這兩個用例中的兩個：使用概念調試現有皮膚科 AI 模型的錯誤，以及使用事後概念瓶頸模型開發可解釋模型。</paragraph>
...
摘要：不安的多臂強盜 (RMAB) 是一種流行的決策理論架構，已用於模擬公共衛生、野生動物保育、通訊系統等領域中的真實世界順序決策問題。已部署的 RMAB 系統通常分兩個階段運作：第一階段預測定義 RMAB 實例的未知參數，第二階段採用最佳化演算法來解決已建構的 RMAB 實例。在這項工作中，我們提供並分析了在公共衛生領域部署 RMAB 系統的首次部署結果，目的是改善孕產婦和兒童健康。我們的分析著重於了解預測準確度與已部署 RMAB 系統的整體效能之間的關係。這對於決定投資於提高預測準確度以改善最終系統效能的價值至關重要，並且對於診斷、監控已部署的 RMAB 系統很有用。使用我們已部署 RMAB 系統的真實世界資料，我們證明整體預測準確度的提升甚至可能伴隨著 RMAB 系統效能的下降——廣泛投資資源以提高整體預測準確度可能無法產生預期的結果。在此之後，我們開發了以決策為重點的評估指標來評估預測組成，並證明它更能（在經驗上和理論上）說明已部署 RMAB 系統的整體效能。
...
摘要：可解释人工智能包含开发机制，允许决策系统和人类进行交互，让前者的决策变得可理解。这在敏感环境中尤为重要，例如在医疗领域。我们提出一个用例研究，用于皮肤损伤诊断，说明如何为从业者提供对最先进的深度神经网络分类器的决策的解释，该分类器经过训练可以根据示例表征皮肤损伤。我们的框架包含一个训练过的分类器，一个解释模块在其上运行。后者能够为分类诊断提供示例和反例，从而允许医生与自动诊断系统进行交互。示例是通过对抗性自动编码器生成的。我们说明了系统在代表性示例上的行为。
...
摘要：<paragraph>人工智慧（AI）應用中的演算法公平性對於一個更美好的社會至關重要。公平性作為社會機制的基礎公理，包含多個面向。儘管機器學習（ML）社群已將交叉性視為統計均等性問題，特別是在歧視議題上，但新興的文獻探討了另一個面向——單調性。根據領域專業知識，單調性在許多與公平性相關的領域中扮演至關重要的角色，其中違反可能會誤導人類決策並導致災難性的後果。在本文中，我們首先系統性地評估套用單調神經加法模型（MNAM）的重要性，該模型使用公平感知 ML 演算法來強制執行個別和成對單調性原則，以確保 AI 倫理和社會的公平性。我們透過理論推理、模擬和廣泛的實證分析的混合方法發現，在所有公平性領域（包括犯罪學、教育、醫療保健和金融）中，考慮單調性公理至關重要。我們的研究有助於 AI 倫理、可解釋 AI（XAI）和人機互動（HCI）介面中的跨學科研究。透過證明不符合單調性的災難性後果，我們探討了單調性要求在 AI 應用中的重要性。此外，我們證明 MNAM 是一種有效的公平感知 ML 方法，透過施加單調性限制來整合人類智慧。</paragraph>
...
摘要：<paragraph>在安全關鍵應用程式中，例如法律或醫療領域，解釋 AI 模型的預測至關重要。對於預測的一種解釋形式是萃取依據，亦即引導模型對該實例提出其預測的實例特徵子集。例如，子詞組「他偷了手機」可以是「偷竊」預測的萃取依據。先前關於產生萃取依據的研究通常採用兩階段模型：選擇器，用於選擇最重要的特徵（即依據），然後是預測器，它根據所選特徵獨家進行預測。這些研究的一個缺點是，學習選擇特徵的主要信號來自於將預測器給出的答案與真實答案進行比較。在這項研究中，我們建議透過資訊校準方法從預測器中擷取更多資訊。更精確地說，我們聯合訓練兩個模型：一個是典型的類神經網路模型，它以準確但黑盒的方式解決手邊的任務，另一個是選擇器預測器模型，它還為其預測產生依據。第一個模型用作第二個模型的指南。我們使用對抗技術校準兩個模型萃取的資訊，使得它們之間的差異成為遺漏或過度選擇的特徵的指標。此外，對於自然語言任務，我們提出一個基於語言模型的正則化器，以鼓勵萃取流利的依據。在情緒分析任務、仇恨言論辨識任務以及法律領域的三個任務上的實驗結果顯示了我們在依據萃取方面的做法的有效性。</paragraph>
...
摘要：最近在醫療保健中獲得認證的人工智慧 (AI) 工具激增，已重新引發有關採用此技術的爭論。此類爭論的一條主軸涉及可解釋 AI (XAI) 及其承諾讓 AI 裝置更透明且值得信賴。在醫療 AI 領域中活躍的少數聲音已表達了對可解釋 AI 技術的可靠性，尤其是特徵歸因方法的疑慮，質疑其在準則和標準中的使用和納入。儘管有合理的疑慮，我們主張對事後局部可解釋性方法的可行性提出批評，將問題概括為圖像資料特有的問題，等於連同洗澡水把嬰兒也一起倒掉。我們從將問題描述為解釋和人類理解之間缺乏語意匹配開始。為了了解何時可以可靠地使用特徵重要性，我們區分了低階和高階特徵的特徵重要性。我們主張對於低階特徵附帶明確語意的資料類型，例如電子健康紀錄 (EHR) 等表格資料，可以取得語意匹配，因此特徵歸因方法仍可被用於有意義且有用的方式。最後，我們勾勒出一個程序來測試是否已達成語意匹配。
...
摘要：儘管 AI 的機器學習 (ML) 模型在醫療領域取得高性能，但它們並非沒有錯誤。讓臨床醫生能夠辨識出錯誤的模型建議對於建立對醫療 AI 的信任至關重要。可解釋 AI (XAI) 旨在透過釐清 AI 推理以支援最終使用者來滿足此需求。最近，幾項關於生物醫學影像的研究取得了令人滿意的結果。儘管如此，對於使用表格資料的模型，其解決方案仍不足以滿足臨床醫生的需求。本文提出了一種方法，以支援臨床醫生辨識出使用表格資料訓練的 ML 模型的失敗。我們的方法建立在三個主要支柱上：透過利用臨床背景潛在空間分解特徵集、評估整體解釋的臨床關聯性，以及基於潛在空間相似度 (LSS) 的局部解釋。我們在 ML 基於感染所導致的早產兒發病率識別上展示了我們的做法。由於模型故障而導致的死亡風險、終身殘疾和抗生素抗藥性是此領域中一個開放的研究問題。我們的方法成功辨識出兩個模型的錯誤分類案例。透過將局部解釋脈絡化，我們的解決方案為臨床醫生提供了可行的見解，以支援他們自主做出明智的最終決策。
...
摘要：<paragraph>我們正見證著 AI 經濟與社會的興起，其中 AI 技術正日益影響著醫療保健、商業、交通和日常生活中的許多方面。已經報導了許多 AI 系統甚至超越人類專家準確性的成功案例。然而，AI 系統可能會產生錯誤、可能表現出偏見、可能對數據中的雜訊很敏感，並且經常缺乏技術和司法透明度，導致信任度降低以及在採用方面的挑戰。這些最近的缺點和擔憂已在科學和一般新聞媒體中得到記錄，例如自動駕駛汽車事故、醫療保健中的偏見、有色人種的招聘和面部識別系統、看似正確的醫療決策後來發現是出於錯誤的原因等。這導致許多政府和監管倡議的出現，要求可信賴且合乎道德的 AI 提供準確性和穩健性、某種形式的可解釋性、人類控制和監督、消除偏見、司法透明度和安全性。提供可信賴的 AI 系統的挑戰激勵了對可解釋 AI 系統 (XAI) 的深入研究。XAI 的目的是提供人類可以理解的資訊，說明 AI 系統如何做出決策。在本文中，我們首先簡要總結當前的 XAI 工作，然後挑戰準確性與可解釋性互斥且僅專注於深度學習的最新論點。然後，我們提出我們對在高風險可信賴 AI 系統交付的完整生命週期中使用 XAI 的建議，例如開發、驗證和認證，以及可信賴的生產和維護。</paragraph>
...
摘要：深度神經網路 (DNN) 可偵測資料中的模式，並已在許多電腦視覺應用中展現出多功能性和強大的效能。然而，DNN 本身容易犯下違反簡單常識概念的明顯錯誤，且在使用明確知識來引導其搜尋和決策制定方面能力有限。雖然整體 DNN 效能指標可能良好，但這些明顯的錯誤加上缺乏可解釋性，已阻礙了其在醫療影像分析等關鍵任務中的廣泛採用。本文的目的是介紹 SimpleMind，一個專注於醫療影像理解的認知 AI 開源軟體架構。它允許建立一個知識庫，以直觀的人類可讀形式描述影像物件的預期特性和關係。SimpleMind 架構透過以下方式為 DNN 帶來思考能力：(1) 提供方法，根據知識庫推論影像內容，例如空間推論和條件推論，以檢查 DNN 輸出；(2) 以通用軟體代理的形式應用程序知識，將這些代理鏈結在一起以完成影像前處理、DNN 預測和結果後處理；以及 (3) 執行所有知識庫參數的自動共同最佳化，以適應代理到特定問題。SimpleMind 能對多個偵測物件進行推理以確保一致性，並在 DNN 輸出之間提供交叉檢查。這種機器推理透過可解釋的模型和可解釋的決策改善了 DNN 的可靠性和可信度。提供的範例應用程式展示了 SimpleMind 如何透過將深度神經網路嵌入認知 AI 架構中來支援和改善它們。
...
摘要：在過去十年中，基於深度學習方法的視覺感知任務進展迅速，在從自動駕駛到臨床決策支援系統的眾多應用領域中得到廣泛採用。儘管這些基於深度學習的模型表現令人印象深刻，但它們的決策過程仍然相當不透明，這使得它們在人類關鍵任務中的部署成為一項有風險的嘗試。這反過來又使得理解這些模型做出的決策對於它們的可靠部署至關重要。可解釋 AI (XAI) 方法試圖通過為這種黑盒深度學習方法提供解釋來解決這個問題。在本文中，我們對計算機視覺中的基於歸因的 XAI 方法進行了全面的調查，並回顧了現有的基於梯度的、基於擾動的和基於對比的 XAI 方法的文獻，並提供了關於開發和評估穩健 XAI 方法的關鍵挑戰的見解。
...
摘要：自 10 年代中期以来，深度学习 (DL) 时代持续至今，每年都带来新的最高级和创新。然而，这些创新转化为实际应用的速度落后于这一快速步伐。特别是安全关键型应用，其基础是严格的法规和道德要求，这些要求需要得到满足，并且仍然是争论的活跃领域。可解释 AI (XAI) 和隐私保护机器学习 (PPML) 都是至关重要的研究领域，旨在减轻 DL 中流行的数据饥渴黑盒模型的一些缺点。尽管在各个领域都有活跃的研究活动，但尚未关注它们的相互作用。这项工作首次调查了隐私学习技术对基于 DL 的模型的生成解释的影响。在涵盖多个领域的各种图像和时间序列数据集以及不同的隐私技术、XAI 方法和模型架构的广泛实验分析中，研究了隐私训练对生成解释的影响。研究结果表明，通过引入隐私，解释发生了不可忽视的变化。除了报告 PPML 对 XAI 的个别影响外，本文还为实际应用中技术的选用提供了明确的建议。通过揭示这些关键技术的相互依赖性，这项工作是克服安全关键领域中实际可应用 AI 的剩余障碍的第一步。
...
摘要：肺結核 (TB) 是一種傳染性細菌疾病，是造成死亡的重要原因，特別是在低收入國家，$2020$ 年全球報告約一千萬例新病例。雖然肺結核是可以治療的，但未遵循藥物治療方案是造成發病率和死亡率的重要原因。因此，主動找出有中斷藥物治療風險的患者，可以採取矯正措施來減輕不良後果。使用極端不遵從的代理測量和來自印度四個邦的近 $700,000$ 名患者的數據集，我們制定並解決了基於自訂排名度量的機器學習 (ML) 問題，以早期預測不遵從。我們訓練 ML 模型並針對基準進行評估，與基於規則的基準相比，提升了約 $\sim 100\%$，與隨機分類器相比，提升了約 $\sim 214\%$，並考慮了全國範圍內的大規模未來部署。我們在過程中處理了各種問題，包括數據品質、高基數分類資料、低目標流行率、分佈轉移、群組間的差異、演算法公平性，以及對健壯性和可解釋性的需求。我們的發現表明，不遵從患者的風險分層是一種可行的、可大規模部署的 ML 解決方案。作為印度中央結核病部門的官方 AI 合作夥伴，我們正在與多個城市和州級試點合作，目標是全印度部署。
...
摘要：可解釋人工智慧 (XAI) 透過提升終端使用者對機器信任，轉變人工智慧 (AI) 領域。隨著連網裝置數量持續增加，物聯網 (IoT) 市場需要獲得終端使用者的信賴。然而，現有文獻仍缺乏對於 XAI 在 IoT 中使用的系統性且全面的調查工作。為了彌補這個不足，我們在本文中探討 XAI 架構，重點在於它們的特性和對 IoT 的支援。我們說明廣泛使用的 XAI 服務，以用於 IoT 應用程式，例如安全強化、醫療物聯網 (IoMT)、工業物聯網 (IIoT) 和城市物聯網 (IoCT)。我們也建議在這些應用程式中，透過適當的範例在 IoT 系統上實作 XAI 模型，並總結出對未來工作的關鍵推論。此外，我們提出邊緣 XAI 架構的尖端發展，以及第六代 (6G) 通訊服務對 IoT 應用程式的支援，並附上關鍵推論。簡而言之，本文構成第一個針對 XAI 基礎架構發展的全盤彙編，這些架構專門針對未來 IoT 使用案例的需求而量身打造。
...
摘要：美國每年約有 125 萬人接受燒傷治療。精確的燒傷分類是醫療 AI 領域中的一個重要面向。在這項研究中，我們提出一個可解釋的人機協作架構，用於改善燒傷超音波分類模型。我們的架構利用基於 LIME 分類說明器的說明系統，來驗證並整合燒傷專家的知識，建議新的功能並確保模型的有效性。使用此架構，我們發現 B 模式超音波分類器可以透過提供紋理特徵來增強。更具體地說，我們確認基於超音波影像的灰階共生矩陣 (GLCM) 的紋理特徵可以提高轉移學習燒傷深度分類器的準確度。我們在豬隻受試者的真實資料上測試我們的假設。我們展示了在根據我們的架構修改後，燒傷深度分類的準確度從約 88% 提升至約 94%。
...
摘要：在醫療保健領域，AI 系統在診斷、治療和監控方面為臨床醫生和患者提供支援，但許多系統的解釋性不佳，仍然對實際應用構成挑戰。克服這一障礙是可解釋 AI (XAI) 的目標。然而，解釋可能會被不同地感知，因此無法為所有人解決黑箱問題。以人為中心的 AI 領域通過將 AI 適應使用者來解決這個問題。我們提出了一個以使用者為中心的角色概念來評估 XAI，並使用它來調查行動健康壓力監控應用程式中各種解釋樣式和內容的最終使用者偏好。我們的線上調查結果顯示，使用者的年齡層和個性，以及解釋類型，都會影響解釋偏好，這表示這些是 XAI 設計的基本特徵。我們將結果歸納為三種類型的使用者角色：重視權力、休閒和隱私的使用者。我們的見解讓互動式、以人為中心的 XAI 更接近實際應用。
...
摘要：青光眼是所有視覺缺陷中僅次於白內障的第二大失明原因，其主要發生原因是焦慮或憂鬱症導致眼壓過高，進而損害視神經並造成視力問題。傳統的青光眼篩檢程序耗時且需要醫療專業人員持續關注，但即使如此，由於時間限制和壓力，他們仍時常無法正確分類，導致治療錯誤。儘管已做出許多努力來自動化整個青光眼分類程序，但這些現有模型通常具有黑盒特性，使用戶無法理解預測背後的主要原因，因此醫療從業人員通常無法依賴這些系統。在本文中，我們在比較了各種預訓練模型後，提出了一個能夠以 94.71% 的準確率分類青光眼的遷移學習模型。此外，我們利用了局部可解釋模型不可知解釋 (LIME)，為我們的系統引入了可解釋性。這項改進讓醫療專業人員能夠獲得重要且全面的資訊，協助他們做出判斷。它也降低了傳統深度學習模型的不透明性和脆弱性。
...
摘要：深度學習在偵測物件和分類影像方面展現出絕佳的效能，確保了分析醫學影像的極大前景。
將深度學習的成功轉譯至醫學影像，其中醫生需要了解潛在的過程，需要具備詮釋和說明神經網路預測的能力。深度神經網路的可解釋性通常依賴於估計輸入特徵（例如像素）對於結果（例如類別機率）的重要性。然而，已經開發出許多重要性估計器（也稱為顯著性圖），但尚不清楚哪些對於醫學影像應用更為相關。在目前的研究中，我們探討了幾個重要性估計器在說明卷積深度網路對電腦斷層掃描 (CT) 影像分類的效能，使用了三個不同的評估指標。首先，以模型為中心的保真度衡量在特定輸入受到擾動時模型準確度的下降。其次，在像素層級上，透過受試者操作特性 (ROC) 曲線衡量重要性分數與專家定義的分割遮罩之間的一致性。第三，我們透過 Dice 相似性係數 (DSC) 衡量基於 XRAI 的圖與分割遮罩之間的區域重疊。整體而言，兩個版本的 SmoothGrad 在保真度和 ROC 排名中名列前茅，而積分梯度和 SmoothGrad 則在 DSC 評估中表現出色。有趣的是，以模型為中心（保真度）和以人為中心（ROC 和 DSC）的評估之間存在著重大的差異。分割遮罩中嵌入的專家期望和直覺不一定與模型如何得出其預測相符。了解可解釋性中的這種差異將有助於利用深度學習在醫學中的力量。
...
摘要：深度學習方法，特別是卷積神經網路，已成為醫學影像運算任務中強大的工具。雖然這些複雜模型提供了極佳的效能，但其黑箱性質可能會阻礙在高風險決策中採用於現實世界。在本文中，我們提出一個互動式系統，利用最先進的可解釋性技術來協助放射科醫師進行乳癌篩檢。我們的系統將深度學習模型整合到放射科醫師的工作流程中，並提供新穎的互動方式，以促進對模型決策過程的理解。此外，我們證明我們的系統可以逐步利用使用者互動來提供更精細的可解釋性報告，而標記開銷很低。由於所採用的可解釋性技術具有通用性，因此我們的系統與領域無關，可用於許多不同的醫學影像運算任務，這為我們如何利用視覺分析來轉換原本靜態的可解釋性技術以擴充人類決策制定並促進採用醫療 AI 提供了新的觀點。
...
摘要：如今，人工智能 (AI) 已成為醫療保健應用程式中不可或缺的組成部分，無論是臨床或遠距醫療，但效能最好的 AI 系統通常過於複雜，無法自行解釋。可解釋 AI (XAI) 技術被定義為揭示系統預測和決策背後的推理，在處理敏感且個人的健康資料時，它們變得更加重要。值得注意的是，XAI 並未在不同的研究領域和資料類型中獲得相同的關注，特別是在醫療保健方面。特別是，許多臨床和遠距醫療應用程式分別基於表格和時間序列資料，而 XAI 並不常在這些資料類型上進行分析，而電腦視覺和自然語言處理 (NLP) 則為參考應用程式。為了提供最適合醫療保健領域中表格和時間序列資料的 XAI 方法概述，本文回顧了過去 5 年的文獻，說明產生的解釋類型以及評估其相關性和品質所付出的努力。具體來說，我們將臨床驗證、一致性評估、客觀且標準化的品質評估以及以人為中心的品質評估確定為確保對最終使用者進行有效解釋的主要特徵。最後，我們強調了該領域的主要研究挑戰以及現有 XAI 方法的限制。
...
摘要：<paragraph>目標：本研究評估了一種市售的可解釋 AI 演算法在擴增臨床醫師識別胸部 X 光片 (CXR) 上肺癌的能力方面的影響。
設計：這項回溯性研究評估了 11 位臨床醫師在有和沒有市售 AI 演算法（紅點，Behold.ai）協助下從胸部 X 光片偵測肺癌的表現，該演算法預測 CXR 中疑似肺癌。臨床醫師表現與臨床確診相較評估。
環境：這項研究分析了 NHS 醫院的匿名患者資料；該資料集包含 400 張 2020 年接受 CXR 的成年患者（18 歲以上）胸部 X 光片，以及對應的臨床文字報告。
參與者：由 11 位臨床醫師（顧問放射科醫師、放射科醫師受訓者和報告放射技師）組成的讀者小組參與這項研究。
主要結果指標：臨床醫師在有和沒有 AI 輸入的情況下，偵測 CXR 上肺癌的整體準確度、靈敏度、特異性和精準度。臨床醫師之間的同意率和表現標準差也在有和沒有 AI 輸入的情況下進行評估。
結果：臨床醫師使用 AI 演算法可改善肺部腫瘤偵測的整體表現，整體增加 17.4% 的肺癌在 CXR 上被識別，否則將會遺漏，整體增加較小腫瘤的偵測，分別增加 24% 和 13% 的第 1 期和第 2 期肺癌偵測，以及標準化臨床醫師表現。
結論：本研究顯示 AI 演算法在改善早期肺癌診斷和透過整體改善讀者表現促進健康公平方面具有很大的臨床效用，且不會影響下游影像資源。</paragraph>
...
摘要：在 AI 驅動的醫療保健領域中，缺乏足夠龐大的開放式醫療資料庫是最大的挑戰之一。使用生成對抗網路 (GAN) 所建立的合成資料，似乎是緩解隱私政策問題的良好解決方案。另一種治療方式是在不交換本地資料樣本的情況下，跨多個醫療機構建立分散式協定。在本文中，我們探討了集中式和分散式設定中的無條件和條件式 GAN。集中式設定模擬了針對龐大但高度不平衡的皮膚病變資料集所進行的研究，而分散式設定則模擬了具有三個機構的更實際的醫院情境。我們評估了模型在保真度、多樣性、訓練速度和在生成的合成資料上訓練的分類器的預測能力方面的效能。此外，我們透過探索潛在空間和嵌入投影來提供可解釋性，重點放在全局和局部解釋上。計算實像與其在潛在空間中的投影之間的距離，證明了訓練過的 GAN 的真實性和概括性，這是此類應用程式的主要問題之一。已公開在 \url{https://github.com/aidotse/stylegan2-ada-pytorch} 提供執行研究的開放原始碼。
...
摘要：在醫院的世界中，有許多複雜的組合問題，解決這些問題對於提高患者滿意度和所提供的照護品質非常重要。醫療保健中的問題很複雜，因為解決它們需要考慮到許多限制和不同類型的資源。此外，必須在短時間內評估解決方案，以確保在實際情況中可以使用。我們計畫針對這類問題提出解決方案，既擴展已測試的解決方案，又根據文獻並在有真實資料時使用真實資料，為新問題建模解決方案。解決這類問題很重要，但由於歐盟委員會制定了《一般資料保護規範》，規定每個人都有權要求解釋 AI 做出的決定，因此在未開發出可解釋性方法的情況下，使用基於 AI 的求解器（例如基於 Answer Set 程式設計的求解器）將受到限制。因此，研究的另一部分將致力於研究和提出新的方法來解釋所獲得的解決方案。
...
摘要：在現代動態不斷發展的社會中，越來越多人飽受慢性病和嚴重疾病所苦，醫生和病患需要特殊且精密的醫療保健支持。因此，傑出的醫療保健利害關係人已認知到開發此類服務的重要性，以讓病患的生活更輕鬆。此類支持需要蒐集大量病患複雜的資料，例如臨床、環境、營養、日常活動、各種來自智慧穿戴式裝置的資料、來自配備感測器的服飾的資料等。整體病患資料必須適當地彙總、處理、分析，並提供給醫生和照護人員，以建議適當的治療和行動，以改善病患的健康相關參數和整體健康狀況。進階的人工智慧技術提供分析此類大數據、使用它們，並衍生新知識以支援個人化醫療決策的機會。新方法（例如基於進階機器學習、聯邦學習、遷移學習、可解釋人工智慧的方法）為未來更優質地使用健康和醫療資料開啟新途徑。在本文中，我們將提出一些在個人化醫療決策中應用一系列人工智慧方法領域的關鍵面向和特徵範例。
...
摘要：<paragraph>持續監控已訓練的 ML 模型，以確定何時應信任或不信任其預測，對於其安全部署至關重要。這樣的架構應具有高性能、可解釋性、事後且可操作性。我們提出 TRUST-LAPSE，一個用於持續模型監控的「不信任」評分架構。我們使用一連串的潛在空間嵌入來評估每個輸入範例的模型預測的可信度。具體來說，(a) 我們的潛在空間不信任分數使用潛在空間中的距離量度（馬氏距離）和相似度量度（餘弦相似度）來估計不信任，(b) 我們的順序不信任分數則在非參數、基於滑動視窗的演算法中，確定過去輸入表示的順序中相關性的偏差，以進行可操作的持續監控。我們透過兩個下游任務來評估 TRUST-LAPSE：(1) 分布式偏移輸入偵測，以及 (2) 資料漂移偵測。我們使用公開的資料集在不同的領域（音訊和視覺）中進行評估，並進一步在具有挑戰性的、真實世界的腦電圖 (EEG) 資料集上，針對癲癇偵測基準測試我們的做法。我們的潛在空間不信任分數達到了最先進的結果，在 AUROC 上分別為 84.1（視覺）、73.9（音訊）和 77.1（臨床 EEG），優於基準線 10 個百分點以上。我們揭露了流行基準線中的嚴重失敗，這些基準線對輸入語義內容不敏感，使其不適合於真實世界的模型監控。我們表明，我們的順序不信任分數達到了很高的漂移偵測率；在所有領域中，超過 90% 的串流顯示錯誤小於 20%。透過廣泛的定性和定量評估，我們表明我們的信任分數更強健，並提供可解釋性，以便於實務採用。</paragraph>
...
摘要：機器學習的普及提高了不公平模型在高風險應用中部署的風險，例如司法系統、藥物/疫苗設計和醫療診斷。儘管有從頭訓練公平模型的有效方法，但如何自動揭示和解釋訓練模型的不公平性仍然是一項具有挑戰性的任務。以可解釋的方式揭示機器學習模型的不公平性是邁向公平且值得信賴的人工智慧的關鍵一步。在本文中，我們系統性地應對了通過挖掘可解釋證據 (RUMIE) 來揭示不公平模型的新任務。關鍵思想是以一組數據實例的形式找到確鑿的證據，這些實例受到模型的最大歧視。為了使證據具有可解釋性，我們還找到了一組人類可理解的關鍵屬性和決策規則，這些屬性和規則描述了受到歧視的數據實例，並將其與其他未受到歧視的數據區分開來。正如在許多真實世界數據集上進行的廣泛實驗所證明的，我們的模型找到了高度可解釋且確鑿的證據，可以有效地揭示訓練模型的不公平性。此外，它比所有基線方法的可擴展性都要高得多。
...
摘要：可解釋 AI (XAI) 在安全關鍵系統中是必要的，例如在臨床診斷中，因為致命決策的風險很高。然而，目前 XAI 就像是一組鬆散的方法，而不是一個定義良好的流程。在這項工作中，我們詳細說明了 XAI 中最大的子群體，可解釋機器學習 (IML) 與古典統計之間的概念相似性。基於這些相似性，我們提出了沿著統計過程的 IML 形式化。採用這種統計觀點使我們能夠將機器學習模型和 IML 方法解釋為精密的統計工具。基於這種解釋，我們推論出三個關鍵問題，我們將它們確定為在安全關鍵設置中 IML 成功和採用至關重要的問題。通過制定這些問題，我們進一步旨在引發關於 IML 與古典統計之間的區別以及我們的觀點對該領域未來的影響的討論。
...
摘要：人工智慧已普及至各個領域與學科，生物醫學影像與訊號處理也不例外。對此主題日益增長且廣泛的興趣，引發了大量的研究活動，並反映在指數級的研究工作中。透過研究大量且多樣化的生物醫學資料，機器與深度學習模型已革新了各種任務，例如建模、分割、配準、分類與合成，其表現優於傳統技術。然而，將結果轉化為生物學/臨床可解釋資訊的困難，阻礙了其在該領域的充分利用。可解釋人工智慧 (XAI) 嘗試透過提供讓模型可解釋並提供說明的方法，來填補這種轉譯差距。迄今已提出不同的解決方案，並獲得社群日益增長的興趣。本文旨在提供生物醫學資料處理中 XAI 的概述，並指出 2022 年 3 月將出現在 IEEE 訊號處理雜誌中的生物醫學影像與訊號處理深度學習特刊。
...
摘要：<paragraph>在使用醫療影像進行診斷時，無論是臨床醫生或人工智慧 (AI) 系統，影像品質都很重要。當影像品質不佳時，產生影像的醫療檢查通常需要重新進行。遠距醫療中常見的問題是，只有在患者離開診所後才會標記品質問題，這表示他們必須回來重新進行檢查。對於居住在偏遠地區的人來說，這可能特別困難，而這些人佔巴西一家數位醫療保健組織 Portal Telemedicina 的病患很大一部分。在本文中，我們報告關於 (i) 開發一種 AI 系統，用於標記和說明低品質醫療影像的即時工作，(ii) 一項訪談研究，以了解在 OurCompany 使用 AI 系統的利害關係人對說明的需求，以及 (iii) 一項縱向使用者研究設計，用於檢查在我們的診所中，將說明納入技術人員工作流程的效果。據我們所知，這將是第一個評估 XAI 方法對最終使用者（使用 AI 系統但沒有特定 AI 專業知識的利害關係人）影響的縱向研究。我們歡迎對我們的實驗設置提供回饋和建議。</paragraph>
